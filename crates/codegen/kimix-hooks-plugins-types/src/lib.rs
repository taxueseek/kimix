...
//!
//! This crate defines the wire format for `Kimix/hooks/*` and `Kimix/plugins/*`
//! ACP extension methods. It is dependency-free (only `serde`) so both
//! `Kimix-shell` and `Kimix-tui` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `Kimix-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `Kimix-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Kimix's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `Kimix-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.kimix/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}

/// Response for `Kimix/hooks/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `Kimix/plugins/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `Kimix/mcp/list` as consumed by the pager.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `Kimix/hooks/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `Kimix/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Shared action response for both `Kimix/hooks/action` and `Kimix/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.kimix/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.kimix/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.kimix/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserKimix),
            conflict: None,
        };
...
// ---------------------------------------------------------------------------
// Marketplace types (wire format for Kimix/marketplace/* ACP endpoints)
// ---------------------------------------------------------------------------

/// Response for `Kimix/marketplace/list`.
...
/// Request wrapper for `Kimix/marketplace/action`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
