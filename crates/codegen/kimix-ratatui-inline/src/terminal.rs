...
impl<'a> From<OurFrame<'a>> for Frame<'a> {
    fn from(value: OurFrame<'a>) -> Self {
        // SAFETY: OurFrame is a repr(transparent) wrapper over Frame with
        // identical layout. The size assertion at compile time guarantees
        // the transmute is safe — if the layouts ever diverge, the assert
        // catches it before any UB occurs.
        assert_eq!(
...
impl<'a> From<Frame<'a>> for OurFrame<'a> {
    fn from(value: Frame<'a>) -> Self {
        // SAFETY: Inverse of the OurFrame → Frame transmute above.
        // Same layout guarantee via size assertion.
        assert_eq!(
            std::mem::size_of::<Frame>(),
            std::mem::size_of::<OurFrame>()
        );
...
fn diff_large<'a>(prev: &Buffer, next: &'a Buffer) -> Vec<(u16, u16, &'a Cell)> {
    ...
    for (i, (current, previous)) in next_buffer.iter().zip(previous_buffer.iter()).enumerate() {
        if current.diff_option != ratatui::buffer::CellDiffOption::Skip
