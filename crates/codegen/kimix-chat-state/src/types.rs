...
/// emitter in `Kimix-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (Kimix login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `kimix_sampling_types::SamplingConfig` (which is secret-free).
