...
impl ChatStateActor {
    ...
    pub(super) fn truncate_to_prompt_index(&mut self, target_prompt_index: usize) {
        ...
        for (i, item) in self.state.conversation.iter().enumerate() {
            if matches!(item, kimix_sampling_types::ConversationItem::User(_)) {
    ...
    pub(super) fn check_auto_compact_needed(
        ...
        if kimix_token_estimation::exceeds_threshold(self.state.total_tokens, cw, threshold_percent)
        {
            let utilization_percent =
                kimix_token_estimation::usage_percentage_truncated_u8(self.state.total_tokens, cw);
    ...
    pub(super) fn get_last_model_metadata(&self) -> crate::commands::ModelMetadata {
        self.state
            ...
            .rev()
            .find_map(|item| {
                if let kimix_sampling_types::ConversationItem::Assistant(a) = item {
    ...
    pub(super) fn has_dangling_tool_calls(&self) -> bool {
        kimix_sampling_types::has_dangling_tool_calls(&self.state.conversation)
    }
    ...
    pub(super) fn get_first_user_text(&self) -> Option<String> {
        self.state.conversation.iter().find_map(|item| {
            if let kimix_sampling_types::ConversationItem::User(u) = item {
                ...
                u.content.first().and_then(|part| {
                    if let kimix_sampling_types::ContentPart::Text { text } = part {
    ...
    pub(super) fn get_conversation_item_at(
        &self,
        index: usize,
    ) -> Option<kimix_sampling_types::ConversationItem> {
    ...
    pub(super) fn get_conversation_counts(&self) -> ConversationCounts {
        ...
        for item in &self.state.conversation {
            match item {
                kimix_sampling_types::ConversationItem::User(_) => counts.user += 1,
                kimix_sampling_types::ConversationItem::Assistant(_) => {
                    counts.assistant += 1;
                }
                kimix_sampling_types::ConversationItem::ToolResult(_) => {
                    counts.tool_result += 1;
                }
                kimix_sampling_types::ConversationItem::System(_) => {}
                kimix_sampling_types::ConversationItem::BackendToolCall(_) => {}
                kimix_sampling_types::ConversationItem::Reasoning(_) => {}
            }
        }
        counts
    }
    ...
    pub(super) fn get_system_message(&self) -> Option<kimix_sampling_types::ConversationItem> {
        self.state
            .conversation
            .iter()
            .find(|item| matches!(item, kimix_sampling_types::ConversationItem::System(_)))
            .cloned()
    }
}
