...
pub fn estimate_system_message_tokens(item: &ConversationItem) -> u64 {
    match item {
        ConversationItem::System(s) => kimix_token_estimation::estimate_tokens(&s.content),
        _ => 0,
    }
}
...
pub fn estimate_tool_definition_tokens(td: &kimix_sampling_types::ToolDefinition) -> u64 {
    ...
    ((name_len + desc_len + params_len) as u64) / kimix_token_estimation::BYTES_PER_TOKEN
}

/// Sum [`estimate_tool_definition_tokens`] across a slice.
pub fn estimate_tool_definitions_tokens(tds: &[kimix_sampling_types::ToolDefinition]) -> u64 {
    tds.iter().map(estimate_tool_definition_tokens).sum()
}

/// Bytes/4 estimate for a single [`ConversationItem`].
///
/// Images are counted at [`kimix_token_estimation::IMAGE_TOKEN_ESTIMATE`] each.
...
pub fn estimate_item_tokens(item: &ConversationItem) -> u64 {
    ...
    match item {
        ...
        ConversationItem::User(u) => {
            ...
            (bytes as u64) / kimix_token_estimation::BYTES_PER_TOKEN
                + kimix_token_estimation::estimate_image_tokens(images)
        }
        ...
        ConversationItem::ToolResult(tr) => kimix_token_estimation::estimate_tokens(&tr.content),
        ConversationItem::BackendToolCall(b) => {
            kimix_token_estimation::estimate_tokens(&b.text_summary())
        }
        ConversationItem::Reasoning(r) => {
            ...
            let text_bytes = kimix_sampling_types::reasoning_item_text(r).len();
            let enc_bytes = r.encrypted_content.as_deref().map(str::len).unwrap_or(0);
            ((text_bytes + enc_bytes) as u64) / kimix_token_estimation::BYTES_PER_TOKEN
        }
    }
}

/// Estimate token footprint: text bytes / 4, images at the per-image
/// constant defined by [`kimix_token_estimation::IMAGE_TOKEN_ESTIMATE`].
pub fn estimate_conversation_tokens(items: &[ConversationItem]) -> u64 {
    items.iter().map(estimate_item_tokens).sum()
}

/// Kimix's [`ItemTokenCounter`](kimix_compaction::ItemTokenCounter)
/// for the shared compaction engine: the bytes/4 estimate Kimix already
...
/// Kimix estimates instead, reusing [`estimate_item_tokens`] so the
/// per-variant arithmetic (images, reasoning blobs, tool-call args) stays in
/// one place.
pub struct EstimatedItemTokenCounter;

impl kimix_compaction::ItemTokenCounter<ConversationItem> for EstimatedItemTokenCounter {
...
mod tests {
    ...
    fn estimated_item_token_counter_matches_estimate_item_tokens() {
        use kimix_compaction::ItemTokenCounter;
    ...
    fn estimate_tool_definition_tokens_counts_name_desc_params() {
        ...
        let td = kimix_sampling_types::ToolDefinition::function(
            "search",
            Some("find a file"),
            serde_json::json!({}),
        );
    ...
    fn estimate_tool_definitions_tokens_sums_across_slice() {
        let a = kimix_sampling_types::ToolDefinition::function(
            "a",
            None::<&str>,
            serde_json::json!({}),
        );
        let b = kimix_sampling_types::ToolDefinition::function(
            "b",
            None::<&str>,
            serde_json::json!({}),
        );
