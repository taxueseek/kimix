...
async fn record_last_turn_usage_round_trip() {
    use kimix_sampling_types::TokenUsage;
...
async fn update_sampling_config_is_queryable() {
    ...
    assert_eq!(config.model, "kimix-3");
...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_kimix_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_kimix_req_id, Some("req-1".to_string()));
}
...
async fn build_request_with_tool_definitions() {
    use kimix_sampling_types::ToolSpec;
...
async fn build_request_uses_sampling_config() {
    ...
    assert_eq!(request.model, Some("kimix-3".to_string()));
...
/// Simulates the exact sequence that `Kimix-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools = ConversationItem::Assistant(kimix_sampling_types::AssistantItem {
...
async fn dangling_tool_calls_after_crash_are_repaired_on_load() {
    ...
    let crashed_conversation = vec![
        ...
        ConversationItem::Assistant(kimix_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use kimix_sampling_types::{ContentPart, UserItem};
...
async fn get_last_model_metadata_returns_both_fields() {
    let h = TestHarness::with_conversation(vec![
        ...
        ConversationItem::Assistant(kimix_sampling_types::AssistantItem {
            content: "hello".into(),
            tool_calls: vec![],
            model_id: Some("kimix-4.5".into()),
            model_fingerprint: Some("fp_abc123".into()),
            reasoning_effort: None,
        }),
    ]);
    let meta = h.handle.get_last_model_metadata().await;
    assert_eq!(meta.resolved_model_id.as_deref(), Some("kimix-4.5"));
...
async fn sampling_config_survives_compaction_replacement() {
    use kimix_sampling_types::ApiBackend;
    ...
    assert_eq!(pre_meta.resolved_model_id.as_deref(), Some("kimix-4.5"));
    ...
    assert_eq!(post.model, "kimix", "BUG: model changed after compaction");
    assert_eq!(
...
/// If that model slug is e.g. "Kimix" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            kimix_sampling_types::AssistantItem {
...
async fn context_window_downgrade_triggers_auto_compact() {
    ...
    // Initial config: 500k context, Responses backend (matches Kimix-4.5)
    let config = SamplingConfig {
    ...
    assert_eq!(post.model, "kimix-4.5", "model slug must not change");
    assert_eq!(
        post.api_backend,
        ApiBackend::Responses,
...
fn serialize_via_public_api(req: &kimix_sampling_types::ConversationRequest) -> serde_json::Value {
    ...
    kimix_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &kimix_sampling_types::ConversationRequest,
    extended: &kimix_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(kimix_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(kimix_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use kimix_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
...
// ============================================================================
// Out-of-band history repair (Kimix/session/repair)
// ============================================================================
