...
impl ChatStateActor {
    ...
    /// Out-of-band history repair (`Kimix/session/repair`): run
    ...
    ///
    /// # Design Note — ContextMemory Separation
    ///
    /// This function modifies the storage layer (`self.state.conversation`) directly.
    /// A future optimization would move this logic to the projection layer
    /// (`request_builder.rs`) so the storage layer remains append-only, improving
    /// prefix stability and cache hit rates. This is the "ContextMemory + Projector"
    /// pattern — storage layer is append-only, projection layer handles pruning.
    ///
    /// # What this does
    ///
        ...
        self.state
            ...
            .map(|item| match item {
                ...
                ConversationItem::Reasoning(r) => {
                    kimix_sampling_types::reasoning_item_text(r).len()
    ...
    pub(super) fn record_last_turn_usage(&mut self, usage: kimix_sampling_types::TokenUsage) {
    ...
    pub(super) fn record_model_call_usage(
        ...
        usage: &kimix_sampling_types::TokenUsage,
