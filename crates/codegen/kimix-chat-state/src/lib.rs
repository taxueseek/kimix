//! Kimix-chat-state — Actor-based chat state management for xAI agents.
//!

//! This crate extracts conversation state management from `Kimix-shell`'s
//! `acp_session.rs` into a standalone actor. It follows the same actor pattern
//! as `Kimix-hunk-tracker`:
//!
