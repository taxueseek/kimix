...
//!
//! Extracted from `Kimix-shell/src/agent/subagent/` `resolve_effective_overrides()`.
use std::collections::HashMap;
use std::path::Path;

use kimix_tool_types::{SubagentCapabilityMode, SubagentIsolationMode};
use kimix_tools::implementations::kimix::task::types::SubagentRuntimeOverrides;
...
mod tests {
    use super::*;
    use kimix_tools::implementations::kimix::task::types::ModelOverrideProvenance;
    ...
    fn explicit_model_overrides_role() {
        let overrides = make_overrides(Some("kimix-light"), None, None, None, None);
        let role = SubagentRole {
            model: Some("kimix-3".into()),
            ..Default::default()
        };
        let result =
            resolve_effective_overrides(&overrides, Some(&role), &empty_personas(), None, None);
        assert_eq!(result.model.as_deref(), Some("kimix-light"));
    }
    ...
    fn role_model_used_when_no_explicit() {
        ...
        let result =
            resolve_effective_overrides(&overrides, Some(&role), &empty_personas(), None, None);
        assert_eq!(result.model.as_deref(), Some("kimix-3"));
    }
    ...
    fn persona_not_found_error_is_non_fatal() {
        ...
        let overrides = make_overrides(Some("kimix-3"), Some("missing"), None, None, None);
        let role = SubagentRole {
            model: Some("kimix-light".into()),
            default_isolation: Some("worktree".into()),
            ..Default::default()
        };
        let result =
    ...
    fn persona_file_error_returns_early_with_defaults() {
        ...
        let overrides = make_overrides(Some("kimix-3"), Some("broken"), None, None, None);
        let dir = tempfile::tempdir().unwrap();
        let mut personas = HashMap::new();
        personas.insert(
            "broken".to_string(),
