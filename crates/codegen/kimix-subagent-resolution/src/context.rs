...
//!
//! Extracted from `Kimix-shell/src/agent/subagent/` `normalize_forked_context()`.
use std::borrow::Cow;
...
///
/// See also: `Kimix-chat-state::compaction_utils::strip_system_tags` which
...
/// `Kimix-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use kimix_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(kimix_sampling_types::synthesized_reasoning_item(text))
    }
