...
//!
//! Extracted from `Kimix-shell/src/agent/subagent/` resume validation block.
...
mod tests {
    ...
    fn matching_type_and_persona_source_has_model() {
        let source = make_source("general-purpose", Some("impl"), Some("kimix-3"));
    ...
    fn source_model_is_not_validated() {
        ...
        let source = make_source("general-purpose", None, Some("kimix-3"));
        let result = validate_resume_identity("general-purpose", None, &source);
        assert!(result.is_ok());
    }
