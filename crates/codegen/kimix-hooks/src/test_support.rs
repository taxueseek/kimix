//! Test-only helpers shared across `Kimix-hooks` unit + integration tests.
//!
...
mod tests {
    ...
    fn restores_previous_value_on_normal_return() {
        let key = "KIMIX_HOOKS_TEST_SUPPORT_RESTORE";
    ...
    fn restores_previous_unset_state_on_normal_return() {
        let key = "KIMIX_HOOKS_TEST_SUPPORT_UNSET_RESTORE";
    ...
    fn restores_after_panic() {
        let key = "KIMIX_HOOKS_TEST_SUPPORT_PANIC_RESTORE";
        // SAFETY: see module-level note.
        unsafe {
            std::env::remove_var(key);
        }
    ...
    fn allows_explicit_unset() {
        let key = "KIMIX_HOOKS_TEST_SUPPORT_EXPLICIT_UNSET";
        // SAFETY: see module-level note.
        unsafe {
