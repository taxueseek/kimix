...
//! `kimix_config::expand_env_vars_in_string`, but with an additional
...
const SENTINEL_PREFIX: &str = "\u{f8ff}__KIMIX_HOOKS_MASK_";
...
/// dispatcher's always-set `KIMIX_HOOK_*` vars) survive the load-time pass.
///
...
mod tests {
    ...
    fn extra_takes_precedence_over_process_env() {
        with_env_var(
            "KIMIX_HOOKS_ENV_EXPAND_TEST_PRECEDENCE",
            Some("from-process"),
            || {
                let mut extra = HashMap::new();
                extra.insert(
                    "KIMIX_HOOKS_ENV_EXPAND_TEST_PRECEDENCE".to_string(),
                    "from-extra".to_string(),
                );
                let out =
                    expand_env_vars_with_extra("${KIMIX_HOOKS_ENV_EXPAND_TEST_PRECEDENCE}", &extra);
    ...
    fn falls_back_to_process_env() {
        with_env_var(
            "KIMIX_HOOKS_ENV_EXPAND_TEST_FALLBACK",
            Some("/from/proc/env"),
            || {
                let extra = HashMap::new();
                let out =
                    expand_env_vars_with_extra("${KIMIX_HOOKS_ENV_EXPAND_TEST_FALLBACK}/x", &extra);
    ...
    fn preserves_unresolved_references() {
        ...
        with_env_var("KIMIX_HOOKS_ENV_EXPAND_NEVER_SET", None, || {
            let extra = HashMap::new();
            let input = "${KIMIX_HOOKS_ENV_EXPAND_NEVER_SET}/x.sh";
    ...
    fn preserves_default_modifier_when_var_unset() {
        let extra = HashMap::new();
        with_env_var("KIMIX_HOOKS_ENV_EXPAND_MODIFIER_UNSET", None, || {
            let input = "${KIMIX_HOOKS_ENV_EXPAND_MODIFIER_UNSET:-/default/path.sh}";
            let out = expand_env_vars_with_extra(input, &extra);
            assert_eq!(out, input);
        });
    }
    ...
    fn preserves_default_modifier_when_var_set() {
        let mut extra = HashMap::new();
        extra.insert(
            "KIMIX_HOOKS_DEFAULT_SET".to_string(),
            "/from/extra".to_string(),
        );
        let input = "${KIMIX_HOOKS_DEFAULT_SET:-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_no_colon_default_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_NCD-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_assignment_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_ASSIGN:=/assigned/path.sh}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_error_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_ERR:?error message}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_alternate_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_ALT:+/used/if/set}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_suffix_strip_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_SUFFIX%.sh}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_prefix_strip_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_PREFIX#prefix/}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_substitution_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_SUB/foo/bar}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn preserves_substring_modifier() {
        let extra = HashMap::new();
        let input = "${KIMIX_HOOKS_SUBSTR:0:5}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn mixed_plain_and_modifier_only_plain_expanded() {
        let mut extra = HashMap::new();
        extra.insert("KIMIX_HOOKS_PLAIN".to_string(), "/usr/local".to_string());
        let input = "${KIMIX_HOOKS_PLAIN}/${KIMIX_HOOKS_DEFER:-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, "/usr/local/${KIMIX_HOOKS_DEFER:-/fallback}");
    }
    ...
    fn empty_extra_value_resolves_to_empty_for_plain_form() {
        let mut extra = HashMap::new();
        extra.insert("KIMIX_HOOKS_EMPTY".to_string(), "".to_string());
        let out = expand_env_vars_with_extra("[${KIMIX_HOOKS_EMPTY}]", &extra);
        assert_eq!(out, "[]");
    }
    ...
    fn empty_extra_value_does_not_trigger_default() {
        let mut extra = HashMap::new();
        extra.insert("KIMIX_HOOKS_EMPTY_MOD".to_string(), "".to_string());
        let input = "${KIMIX_HOOKS_EMPTY_MOD:-/fallback}";
        let out = expand_env_vars_with_extra(input, &extra);
        assert_eq!(out, input);
    }
    ...
    fn extra_values_are_not_recursively_expanded() {
        with_env_var(
            "KIMIX_HOOKS_RECURSION_BAR",
            Some("should-not-appear"),
            || {
                let mut extra = HashMap::new();
                extra.insert(
                    "KIMIX_HOOKS_RECURSION_FOO".to_string(),
                    "$KIMIX_HOOKS_RECURSION_BAR".to_string(),
                );
                let out = expand_env_vars_with_extra("${KIMIX_HOOKS_RECURSION_FOO}", &extra);
                assert_eq!(out, "$KIMIX_HOOKS_RECURSION_BAR");
            },
        );
    }
    ...
    /// `"\u{f8ff}__KIMIX_HOOKS_MASK__\u{f8ff}"`. A user-supplied
    ...
    fn expand_preserves_pre_existing_legacy_fixed_sentinel_in_extra() {
        let legacy_sentinel = "\u{f8ff}__KIMIX_HOOKS_MASK__\u{f8ff}";
        let mut extra = HashMap::new();
