...
pub struct HooksMap {
    ...
    /// Event names present in the JSON but not recognized by Kimix.
...
pub struct RawHandler {
    ...
    /// keys (`KIMIX_HOOK_EVENT`, `KIMIX_HOOK_NAME`, `KIMIX_SESSION_ID`,
    /// `KIMIX_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at load
...
pub struct HookSpec {
    ...
    ///    (`KIMIX_HOOK_EVENT`, `KIMIX_HOOK_NAME`, `KIMIX_SESSION_ID`,
    ///    `KIMIX_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`) are stripped at
    ///    load time and a tracing warning is emitted.
    /// 2. Plugin-injected vars merged in by the plugin adapter
    ///    (`Kimix-agent::plugins::hooks_adapter`). The adapter sets
    ///    `KIMIX_PLUGIN_ROOT`, `CLAUDE_PLUGIN_ROOT`, `KIMIX_PLUGIN_DATA`,
    ...
    ///    (`KIMIX_HOOK_EVENT`, `KIMIX_HOOK_NAME`, `KIMIX_SESSION_ID`,
    ///    `KIMIX_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`). These are
...
mod tests {
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.kimix/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.kimix/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "KIMIX_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "KIMIX_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "KIMIX_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "KIMIX_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "KIMIX_HOOKS_PARSE_TEST_MATCHER_VAR";
    ...
    fn parse_hook_file_matcher_braced_var_is_not_env_expanded() {
        ...
        let key = "KIMIX_HOOKS_PARSE_TEST_MATCHER_BRACED";
    ...
    /// User attempts to set runner-reserved keys (KIMIX_HOOK_*,
    /// KIMIX_SESSION_ID, KIMIX_WORKSPACE_ROOT, CLAUDE_PROJECT_DIR) via
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "KIMIX_HOOK_EVENT": "spoofed",
                                    "KIMIX_HOOK_NAME": "spoofed",
                                    "KIMIX_SESSION_ID": "spoofed",
                                    "KIMIX_WORKSPACE_ROOT": "/etc",
