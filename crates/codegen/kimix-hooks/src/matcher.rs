use kimix_tools::types::{claude_names_for, kimix_names_for};
use regex::Regex;
...
///   **exact** match against each name (after external→Kimix alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the Kimix tool `run_terminal_command`).
///
...
/// plus any Kimix tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to Kimix registry in
/// `Kimix-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for kimix_name in kimix_names_for(term) {
            push(kimix_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_kimix_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // Kimix name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_kimix_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // Kimix equivalent
        assert!(m.is_match("hashline_edit")); // second Kimix alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_read_matches_kimix_tool() {
    ...
    #[test]
    fn regex_against_claude_alias_matches_kimix_tool() {
        // A regex written against an external alias still matches the Kimix tool
