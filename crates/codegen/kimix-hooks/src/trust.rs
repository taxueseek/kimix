...
// (`~/.kimix/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_kimix_home>/trusted-hook-projects`), or `None` when no user Kimix home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(kimix_config::user_kimix_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user kimix home (set $KIMIX_SHARE_DIR or $HOME)".to_string())?;
...
/// Returns the path to `$KIMIX_SHARE_DIR/disabled-hooks`, or `None` when no user Kimix
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(kimix_config::user_kimix_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let kimix_dir = dir.join(".kimix");
        std::fs::create_dir_all(&kimix_dir).unwrap();
        kimix_dir.join("trusted-hook-projects")
    }
