...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("KIMIX_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        // per-hook extra_env (plugin vars), or Kimix's own process env. The
        ...
        #[cfg(not(unix))]
        {
            let inv = kimix_config::shell::shell_command_argv(&command_str);
    ...
    // `kimix_tools::util::detach_command`: Unix uses the same setsid /
    ...
    // plugin) can spoof `KIMIX_HOOK_EVENT`, `KIMIX_HOOK_NAME`, `KIMIX_SESSION_ID`,
    // `KIMIX_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR` -- which are the
    ...
    let mut child = match cmd
        ...
        .env("KIMIX_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("KIMIX_HOOK_NAME", &spec.name)
        .env("KIMIX_SESSION_ID", ctx.session_id)
        .env("KIMIX_WORKSPACE_ROOT", ctx.workspace_root)
        // Compatibility alias for external hooks that read this env name.
        // Same value as `KIMIX_WORKSPACE_ROOT`; native `.Kimix` hooks should use
...
///   hooks),
/// * the Kimix process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_relative_path() {
        let spec = HookSpec {
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/project/.kimix/hooks"),
            extra_env: std::collections::HashMap::new(),
        };
        assert_eq!(
            resolve_command_path(&spec),
            Some(std::path::PathBuf::from(
                "/project/.kimix/hooks/bin/check.sh"
            ))
        );
    }
    ...
    fn find_unresolved_skips_runner_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${KIMIX_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${KIMIX_SESSION_ID}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.Kimix-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".kimix-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.kimix-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.kimix-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.kimix-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
