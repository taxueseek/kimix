...
Sample hooks for Kimix. Copy to `~/.kimix/hooks/` to enable globally, or to `<project>/.kimix/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.kimix/hooks/bin
cp examples/hooks/safe-shell.json ~/.kimix/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.kimix/hooks/bin/
chmod +x ~/.kimix/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.kimix/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.kimix/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.kimix/hooks/bin/
chmod +x ~/.kimix/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.kimix/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.kimix/hooks/bin
cp examples/hooks/session-log.json ~/.kimix/hooks/
cp examples/hooks/bin/session-log.sh ~/.kimix/hooks/bin/
chmod +x ~/.kimix/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.kimix/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.kimix/hooks/bin
cp examples/hooks/tool-logger.json ~/.kimix/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.kimix/hooks/bin/
chmod +x ~/.kimix/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Kimix names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.kimix/hooks/`. The hook stops running on the next session.