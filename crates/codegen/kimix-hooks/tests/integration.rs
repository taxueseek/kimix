//! Integration tests for Kimix-hooks.
//!
...
use kimix_hooks::dispatcher;
use kimix_hooks::event::*;
use kimix_hooks::result::HookDecision;
use kimix_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        kimix_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$KIMIX_HOOK_EVENT" > {f}; echo "NAME=$KIMIX_HOOK_NAME" >> {f}; echo "SESSION=$KIMIX_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`KIMIX_HOOK_EVENT`, `KIMIX_HOOK_NAME`,
/// `KIMIX_SESSION_ID`, `KIMIX_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$KIMIX_HOOK_EVENT" > {f}; echo "NAME=$KIMIX_HOOK_NAME" >> {f}; echo "SESSION=$KIMIX_SESSION_ID" >> {f}; echo "ROOT=$KIMIX_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "KIMIX_HOOK_EVENT": "spoofed_event",
                                "KIMIX_HOOK_NAME": "spoofed_name",
                                "KIMIX_SESSION_ID": "spoofed_session",
                                "KIMIX_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "KIMIX_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed KIMIX_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "KIMIX_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed KIMIX_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "KIMIX_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed KIMIX_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use kimix_hooks::result::HookRunResult;
    let info = match run {
