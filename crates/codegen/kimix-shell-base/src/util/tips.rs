...
//! persisted to `~/.kimix/tip_cursor.json`.
...
fn cursor_path(kimix_home: &Path) -> PathBuf {
    kimix_home.join(CURSOR_FILE)
}

/// Load the cursor from `~/.kimix/tip_cursor.json`. Returns 0 on any error.
fn load_cursor(kimix_home: &Path) -> u64 {
    let text = match std::fs::read_to_string(cursor_path(kimix_home)) {
        Ok(t) => t,
        Err(_) => return 0,
    };
...
/// Save the cursor to `~/.kimix/tip_cursor.json`. Silently ignores write errors.
fn save_cursor(kimix_home: &Path, cursor: u64) {
    if let Ok(text) = serde_json::to_string(&TipState { cursor }) {
        let _ = std::fs::write(cursor_path(kimix_home), text);
    }
}
...
/// cursor in `~/.kimix/tip_cursor.json`, so every session sees the next tip
...
pub fn pick_and_advance(tips: &[String], kimix_home: &Path) -> Option<String> {
    if tips.is_empty() {
        return None;
    }
    let cursor = load_cursor(kimix_home);
    let tip = tips[cursor as usize % tips.len()].clone();
    save_cursor(kimix_home, cursor + 1);
    Some(tip)
}
