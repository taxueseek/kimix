//! Memory-system configuration value types, extracted from Kimix-shell
...
//! `Kimix-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.kimix/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.kimix/memory/` are scanned:
