...
//! do not provide. When `$HOME` (and thus `~/.kimix`) is NFS-mounted on
...
impl JournalMode {
    ...
    /// yet), so callers must create it first. `KIMIX_SQLITE_JOURNAL_MODE`
    /// (`wal`|`truncate`) overrides detection as a field kill-switch.
    pub fn for_db_path(db_path: &Path) -> Self {
        let env = std::env::var("KIMIX_SQLITE_JOURNAL_MODE").ok();
        match mode_from_env(env.as_deref()) {
            EnvOverride::Mode(mode) => {
                ...
                tracing::info!(
                    ...
                    mode = mode.as_str(),
                    source = "env",
                    "sqlite journal mode forced by KIMIX_SQLITE_JOURNAL_MODE"
                );
                return mode;
            }
            EnvOverride::Invalid => {
                ...
                tracing::warn!(
                    ...
                    "invalid KIMIX_SQLITE_JOURNAL_MODE (accepted: wal, truncate); using detection"
                );
            }
            EnvOverride::Unset => {}
        }
    ...
    /// `set_journal_mode` in `Kimix-fast-worktree`).
...
/// Parse result of the `KIMIX_SQLITE_JOURNAL_MODE` kill-switch.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum EnvOverride {
    Unset,
...
/// `KIMIX_SQLITE_JOURNAL_MODE` override parsing (pure for testability).
...
mod tests {
    ...
    fn windows_unc_classifies() {
        assert!(is_windows_unc(r"\\server\share\kimix"));
        assert!(is_windows_unc(r"\\?\UNC\server\share\kimix"));
        assert!(is_windows_unc(r"\\?\unc\server\share"));
        assert!(!is_windows_unc(r"\\?\C:\Users\x"));
        assert!(!is_windows_unc(r"\\.\pipe\kimix"));
        assert!(!is_windows_unc(r"C:\Users\x"));
