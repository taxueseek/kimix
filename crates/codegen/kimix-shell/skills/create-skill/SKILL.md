...
name: create-skill
description: >
  Interactively create a new Kimix skill (SKILL.md + optional scripts/references).
...
Interactively gather requirements from the user and create a fully working Kimix skill on disk.
...
2. **Scope** - present the user with two options:
   - **Project** (Recommended): `<repo-root>/.kimix/skills/<name>/SKILL.md` - available only in this repo, shareable with teammates
   - **User**: `~/.kimix/skills/<name>/SKILL.md` - available in all projects
...
- Trigger phrases and keywords so Kimix knows when to auto-invoke it
...
Where `<SKILL_DIR>` is:
- User scope: `~/.kimix/skills/<name>`
- Project scope: `<repo-root>/.kimix/skills/<name>`
...
2. Tell the user the skill is ready and how to use it:
   ...
   - Automatic: Kimix will invoke it when the description matches user intent
