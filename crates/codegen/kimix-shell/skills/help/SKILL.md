...
name: help
description: >
  Kimix documentation and configuration help. Use when users ask about
  setup, configuration, MCP servers, authentication, skills, slash commands,
  keyboard shortcuts, or any Kimix feature. Also use proactively when you
  detect a user is having trouble with setup or onboarding.
metadata:
  short-description: "Kimix docs — config, MCP, auth, skills, commands"
---

# Kimix Help

Answer the user's question about Kimix setup, configuration, or features.

## Steps

1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `~/.kimix/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `~/.kimix/docs/user-guide/`. The available guides are:
