//! `Kimix/hooks/*` extension handlers.
//!
...
use kimix_hooks::event::{HookEventEnvelope, HookEventName};
use kimix_hooks::matcher::HookMatcher;
use kimix_hooks_plugins_types::{HookEvent, HookHandlerType, HookInfo};
...
pub fn hook_spec_to_info(spec: &kimix_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: kimix_hooks::trust::is_hook_disabled(&spec.name),
    }
}

// Wire types for client-registered hooks (`Kimix/hooks/run`); the gate that uses
...
/// for both the `Kimix/hooks/run` request (gate) and the `Kimix/hooks/event` notification
...
/// Response payload for `Kimix/hooks/run` (client to agent). `Default` (used on
...
/// Parse client hooks from `session/new` `_meta["Kimix/hooks"]`, shaped
...
pub(crate) fn parse_client_hooks(meta: Option<&acp::Meta>) -> ClientHooks {
    ...
    for (event_name, value) in map {
        ...
        let Ok(event) = HookEventName::deserialize(de) else {
            tracing::warn!(event = %event_name, "ignoring unknown kimix/hooks event");
            continue;
        };
        let Some(array) = value.as_array() else {
            tracing::warn!(event = %event_name, "kimix/hooks event value is not an array; skipping");
            continue;
        };
...
/// clear) when the request meta carries `Kimix/hooks`, else `None` so a reconnect that
...
pub(crate) fn reconnect_client_hooks(meta: Option<&acp::Meta>) -> Option<ClientHooks> {
    meta.and_then(|m| m.get("kimix/hooks"))
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let group = WireGroup::deserialize(value)
        .inspect_err(|err| tracing::warn!(%event, %err, "ignoring malformed kimix/hooks group"))
        .ok()?;
    if group.hook_callback_ids.is_empty() {
        tracing::warn!(%event, "ignoring kimix/hooks group with no hookCallbackIds");
        return None;
    }
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern) => match HookMatcher::new(pattern) {
            ...
            Err(err) => {
                tracing::warn!(%event, pattern, %err, "ignoring kimix/hooks group with invalid matcher");
                return None;
            }
        },
    };
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "kimix/hooks/action" => {
            let req: kimix_hooks_plugins_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn parse_client_hooks_canonicalizes_subagent_alias() {
        let meta = serde_json::json!({
            "kimix/hooks": { "SubagentEnd": [{ "hookCallbackIds": ["cb"] }] }
        });
        let hooks = parse_client_hooks(meta.as_object());
    ...
    /// Reconnect refresh applies hooks only when the load meta carries `Kimix/hooks`:
    ...
    fn reconnect_client_hooks_only_when_key_present() {
        ...
        let cleared = reconnect_client_hooks(serde_json::json!({ "kimix/hooks": {} }).as_object());
        assert!(cleared.is_some_and(|h| h.is_empty()));

        let set = reconnect_client_hooks(
            serde_json::json!({
                "kimix/hooks": { "PreToolUse": [{ "hookCallbackIds": ["cb"] }] }
            })
            .as_object(),
        );
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use kimix_hooks::event::{HookEventEnvelope, HookPayload};
