...
impl PromptUsage {
    ...
    pub fn project_from_ledger(
        ledger: Option<&kimix_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&kimix_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &kimix_chat_state::UsageTotals) -> Self {
        ...
        let kimix_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
            reasoning_tokens,
...
impl From<&kimix_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &kimix_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<kimix_hooks_plugins_types::HookInfo>,
    ...
    ///
    /// Emitted by the `Kimix/recap` ext method: on demand via the `/recap`
    ...
    /// outcome. Rides the persisted `_Kimix/session/update` rail (unlike the
    /// fire-and-forget `Kimix/session/prompt_complete` notification), so a
...
/// detailed migration guidance (`kimix logout` / `kimix login`) in the
...
pub struct CompactionSegmentFile {
    pub items: Vec<kimix_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: kimix_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    /// Note: this is a per-artifact ID, not the model API's `x_kimix_req_id`
    ...
    /// or `"detailed"` (10-section structured prompt for Kimix and similar agents).
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<kimix_chat_state::compaction_utils::CompactionAttempt>,
}
...
pub struct RecapRequestFile {
    ...
    /// API's `x_kimix_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
    ...
    pub x_kimix_req_id: String,
    /// Sampling conversation id (`recap-{uuid}`).
    pub x_kimix_conv_id: String,
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_kimix_req_id: "xai-recap-abc".into(),
            x_kimix_conv_id: "recap-abc".into(),
        ...
        assert_eq!(parsed.x_kimix_req_id, "xai-recap-abc");
        assert_eq!(
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use kimix_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.kimix/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.kimix/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.kimix/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.kimix/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.kimix/memory/MEMORY.md".into(),
    ...
    fn make_goal_updated_full() -> SessionUpdate {
        SessionUpdate::GoalUpdated {
            ...
            live_tokens_by_model: vec![("kimix-4".into(), 6_000), ("kimix-3".into(), 4_000)],
    ...
    fn goal_updated_serializes_snake_case_tag() {
        ...
        assert_eq!(json["live_tokens_by_model"][0][0], "kimix-4");
    ...
    fn model_changed_envelope_carries_session_id_at_top_level() {
        ...
        assert_eq!(json["update"]["model_id"], "kimix-4");
    }
