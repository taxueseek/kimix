//! `Kimix/feedback`, `Kimix/feedback/dismiss`, `Kimix/btw`, and `Kimix/review/*`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "kimix/feedback" | "kimix/feedback/dismiss" => {
            tracing::info!("handling user feedback");
            handle_feedback(agent, args).await
        }
        m if m.starts_with("kimix/review") => {
...
/// Handle `Kimix/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set KIMIX_FEEDBACK_ENABLED=true or \
...
///
/// Methods:
/// - `Kimix/review/comment`: record a new inline code comment
/// - `Kimix/review/comment/delete`: record a tombstone event for a deleted comment
async fn handle_review(args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "kimix/review/comment" => {
