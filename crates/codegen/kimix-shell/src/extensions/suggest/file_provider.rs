...
mod tests {
    ...
    async fn suggest_var_prefix_lists_expansion_inserts_raw() {
        ...
        let _env = kimix_test_support::EnvGuard::set("KIMIX_SUGGEST_TEST_DIR", tmp.path());
        let ctx = SuggestContext::new(
            "cat $KIMIX_SUGGEST_TEST_DIR/no".into(),
            "cat $KIMIX_SUGGEST_TEST_DIR/no".len(),
            "/ignored".into(),
        );
        let results = FilePathProvider.suggest(&ctx).await;
        assert_eq!(results.len(), 1);
        assert_eq!(
            results[0].token_text.as_deref(),
            Some("$KIMIX_SUGGEST_TEST_DIR/notes.md")
        );
    }
