...
///
/// Priority order: local Kimix bash history > shell history > cross-CWD history.
...
fn scan_cross_cwd_prompts() -> Vec<String> {
    let sessions_dir = crate::util::kimix_home::kimix_home().join("sessions");
    ...
    for (dir, _) in dirs.iter().take(MAX_CROSS_CWD_DIRS) {
        ...
        let cwd = match crate::util::kimix_home::decode_cwd_from_dirname(dir) {
