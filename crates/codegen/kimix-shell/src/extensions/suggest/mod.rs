...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "kimix/suggest" => handle_suggest(agent, args).await,
        "kimix/suggestPrompt" => handle_suggest_prompt(agent, args).await,
        _ => Err(acp::Error::method_not_found()),
    }
}

/// Request/response for `Kimix/suggestPrompt` — predict the user's likely next
...
/// forever. Reasoning models (e.g. `kimix`) can take ~30s on a cold
