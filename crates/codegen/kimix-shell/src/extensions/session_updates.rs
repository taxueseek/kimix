//! ACP extension handler for bulk session updates (`Kimix/session/updates`).
//!
...
//! the update type (`"session/update"` for ACP, `"_Kimix/session/update"` for
...
/// Send updates as chunked `_Kimix/session/updates/chunk` notifications.
...
mod tests {
    ...
    fn dummy_gateway() -> kimix_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        kimix_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("kimix/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_kimix/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    async fn handle_falls_back_to_id_lookup_for_divergent_cwd() {
        ...
        // Clean up the dir written under the real Kimix home.
        let _ = std::fs::remove_dir_all(&child_dir);
    }

    fn capturing_gateway() -> (
        kimix_acp_lib::AcpAgentGatewaySender,
        tokio::sync::mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>,
    ) {
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        (kimix_acp_lib::AcpAgentGatewaySender::new(tx), rx)
    }
    ...
    fn xai_rewind(target: usize) -> String {
        format!(
            r#"{{"timestamp":0,"method":"_kimix/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"rewind_marker","target_prompt_index":{target}}}}}}}"#
        )
    }
