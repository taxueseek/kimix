...
//! using the Kimix-codebase-graph index.
//!
...
//! | Method | Description |
//! |--------|-------------|
//! | `Kimix/code/goto-definition` | Definition location(s) for symbol at position |
//! | `Kimix/code/goto-references` | Reference location(s) for symbol at position |
//! | `Kimix/code/find-definitions` | All definitions of a symbol by name |
//! | `Kimix/code/find-references` | All references to a symbol by name |
//! | `Kimix/code/status` | Indexing status |
...
/// Reason string for the `Kimix/code/status` response.
///
...
pub async fn handle(
    ...
    use kimix_workspace::workspace_ops::*;

    match args.method.as_ref() {
        "kimix/code/goto-definition" => {
...
fn to_code_nav_ext_response(resp: kimix_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for kimix-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise kimix/codeNavigation.enabled to use code navigation"
        }
