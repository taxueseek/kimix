...
use kimix_workspace::session::git::{
...
/// Request for Kimix/git/checkout_commit extension method.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
async fn resolve_git_root(
    ...
    if let Some(sid) = session_id {
        if let Some(cwd) = agent.get_session_cwd(sid) {
            let result = ops
                .dispatch(
                    &kimix_workspace::workspace_ops::GitResolveRootReq { cwd },
                    None,
                )
                .await
                .map_err(|e| {
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "kimix/git/status" => {
            ...
            if let Some(ref git_root) = git_root {
                let current_commit = ops
                    .dispatch(
                        &kimix_workspace::workspace_ops::GitCurrentCommitReq {
        ...
        "kimix/git/checkout_session_head" => {
            ...
            let result = ops
                .dispatch(
                    &kimix_workspace::workspace_ops::GitCheckoutCommitReq {
                        git_root: git_root.clone(),
                        head_commit,
