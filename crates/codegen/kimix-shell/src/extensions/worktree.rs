//! Handler for Kimix/git/worktree/* extension methods.
...
impl WorktreeNotificationSender for GatewayWorktreeNotifier {
    async fn send_worktree_status(&self, progress: WorktreeStatus) {
        ...
        let notification = acp::ExtNotification::new("kimix/git/worktree/status", params.into());
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "kimix/git/worktree/create_from_worktree" => {
            let mut req =
            ...
            let result = ops
                .dispatch(
                    &kimix_workspace::workspace_ops::PrepareWorktreeFromWorktreeReq {
                        inner: req.clone(),
                    },
                    None,
                )
        ...
        "kimix/git/worktree/create_from_worktree_sync" => {
            ...
            let resolved_root = ops
                .dispatch(
                    &kimix_workspace::workspace_ops::GitResolveRootReq {
            ...
            if let Some(git_root) = resolved_root {
                let vcs_kind = ops
                    .dispatch(
                        &kimix_workspace::workspace_ops::DetectVcsKindReq {
                            path: git_root.clone(),
                        },
                        None,
                    )
                    .await
                    .unwrap_or(kimix_workspace::session::git::VcsKind::Git);
                if vcs_kind.is_jj() {
            ...
            let result = ops
                .dispatch(
                    &kimix_workspace::workspace_ops::CreateWorktreeFromWorktreeSyncReq {
                        inner: req.into_wire(),
                    },
                    None,
                )
        ...
        "kimix/git/worktree/list" => {
            let req: kimix_workspace::workspace_ops::WorktreeListReq =
        ...
        "kimix/git/worktree/show" => {
            let req = serde_json::from_str::<ShowWorktreeRequest>(args.params.get())?;
            let op = kimix_workspace::workspace_ops::WorktreeShowReq {
        ...
        "kimix/git/worktree/gc" => {
            ...
            let op = kimix_workspace::workspace_ops::WorktreeGcReq {
                dry_run: req.dry_run,
                max_age_secs,
                force: req.force,
            };
        ...
        "kimix/git/worktree/db/stats" => {
            let result = ops
                .dispatch(&kimix_workspace::workspace_ops::WorktreeDbStatsReq {}, None)
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            to_response(Ok(result))
        }
        "kimix/git/worktree/db/rebuild" => {
            let result = ops
                .dispatch(
                    &kimix_workspace::workspace_ops::WorktreeDbRebuildReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "kimix/git/worktree/db/path" => {
            let result = ops
                .dispatch(&kimix_workspace::workspace_ops::WorktreeDbPathReq {}, None)
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            to_response(Ok(result))
        }
...
mod tests {
    ...
    fn db_path_response_serializes() {
        let resp = WorktreeDbPathResponse {
            path: "/home/user/.kimix/worktrees.db".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"path\":\"/home/user/.kimix/worktrees.db\""));
    }
