...
pub fn loaded_plugin_to_info(plugin: &kimix_agent::plugins::LoadedPlugin) -> PluginInfo {
    use kimix_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &kimix_agent::plugins::PluginOrigin) -> PluginOrigin {
    use kimix_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectKimix => PluginOrigin::ProjectKimix,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserKimix => PluginOrigin::UserKimix,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "kimix/plugins/action" => {
            let req: kimix_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use kimix_agent::plugins::PluginOrigin as AgentOrigin;
    use kimix_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> kimix_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        kimix_agent::plugins::LoadedPlugin {
