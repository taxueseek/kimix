...
//! - `Kimix/session/rename`                  rename a session locally
//! - `Kimix/session/delete`                  delete a session locally
//! - `Kimix/session/update_mcp_servers`      mid-session MCP server swap
//! - `Kimix/session/fork`                    fork a session into a new one
//! - `Kimix/internal/reload_all_mcp_servers` config hot-reload, all sessions
//! - `Kimix/internal/reload_project_mcp_servers` config hot-reload, cwd-scoped
//! - `Kimix/internal/reload_skills`          skills file watcher fan-out
//! - `Kimix/internal/reload_models`          model list hot-reload from config.toml
//! - `Kimix/internal/reload_models_cache`    model catalog hot-reload from disk cache
//! - `Kimix/plugins/reload`                  rebuild shared plugin registry
//! - `Kimix/commands/list`                   list slash commands
use std::path::Path;
use std::sync::Arc;
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "kimix/session/rename" => handle_session_rename(agent, args).await,
        "kimix/session/delete" => handle_session_delete(agent, args).await,
        "kimix/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        "kimix/session/fork" => handle_session_fork(agent, args).await,
        "kimix/internal/reload_all_mcp_servers" => handle_reload_all_mcp_servers(agent).await,
        "kimix/internal/reload_project_mcp_servers" => {
            handle_reload_project_mcp_servers(agent, args).await
        }
        "kimix/internal/reload_skills" => handle_reload_skills(agent),
        "kimix/internal/reload_models" => handle_reload_models(agent),
        "kimix/internal/reload_models_cache" => handle_reload_models_cache(agent),
        "kimix/plugins/reload" => handle_plugins_reload(agent).await,
        "kimix/commands/list" => handle_commands_list(agent, args).await,
...
async fn handle_session_rename(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    if req.kind == SessionKind::Chat {
        return Err(acp::Error::invalid_request()
            .data("chat conversations are not available in kimix (local sessions only)"));
    }
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Local disk + FTS eviction. Mirrored by the `kimix sessions delete <id>`
    // CLI path.
...
/// hot-reload watcher when `<cwd>/.Kimix/config.toml`,
...
/// Hot-reload the model catalog from `~/.kimix/models_cache.json` after an
...
async fn handle_plugins_reload(agent: &MvpAgent) -> ExtResult {
    ...
    // Explicit desktop `Kimix/plugins/reload`: force a full local-install re-copy.
    agent
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `Kimix/commands/list` (the pull used by Kimix-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .Kimix/plugins under it, plus
...
async fn handle_session_fork(_agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
