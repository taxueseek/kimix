//! `Kimix/memory/flush`, `Kimix/memory/rewrite`, and `Kimix/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("kimix/compact_conversation") => handle_compact(agent, args).await,
        "kimix/memory/flush" => handle_flush(agent, args).await,
        "kimix/memory/rewrite" => handle_rewrite(agent, args).await,
