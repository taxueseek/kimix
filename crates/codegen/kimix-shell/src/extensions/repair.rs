//! `Kimix/session/repair` — out-of-band recovery for sessions bricked by
...
//!
//! Repairs via [`kimix_chat_state::compaction_utils::repair_history`]. Resident
...
use kimix_chat_state::compaction_utils::HistoryRepairReport;
...
/// Response payload for `Kimix/session/repair`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "kimix/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `kimix_root` is injectable for tests.
async fn repair_on_disk(
    kimix_root: &std::path::Path,
    session_id: &str,
    dry_run: bool,
) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &kimix_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(kimix_root.to_path_buf());
    ...
    let report = kimix_chat_state::compaction_utils::repair_history(&mut chat_history);
