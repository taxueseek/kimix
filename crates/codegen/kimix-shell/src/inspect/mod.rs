//! `kimix inspect` — configuration introspection.
//!
//! Shows everything Kimix discovers in the current directory: project
...
use kimix_tools::util::truncate::estimate_tokens;
...
pub struct ConfigSources {
    ...
    /// .Kimix/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `kimix inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = kimix_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = kimix_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = kimix_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        kimix_version: kimix_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if has_rules_directory(file_path, ".kimix")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = kimix_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        kimix_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let kimix_home = Some(crate::util::kimix_home::kimix_home());
    ...
    // `kimix_agent::prompt::agents_md::read_agents_config_with_paths`
...
/// which handles both Kimix TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use kimix_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[kimix_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = kimix_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        kimix_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);

    let home_dir = dirs::home_dir();
    let kimix_home = kimix_config::kimix_home();

    let mut entries: Vec<HookEntry> = registry
        .all_hooks()
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&kimix_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &kimix_agent::plugins::PluginRegistry,
    skills_config: &kimix_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = kimix_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &kimix_home);
...
/// `<kimix_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
/// discovery: deployed pagers parse `Kimix/skills/list` into a typed
...
fn skill_entry_source(
    s: &kimix_agent::prompt::skills::SkillInfo,
    kimix_home: &Path,
) -> ConfigSource {
    use kimix_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, kimix_home) {
...
fn list_agents(
    ...
    let agents = kimix_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[kimix_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                kimix_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                kimix_agent::plugins::PluginScope::Project => Scope::Project,
                kimix_agent::plugins::PluginScope::User => Scope::User,
                kimix_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: kimix_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    kimix_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = kimix_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `kimix mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.Kimix/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_kimix_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.kimix_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `kimix mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use kimix_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use kimix_tools::implementations::skills::types::SkillScope;
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.kimix/requirements.toml";
    ...
    fn model_override_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("kimix-4.5"));

        let human = render_model_override_warnings(&warnings);
        assert!(human.contains("Model Overrides"), "{human}");
        assert!(
            human.contains("[model.\"kimix-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"kimix-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["modelKey"], "kimix-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.kimix");

        let s = skill_fixture("a", "/repo/.kimix/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.kimix/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.kimix/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.kimix/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<kimix_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.kimix/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Bundled name in a user dir outside <kimix_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <kimix_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.kimix/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <kimix_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.kimix/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.kimix dirs.
        ...
        let registry = kimix_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
