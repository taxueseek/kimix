// claude_import.rs
// Scans Claude settings and generates TOML patches for .Kimix/config.toml.
//
...
use kimix_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use kimix_workspace::permission::rules::parse_permission_rule;
use kimix_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.kimix/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.Kimix/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.kimix/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.Kimix/config.toml`.
...
// The marker `[claude_compat] imported = true` in `~/.kimix/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.kimix/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.kimix/config.toml`.
///
...
pub fn mark_claude_imported() -> anyhow::Result<()> {
    let path = crate::util::kimix_home::kimix_home().join("config.toml");
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.Kimix/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.Kimix/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        let global_path = crate::util::kimix_home::kimix_home().join("config.toml");
        ...
        // Hooks are written separately to ~/.kimix/hooks/imported-from-claude.json.
        let hooks_dir = crate::util::kimix_home::kimix_home().join("hooks");
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".kimix").join("config.toml");
        ...
        let hooks_dir = project_root.join(".kimix").join("hooks");
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .Kimix/hooks/ JSON files in apply_hooks_to_dir,
...
///
/// The output JSON is the same shape that `Kimix-hooks` natively understands
/// (Claude-compatible). The native hooks loader scans `.Kimix/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .kimix/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_KIMIX_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn gate_load_claude_env_returns_empty_when_marker_set() {
        let _g = MarkerGuard;
        refresh_marker_cache(true);
        // `load_claude_env_with_project` lives in Kimix-workspace, whose gate
        ...
        unsafe { std::env::set_var("_KIMIX_CLAUDE_MARKER_OVERRIDE", "1") };
        let dir = tempfile::tempdir().unwrap();
        let env = kimix_workspace::permission::claude_settings::load_claude_env_with_project(
            dir.path(),
            true,
        );
        assert!(
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = kimix_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.kimix) don't interfere.
        let has_probe = |reg: &kimix_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `KIMIX_SHARE_DIR` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        // managed settings, and the developer's real `~/.kimix/config.toml`.
        // We can't isolate `kimix_home()` because it's `OnceLock`-cached.
        // Instead, assert on rule *provenance*: no rule should originate from
        // our tempdir's `.claude/settings.json`. The dev's real ~/.kimix
        // config rules (if any) are out of scope for this test.
        let resolved =
            kimix_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use kimix_workspace::permission::types::RequirementSource;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.kimix/config.toml` may legitimately have the marker
        // set during local testing, and we can't override `kimix_home()`
