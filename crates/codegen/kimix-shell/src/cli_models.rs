//! Data APIs for `kimix models`. Clients own display.
...
/// Status for the `kimix models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `kimix_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = kimix_env::oauth_host();
            let host = origin
                .strip_prefix("https://")
...
mod tests {
    ...
    ///
    /// Uses `KIMIX_AUTH_PATH` (not `KIMIX_SHARE_DIR`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("KIMIX_AUTH"),
            EnvGuard::set("KIMIX_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("KIMIX_DEPLOYMENT_KEY"),
            EnvGuard::unset("KIMIX_WS_ORIGIN"),
            EnvGuard::unset("KIMIX_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("KIMIX_AUTH", &json);
