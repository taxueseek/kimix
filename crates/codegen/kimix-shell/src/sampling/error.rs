...
//! The canonical error types now live in `kimix_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a Kimix-shell dependency).

// Re-export everything from the standalone crate.
pub use kimix_sampling_types::error::*;
...
/// Kimix draw on the SAME subscription quota, so the message says so — a user
...
pub static RATE_LIMITED_USER_MESSAGE_OAUTH: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| {
        format!(
            "You\u{2019}ve hit the usage limit of your Kimi subscription. Note that Kimix and the \
             official Kimi CLI share the same subscription quota. Upgrade your plan at {} or try \
             again later.",
            kimix_env::upgrade_page_url()
        )
    });
...
/// This stays in Kimix-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::Api {
            ...
            StatusCode::FORBIDDEN => {
                let message = if message.contains("requires a Kimix subscription")
                    && crate::agent::auth_method::has_xai_api_key_env()
                {
                    format!(
                        "{message}\n\nYou have an API key set (XAI_API_KEY). \
                         ...
                         To use your API key, run `kimix logout` or type /logout in the TUI."
                    )
                } else {
                    message
                };
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                kimix_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: kimix_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != kimix_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == kimix_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = kimix_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &kimix_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn rate_limited_user_message_oauth_vs_api_key() {
        ...
        assert!(RATE_LIMITED_USER_MESSAGE_OAUTH.contains(kimix_env::upgrade_page_url()));
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        let prev_legacy = std::env::var("KIMIX_CODE_XAI_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("XAI_API_KEY");
            std::env::remove_var("KIMIX_CODE_XAI_API_KEY");
            if let Some(k) = key {
        ...
        unsafe {
            ...
            if let Some(v) = prev_legacy {
                std::env::set_var("KIMIX_CODE_XAI_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("xai-test"), || {
            ...
            assert!(
                msg.contains("kimix logout"),
                "should suggest kimix logout when API key is available: {msg}"
            );
            assert!(
                msg.contains("/logout"),
