// Re-export all types from the standalone Kimix-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use kimix_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `Kimix-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `Kimix-sampler` crate can reference them without a circular
// dep on `Kimix-shell`.

// Tests for the types now live in Kimix-sampling-types crate.

use kimix_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `Kimix-sampling-types` or
/// `Kimix-tools`) so neither of those crates needs to depend on
