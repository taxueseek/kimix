...
const NOTIFICATION_METHOD: &str = "kimix/terminal/pty/notification";
...
/// `KIMIX_SHELL`) since `$SHELL` is absent.
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use kimix_config::shell::{WindowsShell, detect_windows_shell};
