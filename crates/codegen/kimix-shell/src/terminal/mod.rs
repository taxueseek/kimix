...
/// Resolved absolute path to bash. On Unix uses the `kimix_config` shell
/// resolution cascade (`$KIMIX_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        kimix_config::shell::unix_shell_path(kimix_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `Kimix-tools`.
pub use kimix_tools::util::pager_env;
...
/// tool output. Used when the client sets `Kimix/bashOutputNoColor: true` (e.g.
/// Kimix-tui which renders its own UI and doesn't need raw ANSI codes).
///
