//! AcpTerminalAdapter: implements `Kimix-tools::TerminalBackend` using ACP gateway calls.
//!

//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates Kimix-tools' `TerminalBackend` trait into ACP protocol calls:
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            if !poll_for_terminal_exit(&gateway, &session_id, &terminal_id, None).await {
                ...
                let snapshot = {
                    let mut tasks = tasks.lock().expect("lock poisoned");
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (Kimix-desktop) spawns with `shell: true`
...
/// Wraps Kimix-shell's ACP gateway to satisfy Kimix-tools' TerminalBackend.
...
mod tests {
    ...
    fn task_map_insert_and_mark_completed() {
        ...
        {
            let mut map = tasks.lock().expect("lock poisoned");
        ...
        {
            let map = tasks.lock().expect("lock poisoned");
