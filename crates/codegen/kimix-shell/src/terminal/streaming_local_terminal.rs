...
/// after we stop waiting. Mirrors the bound the `Kimix-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("KIMIX_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for kimix_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
