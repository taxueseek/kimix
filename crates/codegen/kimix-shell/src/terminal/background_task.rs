...
/// Path format: `~/.kimix/sessions/{session_id}/tasks/{task_id}.log`
pub fn get_task_output_path(session_id: &str, task_id: &str) -> PathBuf {
    use crate::util::kimix_home::kimix_home;

    let tasks_dir = kimix_home().join("sessions").join(session_id).join("tasks");
...
pub struct BackgroundTaskManifestEntry {
    ...
    pub cwd: String,
    #[serde(default)]
    pub kind: kimix_tools::computer::types::TaskKind,
}
...
pub fn format_resumed_tasks_reminder(entries: &[BackgroundTaskManifestEntry]) -> String {
    ...
    for entry in entries {
        ...
        let kind_label = match entry.kind {
            kimix_tools::computer::types::TaskKind::Monitor => " [monitor]",
...
mod tests {
    ...
    fn format_reminder_labels_monitor_tasks() {
        ...
        entry.kind = kimix_tools::computer::types::TaskKind::Monitor;
        let reminder = format_resumed_tasks_reminder(&[entry]);
