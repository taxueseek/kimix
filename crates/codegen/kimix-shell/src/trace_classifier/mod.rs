...
/// two-function shape in `Kimix-tools` (F29).
...
/// `Kimix-tools` — existing items keep their prior content when
...
/// binary. Wraps a `kimix_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: kimix_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: kimix_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_kimix_conv_id: session_id_str.clone(),
        x_kimix_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
        x_kimix_session_id: session_id_str,
        x_kimix_agent_id: Some(crate::util::agent_id::agent_id()),
...
pub struct RunArgs {
    ...
    /// Kimix-home directory containing the `auth.json` to consult as
    /// the third API-key fallback. Defaults to
    /// `kimix_shell::util::kimix_home::kimix_home()` when `None`.
    /// Exposed as a CLI flag for testability.
    pub kimix_home: Option<PathBuf>,
}
...
/// provider command (if `KIMIX_AUTH_PROVIDER_COMMAND` is set) is
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, kimix_home: &Path) -> Result<String> {
    ...
    if let Some(key) = non_interactive_auth_key(kimix_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
        "no API key: pass --api-key, set XAI_API_KEY, or run `kimix login` to populate \
         <kimix-home>/auth.json. An expired OIDC token is auto-refreshed when a refresh_token \
         is present; if not, re-login is required."
    ))
}

/// Build an `AuthManager` for `kimix_home`, install the
...
/// explicit `kimix_home` so `--Kimix-home` overrides work in test
/// fixtures (the upstream helper hardcodes the global `kimix_home()`
/// path).
///
/// Returns:
...
async fn non_interactive_auth_key(kimix_home: &Path) -> Result<Option<String>> {
    use crate::auth::{AuthError, AuthManager, KimiCodeConfig};

    let manager = std::sync::Arc::new(AuthManager::new(kimix_home, KimiCodeConfig::default()));
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `kimix login` to re-authenticate, \
...
) -> Result<kimix_sampler::SamplingClient> {
    let default_home;
    let kimix_home_path: &Path = match kimix_home {
        Some(p) => p,
        None => {
            default_home = crate::util::kimix_home::kimix_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, kimix_home_path).await?;
    let config = kimix_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..kimix_sampler::SamplerConfig::default()
    };
    kimix_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use kimix_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(req.x_kimix_conv_id.as_deref(), Some("sess-x"));
        assert_eq!(req.x_kimix_session_id.as_deref(), Some("sess-x"));
        let req_id = req.x_kimix_req_id.as_deref().expect("req id");
    ...
    fn write_auth_json(kimix_home: &Path, key: &str) {
        ...
        std::fs::write(kimix_home.join("auth.json"), body.to_string()).expect("write auth.json");
    }
    ...
    fn write_empty_auth_json(kimix_home: &Path) {
        std::fs::write(kimix_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    fn write_fresh_oidc_auth_json(kimix_home: &Path, key: &str) {
        let scope = crate::auth::KimiCodeConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    fn write_expired_oidc_auth_json_no_refresh(kimix_home: &Path, key: &str) {
        let scope = crate::auth::KimiCodeConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    async fn resolve_api_key_precedence() {
        ...
        let kimix_home = tmp.path();
        // No auth.json at all → AuthManager sees nothing on disk.
        write_empty_auth_json(kimix_home);
        ...
        assert!(
            resolve_api_key(None, kimix_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), kimix_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), kimix_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), kimix_home)
                .await
                .expect("ok"),
            "from-flag",
        );
        ...
        assert_eq!(
            resolve_api_key(None, kimix_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), kimix_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), kimix_home).await.expect("ok"),
            "from-env",
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let kimix_home = tmp.path();
        write_auth_json(kimix_home, "from-auth-json");
        // `with_no_env` returns sync; we re-implement the clearing
        // inline so we can `.await` the resolver.
        let key =
            with_env_isolated(async { resolve_api_key(None, kimix_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("XAI_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, kimix_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, kimix_home)
                .await
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(kimix_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, kimix_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(kimix_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, kimix_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_refreshable_oidc_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let kimix_home = tmp.path();
        write_fresh_oidc_auth_json(kimix_home, "fresh-oidc-key");
        let key =
            with_env_isolated(async { resolve_api_key(None, kimix_home).await.expect("ok") }).await;
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        ...
        let tmp = tempfile::tempdir().expect("tempdir");
        let kimix_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(kimix_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, kimix_home).await }).await;
        assert!(
            err.is_err(),
    ...
    /// * `KIMIX_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `KIMIX_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.kimix/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `KIMIX_AUTH_PROVIDER_COMMAND` — selects an external
    ...
    async fn run_rejects_missing_trace_path() {
        ...
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            kimix_home: Some(tmp.path().to_path_buf()),
        };
