...
/// Router for Kimix/session/* and Kimix/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "kimix/session/info" => handle_session_info(agent, args).await,
        "kimix/session/close" => handle_session_close(agent, args).await,
        "kimix/session/list" => handle_session_list(agent, args).await,
        "kimix/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("kimix/session_summaries/") => {
...
/// `Kimix/sessions/list` — the FleetView roster. Returns every
...
/// `Kimix/sessions/changed` broadcast.
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via kimix/session/close");
    } else {
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "kimix/session_summaries/session_list" => {
        ...
        "kimix/session_summaries/workspace_list" => {
        ...
        "kimix/session_summaries/workspace_list_recent" => {
