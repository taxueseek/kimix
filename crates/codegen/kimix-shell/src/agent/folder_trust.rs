...
//! inside a cloned repository (`.mcp.json`, project `.Kimix/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.Kimix/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`kimix_workspace::trust::TrustStore`]
//! reads/writes — lives in `Kimix-workspace` (client-side); this module keeps
...
//! on [`kimix_workspace::folder_trust::decide`]; the consume-side nuance is
...
use kimix_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};
use parking_lot::Mutex;

// Decision-side (scan/decide/prompt/store) relocated to `Kimix-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `kimix_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use kimix_workspace::folder_trust::grant_folder_trust;
use kimix_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.kimix/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`kimix_agent::plugins::TrustStore` at `~/.kimix/trusted-plugins`, plus the
...
/// [`kimix_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = kimix_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// `Kimix/folderTrust.interactive`); the TTY-based [`decide_inputs`] default is
...
/// ([`kimix_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.Kimix/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    kimix_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: kimix_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != kimix_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `kimix mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `kimix --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `kimix mcp doctor`). It MUST
...
///
/// Sources: project `.Kimix/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.kimix/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.Kimix/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            kimix_tools::implementations::lsp::config::LspServerConfig,
            kimix_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, kimix_tools::implementations::lsp::config::LspServerConfig>
{
    kimix_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use kimix_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(kimix_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; KIMIX_SHARE_DIR-isolated so the seed can't touch the real user file and
        // `#[serial]` because KIMIX_SHARE_DIR is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. KIMIX_SHARE_DIR-isolated so the grant writes to a temp
        // store; `#[serial]` because KIMIX_SHARE_DIR is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. KIMIX_SHARE_DIR-isolated so the grant writes to a temp store;
        // `#[serial]` because KIMIX_SHARE_DIR is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; KIMIX_SHARE_DIR-isolated store; KIMIX_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".kimix").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. KIMIX_SHARE_DIR-isolated so the trust store is empty;
        // KIMIX_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = kimix_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. KIMIX_SHARE_DIR-isolated so the trust store is empty;
        // KIMIX_FOLDER_TRUST unset so the default-on feature flag applies.
        use kimix_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). KIMIX_SHARE_DIR-isolated so the trust store is empty.
        use kimix_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. KIMIX_SHARE_DIR-isolated (empty store).
        use kimix_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".kimix").join("agents");
        ...
        .unwrap();
        let def = kimix_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.Kimix/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). KIMIX_SHARE_DIR-isolated (empty
        // store); KIMIX_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".kimix").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + KIMIX_SHARE_DIR-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. KIMIX_SHARE_DIR-isolated so the seeded store is the temp one;
        // KIMIX_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // KIMIX_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(kimix_version::TEST_VERSION_ENV);
        if option_env!("KIMIX_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.Kimix/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. KIMIX_SHARE_DIR-isolated (empty store);
        // KIMIX_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".kimix").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // KIMIX_SHARE_DIR-isolated (empty store); KIMIX_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".kimix").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // KIMIX_SHARE_DIR-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); KIMIX_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use kimix_agent::plugins::discovery::DiscoveryConfig;
        use kimix_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let plugin = tmp.path().join(".kimix").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // KIMIX_SHARE_DIR-isolated so the grant writes to a temp store; KIMIX_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".kimix").join("hooks");
        ...
        let has_project_hook = |reg: &kimix_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = kimix_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        use kimix_tools::implementations::lsp::config::LspServerConfig;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.kimix/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.kimix/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use kimix_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use kimix_tools::types::config_source::ConfigSource;

        // A `<cwd>/.Kimix/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.kimix/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.Kimix/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let kimix = tmp.path().join(".kimix");
        std::fs::create_dir_all(&kimix).unwrap();
        std::fs::write(kimix.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.Kimix/config.toml [mcp_servers]`).
    ...
    /// `.Kimix/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `kimix --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(kimix_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("KIMIX_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no KIMIX_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(kimix_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(kimix_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("KIMIX_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // KIMIX_SHARE_DIR so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("KIMIX_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".kimix").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".kimix").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with KIMIX_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. KIMIX_SHARE_DIR-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(kimix_version::TEST_VERSION_ENV);
        if option_env!("KIMIX_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. KIMIX_SHARE_DIR-isolated so the store starts
        // empty; `#[serial]` because KIMIX_SHARE_DIR is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let tmp = repo_tmp();
        ...
        // KIMIX_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(kimix_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; KIMIX_SHARE_DIR-isolated and
        // KIMIX_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".kimix").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".kimix").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.Kimix/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".kimix")).unwrap();
        std::fs::write(tmp.path().join(".kimix").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
