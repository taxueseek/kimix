...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        kimix_log::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::kimix_home::kimix_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    kimix_workspace::permission::cleanup_stale_permission_state(
        ...
        kimix_workspace::trust::migrate_legacy_hook_trust();
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("kimix-web") => client_type = ClientType::KimixWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("kimix-code-extension") => client_type = ClientType::Extension,
                Some("kimix-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        *self.client_type.borrow_mut() = client_type;
        ...
        tracing::info!(
            ...
            "code-nav capability initialized from initialize request; \
             index will start lazily on first kimix/code/* request if eligible"
        );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        kimix_log::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "kimix_home" : crate ::util::kimix_home::kimix_home().display()
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                .agent_capabilities(
                    acp::AgentCapabilities::new()
                        ...
                        .meta(
                            serde_json::json!(
                                { "kimix/fs_notify" : true, "kimix/hooks" : { "blockingEvents"
                                : [kimix_hooks::event::HookEventName::PreToolUse],
                                "decisions" : ["deny"], }, }
                            )
                                .as_object()
                                .cloned(),
                ...
                .meta({
                    let metadata = parse_json_object_env("KIMIX_AGENT_METADATA");
                    serde_json::json!(
                        { "kimixShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (kimix_mcp::wire::MCP_SDK) :
                        ...
                        kimix_version::VERSION, "agentId" : crate::util::agent_id::agent_id(),
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            ...
            auth_method::XAI_SESSION_METHOD_ID => {
                // Native xAI OIDC (device-code) or import of an existing
                // ~/.grok session into ~/.kimix/auth.json scope oidc/xai.
                self.authenticate_xai_session(arguments).await
            }
            auth_method::GROK_SESSION_METHOD_ID => {
                // Legacy Grok CLI bridge: read ~/.grok/auth.json and import.
                match crate::auth::load_grok_session_token() {
                    Some(tok) => {
                        let auth_file =
                            crate::util::kimix_home::kimix_home().join("auth.json");
                        let _ = crate::auth::import_grok_token_into_kimix(&auth_file, &tok);
                        let mut sampling_config = self.sampling_config.borrow_mut();
                        sampling_config.api_key = Some(tok.access_token);
                        tracing::info!("auth: grok_session handler set api_key (SessionToken)");
                        kimix_log::unified_log::info(
                            "auth: grok_session bridge authenticated",
                            None,
                            None,
                        );
                        self.set_auth_method(arguments.method_id.clone());
                        emit_login_span(true, "grok_session", None, None);
                        Ok(Default::default())
                    }
                    None => {
                        tracing::warn!("auth: grok_session bridge failed - no usable token");
                        kimix_log::unified_log::warn(
                            "auth: grok_session bridge failed - run `grok login` or pick xAI Session",
                            None,
                            None,
                        );
                        emit_login_span(false, "grok_session", None, Some("no_token"));
                        Err(
                            acp::Error::auth_required().data(
                                "No Grok session found. Choose 「xAI Session」 on /login, \
                                 or run `grok login` / `kimix login --xai` first.",
                            ),
                        )
                    }
                }
    ...
    async fn new_session(
        ...
        let _client_identifier = arguments
            .meta
            .as_ref()
            .and_then(|m| m.get("clientIdentifier"))
            .and_then(|v| v.as_str())
        ...
        let (git_root, is_git_repo, discovery_failed) = match kimix_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
        ...
        if let Some(obj) = meta.as_object_mut() {
            obj.insert("kimix/sessionConfig".to_string(), session_config_value);
            obj.insert("kimix/sessionDetail".to_string(), session_detail_value);
        }
        Ok(
    ...
    async fn load_session(
        ...
        let persist_data = arguments
            .meta
            .as_ref()
            .and_then(|m| m.get("kimix/persist"))
            .cloned();
        ...
        let restore_code_requested = request_meta
            .as_ref()
            .and_then(|m| m.get("kimix/restore_code"))
            .and_then(|v| v.as_bool())
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            kimix_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = kimix_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : kimix_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a kimix worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            kimix_log::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use kimix_workspace::session::git::RestoreKind;
            let outcome = kimix_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let load_envrc = {
            let skip_envrc = request_meta
                .as_ref()
                .and_then(|m| m.get("kimix/skip_envrc"))
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            if skip_envrc {
                false
        ...
        let preloaded_envrc = kimix_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let prompt_display_cwd = request_meta
            .as_ref()
            .and_then(|m| m.get("kimix/display_cwd"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        let persisted_platform =
            kimix_models::parse_managed_model_key(persisted_model.0.as_ref()).map(|(p, _)| p);
        let same_family_fallback = available
            .keys()
            .find(|id| {
                kimix_models::parse_managed_model_key(id.0.as_ref()).map(|(p, _)| p)
                    == persisted_platform
            })
            .cloned()
        ...
        if let Some(persist) = persist_data {
            response_meta_map.insert("kimix/persist".to_string(), persist);
        }
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    kimix_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = kimix_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = kimix_workspace::session::git::detect_head_divergence(
            ...
            .and_then(|h| h.current_prompt_id.lock().ok().and_then(|g| g.clone()))
        {
            response_meta_map
                .insert(
                    "kimix/runningPromptId".to_string(),
        ...
        response_meta_map.insert("kimix/sessionConfig".to_string(), session_config_value);
        response_meta_map.insert("kimix/sessionDetail".to_string(), session_detail_value);
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            kimix_file_utils::trace_context::link_current_span_to_meta(
        ...
        let _turn_started_at = chrono::Utc::now().to_rfc3339();
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: kimix_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                respond_to: tx,
                persist_ack: None,
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            kimix_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            "kimix/getApiKey" | "kimix/setApiKey" => {
                crate::extensions::auth::handle(self, &args).await
            }
            "kimix/session/info" | "kimix/session/close" | "kimix/session/list"
            | "kimix/sessions/list" => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            "kimix/session/updates" => {
                crate::extensions::session_updates::handle(&args, &self.gateway).await
            }
            "kimix/session/load_history" => {
                crate::extensions::chat_conversation_history::handle(self, &args).await
            }
            "kimix/session/search" => {
                crate::extensions::session_search::handle(&args).await
            }
            "kimix/session/resolve_local_for_worktree_resume"
            | "kimix/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            "kimix/session/rename" | "kimix/session/delete"
            | "kimix/session/update_mcp_servers" | "kimix/session/fork"
            | "kimix/internal/reload_all_mcp_servers"
            | "kimix/internal/reload_project_mcp_servers" | "kimix/internal/reload_skills"
            | "kimix/internal/reload_models" | "kimix/internal/reload_models_cache"
            | "kimix/plugins/reload"
            | "kimix/commands/list" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "kimix/session/repair" => crate::extensions::repair::handle(self, &args).await,
            "kimix/billing" => crate::extensions::billing::handle(self, &args).await,
            ...
            "kimix/interject" => crate::extensions::interject::handle(self, &args).await,
            "kimix/feedback" | "kimix/feedback/dismiss" | "kimix/btw" => {
                crate::extensions::feedback::handle(self, &args).await
            }
            "kimix/recap" => crate::extensions::recap::handle(self, &args).await,
            "kimix/rollout/survey" => {
                crate::extensions::rollout::handle(self, &args).await
            }
            "kimix/prompt_history" => {
                crate::extensions::prompt_history::handle(self, &args).await
            }
            "kimix/suggest" => crate::extensions::suggest::handle(self, &args).await,
            "kimix/suggestPrompt" => crate::extensions::suggest::handle(self, &args).await,
            s if s.starts_with("kimix/auth/") => {
                crate::extensions::auth::handle(self, &args).await
            }
            s if s.starts_with("kimix/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            s if s.starts_with("kimix/git/worktree/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            s if s.starts_with("kimix/git/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::git::handle(self, &ops, &args).await
            }
            s if s.starts_with("kimix/compact_conversation") => {
                crate::extensions::memory::handle(self, &args).await
            }
            s if s.starts_with("kimix/plugins/") => {
                crate::extensions::plugins::handle(self, &args).await
            }
            s if s.starts_with("kimix/hooks/") => {
                crate::extensions::hooks::handle(self, &args).await
            }
            s if s.starts_with("kimix/hunk-tracker/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::hunk_tracker::handle(self, &ops, &args).await
            }
            s if s.starts_with("kimix/pr/") => {
            ...
            s if s.starts_with("kimix/review") => {
                crate::extensions::feedback::handle(self, &args).await
            }
            s if s.starts_with("kimix/debug/") => {
                crate::extensions::debug::handle(self, &args).await
            }
            s if s.starts_with("kimix/rewind") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "kimix/yolo_mode_changed"
        ...
        if args.method.as_ref() == "kimix/permissions/reset" {
        ...
        if args.method.as_ref() == "kimix/internal/evict_sessions" {
            self.handle_evict_sessions(&args.params).await;
        }
        if args.method.as_ref() == "kimix/toggle_plan_mode"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if matches!(
            args.method.as_ref(), "kimix/queue/remove" | "kimix/queue/reorder" |
            "kimix/queue/clear" | "kimix/queue/edit" | "kimix/queue/interject"
        )
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        ...
        if args.method.as_ref() == "kimix/terminal/pty/input"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "_kimix/session/update" {
        ...
        if args.method.as_ref() == "kimix/telemetry/non_git_decision" {
        ...
        if args.method.as_ref() == "kimix/telemetry/multi_agent_followup" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                let _total_agents = 1 + params.other_agents.len();
            } else {
                tracing::warn!("Failed to parse multi-agent followup telemetry params");
            }
        }
        if args.method.as_ref() == "kimix/telemetry/multi_agent_apply" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                let _total_agents = 1 + params.discarded_agents.len();
            } else {
                tracing::warn!("Failed to parse multi-agent apply telemetry params");
            }
        }
        if args.method.as_ref() == "kimix/telemetry/multi_agent_discard" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                let _total = params.discarded_agents.len();
            } else {
                tracing::warn!("Failed to parse multi-agent discard telemetry params");
            }
        }
        if args.method.as_ref() == kimix_log::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                kimix_log::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            kimix_log::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
