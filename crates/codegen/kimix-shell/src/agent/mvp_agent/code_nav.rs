...
impl MvpAgent {
    /// Parse the `Kimix/codeNavigation.enabled` capability from an initialize
    ...
    pub(crate) fn parse_code_nav_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("kimix/codeNavigation"))
    ...
    pub(crate) fn start_codebase_index_for_code_nav(
        ...
        cwd: &std::path::Path,
    ) -> Option<(std::sync::Arc<kimix_codebase_graph::IndexManagerHandle>, bool)> {
    ...
    pub(super) fn code_nav_eligibility_inner(
        ...
        if !code_nav_enabled {
            tracing::info!(
                ...
                "code-nav eligibility check: skipping (kimix/codeNavigation.enabled not advertised)"
            );
        ...
        let git_root = kimix_workspace::session::git::find_git_root_from_path(cwd).ok();
        match &setting {
    ...
    pub fn code_nav_eligibility_for_request(
        ...
        let session_id = match session_id {
            ...
            // global state.  Callers must provide sessionId for Kimix/code/* requests.
    ...
    pub(super) fn indexed_roots_for(&self, cwd: &std::path::Path) -> Vec<String> {
        ...
        if let Ok(git_root) = kimix_workspace::session::git::find_git_root_from_path(cwd)
    ...
    pub(super) fn get_or_create_codebase_index(
        &self,
        cwd: PathBuf,
    ) -> (std::sync::Arc<kimix_codebase_graph::IndexManagerHandle>, bool) {
    ...
    pub fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<std::sync::Arc<kimix_codebase_graph::IndexManagerHandle>> {
