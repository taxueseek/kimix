...
mod hunk_tracking_mode {
    ...
    use kimix_hunk_tracker::TrackingMode;
    #[test]
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_kimix() {
    let prev = std::env::var("KIMIX_AGENT").ok();
    unsafe {
        std::env::remove_var("KIMIX_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("KIMIX_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return Kimix.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = kimix_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `Kimix-plan`, models in the catalog that still declare
/// `agent_type = "Kimix"` explicitly must NOT preempt an ACP
/// profile. Any value in the `Kimix*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_kimix_family() {
    let prev = std::env::var("KIMIX_AGENT").ok();
    unsafe {
        std::env::remove_var("KIMIX_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["kimix", "kimix-plan", "kimix-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for kimix family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("KIMIX_AGENT", v) }
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".kimix").join("agents");
    ...
    assert_eq!(
        def.model,
        kimix_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("KIMIX_AGENT", v) },
        None => unsafe { std::env::remove_var("KIMIX_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `kimix` (a client profile name) to
/// `Kimix-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("kimix", "kimix-plan"));
    assert!(harnesses_are_compatible("kimix-plan", "kimix"));
    assert!(harnesses_are_compatible("kimix", "kimix"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("kimix-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "kimix-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_kimix_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "kimix-plan"),
        "kimix-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_kimix() {
    assert_eq!(
        resolve_required_agent_type(None, "kimix"),
        "kimix"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "kimix-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "kimix-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "kimix-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    let builder = kimix_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: kimix_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        app_builder_deployer_config: kimix_tools::implementations::kimix::deploy_app::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = kimix_hunk_tracker::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        kimix_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: kimix_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            kimix_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(kimix_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: kimix_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "kimix".to_string(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: kimix_workspace::WorkspaceOps::for_test(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    let sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("kimix-3-fast", false, None)),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("kimix-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("kimix-3", false, None)),
        (sid_b.clone(), make_test_handle("kimix-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use kimix_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("kimix-3", false, Some("kimix-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("kimix-3", false, Some("kimix-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("kimix-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("kimix-3", true, Some("kimix-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("kimix-3", true, Some("kimix-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("kimix-tui"), false);
...
fn parse_code_nav_capability_present_and_true() {
    ...
    meta.insert(
        "kimix/codeNavigation".to_string(),
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("kimix-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("kimix-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::KimixWeb) {
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let kimix_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("KIMIX_SHARE_DIR", kimix_home.path());
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("kimix-3", false, None);
    ...
    *prompt_id.lock().expect("lock poisoned") = Some("turn-1".to_string());
    assert_eq!(agent.resident_activity(&sid), RosterActivity::Working);
    pending.lock().expect("lock poisoned").insert(
        "call-1".to_string(),
...
/// Drain the agent gateway, returning the first `Kimix/sessions/changed`
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
/// an `Kimix/sessions/changed` upsert carrying the *overridden* activity, so
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("kimix-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("kimix-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("kimix-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("kimix-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("kimix-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("kimix-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::KimixWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::KimixTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::KimixWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
fn bg_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":1,"method":"_kimix/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_backgrounded","task_id":"{task_id}","command":"sleep 99","cwd":"/tmp"}}}}}}"#
    )
}
fn completed_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":2,"method":"_kimix/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_completed","task_snapshot":{{"task_id":"{task_id}","completed":true}}}}}}}}"#
    )
}
...
fn orphaned_tasks_ignores_unrelated_updates() {
    let tmp = tempfile::tempdir().unwrap();
    let bg = bg_line("t1");
    let unrelated = r#"{"timestamp":1,"method":"_kimix/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"auto_compact_started","percentage":80}}}"#;
...
fn orphaned_tasks_filters_rewind_dead_branches() {
    ...
    let rewind = r#"{"timestamp":3,"method":"_kimix/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":0,"created_at":"2025-01-01T00:00:00Z"}}}"#;
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    assert_eq!(agent.auth_type(), kimix_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), kimix_chat_state::AuthType::ApiKey,);
}
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    ...
    use kimix_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("KIMIX_DISABLE_API_KEY_AUTH");
...
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_kimix_com_without_credentials() {
...
fn parse_session_kind_matrix() {
    ...
    let cases: &[(&str, serde_json::Value, SessionKind)] = &[
        (
            "chat",
            json!({ "kimix/session" : { "kind" : "chat" } }),
            SessionKind::Chat,
        ),
        (
            "build",
            json!({ "kimix/session" : { "kind" : "build" } }),
            SessionKind::Build,
        ),
        (
            "chat_malformed_sibling",
            json!({ "kimix/session" : { "kind" : "chat", "facets" : "not-a-map" } }),
            SessionKind::Chat,
        ),
        (
            "unknown_kind",
            json!({ "kimix/session" : { "kind" : "frob" } }),
            SessionKind::Build,
        ),
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("kimix-4.5"), Some("kimix-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("kimix-4.5"), None),
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = kimix_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("kimix-tui"));
    ...
    if let Some(pid) = running_prompt {
        *handle.current_prompt_id.lock().expect("lock poisoned") = Some(pid.to_string());
    }
...
/// Drive `Kimix/internal/evict_sessions` through the real `ext_notification`
...
/// `Kimix/internal/evict_sessions` notification — the realistic shape of a
...
async fn drive_disconnect_many(agent: &MvpAgent, sids: &[&acp::SessionId]) {
    ...
    agent
        .ext_notification(acp::ExtNotification::new(
            "kimix/internal/evict_sessions",
            raw.into(),
        ))
        .await
        .expect("evict_sessions notification must be handled");
}
/// Drive `Kimix/session/close` through the real `ext_method` dispatch
...
fn disconnect_keeps_resident_on_poisoned_lock() {
    run_local_for_bridge_test(|| async {
        ...
        let _ = std::thread::spawn(move || {
            let _g = poison_target.lock().expect("lock poisoned");
...
fn disconnect_keeps_resident_when_plan_approval_parked() {
    run_local_for_bridge_test(|| async {
        ...
        let (handle, _cmd_tx, cmd_rx) = make_live_session_handle(&sid, None);
        handle.pending_interactions.lock().expect("lock poisoned").insert(
...
/// Mixed batch in a *single* `Kimix/internal/evict_sessions` notification —
...
/// `Kimix/session/close` dispatch (`ext_method` → `handle_session_close`),
...
/// `Kimix/folder_trust/request` round-trip.
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, KimiCodeConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
/// Pull the next `Kimix/folder_trust/request` reverse-request off the gateway and
/// answer it with `outcome`. Returns the request's decoded params.
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let kimix_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
        panic!("expected an ext_method reverse-request, got a different message");
    };
    assert_eq!(args.request.method.as_ref(), "kimix/folder_trust/request");
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use kimix_test_support::EnvGuard;
    use kimix_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
    let _sim = EnvGuard::set(kimix_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, kimix_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(kimix_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use kimix_test_support::EnvGuard;
    use kimix_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
    let _sim = EnvGuard::set(kimix_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(kimix_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
mod direct_hub_cloud_removed {
    ...
    fn cloud_server_id_meta_is_hard_error() {
        let meta = serde_json::json!({ "kimix/cloud_server_id" : "srv-123" });
    ...
    fn cloud_server_id_null_still_present_is_hard_error() {
        let meta = serde_json::json!({ "kimix/cloud_server_id" : null });
        let err = reject_direct_hub_cloud_meta(meta.as_object()).expect_err("must reject");
        assert_direct_hub_error(err);
    }
    #[test]
    fn cloud_server_id_with_gateway_meta_still_hard_error() {
        let meta = serde_json::json!(
            { "kimix/cloud_server_id" : "srv-legacy", "envId" : "env-1",
            "kimix/cloud_existing_workspace" : { "server_id" : "ws-1", "cwd" :
            "/workspace" } }
        );
    ...
    async fn emit_settings_update_carries_permission_mode_from_cfg() {
        ...
        local
            .run_until(async {
                ...
                let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
                    panic!("expected ExtNotification, got {msg:?}");
                };
                assert_eq!(args.request.method.as_ref(), "kimix/settings/update");
