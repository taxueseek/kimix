...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    ...
    local
        .run_until(async {
            ...
            let cwd = kimix_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) =
                kimix_workspace::permission::spawn_permission_manager(
                    ...
                    gateway,
                    cwd,
                    kimix_workspace::permission::types::ClientType::Generic,
            ...
            for access in [
                kimix_workspace::permission::AccessKind::Read(Some(".env".into())),
                kimix_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        kimix_workspace::permission::Decision::PolicyDeny(_)
                    ),
