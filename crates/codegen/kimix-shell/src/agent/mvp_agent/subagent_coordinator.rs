        ...
        let (
            ...
            (
                ...
                ps.map(|h| h.tool_context.hunk_tracker_handle.clone())
                    .unwrap_or_else(kimix_hunk_tracker::HunkTrackerHandle::noop),
                ...
                ps.map(|h| h.tool_context.fs.inner().clone())
                    .unwrap_or_else(|| {
                        let cwd = ps
                            ...
                            .unwrap_or_default();
                        std::sync::Arc::new(kimix_workspace::file_system::LocalFs::new(cwd))
                    }),
        ...
        Some(crate::agent::subagent::SubagentSpawnContext {
            ...
            task_output_tool_name: {
                ...
                sessions
                    ...
                    .unwrap_or_else(|| {
                        kimix_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL
                            .to_string()
                    })
            },
