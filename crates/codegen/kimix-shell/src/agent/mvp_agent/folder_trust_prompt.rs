...
//! (`Kimix/folder_trust/request`) that asks a GUI client (Kimix-desktop) to decide
...
//! `Kimix/folderTrust.interactive` AND the folder-trust feature flag is on AND the
//! verdict is [`kimix_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.Kimix/lsp.json` servers start on the NEXT session open (the durable grant
...
/// ACP `Kimix/folder_trust/request` payload (agent → GUI client). Serialized as
...
/// ACP `Kimix/folder_trust/request` response (GUI client → agent).
...
impl MvpAgent {
    /// Parse the `Kimix/folderTrust.interactive` capability from an initialize
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("kimix/folderTrust"))
    ...
    /// `Kimix/folderTrust.interactive` AND [`folder_trust::prompt_warranted`]
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = kimix_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                kimix_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("kimix/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a kimix_agent::plugins::SharedPluginRegistryHandle,
    compat: &'a kimix_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.Kimix/hooks`, `.cursor/hooks.json`),
