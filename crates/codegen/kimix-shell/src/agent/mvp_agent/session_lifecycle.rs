...
impl MvpAgent {
    ...
    /// session end — a terminal/explicit close (`Kimix/session/close`). It must
    ...
    /// (`Kimix/session/close`). Finalizes the cloud replica (genuine session
    ...
    /// `Kimix/sessions/changed` notification with the session in `removed` so
    ...
    /// `Kimix/sessions/changed` notification with the current entry in
    ...
    /// Emit an `Kimix/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `Kimix/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `Kimix/sessions/changed` special-case in `leader/server.rs`).
