...
use kimix_workspace::file_system::AsyncFileSystem;
...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<kimix_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn kimix_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub sampling_config: kimix_sampler::SamplerConfig,
    ...
    pub parent_terminal_backend: Option<Arc<dyn kimix_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<kimix_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_config: kimix_tools::implementations::WebSearchConfig,
    /// Resolved config for web fetch.
    pub web_fetch_config: kimix_tools::implementations::kimix::web_fetch::WebFetchConfig,
    ...
    pub parent_chat_state: Option<kimix_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<kimix_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<kimix_tools::registry::types::ToolConfig>>,
    /// Parent session's agent config snapshot.
    pub agent_config: Option<crate::agent::config::Config>,
    pub hook_registry: Option<std::sync::Arc<kimix_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<kimix_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<kimix_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: kimix_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<kimix_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "Kimix").
    ...
    pub parent_tool_snapshot: Option<Vec<kimix_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<kimix_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: kimix_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: kimix_tools::types::compat::CompatConfig,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut kimix_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("KIMIX_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("KIMIX_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use kimix_subagent_resolution::ResumeSourceData;
...
use kimix_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &kimix_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (kimix_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &kimix_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (kimix_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &kimix_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &kimix_sampler::SamplerConfig,
) {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(kimix_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[kimix_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<kimix_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<kimix_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        kimix_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[kimix_sampling_types::conversation::ConversationItem],
) -> bool {
    use kimix_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<kimix_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = kimix_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        kimix_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens =
                    kimix_chat_state::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &kimix_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use kimix_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<kimix_agent::config::AgentDefinition> {
    let mut def = kimix_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<kimix_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || kimix_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = kimix_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
/// agent definition.
///
/// The harness flavor (alternate vs Kimix) normally follows the PARENT
/// agent: `KimixOrchestrator` parents give children
...
/// (so a Kimix session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut kimix_agent::config::AgentDefinition,
) {
...
fn summarize_tool_config(
    config: &kimix_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: kimix_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> kimix_agent::config::PermissionMode {
    use kimix_agent::config::PermissionMode;
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    kimix_workspace::session::git::find_main_repo_root_from_path(&source_cwd).unwrap_or(source_cwd)
}
...
/// auto-backgrounded (non-destructively). Env override: `KIMIX_SUBAGENT_AWAIT_BUDGET_MS`.
const SUBAGENT_AWAIT_BUDGET: std::time::Duration = std::time::Duration::from_secs(600);
fn subagent_await_budget() -> std::time::Duration {
    std::env::var("KIMIX_SUBAGENT_AWAIT_BUDGET_MS")
        .ok()
...
fn cancellation_error_message(
    category: Option<kimix_file_utils::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<kimix_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    ...
    let message = kimix_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = kimix_tools::reminders::wrap_reminder(&message);
...
/// `Kimix/subagent/list_running` on reconnect.
