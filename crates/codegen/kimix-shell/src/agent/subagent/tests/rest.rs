...
fn normalize_forked_context_strips_project_layout() {
    use kimix_sampling_types::conversation::ConversationItem;
    let big_layout = "<project_layout>\nline1\nline2\nline3\n</project_layout>";
    let items = vec![
        ConversationItem::system("sys"), ConversationItem::user(big_layout),
        ConversationItem::assistant("ack"),
    ];
    let (conv, _) = kimix_subagent_resolution::context::normalize_forked_context(
        items,
    );
    if let ConversationItem::User(u) = &conv[1] {
        let text = u
            ...
            .filter_map(|p| match p {
                kimix_sampling_types::conversation::ContentPart::Text { text } => {
                    Some(text.as_ref())
                }
                _ => None,
            })
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = kimix_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = kimix_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = kimix_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        kimix_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
        ...
        .filter(|i| {
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                            p, kimix_sampling_types::conversation::ContentPart::Text {
...
#[test]
fn coordinator_concurrency_configured_and_unlimited() {
    assert_eq!(SubagentCoordinator::new(4).max_concurrency(), 4);
    assert_eq!(SubagentCoordinator::new(0).max_concurrency(), 0);
    // Unlimited still creates a semaphore (permits = 1) but the spawn path
    // skips acquiring when `max_concurrency == 0`, so no artificial limit.
    assert_eq!(SubagentCoordinator::new(0).concurrency_semaphore().available_permits(), 1);
}
#[test]
    ...
    coordinator
        ...
        .insert(
            ...
            CompletedSubagent {
                ...
                snapshot_ref: None,
                effective_model_id: "kimix-3".into(),
                block_waited: false,
...
fn snapshot_ref_field_in_meta_roundtrips() {
    let meta = SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/kimix-wt/sa-snap".into()),
        snapshot_ref: Some("refs/kimix/subagent-snapshots/sa-snap".into()),
        effective_model_id: None,
    };
    let json = serde_json::to_string(&meta).unwrap();
    assert!(json.contains("snapshot_ref"));
    assert!(json.contains("refs/kimix/subagent-snapshots/sa-snap"));
    let parsed: SubagentMeta = serde_json::from_str(&json).unwrap();
    assert_eq!(
        parsed.snapshot_ref.as_deref(), Some("refs/kimix/subagent-snapshots/sa-snap")
    );
}
#[test]
...
fn snapshot_test_meta(id: &str) -> SubagentMeta {
    SubagentMeta {
        ...
        resumed_from: None,
        child_cwd: None,
        worktree_path: Some("/tmp/kimix-wt/subagent-x".into()),
        snapshot_ref: None,
        effective_model_id: None,
    }
}
...
fn update_subagent_meta_snapshot_ref_persists_to_disk() {
    ...
    assert!(
        update_subagent_meta_snapshot_ref(dir.path(), "refs/kimix/subagents/sa-write",
    ...
    assert_eq!(reread.snapshot_ref.as_deref(), Some("refs/kimix/subagents/sa-write"));
    assert_eq!(reread.status, "completed");
    assert_eq!(reread.worktree_path.as_deref(), Some("/tmp/kimix-wt/subagent-x"));
}
...
fn update_subagent_meta_snapshot_ref_reports_failure_when_meta_missing() {
    let dir = tempfile::TempDir::new().unwrap();
    assert!(
        ! update_subagent_meta_snapshot_ref(dir.path(), "refs/kimix/subagents/sa-missing",
        "completed")
    );
}
...
fn snapshot_ref_write_promotes_nonterminal_status_to_terminal() {
    ...
    assert!(
        update_subagent_meta_snapshot_ref(dir.path(), "refs/kimix/subagents/x",
        "completed")
    );
    let data = std::fs::read_to_string(dir.path().join("meta.json")).unwrap();
    let reread: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(Some("refs/kimix/subagents/x"), reread.snapshot_ref.as_deref());
...
async fn set_completed_snapshot_ref_updates_in_memory_entry() {
    ...
    coordinator
        .set_completed_snapshot_ref("sa-mem", "refs/kimix/subagents/sa-mem".into());
    let after = coordinator
        .resumable_source_for("sa-mem", "session-A", Path::new("/tmp"))
        .unwrap();
    assert_eq!(after.snapshot_ref.as_deref(), Some("refs/kimix/subagents/sa-mem"));
}
/// Unknown id is a no-op (entry already TTL-evicted; meta.json still holds it).
#[test]
fn set_completed_snapshot_ref_unknown_id_is_noop() {
    let mut coordinator = SubagentCoordinator::new(4);
    coordinator.set_completed_snapshot_ref("ghost", "refs/kimix/subagents/ghost".into());
    assert!(
...
async fn completion_snapshot_sequence_persists_ref_then_removes_worktree() {
    kimix_test_utils::require_git!();
    use kimix_test_utils::git::{git_commit_all, init_git_repo};
    ...
    kimix_fast_worktree::WorktreeBuilder::new(&repo, &wt)
        .standalone(true)
        .create()
        .unwrap();
    ...
    let ref_name = "refs/kimix/subagents/glue-1";
...
async fn gate_on_completion_clears_model_facing_worktree_path_but_resume_retains_it() {
    let wt = PathBuf::from("/tmp/kimix-wt/subagent-disp-1");
    let mut coordinator = SubagentCoordinator::new(4);
    ...
    coordinator
        .set_completed_snapshot_ref("disp-1", "refs/kimix/subagents/disp-1".into());
    ...
    assert_eq!(Some("refs/kimix/subagents/disp-1"), src.snapshot_ref.as_deref());
}
...
async fn gate_on_completion_retains_worktree_path_when_not_removed() {
    let wt = PathBuf::from("/tmp/kimix-wt/subagent-keep-1");
    let mut coordinator = SubagentCoordinator::new(4);
...
async fn disposal_completes_before_subagent_is_observable() {
    ...
    let ref_name = "refs/kimix/subagents/order-1";
    let snapshot_ref = crate::session::worktree::snapshot_subagent_worktree(
            &wt,
            &repo,
            ref_name,
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.kimix/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
        worktree.as_deref(),
        Some(Path::new("/home/user/.kimix/worktrees/myrepo/subagent-sub-wt",)),
...
fn resume_inherited_cwd_requires_existing_non_worktree_dir() {
    ...
    let missing = ResumeSourceData {
        child_cwd: "/no/such/dir/kimix-missing".into(),
        ..present.clone()
    };
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.kimix/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_for_window_safety() {
    ...
    let estimated = kimix_chat_state::estimate_conversation_tokens(&conversation);
    ...
    assert_eq!(kimix_chat_state::estimate_conversation_tokens(& []), 0);
}
#[test]
fn token_estimation_accounts_for_images() {
    use kimix_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
    let text_only = vec![
        ConversationItem::User(UserItem { content : vec![ContentPart::Text { text :
        "describe this".into(), }], synthetic_reason : None, ..Default::default() })
    ];
    let text_tokens = kimix_chat_state::estimate_conversation_tokens(&text_only);
    let with_image = vec![
        ConversationItem::User(UserItem { content : vec![ContentPart::Text { text :
        "describe this".into(), }, ContentPart::Image { url : "data:image/png;base64,abc"
        .into(), },], synthetic_reason : None, ..Default::default() })
    ];
    let image_tokens = kimix_chat_state::estimate_conversation_tokens(&with_image);
...
fn durable_fallback_roundtrips_child_cwd_and_worktree() {
    ...
    let meta = SubagentMeta {
        ...
        worktree_path: Some("/tmp/kimix-wt/sa-dur".into()),
        snapshot_ref: None,
        effective_model_id: Some("kimix-3".into()),
    };
    ...
    assert_eq!(loaded.worktree_path.as_deref(), Some("/tmp/kimix-wt/sa-dur"));
...
fn durable_fallback_rejects_running_status() {
    let dir = std::env::temp_dir()
        .join("kimix-test-durable-status")
        .join(uuid::Uuid::now_v7().to_string());
...
fn drain_cancelled_finish_broadcasts(
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        ...
        assert_eq!(args.request.method.as_ref(), "kimix/session_notification");
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
        kimix_subagent_resolution::validate_resume_identity("general-purpose", None, &
        source,).is_ok()
    );
    assert_eq!(
        source.model_id.as_deref(), Some("kimix-3"),
...
fn durable_meta_roundtrips_effective_model_id() {
    let dir = std::env::temp_dir()
        .join("kimix-test-model-roundtrip")
        .join(uuid::Uuid::now_v7().to_string());
    let _ = std::fs::create_dir_all(&dir);
    let meta = SubagentMeta {
    ...
    let loaded: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(
        loaded.effective_model_id.as_deref(), Some("kimix-3"),
...
fn resume_model_pinning_overrides_default_resolution() {
    let source_model = Some("kimix-3".to_string());
    let resolved_model = "kimix-light";
...
async fn read_parent_sampling_config_keeps_auto_catalog_id_with_routing_slug() {
    ...
    models.insert("auto".to_string(), test_model_entry("kimix-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "kimix-4.5", "composer-2-fast", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "kimix-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
#[tokio::test]
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    let mut models = indexmap::IndexMap::new();
    models.insert("kimix-4.5".to_string(), test_model_entry("kimix-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "kimix-4.5", "auto", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "kimix-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
...
async fn read_parent_sampling_config_resolves_backend_search_from_catalog() {
    let mut entry = test_model_entry("kimix-4.5");
    entry.info.supports_backend_search = true;
    let mut models = indexmap::IndexMap::new();
    models.insert("auto".to_string(), entry);
    let mut ctx = ctx_with_parent_chat_state("auto", "kimix-4.5", "auto", models);
...
async fn runtime_override_wins_over_subagents_models_pin_in_precedence_path() {
    use kimix_agent::config::ModelOverride;
    let build_ctx = || {
        let mut models = indexmap::IndexMap::new();
...
async fn resolve_subagent_inherits_parent_model_without_pins() {
    use kimix_agent::config::ModelOverride;
    for parent_model in ["kimix-4.5", "composer-2-fast", "my-custom-byok-model"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
...
/// (`Kimix-4.5`) and a custom parent (`composer-2-fast`)
...
async fn resolve_subagent_config_override_pin_applies_for_any_parent() {
    use kimix_agent::config::ModelOverride;
    for parent_model in ["kimix-4.5", "composer-2-fast"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
        ctx.sampling_config.model = parent_model.to_string();
        ctx.model_id = acp::ModelId::new(parent_model);
        ctx.available_models
...
async fn resolve_subagent_agent_definition_pin_applies_for_light_parent() {
    use kimix_agent::config::ModelOverride;
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.sampling_config.model = "kimix-4.5".to_string();
    ctx.model_id = acp::ModelId::new("kimix-4.5");
    ctx.available_models
        .insert("pinned-model".to_string(), test_model_entry("pinned-model"));
...
async fn resolve_subagent_config_override_unknown_model_falls_through_to_inherit() {
    ...
    let (config, model_id) = resolve_subagent_sampling_config(
        ...
        .await;
    assert_eq!(config.model, "kimix-4.5");
    assert_eq!(model_id.0.as_ref(), "kimix-4.5");
}
...
fn non_cursor_persona_injected_as_system_reminder() {
    use kimix_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(& result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &kimix_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn make_test_skill(
    ...
    kimix_tools::implementations::skills::types::SkillInfo {
        ...
        scope: kimix_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
