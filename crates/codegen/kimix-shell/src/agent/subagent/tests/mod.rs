...
async fn subagent_inherits_session_cli_overrides() {
    use kimix_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    ...
    const PIN: &str = kimix_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let broadcast_id = match gateway_rx.try_recv().expect("broadcast must fire") {
        kimix_acp_lib::AcpClientMessage::ExtNotification(args) => {
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
        resume_worktree_action(true, Some("refs/kimix/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
    assert_eq!(
        resume_worktree_action(false, Some("refs/kimix/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn kimix_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let auto_wake = kimix_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
...
fn dummy_tracker(
    ...
    let cwd = kimix_paths::AbsPathBuf::new(PathBuf::from("/tmp")).unwrap();
    let fs: Arc<dyn kimix_workspace::file_system::AsyncFileSystem> = Arc::new(
        kimix_workspace::file_system::LocalFs::new(PathBuf::from("/tmp")),
    );
    ...
    let handle = SessionHandle {
        ...
        max_turns: None,
        hunk_tracker_handle: kimix_hunk_tracker::HunkTrackerHandle::noop(),
...
fn explicit_override_takes_precedence_over_role() {
    let overrides = SubagentRuntimeOverrides {
        ...
        capability_mode: Some(kimix_tool_types::SubagentCapabilityMode::All),
        ..Default::default()
    };
    let role = kimix_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
        resolved.capability_mode, Some(kimix_tool_types::SubagentCapabilityMode::All)
    );
}
#[test]
fn role_default_used_when_no_explicit_override() {
    ...
    assert_eq!(
        resolved.capability_mode, Some(kimix_tool_types::SubagentCapabilityMode::ReadOnly)
    );
}
#[test]
...
fn partial_override_fills_from_role() {
    ...
    assert_eq!(resolved.model.as_deref(), Some("explicit-model"));
    assert_eq!(
        resolved.capability_mode, Some(kimix_tool_types::SubagentCapabilityMode::Execute)
    );
}
#[test]
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = kimix_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = kimix_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(kimix_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn resume_vs_fork_helper_shapes_differ() {
    ...
    assert!(
        ...
        kimix_sampling_types::conversation::ContentPart::Text { text } if text
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use kimix_sampling_types::conversation::{
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(kimix_sampling_types::synthesized_reasoning_item("thinking",)),
        ConversationItem::assistant("ack"),
    ];
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use kimix_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
...
async fn bootstrap_fork_live_parent_chat_state_is_forked_with_marker() {
    ...
    let req = bootstrap_test_request(true);
    let mut ctx = ctx_with_toggle(HashMap::new());
    let chat = spawn_test_parent_chat_state("kimix-4.5");
...
fn subagent_await_budget_default_and_override() {
    unsafe { std::env::remove_var("KIMIX_SUBAGENT_AWAIT_BUDGET_MS") };
    assert_eq!(SUBAGENT_AWAIT_BUDGET, std::time::Duration::from_secs(600));
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("KIMIX_SUBAGENT_AWAIT_BUDGET_MS", "1500") };
    assert_eq!(subagent_await_budget(), std::time::Duration::from_millis(1500));
    unsafe { std::env::set_var("KIMIX_SUBAGENT_AWAIT_BUDGET_MS", "0") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("KIMIX_SUBAGENT_AWAIT_BUDGET_MS", "not-a-number") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::remove_var("KIMIX_SUBAGENT_AWAIT_BUDGET_MS") };
}
#[test]
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("Kimix:read_file");
...
/// Regression: on the DEFAULT Kimix host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    ...
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::KimixPlan.as_ref().to_string(),
    );
...
async fn background_unknown_type_emits_subagent_finished_notification() {
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        if let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            ...
            assert_eq!(req.method.as_ref(), "kimix/session_notification");
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    ...
    kimix_fast_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    kimix_fast_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn subagent_auth_type_rule() {
    ...
    let byok = byok_model_entry("kimix-byok");
    let plain = test_model_entry("kimix-plain");
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    models.insert("kimix-3".to_string(), test_model_entry("kimix-3-2025-02-15"));
    assert!(
        super::handle_request::task_model_override_error(Some("kimix-3"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
        "key lookup should succeed"
    );
    assert!(
        super::handle_request::task_model_override_error(Some("kimix-3-2025-02-15"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(! error.contains("kimix models"));
    }
    assert!(
...
fn test_sampling_config(model_slug: &str) -> kimix_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> kimix_chat_state::ChatStateHandle {
    let (mock, _persistence_rx) = kimix_chat_state::MockChatPersistence::new();
    let (event_tx, _event_rx) = mpsc::unbounded_channel();
    let token = tokio_util::sync::CancellationToken::new();
    kimix_chat_state::ChatStateActor::spawn(
        vec![],
