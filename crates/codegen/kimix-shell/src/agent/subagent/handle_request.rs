...
pub(crate) async fn handle_subagent_request(
    ...
    {
        use kimix_tool_types::SubagentIsolationMode;
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(kimix_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = kimix_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != kimix_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != kimix_tool_types::SubagentIsolationMode::None
    {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(
            ...
            Err(e) => {
                ...
                std::env::temp_dir().join("kimix-subagent-worktrees").join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: kimix_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
                let mut builder = kimix_fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        kimix_fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
                    .creation_mode(creation_mode)
                    .worktree_kind(kimix_fast_worktree::WorktreeKind::Subagent)
    ...
    {
        use kimix_tools::implementations::kimix::task::MAX_SUBAGENT_DEPTH;
    ...
    {} else if context_source != InitialContextSource::Resumed && !verbatim_mirror_fork
        && let Some(ref pi) = effective_runtime.persona_instructions {
            let reminder = kimix_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = kimix_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            kimix_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = kimix_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = kimix_chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<kimix_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = kimix_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        let memory_tools: Vec<kimix_tools::registry::types::ToolConfig> = vec![
            (& kimix::ReadFileTool).into(), (& kimix::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = kimix_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == kimix_hooks::event::HookEventName::Stop {
                            s.event = kimix_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    kimix_agent::config::McpServerRef::Named(name) => {
                    ...
                    kimix_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            kimix_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    let _mcp_owned_count = agent_mcp_servers.len() as u32;
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    kimix_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                kimix_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    kimix_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
            ...
            kimix_agent::prompt::context::PromptAudience::Subagent,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/kimix/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
