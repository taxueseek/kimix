...
/// Parse a `Kimix/queue/{remove,reorder,clear,edit,interject}` ext-notification's
...
pub(super) fn parse_queue_edit_command(
    ...
    match method {
        ...
        "kimix/queue/clear" => Some(SessionCommand::ClearQueue { owner }),
...
mod tests {
    ...
    /// Each `Kimix/queue/*` ext-notification maps to the
    ...
    fn parse_queue_edit_command_maps_each_method() {
        ...
        match parse_queue_edit_command("kimix/queue/remove", &p, Some("kimix-tui".into())) {
            Some(SessionCommand::RemoveQueuedPrompt {
                id,
                expected_version,
                owner,
                ...
                assert_eq!(owner.as_deref(), Some("kimix-tui"));
            }
        ...
        match parse_queue_edit_command("kimix/queue/remove", &p, None) {
            Some(SessionCommand::RemoveQueuedPrompt {
                expected_version, ..
        ...
        match parse_queue_edit_command("kimix/queue/reorder", &p, None) {
            Some(SessionCommand::ReorderQueue { ordered_ids }) => {
        ...
        match parse_queue_edit_command("kimix/queue/edit", &p, Some("kimix-vscode".into())) {
            Some(SessionCommand::EditQueuedPrompt {
                id,
                new_text,
                editor,
                ...
                assert_eq!(editor.as_deref(), Some("kimix-vscode"));
            }
        ...
        match parse_queue_edit_command("kimix/queue/interject", &p, Some("kimix-tui".into())) {
            Some(SessionCommand::InterjectQueuedPrompt {
                id,
                expected_version,
                owner,
        ...
        match parse_queue_edit_command("kimix/queue/interject", &p, None) {
        ...
        assert!(
            parse_queue_edit_command("kimix/queue/interject", &serde_json::json!({}), None)
                .is_none()
        );

        // unknown method → None.
        assert!(
            parse_queue_edit_command("kimix/queue/bogus", &serde_json::json!({}), None).is_none()
        );
        // remove without id → None (can't target an entry).
        assert!(
            parse_queue_edit_command("kimix/queue/remove", &serde_json::json!({}), None).is_none()
        );
    }
}
