...
pub(crate) fn log_model_override_warnings(warnings: &[ModelOverrideWarning]) {
    ...
    if !warnings.is_empty() {
        tracing::warn!(
            ...
            "model_override: parsed with warnings; run `kimix inspect` for details"
        );
    }
}
...
mod tests {
    ...
    fn duplicate_compactions_keys_keeps_model() {
        ...
        let model = cfg
            .config_models
            .get("kimix-4.5")
            .expect("kimix-4.5 must remain in catalog");
        assert_eq!(
        ...
        assert!(resolved.contains_key("kimix-4.5"));
    }
    ...
    fn legacy_alias_alone_parses_without_warning() {
        ...
        let model = cfg.config_models.get("kimix-4.5").unwrap();
        assert_eq!(
            model.compactions_remaining,
    ...
    fn invalid_reasoning_effort_skips_field_keeps_model() {
        ...
        let model = cfg
            ...
            .get("kimix-4.5")
            .expect("kimix-4.5 must remain in catalog");
        assert_eq!(model.model.as_deref(), Some("kimix-4.5"));
    ...
    fn unknown_field_warns_but_keeps_known_fields() {
        ...
        let entry = models.get("kimix-4.5").unwrap();
        assert_eq!(entry.model.as_deref(), Some("kimix-4.5"));
        assert_eq!(
        ...
        assert_eq!(
            warnings,
            vec![ModelOverrideWarning {
                model_key: Some("kimix-4.5".to_owned()),
    ...
    fn non_table_model_section_warns_and_is_ignored() {
        let (models, warnings) = parse_raw(r#"model = "kimix-4""#);
    ...
    fn fully_populated_override() -> ConfigModelOverride {
        ConfigModelOverride {
            ...
            capabilities: vec![kimix_models::ModelCapability::Thinking],
            supports_backend_search: Some(false),
            auth_source: Some("grok_session".into()),
