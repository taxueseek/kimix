...
pub mod config;
pub mod hooks;
pub mod kimix_auth_credentials;

// The foundation utilities live in `Kimix-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `kimix_shell::util::…` users compile
// unchanged.
pub use kimix_shell_base::util::*;
