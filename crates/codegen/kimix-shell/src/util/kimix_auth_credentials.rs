...
use std::sync::Arc;
/// Credentials for authenticating with kimix backend services.
///
...
#[derive(Clone)]
pub struct KimixAuthCredentials {
...
impl std::fmt::Debug for KimixAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("KimixAuthCredentials")
            .field(
                "user_token",
...
impl KimixAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your KIMIX_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `kimix login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> KimixAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> KimixAuthCredentials {
...
impl kimix_auth::HttpAuth for KimixAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        KimixAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = KimixAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = KimixAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = KimixAuthCredentials::new(Some("static-token".into()));
