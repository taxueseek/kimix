...
use kimix_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .Kimix/hooks/ directories are still scanned (they hold
    ...
    // user_kimix_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .Kimix.
    ...
    if let Some(ref kimix) = kimix {
        global.push(kimix.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = kimix
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".kimix").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    kimix_hooks::discovery::HookRegistry,
    Vec<kimix_hooks::error::HookError>,
) {
