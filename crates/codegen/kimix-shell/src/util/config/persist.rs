...
/// Process-wide write lock for `~/.kimix/config.toml`.
///
...
mod tests {
    ...
    fn merge_section_updates_modeled_fields_preserving_unmodeled() {
        ...
        ui.insert(
            ...
            TomlValue::String("kimixday".into()),
        );
        table.insert("ui".into(), TomlValue::Table(ui));
...
"#;
        ...
        assert_eq!(cfg.ui.auto_light_theme.as_deref(), Some("kimixday"));
...
"#;
        ...
        cfg.models.default = Some("kimix-4".to_string());
    ...
    fn models_config_serializes_only_some_fields() {
        let m = crate::agent::config::ModelsConfig {
            default: Some("kimix-3".to_string()),
            ..Default::default()
        };
        ...
        if let TomlValue::Table(t) = v {
            ...
            assert_eq!(t.get("default").and_then(|x| x.as_str()), Some("kimix-3"));
        } else {
    ...
    fn merge_section_models_only_updates_set_fields_preserves_others() {
        ...
        let cfg = crate::agent::config::ModelsConfig {
            default: Some("kimix-new".to_string()),
            ..Default::default()
        };
        merge_section(&mut table, "models", &cfg);
        let m = table.get("models").unwrap().as_table().unwrap();
        assert_eq!(m.get("default").and_then(|v| v.as_str()), Some("kimix-new"));
        assert_eq!(
    ...
    fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
        ...
        let root: TomlValue = toml::from_str(original).unwrap();
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("kimix-persisted".to_string());
        ...
        assert_eq!(reloaded.models.default.as_deref(), Some("kimix-persisted"));
        let cfg2 = crate::agent::config::Config::new_from_toml_cfg(&reloaded_root)
            .expect("new_from_toml_cfg");
        assert_eq!(cfg2.models.default.as_deref(), Some("kimix-persisted"));
    }
    ...
    mod resolve_auto_compact {
        ...
        const TEST_MODEL: &str = "kimix-4.5";
        const OTHER_MODEL: &str = "kimix-4.3";

        /// Serialize tests that mutate `KIMIX_AUTO_COMPACT_THRESHOLD_PERCENT`.
    ...
    fn settings_helpers_target_correct_ui_fields() {
        ...
        let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("kimixday".to_string()));
        assert_eq!(cfg.ui.auto_light_theme, Some("kimixday".to_string()));
