...
/// | env          | `KIMIX_MCP_LIVENESS_WATCHERS` (handled by `BoolFlag::env`)       |
/// | config       | `[features] mcp_liveness_watchers` in `~/.kimix/config.toml`     |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `KIMIX_MCP_AUTO_RESTART` (handled by `BoolFlag::env`)            |
/// | config       | `[features] mcp_auto_restart` in `~/.kimix/config.toml`          |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `KIMIX_MCP_PUSH_SERVER_STATUS` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_push_server_status` in `~/.kimix/config.toml`    |
...
/// | cli          | (none — no CLI flag)                                                |
/// | env          | `KIMIX_MCP_RECURSIVE_CONFIG_WATCH` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_recursive_config_watch` in `~/.kimix/config.toml`    |
...
/// Kept in sync with `kimix_mcp::servers`'s standalone fallback.
...
/// Env override for the MCP startup timeout, in seconds (Kimix-native).
const ENV_MCP_STARTUP_TIMEOUT_SECS: &str = "KIMIX_MCP_STARTUP_TIMEOUT_SECS";
...
/// `KIMIX_MCP_STARTUP_TIMEOUT_SECS` secs) > effective `config.toml [mcp]` >
...
/// `KIMIX_MCP_STARTUP_TIMEOUT_SECS` (secs). Unparseable/zero values are ignored.
...
pub const DEFAULT_MAX_MCP_OUTPUT_BYTES: usize = kimix_tools::MCP_MAX_OUTPUT_BYTES;
...
pub fn cache_remote_max_mcp_output_bytes(remote: Option<u64>) {
    kimix_tools::set_mcp_max_output_bytes(resolve_max_mcp_output_bytes(remote));
}
...
///   2. env `KIMIX_MAX_MCP_OUTPUT_BYTES` / `MAX_MCP_OUTPUT_BYTES`
///      (Kimix-native wins when both set)
...
pub fn resolve_max_mcp_output_bytes(remote: Option<u64>) -> usize {
    ...
    resolve_max_mcp_output_bytes_precedence(
        requirements,
        kimix_tools::mcp_max_output_bytes_from_env(),
...
/// `.Kimix/config.toml` chain (`cwd` → git root), deepest file wins.
///
...
fn project_max_mcp_output_bytes(cwd: &std::path::Path) -> Option<usize> {
    ...
    for config_path in crate::config::find_project_configs(cwd) {
        if let Ok(toml_val) = kimix_config::load_config_file(&config_path)
...
pub fn resolve_max_mcp_output_bytes_for_cwd(cwd: &std::path::Path) -> Option<usize> {
    let requirements = crate::config::load_merged_requirements()
        .as_ref()
        .and_then(max_mcp_output_bytes_from_toml);
    if requirements.is_some() || kimix_tools::mcp_max_output_bytes_from_env().is_some() {
        return None;
    }
...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        std::fs::create_dir_all(sub.join(".kimix")).unwrap();
        std::fs::create_dir_all(root.join(".kimix")).unwrap();
        std::fs::write(
            root.join(".kimix/config.toml"),
        ...
        std::fs::write(sub.join(".kimix/config.toml"), "[ui]\nvim_mode = true\n").unwrap();
        assert_eq!(super::project_max_mcp_output_bytes(&sub), Some(30_000));

        // No .Kimix files with the key anywhere → None.
        std::fs::remove_file(root.join(".kimix/config.toml")).unwrap();
