...
/// [`kimix_config::env_bool`] parser (`KIMIX_TOOLS_FIND_BFS` /
/// `KIMIX_TOOLS_GREP_UGREP`, plus the `KIMIX_FIND_BFS` / `KIMIX_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.kimix/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`kimix_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = kimix_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = kimix_config::env_bool(primary).or_else(|| kimix_config::env_bool(alias));
    ...
    (
        resolve("KIMIX_TOOLS_FIND_BFS", "KIMIX_FIND_BFS", "find_bfs"),
        resolve("KIMIX_TOOLS_GREP_UGREP", "KIMIX_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_PERSISTENT_SHELL: &str = "KIMIX_PERSISTENT_SHELL";
...
mod persistent_local_shell_tests {
    ...
    // KIMIX_PERSISTENT_SHELL is process-global (the documented kill-switch a dev
...
/// the shared [`kimix_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "KIMIX_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`KIMIX_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            kimix_tools::implementations::kimix::ask_user_question::RESPONSE_TIMEOUT.as_secs(),
        )
}

/// Resolve `[toolset.ask_user_question] timeout_secs` (positive seconds).
///
/// Precedence: requirements > env (`KIMIX_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        kimix_tools::implementations::kimix::ask_user_question::response_timeout_env_secs(),
...
) -> kimix_tools::implementations::kimix::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
mod ask_user_question_timeout_tests {
    ...
    use kimix_tools::implementations::kimix::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = kimix_tools::implementations::kimix::ask_user_question::RESPONSE_TIMEOUT.as_secs();
...
mod tests {
    ...
    // Assumes KIMIX_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
