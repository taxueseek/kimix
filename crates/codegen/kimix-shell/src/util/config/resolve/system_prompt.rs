pub const ENV_SYSTEM_PROMPT_LABEL: &str = "KIMIX_SYSTEM_PROMPT_LABEL";

pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = kimix_agent::DEFAULT_SYSTEM_PROMPT_LABEL;

/// Resolve system-prompt identity label.
/// Precedence: env → config per-model → `[agent]` → GB per-model → GB global →
/// `"Kimix"`. Empty/whitespace falls through.
///
...
mod system_prompt_label_tests {
    ...
    /// Serialize access to `KIMIX_SYSTEM_PROMPT_LABEL` and clear it for tier tests.
