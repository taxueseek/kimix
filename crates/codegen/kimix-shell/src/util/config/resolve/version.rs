...
///
/// Reads `stable_version` from `~/.kimix/version.json` (written by the
...
///
/// This is a lightweight duplicate of `kimix_update::channel_name()` for
/// use in `Kimix-shell` which cannot depend on `Kimix-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    use std::sync::OnceLock;
    static NAME: OnceLock<Option<&'static str>> = OnceLock::new();
    *NAME.get_or_init(|| {
        let version_path = crate::util::kimix_home::kimix_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(kimix_version::VERSION).ok()?;
