...
pub(crate) const ENV_AUTO_PERMISSION_MODE: &str = "KIMIX_AUTO_PERMISSION_MODE";

/// Crate-wide serialization lock for tests that mutate
/// `KIMIX_AUTO_PERMISSION_MODE`. Every test reading the gate (here and in
...
/// two can't drift. Precedence: requirement > env (`KIMIX_AUTO_PERMISSION_MODE`)
...
///
/// requirements > env (`KIMIX_AUTO_PERMISSION_MODE`) > `[auto_mode] enabled` in
...
pub fn auto_mode_classifier_defaults(
    ...
    kimix_workspace::permission::ClassifierPromptType,
    Option<kimix_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(kimix_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(kimix_sampling_types::ReasoningEffort::Low)
    });
...
mod auto_permission_mode_gate_tests {
    ...
    // `KIMIX_AUTO_PERMISSION_MODE` is process-global; serialize every test that
