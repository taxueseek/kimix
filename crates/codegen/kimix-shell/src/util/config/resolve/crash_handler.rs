...
pub(crate) const ENV_CRASH_HANDLER: &str = "KIMIX_CRASH_HANDLER";
...
/// Precedence: requirements > env (`KIMIX_CRASH_HANDLER`) >
...
/// Merge system-managed policy (`/etc/Kimix`) under home `managed_config.toml`
...
fn load_managed_toml_layers() -> Option<TomlValue> {
    ...
    match (system, managed) {
        ...
        (None, Some(m)) => Some(m),
        (Some(mut s), Some(m)) => {
            kimix_config::deep_merge_toml(&mut s, &m);
            Some(s)
        }
    }
}
...
mod crash_handler_gate_tests {
    ...
    // `KIMIX_CRASH_HANDLER` is process-global; serialize and force it unset at
