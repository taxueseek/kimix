...
/// `KIMIX_TIPS_OVERRIDE` env var overrides everything (debug builds only).
...
pub fn resolve_tips(
    ...
    if let Ok(raw) = std::env::var("KIMIX_TIPS_OVERRIDE") {
...
pub fn resolve_tips_from_disk(
    ...
    crate::util::tips::pick_and_advance(&all, kimix_home)
}
