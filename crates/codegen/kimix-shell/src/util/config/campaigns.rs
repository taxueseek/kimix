...
pub fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_kimix_home() else {
        return;
    };
...
fn dismiss_campaign_ids_at(
    ...
    // several Kimix processes share `$KIMIX_SHARE_DIR`; the in-process mutex alone would
...
/// `KIMIX_CAMPAIGNS_OVERRIDE` JSON array replaces all sources (`[]` = none; beats
...
pub fn campaigns_override() -> Option<Vec<CampaignEntry>> {
    let json = std::env::var("KIMIX_CAMPAIGNS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<CampaignOverride>>(&json) {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "invalid KIMIX_CAMPAIGNS_OVERRIDE JSON; suppressing all campaigns");
            Some(Vec::new())
        }
    }
}
...
/// The single campaign-resolution path: `KIMIX_CAMPAIGNS_OVERRIDE` (replaces all
...
/// `KIMIX_CAMPAIGNS_OVERRIDE`. For one-shot CLI entrypoints that never fetch
...
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<kimix_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    /// `KIMIX_CAMPAIGNS_OVERRIDE` applies despite the kill switch; without it the
    ...
    fn override_beats_kill_switch() {
        ...
        let _env = EnvGuard::unset("KIMIX_CAMPAIGNS_OVERRIDE");
        let active = resolve_active_campaigns_from_layers(&layers, &base, &[], &HashSet::new());
        assert!(
            active.is_empty(),
    ...
    /// Invalid `KIMIX_CAMPAIGNS_OVERRIDE` JSON fails toward *no campaigns*: the
    ...
    fn invalid_override_json_suppresses_all_campaigns() {
        let _env = EnvGuard::set("KIMIX_CAMPAIGNS_OVERRIDE", "{ not json");
    ...
    /// made while `KIMIX_CAMPAIGNS=0` must still record the dismissal, or a
    /// later re-enabled campaign would override the user's explicit choice.
    #[test]
    #[serial]
    fn dismiss_resolution_ignores_kill_switch() {
        let _over = EnvGuard::unset("KIMIX_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::set("KIMIX_CAMPAIGNS", "0");
    ...
    /// `KIMIX_CAMPAIGNS_OVERRIDE="[]"` replaces all sources with nothing — even
    /// layer + remote campaigns resolve to empty.
    #[test]
    #[serial]
    fn override_empty_means_none() {
        let _env = EnvGuard::set("KIMIX_CAMPAIGNS_OVERRIDE", "[]");
