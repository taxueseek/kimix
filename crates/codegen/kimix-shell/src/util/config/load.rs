...
/// 1. Nearest project `.Kimix/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.kimix/config.toml` `[permission]` section
///
...
"#;
            ...
            && let Some(TomlValue::Table(models)) = table.get("models")
        {
            let default = models
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            assert_eq!(default.as_deref(), Some("kimix-code-fast-1"));
        } else {
