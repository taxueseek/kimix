...
use kimix_tools::types::compat::{CompatConfig, CompatConfigToml};
...
pub use kimix_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `Kimix-config-types` (config
// dependency inversion); re-exported so `crate::util::config::*` paths keep working.
pub use kimix_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
/// Walks from cwd up to the git repo root checking `.Kimix/config.toml` at each level.
/// Project-scoped `.Kimix/config.toml` entries override global `~/.kimix/config.toml`
...
/// `.Kimix/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.Kimix/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.kimix/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.Kimix/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .kimix/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.kimix/config.toml`.
...
/// Upsert an MCP server entry in `~/.kimix/config.toml`.
///
...
/// config file, e.g. a project-scoped `.Kimix/config.toml`.
...
/// Delete an MCP server entry from `~/.kimix/config.toml`.
///
...
/// config file, e.g. a project-scoped `.Kimix/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = kimix_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.Kimix/config.toml` files.
...
// `.mcp.json` discovery moved to `Kimix-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use kimix_workspace::project_config::{
...
pub const MCP_CONFIG_FILES_ENV: &str = "KIMIX_MCP_CONFIG_FILES";
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.Kimix/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::kimix_home::kimix_home().join("config.toml")
}

/// Path to the user-level config file (`~/.kimix/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.Kimix/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".kimix").join("config.toml")
}
...
mod tests {
    ...
    /// var via the kimix_env guard (process-global state).
    #[test]
    #[serial_test::serial(kimix_mcp_config_files_env)]
    fn cli_mcp_config_files_load_in_order_with_later_files_winning() {
        ...
        let _guard =
            kimix_env::EnvVarGuard::set(super::MCP_CONFIG_FILES_ENV, joined.to_str().unwrap());
    ...
    fn cli_mcp_config_files_absent_env_is_empty() {
        let _guard = kimix_env::EnvVarGuard::remove(super::MCP_CONFIG_FILES_ENV);
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.kimix/skills", "~/.kimix/skills/special/SKILL.md"]
ignore = ["~/.kimix/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.kimix/skills", "~/.kimix/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.kimix/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${KIMIX_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
