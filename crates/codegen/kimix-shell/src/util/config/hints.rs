...
/// env master `KIMIX_CONTEXTUAL_HINTS` (all-on/off) > user config
...
pub fn resolve_contextual_hints(
    ...
    let resolve_tip = |user: Option<bool>, feature_flag: Option<bool>| -> bool {
        BoolFlag::env("KIMIX_CONTEXTUAL_HINTS")
            .config(user)
...
fn merge_hints_config_layers(
    ...
    if let Some(m) = managed {
        kimix_config::deep_merge_toml(&mut merged, m);
    }
    if let Some(u) = user {
        kimix_config::deep_merge_toml(&mut merged, u);
    }
    if let Some(r) = requirements {
        kimix_config::deep_merge_toml(&mut merged, r);
    }
    merged
}
...
mod tests {
    ...
    const ENV_CONTEXTUAL_HINTS: &str = "KIMIX_CONTEXTUAL_HINTS";

    // `KIMIX_CONTEXTUAL_HINTS` is process-global; serialize the tests reading it
