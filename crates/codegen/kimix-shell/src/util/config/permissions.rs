...
pub use kimix_config_types::PermissionMode;
...
/// `Kimix-workspace`.
use kimix_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    fn effective_auto_for_launch_cli_auto_not_yolo() {
        ...
        unsafe { std::env::set_var("KIMIX_AUTO_PERMISSION_MODE", "1") };
        ...
        unsafe { std::env::remove_var("KIMIX_AUTO_PERMISSION_MODE") };
    }
    ...
    /// With the gate forced OFF (`KIMIX_AUTO_PERMISSION_MODE=0`), explicit
    ...
    fn effective_auto_for_launch_inert_when_gate_off() {
        let _g = crate::util::config::resolve::AUTO_PERMISSION_MODE_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        unsafe { std::env::set_var("KIMIX_AUTO_PERMISSION_MODE", "0") };
        assert!(
    ...
    // definition in `kimix_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = kimix_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
