...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.kimix/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.Kimix/config.toml`,
    ...
    /// (`<cwd>/.Kimix/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// Kimix process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.Kimix/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.kimix/models_cache.json` was rewritten on disk (possibly by another
    ...
    ModelsCacheChanged,
    /// Updated UI settings — agent broadcasts `Kimix/config_changed` to IPC clients.
    Ui {
        theme: Option<String>,
        yolo: bool,
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.kimix_home.join("auth.json");
            ...
            // same tick (e.g. `~/.kimix/config.toml` AND
    ...
    fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.kimix_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.kimix/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.Kimix/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.Kimix/config.toml` | `<cwd>`           |
...
/// for `.Kimix/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.kimix/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `kimix inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> kimix_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> kimix_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    /// The hash must reflect ancestor `.Kimix/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".kimix").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .kimix/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_empty() {
        ...
        assert_eq!(skills, kimix_agent::prompt::skills::SkillsConfig::default());
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.kimix/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.kimix/skills".to_string()]);
...
theme = "dark"
yolo = true
fork_secondary_model = "kimix-4.5"
"#,
        )
        .unwrap();
        let (theme, yolo, fork) = extract_ui_fields(&config);
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("kimix-4.5"));
    }
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.Kimix/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.kimix/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.kimix/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.kimix/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
