...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.kimix/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** Kimix process
    /// sharing the same `~/.kimix` (the writer may also be this process;
...
/// Watches `~/.kimix/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.Kimix/config.toml`, `.mcp.json`,
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.Kimix/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let kimix_home_buf = kimix_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(kimix_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(kimix_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(kimix_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(kimix_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %kimix_home.display(),
                    error = %e,
                    "failed to watch kimix home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.Kimix/config.toml` so the loop above
        // watches `<cwd>/.Kimix/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.Kimix/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.Kimix/config.toml` trigger a
    ...
    /// `<cwd>/.Kimix/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.Kimix/` does not yet
/// exist at session-open time, the `.Kimix/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.Kimix/` followed by a
/// write to `<cwd>/.Kimix/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let kimix_dir = cwd.join(".kimix");
    if let Err(e) = debouncer
        .watcher()
        .watch(&kimix_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .kimix directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let kimix_dir = cwd.join(".kimix");
    if let Err(e) = debouncer.watcher().unwatch(&kimix_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .kimix directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.Kimix/` hasn't
...
/// Watches skill directories (`~/.kimix/skills/`, `<repo>/.Kimix/skills/`, etc.)
...
/// recursively: `kimix_home` (`~/.kimix`, or `$KIMIX_SHARE_DIR`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.kimix` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, kimix_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, kimix_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".kimix", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, kimix_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(kimix_home) {
        return true;
    }
...
/// `~/.kimix` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](kimix_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let kimix_home = kimix_tools::util::kimix_home::kimix_home();
        ...
        let dirs_to_watch = kimix_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &kimix_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &kimix_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only kimix_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_kimix_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let kimix_home = home.join(".kimix");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &kimix_home, Some(home));

        // kimix_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&kimix_home));
        ...
        assert!(!g(&home.join("repo").join(".kimix")));
    }

    /// Regression for the ~/.kimix inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.kimix/worktrees/`) must not
    ...
    /// A write to `<kimix_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another Kimix process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.Kimix/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let kimix_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_kimix = cwd.path().join(".kimix");
        fs::create_dir_all(&project_kimix).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_kimix.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_kimix.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.kimix/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.Kimix/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_kimix = new_cwd.path().join(".kimix");
        fs::create_dir_all(&project_kimix).unwrap();
        fs::write(project_kimix.join("config.toml"), "").unwrap();
