...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "KIMIX_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$KIMIX_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${KIMIX_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$KIMIX_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$KIMIX_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the KIMIX_MEMORY env var.
...
/// Run `f` with KIMIX_MEMORY explicitly unset.
fn without_kimix_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("KIMIX_MEMORY", None, f)
}
/// Run `f` with KIMIX_MEMORY set to a specific value.
fn with_kimix_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("KIMIX_MEMORY", Some(value), f)
}
#[test]
fn memory_config_default_disabled() {
    without_kimix_memory(|| {
...
fn memory_config_env_var_zero_does_not_enable() {
    with_kimix_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "KIMIX_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_kimix_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "KIMIX_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_kimix_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "KIMIX_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_kimix_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "KIMIX_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_kimix_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override KIMIX_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_kimix_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, KIMIX_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_kimix_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override KIMIX_MEMORY=1");
        },
    );
}
#[test]
...
"#;
        ...
        assert_eq!(mem.flush.flush_model.as_deref(), Some("kimix-4"));
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$KIMIX_TEST_USER $KIMIX_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the KIMIX_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with KIMIX_SUBAGENTS explicitly unset.
fn without_kimix_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("KIMIX_SUBAGENTS", None, f)
}
/// Run `f` with KIMIX_SUBAGENTS set to a specific value.
fn with_kimix_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("KIMIX_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_kimix_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_kimix_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(! sa.enabled, "KIMIX_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_kimix_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(
                ! sa.enabled, "KIMIX_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_kimix_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override KIMIX_SUBAGENTS=0");
        },
    );
}
#[test]
fn subagents_config_models_parsed() {
    without_kimix_subagents(|| {
        ...
        assert_eq!(sa.models.get("explore").unwrap(), "kimix-3-fast");
        assert_eq!(sa.models.get("plan").unwrap(), "kimix-4.5");
    });
}
#[test]
    ...
    with_kimix_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(sa.enabled, "KIMIX_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "kimix-3-fast");
        },
    );
}
...
fn with_model_overrides_env_full<T>(
    ...
    with_env_var_opt(
        "KIMIX_SESSION_SUMMARY_MODEL",
        ss,
        || with_env_var_opt(
            "KIMIX_IMAGE_DESCRIPTION_MODEL",
            id,
            || with_env_var_opt("KIMIX_PROMPT_SUGGESTIONS_MODEL", ps, f),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_kimix() {
    with_model_overrides_env(
        None,
        None,
        || {
...
#[test]
fn model_overrides_default_session_summary_is_kimix() {
    with_model_overrides_env(
        None,
        None,
        || {
...
fn without_kimix_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    let _guard = TOOLS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("KIMIX_RESPECT_GITIGNORE", None, f)
}
fn with_kimix_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = TOOLS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("KIMIX_RESPECT_GITIGNORE", Some(value), f)
}
#[test]
fn tools_config_default_disabled() {
    without_kimix_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_kimix_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "KIMIX_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".kimix/prompts/impl.md"
        "#;
    ...
    assert_eq!(researcher.model.as_deref(), Some("kimix-3"));
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".kimix/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".kimix/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".kimix").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".kimix/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".kimix").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".kimix").join("bundled");
    std::fs::create_dir_all(workspace.join(".kimix").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".kimix").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".kimix").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".kimix").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".kimix/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".kimix/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".kimix/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".kimix/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".kimix/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".kimix/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
    ...
    std::fs::remove_file(home.join(".kimix/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".kimix/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.kimix/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
fn roles_coexist_with_models_and_toggle() {
    ...
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.enabled);
    assert_eq!(cfg.models.get("explore").map(| s | s.as_str()), Some("kimix-fast"));
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "KIMIX_CODE_BASE_URL",
        "KIMIX_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
coding_api_base_url = "https://cli-chat-proxy.kimix.com/v1"
...
"#,
    ...
    assert_eq!(
        ...
        "https://cli-chat-proxy.kimix.com/v1/deployment/config"
    );
...
#[test]
fn validate_hooks_path_rejects_outside_kimix_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
        msg.contains("must be under ~/.kimix/"),
        "should mention ~/.kimix/ restriction, got: {msg}"
    );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let kimix_home = crate::util::kimix_home::kimix_home();
    let traversal = format!("{}/../evil", kimix_home.display());
...
#[test]
fn validate_hooks_path_accepts_kimix_hooks_subdir() {
    let kimix_home = crate::util::kimix_home::kimix_home();
    let valid_path = kimix_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.kimix/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use kimix_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own Kimix yolo.
#[test]
...
fn simulate_release_build() -> kimix_test_support::EnvGuard {
    kimix_test_support::EnvGuard::set(kimix_version::TEST_VERSION_ENV, "0.0.0-sim")
}
...
/// verdicts untouched). KIMIX_SHARE_DIR-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores KIMIX_SHARE_DIR even on panic). No user-global
/// `$KIMIX_SHARE_DIR/config.toml` is seeded: `kimix_home()` is `OnceLock`-cached,
...
fn resolve_effective_plugins_config_gates_project_paths_on_folder_trust() {
    ...
    let kimix = repo.path().join(".kimix");
...
/// in `Kimix-agent`. An ABSOLUTE plugin path is used so the merged
...
/// KIMIX_SHARE_DIR-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use kimix_agent::plugins::{TrustStore, discover_plugins};
    use kimix_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("KIMIX_SHARE_DIR", home.path());
    let _flag = EnvGuard::unset("KIMIX_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. KIMIX_SHARE_DIR-isolated (empty
/// store); KIMIX_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let kimix = repo.path().join(".kimix");
    std::fs::create_dir_all(&kimix).unwrap();
    std::fs::write(kimix.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {
