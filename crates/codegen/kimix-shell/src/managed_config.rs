...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    for name in [
        "managed_config.toml",
        "requirements.toml",
        kimix_config::signed_policy::SIGNATURE_SIDECAR_FILE,
...
pub fn clear_orphan() {
    ...
    let home = crate::util::kimix_home::kimix_home();
...
/// files (TUI tick vs `kimix login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `kimix setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `KIMIX_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("KIMIX_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `Kimix-http` classifier.
...
/// Override with `KIMIX_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("KIMIX_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** KimixDeployment UUID (the id
...
/// Resolve deployment key from `KIMIX_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (Kimix setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("KIMIX_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.kimix/`, trying the
...
struct SyncOutcome {
    ...
    /// `kimix setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
/// read-only `kimix setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if kimix_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(kimix_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        kimix_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
...
/// Post-login hook for `kimix login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `kimix setup` could install config for.
...
/// `kimix setup`); ceasing to serve `fail_closed` rolls back.
pub fn managed_policy_gate() -> Result<(), String> {
    // Skip under the lib unit-test build only: `bootstrap` reaches this without a staged
    // `KIMIX_SHARE_DIR` and would flake on the dev machine's real marker/auth. The pure decision
...
/// self-healing class as deleting the user-writable files outright; /etc/Kimix and MDM are unaffected.
...
/// Outcome of the `kimix setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.kimix`.
    Installed,
...
/// Result of `kimix setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `kimix setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `kimix setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `kimix setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `kimix setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && kimix_config::signed_policy::verification_active()
...
/// Run the `kimix setup` sync for the current principal. The caller must check
