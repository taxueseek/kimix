//! `kimix mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = kimix_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.kimix/config.toml".to_string(),
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use kimix_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
/// `kimix mcp auth <name>` (kimi-cli `mcp auth` parity): start the named
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `kimix mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `kimix mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `kimix mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `kimix mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
