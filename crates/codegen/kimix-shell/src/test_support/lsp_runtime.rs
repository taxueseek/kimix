...
use kimix_tools::implementations::kimix::task::types::{SubagentRequest, SubagentResult};
use std::collections::HashMap;
use std::path::PathBuf;
use std::sync::Arc;
use tokio::sync::{mpsc, oneshot};
pub(crate) type GatewayOut = <acp::AgentSide as kimix_acp_lib::AcpSide>::OutMessage;
...
pub(crate) fn ctx_with_toggle(toggle: HashMap<String, bool>) -> SubagentSpawnContext {
    ...
    SubagentSpawnContext {
        ...
        fs: Arc::new(kimix_workspace::file_system::LocalFs::new(PathBuf::from(
            "/tmp",
        ))),
        ...
        user_max_effective_context_tokens: None,
        ...
        auth_manager: Arc::new(crate::auth::AuthManager::new(
            std::path::Path::new("/tmp/nonexistent-kimix-test"),
        ...
        parent_skills: None,
        parent_skills_config: kimix_agent::prompt::skills::SkillsConfig::default(),
        parent_compat: kimix_tools::types::compat::CompatConfig::default(),
        auto_wake_delivered: None,
        task_output_tool_name: kimix_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL
            .to_string(),
...
impl kimix_tools::implementations::lsp::LspBackend for DummyLspDispatch {
    ...
    async fn dispatch(
        &self,
        _input: &kimix_tools::implementations::lsp::LspToolInput,
    ) -> kimix_tools::implementations::lsp::LspToolResult {
        kimix_tools::implementations::lsp::LspToolResult {
            text: String::new(),
            is_error: false,
        }
    ...
    ) -> Option<kimix_tools::implementations::lsp::DiagnosticsSummary> {
        None
    }
    ...
    ) -> Vec<kimix_tools::implementations::lsp::FileDiagnosticEntry> {
        vec![]
    }
}
