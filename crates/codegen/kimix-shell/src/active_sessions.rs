//! Tracks open TUI sessions in `~/.kimix/active_sessions.json` for crash
...
pub fn try_unregister(session_id: &acp::SessionId) -> io::Result<bool> {
    try_unregister_in(&crate::util::kimix_home::kimix_home(), session_id)
}

/// Remove entries with dead PIDs and return them.
pub fn collect_crashed() -> io::Result<Vec<ActiveSession>> {
    collect_crashed_in(&crate::util::kimix_home::kimix_home())
}
