...
pub enum ClientMode {
    /// Stdio mode (Kimix agent stdio, Kimix -p) - uses local IPC.
...
pub struct ClientCapabilities {
    ...
    /// Whether this client has advertised `Kimix/codeNavigation.enabled`.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `kimix update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `kimix update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `kimix update` just installed; the leader uses
