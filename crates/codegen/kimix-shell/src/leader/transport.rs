...
//!   into `\\.\pipe\Kimix-leader-<hash>` so callers keep their path-based API.
//!
...
mod windows_impl {
    ...
    /// Deterministic leaf name (`Kimix-leader-<hash>`) for a filesystem path.
    ///
    ...
    fn pipe_leaf_name(path: &Path) -> std::ffi::OsString {
        ...
        std::ffi::OsString::from(format!("kimix-leader-{hash:016x}"))
    }
    ...
    mod tests {
        ...
        fn pipe_name_is_deterministic() {
            let a = path_to_pipe_name(Path::new("/tmp/kimix.sock"));
            let b = path_to_pipe_name(Path::new("/tmp/kimix.sock"));
            assert_eq!(a, b);
        }
        ...
        fn pipe_name_has_correct_prefix() {
            ...
            assert!(s.starts_with(r"\\.\pipe\kimix-leader-"), "got: {s}");
        }
        ...
        fn pipe_name_is_bounded() {
            ...
            // \\.\pipe\Kimix-leader- (20 chars) + 16 hex chars = 36 total
        ...
        async fn listener_is_ready_tracks_pipe_lifecycle() {
            ...
            let path =
                std::env::temp_dir().join(format!("kimix-ready-probe-{}.sock", std::process::id()));
