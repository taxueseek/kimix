...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "Kimix-tui", "Kimix-code-extension").
...
/// `params._meta["Kimix/leaderClientId"]`.
///
...
fn extract_target_client_id(json: &serde_json::Value) -> Option<ClientId> {
    ...
    params
        .get("_meta")
        .and_then(|m| m.get("kimix/leaderClientId"))
        .or_else(|| {
            params
                .get("params")
...
///
/// - `Kimix/sessions/changed` — roster delta; every open dashboard must stay
///   in sync.
/// - `Kimix/models/update` — the model catalog changed (config.toml
...
/// - `Kimix/mcp/servers_updated` — the MCP catalog resolved/changed (managed
...
///
/// (`Kimix/settings/update` stays non-broadcast — it carries auth/gate state.)
///
/// Matched via [`method_of`], NOT the raw top-level `method`: agent ext
/// notifications arrive `_`-prefixed on the wire (`_Kimix/sessions/changed`),
/// so a raw compare would miss the production form.
fn is_machine_wide_broadcast_notification(json: &serde_json::Value) -> bool {
    matches!(
        method_of(json),
        Some("kimix/sessions/changed" | "kimix/models/update" | "kimix/mcp/servers_updated")
    )
}
/// Whether a payload is the `Kimix/scheduled_task_inject_prompt` notification.
///
...
///   - direct:  `{"method":"Kimix/foo", ...}`                                 -> `Kimix/foo`
///   - wrapped: `{"method":"_kimix/foo","params":{"method":"Kimix/foo",...}}`  -> `Kimix/foo`
///
...
fn is_scheduled_task_inject_prompt(json: &serde_json::Value) -> bool {
    method_of(json) == Some("kimix/scheduled_task_inject_prompt")
}
...
fn is_interaction_request(json: &serde_json::Value) -> bool {
    matches!(
        method_of(json),
        Some("session/request_permission" | "kimix/ask_user_question" | "kimix/exit_plan_mode")
    )
}
...
/// `Kimix/session_notification` whose `update.sessionUpdate ==
...
fn extract_interaction_resolved_tool_call_id(json: &serde_json::Value) -> Option<String> {
    if method_of(json) != Some("kimix/session_notification") {
        return None;
    }
...
fn extract_session_id_from_prompt_complete(json: &serde_json::Value) -> Option<String> {
    ...
    if method != "kimix/session/prompt_complete" {
        return None;
    }
    json.get("params")?
        .get("sessionId")
...
fn inject_capabilities_into_session_new(
    ...
    if let Some(params) = json.get_mut("params").and_then(|p| p.as_object_mut()) {
        ...
        if let Some(meta_obj) = meta.as_object_mut() {
            ...
            if !meta_obj.contains_key("kimix/leaderClientId") {
                meta_obj.insert(
                    "kimix/leaderClientId".to_string(),
...
/// Extract yolo_mode change from Kimix/yolo_mode_changed notification.
///
...
fn extract_yolo_mode_change(json: &serde_json::Value) -> Option<bool> {
    let method = json.get("method")?.as_str()?;
    if method != "kimix/yolo_mode_changed" {
        return None;
    }
    let params = json.get("params")?;
    params.get("yolo_mode").and_then(|v| v.as_bool())
}
/// Extract the auto-mode intent from an `Kimix/yolo_mode_changed` notification, so
...
fn inject_client_identity_into_yolo_notification(
    ...
    let is_yolo = json
        .get("method")
        .and_then(|m| m.as_str())
        .is_some_and(|m| m == "kimix/yolo_mode_changed");
    if !is_yolo {
        return false;
    }
    let mut mutated = false;
...
fn make_version_mismatch_notification(
    ...
    Some(
        serde_json::json!(
            { "jsonrpc" : "2.0", "method" : "kimix/leader/version_mismatch", "params" : {
            ...
            format!("Client version {client_version} differs from leader version \
                     {leader_version}. Restart the kimix binary to use the same version.")
            } }
        )
        .to_string(),
    )
...
pub async fn run_leader_server(
    ...
    loop {
        tokio::select! {
            ...
            kimix_log::unified_log::info("leader.client.registered", None,
            ...
            kimix_log::unified_log::info("leader.client.disconnected", None,
            ...
            "kimix/internal/evict_sessions", "params" : { "sessionIds" : detached_sessions
            ...
            kimix_log::unified_log::warn("leader.response.orphaned", None,
            ...
            kimix_log::unified_log::warn("leader.response.send_failed", None,
...
mod tests {
    ...
    async fn relaunch_drain_waits_for_agent_activity_and_flushes_sessions() {
        ...
        *prompt_id.lock().expect("lock poisoned") = Some("prompt-1".to_string());
        ...
        *prompt_id.lock().expect("lock poisoned") = None;
    ...
    async fn initialize_gets_client_identifier_injected() {
        ...
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "kimix-tui",
    ...
    async fn initialize_preserves_existing_client_identifier() {
        ...
        .unwrap();
        let _: ServerMessage = read_message(&mut reader).await.unwrap();
        let payload = r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"kimix-web"}}}"#;
        write_message(
            &mut writer,
            &ClientMessage::Acp {
                payload: payload.into(),
        ...
        let json: serde_json::Value = serde_json::from_str(&received).unwrap();
        assert_eq!(
            json["params"]["_meta"]["clientIdentifier"], "kimix-web",
    ...
    fn is_scheduled_task_inject_prompt_detects_only_inject() {
        assert!(is_scheduled_task_inject_prompt(&pv(
            r#"{"method":"kimix/scheduled_task_inject_prompt","params":{"sessionId":"s1","taskId":"t1","prompt":"echo hi"}}"#
        )));
        assert!(is_scheduled_task_inject_prompt(&pv(
            r#"{"method":"_kimix/scheduled_task_inject_prompt","params":{"method":"kimix/scheduled_task_inject_prompt","params":{"sessionId":"s1","taskId":"t1","prompt":"echo hi"}}}"#
        )));
        assert!(!is_scheduled_task_inject_prompt(&pv(
            r#"{"method":"kimix/scheduled_task_fired","params":{"sessionId":"s1"}}"#
        )));
        assert!(!is_scheduled_task_inject_prompt(&pv(
    ...
    fn is_interaction_request_detects_only_interaction_methods() {
        for m in [
            ...
            "kimix/ask_user_question",
            "kimix/exit_plan_mode",
        ] {
        ...
        for m in ["kimix/ask_user_question", "kimix/exit_plan_mode"] {
            let payload = format!(
        ...
        assert!(!is_interaction_request(&pv(
            r#"{"jsonrpc":"2.0","method":"kimix/sessions/changed","params":{}}"#
        )));
    }
    #[test]
    fn extract_interaction_tool_call_id_handles_direct_and_nested() {
        assert_eq!(
            ...
            pv(r#"{"id":1,"method":"kimix/ask_user_question","params":{"sessionId":"s","toolCallId":"tc-q"}}"#))
            .as_deref(), Some("tc-q")
        );
        assert_eq!(
            extract_interaction_tool_call_id(&
        ...
        assert_eq!(
            extract_interaction_tool_call_id(&
            pv(r#"{"id":1,"method":"_kimix/ask_user_question","params":{"method":"kimix/ask_user_question","params":{"sessionId":"s","toolCallId":"tc-w"}}}"#))
            .as_deref(), Some("tc-w")
        );
        assert_eq!(
    ...
    fn extract_interaction_resolved_tool_call_id_matches_only_resolved() {
        assert_eq!(
            ...
            pv(r#"{"method":"kimix/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-r"}}}"#))
            .as_deref(), Some("tc-r")
        );
        assert_eq!(
            extract_interaction_resolved_tool_call_id(&
            pv(r#"{"method":"_kimix/session_notification","params":{"method":"kimix/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-rw"}}}}"#))
            .as_deref(), Some("tc-rw")
        );
        assert_eq!(
            extract_interaction_resolved_tool_call_id(&pv(
                r#"{"method":"kimix/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"pending_interaction","tool_call_id":"tc-r","kind":"permission"}}}"#
            )),
            None
        );
    }
    ...
    fn event_seq_of_parses_acp_and_ext_and_handles_missing() {
        ...
        let ext = pv(
            r#"{"params":{"method":"kimix/session/update","params":{"sessionId":"019e-aa","_meta":{"eventId":"019e-aa-7"}}}}"#,
        );
    ...
    fn inject_capabilities_adds_default_model_to_session_new() {
        ...
        let caps = ClientCapabilities {
            yolo_mode: false,
            default_model: Some("kimix-3-fast".to_string()),
            ..Default::default()
        };
        let mut json = pv(&payload);
        assert!(inject_capabilities_into_session_new(
        ...
        assert_eq!(json["params"]["_meta"]["modelId"], "kimix-3-fast");
        assert!(json["params"]["_meta"].get("yoloMode").is_none());
    }
    #[test]
    ...
    fn extract_yolo_mode_change_returns_value() {
        ...
        let payload =
            r#"{"jsonrpc":"2.0","method":"kimix/yolo_mode_changed","params":{"yolo_mode":false}}"#;
    ...
    fn extract_auto_mode_change_explicit_flag_wins() {
        let payload =
            r#"{"jsonrpc":"2.0","method":"kimix/yolo_mode_changed","params":{"auto_mode":true}}"#;
        assert_eq!(extract_auto_mode_change(&pv(payload)), Some(true));
        let payload =
            r#"{"jsonrpc":"2.0","method":"kimix/yolo_mode_changed","params":{"auto_mode":false}}"#;
        assert_eq!(extract_auto_mode_change(&pv(payload)), Some(false));
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/yolo_mode_changed","params":{"auto_mode":false,"permission_mode":"auto"}}"#;
        assert_eq!(extract_auto_mode_change(&pv(payload)), Some(false));
    }
    /// Branch 2: with no explicit flag, derive from `permission_mode`.
    #[test]
    fn extract_auto_mode_change_derives_from_permission_mode() {
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/yolo_mode_changed","params":{"permission_mode":"auto"}}"#;
        assert_eq!(extract_auto_mode_change(&pv(payload)), Some(true));
        for mode in ["ask", "always-approve", "default"] {
            let payload = format!(
                r#"{{"jsonrpc":"2.0","method":"kimix/yolo_mode_changed","params":{{"permission_mode":"{mode}"}}}}"#
            );
            assert_eq!(
    ...
    fn extract_model_id_from_set_model_returns_value() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-123","modelId":"kimix-3-fast"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        assert_eq!(
            extract_model_id_from_set_model(&pv(&payload)),
            Some("kimix-3-fast".to_string())
        );
    }
    #[test]
    fn extract_model_id_from_set_model_handles_snake_case() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"session_id":"sess-123","model_id":"kimix-3"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        assert_eq!(
            extract_model_id_from_set_model(&pv(&payload)),
            Some("kimix-3".to_string())
        );
    }
    #[test]
    fn extract_model_id_from_set_model_returns_none_for_other_methods() {
        let payload =
            r#"{"jsonrpc":"2.0","method":"other/method","id":1,"params":{"modelId":"kimix-3"}}"#;
    ...
    fn patch_initialize_response_patches_current_model_id() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"modelState":{"currentModelId":"kimix-3","availableModels":[]}}}}"#,
        );
        let default_model = Some("kimix-3-fast".to_string());
    ...
    fn patch_initialize_response_preserves_other_fields() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"kimixShell":true,"modelState":{"currentModelId":"kimix-3","availableModels":[{"modelId":"kimix-3"},{"modelId":"kimix-3-fast"}]}}}}"#,
        );
        let default_model = Some("kimix-3-fast".to_string());
        assert!(patch_initialize_response_model(&mut json, &default_model));
        assert_eq!(json["result"]["meta"]["kimixShell"], true);
        assert_eq!(
            json["result"]["meta"]["modelState"]["currentModelId"],
            "kimix-3-fast"
        );
    ...
    fn patch_initialize_response_noop_when_no_default_model() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"meta":{"modelState":{"currentModelId":"kimix-3"}}}}"#,
        );
        let before = json.clone();
    ...
    fn patch_initialize_response_noop_for_non_initialize_response() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","id":1,"result":{"session_id":"sess-1","models":{"currentModelId":"kimix-3","availableModels":[]}}}"#,
        );
        let before = json.clone();
        assert!(!patch_initialize_response_model(
            &mut json,
    ...
    fn extract_session_id_from_nested_params_works() {
        let payload = r#"{"jsonrpc":"2.0","method":"_kimix/session_notification","params":{"method":"kimix/session_notification","params":{"sessionId":"sess-nested"}}}"#;
        assert_eq!(
            extract_session_id(&pv(payload)),
            Some("sess-nested".to_string())
        );
        let payload = r#"{"jsonrpc":"2.0","method":"_kimix/fs_notify","params":{"method":"kimix/fs_notify","params":{"session_id":"sess-nested-2","event":{}}}}"#;
        assert_eq!(
            extract_session_id(&pv(payload)),
    ...
    fn extract_session_id_from_prompt_complete_works() {
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/session/prompt_complete","params":{"sessionId":"sess-prompt"}}"#;
        assert_eq!(
    ...
    fn extract_session_id_from_prompt_complete_ignores_other_methods() {
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-prompt"}}"#;
        assert_eq!(extract_session_id_from_prompt_complete(&pv(payload)), None);
    }
    #[test]
    fn extract_child_session_event_spawned() {
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-1"}}}"#;
    ...
    fn extract_child_session_event_finished() {
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-2"}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_nested_ext_notification() {
        let payload = r#"{"jsonrpc":"2.0","method":"_kimix/session_notification","params":{"method":"kimix/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-3"}}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_nested_ext_notification_finished() {
        let payload = r#"{"jsonrpc":"2.0","method":"_kimix/session_notification","params":{"method":"kimix/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-4"}}}}"#;
        match extract_child_session_event(&pv(payload)) {
    ...
    fn extract_child_session_event_none_for_other_updates() {
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#;
        assert!(extract_child_session_event(&pv(payload)).is_none());
    }
    #[test]
    fn extract_child_session_event_none_without_child_id() {
        let payload = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned"}}}"#;
        assert!(extract_child_session_event(&pv(payload)).is_none());
    }
    #[test]
    ...
    fn inject_capabilities_does_not_override_existing_client_identifier() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "kimix-tui", ClientId(1));
    ...
    fn inject_capabilities_adds_leader_client_id_to_session_load() {
        ...
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
        inject_capabilities_into_session_new(&mut json, &caps, "kimix-tui", ClientId(42));
        assert_eq!(
            json["params"]["_meta"]["kimix/leaderClientId"].as_u64(),
            Some(42)
        );
    }
    #[test]
    fn inject_capabilities_does_not_override_existing_leader_client_id() {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-1","_meta":{{"kimix/leaderClientId":7}}}}}}"#,
            AGENT_METHOD_NAMES.session_load
        );
        let caps = ClientCapabilities::default();
        let mut json = pv(&payload);
    ...
    fn extract_target_client_id_some_when_meta_present() {
        let direct = r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"sess-1","_meta":{"kimix/leaderClientId":9}}}"#;
        assert_eq!(extract_target_client_id(&pv(direct)), Some(ClientId(9)));
        let nested = r#"{"jsonrpc":"2.0","method":"_kimix/session/update","params":{"params":{"sessionId":"sess-1","_meta":{"kimix/leaderClientId":11}}}}"#;
    ...
    fn inject_client_identity_adds_identifier_to_initialize() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "kimix-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated, "should have injected the identifier");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "kimix-tui");
    }
    #[test]
    fn inject_client_identity_does_not_override_existing() {
        let mut json = pv(
            r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"kimix-web"}}}"#,
        );
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "kimix-tui");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(!mutated, "existing identifier means nothing was injected");
        assert_eq!(json["params"]["_meta"]["clientIdentifier"], "kimix-web");
    }
    #[test]
    ...
    fn inject_client_identity_preserves_existing_meta() {
        ...
        let (mutated, was_initialize) =
            inject_client_identity_into_initialize(&mut json, "kimix-code-extension");
        assert!(was_initialize, "should have detected an initialize message");
        assert!(mutated);
        assert_eq!(json["params"]["_meta"]["foo"], "bar");
        assert_eq!(
    ...
    fn version_mismatch_notification_contains_correct_fields() {
        ...
        assert_eq!(json["method"], "kimix/leader/version_mismatch");
    ...
    async fn set_model_updates_default_model_for_next_session_new() {
        ...
        write_message(
            ...
            &ClientMessage::Register {
                ...
                capabilities: ClientCapabilities {
                    yolo_mode: false,
                    default_model: Some("kimix-original".to_string()),
                    ..Default::default()
                },
            },
        )
        ...
        let set_model_payload = format!(
            r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-1","modelId":"kimix-4.5"}}}}"#,
            AGENT_METHOD_NAMES.session_set_model
        );
        write_message(
            &mut writer,
        ...
        let json: serde_json::Value = serde_json::from_str(&forwarded).unwrap();
        assert_eq!(
            json["params"]["_meta"]["modelId"], "kimix-4.5",
    ...
    async fn ext_notification_with_nested_session_id_routes_correctly() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"_kimix/session_notification","params":{"method":"kimix/session_notification","params":{"sessionId":"sess-A","update":{"sessionUpdate":"retry_state","attempt":1,"maxRetries":3,"reason":"transient"}}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
        ...
        match msg {
            ServerMessage::Acp { payload } => {
                let json: serde_json::Value = serde_json::from_str(&payload).unwrap();
                assert_eq!(json["method"], "_kimix/session_notification");
            }
            other => panic!("Expected Acp message, got {:?}", other),
        }
    ...
    async fn high_throughput_replay_no_drops() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let (mut reader, mut writer) = connect_and_register(&sock_path, "kimix-tui").await;
    ...
    /// sends an `Kimix/internal/evict_sessions` notification through acp_tx
    ...
    async fn evict_sessions_notification_on_disconnect() {
        ...
        assert_eq!(json["method"], "kimix/internal/evict_sessions");
    ...
    /// A `Kimix/scheduled_task_inject_prompt` (cron `/loop` fire) must be routed
    ...
    async fn scheduled_task_inject_prompt_routes_to_driver_only() {
        ...
        let _ = next_acp_payload(&mut reader_b).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let inject = r#"{"method":"_kimix/scheduled_task_inject_prompt","params":{"method":"kimix/scheduled_task_inject_prompt","params":{"sessionId":"sess-cron","taskId":"task-1","prompt":"echo hello","humanSchedule":"every 1m"}}}"#;
    ...
    async fn interaction_request_broadcasts_to_all_subscribers() {
        ...
        let _ = next_acp_payload(&mut reader_b).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":501,"method":"_kimix/ask_user_question","params":{"method":"kimix/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-q","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
    ...
    async fn pending_interaction_replayed_to_late_joiner() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":601,"method":"_kimix/ask_user_question","params":{"method":"kimix/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-late","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
    ...
    async fn reattached_client_backfilled_into_child_routes() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-sub","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub"}}}"#;
        ...
        let child_reverse = r#"{"jsonrpc":"2.0","id":777,"method":"kimix/child_thing","params":{"sessionId":"child-sub"}}"#;
    ...
    async fn replayed_spawn_registers_child_route_for_loading_client() {
        ...
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-fresh","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-fresh"}}}}}}"#,
            a_id.0
        );
    ...
    async fn late_attacher_backfilled_into_existing_child_routes() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-sub2","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub2"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
    ...
    async fn replayed_finished_does_not_tear_down_live_child_route() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-tear","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-tear"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-tear","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-tear"}}}}}}"#,
            b_id.0
        );
    ...
    async fn backfill_covers_nested_children() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_child = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-nest"}}}"#;
        response_tx.send(spawned_child.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let spawned_grandchild = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"child-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-nest"}}}"#;
    ...
    async fn intermediate_finish_reparents_live_grandchild_for_root_backfill() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_a = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-a"}}}"#;
        response_tx.send(spawned_a.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "child-a").await;
        let spawned_b = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"child-a","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-b"}}}"#;
        response_tx.send(spawned_b.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "grandchild-b").await;
        let finished_a = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-a"}}}"#;
    ...
    async fn live_finished_prunes_index_so_reattach_skips_dead_child() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-dead"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let finished_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-dead"}}}"#;
    ...
    async fn detached_live_finished_prunes_index_so_reattach_skips_dead_child() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-detach"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        drop(reader_a);
        drop(writer_a);
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-detach"}}}"#;
        response_tx.send(finished_live.to_string()).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
        let (mut reader_a2, mut writer_a2) = connect_and_register(&sock_path, "client-a").await;
    ...
    async fn mid_burst_disconnect_still_prunes_dead_child_route() {
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-leak"}}}}}}"#,
            a_id.0
        );
        response_tx.send(spawned_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-leak"}}}}}}"#,
            a_id.0
        );
        response_tx.send(finished_replay).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
    ...
    async fn orphaned_replayed_finished_leaves_held_route_untouched() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-hold","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-hold"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        ...
        tokio::time::sleep(Duration::from_millis(50)).await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-hold","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-hold"}}}}}}"#,
            b_id.0
        );
        response_tx.send(finished_replay).unwrap();
        tokio::time::sleep(Duration::from_millis(30)).await;
    ...
    async fn replayed_spawn_unions_into_existing_live_route() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-union","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let (mut reader_b, mut writer_b, b_id) =
            connect_register_get_id(&sock_path, "client-b").await;
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-union","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}}}}"#,
            b_id.0
        );
        response_tx.send(spawned_replay).unwrap();
    ...
    async fn replayed_finished_last_subscriber_prunes_dead_child() {
        ...
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-last"}}}}}}"#,
            a_id.0
        );
        response_tx.send(spawned_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        let finished_replay = format!(
            r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"kimix/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-last"}}}}}}"#,
            a_id.0
        );
        response_tx.send(finished_replay).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_finished").await;
    ...
    async fn mid_load_child_delta_reaches_loader_via_request_side_backfill() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let spawned_live = r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-midload","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-midload"}}}"#;
        response_tx.send(spawned_live.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
        drop(reader_a);
        drop(writer_a);
    ...
    async fn pending_interaction_survives_disconnect_and_replays_on_reconnect() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":801,"method":"_kimix/ask_user_question","params":{"method":"kimix/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-reconnect","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "ask_user_question").await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        drop(reader_a);
    ...
    async fn interaction_raised_with_no_subscriber_is_cached_and_replayed_on_first_attach() {
        ...
        let (sock_path, cancel, response_tx, mut acp_rx) =
            setup_persistent_server_with_agent(&temp).await;
        let req = r#"{"jsonrpc":"2.0","id":901,"method":"_kimix/ask_user_question","params":{"method":"kimix/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-nosub","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
    ...
    async fn resolved_interaction_not_replayed_to_late_joiner() {
        ...
        let _ = next_acp_payload(&mut reader_a).await;
        tokio::time::sleep(Duration::from_millis(30)).await;
        let req = r#"{"jsonrpc":"2.0","id":701,"method":"_kimix/ask_user_question","params":{"method":"kimix/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-ev","questions":[]}}}"#;
        response_tx.send(req.to_string()).unwrap();
        let _ = next_acp_payload_matching(&mut reader_a, "ask_user_question").await;
        let resolved = r#"{"method":"_kimix/session_notification","params":{"method":"kimix/session_notification","params":{"sessionId":"sess-int","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-ev"}}}}"#;
    ...
    /// `Kimix/sessions/changed` is a machine-wide roster notification with no
    ...
    async fn roster_changed_broadcasts_to_all_clients() {
        ...
        tokio::time::sleep(Duration::from_millis(20)).await;
        let changed = r#"{"jsonrpc":"2.0","method":"kimix/sessions/changed","params":{"upserted":[{"sessionId":"sess-roster","cwd":"/repo","isWorktree":false,"yolo":false,"activity":"working","resident":true,"lastChangeUnixMs":1,"origin":{"kind":"local"}}],"removed":[]}}"#;
    ...
    /// `Kimix/models/update` is a machine-wide catalog notification with no
    ...
    /// ext notifications arrive `_`-prefixed (`_Kimix/models/update`).
    #[tokio::test]
    async fn models_update_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_kimix/models/update","params":{"currentModelId":"kimix-new","availableModels":[{"modelId":"kimix-new","name":"Kimix New"}]}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
            got_a.as_deref().is_some_and(|p| p.contains("kimix-new")),
            "client A must receive the models broadcast, got {got_a:?}"
        );
        assert!(
            got_b.as_deref().is_some_and(|p| p.contains("kimix-new")),
            "client B must receive the models broadcast, got {got_b:?}"
        );
        cancel.cancel();
    }
    /// `Kimix/mcp/servers_updated` is a machine-wide MCP-catalog notification
    ...
    async fn mcp_servers_updated_broadcasts_to_all_clients() {
        ...
        let (mut reader_b, _writer_b) = connect_and_register(&sock_path, "client-b").await;
        tokio::time::sleep(Duration::from_millis(20)).await;
        let update = r#"{"jsonrpc":"2.0","method":"_kimix/mcp/servers_updated","params":{"method":"kimix/mcp/servers_updated","params":{"mcpServers":[{"name":"team_slack","source":"local"}]}}}"#;
        response_tx.send(update.to_string()).unwrap();
        let got_a = next_acp_payload(&mut reader_a).await;
        let got_b = next_acp_payload(&mut reader_b).await;
        assert!(
    ...
    fn machine_wide_broadcast_classifier_matches_both_wire_forms() {
        ...
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"kimix/models/update","params":{}}"#
        )));
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"kimix/mcp/servers_updated","params":{}}"#
        )));
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"_kimix/sessions/changed","params":{}}"#
        )));
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"_kimix/models/update","params":{}}"#
        )));
        assert!(is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"_kimix/mcp/servers_updated","params":{"method":"kimix/mcp/servers_updated","params":{"mcpServers":[]}}}"#
        )));
        assert!(!is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"s"}}"#
        )));
        assert!(!is_machine_wide_broadcast_notification(&pv(
            r#"{"jsonrpc":"2.0","method":"kimix/settings/update","params":{}}"#
        )));
    }
    ...
    fn inject_capabilities_sets_code_nav_enabled_true() {
        ...
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "kimix-web", ClientId(1));
        assert_eq!(
    ...
    fn inject_capabilities_sets_code_nav_enabled_false() {
        ...
        let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"kimix-tui"}}}"#;
        let mut json = pv(payload);
        inject_capabilities_into_session_new(&mut json, &caps, "kimix-tui", ClientId(1));
        assert_eq!(
            json["params"]["_meta"]["codeNavEnabled"],
    ...
    fn inject_capabilities_two_clients_stay_isolated() {
        ...
        inject_capabilities_into_session_new(&mut web_json, &web_caps, "kimix-web", ClientId(1));
        let mut tui_json = pv(session_new);
        inject_capabilities_into_session_new(&mut tui_json, &tui_caps, "kimix-tui", ClientId(2));
        assert_eq!(
    ...
    async fn subagent_child_session_routed_after_spawned() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-123"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"child-123","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
    ...
    async fn subagent_child_session_cleaned_up_on_finished() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-456"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-456"}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"child-456","update":{"sessionUpdate":"message_delta"}}}"#
                    .into(),
            )
            .unwrap();
        let timeout_result: Result<Result<ServerMessage, _>, _> =
    ...
    async fn subagent_child_session_not_leaked_to_other_client() {
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"_kimix/session_notification","params":{"method":"kimix/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-789"}}}}"#
                    .into(),
            )
            .unwrap();
        let _: ServerMessage =
        ...
        response_tx
            .send(
                r#"{"jsonrpc":"2.0","method":"kimix/session_notification","params":{"sessionId":"child-789","update":{"sessionUpdate":"message_delta"}}}"#
                    .into(),
            )
            .unwrap();
        let msg: ServerMessage =
    ...
    async fn leader_client_id_unicasts_to_target_only() {
        ...
        response_tx
            .send(
                format!(
                    r#"{{"jsonrpc":"2.0","method":"session/update","params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"agent_message_chunk"}},"_meta":{{"kimix/leaderClientId":{}}}}}}}"#,
                    id_a
                ),
            )
            .unwrap();
    ...
    async fn leader_client_id_dropped_when_target_disconnected() {
        ...
        response_tx
            .send(
                format!(
                    r#"{{"jsonrpc":"2.0","method":"session/update","params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"agent_message_chunk"}},"_meta":{{"isReplay":true,"kimix/leaderClientId":{}}}}}}}"#,
                    id_a
                ),
            )
            .unwrap();
        response_tx
            .send(
                format!(
                    r#"{{"jsonrpc":"2.0","method":"_kimix/session/update","params":{{"params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"hook_annotation","message":"m"}},"_meta":{{"isReplay":true,"kimix/leaderClientId":{}}}}}}}}}"#,
                    id_a
                ),
            )
            .unwrap();
