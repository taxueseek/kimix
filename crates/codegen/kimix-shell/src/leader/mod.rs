//! Leader-follower IPC architecture for Kimix-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.kimix/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//! ```ignore
//! use kimix_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
const CLIENT_LEADER_VERSION: &str = kimix_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`kimix -p`).
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::kimix_home::kimix_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : kimix_version::VERSION,
                            }
                        )),
                    );
                }
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`kimix -p` where hanging forever is unacceptable.
...
///     "Kimix-tui", ClientMode::Stdio, caps, status_tx,
/// );
///
...
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "Kimix-tui", "vscode")
...
///
/// For a **managed install** — the running binary lives under `kimix_home`
/// (e.g. `~/.kimix/...`) — prefer the managed `~/.kimix/bin/Kimix` symlink. After an
/// auto-update or `kimix update` atomically swaps that symlink, `current_exe()`
...
/// `kimix_update::auto_update::resolve_restart_exe`.
///
/// For a **dev / out-of-tree binary** (`cargo run`, integration tests, installs
/// not under `kimix_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.kimix/bin/Kimix` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::kimix_home::kimix_home())
}
fn resolve_binary_with_home(kimix_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(kimix_home, std::env::current_exe().ok())
}
/// Binary file name for the managed Kimix install (`kimix` / `Kimix.exe`).
fn managed_kimix_bin_name() -> &'static str {
    if cfg!(windows) { "kimix.exe" } else { "kimix" }
}
...
fn resolve_binary_impl(
    ...
    let managed_bin = kimix_home.join("bin").join(managed_kimix_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, kimix_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess() -> Result<u32, ConnectionError> {
    ...
    let log_path = crate::util::kimix_home::kimix_home().join("leader.log");
    ...
    let leader_log = std::env::var("KIMIX_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "kimix_shell=info,kimix_acp_lib=warn,kimix_mcp=warn".into());
...
mod tests {
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("kimix"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("kimix-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("kimix")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("kimix-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("kimix");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("kimix-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_kimix_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_kimix_bin_name());
