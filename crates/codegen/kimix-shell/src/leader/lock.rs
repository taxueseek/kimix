...
use fs2::FileExt;
use kimix_workspace::util::is_lock_contended;
...
/// `~/.kimix/leader.sock` — e.g. run a local branch build's leader without
...
/// path.
pub const LEADER_SOCKET_ENV: &str = "KIMIX_LEADER_SOCKET";
...
/// `.lock` when set, else the default under Kimix home.
pub fn default_lock_path() -> PathBuf {
    resolve_lock_path(leader_socket_override(), &kimix_home())
}
...
/// else the default under Kimix home.
pub fn default_socket_path() -> PathBuf {
    resolve_socket_path(leader_socket_override(), &kimix_home())
}
...
impl LeaderLock {
    /// Create a new LeaderLock using the default paths in Kimix home
...
mod tests {
    ...
    fn override_socket_path_wins_over_default_derivation() {
        let root = Path::new("/home/u/.kimix");
        let override_sock = PathBuf::from("/home/u/.kimix/leader-branch.sock");
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.kimix/leader-branch.lock")
        );
    }
