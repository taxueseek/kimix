...
use kimix_config::signed_policy::now_unix;
...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that KIMIX_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    DeploymentKeyRejected,
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `kimix login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.kimix. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<kimix_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        &self,
    ) -> Option<kimix_config::signed_policy::SignatureEnvelope> {
        self.signature_sidecar_with(kimix_config::signed_policy::embedded_key_id_trusted)
    }
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: kimix_config::signed_policy::SignatureEnvelope,
    pub(super) payload: kimix_config::signed_policy::SignedPayload,
}
...
mod tests {
    ...
    fn signature_sidecar_picks_trusted_envelope_then_falls_back() {
        use kimix_config::signed_policy::SignatureEnvelope;
