//! Notification bridge: translates `Kimix-tools` `ToolNotification` events
//! into `Kimix-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use kimix_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    /// `Kimix/incrementalBashOutput` capability.
    ...
    pub auto_wake_delivered: kimix_tools::reminders::task_completion::AutoWakeDeliveredIds,
    ...
    /// next-turn exit reminder. Kimix-build leaves this `false` — its
...
async fn emit_current_mode_update(
    ...
    mode: kimix_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::BashExecutionBackgrounded(bg) => {
            ...
            // Forward as Kimix/task_backgrounded ExtNotification so the TUI can
            ...
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("kimix/task_backgrounded", params.into());
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor = task_snapshot.kind == kimix_tools::computer::types::TaskKind::Monitor;
            ...
            // stdout. The pager still receives Kimix/task_completed below for UI.
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    kimix_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    kimix_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = kimix_tools::reminders::wrap_reminder(&body);
            ...
            if let Some(params) = params {
                let notification: acp::ExtNotification =
                    acp::ExtNotification::new("kimix/task_completed", params.into());
                config.gateway.forward_fire_and_forget(notification);
            }
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, kimix_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
        ...
        ToolNotification::PlanModeExited(exited) => {
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                emit_current_mode_update(config, kimix_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use kimix_tools::computer::types::TaskKind;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>,
        mpsc::UnboundedReceiver<PersistenceMsg>,
        mpsc::UnboundedReceiver<SessionCommand>,
    ) {
        let (gateway_tx, gateway_rx) = mpsc::unbounded_channel();
        let gateway = kimix_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
    ...
    /// drain it). The pager's `Kimix/task_completed` notification still fires.
    #[tokio::test]
    async fn bash_task_completed_suppresses_auto_wake_during_goal_loop() {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "kimix/task_completed"
            {
                found_ext = true;
            }
        }
        assert!(
            found_ext,
            "kimix/task_completed ExtNotification must still be sent for UI"
        );
    }
    ...
    /// `will_wake` off the emitted `Kimix/task_completed` params.
    fn task_completed_will_wake(
        gateway_rx: &mut mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>,
    ) -> Option<bool> {
        while let Ok(msg) = gateway_rx.try_recv() {
            if let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "kimix/task_completed"
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(kimix_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            kimix_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let broadcast_id = match gateway_rx.try_recv().expect("chunk must be broadcast") {
            kimix_acp_lib::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            kimix_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            kimix_tools::notification::types::BashExecutionBackgrounded {
                base: kimix_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            kimix_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn legacy_monitor_event_without_owner_is_injected() {
        // Legacy / non-Kimix backends record no owner; such events must
    ...
    async fn block_waited_task_skips_auto_wake_prompt() {
        ...
        // The Kimix/task_completed ExtNotification for UI updates must still be sent.
        let mut found_ext = false;
        while let Ok(msg) = gateway_rx.try_recv() {
            if let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "kimix/task_completed"
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        ...
        let notification =
            ToolNotification::PlanModeExited(kimix_tools::notification::types::PlanModeExited {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let kimix_acp_lib::AcpClientMessage::SessionNotification(args) = msg
    ...
    /// Default (Kimix) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
        ...
        let notification =
            ToolNotification::PlanModeExited(kimix_tools::notification::types::PlanModeExited {
                tool_call_id: "tc-exit-kimix".into(),
                plan_content: Some("- step 1".into()),
                plan_file_path: "/tmp/test-session/plan.md".into(),
            });
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification =
            ToolNotification::PlanModeEntered(kimix_tools::notification::types::PlanModeEntered {
