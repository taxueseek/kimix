...
/// Production Kimix foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production Kimix opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `KIMIX_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> kimix_tools::implementations::kimix::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("KIMIX_WEB_FETCH_PROXY"))
...
/// from `kimix_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `Kimix` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `KimixHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "KimixHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "KimixHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"Kimix:read_file"));
        assert!(ids.contains(&"Kimix:search_replace"));
        assert!(ids.contains(&"Kimix:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"KimixHashline:hashline_read"));
        assert!(ids.contains(&"KimixHashline:hashline_edit"));
        assert!(ids.contains(&"KimixHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            kimix_agent::config::AgentDefinition::plan(),
            kimix_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "Kimix:search_replace"),
            ...
            assert!(
                ids.contains(&"KimixHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"Kimix:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"KimixHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production kimix must set the 10h foreground ceiling"
        );
    }
