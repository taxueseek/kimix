...
// Re-export from Kimix-chat-state — canonical definition lives there.
pub use kimix_chat_state::compaction_utils::extract_user_query;
...
pub async fn compute_vcs_status_block(
    ...
    use kimix_workspace::file_system::{git_status, jj_status};
...
// Tests for extract_user_query now live in kimix_chat_state::compaction_utils.

#[cfg(test)]
mod tests {
    use super::*;
    use kimix_workspace::file_system::FsError;
    ...
    async fn construct_user_message_returns_without_git_status_on_bad_dir() {
        let dir = std::path::Path::new("/tmp/nonexistent-kimix-test-dir");
