...
pub struct TurnDeltaSnapshot {
    ...
    /// Per-turn cache hit rate (0.0-1.0), derived from `turn_cached_input_tokens / turn_input_tokens`.
    #[serde(skip)]
    pub turn_cache_hit_rate: f64,
}
...
pub struct SessionSignals {
    ...
    // === Cache Performance ===
    /// Most recent turn's cache hit rate (0.0-1.0).
    /// Updated at each turn-end snapshot from `turn_cached_input_tokens / turn_input_tokens`.
    /// Used by the TUI context bar to display cache efficiency.
    #[serde(default)]
    pub last_cache_hit_rate: f64,

    // === Bash tool patterns (Kimix) ===
...
pub enum SignalEvent {
    ...
    /// Update the persistent cache hit rate (most recent turn's rate).
    SetCacheHitRate(f64),
...
impl SessionSignalsHandle {
    ...
    /// Update the persistent cache hit rate stored in session signals.
    ///
    /// Called after each turn-end snapshot to record the most recent turn's
    /// cache hit rate for display in the TUI context bar.
    pub async fn set_last_cache_hit_rate(&self, rate: f64) {
        let _ = self.tx.send(SignalEvent::SetCacheHitRate(rate));
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    kimix_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
    ...
    pub async fn run(mut self) {
        while let Some(event) = self.rx.recv().await {
            match event {
                ...
                SignalEvent::SetCacheHitRate(rate) => {
                    self.signals.last_cache_hit_rate = rate;
                }
...
mod tests {
    ...
    async fn test_model_tracking() {
        ...
        // Set primary model
        handle.set_primary_model("kimix-3");

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.primary_model_id, Some("kimix-3".to_string()));
        assert_eq!(snapshot.models_used, vec!["kimix-3".to_string()]);

        // Record additional model usage
        handle.record_model_usage("kimix-4");
        handle.record_model_usage("kimix-3"); // Duplicate

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.models_used.len(), 2);
        assert!(snapshot.models_used.contains(&"kimix-3".to_string()));
        assert!(snapshot.models_used.contains(&"kimix-4".to_string()));
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        handle.seed_counts(
            ...
            vec!["kimix-3".to_string(), "kimix-4".to_string()],
        );
        ...
        handle.record_model_usage("kimix-3"); // existing model
        handle.record_model_usage("kimix-4.5"); // new model
        ...
        assert_eq!(snapshot.models_used.len(), 3); // kimix-3 not duplicated, kimix-5 added
        assert!(snapshot.models_used.contains(&"kimix-4.5".to_string()));
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        handle1.record_model_usage("kimix-3");
        ...
        handle1.record_assistant_message();
        handle1.record_model_usage("kimix-4");
        ...
        assert!(restored.models_used.contains(&"kimix-3".to_string()));
        assert!(restored.models_used.contains(&"kimix-4".to_string()));
        ...
        // ITL stats must survive the restore (regression test for Kimix-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the Kimix-critique bug describes: the
        ...
        handle2.record_model_usage("kimix-3"); // existing model (should dedup)
        ...
        assert_eq!(after_turn.models_used.len(), 2); // kimix-3 not duplicated
