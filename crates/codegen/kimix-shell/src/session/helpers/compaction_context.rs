...
//! shell-specific types (`kimix_tools::MemoryBackend`, memory context).
//!

//! The core [`CompactionStateContext`] struct and its builder live in
//! `kimix_chat_state::compaction_utils`. This module adds system-reminder
//! rendering that requires dependencies not available in `Kimix-chat-state`.
//!

//! The three **common** active-agent sections (background tasks, TODO list,
//! running subagents) are formatted by
//! [`kimix_compaction::reminder`] so Kimix-chat and Kimix stay in lockstep.
...
pub fn to_system_reminder_sync(
    ...
    skills: &[kimix_tools::implementations::skills::types::SkillInfo],
...
pub async fn to_system_reminder(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[kimix_tools::implementations::skills::types::SkillInfo],
    memory_backend: Option<&dyn kimix_tools::types::memory_backend::MemoryBackend>,
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
...
fn to_system_reminder_inner(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[kimix_tools::implementations::skills::types::SkillInfo],
    memory_results: &[kimix_tools::types::memory_backend::MemorySearchResult],
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
    ...
    // the startup `<system-reminder>` (name + short desc / Use when; no Absolute path).
    if let Some(listing) =
        kimix_tools::types::skill_discovery_tracker::format_compaction_skill_listing(skills)
    {
    ...
    if !ctx.connected_mcp_servers.is_empty() {
        use kimix_tools::implementations::search_tool::format_compaction_server_line;
