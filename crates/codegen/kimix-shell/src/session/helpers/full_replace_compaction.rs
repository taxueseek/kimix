//! Kimix's L5 wiring onto the shared full-replace engine
//! (`kimix_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](kimix_compaction::sample_full_replace_summary);
//! this module adapts Kimix's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](kimix_compaction::FullReplaceError).
use std::sync::Mutex;
use std::time::Duration;

use agent_client_protocol as acp;
use async_trait::async_trait;
use kimix_compaction::{
...
use kimix_sampling_types::{ConversationItem, HostedTool, ToolSpec};
...
/// [`CompactionSampler`] for Kimix's full-replace pass.
///
...
/// else the structured Kimix prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the Kimix prompt,
...
impl ShellCompactionSampler {
    ...
    pub(crate) fn take_last_success(&self) -> Option<CompactOutput> {
        self.last_success.lock().expect("lock poisoned").take()
    }
}
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured Kimix), ignoring the shared
        ...
        .await
        {
            Ok(output) => {
                ...
                *self.last_success.lock().expect("lock poisoned") = Some(output);
...
/// Map Kimix's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces Kimix's per-attempt telemetry:
...
impl ShellFullReplaceObserver {
    ...
    pub(crate) fn attempt_count(&self) -> u32 {
        self.state.lock().expect("lock poisoned").attempts
    }
