...
/// Memory log target — matches `kimix_log::memory_log::TARGET`.
const LOG: &str = "kimix_memory";
...
pub fn should_flush(
    ...
    let should = kimix_token_estimation::exceeds_threshold_with_headroom(
        total_tokens,
        context_window,
...
pub fn is_duplicate(content: &str, db_path: &std::path::Path) -> bool {
    ...
    let conn = match kimix_sqlite_journal::JournalMode::for_db_path(db_path).open_readonly(db_path)
    {
        Ok(c) => c,
        Err(_) => {
