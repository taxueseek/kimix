...
///    (Kimix/SGLang via ChatCompletions/Responses) keeps reasoning VERBATIM so
...
pub(crate) fn build_recap_items(
    ...
    let mut items = if strip_reasoning {
        kimix_chat_state::compaction_utils::strip_reasoning_blocks(conversation)
    } else {
        conversation
    };
...
/// `max_prompt_length` for current `kimix` / `Kimix-4.5` product backends
...
/// that unlikely for normal Kimix sessions).
///
/// * Fast path — if the whole snapshot already fits, returns
///   `build_recap_items(...)` verbatim (keeps the Kimix prefix KV cache warm;
...
pub(crate) fn budget_recap_items(
    ...
    // verbatim path keeps the Kimix prefix cache warm.
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use kimix_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
        use std::sync::Arc;
    ...
    fn mk_tool_call(id: &str, args: &str) -> kimix_sampling_types::ToolCall {
        use std::sync::Arc;
    ...
    #[test]
    fn budget_over_budget_strips_reasoning_even_on_kimix() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("z".repeat(40_000)),
        ];
        // Kimix backend => strip_reasoning=false, but the over-budget branch must
    ...
    #[test]
    fn budget_fast_path_keeps_reasoning_on_kimix() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("small"),
        ];
        // Fits under a large window on Kimix (strip_reasoning=false) => verbatim,
        // reasoning kept so the prefix KV cache stays warm.
        let out = budget_recap_items(conv, "system-reminder", false, 256_000);
        assert!(
            out.iter()
                .any(|i| matches!(i, ConversationItem::Reasoning(_))),
            "fits path on kimix must keep reasoning verbatim"
        );
    }
