...
pub(crate) fn is_cursor_user_template(
    _template: &kimix_agent::prompt::user_message::UserMessageTemplate,
) -> bool {
    false
}
/// `false` twin of [`is_cursor_system_template`]; see [`is_cursor_user_template`].
pub(crate) fn is_cursor_system_template(
    _template: &kimix_agent::prompt::context::TemplateOverride,
) -> bool {
    false
}
...
impl PromptOrigin {
    ...
    pub fn from_prompt_id(prompt_id: &str) -> Self {
        if let Some(task_id) = prompt_id.strip_prefix("task-completed-") {
            ...
            Self::PlanResume
        } else if let Some(rest) = prompt_id.strip_prefix("sub-")
            && let Some(end) = rest.find('-')
        {
            let subagent_id = rest[..end].to_string();
            Self::SubagentTurn { subagent_id }
        } else {
            Self::User
        }
    }
    ...
    pub fn is_synthetic(&self) -> bool {
        !matches!(self, Self::User | Self::SubagentTurn { .. })
    }
...
/// Client-requested fs notification mode (was kimix_fsnotify::FsNotifyMode).
...
pub use kimix_shared::session::info;
pub mod kimix_recall;
pub mod managed_mcp;
...
pub mod persistence;
pub use kimix_shared::placeholder_images;
pub mod plan_mode;
pub mod prompt_history;
pub mod prompt_parser;
pub(crate) mod queue_recovery;
