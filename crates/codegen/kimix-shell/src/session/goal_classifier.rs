...
/// `KIMIX_GOAL_CLASSIFIER_MAX` or remote `goal_classifier_max_runs` to
/// raise it arbitrarily (only the `GOAL_CLASSIFIER_MAX_RUNS_MIN` floor
/// is enforced).
pub(crate) const GOAL_CLASSIFIER_MAX_RUNS_DEFAULT: u32 = 10;

/// Floor for `KIMIX_GOAL_CLASSIFIER_MAX` / remote `goal_classifier_max_runs`.
...
/// attempt. Override via `KIMIX_GOAL_VERIFIER_N` (clamped 1..=5) or the
...
/// Lower/upper bounds for `KIMIX_GOAL_VERIFIER_N` / remote
...
mod tests {
    ...
    async fn channel_spawner_request_is_harness_internal() {
        use kimix_tools::implementations::kimix::task::types::{SubagentEvent, SubagentResult};
    ...
    fn validate_details_path_in_root_keys_on_injected_temp_root() {
        ...
        assert!(
            validate_details_path_in_root(
                Path::new("/var/folders/zz/T/kimix-goal-abc/goal-classifier-abc-1.md"),
                mac_like,
            )
            .is_ok(),
        ...
        assert!(
            validate_details_path_in_root(
                Path::new("/tmp/kimix-goal-abc/goal-classifier-abc-1.md"),
                Path::new("/tmp"),
            )
            .is_ok(),
    ...
    fn gap_fingerprint_is_stable_across_scratch_path_churn() {
        ...
        let c = gap_fingerprint(&[
            "no captured output in /var/folders/x1/T/kimix-goal-1/out.log for criterion 2",
        ]);
        let d = gap_fingerprint(&[
            "no captured output in /var/folders/x1/T/kimix-goal-2/out.log for criterion 2",
        ]);
        assert_eq!(c, d);
    ...
    fn verifier_template_renders_per_agent_type_and_falls_back() {
        use kimix_tools::implementations::kimix::task::types::SubagentTypeSummary;
        let mut tool_names = std::collections::HashMap::new();
        tool_names.insert(
            kimix_tools::types::tool::ToolKind::Read,
            "cursor_read".to_string(),
        );
        tool_names.insert(
            kimix_tools::types::tool::ToolKind::ListDir,
            "cursor_ls".to_string(),
        );
        tool_names.insert(
            kimix_tools::types::tool::ToolKind::Search,
        ...
        // Kimix explicit render: no leftover placeholder either.
        let kimix = RoleToolNames::from_summary(&summary_with(&[
            (kimix_tools::types::tool::ToolKind::Read, "read_file"),
            (kimix_tools::types::tool::ToolKind::ListDir, "list_dir"),
            (kimix_tools::types::tool::ToolKind::Search, "grep"),
        ]))
        .apply(GOAL_VERIFIER_PROMPT_TEMPLATE);
        assert_no_tool_placeholders(&kimix);
    ...
    fn cold_and_resume_renders_are_symmetric_for_an_index() {
        let summary = summary_with(&[
            (kimix_tools::types::tool::ToolKind::Read, "cursor_read"),
            (kimix_tools::types::tool::ToolKind::ListDir, "cursor_ls"),
            (kimix_tools::types::tool::ToolKind::Search, "cursor_grep"),
        ]);
    ...
    async fn verification_stage_renders_per_index_tool_names() {
        ...
        let prompts = observed.prompts.lock().expect("lock poisoned");
        let idxs = observed.skeptic_idxs.lock().expect("lock poisoned");
    ...
    fn render_skeptic_prompt_substitutes_kind_lens_and_leaves_no_placeholder() {
        let body = render_skeptic_prompt(
            ...
            "/tmp/kimix-goal-x/skeptic-0",
            "/tmp/kimix-goal-x/implementer",
            None,
            &RoleToolNames::inherit_defaults(),
            true,
        );
        ...
        assert!(body.contains("/tmp/kimix-goal-x/skeptic-0"));
        assert!(body.contains("/tmp/kimix-goal-x/implementer"));
        assert!(
    ...
    fn collect_events() -> (Arc<Mutex<Vec<String>>>, impl Fn(Event) + Send + Sync) {
        ...
        let emit = move |e: Event| {
            ...
            log_clone.lock().expect("lock poisoned").push(tag);
        };
        (log, emit)
    }
    ...
    fn stage_inputs_resume<'a>(
        ...
        VerificationStageInputs {
            ...
            implementer_scratch_dir: Path::new("/tmp/kimix-goal-test/implementer"),
    ...
    async fn fired_event_reports_effective_cap_not_default() {
        ...
        let emit = move |e: Event| {
            if let Event::GoalClassifierFired { max_runs, .. } = e {
                *cap_clone.lock().expect("lock poisoned") = Some(max_runs);
            }
        };
        let wsp = tempfile::tempdir().unwrap();
        let vid = unique_verifier_id();
    ...
    async fn verification_stage_n1_not_refuted_returns_achieved() {
        ...
        assert!(
            p.contains("/tmp/kimix-goal-test/implementer"),
    ...
    async fn verification_stage_resumes_skeptic0_on_later_attempt() {
        ...
        let resume_froms = observed.resume_froms.lock().expect("lock poisoned");
        assert_eq!(
    ...
    async fn cold_fallback_after_resume_failure_carries_pool0_model_on_request() {
        ...
        let spawns = captured.lock().expect("lock poisoned").clone();
    ...
    fn resolve_goal_verifier_count_env_clamps() {
        unsafe { std::env::set_var("KIMIX_GOAL_VERIFIER_N", "0") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MIN
        );
        unsafe { std::env::set_var("KIMIX_GOAL_VERIFIER_N", "99") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MAX
        );
        unsafe { std::env::set_var("KIMIX_GOAL_VERIFIER_N", "garbage") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_COUNT,
            "invalid env falls through to the default"
        );
        unsafe { std::env::remove_var("KIMIX_GOAL_VERIFIER_N") };
    }
    ...
    fn resolve_goal_verifier_count_precedence_and_clamp() {
        ...
        // env > config.
        unsafe { std::env::set_var("KIMIX_GOAL_VERIFIER_N", "2") };
    ...
    fn resolve_goal_classifier_max_runs_env_clamps_and_no_ceiling() {
        unsafe { std::env::set_var("KIMIX_GOAL_CLASSIFIER_MAX", "0") };
        assert_eq!(
        ...
        unsafe { std::env::set_var("KIMIX_GOAL_CLASSIFIER_MAX", "999") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::set_var("KIMIX_GOAL_CLASSIFIER_MAX", "garbage") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::remove_var("KIMIX_GOAL_CLASSIFIER_MAX") };
    }
    ...
    fn resolve_goal_classifier_max_runs_precedence_and_floor() {
        ...
        // env > config.
        unsafe { std::env::set_var("KIMIX_GOAL_CLASSIFIER_MAX", "4") };
    ...
    fn resolve_strategist_every_default_tracks_cap_floored_at_one() {
        unsafe { std::env::remove_var("KIMIX_GOAL_STRATEGIST_EVERY") };
    ...
    fn resolve_strategist_every_precedence_and_floor() {
        ...
        // env > config + remote.
        unsafe { std::env::set_var("KIMIX_GOAL_STRATEGIST_EVERY", "7") };
        let r = cfg_strategist(Some(3), Some(4)).resolve_goal_strategist_every(10);
        assert_eq!(r.value, 7);
        assert_eq!(r.source, ConfigSource::Env);
        // invalid env falls through to the default (cap/2).
        unsafe { std::env::set_var("KIMIX_GOAL_STRATEGIST_EVERY", "not-a-number") };
        assert_eq!(
            cfg_strategist(None, None)
                .resolve_goal_strategist_every(10)
                .value,
    ...
    async fn run_one_skeptic_fails_closed_when_scratch_root_squatted() {
        ...
        let inputs = SkepticInputs {
            ...
            attempt: 1,
            kind_lens: "",
            implementer_scratch: "/tmp/kimix-goal-test/implementer",
            scratch_dir_ready: true,
            prior_gaps: None,
        };
