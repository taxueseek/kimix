...
//!
//! Hooks registered at `session/new` (`_meta["Kimix/hooks"]`) come in two flavors,
//! both matched by the agent ([`kimix_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **`PreToolUse` gate**: an awaited reverse *request* `Kimix/hooks/run`; a `deny`
//!   blocks the tool.
//! - **All other events**: fire-and-forget *notifications* `Kimix/hooks/event`,
...
use kimix_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
...
const HOOK_EVENT_METHOD: &str = "kimix/hooks/event";
const HOOK_RUN_METHOD: &str = "kimix/hooks/run";

/// Default per-callback bound for a client's `Kimix/hooks/run` reply; on timeout the gate
...
/// Outcome of the `Kimix/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "kimix/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed kimix/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "kimix/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("kimix/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
impl SessionActor {
    ...
    /// `Kimix/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = kimix_hooks::dispatcher::extract_tool_name(envelope)
        ...
        let mut pending: FuturesUnordered<_> = groups
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    let _started = tokio::time::Instant::now();
    ...
    /// Issue one `Kimix/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `Kimix/hooks/event` notification to each matching registered callback.
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    kimix_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
