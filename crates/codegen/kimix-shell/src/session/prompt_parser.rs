...
use kimix_workspace::file_system::{
...
///
/// Some templates put `<user_query>` last (context first); Kimix puts it first.
...
pub struct ParsedPrompt {
    ...
    /// Kimix mode may include editor open/focus metadata; the compat mode does not.
...
impl ParsedPrompt {
    ...
    ///
    /// Layout:
    /// - **Kimix mode:** `<user_query>` + `<skill_information>` + context
...
///
/// When `is_cursor` is false, produces original Kimix-format output:
...
{}
    ...
    if !resource_links.is_empty() {
        ...
        context.push_str(&render_resource_links_kimix(resource_links));
    }
    (context, query)
}
...
/// Kimix-format resource links: `<focused_files>` / `<open_files>` with
/// metadata inside a `<system-reminder>` wrapper.
fn render_resource_links_kimix(resource_links: &[acp::ResourceLink]) -> String {
...
mod tests {
    ...
    /// Shorthand: render + assemble for Kimix mode.
    fn render_kimix(
        message: &str,
        embedded: Vec<String>,
    ...
    #[test]
    fn test_kimix_render_plain_message() {
        let result = render_kimix("hello", vec![], vec![], &[], false);
        assert_eq!(result, "<user_query>\nhello\n</user_query>");
    }
    #[test]
    fn test_kimix_render_with_attachments_uses_system_reminder_wrapper() {
        let result = render_kimix(
            "check this",
        ...
        assert!(
            ...
            "Kimix should start with <user_query>, got: {result}"
        );
    }
    #[test]
    fn test_kimix_render_user_query_first() {
        let link = acp::ResourceLink::new("doc.md", "file:///doc.md")
            .title(Some("My Doc".into()))
            .size(Some(1024));
        let result = render_kimix("hello", vec![], vec![], &[link], false);
        let uq_pos = result.find("<user_query>").unwrap();
        let rr_pos = result.find("Referenced resources:").unwrap();
        assert!(
            uq_pos < rr_pos,
            "Kimix: <user_query> ({uq_pos}) should come before resource links ({rr_pos})\ngot: {result}"
        );
        assert!(result.contains("<system-reminder>"));
    }
    #[test]
    fn test_kimix_render_resource_links_use_focused_files_format() {
        let links = vec![
        ...
        let result = render_kimix("hello", vec![], vec![], &links, false);
