...
pub use kimix_shell_base::session_types::{ContextInfo, TokenUsageCategory, count_detail};
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `kimix`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    model == kimix_models::PlatformId::KimiCode.managed_model_key(crate::models::default_model())
...
/// Full wire response for `Kimix/session/info`.
///
...
mod tests {
    ...
    fn desktop_client_type_deserializes_and_round_trips() {
        ...
        let submission = input.to_submission(Some("kimix-3".into()), None, None, Some(5));
        assert_eq!(
