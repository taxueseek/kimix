...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: kimix_tools::implementations::Kimix::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"Kimix-plan"`)
...
mod tests {
    ...
    fn planner_template_default_render_preserves_wording_and_has_no_placeholders() {
        ...
        // (Kimix) tool names; guards against accidental wording drift.
    ...
    impl GoalPlannerSpawner for MockSpawner {
        async fn spawn_planner(
            ...
            *self.last_prompt.lock().expect("lock poisoned") = Some(prompt.primary);
            if self.write_plan {
    ...
    async fn fail_open_retry_emits_spawn_failed_event() {
        ...
        let writer = kimix_file_utils::events::EventWriter::open(dir.path());
        let ov = RoleSpawnOverride {
            model: Some("m".into()),
            agent_type: Some("t".into()),
        };
