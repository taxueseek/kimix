//! In-process SDK MCP servers over the ACP reverse channel (`Kimix/mcp/sdk_call`).
//!
//! The official `Kimix-agent-sdk` lets a host define in-process tools (`@tool` /
//! `create_sdk_mcp_server`). When `transport="acp"`, the SDK registers them in
//! `session/new` `_meta["Kimix/mcp/servers"] = [{ "name", "serverId" }]` and the agent
//! invokes their tools by sending each MCP JSON-RPC message back to the client as a
//! reverse `Kimix/mcp/sdk_call` request — handled here by [`GatewayAcpInvoker`].
//!
//! NOTE: the *reverse* route (agent -> client, `Kimix/mcp/sdk_call`) invokes a tool that
//! lives in the SDK's process. It is the zero-IPC mirror of the *forward* route (client
//! -> agent, `Kimix/mcp/call` in `extensions::mcp`), which invokes a tool on a server the
...
use kimix_acp_lib::AcpAgentGatewaySender;
use kimix_mcp::acp_transport::AcpReverseInvoker;
use kimix_mcp::servers::AcpServerEntry;
use kimix_mcp::wire;

/// Parse `_meta["Kimix/mcp/servers"]` into [`AcpServerEntry`] registrations. Each entry
...
pub fn parse_acp_mcp_servers(meta: Option<&acp::Meta>) -> Vec<AcpServerEntry> {
    ...
    for entry in array {
        let server: AcpServerEntry = match serde_json::from_value(entry.clone()) {
            Ok(server) => server,
            Err(err) => {
                tracing::warn!(entry = %entry, %err, "ignoring malformed kimix/mcp/servers entry");
                continue;
            }
        };
        if !seen.insert(server.name.clone()) {
            tracing::warn!(name = %server.name, "ignoring duplicate kimix/mcp/servers entry");
            continue;
        }
        servers.push(server);
    }
...
///
/// Each [`invoke`](AcpReverseInvoker::invoke) sends one `Kimix/mcp/sdk_call` reverse request
...
/// Reverse `Kimix/mcp/sdk_call` params. Declares the on-wire field names once (mirrors
