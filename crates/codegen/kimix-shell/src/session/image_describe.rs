...
//! content blocks, the session calls a vision-capable Kimix model
...
use kimix_chat_state::compaction_utils::{extract_real_user_queries, extract_user_query};
use kimix_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
use kimix_tools::util::truncate::truncate_middle;
...
mod tests {
    use super::*;
    use kimix_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![kimix_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
