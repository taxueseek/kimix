    ...
    builder
        ...
        .filter_entry(|entry| {
            ...
            entry
                ...
                .map(|name| {
                    name != ".git"
                        && !kimix_file_utils::skip_dir_set().contains(name.to_lowercase().as_str())
                })
                .unwrap_or(true)
        });
...
mod tests {
    use super::*;
    use kimix_sampling_types::{AssistantItem, UserItem};
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![kimix_sampling_types::ContentPart::Text { text: text.into() }],
    ...
    fn evidence_packet_with_file_path_renders_path_not_content() {
        let packet = build_classifier_evidence_packet(
            ...
            Some(Path::new("/home/u/.kimix/sessions/s1/goal/plan.md")),
            None,
            "I did it.",
        );
        assert_eq!(
            ...
            "OBJECTIVE:\ndo X\n\n\
             ...
             CHANGED_FILES:\n- js/main.js\n\n\
             PLAN_FILE: /home/u/.kimix/sessions/s1/goal/plan.md\n\n\
             PLAN_CHANGES: (none)\n\n\
             FINAL_RESPONSE:\nI did it.\n",
        );
    }
    ...
    fn evidence_packet_plan_path_with_spaces_and_unicode_round_trips() {
        let plan = "/tmp/kimix sessions/✓ goal/plan.md";
        let packet = build_classifier_evidence_packet(
            "obj",
            ChangesRef::Unavailable,
            &[],
    ...
    async fn capture_changes_diff_walkdir_skips_all_well_known_directories() {
        let tmp = tempfile::tempdir().unwrap();
        let goal_created_at = now_unix_seconds() - 60;
        for sub in kimix_file_utils::SKIP_DIR_NAMES {
