...
use kimix_agent_lifecycle::LocalExtensionRegistryBuilder;
use kimix_agent_lifecycle::{LocalSessionLifecycleContributor, LocalTurnLifecycleContributor};
use kimix_agent_lifecycle::{
...
/// `KIMIX_IDLE_NOTIFICATION_DELAY_MS` overrides it (used by E2E tests).
fn idle_notification_delay() -> Duration {
    resolve_idle_notification_delay(std::env::var("KIMIX_IDLE_NOTIFICATION_DELAY_MS").ok())
}
...
/// Covered by the headless E2E via `KIMIX_IDLE_NOTIFICATION_DELAY_MS`.
...
mod idle_notification_delay_tests {
    ...
    /// Pins the public `KIMIX_IDLE_NOTIFICATION_DELAY_MS` contract: a valid override is interpreted as milliseconds (the E2E seam depends on this).
    #[test]
