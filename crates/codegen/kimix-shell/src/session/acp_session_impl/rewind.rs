...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &kimix_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            kimix_workspace::session::file_state::FlexiblePath,
            Option<String>,
    ...
    /// Out-of-band history repair (`Kimix/session/repair`) for a resident
    /// session: run `kimix_chat_state::compaction_utils::repair_history` inside
    ...
    pub(super) async fn handle_repair_history(
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(kimix_chat_state::commands::RepairHistoryBlocked);
        }
