...
impl SessionActor {
    ...
    /// Emit per-server `Kimix/mcp/tools_changed` notifications.
    ///
    ...
    /// Handle explicit auth trigger from the client (Kimix/mcp/auth_trigger).
    ///
    ...
    /// or removed it from `~/.kimix/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.kimix/config.toml` + the project
    /// `.Kimix/config.toml`) on every call. The auto-restart task
    /// calls this at most:
    ...
    /// `KimixClientHandler`, and the liveness watcher is armed —
    ...
    ///
    /// The `KimixClientHandler` constructed inside `try_handshake`
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(kimix_mcp::servers::McpClientEvent::ToolsChanged {
                server: server.to_string(),
            });
        }
        let arc_client = std::sync::Arc::new(new_client);
        let _ = arc_client
            .arm_liveness_watcher(kimix_mcp::liveness::DEFAULT_POLL_INTERVAL)
            .await;
        {
            let mut mcp_state = self.mcp_state.lock().await;
            mcp_state
    ...
    pub(super) async fn ensure_mcp_tools_initialized(&self) {
        ...
        if mcp_server_configs.is_empty() && !has_acp {
            ...
            if mcp_state.generation() == generation {
                ...
                self.events
                    .emit(kimix_file_utils::events::Event::McpInitCancelled {
        ...
        let mcp_clients: Vec<_> = mcp_results
            ...
            .filter_map(|result| match result {
                ...
                Err(e) => {
                    ...
                    self.events
                        .emit(kimix_file_utils::events::Event::McpServerFailed {
                            server_name: sname,
        ...
        let _mcp_strategy = self.mcp_strategy;
        ...
        tokio::task::spawn_local(async move {
            ...
            for client in mcp_clients.iter() {
                ...
                futs.push(async move {
                    ...
                    ew.emit(kimix_file_utils::events::Event::McpServerStarting {
            ...
            > = std::collections::HashMap::new();
            {
                ...
                if mcp_state.generation() != generation {
                    ...
                    event_writer.emit(kimix_file_utils::events::Event::McpInitCancelled {
                        reason: MCP_INIT_CANCELLED_CONFIG_CHANGED.to_string(),
                    });
                    return;
                }
                    ...
                    match result {
                        Ok((server_name, registrations, elapsed, timeout_sec)) => {
                            ...
                            for reg in registrations {
                                ...
                                if reg.model_visible {
                                        ...
                                        .await
                                    {
                                        ...
                                        event_writer
                                            .emit(kimix_file_utils::events::Event::McpToolRegistrationFailed {
                                                server_name: server_name.clone(),
                            ...
                            let transport_str = server_transport_map
                                ...
                                .unwrap_or("unknown");
                            event_writer.emit(
                                kimix_file_utils::events::Event::McpServerConnected {
                                    server_name: server_name.clone(),
                        ...
                        Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                            ...
                            let error_type_label = match error_cat {
                                kimix_file_utils::events::McpErrorCategory::AuthRequired => "auth",
                                kimix_file_utils::events::McpErrorCategory::Timeout => "timeout",
                                _ => "handshake_failed",
                            };
                            let transport_str = server_transport_map
                                .get(server_name.as_str())
                            ...
                            event_writer.emit(kimix_file_utils::events::Event::McpServerFailed {
                                server_name: server_name.clone(),
                            ...
                            let detail = (!needs_auth).then(|| {
                                kimix_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                    .into_owned()
                            });
                ...
                event_writer.emit(kimix_file_utils::events::Event::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<kimix_tools::types::tool_index::ServerSummary> {
        use kimix_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            kimix_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
        if let Some(hint) = self.rendered_mcp_hint().await {
            text.push_str(&hint);
        }
