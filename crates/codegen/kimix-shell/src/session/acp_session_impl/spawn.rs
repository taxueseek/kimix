...
fn drop_cli_catchall_allows(
    rules: Vec<kimix_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<kimix_workspace::permission::types::PermissionRule>,
    Vec<kimix_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if kimix_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use kimix_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use kimix_workspace::permission::rules::parse_permission_rule;
    use kimix_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: kimix_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<kimix_sampler::SharedAttributionCallback>,
    ...
    web_search_config: kimix_tools::implementations::WebSearchConfig,
    web_fetch_config: kimix_tools::implementations::kimix::web_fetch::WebFetchConfig,
    app_builder_deployer_config: kimix_tools::implementations::kimix::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: kimix_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<kimix_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<kimix_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<kimix_workspace::permission::PermissionHandle>,
    api_key_provider: Option<kimix_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<kimix_hooks::discovery::HookRegistry>>,
    workspace_ops: kimix_workspace::WorkspaceOps,
    cli_permission_rules: Vec<kimix_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn kimix_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        kimix_tools::implementations::kimix::scheduler::types::SchedulerHandle,
    >,
    max_turns: Option<usize>,
    forked_tool_override: Option<Vec<ToolSpec>>,
) -> Result<(SessionHandle, String, tokio::sync::oneshot::Receiver<()>), kimix_agent::AgentBuildError>
{
    if max_turns == Some(0) {
        return Err(kimix_agent::AgentBuildError::InvalidConfig(
    ...
    let (permissions, deny_read_globs) = if let Some(handle) = inherited_permission_handle {
        ...
        let mut permission_config =
            kimix_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = kimix_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("kimix: --allow catch-all ignored: {reason}");
            }
        }
        if !cli_permission_rules.is_empty() {
            match &mut permission_config {
                ...
                None => {
                    permission_config =
                        Some(kimix_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(kimix_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let (permissions, _permission_events_rx) =
            kimix_workspace::permission::spawn_permission_manager(
    ...
    let web_search_config = if disable_web_search {
        kimix_tools::implementations::WebSearchConfig::Disabled
    } else {
        web_search_config
    };
    ...
    let context_window_override = std::env::var("KIMIX_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "KIMIX_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = kimix_sampling_types::SamplingConfig {
    ...
    let chat_state_handle = kimix_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let auto_wake_delivered =
        kimix_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
    ...
    let index_root_for_session =
        kimix_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        kimix_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    ...
    let terminal_backend: std::sync::Arc<dyn kimix_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn kimix_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn kimix_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(kimix_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(kimix_tools::computer::local::LocalFs)
        };
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        kimix_tools::implementations::kimix::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn kimix_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : kimix_log::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : kimix_log::memory_log::TARGET, removed,
        ...
        let params = crate::session::memory::MemoryBackendParams {
            ...
            auth_credentials: auth_manager.as_ref().map(|am| {
                ...
                ) as std::sync::Arc<dyn kimix_auth::AuthCredentialProvider>
            }),
        };
        ...
        let _mc = memory_config.as_ref();
        let _total_chunks = storage.total_chunk_count();
        Some(backend)
    } else {
        tracing::debug!(
            target : kimix_log::memory_log::TARGET,
    ...
    let mcp_state = {
        ...
        if !acp_mcp_servers.is_empty() {
            ...
            tracing::info!(
                ...
                "Registered in-process SDK MCP servers (kimix/mcp/sdk_call)"
            );
        }
    ...
    let resolved_task_output =
        kimix_tools::reminders::task_completion::resolve_task_output_tool_name(agent.tool_bridge())
            .await;
    let resolved_read =
        kimix_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge()).await;
    ...
    let scheduler_handle_for_handle = {
        ...
        res.get::<kimix_tools::implementations::kimix::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::KimixTUI => crate::session::feedback_types::ClientType::Tui,
        ClientType::KimixWeb => crate::session::feedback_types::ClientType::Web,
        ...
        ClientType::KimixPager => crate::session::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = kimix_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = kimix_sampler::RetryPolicy {
        max_retries: max_retries.unwrap_or(5),
        rate_limit_retry_threshold: 2,
    };
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<kimix_sampler::SamplingEvent>();
    let sampler_handle = kimix_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<kimix_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<kimix_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = kimix_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        kimix_tools::implementations::kimix::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: kimix_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match kimix_workspace::session::git::discover_git_root(root) {
                kimix_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    kimix_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => kimix_workspace::session::git::VcsKind::None,
            }
        },
        ...
        queue_persistence_enabled: remote_settings
            .as_ref()
            .and_then(|s| s.queue_persistence_enabled)
            .unwrap_or(true),
        queue_writer: if remote_settings
            .as_ref()
            .and_then(|s| s.queue_persistence_enabled)
            .unwrap_or(true)
        {
            kimix_file_utils::events::QueueEventWriter::open(
                &crate::session::persistence::session_dir(&session_info),
            )
        } else {
            kimix_file_utils::events::QueueEventWriter::noop()
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(kimix_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(kimix_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            kimix_tools::implementations::kimix::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        let _session_id_for_reindex = session_info.id.to_string();
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : kimix_log::memory_log::TARGET, files = files.len(),
    ...
    }
    {
        ...
        use kimix_tools::implementations::kimix::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use kimix_tools::implementations::kimix::ask_user_question::AskUserQuestionMode;
    ...
    tokio::task::spawn_local(async move {
        kimix_log::session_ctx::with_session_ctx(
            &telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
pub(crate) async fn spawn_session_on_thread(
    ...
    let (init_tx, init_rx) =
        tokio::sync::oneshot::channel::<Result<SessionInitResult, kimix_agent::AgentBuildError>>();
            ...
            local.block_on(&rt, async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent" : tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = kimix_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
