...
impl SessionActor {
    ...
    pub(super) fn broadcast_queue_changed(&self, state: &State) {
        ...
        tracing::debug!(
            ...
            session = self.session_info.id.0.as_ref(),
            "broadcasting kimix/queue/changed to subscribers",
        );
    ...
    ///
    /// Always re-broadcasts `Kimix/queue/changed` so every client reconciles
    ...
    /// 3. Re-broadcast `Kimix/queue/changed` so every subscriber renders the
