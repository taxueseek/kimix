...
pub(super) struct MemoryFlushSnapshot {
    counts: kimix_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &kimix_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                kimix_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                kimix_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        &self,
        _telem: &super::memory_state::MemoryTelemetry,
        _total_chunks_at_end: usize,
        _session_end_result: &str,
    ) {
    }
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_kimix_req_id: Some(format!("xai-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: kimix_log::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_kimix_conv_id: Some(session_id.clone()),
                x_kimix_req_id: Some(format!("xai-flush-{}", uuid::Uuid::new_v4())),
                x_kimix_session_id: Some(session_id.clone()),
                x_kimix_agent_id: Some(crate::util::agent_id::agent_id()),
                ..Default::default()
            };
        ...
        let (outcome, _response_len, _accepted_len, _was_truncated, flush_path) = match result {
            Ok(response_text) => {
        ...
        tracing::info!(target: kimix_log::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
    ...
    /// LLM call using the `kimix` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    kimix_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                kimix_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events =
                    kimix_sampler::stream_responses(raw, meta, request_id, idle_timeout, doom_loop);
                kimix_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = kimix_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                kimix_sampler::collect_response(events).await
            }
        };
