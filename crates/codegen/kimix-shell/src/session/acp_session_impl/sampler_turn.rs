...
/// `SamplingError::is_auth_error` in Kimix-sampling-types: 403 is
...
pub(super) fn is_auth_tool_error(err: &kimix_tool_runtime::ToolError) -> bool {
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<kimix_tools::types::output::ToolRunResult, kimix_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                kimix_tools::types::output::ToolRunResult,
                kimix_tool_runtime::ToolError,
            >,
        >,
{
...
impl SessionActor {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl kimix_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = kimix_file_utils::trace_context::current_traceparent()
        ...
        impl kimix_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| kimix_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        // CRITICAL: never attach AuthManagerBearerResolver for Grok
        // cli-chat-proxy. AuthManager holds the Kimi Code OAuth token;
        // the sampler's bearer_resolver overrides api_key on every request
        // and would send the Kimi JWT → 401 "no auth context".
        let is_grok_proxy = crate::auth::is_cli_chat_proxy_url(&cfg.base_url);
        let use_bearer_resolver = gate.active() && !is_grok_proxy;
        self.log_auth_gate_unknown("reconstruct_full_config", gate, &cfg.base_url);
        // For Grok proxy, always re-resolve the live Grok CLI token into
        // credentials so recovery/model-switch cannot leave a stale Kimi key.
        if is_grok_proxy {
            if let Some(tok) = crate::auth::load_grok_session_token() {
                creds.api_key = Some(tok.access_token);
            } else {
                let auth_path = crate::util::kimix_home::kimix_home().join("auth.json");
                if let Some(t) = crate::auth::load_xai_session_token_sync(&auth_path) {
                    creds.api_key = Some(t);
                }
            }
            // Persist so recovery retries and concurrent turns see the same key.
            self.chat_state_handle.update_credentials(creds.clone());
        }
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> kimix_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<kimix_workspace::permission::ClassifierMessage>,
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            kimix_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            kimix_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            kimix_workspace::permission::classifier_output_json_schema(),
                        ),
                        reasoning_effort: classifier_reasoning_effort,
                        x_kimix_conv_id: Some(session_id.clone()),
                        x_kimix_req_id: Some(format!("xai-perm-auto-{}", uuid::Uuid::new_v4())),
                        x_kimix_session_id: Some(session_id),
        ...
        let clf =
            kimix_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<kimix_sampler::SamplerConfig> {
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(kimix_sampler::SamplingClient, String)> {
        ...
        let client = kimix_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
                tracing::warn!(
                    error = % e,
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<kimix_sampler::SamplingClient, acp::Error> {
    ...
    /// `Kimix-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        ...
        // Grok / xAI cli-chat-proxy sessions must NOT refresh the Kimi Code
        // OAuth session (that was the dual-login footgun: 401 on Grok →
        // Kimi refresh → still 401, and users were sent to `kimix login`).
        let base_url_for_recovery = self
            .chat_state_handle
            .get_sampling_config()
            .await
            .map(|c| c.base_url)
            .unwrap_or_default();
        let is_xai_proxy = crate::auth::is_cli_chat_proxy_url(&base_url_for_recovery);

        if matches!(error.kind, SamplingErrorKind::Auth) && is_xai_proxy {
            // Prefer ~/.grok (cli-chat-proxy requires Grok CLI tokens).
            let auth_path = crate::util::kimix_home::kimix_home().join("auth.json");
            let recovered = crate::auth::load_grok_session_token()
                .map(|t| {
                    let _ = crate::auth::import_grok_token_into_kimix(&auth_path, &t);
                    t.access_token
                })
                .or_else(|| crate::auth::load_xai_session_token_sync(&auth_path));
            if let Some(token) = recovered {
                // Write into chat_state credentials so reconstruct_full_config
                // (which no longer uses AuthManager for Grok) picks it up.
                let mut creds = self.chat_state_handle.get_credentials().await;
                creds.api_key = Some(token);
                self.chat_state_handle.update_credentials(creds);
                tracing::info!(
                    session_id = %self.session_info.id.0,
                    "auth recovery: Grok CLI session reloaded into credentials, retrying"
                );
                kimix_log::unified_log::info(
                    "auth recovery: Grok CLI session reloaded into credentials, retrying",
                    Some(self.session_info.id.0.as_ref()),
            ...
            tracing::warn!(
                session_id = %self.session_info.id.0,
                "auth recovery: no Grok CLI token — run `grok login` (required for cli-chat-proxy)"
            );
            // Fall through to surface the 401; do not run Kimi recovery.
        ...
        let client_version = kimix_version::VERSION;
        let is_model_404 =
        ...
        let error_type = if kimix_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = kimix_sampler::RequestId::random();
