...
/// - `file_path` — Kimix (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), kimix_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — Kimix (`read_file`, via `#[serde(rename)]`)
///
...
/// `list_dir`=`target_directory`. Grammar lives in `kimix_chat_state`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<kimix_chat_state::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    kimix_chat_state::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = kimix_tools::util::strip_redundant_session_cd(
            &command,
        ...
        // execute tool by kind so the Kimix/tool identity still stamps.
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(kimix_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from Kimix-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, Kimix-shell would sanitize
