...
impl SessionActor {
    ...
    ) -> Option<kimix_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
        let is_cursor = self.is_cursor_harness();
        if is_cursor {
            bridge.set_skill_listing_xml_format(true).await;
        }
        // Clear stale system-reminders before injecting the new baseline.
        conversation.retain(|item| {
            !matches!(
                item, ConversationItem::User(u) if u.synthetic_reason ==
                Some(kimix_sampling_types::SyntheticReason::SystemReminder)
            )
        });
        // NOTE: if apply_pending_skill_update returns None and reminders were
        // cleared above, the baseline is lost until the next skill change.
        // This is intentional: on first call there is always a pending update;
        // subsequent calls with no changes correctly produce no reminder.
    ...
    pub(super) async fn reload_skills_from_disk(&self) -> usize {
        ...
        let new_skills = kimix_agent::prompt::skills::list_skills_with_plugins(
            Some(cwd),
            &skills_config,
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &kimix_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ) -> Option<ConversationItem> {
        use kimix_tools::types::skill_discovery_tracker::SkillUpdateKind;
        let is_cursor = self.is_cursor_harness();
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn kimix_auth::AuthCredentialProvider> = Arc::new(
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = kimix_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(kimix_tools::types::resources::DenyReadGlobs(
    ...
    pub(super) async fn build_session_info(&self) -> SessionInfoData {
        ...
        let system_prompt_tokens = system_message
            .as_ref()
            .map(kimix_chat_state::estimate_system_message_tokens)
            .unwrap_or(0);
        ...
        let tool_definitions_tokens =
            kimix_chat_state::estimate_tool_definitions_tokens(&tool_defs);
        ...
        let free_tokens = kimix_token_estimation::free_tokens(context_window, total_tokens);
        let usage_pct = kimix_token_estimation::usage_percentage_u8(total_tokens, context_window);
