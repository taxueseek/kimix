...
impl SessionActor {
    ...
    /// sees "Kimix is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let request = ConversationRequest {
            ...
            // Don't pass `reasoning_effort` — `Kimix-4.5` (and
            // other tool-flavoured Kimix variants) reject the field at
            ...
            x_kimix_conv_id: Some(session_id_str.clone()),
            x_kimix_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_kimix_session_id: Some(session_id_str),
