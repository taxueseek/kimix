...
impl SessionActor {
    ...
    /// Emit `Kimix/git_head_changed` after an edit/shell command that may have
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            kimix_workspace::session::git::get_worktree_info(cwd),
            kimix_workspace::session::git::get_branch(cwd),
        );
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("kimix/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<kimix_tools::implementations::kimix::task::types::SubagentOutstandingReply> {
    ...
    pub(super) async fn handle_completion(&self, prompt_id: String, result: PromptTurnResult) {
        ...
        // replayed `_Kimix/session/update` rail so a viewer that re-attaches
    ...
    pub(super) fn classify_install_error(
        err: &kimix_agent::plugins::install_registry::InstallError,
    ) -> String {
