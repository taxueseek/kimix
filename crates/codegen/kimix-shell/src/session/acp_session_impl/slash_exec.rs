...
impl SessionActor {
    ...
    pub(super) async fn execute_builtin_slash_command(
        ...
        match action {
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.kimix/.",
                    )
                    .await;
                } else {
                    // CWE-427: Use shared add_hooks_path() which validates
                    // paths are under ~/.kimix/ to prevent hook path injection.
            ...
            BuiltinAction::SessionInfo => {
                ...
                let context_pct = kimix_token_estimation::usage_percentage(ctx.used, ctx.total);
                ...
                if source.is_empty() {
                    ...
                    if !trust {
                        ...
                        let source_desc = match &install_source {
                            kimix_agent::plugins::git_install::InstallSource::Git {
                                url, ..
                            } => {
                                format!("remote git repo: {url}")
                            }
                            kimix_agent::plugins::git_install::InstallSource::Local {
                                path,
                                ..
                            } => {
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn kimix_tools::types::memory_backend::MemoryBackend,
    ...
    async fn execute_feedback_command(self: &Arc<Self>, text: String) -> PromptTurnResult {
        ...
        let (sampling_config, model_metadata, _credentials) = tokio::join!(
        ...
        let client_version = Some(kimix_version::VERSION.to_string());
