...
impl SessionActor {
    ...
    pub(super) async fn handle_session_mode(&self, session_mode_id: acp::SessionModeId) {
        ...
        *self.current_prompt_mode.lock() = prompt_mode;
        ...
        if mode.is_decision() {
            self.inject_decision_mode_prompt().await;
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                SessionMode::Decision.as_id(),
            ));
            tracing::info!(
    ...
    /// Inject the Decision mode analysis-framework prompt into the conversation.
    ///
    /// Pushes a system-reminder that instructs the model to apply MECE
    /// enumeration, quantitative trade-off analysis, and expected-value
    /// convergence before executing. Called once when entering Decision mode
    /// via Shift+Tab.
    ///
    /// Decision mode answers "which one?" — given a goal and current state,
    /// enumerate all feasible paths (MECE), evaluate each with expected value
    /// (benefit × probability − cost), and converge on the optimal path.
    async fn inject_decision_mode_prompt(&self) {
        let tag = self.reminder_wrapper_tag();
        let content = format!(
            "<{tag}>
You are now in Decision mode. This mode answers: **which path to take?**

Apply this framework to every task:

1. **Enumerate options (MECE)** — list ALL feasible approaches, ensuring Mutually Exclusive and Collectively Exhaustive coverage. Never jump to the first idea.

2. **Evaluate each option** — for each approach, assess:
   - Benefit: what value does it deliver? (high/medium/low)
   - Cost: time, complexity, risk (high/medium/low)
   - Success probability: how likely to work? (0-100%)
   - Expected value = Benefit × Probability − Cost

3. **Converge on optimal** — pick the option with the highest expected value. Explain why.

4. **State your decision** — output a clear recommendation table before writing any code.

Output format:
```
## Decision Analysis

| Option | Benefit | Cost | Success % | Expected Value |
|--------|---------|------|-----------|----------------|
| A. ... | High | Med | 90% | ★★★ |
| B. ... | Med | Low | 95% | ★★ |
| C. ... | High | High | 70% | ★★ |
...
```
        ...
        self.push_system_reminder_with_tag(&content, tag);
        tracing::info!(
            session_id = % self.session_info.id.0,
            "Decision mode: injected MECE analysis-framework system-reminder"
        );
    }
    ...
    /// first-entry/reentry distinction), or Kimix's reentry/full variant.
