    ...
    pub(super) fn into_parts(
        ...
        Option<
            tokio::sync::oneshot::Sender<
                kimix_tools::implementations::kimix::update_goal::UpdateGoalAck,
            >,
        >,
    ) {
        match self {
...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: kimix_tools::implementations::kimix::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`KIMIX_GOAL_PLANNER`) and a plan
...
/// Override with `KIMIX_GOAL_REVERIFY_AFTER` (floored at 1).
...
mod fold_tokens_by_model_tests {
    ...
    fn mixed_models_sum_marginals_sorted_desc() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("kimix-3")),
            rec(Some("g1"), 100, 500, Some("kimix-4")), // marginal 400
            rec(Some("g1"), 0, 50, Some("kimix-3")),    // kimix-3 total 150
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(
            out,
            vec![("kimix-4".to_owned(), 400), ("kimix-3".to_owned(), 150)]
        );
    }
    ...
    fn single_distinct_model_collapses_to_one_entry() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("kimix-4")),
            rec(Some("g1"), 0, 200, None), // folds under current = kimix-4
        ];
        let out = fold_tokens_by_model(&records, "g1", "kimix-4");
        assert_eq!(out, vec![("kimix-4".to_owned(), 300)]);
    }
    ...
    fn other_goal_records_excluded() {
        let records = vec![
            rec(Some("g1"), 0, 100, Some("kimix-4")),
            rec(Some("g2"), 0, 999, Some("kimix-4")),
            rec(None, 0, 999, Some("kimix-4")),
        ];
        let out = fold_tokens_by_model(&records, "g1", "cur");
        assert_eq!(out, vec![("kimix-4".to_owned(), 100)]);
    }

    #[test]
    fn last_below_anchor_does_not_underflow() {
        let records = vec![rec(Some("g1"), 500, 100, Some("kimix-4"))];
    ...
    fn captured_model_survives_mid_goal_current_model_switch() {
        // A record captured `Kimix-4` at spawn keeps it even though the
        // current model at aggregation time is `Kimix-3`.
        let records = vec![rec(Some("g1"), 0, 100, Some("kimix-4"))];
        let out = fold_tokens_by_model(&records, "g1", "kimix-3");
        assert_eq!(out, vec![("kimix-4".to_owned(), 100)]);
    }
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use kimix_tools::implementations::kimix::UPDATE_GOAL_TOOL_NAME;
...
impl SessionActor {
    ...
    pub(super) async fn set_goal_loop_active_resource(&self, active: bool) {
        ...
        self.agent
            ...
            .tool_bridge()
            .update_resource(
                kimix_tools::implementations::kimix::task::types::GoalLoopActive(active),
            )
            .await;
    }
}
