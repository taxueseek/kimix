...
) -> kimix_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> kimix_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use kimix_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> kimix_hooks::runner::RunContext<'_> {
        kimix_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[kimix_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, kimix_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: kimix_hooks::event::HookEventName,
        payload: kimix_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            kimix_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx).await;
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: kimix_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
