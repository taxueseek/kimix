...
/// Prefix on `x_kimix_req_id` for laziness-classifier sampler calls.
        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let kimix_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = kimix_sampling_types::reasoning_item_text(r);
...
const CLASSIFIER_TURN_MAX_LEN: usize = kimix_workspace::permission::CLASSIFIER_TURN_MAX_LEN;
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<kimix_workspace::permission::ClassifierTurn> {
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                if !text.is_empty() {
                    ...
                    let text =
                        kimix_tools::util::truncate_str_with_marker(&text, CLASSIFIER_TURN_MAX_LEN)
                            .into_owned();
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args =
                        kimix_tools::util::truncate_str_with_marker(&args, CLASSIFIER_TURN_MAX_LEN)
                            .into_owned();
