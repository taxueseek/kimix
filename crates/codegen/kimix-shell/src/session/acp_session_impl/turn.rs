...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<&kimix_tools::implementations::kimix::task::types::SubagentOutstandingReply>,
    ) -> Self {
        match reply {
            None => Self {
                fail_closed: true,
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        _prompt_client_identifier: Option<String>,
        _prompt_screen_mode: Option<String>,
        verbatim: bool,
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let turn_start_input = kimix_agent_lifecycle::TurnStartInput::new(
        ...
        self.send_before_turn_event(kimix_tool_protocol::turn_hook::BeforeTurnPayload {
            turn_number: self.chat_state_handle.get_prompt_index().await as u64,
            model_id: model_id.clone(),
            yolo_mode: self.permissions.is_yolo_mode(),
            conversation_message_count: msg_count,
            session_relationship: kimix_tool_protocol::turn_hook::DEFAULT_SESSION_RELATIONSHIP
                .to_string(),
            schema_version: crate::session::events::EVENT_SCHEMA_VERSION.to_string(),
        })
        .await;
        let _turn_idx = self.chat_state_handle.get_prompt_index().await as u64;
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = kimix_tools::util::base64_images::extract_base64_images(query);
        ...
        let mut user_message = if user_images.is_empty() {
            user_message
        ...
        self.dispatch_hook(
            kimix_hooks::event::HookEventName::UserPromptSubmit,
            kimix_hooks::event::HookPayload::UserPromptSubmit {
        ...
        let _doom_event_model = turn_model_id.clone();
        ...
        let _bridge_outcome = turn_result_to_hook_outcome(&result);
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                // ── Kimix: mark turn completed for prune tracking ───────
                crate::session::kimix_recall::record_assistant_response(&format!(
                    "[turn {} completed]",
                    current_prompt_index
                ));
                // ─────────────────────────────────────────────────────────
                ...
                self.send_after_turn_event(kimix_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: kimix_tool_protocol::turn_hook::TurnHookOutcome::Completed,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                self.send_after_turn_event(kimix_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: kimix_tool_protocol::turn_hook::TurnHookOutcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
            ...
            Err(err) => {
                ...
                self.send_after_turn_event(kimix_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: kimix_tool_protocol::turn_hook::TurnHookOutcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                .await;
                let _error_category = Self::classify_turn_error(err);
                self.dispatch_hook(
                    kimix_hooks::event::HookEventName::StopFailure,
                    kimix_hooks::event::HookPayload::StopFailure {
                        error: format!("{err}"),
                    },
                    Some(prompt_id),
                    None,
        ...
        self.dispatch_hook(
            kimix_hooks::event::HookEventName::Stop,
            kimix_hooks::event::HookPayload::Stop {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
                    contributor
                        .on_turn_done(&kimix_agent_lifecycle::TurnDoneInput)
                        .await;
                }
            }
            Ok(TurnOutcome::Cancelled { .. }) | Ok(TurnOutcome::MaxTurnsReached { .. }) => {
                let input = kimix_agent_lifecycle::TurnAbortInput::new(
                    kimix_agent_lifecycle::TurnAbortReason::Interrupted,
                );
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
            ...
            Err(err) => {
                ...
                let input = kimix_agent_lifecycle::TurnErrorInput { message: &message };
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
    ...
    /// monitor events only via the structured `Kimix/monitor_event` channel.
    pub(crate) async fn inject_pending_monitor_events(&self) {
        let Some(buffer) = &self.tool_context.monitor_event_buffer else {
            return;
        };
        let mine = kimix_tools::implementations::kimix::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = kimix_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = kimix_tools::reminders::wrap_reminder(&body);
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use kimix_tools::types::memory_backend::MemoryBackend as _;
        ...
        let _inject_start = std::time::Instant::now();
        ...
        let _top_score = inject_results
            .as_ref()
            .and_then(|r| r.first())
            .map_or(0.0, |r| r.score);
        let _total_snippet_chars: usize = inject_results
            .as_ref()
            .map_or(0, |r| r.iter().map(|s| s.snippet.len()).sum());
        tracing::info!(
            target : kimix_log::memory_log::TARGET, configured_min_score,
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<kimix_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        &self,
        req_id: &str,
        _conv_turn_start: std::time::Instant,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            request.x_kimix_session_id = Some(self.session_info.id.to_string());
            request.x_kimix_turn_idx =
                Some(self.chat_state_handle.get_prompt_index().await.to_string());
            request.x_kimix_agent_id = Some(crate::util::agent_id::agent_id());
            if request.x_kimix_deployment_id.is_none() {
                request.x_kimix_deployment_id = crate::managed_config::resolve_deployment_id(
            ...
            {
                let _model_id = self.current_model_id().await;
            }
...
mod user_echo_broadcast_tests {
    ...
    /// the text from the `Kimix/session/interjection` broadcast, so a live
