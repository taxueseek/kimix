...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => kimix_tool_types::task_output_waits_from_json(args),
...
fn interrupted_wait_tool_result_with_msg(args: &serde_json::Value, msg: &str) -> ToolRunResult {
    use kimix_tool_types::{TaskOutputOutput, TaskOutputResult};
    let task_id = args
        .get("task_ids")
...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// Kimix-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as Kimix edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &kimix_tools::implementations::kimix::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
///
/// Uses `kimix_acp_lib`'s TYPED [`AcpChannelFailure`](kimix_acp_lib::AcpChannelFailure)
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        kimix_acp_lib::acp_channel_failure(err),
        Some(kimix_acp_lib::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    /// Merge the canonical `Kimix/tool` identity envelope into a tool-call
    ...
    pub(super) fn stamp_tool_meta(
        ...
        kimix_tools::normalization::merge_tool_meta(
            &toolset,
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(kimix_file_utils::events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, result)) = dispatch_rx.recv().await {
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(kimix_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self.hook_event_active(kimix_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            kimix_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            kimix_hooks::event::HookEventName::PostToolUseFailure,
                            kimix_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    kimix_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    kimix_hooks::event::HookEventName::PostToolUse,
                    kimix_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(kimix_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                kimix_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                kimix_hooks::event::HookEventName::PreToolUse,
                None,
                kimix_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    kimix_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let kimix_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        kimix_file_utils::events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        kimix_file_utils::events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        kimix_file_utils::events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        kimix_file_utils::events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        kimix_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        kimix_hooks::event::HookEventName::PermissionDenied,
                        kimix_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    /// Issue the `Kimix/exit_plan_mode` reverse-request and await the user's
    ...
    pub(super) async fn request_plan_approval(
        ...
        kimix_tools::implementations::kimix::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use kimix_tools::implementations::kimix::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                kimix_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![acp::ToolCallLocation::new(read_file.path).line(
                    kimix_tools::normalization::norm_offset_i64(read_file.offset).map(|l| l as u32),
                )],
                Vec::new(),
            ),
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        kimix_tool_types::WaitMode::WaitAny => "wait_any",
                        kimix_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
    ...
    /// `kimix_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use kimix_tools::util::git_detect;
        match &result.output {
            kimix_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
            ...
            kimix_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: kimix_tools::util::git_detect::PrRef,
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            kimix_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                kimix_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let kimix_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                kimix_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | kimix_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            kimix_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            kimix_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`kimix_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: kimix_sampler::SamplingEvent,
    ) {
        use kimix_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == kimix_sampler::SamplingErrorKind::DoomLoopDetected {
...
fn execute_tool_call_parts(
    ...
    let display = kimix_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use kimix_tools::types::ToolInput;
    use kimix_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use kimix_tools::implementations::kimix::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use kimix_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// Kimix edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn kimix_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "kimix tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use kimix_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use kimix_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use kimix_tools::implementations::kimix::exit_plan_mode::ExitPlanModeExtResponse;
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&kimix_acp_lib::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    use kimix_tool_types::TaskOutputOutput;
    use kimix_tools::types::output::ToolOutput;
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use kimix_tool_types::PendingInterjection;
