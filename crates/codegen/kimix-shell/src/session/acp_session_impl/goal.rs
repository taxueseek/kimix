...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &kimix_tools::implementations::kimix::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<kimix_tools::implementations::kimix::update_goal::UpdateGoalEnvelope>,
    ) {
        use kimix_tools::implementations::kimix::update_goal::{RejectReason, UpdateGoalAck};
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        use kimix_tools::implementations::kimix::task::backend::{ChannelBackend, SubagentBackend};
        use kimix_tools::implementations::kimix::task::types::SubagentDescribeOutcome;
        ...
        //    silently run Kimix flavor. Non-strict names (Kimix
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if kimix_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    /// agrees with `from_summary` — `search_replace` on the default Kimix
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default Kimix
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use kimix_sampling_types::conversation::SyntheticReason;
