...
impl SessionActor {
    ...
    pub(super) fn do_hooks_trust_project(cwd: &str) -> Result<std::path::PathBuf, String> {
        let root =
            kimix_workspace::session::git::find_git_root_from_path(std::path::Path::new(cwd))
                .map_err(|_| {
    ...
    pub(super) async fn reseed_mcp_output_cap(&self) {
        ...
        let existing = resources
            .get::<kimix_tools::types::resources::TruncationCfg>()
            .map(|c| c.0.clone());
        match (resolved, existing) {
            (resolved, Some(mut cfg)) => {
                if cfg.mcp_max_output_bytes != resolved {
                    cfg.mcp_max_output_bytes = resolved;
                    resources.insert(kimix_tools::types::resources::TruncationCfg(cfg));
                }
            }
            (Some(v), None) => {
                resources.insert(kimix_tools::types::resources::TruncationCfg(
    ...
    pub(super) async fn handle_hooks_action(
        ...
        use kimix_hooks_plugins_types::{ActionOutcome, HooksAction, OutcomeStatus};
        ...
        match action {
            ...
            HooksAction::Add { path } => {
                ...
                // CWE-427: add_hooks_path() validates path is under ~/.kimix/.
            ...
            HooksAction::Disable { hook_name } => {
                match kimix_hooks::trust::disable_hook(&hook_name) {
                    Ok(()) => ActionOutcome {
                        status: OutcomeStatus::Success,
            ...
            HooksAction::Enable { hook_name } => {
                match kimix_hooks::trust::enable_hook(&hook_name) {
            ...
            HooksAction::ToggleSource {
                ...
                for name in &hook_names {
                    let ok = if disable {
                        kimix_hooks::trust::disable_hook(name).is_ok()
                    } else {
                        kimix_hooks::trust::enable_hook(name).is_ok()
                    };
                    if ok {
                        toggled += 1;
                    }
    ...
    pub(super) async fn handle_plugins_action(
        ...
        use kimix_hooks_plugins_types::{ActionOutcome, OutcomeStatus, PluginsAction};
        ...
        match action {
            ...
            PluginsAction::Install { source } => {
                ...
                let install_source =
                    kimix_agent::plugins::git_install::parse_install_source(&source, cwd);
                let registry = kimix_agent::plugins::InstallRegistry::load();
                match kimix_agent::plugins::git_install::install_from_source(
                    &install_source,
                    &registry,
                ) {
                    Ok(result) => {
                        let repo = kimix_agent::plugins::git_install::build_installed_repo(
                            &result,
                            &install_source,
                        );
            ...
            PluginsAction::Uninstall {
                ...
                let mut registry = kimix_agent::plugins::InstallRegistry::load();
                match registry.find_plugin(plugin_name) {
                    ...
                    Some((repo_key, repo, _plugin)) => {
                        ...
                        if let Err(e) =
                            kimix_agent::plugins::git_install::remove_repo_path(&repo_path)
                        {
            ...
            PluginsAction::Update { plugin_id } => {
                ...
                let repos_to_update: Vec<(
                    String,
                    kimix_agent::plugins::install_registry::InstalledRepo,
                ...
                for (key, repo) in &repos_to_update {
                    match kimix_agent::plugins::git_install::update_repo(key, repo) {
                        Ok(status) => {
                            use kimix_agent::plugins::git_install::UpdateStatus;
                            match status {
        ...
        if let Some(ref pr) = plugin_registry_snapshot {
            for plugin in pr.active_plugins() {
                if let Some(ref hooks_path) = plugin.hooks_path {
                    let (specs, warnings) = kimix_agent::plugins::hooks_adapter::parse_plugin_hooks(
                        hooks_path,
                        &plugin.name,
                        &plugin.root_str(),
                        &plugin.data_dir_str(),
                ...
                if let Some(ref inline_value) = plugin.inline_hooks {
                    let (specs, warnings) =
                        kimix_agent::plugins::hooks_adapter::parse_plugin_hooks_from_value(
                            inline_value,
                            &plugin.name,
                            &plugin.root_str(),
                            &plugin.data_dir_str(),
    ...
    pub(super) async fn reload_plugins_impl(
        self: &Arc<Self>,
        handle: &kimix_agent::plugins::SharedPluginRegistryHandle,
        force: bool,
    ) -> String {
        let session_cwd = std::path::Path::new(&self.session_info.cwd);

        let sid = self.session_info.id.0.as_ref();
        kimix_log::unified_log::info("reload_plugins_impl: start", Some(sid), None);
    ...
    pub(crate) fn preserve_session_plugin_dirs(
        &self,
        incoming: Option<std::sync::Arc<kimix_agent::plugins::PluginRegistry>>,
    ...
    pub(super) async fn apply_plugin_registry_snapshot(
        self: &Arc<Self>,
        new_registry_snapshot: Option<std::sync::Arc<kimix_agent::plugins::PluginRegistry>>,
    ) -> (usize, bool, usize) {
        let sid = self.session_info.id.0.as_ref();
        let session_cwd = std::path::Path::new(&self.session_info.cwd);
        ...
        *self.plugin_registry.borrow_mut() = new_registry_snapshot.clone();
        ...
        if let Some(ref new_registry) = new_registry_snapshot {
            ...
            hooks_reloaded = new_specs.len();
            {
                ...
                if let Some(ref mut arc_reg) = *reg {
                    ...
                    let (mut new_reg, _) =
                        kimix_hooks::discovery::load_hooks_from_sources(&[], &[]);
        ...
        let mcp_changed = if let Some(diff) = mcp_diff {
            ...
            {
                let _ = tx.send(kimix_mcp::servers::McpClientEvent::ConfigDiff {
