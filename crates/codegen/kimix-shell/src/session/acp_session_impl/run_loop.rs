...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<kimix_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<kimix_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
                   ...
                   } else { tracing::debug!(target : kimix_log::memory_log::TARGET,
                   ...
                   kimix_log::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
                   ...
                   Some(kimix_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
                   ...
                   Some(kimix_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
                   reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
                   body_bytes_after, }) => {
                   kimix_log::unified_log::info("shell.image_budget", Some(session
                   ...
                   })),); } Some(kimix_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
                   Some(kimix_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
                   ...
                   .fire_hook(kimix_hooks::event::HookEventName::SessionEnd, None,
                   kimix_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
                   ...
                   kimix_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kimix_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
                   session.send_hook_execution("session_end", None, None, & results). await; }
                   let envelope = session.fire_hook(kimix_hooks::event::HookEventName::Stop,
                   None, kimix_hooks::event::HookPayload::Stop { reason : "channel_closed"
                   .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
                   .clone() { let ctx = session.hook_run_ctx(); let results =
                   kimix_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kimix_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
                   ...
                   kimix_log::memory_log::TARGET, result = ? result, tool_searches =
                   ...
                   { let
                   _model_id = session.current_model_id(). await; if let Some(_signals) = session
                   ...
                   .update_credentials(kimix_chat_state::Credentials { api_key : r.api_key,
                   ...
                   .send(kimix_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
                   ...
                   .push_capped(kimix_tools::implementations::kimix::task::types::MonitorEventNotification
                   ...
                   kimix_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
                   ...
                   .send(kimix_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
                   ...
                   .emit(kimix_file_utils::events::Event::McpServerToggled { server_name :
                   ...
                   session.fire_hook(kimix_hooks::event::HookEventName::SessionStart, None,
                   kimix_hooks::event::HookPayload::SessionStart { source, model_id : None,
                   agent_type : None, },); if let Some(registry) = session.hook_registry
                   .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
                   kimix_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kimix_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
                   ...
                   .fire_hook(kimix_hooks::event::HookEventName::SessionEnd, None,
                   kimix_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
                   .to_string(), turn_count : None, tool_call_count : None, },); if let
                   Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
                   .hook_run_ctx(); let results =
                   kimix_hooks::dispatcher::dispatch_non_blocking(& registry,
                   ...
                   session.send_hook_execution("session_end", None, None, & results). await; }
                   let envelope = session.fire_hook(kimix_hooks::event::HookEventName::Stop,
                   None, kimix_hooks::event::HookPayload::Stop { reason : "shutdown"
                   .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
                   .clone() { let ctx = session.hook_run_ctx(); let results =
                   kimix_hooks::dispatcher::dispatch_non_blocking(& registry,
                   kimix_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[kimix_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = kimix_chat_state::compaction_utils::extract_user_query(&raw);
