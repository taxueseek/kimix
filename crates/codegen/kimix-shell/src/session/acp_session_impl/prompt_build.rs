...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.Kimix/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.kimix/`, `~/.claude/`,
/// `~/.kimix/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<kimix_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<kimix_agent::prompt::user_message::RuleEntry>,
    Vec<kimix_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.kimix/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = kimix_agent::prompt::user_message::RuleEntry::from(f);
...
/// end for Kimix ordering), so a failed offload never
...
impl SessionActor {
    ...
    async fn build_templated_user_message(
        ...
        use kimix_agent::prompt::agents_md::read_agents_config_with_paths;
        use kimix_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use kimix_workspace::file_system::{git_status_short, jj_status};
        use kimix_workspace::session::git::VcsKind;
    ...
    ) -> Vec<kimix_agent::prompt::user_message::McpServerEntry> {
        use kimix_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &kimix_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = kimix_sampler::SamplingClient::new(sampler_config).map_err(|e| {
