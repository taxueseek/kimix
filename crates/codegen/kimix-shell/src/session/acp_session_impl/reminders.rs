...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> kimix_agent::ReminderPolicy {
    let mut policy = kimix_agent::ReminderPolicy::default();
...
/// Wrapped in Kimix's `<system-reminder>` shape by [`SessionActor::push_system_reminder`].
...
pub(super) fn todo_gate_active(
    policy: &kimix_agent::system_reminder::ReminderPolicy,
    audience: kimix_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    pub(super) async fn drain_between_turn_completions(&self) {
        ...
        if !bash_completions.is_empty() {
            ...
            if goal_loop_active {
                ...
                let task_output_name =
                    kimix_tools::reminders::task_completion::resolve_task_output_tool_name(&bridge)
                        .await;
                let read_tool_name =
                    kimix_tools::reminders::task_completion::resolve_read_tool_name(&bridge).await;
                let reminder =
                    kimix_tools::reminders::task_completion::format_between_turn_bash_completions(
                        &bash_completions,
        ...
        let reminder =
            kimix_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) fn todo_gate_policy(&self) -> Option<kimix_agent::system_reminder::TodoGateConfig> {
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(kimix_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();
