...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(&self, question: &str) -> Result<String, String> {
        ...
        let mut items: Vec<ConversationItem> =
            kimix_chat_state::compaction_utils::strip_reasoning_blocks(
        ...
        let request = ConversationRequest {
            ...
            temperature: None,
            x_kimix_conv_id: Some(btw_session_id.clone()),
            x_kimix_req_id: Some(format!("xai-btw-{}", uuid::Uuid::new_v4())),
            x_kimix_session_id: Some(parent_session_id.clone()),
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let x_kimix_conv_id = format!("recap-{}", uuid::Uuid::new_v4());
        let x_kimix_req_id = format!("xai-recap-{}", uuid::Uuid::new_v4());
        ...
        let request = ConversationRequest {
            ...
            model: Some(model.clone()),
            temperature: None,
            x_kimix_conv_id: Some(x_kimix_conv_id.clone()),
            x_kimix_req_id: Some(x_kimix_req_id.clone()),
            x_kimix_session_id: Some(self.session_info.id.to_string()),
            x_kimix_agent_id: Some(crate::util::agent_id::agent_id()),
            ..Default::default()
        };
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. Kimix
    ...
    fn persist_recap_request_artifact(
        ...
        let artifact = RecapRequestFile {
            ...
            x_kimix_req_id: x_kimix_req_id.to_owned(),
            x_kimix_conv_id: x_kimix_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// (`KIMIX_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    ...
    /// a reasoning model's response, and some models (e.g. `kimix`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_kimix_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_kimix_req_id: Some(format!("xai-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_kimix_session_id: Some(self.session_info.id.to_string()),
            x_kimix_agent_id: Some(crate::util::agent_id::agent_id()),
            ..Default::default()
        };
        ...
        tracing::debug!(
            raw_preview = %kimix_tools::util::truncate_str(raw.trim(), 60),
