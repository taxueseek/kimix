...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, kimix_chat_state::UsageTotals)],
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    kimix_hooks::event::HookEventName::SubagentStart,
                    None,
                    kimix_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = kimix_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        kimix_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            XaiSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    kimix_hooks::event::HookEventName::SubagentEnd,
                    None,
                    kimix_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            kimix_hooks::event::HookEventName::Notification,
            None,
            kimix_hooks::event::HookPayload::Notification {
