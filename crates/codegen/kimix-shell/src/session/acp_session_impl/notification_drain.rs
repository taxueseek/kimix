...
impl SessionActor {
    ...
    pub(super) async fn emit_session_idle_if_idle(&self) {
        ...
        for contributor in self.extension_registry.session_lifecycle_contributors() {
            contributor
                .on_session_idle(&kimix_agent_lifecycle::SessionIdleInput)
                .await;
        }
    }
    ...
    pub(super) fn sweep_monitor_buffer_into_pending(
        ...
        for event in kimix_tools::implementations::kimix::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        ) {
    ...
    pub(super) fn drain_notifications_into_turn(
        ...
        use kimix_tools::implementations::kimix::task::types::MonitorEventNotification;
        ...
        if let (Some(idx), Some(batch)) = (
            monitor_section_idx,
            kimix_tools::reminders::task_completion::format_monitor_events(
                &monitor_events,
