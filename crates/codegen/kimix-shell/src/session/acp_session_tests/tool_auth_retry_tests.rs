...
use kimix_tools::types::output::{ToolOutput, ToolRunResult};
...
fn ok_result(text: &str) -> Result<ToolRunResult, kimix_tool_runtime::ToolError> {
    Ok(ToolRunResult {
...
fn err(msg: &str) -> Result<ToolRunResult, kimix_tool_runtime::ToolError> {
...
fn http_err(status: u16, msg: &str) -> Result<ToolRunResult, kimix_tool_runtime::ToolError> {
    Err(kimix_tool_runtime::ToolError::new(
        kimix_tool_runtime::ToolErrorKind::Custom,
        msg.to_owned(),
    )
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, kimix_tool_runtime::ToolError)> = vec![
        ...
        (
            true,
            kimix_tool_runtime::ToolError::new(
                kimix_tool_runtime::ToolErrorKind::Custom,
        ...
        // the inference path's gate in Kimix-sampling-types/src/error.rs:
        ...
        (
            true,
            kimix_tool_runtime::ToolError::invalid_arguments(
        ...
        (
            true,
            kimix_tool_runtime::ToolError::invalid_arguments(r#"{"error":"invalid_token"}"#),
        ),
        // Fallback path: case-insensitive "unauthorized" anywhere in
        // the message body.
        (
            true,
            kimix_tool_runtime::ToolError::invalid_arguments("UNAUTHORIZED"),
        ),
        // Negative: transport failure must not trigger a token refresh.
        (
            false,
            kimix_tool_runtime::ToolError::invalid_arguments("tool call timed out after 60s"),
        ),
        // Negative: structural not-found error; not a network response.
        (
            false,
            kimix_tool_runtime::ToolError::not_found(
                kimix_tool_protocol::ToolId::new("no_such_tool").expect("valid"),
        ...
        (
            false,
            kimix_tool_runtime::ToolError::invalid_arguments("request id req_401abc failed"),
        ),
    ];
