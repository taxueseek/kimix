...
async fn create_injection_ready_actor(
    initial_conversation: Vec<kimix_sampling_types::ConversationItem>,
) -> SessionActor {
    ...
    local
        .run_until(async {
            let actor = create_injection_ready_actor(vec![
                kimix_sampling_types::ConversationItem::system("You are a helpful assistant."),
                kimix_sampling_types::ConversationItem::user(
            ...
            assert!(
                reminder.contains(kimix_chat_state::MEMORY_CONTEXT_OPEN_TAG),
        ...
        .run_until(async {
            let persisted_block =
                crate::session::helpers::memory_context::format_memory_reminder(&[
                    kimix_tools::types::memory_backend::MemorySearchResult {
                ...
                .unwrap();
            let actor = create_injection_ready_actor(vec![
                kimix_sampling_types::ConversationItem::system(format!(
