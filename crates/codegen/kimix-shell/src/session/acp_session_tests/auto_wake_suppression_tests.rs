...
use kimix_tools::reminders::task_completion::consumed_completion_ids;
use kimix_tools::types::output::{BashOutput, TextOutput, ToolOutput};
    ...
    local
        .run_until(async {
            let (gateway_tx, _) = tokio::sync::mpsc::unbounded_channel::<
                kimix_acp_lib::AcpClientMessage,
            >();
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use kimix_tools::reminders::task_completion::ReportedTaskCompletions;
...
async fn between_turn_drain_suppresses_auto_wake_delivered_subagents() {
    ...
    use kimix_tools::reminders::task_completion::AutoWakeDeliveredIds;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
            ...
            tokio::task::spawn_local(async move {
                while let Some(ev) = rx.recv().await {
                    if let SubagentEvent::Completions(req) = ev {
                        *captured_task.lock().expect("lock poisoned") = req.suppress_ids.clone();
            ...
            let suppress = captured.lock().expect("lock poisoned").clone();
            assert!(
...
struct OneTaskTerminal {
    tasks: Vec<kimix_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl kimix_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: kimix_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        kimix_tools::computer::types::TerminalRunResult,
        kimix_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<kimix_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> kimix_tools::computer::types::KillOutcome {
        kimix_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<kimix_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<kimix_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> kimix_tools::computer::types::TaskSnapshot {
    kimix_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
