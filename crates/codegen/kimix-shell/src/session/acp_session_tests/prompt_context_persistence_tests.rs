...
fn test_json_round_trip() {
    let ctx = kimix_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: kimix_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `kimix_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = kimix_agent::PromptContext::default();
    ...
    let loaded: kimix_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a Kimix subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: kimix_agent::PromptContext = serde_json::from_str(
...
fn test_system_prompt_matches_chat_history_system_message() {
    ...
    let system_prompt = "You are a Kimix subagent.\n\n<tool_calling>\n...";
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<kimix_agent::PromptContext, _> = serde_json::from_str(&json);
...
fn fake_prompt_path() -> std::path::PathBuf {
    std::path::PathBuf::from("/tmp/kimix-test-home/sessions/cwd/sid/prompts/prompt_0.txt")
}
...
fn build_truncated_preserves_small_query_truncates_context() {
    ...
    assert!(
        ...
        "kimix ordering: query block first"
    );
...
//
// `kimix_home()` is a process-wide `OnceLock`, so the real async method is
