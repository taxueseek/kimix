...
//!
//! These tests verify the parallel dispatch path (KIMIX_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and Kimix's
...
#[test]
fn lock_path_for_args_matches_kimix_file_path() {
    // Kimix search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_kimix_target_file() {
    // Kimix read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_kimix_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. Kimix search_replace + StrReplace
    ...
    assert_eq!(lock_path_for_args(&kimix), lock_path_for_args(&compat));
}
