        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_kimix_req_id.starts_with("xai-recap-"),
                        "req id: {}",
                        artifact.x_kimix_req_id
                    );
    ...
    local
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    let est =
                        kimix_chat_state::estimate_conversation_tokens(&artifact.chat_history);
                    assert!(
                        est <= PROMPT_BUDGET,
...
/// shape, so it also covers the laxer Kimix ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use kimix_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use kimix_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // Kimix backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = kimix_sampling_types::build_messages_request(&req);
