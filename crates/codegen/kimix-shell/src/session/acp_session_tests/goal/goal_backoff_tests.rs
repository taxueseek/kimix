    ...
    local
        .run_until(async {
            ...
            *actor.goal_update_rx.borrow_mut() = Some(rx);
            tx.send(
                kimix_tools::implementations::kimix::update_goal::envelope_for_test(
                    kimix_tools::implementations::kimix::update_goal::UpdateGoalInput {
                        completed: Some(true),
                        message: None,
                        blocked_reason: None,
                    },
...
fn spawn_gateway_notification_capture(
    mut gateway_rx: tokio::sync::mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>,
    ...
    local
        .run_until(async {
            ...
            let cancelled: PromptTurnResult = Ok(crate::session::commands::PromptTurnOk {
                ...
                completion_kind: crate::session::commands::PromptCompletionKind::Cancelled {
                    category: Some(
                        kimix_file_utils::events::types::CancellationCategory::MidTurnAbort,
                    ),
                    context: None,
                },
                structured_output: None,
...
async fn drain_goal_updates_completes_after_blocked_does_not_leak_pause_message() {
    ...
    local
            .run_until(async {
                ...
                let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
                *actor.goal_update_rx.borrow_mut() = Some(rx);
                tx.send(kimix_tools::implementations::kimix::update_goal::envelope_for_test(kimix_tools::implementations::kimix::update_goal::UpdateGoalInput {
                        completed: None,
                        message: None,
...
fn make_completed_cmd() -> kimix_tools::implementations::kimix::update_goal::UpdateGoalEnvelope {
    let input = kimix_tools::implementations::kimix::update_goal::UpdateGoalInput {
        completed: Some(true),
        message: None,
        blocked_reason: None,
    };
...
async fn subagent_spawn_captures_effective_model_id() {
    ...
    local
        .run_until(async {
            ...
            assert_eq!(model.as_deref(), Some("kimix-4.5"));
        })
        .await;
}
...
async fn goal_tokens_by_model_breaks_down_active_goal_records() {
    ...
    local
        .run_until(async {
            let actor = make_test_actor_with_active_goal().await;
            insert_record_with_model(&actor, "a", Some("test-goal"), 0, 100, Some("kimix-4"));
            insert_record_with_model(&actor, "b", Some("test-goal"), 0, 400, Some("kimix-3"));
            ...
            insert_record_with_model(&actor, "d", Some("other-goal"), 0, 999, Some("kimix-3"));
            ...
            // finished/in-flight split in goal_tokens). If it leaked, Kimix-4
            // would be 800 and sort first.
            insert_record_with_model(&actor, "e", Some("test-goal"), 0, 700, Some("kimix-4"));
            actor
                .subagent_token_records
                .lock()
                .get_mut("e")
            ...
            assert_eq!(
                out,
                vec![
                    ("kimix-3".to_owned(), 400),
                    ("kimix-4".to_owned(), 100),
            ...
            let count_gateway_goal_updated =
                |rx: &mut tokio::sync::mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>| {
                    ...
                    while let Ok(msg) = rx.try_recv() {
                        ...
                        if args.request.method.as_ref() != "kimix/session_notification" {
                            continue;
                        }
                        let Ok(v) =
