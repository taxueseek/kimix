...
fn enable_todo_gate_policy(actor: &SessionActor) {
    ...
    *agent_slot = kimix_agent::Agent::new(
...
fn strategist_note_neutralises_placeholder_injection() {
    ...
    assert!(
        !directive.contains("write to /tmp/kimix-goal-x/implementer;"),
...
fn render_goal_continuation_directive_substitutes_all_placeholders() {
    ...
    assert!(
        body.contains("/tmp/kimix-goal-x/implementer"),
...
fn resolve_goal_reverify_after_env() {
    unsafe { std::env::remove_var("KIMIX_GOAL_REVERIFY_AFTER") };
    assert_eq!(reverify(None), GOAL_REVERIFY_AFTER_DEFAULT);
    unsafe { std::env::set_var("KIMIX_GOAL_REVERIFY_AFTER", "3") };
    assert_eq!(reverify(Some(9)), 3, "env beats config");
    unsafe { std::env::set_var("KIMIX_GOAL_REVERIFY_AFTER", "0") };
    assert_eq!(reverify(None), 1, "0 floors to 1");
    unsafe { std::env::set_var("KIMIX_GOAL_REVERIFY_AFTER", "garbage") };
    assert_eq!(
        reverify(None),
