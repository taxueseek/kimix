...
//!
//! Tests that mutate `KIMIX_GOAL_CLASSIFIER` carry the
...
use kimix_tools::implementations::kimix::update_goal::{RejectReason, UpdateGoalInput};
use serial_test::serial;
use std::collections::VecDeque;
use std::sync::Arc as StdArc;
use std::sync::atomic::{AtomicUsize, Ordering as SeqOrd};
use tokio::sync::Notify;
const ENV_FLAG: &str = "KIMIX_GOAL_CLASSIFIER";
...
struct MockCoordinator {
    ...
    describe_outcome: StdArc<
        parking_lot::Mutex<
            kimix_tools::implementations::kimix::task::types::SubagentDescribeOutcome,
        >,
    >,
...
fn seed_channel(actor: &SessionActor, cmds: Vec<UpdateGoalInput>) {
    ...
    *actor.goal_update_rx.borrow_mut() = Some(rx);
    for cmd in cmds {
        tx.send(kimix_tools::implementations::kimix::update_goal::envelope_for_test(cmd))
            .unwrap();
    }
    drop(tx);
}
...
fn seed_channel_with_acks(
    ...
    tokio::sync::oneshot::Receiver<kimix_tools::implementations::kimix::update_goal::UpdateGoalAck>,
> {
    let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
    *actor.goal_update_rx.borrow_mut() = Some(rx);
...
async fn goal_classifier_stall_early_exit_pauses_with_no_progress() {
    ...
    local
        .run_until(async {
            ...
            let (actor, tmp) = make_actor(Some(coord.tx.clone()), true).await;
            use kimix_tools::implementations::kimix::update_goal::UpdateGoalAck;
...
async fn goal_classifier_env_override_disables_when_remote_enabled() {
    ...
    assert!(
        !resolved.value,
        "KIMIX_GOAL_CLASSIFIER=0 must disable even with remote=true",
    );
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
async fn update_goal_tool_blocks_until_classifier_verdict_when_enabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                kimix_tools::implementations::kimix::update_goal::UpdateGoalAck::ClassifierAchieved
                { .. },),
...
async fn update_goal_tool_returns_immediately_when_classifier_disabled() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                matches!(ack,
                kimix_tools::implementations::kimix::update_goal::UpdateGoalAck::CompletedWithoutClassifier,)
            );
        })
        .await;
}
...
async fn update_goal_tool_returns_immediately_for_blocked_reason() {
    ...
    local
        .run_until(async {
            ...
            assert!(matches!(
                ack,
                kimix_tools::implementations::kimix::update_goal::UpdateGoalAck::Accepted { .. },
            ));
        })
        .await;
}
...
fn render_ack_classifier_achieved_is_success() {
    use kimix_tools::implementations::kimix::update_goal::{UpdateGoalAck, render_ack_into_output};
...
fn tool_error_code(err: &kimix_tool_runtime::ToolError) -> &str {
    err.details
        .as_ref()
...
use kimix_tools::implementations::kimix::task::types::SubagentDescribeOutcome;
...
async fn resolve_role_override_toolset_incapable_fails_open() {
    ...
    local
        .run_until(async {
            let mut summary =
                kimix_tools::implementations::kimix::task::types::SubagentTypeSummary {
                    can_read: true,
                    ..Default::default()
                };
            summary
                .tool_names
                .insert(kimix_tools::types::tool::ToolKind::Read, "read_file".into());
            let (ov, log) = run_resolve(
                &role_pair("good-model", "general-purpose"),
                RoleCapability::Skeptic,
                &[("good-model", true)],
...
/// `Kimix-plan` (non-strict), and `opencode` (non-strict + unrepresentable
...
async fn resolve_role_override_harness_flavor_representability_gate() {
    ...
    local
        .run_until(async {
            ...
            let honored = ["kimix-plan", "opencode"];
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                kimix_sampling_types::conversation::ConversationItem::assistant(SUMMARY),
            );
            let _ = actor.run_verification_stage_for_drain(1, 3).await;
            assert_eq!(
                actor
...
async fn reverification_replays_anchor_and_does_not_overwrite_it() {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                kimix_sampling_types::conversation::ConversationItem::assistant(NOTE),
            );
