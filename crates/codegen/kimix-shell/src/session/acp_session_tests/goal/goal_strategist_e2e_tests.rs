...
//!
//! Tests mutate `KIMIX_GOAL_CLASSIFIER` so they carry `serial`.
use super::support::*;
use super::*;
...
use kimix_tools::implementations::kimix::update_goal::UpdateGoalInput;
...
async fn answer_strategist(
    ...
    req: Box<kimix_tools::implementations::kimix::task::types::SubagentRequest>,
) {
    match behaviour {
...
async fn make_actor(
    ...
    actor.tool_context.cwd =
        kimix_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
