        ...
        .run_until(async {
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
                panic!("expected an kimix/hooks/event ext notification");
            };
            assert_eq!(args.request.method.as_ref(), "kimix/hooks/event");
...
/// `Kimix/hooks/run` request is answered with a deny and `run_pre_tool_use_client_hook`
    ...
    local
        .run_until(async {
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            // Answer the Kimix/hooks/run reverse request with a deny; ack the UI
            // notifications `deny_tool` emits so it can't block the gate.
            tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    match msg {
                        kimix_acp_lib::AcpClientMessage::ExtMethod(args) => {
                        ...
                        kimix_acp_lib::AcpClientMessage::SessionNotification(args) => {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_with_tools(vec![kimix_tools::registry::types::ToolConfig::for_tool::<
                    kimix_tools::implementations::use_tool::UseTool,
                >()])
                .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        kimix_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
                    callback_ids: vec!["cb_0".to_string()],
                    timeout: None,
                }],
    ...
    local
        .run_until(async {
            ...
            *subagent.client_hooks.borrow_mut() = parent.client_hooks.borrow().clone();
            ...
            assert_eq!(
                seen_subagent_type.lock().expect("lock poisoned").as_deref(),
                Some("code-reviewer"),
...
/// post-tool event is observed as a fire-and-forget `Kimix/hooks/event` notification.
#[tokio::test(flavor = "current_thread")]
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_kimix_agent_with_todo().await;
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            // Collect the `hookEventName` of every `Kimix/hooks/event` notification queued.
            let drain =
                |rx: &mut tokio::sync::mpsc::UnboundedReceiver<kimix_acp_lib::AcpClientMessage>| {
                    ...
                    while let Ok(msg) = rx.try_recv() {
                        if let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                            && args.request.method.as_ref() == "kimix/hooks/event"
                        {
                            let params: serde_json::Value =
                                serde_json::from_str(args.request.params.get()).unwrap();
    ...
    local
        .run_until(async {
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            // Answer the reverse Kimix/hooks/run request with a deny carrying a reason;
