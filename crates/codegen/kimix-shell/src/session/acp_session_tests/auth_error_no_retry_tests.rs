...
/// attribution.
fn auth_error() -> kimix_sampler::SamplingErrorInfo {
    kimix_sampler::SamplingErrorInfo {
        kind: kimix_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: kimix_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        kimix_chat_state::AuthType::SessionToken => "cached_token",
        kimix_chat_state::AuthType::ApiKey => "xai.api_key",
    };
...
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(kimix_chat_state::Credentials {
...
fn model_not_found_error() -> kimix_sampler::SamplingErrorInfo {
    kimix_sampler::SamplingErrorInfo {
            kind: kimix_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model kimix does not exist or your team does not have access".into(),
...
fn unauthorized_401_error() -> kimix_sampler::SamplingErrorInfo {
    kimix_sampler::SamplingErrorInfo {
            kind: kimix_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.kimix.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=xai-kimix-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
