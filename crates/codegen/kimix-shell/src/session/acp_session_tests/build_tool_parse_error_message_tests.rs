...
fn test_malformed_json_includes_original_args_and_position() {
    ...
    let err: kimix_tool_runtime::ToolError = serde_json::from_str::<serde_json::Value>(bad_args)
        .unwrap_err()
        .into();
...
fn test_valid_json_no_invalid_json_note() {
    ...
    let err =
        kimix_tool_runtime::ToolError::invalid_arguments("missing field `old_string`".to_string());
...
fn test_empty_arguments_no_extra_content() {
    let err =
        kimix_tool_runtime::ToolError::invalid_arguments("missing field `file_path`".to_string());
...
fn test_non_ascii_arguments_truncated_safely() {
    ...
    let err = kimix_tool_runtime::ToolError::invalid_arguments("missing field".to_string());
    // Must not panic.
    let msg = build_tool_parse_error_message("search_replace", &err, &long_args);
