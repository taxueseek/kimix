...
#[cfg(test)]
#[cfg(test)]
pub(crate) async fn test_agent_default() -> kimix_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> kimix_agent::Agent {
    use kimix_tools::implementations::kimix::update_goal::UpdateGoalTool;
    use kimix_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Kimix-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_kimix_agent_with_todo() -> kimix_agent::Agent {
    use kimix_tools::implementations::kimix::todo::TodoWriteTool;
    use kimix_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> kimix_agent::Agent {
    use kimix_tools::implementations::kimix::enter_plan_mode::EnterPlanModeTool;
    use kimix_tools::implementations::kimix::exit_plan_mode::ExitPlanModeTool;
    use kimix_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<kimix_tools::registry::types::ToolConfig>,
) -> kimix_agent::Agent {
    test_agent_from_config(
        kimix_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        kimix_agent::AgentDefinition::default_kimix(),
        std::sync::Arc::new(kimix_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: kimix_agent::AgentDefinition,
    backend: std::sync::Arc<dyn kimix_tools::computer::types::TerminalBackend>,
) -> kimix_agent::Agent {
    use kimix_tools::computer::local::LocalFs;
    use kimix_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("kimix-test"),
    ...
    kimix_agent::Agent::new(
        definition,
        kimix_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        kimix_agent::ReminderPolicy::default(),
        kimix_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<kimix_acp_lib::AcpClientMessage>,
    persistence_tx: tokio::sync::mpsc::UnboundedSender<PersistenceMsg>,
) -> (
    SessionActor,
    tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
) {
    let cwd = kimix_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(kimix_workspace::file_system::MockFs::new(cwd.to_path_buf()));
    ...
    let actor = SessionActor {
        ...
        permissions: kimix_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
...
pub(crate) async fn build_actor() -> (
    ...
    let (gateway_tx, gateway_rx) =
        tokio::sync::mpsc::unbounded_channel::<kimix_acp_lib::AcpClientMessage>();
