use kimix_tools::computer::local::{LocalTerminalBackend, MockFs};
...
async fn tool_bridge_routes_writes_through_injected_fs() {
    ...
    let ctx = SessionContext {
        ...
        fs,
        cwd: cwd.clone(),
        session_folder: std::env::temp_dir().join("kimix-test-fs"),
        session_env: std::sync::Arc::new(std::collections::HashMap::new()),
        notification_handle: ToolNotificationHandle::noop(),
        owner_session_id: None,
        parent_scheduler_handle: None,
        skills: vec![],
        state_path: std::env::temp_dir().join("kimix-test-fs/tool_state.json"),
        memory_backend: None,
