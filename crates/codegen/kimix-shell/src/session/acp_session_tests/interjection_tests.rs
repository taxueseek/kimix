...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `Kimix/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let kimix_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "kimix/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an kimix/session/interjection broadcast");
                assert_eq!(
