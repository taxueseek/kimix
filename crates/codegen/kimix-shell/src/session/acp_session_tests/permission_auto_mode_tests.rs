...
use kimix_workspace::permission::{AccessKind, ClientType, spawn_permission_manager};
    ...
    local
        .run_until(async {
            ...
            let d = session
                .permissions
                .request(
                    AccessKind::Bash("cargo test -p kimix-workspace".into()),
                    dummy_update,
                    None,
                    None,
                    None,
            ...
            assert!(
                matches!(d, kimix_workspace::permission::Decision::Allow),
            ...
            let d2 = session
                ...
                .await;
            assert!(
                !matches!(d2, kimix_workspace::permission::Decision::Allow),
    ...
    local
        .run_until(async {
            ...
            actor.permissions.set_classifier_transcript(vec![
                kimix_workspace::permission::ClassifierTurn::UserText("please run tests".into()),
            ]);
...
fn build_classifier_turns_captures_tool_use_excludes_text_and_results() {
    use kimix_workspace::permission::ClassifierTurn;
    let conv = vec![
        ...
        super::ConversationItem::assistant_tool_calls(vec![
            kimix_sampling_types::conversation::ToolCall {
