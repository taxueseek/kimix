...
use super::support::*;
use super::*;
/// Build an actor whose toolset parses Kimix `search_replace` plus the plan
...
async fn build_gate_actor() -> SessionActor {
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
        ToolConfig::from_id("Kimix:read_file"),
        ToolConfig::from_id("Kimix:search_replace"),
...
async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conv.iter()
        ...
        .find_map(|item| match item {
            kimix_sampling_types::ConversationItem::ToolResult(tr)
...
/// worst case) still rejects a Kimix edit outside the plan file, without ever
/// reaching the permission layer, and steers the model to `exit_plan_mode`.
#[tokio::test(flavor = "current_thread")]
async fn plan_mode_rejects_kimix_edit_outside_plan_file_despite_allow_all_permissions() {
