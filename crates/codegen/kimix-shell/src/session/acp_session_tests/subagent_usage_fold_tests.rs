...
fn usage_rows() -> Vec<(String, kimix_chat_state::UsageTotals)> {
    vec![(
        "m".into(),
        kimix_chat_state::UsageTotals {
            input_tokens: 40,
            model_calls: 1,
            ..Default::default()
        },
...
async fn subagent_usage_fold_attribution_gate() {
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            *actor.current_prompt_id.lock().expect("lock poisoned") = Some("p-1".into());
            assert_eq!(
                actor
            ...
            for (live, stamped) in [
                ...
                *actor.current_prompt_id.lock().expect("lock poisoned") = live.map(str::to_string);
...
fn usage_drain_outcome_policy_matches_freeze_and_cancel() {
    ...
    use kimix_tools::implementations::kimix::task::types::SubagentOutstandingReply;
...
async fn session_only_incomplete_does_not_stain_live_open_prompt() {
    tokio::task::LocalSet::new()
        .run_until(async {
            let actor = make_actor().await;
            *actor.current_prompt_id.lock().expect("lock poisoned") = Some("p-live".into());
...
fn scripted_outstanding_responder(
    replies: Vec<kimix_tools::implementations::kimix::task::types::SubagentOutstandingReply>,
) -> tokio::sync::mpsc::UnboundedSender<
    kimix_tools::implementations::kimix::task::types::SubagentEvent,
> {
    use kimix_tools::implementations::kimix::task::types::SubagentEvent;
