...
pub(super) async fn make_replay_send_update_fixture() -> ReplaySendUpdateFixture {
    let (gateway_tx, mut gateway_rx) = mpsc::unbounded_channel::<kimix_acp_lib::AcpClientMessage>();
...
async fn channel_tokens_accumulate_into_streaming_capture() {
    use kimix_sampler::{RequestId, SamplingChannel, SamplingEvent};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
async fn completed_event_clears_slot_keeps_prior_uncommitted_segments() {
    use kimix_sampler::{InferenceLatencyStats, RequestId, SamplingChannel, SamplingEvent};
    use kimix_sampling_types::{ConversationItem, ConversationResponse};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let fixture = make_replay_send_update_fixture().await;
...
async fn observe_only_confident_completion_stays_warn_only() {
    ...
    local
        .run_until(async {
            ...
            fixture.actor.doom_loop_recovery = Some(kimix_sampling_types::DoomLoopRecoveryPolicy {
                max_threshold: 8,
                max_retries: 0,
            });
            let actor = Arc::new(fixture.actor);
            *actor
            ...
            actor
                ...
                .await;
            let response = kimix_sampling_types::ConversationResponse {
                items: vec![kimix_sampling_types::ConversationItem::assistant(
                    "answer kept as-is",
                )],
                stop_reason: None,
                usage: None,
                cost_usd_ticks: None,
                message_chunks_emitted: 1,
                doom_loop_signals: vec![kimix_sampling_types::doom_loop::DoomLoopSignal::parse(
...
async fn doom_loop_recovery_stamps_capture_segments_and_counters() {
    use kimix_sampler::{RequestId, SamplingChannel, SamplingErrorKind, SamplingEvent};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let mut fixture = make_replay_send_update_fixture().await;
            fixture.actor.doom_loop_recovery =
                Some(kimix_sampling_types::DoomLoopRecoveryPolicy::default());
            let actor = Arc::new(fixture.actor);
            *actor
                .current_prompt_id
                .lock()
...
async fn tool_call_delta_on_idle_slot_leaves_phase_pending() {
    use kimix_sampler::{RequestId, SamplingEvent};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let fixture = make_replay_send_update_fixture().await;
...
async fn reasoning_only_doomloop_turn_captures_every_generation_as_segments() {
    ...
    use kimix_sampling_types::{EmptyReason, EmptyResponseContext};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let ReplaySendUpdateFixture {
            ...
            *actor
            ...
            let (_chat_tx, chat_rx) = mpsc::unbounded_channel::<kimix_chat_state::ChatStateEvent>();
