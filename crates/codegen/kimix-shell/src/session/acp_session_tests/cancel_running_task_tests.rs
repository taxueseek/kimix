...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(kimix_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            agent
                .tool_bridge()
                .update_resource(
                    kimix_tools::implementations::kimix::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
...
/// user's next queued prompt) and rebroadcasts `Kimix/queue/changed`. The
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![kimix_sampling_types::ToolCall {
                    id: "call-1".into(),
...
async fn cancel_propagates_to_sampler_handle_so_no_further_emission() {
    ...
    local
        .run_until(async {
            ...
            let cfg = kimix_sampler::SamplerConfig {
                api_key: Some("test-key".to_string()),
                ...
                temperature: None,
                top_p: None,
                api_backend: kimix_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                context_window: 100_000,
                force_http1: false,
            ...
            let sampler_handle = kimix_sampler::SamplerActor::spawn(
                cfg,
                kimix_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) =
                tokio::sync::mpsc::unbounded_channel::<kimix_acp_lib::AcpClientMessage>();
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(kimix_sampling_types::UserItem {
                    content: vec![kimix_sampling_types::ContentPart::Text { text: "hi".into() }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use kimix_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(
                    ...
                    matches!(p, kimix_sampling_types::ContentPart::Text { text } if
