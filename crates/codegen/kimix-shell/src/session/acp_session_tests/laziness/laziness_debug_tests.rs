...
fn assistant_with_reasoning_items(
    ...
    if !reasoning_text.is_empty() {
        out.push(ConversationItem::Reasoning(
            kimix_sampling_types::rs::ReasoningItem {
                id: String::new(),
                summary: vec![kimix_sampling_types::rs::SummaryPart::SummaryText(
                    kimix_sampling_types::rs::SummaryTextContent {
...
fn flatten_skips_reasoning_when_encrypted_only() {
    ...
    let items = vec![
        ConversationItem::Reasoning(kimix_sampling_types::rs::ReasoningItem {
            id: String::new(),
            summary: vec![],
            content: None,
...
fn synthetic_user_text(
    text: &str,
    reason: kimix_sampling_types::SyntheticReason,
) -> ConversationItem {
    ConversationItem::User(UserItem {
        content: vec![ContentPart::Text { text: text.into() }],
...
fn window_assistant_text_pin_skips_empty_assistant_turns() {
    ...
    let empty_asst = ConversationItem::Assistant(kimix_sampling_types::AssistantItem {
        content: String::new().into(),
        tool_calls: vec![kimix_sampling_types::ToolCall {
            id: "c".into(),
            name: "read_file".into(),
            arguments: "{}".into(),
        }],
