...
pub struct SessionHandle {
    ...
    pub chat_state_handle: kimix_chat_state::ChatStateHandle,
    ...
    /// `Kimix/codeNavigation.enabled`.  Stored per-session so that in leader
    ...
    /// Exposed so the `Kimix/toggle_plan_mode` handler can toggle plan mode
    ...
    /// Set via `Kimix/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: kimix_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend: Option<std::sync::Arc<dyn kimix_tools::computer::types::TerminalBackend>>,
    ...
    pub tools_notification_handle: Option<kimix_tools::notification::types::ToolNotificationHandle>,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> kimix_chat_state::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            kimix_chat_state::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<kimix_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub async fn get_hooks_list(&self) -> Option<kimix_hooks_plugins_types::HooksListResponse> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<kimix_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
