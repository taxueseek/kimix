...
//! `Kimix-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use kimix_file_utils::events::tracker::EventTracker;
pub(crate) use kimix_file_utils::events::types::{
...
) -> Option<kimix_sampling_types::PriorTurnInterrupt> {
    use kimix_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run Kimix flavor. Distinct from
...
/// (`Kimix-file-utils`). The two enums are intentional mirrors and live in
