...
use kimix_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use kimix_agent::prompt::skills::SkillsConfig;
use kimix_sampler::SamplerConfig as SamplingConfig;
use kimix_sampling_types::truncate_bytes;
...
use kimix_tools::types::compat::CompatConfig;
use kimix_tools::types::output::{
...
pub(crate) use spawn::*;
/// Client-registered hook gates (the `Kimix/hooks/run` reverse request).
mod hooks;
...
pub(crate) struct SessionActor {
    ...
    /// `Kimix-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<kimix_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: kimix_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<kimix_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<kimix_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-Kimix-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<kimix_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    /// Append-only writer for `queue.jsonl` — persists prompt queue mutations
    /// (enqueue/dequeue/edit/reorder/clear) for crash recovery.
    /// `noop()` writer when queue persistence is disabled via config.
    pub(crate) queue_writer: kimix_file_utils::events::QueueEventWriter,
    /// Whether queue persistence is enabled (checks remote settings + config).
    pub(crate) queue_persistence_enabled: bool,
    ...
    pub(crate) agent: std::cell::RefCell<kimix_agent::Agent>,
    /// Dedup slot for `Kimix/git_head_changed`, shared with the fs-watch
    /// `GitHead` consumer (see `git_head_dedup_key`).
    pub(crate) last_reported_branch: Arc<parking_lot::Mutex<Option<String>>>,
    /// Client opted into `Kimix/gitHeadChanged`. When false (headless/SDK),
    ...
    /// Kill-switch (`KIMIX_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<kimix_tools::implementations::kimix::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, kimix_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: kimix_agent_lifecycle::LocalExtensionRegistry,
    ...
    /// Safe: session actor is single-threaded (LocalSet), no concurrent access.
    pub(crate) hook_registry: std::cell::RefCell<Option<Arc<kimix_hooks::discovery::HookRegistry>>>,
    /// Client hooks from `session/new` `_meta["Kimix/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<kimix_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<kimix_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `Kimix-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: kimix_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`kimix_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`kimix_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: kimix_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    fn build_command_availability(
        ...
        use kimix_tools::implementations::memory::{MEMORY_GET_TOOL_NAME, MEMORY_SEARCH_TOOL_NAME};
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names
                .iter()
                .any(|n| n == kimix_tools::implementations::kimix::SCHEDULER_CREATE_TOOL_NAME),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<kimix_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `kimix prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &kimix_agent::PromptContext) {
...
) -> Option<kimix_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod tool_meta_stamp_tests {
    //! Pin the `Kimix/tool` stamps on the harness emission paths: the early
    ...
    use kimix_tools::tool_taxonomy::TOOL_META_KEY;
    use kimix_workspace::permission::PermissionCommand;
    use tokio::sync::mpsc;
    ...
    /// The `Kimix/tool` object from an event's `_meta`, if present.
        ...
        local
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id("Kimix:read_file".to_string())])
                        .await,
                );
                ...
                let t = tool_meta(early.as_ref()).expect("early ToolCall carries kimix/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["namespace"], "kimix");
                assert!(t.get("input").is_none(), "identity-only before parse");
                let refined = refined.expect("refinement ToolCallUpdate emitted");
                let t = tool_meta(refined.as_ref()).expect("refinement carries kimix/tool");
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(update.meta.as_ref())
                    .expect("permission-request ToolCallUpdate carries kimix/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["input"]["path"], "/tmp/stamp.txt");
            })
