...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: kimix_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `Kimix-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-Kimix-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<kimix_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<kimix_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.Kimix/hooks`,
    ...
    /// Out-of-band history repair (`Kimix/session/repair`): fix tool-pairing
    ...
    RepairHistory {
        ...
        respond_to: oneshot::Sender<
            anyhow::Result<kimix_chat_state::compaction_utils::HistoryRepairReport>,
        >,
    },
    GetRewindPoints {
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, kimix_chat_state::UsageTotals)>,
    ...
    /// mid-session via the `Kimix/session/update_mcp_servers` extension method).
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<kimix_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<kimix_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<kimix_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<kimix_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: kimix_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<kimix_hooks_plugins_types::ActionOutcome>,
    },
    ...
    /// This session's plugin registry, as served by `Kimix/plugins/list`.
    PluginsList {
        respond_to: oneshot::Sender<Option<std::sync::Arc<kimix_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// serialized mailbox; the rebroadcast of `Kimix/queue/changed` is the
    ...
    /// `Kimix/queue/changed` is the truth signal for every attached client.
    ...
    /// tool-free model call (default `Kimix-0.1` when available via
    ...
    /// `kimix` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `Kimix/session/interjection` so the originating pager can dedup its
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<kimix_chat_state::TurnCapture>>,
    },
