...
//! other concurrently running Kimix processes (which may wipe or downgrade
...
/// User-configurable overrides via `~/.kimix/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple Kimix processes run concurrently, they each have
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    kimix_sqlite_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
...
fn collect_all_indexable_content_single_pass(updates_path: &Path) -> io::Result<(String, u64)> {
    ...
    for line_result in reader.lines() {
        ...
        // ("_Kimix/session/update"). Dispatch on source first, then tag.
        if !is_xai {
            // ── ACP content events ──────────────────────────────────
            match tag {
