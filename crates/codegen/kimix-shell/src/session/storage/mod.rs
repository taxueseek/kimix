...
pub(crate) const XAI_SESSION_UPDATE_METHOD: &str = "_kimix/session/update";

/// Pre-rebrand spelling of [`XAI_SESSION_UPDATE_METHOD`], as written into
/// `updates.jsonl` by builds that predate the `Kimix/` extension-method
...
pub(crate) struct SessionUpdateEnvelope {
    ...
    /// Either "session/update" for ACP or "_Kimix/session/update" for xAI extensions.
    pub method: String,
    /// The actual notification payload.
    pub params: serde_json::Value,
    /// Optional metadata for analytics: subagent_id, session_kind, etc.
    /// Injected at write time so analytics can separate main vs sub-agent turns
    /// without modifying the ACP protocol layer.
...
impl SessionUpdateEnvelope {
    ...
    pub(crate) fn from_update(update: &SessionUpdate) -> Result<Self, serde_json::Error> {
        Self::from_update_with_meta(update, None)
    }

    /// Create an envelope with optional analytics metadata (_meta field).
    pub(crate) fn from_update_with_meta(
...
/// Like [`load_updates_for_replay`], but resolves the session under a specific Kimix home.
...
mod tests {
    ...
    fn xai_envelope(session_update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_kimix/session/update","params":{{"sessionId":"s","update":{session_update_json}}}}}"#
        )
    }
    ...
    fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
        ...
        let old_xai = r#"{"timestamp":2,"method":"_kimix/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"}}}"#;
        let raw = format!("{a1}\n{old_xai}\n");
        ...
        let new_xai = r#"{"timestamp":2,"method":"_kimix/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"},"_meta":{"eventId":"ev2"}}}"#;
        let raw = format!("{a1}\n{new_xai}\n");
        let prepared = prepare_replay_lines(&raw, Some("ev1"));
        assert!(!prepared.mark_replay);
        assert_eq!(prepared.lines.len(), 1);
    ...
    fn prepare_replay_reports_spawn_without_finish() {
        let spawn = |id: &str, child: &str| {
            format!(
                r#"{{"method":"_kimix/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_spawned","subagent_id":"{id}","parent_session_id":"s","child_session_id":"{child}","subagent_type":"general-purpose","description":"task"}},"_meta":{{"eventId":"s-1"}}}}}}"#
            )
        };
        let finish = |id: &str| {
            format!(
                r#"{{"method":"_kimix/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_finished","subagent_id":"{id}","child_session_id":"c{id}","status":"completed","tool_calls":0,"turns":0,"duration_ms":0}},"_meta":{{"eventId":"s-2"}}}}}}"#
            )
        };
    ...
    /// Session files written before the `Kimix/` extension-method rename carry
