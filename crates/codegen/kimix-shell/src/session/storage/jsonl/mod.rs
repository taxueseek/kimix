...
impl JsonlStorageAdapter {
    ...
    fn session_dir(&self, info: &Info) -> PathBuf {
        match &self.dir_mode {
            SessionDirMode::FromRoot(root) => root
                .join("sessions")
                .join(crate::util::kimix_home::encode_cwd_dirname(&info.cwd))
    ...
    fn scan_session_dirs(&self, cwd: Option<&str>) -> Vec<PathBuf> {
        ...
        if let Some(cwd_str) = cwd {
            let enc = crate::util::kimix_home::encode_cwd_dirname(cwd_str);
    ...
    /// [`kimix_sampling_types::upgrade_legacy_reasoning`] and emit
    ...
    fn read_chat_history_sync(
        ...
        for (line_idx, line) in contents.split(|b| *b == b'\n').enumerate() {
            ...
            let siblings =
                kimix_sampling_types::upgrade_legacy_reasoning(&raw, &mut sibling_btc_ids_seen);
            for sib in siblings {
                match &sib {
...
///    throughout (real Kimix turns emit `[reasoning, assistant, tool
...
/// `Kimix-subagent-resolution/src/context.rs` (it counts turns in the same
...
impl JsonlStorageAdapter {
    ...
    pub fn copy_session_data_sync(
        ...
        if options.strip_reasoning {
            chat_to_copy = kimix_chat_state::compaction_utils::strip_reasoning_blocks(chat_to_copy);
        }
        ...
        let target_summary = crate::session::persistence::Summary {
            ...
            request_id: None,
            kimix_home: crate::session::persistence::kimix_home_string(),
...
async fn next_compaction_segment_index(compaction_dir: &std::path::Path) -> u64 {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        if let Some(n) = entry
            .file_name()
            .to_str()
            .and_then(kimix_chat_state::compaction_transcript::parse_segment_index)
        {
            next = next.max(n + 1);
        }
    }
...
impl StorageAdapter for JsonlStorageAdapter {
    async fn init_session(&self, info: &Info, model_id: acp::ModelId) -> io::Result<Summary> {
        ...
        if Path::new(&summary_path).exists() {
            ...
            summary.sandbox_profile = kimix_sandbox::configured_profile_name().map(String::from);
    ...
    async fn update_current_model_and_agent(
        ...
        reasoning_effort: Option<Option<kimix_sampling_types::ReasoningEffort>>,
    ) -> io::Result<()> {
        self.apply_summary_patch(
            info,
            super::summary_write::SummaryPatch {
    ...
    async fn merge_rewind_points_from(&self, info: &Info, target_index: usize) -> io::Result<()> {
        let points = self.load_rewind_points(info).await?;
        let merged =
            kimix_workspace::session::file_state::merge_rewind_points_from(points, target_index);
    ...
    async fn write_compaction_segment(
        ...
        use kimix_chat_state::compaction_transcript::{
