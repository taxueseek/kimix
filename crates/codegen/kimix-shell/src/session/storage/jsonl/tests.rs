...
async fn write_compaction_segment_numbers_and_indexes_resume_safely() {
    ...
    let seg = |summary: &str| CompactionSegmentFile {
        ...
        detail: kimix_chat_state::CompactionDetail::Verbose,
    ...
    let base = adapter
        .session_dir(&info)
        .join(kimix_chat_state::compaction_transcript::COMPACTION_DIR);
...
async fn test_jsonl_round_trip() {
    ...
    let new_model = acp::ModelId::new("kimix-4.3");
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use kimix_paths::RelPathBuf;
    use kimix_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use kimix_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn test_subagent_notifications_round_trip() {
    ...
    assert_eq!(spawned_json["method"], "_kimix/session/update");
    ...
    assert_eq!(finished_json["method"], "_kimix/session/update");
...
async fn copy_session_data_copies_compaction_segments_when_enabled() {
    ...
    assert!(
        ...
        .join(kimix_chat_state::compaction_transcript::COMPACTION_DIR).exists()
    );
}
#[tokio::test]
...
async fn test_copy_session_data_with_model_override() {
    ...
    let loaded = adapter.load_session(&target_info).await.unwrap();
    assert_eq!(loaded.summary.current_model_id.0.as_ref(), "kimix-3");
...
async fn test_append_feedback_creates_file_and_persists() {
    ...
    let user_entry = LocalFeedbackEntry::UserFeedback(UserFeedbackEntry {
        ...
        submission: Some(FeedbackSubmission {
            ...
            model_id: Some("kimix-3-fast".into()),
            resolved_model_id: Some("kimix-4.5".into()),
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    kimix_sandbox::set_configured_profile("workspace");
    let expected = kimix_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_multi_tool_cycle_turn_with_reasoning() {
    ...
    let mut items = vec![
        ConversationItem::system("sys"), ConversationItem::user("q"),
        ConversationItem::Reasoning(kimix_sampling_types::synthesized_reasoning_item("plan",)),
        ...
        ConversationItem::Reasoning(kimix_sampling_types::synthesized_reasoning_item("reflect",)),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
        ...
        ConversationItem::Reasoning(kimix_sampling_types::synthesized_reasoning_item("mid")),
...
fn scan_session_dirs_finds_all_sessions() {
    ...
    let cwd = crate::util::kimix_home::encode_cwd_dirname("/home/user/project");
...
fn scan_session_dirs_filters_by_cwd() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let cwd_a = crate::util::kimix_home::encode_cwd_dirname("/home/user/project-a");
    let cwd_b = crate::util::kimix_home::encode_cwd_dirname("/home/user/project-b");
...
fn scan_session_dirs_skips_non_directory_entries() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::kimix_home::encode_cwd_dirname("/project");
...
async fn list_sessions_recent_returns_most_recent_by_mtime() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::kimix_home::encode_cwd_dirname("/workspace");
...
async fn list_sessions_sorts_by_last_active_at_over_updated_at() {
    ...
    let cwd = crate::util::kimix_home::encode_cwd_dirname(cwd_path);
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd,
...
async fn list_sessions_recent_sorts_by_updated_at() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::kimix_home::encode_cwd_dirname("/ws");
    let now = chrono::Utc::now();
...
async fn list_sessions_recent_spans_multiple_workspaces() {
    let tmp = TempDir::new().unwrap();
    let cwd_a = crate::util::kimix_home::encode_cwd_dirname("/project-a");
    let cwd_b = crate::util::kimix_home::encode_cwd_dirname("/project-b");
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd_a,
...
/// path end-to-end (loader + serde + kimix_sampling_types::
...
/// the legacy Kimix / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"kimix"}"#,
        ],
    );
    assert_eq!(
        items.len(), 5,
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let kimix_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"kimix"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"kimix"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    assert_eq!(
        legacy_assistant.model_id.as_deref(), Some("kimix"),
    ...
    let kimix_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"kimix"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"kimix"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
