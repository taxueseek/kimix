...
use kimix_tools::types::tool_index::{
...
impl ToolSearchIndex for Bm25ToolSearchIndex {
    fn search_snapshot(&self, query: &str, limit: usize) -> SearchSnapshot {
        let snapshot = self.snapshot.lock().expect("lock poisoned").clone();
    ...
    fn list_server_summaries(&self) -> Vec<ServerSummary> {
        let (tools_snapshot, servers) = {
            let snapshot = self.snapshot.lock().expect("lock poisoned");
...
mod tests {
    ...
    fn search_underscore_joined_identifier_components() {
        let tools = vec![
            ToolMetadata {
                qualified_name: "kimix_com_chronosphere__query_prometheus_range".into(),
                server_name: "kimix_com_chronosphere".into(),
            ...
            ToolMetadata {
                qualified_name: "kimix_com_chronosphere__list_metrics".into(),
                server_name: "kimix_com_chronosphere".into(),
        ...
        let names: Vec<&str> = snap.results.iter().map(|r| r.tool_name.as_str()).collect();
        assert!(names.contains(&"kimix_com_chronosphere__query_prometheus_range"));
        assert!(names.contains(&"kimix_com_chronosphere__list_metrics"));
        ...
        assert_eq!(
            ...
            "kimix_com_chronosphere__query_prometheus_range"
        );
    }
    ...
    fn split_mixed_formats() {
        assert_eq!(
            split_identifier("kimix_com_slack__slack_send_message"),
            vec!["kimix", "com", "slack", "slack", "send", "message"]
        );
    }
    ...
    fn mcp_format_tools() -> Vec<ToolMetadata> {
        vec![
            ...
            ToolMetadata {
                qualified_name: "kimix_com_slack__slack_send_message".into(),
                server_name: "kimix_com_slack".into(),
            ...
            ToolMetadata {
                qualified_name: "kimix_com_chronosphere__QueryPrometheusRange".into(),
                server_name: "kimix_com_chronosphere".into(),
    ...
    fn fmt_exact_snake_server_snake_tool() {
        ...
        let index = Bm25ToolSearchIndex::new(make_snapshot(mcp_format_tools()));
        let snap = index.search_snapshot("kimix_com_slack__slack_send_message", 5);
        assert_eq!(snap.results.len(), 1);
        assert_eq!(
            snap.results[0].tool_name,
            "kimix_com_slack__slack_send_message"
        );
    }
    ...
    fn fmt_exact_snake_server_pascal_tool() {
        ...
        let index = Bm25ToolSearchIndex::new(make_snapshot(mcp_format_tools()));
        let snap = index.search_snapshot("kimix_com_chronosphere__QueryPrometheusRange", 5);
        assert_eq!(snap.results.len(), 1);
        assert_eq!(
            snap.results[0].tool_name,
            "kimix_com_chronosphere__QueryPrometheusRange"
        );
    }
    ...
    fn fmt_snake_server_only_falls_through() {
        let index = Bm25ToolSearchIndex::new(make_snapshot(mcp_format_tools()));
        let snap = index.search_snapshot("kimix_com_slack", 5);
        for r in &snap.results {
            assert_ne!(
                r.tool_name, "kimix_com_slack",
                "should not match a bare server name as a tool"
            );
        }
    }
    ...
    fn production_haystack() -> Vec<ToolMetadata> {
        ...
        vec![
            // ── kimix_com_slack (17 tools) ───────────────────────────
            tool(
                "kimix_com_slack__slack_create_canvas",
                "kimix_com_slack",
                "slack_create_canvas",
            ...
            tool(
                "kimix_com_slack__slack_get_reactions",
                "kimix_com_slack",
                "slack_get_reactions",
            ...
            tool(
                "kimix_com_slack__slack_list_channel_members",
                "kimix_com_slack",
            ...
            tool(
                "kimix_com_slack__slack_read_canvas",
                "kimix_com_slack",
                "slack_read_canvas",
            ...
            tool(
                "kimix_com_slack__slack_read_channel",
                "kimix_com_slack",
                "slack_read_channel",
            ...
            tool(
                "kimix_com_slack__slack_read_file",
                "kimix_com_slack",
                "slack_read_file",
            ...
            tool(
                "kimix_com_slack__slack_read_thread",
                "kimix_com_slack",
                "slack_read_thread",
            ...
            tool(
                "kimix_com_slack__slack_read_user_profile",
                "kimix_com_slack",
            ...
            tool(
                "kimix_com_slack__slack_schedule_message",
                "kimix_com_slack",
            ...
            tool(
                "kimix_com_slack__slack_search_channels",
                "kimix_com_slack",
            ...
            tool(
                "kimix_com_slack__slack_search_emojis",
                "kimix_com_slack",
                "slack_search_emojis",
            ...
            tool(
                "kimix_com_slack__slack_search_public",
                "kimix_com_slack",
                "slack_search_public",
            ...
            tool(
                "kimix_com_slack__slack_search_public_and_private",
                "kimix_com_slack",
            ...
            tool(
                "kimix_com_slack__slack_search_users",
                "kimix_com_slack",
                "slack_search_users",
            ...
            tool(
                "kimix_com_slack__slack_send_message_draft",
                "kimix_com_slack",
            ...
            tool(
                "kimix_com_slack__slack_update_canvas",
                "kimix_com_slack",
                "slack_update_canvas",
    ...
    fn haystack_exact_qualified_name() {
        ...
        let snap = index.search_snapshot("kimix_com_slack__slack_search_public", 5);
        assert_eq!(snap.results.len(), 1);
        assert_eq!(
            snap.results[0].tool_name,
            "kimix_com_slack__slack_search_public"
    ...
    fn haystack_disambiguate_search_public_vs_private() {
        ...
        let pub_pos = names
            .iter()
            .position(|n| *n == "kimix_com_slack__slack_search_public");
        let priv_pos = names
            .iter()
            .position(|n| *n == "kimix_com_slack__slack_search_public_and_private");
        assert!(
            pub_pos.is_some(),
    ...
    fn score_comparison_query_normalization() {
        ...
        let cases: &[(&str, &str)] = &[
            ...
            ("search_public", "kimix_com_slack__slack_search_public"),
