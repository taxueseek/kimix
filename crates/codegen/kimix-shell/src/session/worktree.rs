...
//!
//! Core worktree lifecycle logic lives in [`kimix_workspace::worktree`].
...
use kimix_workspace::session::git::find_git_root_from_path;
pub use kimix_workspace::worktree::*;
use std::path::Path;
...
/// manual `create_from_worktree_sync` path used by `kimix -w --ref`).
async fn create_worktree_for_resume(
    ...
    if find_git_root_from_path(source)
        .ok()
        .is_some_and(|root| kimix_workspace::session::git::detect_vcs_kind(&root).is_jj())
    {
...
async fn cleanup_worktree_on_failure(source_cwd: &str, worktree_path: &str) {
    ...
    if is_jj {
        ...
        match tokio::task::spawn_blocking(move || kimix_fast_worktree::remove_worktree(&wt_path))
            .await
        {
            Ok(Ok(_)) => {}
            Ok(Err(e)) => {
        ...
        if let Ok(root) = find_git_root_from_path(std::path::Path::new(source_cwd)) {
            let _ = kimix_workspace::session::git::git_cli(&root, &["worktree", "prune"]).await;
        }
    }
}
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> kimix_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return kimix_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    kimix_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<kimix_workspace::session::git::RestoreDegree>,
}
/// Thin wire-format adapter over the shared
/// [`kimix_workspace::session::git::build_restore_decision`] helper.
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &kimix_workspace::session::git::CheckoutSessionOutcome,
    kind: kimix_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = kimix_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
pub async fn resume_session_in_worktree(
    ...
    ops: &kimix_workspace::WorkspaceOps,
    ...
    use kimix_workspace::session::git::effective_worktree_path;
    tracing::info!(
    ...
    let restore_degree = if codebase_ok {
        Some(kimix_workspace::session::git::RestoreDegree::Full)
    } else {
        None
    };
...
async fn resume_local_session_in_worktree(
    req: &ResumeSessionInWorktreeRequest,
    #[allow(unused_variables)] ops: &kimix_workspace::WorkspaceOps,
    ...
    if req.restore_code.unwrap_or(restore_code_default) {
        let is_jj = find_git_root_from_path(std::path::Path::new(resolved_source_cwd))
            ...
            .is_some_and(|root| kimix_workspace::session::git::detect_vcs_kind(&root).is_jj());
        if !is_jj {
            if kimix_workspace::session::git::should_warn_registry_disabled(
                is_jj,
                registry_client.is_some(),
            ) {
                kimix_workspace::session::git::warn_registry_disabled_restore(resolved_session_id);
            }
...
pub async fn rehydrate_session_in_worktree(
    ...
    let session_summary_exists = crate::util::kimix_home::sessions_cwd_dir(&req.source_cwd)
        .join(&req.session_id)
        .join("summary.json")
        .exists();
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(kimix_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(kimix_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(kimix_workspace::session::git::RestoreDegree::HeadOnly)
        );
    ...
    fn make_wt_record(path: &str, source_repo: &str) -> kimix_fast_worktree::WorktreeRecord {
        kimix_fast_worktree::WorktreeRecord {
            id: format!("wt-{}", path.replace('/', "-")),
            path: std::path::PathBuf::from(path),
            source_repo: std::path::PathBuf::from(source_repo),
            repo_name: "repo".into(),
            kind: kimix_fast_worktree::WorktreeKind::Session,
    ...
    fn repo_wide_resolve_finds_session_in_sibling_cwd_skipping_remote() {
        ...
        let encoded = crate::util::kimix_home::encode_cwd_dirname(sibling_cwd);
    ...
    async fn resume_in_worktree_falls_through_to_remote_when_not_found_locally() {
        ...
        let ops = kimix_workspace::WorkspaceOps::for_test();
    ...
    async fn checkout_persisted_head_stashes_dirty_worktree_state() {
        ...
        assert!(
            list_out.contains("kimix: pre-restore-code sess-dirty-wt"),
