...
pub enum PendingKind {
    ...
    Question,
    /// `Kimix/exit_plan_mode` plan approval.
    PlanApproval,
}

/// Whether a blocking plan-approval reverse-request is parked in `pending`.
///
/// The resume re-park issues `Kimix/exit_plan_mode` from a detached task
...
mod tests {
    ...
    fn guard_inserts_then_removes() {
        ...
        let gateway = GatewaySender::new(tx);
        {
            ...
            assert_eq!(reg.lock().expect("lock poisoned").len(), 1);
            assert_eq!(
                reg.lock().expect("lock poisoned").get("call-1").copied(),
                Some(PendingKind::Permission)
            );
        }
        assert!(reg.lock().expect("lock poisoned").is_empty());
    }
    ...
    fn has_parked_plan_approval_only_counts_plan_approval() {
        ...
        reg.lock().expect("lock poisoned").remove("plan");
        assert!(!has_parked_plan_approval(&reg));
    }
    ...
    fn has_parked_plan_approval_recovers_poisoned_lock() {
        ...
        let _ = std::thread::spawn(move || {
            let _g = reg_poison.lock().expect("lock poisoned");
