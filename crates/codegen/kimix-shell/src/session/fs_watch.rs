//! Session-level fs-watch policy over [`kimix_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `kimix_fsnotify`. This module
...
use kimix_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
use tokio::sync::mpsc;
...
/// Dedup key for `Kimix/git_head_changed`, shared by the watcher's `GitHead`
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> kimix_workspace::file_system::FileIndexDelta {
    use kimix_workspace::file_system::FileIndexDelta;
...
async fn refresh_codebase_graph_after_head_change(
    ...
    cmd.envs(kimix_tools::util::pager_env());
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `Kimix/gitHeadChanged`; opt-in (absent => off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `Kimix/fs_notify` wire protocol;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = kimix_workspace::session::git::get_branch(&self.cwd).await;
        let commit = kimix_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = kimix_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = kimix_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: kimix_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root = kimix_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || kimix_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = kimix_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(kimix_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: kimix_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
