...
use kimix_chat_state::compaction_utils::{
...
/// Override with `KIMIX_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("KIMIX_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("KIMIX_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("KIMIX_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG KIMIX_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(kimix_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// (`Kimix-chat-state` `replace_conversation`, the authority): it matches the reseed's
...
impl SessionActor {
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    kimix_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    kimix_chat_state::estimate_conversation_tokens(&full_conv),
                );
    ...
    async fn run_compact_inner(
        ...
        let summary_strips_reasoning = sampling_config
            ...
            .map(|c| c.api_backend == ApiBackend::Messages)
            .unwrap_or(false);
        let _model_id = sampling_config.map(|c| c.model).unwrap_or_default();
        let compact_source = trigger_str;
        self.dispatch_hook(
            kimix_hooks::event::HookEventName::PreCompact,
            kimix_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            kimix_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            kimix_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            kimix_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let effective_tool_defs: Vec<kimix_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            kimix_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<kimix_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(kimix_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<kimix_sampling_types::HostedTool> = if use_backend_search {
        ...
        let estimated_input_tokens =
            kimix_chat_state::estimate_conversation_tokens(&simplified_messages);
        ...
        let fr_config = kimix_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match kimix_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(kimix_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(kimix_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(kimix_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = kimix_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    kimix_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                kimix_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                kimix_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use kimix_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn kimix_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn kimix_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : kimix_log::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        kimix_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            kimix_hooks::event::HookEventName::PostCompact,
            kimix_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &kimix_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = kimix_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
...
mod inline_auto_compact_flow_tests {
    ...
    use kimix_workspace::file_system::MockFs;
    use kimix_workspace::permission::PermissionHandle;
    ...
    async fn create_test_actor(
        ...
        gateway_tx: mpsc::UnboundedSender<kimix_acp_lib::AcpClientMessage>,
        ...
        let hunk_tracker_handle = kimix_hunk_tracker::HunkTrackerActor::spawn(
            ...
            cwd.to_path_buf(),
            hunk_tx,
            kimix_hunk_tracker::TrackingMode::AgentOnly,
        ...
        let chat_state_handle = kimix_chat_state::ChatStateActor::spawn(
            vec![],
            kimix_sampling_types::SamplingConfig {
            ...
            Box::new(kimix_chat_state::NullChatPersistence),
            chat_event_tx,
            tokio_util::sync::CancellationToken::new(),
        );
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: kimix_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: kimix_agent_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: kimix_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: kimix_sampler::SamplerHandle::noop(),
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<kimix_acp_lib::AcpClientMessage>();
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use kimix_test_support::MockInferenceServer;
        use std::sync::atomic::Ordering::Relaxed;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): Kimix usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("Kimix usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> kimix_sampler::SamplingErrorInfo {
        kimix_sampler::SamplingErrorInfo {
            kind: kimix_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_triggers_when_tokens_exceed_new_window() {
        ...
        local
            .run_until(async {
                let (gateway_tx, _) = mpsc::unbounded_channel::<kimix_acp_lib::AcpClientMessage>();
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = kimix_sampler::SamplingErrorInfo {
                    kind: kimix_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = kimix_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = kimix_chat_state::CompactionMode::Summary;
