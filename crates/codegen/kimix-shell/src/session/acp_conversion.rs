//! ACP conversion functions for `Kimix-tools`'s `ToolOutput`.
//!
//! These standalone functions convert `kimix_tools::types::output::ToolOutput`
...
use kimix_tool_types::{KillTaskOutput, TaskOutputOutput};
...
/// directory (e.g., `/root/.Kimix/worktrees/project/fork-019cb252-...`). The
...
impl PathRewriter {
    ...
    ///
    /// Handles both plain paths (e.g., `/root/.Kimix/worktrees/project/fork-...`)
    /// and URL-encoded paths (e.g., `%2Froot%2F.Kimix%2Fworktrees%2F...`) that
...
pub fn acp_tool_update(
    ...
    match output {
        ...
        ToolOutput::WebFetch(web_fetch_output) => {
            use kimix_tools::types::output::WebFetchOutput;
        ...
        ToolOutput::Todo(todo_output) => {
            use kimix_tools::types::output::TodoWriteOutput;
        ...
        ToolOutput::AskUserQuestion(ask) => {
            let message = match ask {
                kimix_tools::types::output::AskUserQuestionOutput::UserAnswered { message } => {
                    message.clone()
                }
                kimix_tools::types::output::AskUserQuestionOutput::QuestionsSent {
                    message,
                    ..
                } => message.clone(),
            };
        ...
        ToolOutput::EnterPlanMode(enter) => {
            let message = match enter {
                kimix_tools::types::output::EnterPlanModeOutput::Entered { message, .. } => {
                    message.clone()
                }
            };
            Some(acp::ToolCallUpdate::new(
        ...
        ToolOutput::ExitPlanMode(exit) => {
            let message = match exit {
                kimix_tools::types::output::ExitPlanModeOutput::PlanReady { message, .. } => {
                    message.clone()
                }
                kimix_tools::types::output::ExitPlanModeOutput::EmptyPlan { message, .. } => {
                    message.clone()
                }
            };
            Some(acp::ToolCallUpdate::new(
...
///
/// This converts `Kimix-tools`' TodoItem (which has `id`, `content: Option<String>`,
...
mod tests {
    ...
    fn test_acp_tool_update_todo_returns_completed() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            ...
            todos: vec![],
            state: kimix_tools::implementations::kimix::todo::TodoState::default(),
        }));
        let update = acp_tool_update(&output, "call-1", None, None).unwrap();
        assert_eq!(update.fields.status, Some(acp::ToolCallStatus::Completed));
    }
    ...
    fn test_turn_end_plan_cleanup_preserves_semantics_and_priority() {
        use crate::tools::todo::plan_entry_from_todo_item;
        use kimix_tools::implementations::kimix::todo::{TodoItem, TodoPriority, TodoStatus};
    ...
    fn test_acp_plan_update_todo() {
        let output = ToolOutput::Todo(TodoWriteOutput::TodosUpdated(TodoWriteSuccess {
            summary_for_prompt: "tasks".to_string(),
            todos: vec![kimix_tools::implementations::kimix::todo::TodoItem {
                content: "Task 1".to_string(),
                priority: kimix_tools::implementations::kimix::todo::TodoPriority::Medium,
                status: kimix_tools::implementations::kimix::todo::TodoStatus::Completed,
                meta: None,
            }],
            state: kimix_tools::implementations::kimix::todo::TodoState::default(),
        }));
    ...
    fn test_path_rewriter_new_returns_some_when_different() {
        let rw = PathRewriter::new(
            "/root/.kimix/worktrees/project/ab-123",
    ...
    fn test_path_rewriter_rewrite_text() {
        let rw = PathRewriter::new(
            "/root/.kimix/worktrees/myproject/ab-123",
            Some("/testbed/myproject"),
        )
        .unwrap();
        let input = "File at /root/.kimix/worktrees/myproject/ab-123/src/main.rs";
    ...
    fn test_path_rewriter_rewrite_path() {
        ...
        .unwrap();
        let path = Path::new("/root/.kimix/worktrees/myproject/ab-123/src/lib.rs");
    ...
    fn test_path_rewriter_rewrites_raw_output_json() {
        ...
        let output = ToolOutput::ReadFile(ReadFileOutput::FileContent(FileContent {
            content: "content".to_string(),
            content_concise: None,
            absolute_path: PathBuf::from("/root/.kimix/worktrees/myproject/ab-123/src/main.rs"),
            offset: None,
            limit: None,
            raw_output: "content".to_string(),
            total_lines: 100,
    ...
    fn test_path_rewriter_rewrites_list_dir_raw_output() {
        ...
        let output = ToolOutput::ListDir(ListDirOutput::Content(ListDirContent {
            content: "file1.rs\nfile2.rs".to_string(),
            absolute_root_path: PathBuf::from("/root/.kimix/worktrees/myproject/ab-123/src"),
        }));
        let json = raw_output_json(&output, Some(&rw)).unwrap();
        let round_tripped: ToolOutput = serde_json::from_value(json).unwrap();
        match round_tripped {
    ...
    fn test_path_rewriter_rewrites_bash_command_and_output() {
        ...
        let output = ToolOutput::Bash(BashOutput {
            output: b"listing /root/.kimix/worktrees/myproject/ab-123/src".to_vec(),
            output_for_prompt: String::new(),
            exit_code: 0,
            command: "ls /root/.kimix/worktrees/myproject/ab-123/src".to_string(),
            truncated: false,
            signal: None,
            timed_out: false,
            description: None,
            current_dir: "/root/.kimix/worktrees/myproject/ab-123".to_string(),
    ...
    fn test_path_rewriter_rewrites_search_replace_diff_path() {
        ...
        let output = ToolOutput::SearchReplace(SearchReplaceOutput::EditsApplied(
            SearchReplaceEditsApplied {
                ...
                absolute_path: PathBuf::from("/root/.kimix/worktrees/myproject/ab-123/src/lib.rs"),
        ...
        assert!(
            !raw_str.contains("/root/.kimix/worktrees/myproject/ab-123"),
    ...
    fn test_rewrite_handles_url_encoded_paths() {
        let rw = PathRewriter::new(
            "/root/.kimix/worktrees/project/ab-123-a-overlay",
            Some("/home/user/project"),
        )
        .unwrap();
    ...
    fn test_rewrite_handles_plain_paths() {
        ...
        .unwrap();
        let input = "file: /root/.kimix/worktrees/project/ab-123-a-overlay/src/main.rs";
        let result = rw.rewrite(input);
    ...
    fn test_rewrite_json_handles_url_encoded_paths() {
        ...
        .unwrap();
        let encoded = urlencoding::encode("/root/.kimix/worktrees/project/ab-123");
    ...
    fn test_maybe_rewrite_with_rewriter_sanitizes_error_text() {
        let rw = PathRewriter::new(
            "/root/.kimix/worktrees/project/ab-999",
            Some("/home/user/project"),
        )
        .unwrap();
        let error_text = "Tool `read_file` failed: IO error reading /root/.kimix/worktrees/project/ab-999/src/lib.rs".to_string();
    ...
    fn test_no_rewriter_preserves_original_paths() {
        ...
        match round_tripped {
            ToolOutput::ReadFile(ReadFileOutput::FileContent(fc)) => {
                assert_eq!(
                    fc.absolute_path,
                    PathBuf::from("/root/.kimix/worktrees/myproject/ab-123/src/main.rs")
                );
            }
            other => panic!("Expected ReadFile, got {:?}", other),
        }
