...
//!
//! Canonical definitions live in `kimix_prompt_queue`; this re-export keeps every existing
//! `crate::session::prompt_queue::*` and cross-crate `kimix_shell::session::prompt_queue::*`
//! path resolving without edits.
pub use kimix_prompt_queue::{QueueChanged, QueueEntryMeta, QueueEntryWire};

// Outbound method for broadcast_queue_changed. An ACP routing concern, not a queue concern.
pub const QUEUE_CHANGED_METHOD: &str = "kimix/queue/changed";
...
mod tests {
    ...
    fn queue_changed_serializes_camel_case_with_session_id() {
        let payload = QueueChanged {
            ...
            entries: vec![QueueEntryWire {
                id: "p1".to_string(),
                version: 0,
                owner: Some("kimix-tui".to_string()),
                last_editor: None,
    ...
    fn queue_entry_wire_round_trips_last_editor() {
        let entry = QueueEntryWire {
            ...
            version: 3,
            owner: Some("kimix-tui".to_string()),
            last_editor: Some("kimix-vscode".to_string()),
            kind: "prompt".to_string(),
            text: "hello".to_string(),
            position: 0,
        };
        let json = serde_json::to_value(&entry).unwrap();
        assert_eq!(json["lastEditor"], "kimix-vscode");
