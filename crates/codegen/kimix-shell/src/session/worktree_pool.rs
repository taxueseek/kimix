...
//!   under `~/.kimix/worktree_pool/<instance_id>/` with a `.pid` liveness
//!   file. Startup cleanup only removes directories for dead processes.
use std::path::{Path, PathBuf};
use std::sync::Arc;
use std::time::Duration;

use kimix_fast_worktree::{WorktreeBuilder, WorktreeSync};
...
use kimix_tty_utils::git_command;
...
/// `~/.kimix/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.kimix/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.kimix/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.kimix/worktree_pool/`, reads each `.pid` file, and checks
...
async fn warm_source_repo(repo_path: &Path) {
    ...
    let _ = tokio::task::spawn_blocking(move || {
        ...
        kimix_tools::util::detach_std_command(&mut cmd);
...
/// `CleanAll` mode. Placed under `~/.kimix/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(kimix_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(kimix_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                kimix_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || kimix_fast_worktree::remove_worktree(&path_owned))
            .await;
...
"#;
    ...
    async fn test_count_ready_worktrees() {
        ...
        index
            ...
            .unwrap();
        index.write().expect("rwlock write poisoned");
    ...
    async fn test_adopt_in_fill_loop_creates_deficit() {
        ...
        {
            let mut cache = ADOPTABLE_CACHE.lock().expect("lock poisoned");
