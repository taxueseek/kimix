...
pub(crate) struct RoleToolNames {
    ...
    /// (the default Kimix host's `search_replace` mutator) when `Write`
...
impl RoleToolNames {
    ...
    /// `search_replace` on the default Kimix host) instead of the literal
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default Kimix host's pre-spawn describe probe exposes only
...
pub(crate) mod tests {
    ...
    pub(crate) fn assert_no_tool_placeholders(rendered: &str) {
        static TOOL_PLACEHOLDER_RE: LazyLock<regex::Regex> =
            LazyLock::new(|| regex::Regex::new(r"\{[A-Z_]+_TOOL\}|\{TOOLSET_TOOLS\}").unwrap());
    ...
    #[test]
    fn from_summary_uses_kimix_names() {
        // A Kimix toolset: client names match the literal defaults.
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // on the default Kimix host (the stock `web_search`/`web_fetch`).
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_kimix_host() {
        // Default Kimix host: the pre-spawn describe probe exposes only
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default Kimix parent bridge: no `Write`, only `Edit`
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_kimix_mutator() {
        ...
        // default Kimix host (Edit-only toolset), so a fail-open retry
