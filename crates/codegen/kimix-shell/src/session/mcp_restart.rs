...
//! window containing a [`kimix_mcp::servers::McpClientEventKind::TransportClosed`]
//! or [`kimix_mcp::servers::McpClientEventKind::HandshakeFailed`] key for a
...
//!    [`kimix_mcp::servers::start_mcp_server`] sets
...
//!    push and stop.
//! 5. **Already-Empty** — see the [`kimix_mcp::servers::ClientStateKind::Empty`]
...
use kimix_mcp::servers::{McpClientEventKind, McpServerName};
...
pub trait RestartActions {
    ...
    /// Push an already-built `Kimix/mcp/server_status` payload to the
...
/// ACP `Kimix/mcp/server_status` notification. Failures are logged and
