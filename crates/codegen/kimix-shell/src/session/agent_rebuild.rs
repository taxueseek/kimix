...
//! [`kimix_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`kimix_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`kimix_agent::Agent`] owns an [`kimix_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`kimix_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use kimix_agent::config::AgentDefinition;
use kimix_agent::error::AgentBuildError;
use kimix_agent::prompt::context::PromptAudience;
use kimix_agent::prompt::skills::SkillsConfig;
use kimix_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use kimix_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use kimix_tools::implementations::kimix::ask_user_question::types::UserQuestionRequest;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<kimix_tools::SharedAttributionCallback>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use kimix_tools::implementations::kimix::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(kimix_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(kimix_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        {
            use kimix_tools::implementations::kimix::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            kimix_tools::computer::local::LocalTerminalBackend::new_local(
                kimix_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(kimix_tools::types::tool::ToolKind::Task)
            .expect("Kimix Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("Kimix Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_kimix())
                    .await
