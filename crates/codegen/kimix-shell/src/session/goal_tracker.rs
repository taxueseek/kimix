...
/// scratch root (`<temp_dir>/Kimix-goal-<id>`) and the verifier
...
/// Private per-goal scratch root: `<temp_dir>/Kimix-goal-<verifier_id>`.
///
...
pub(crate) fn goal_scratch_root(verifier_id: &str) -> PathBuf {
    std::env::temp_dir().join(format!("kimix-goal-{verifier_id}"))
}
...
mod tests {
    ...
    /// `Kimix-goal-<12 hex chars>` scratch-root format (and the
    ...
    fn serde_skip_fields_reset_on_deserialize() {
        ...
        t.update_live_progress(
            100,
            vec![("kimix-4".to_owned(), 60), ("kimix-3".to_owned(), 40)],
            200_000,
            50,
            3,
            10,
    ...
    /// `temp_dir()` + `verifier_id`: a `Kimix-goal-<vid>` root with an
    ...
    fn scratch_path_helpers_derive_pinned_layout() {
        ...
        assert_eq!(root, std::env::temp_dir().join("kimix-goal-vid123"));
        assert_eq!(
            implementer_scratch_dir("vid123"),
            std::env::temp_dir()
                .join("kimix-goal-vid123")
                .join("implementer"),
        );
        assert_eq!(
