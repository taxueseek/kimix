...
/// A feedback entry persisted to `~/.kimix/sessions/.../feedback.jsonl`.
///
...
pub enum PersistenceMsg {
    ...
    CurrentModel {
        ...
        /// The active agent definition name (e.g. `"Kimix"`).
...
pub use kimix_shared::session::session_dir;
...
pub fn session_exists_for_cwd(session_id: &str, cwd: &str) -> bool {
    let sessions_root = crate::util::kimix_home::kimix_home().join("sessions");
...
fn session_exists_for_cwd_in_root(session_id: &str, cwd: &str, sessions_root: &Path) -> bool {
    let encoded = crate::util::kimix_home::encode_cwd_dirname(cwd);
...
/// `kimix -r <remote_id>` in the same cwd, this function returns the already-restored
...
fn find_local_child_for_remote_in_root(
    ...
    // `kimix -r <remote_id>` before this fix was deployed.
...
/// Searches across ALL cwd directories under `~/.kimix/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.kimix/sessions/`.
///
...
fn resolve_local_session_any_cwd_in_root(session_id: &str, sessions_root: &Path) -> Option<String> {
    ...
    for entry in entries.flatten() {
        ...
        if is_persisted_session_dir(&session_path) {
            ...
            if let Some(decoded) = crate::util::kimix_home::decode_cwd_from_dirname(&path) {
                return Some(decoded);
            }
        }
    }
...
/// Separated so tests can use a tempdir without touching the real Kimix home.
...
pub fn find_summary_by_session_id(session_id: &str) -> Option<Summary> {
    find_summary_by_session_id_in_root(session_id, &kimix_home().join("sessions"))
}
...
pub fn resumed_session_sandbox_profile(
    ...
    resumed_session_sandbox_profile_in_root(session_id, cwd, &kimix_home().join("sessions"))
}
...
pub struct Summary {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub request_id: Option<String>,
    /// Absolute path to the `.Kimix` directory, used by reconstruction.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub kimix_home: Option<String>,
...
/// Current `kimix_home` as a UTF-8 string, or `None` if the path isn't valid UTF-8.
pub fn kimix_home_string() -> Option<String> {
...
impl Summary {
    pub fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            kimix_workspace::session::git::resolve_persisted_session_git_metadata_sync(
...
/// Collect MCP server stderr logs from `~/.kimix/logs/mcp/` for inclusion in the session archive.
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = kimix_config::kimix_home().join("logs").join("mcp");
...
/// worktree containing this session's cwd so `kimix worktree gc` expires by
...
pub async fn list_summaries(cwd: Option<&str>) -> io::Result<Vec<Summary>> {
    let root_dir = crate::util::kimix_home::kimix_home();
    let storage: Box<dyn StorageAdapter> = Box::new(JsonlStorageAdapter::with_root(root_dir));
...
/// Walk `~/.kimix/sessions/` and delete files with mtime older than `ttl_days`.
...
pub fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "kimix_shell::session::persistence",
...
mod agent_name_persistence_tests {
    ...
    fn summary_round_trips_various_agent_names() {
        for name in ["cursor", "kimix", "kimix-plan", "codex", "browser-use"] {
            let mut summary = Summary::new(
                &Info {
                    id: acp::SessionId::new("test"),
                    cwd: "/tmp".into(),
...
mod session_exists_tests {
    ...
    fn returns_false_when_root_does_not_exist() {
        let root = std::path::PathBuf::from("/nonexistent/kimix/sessions");
...
mod session_exists_for_cwd_tests {
    ...
    fn session_under_different_cwd_is_not_considered_present() {
        ...
        let encoded_a = crate::util::kimix_home::encode_cwd_dirname("/project/alpha");
    ...
    fn resolve_local_session_any_cwd_skips_stub_and_finds_real() {
        ...
        let encoded_a = crate::util::kimix_home::encode_cwd_dirname(cwd_a);
        let dir_a = root.join(&encoded_a).join(session_id);
        fs::create_dir_all(&dir_a).unwrap();
        fs::write(dir_a.join("summary.json"), b"{}").unwrap();
        ...
        let encoded_b = crate::util::kimix_home::encode_cwd_dirname(cwd_b);
...
mod find_local_child_tests {
    ...
    fn returns_none_when_no_child_exists() {
        let tmp = TempDir::new().unwrap();
        let root = tmp.path().join("sessions");
        let encoded = crate::util::kimix_home::encode_cwd_dirname("/work");
    ...
    /// Regression: a second `kimix -r <remote_id>` must return the existing child
...
mod resolve_local_session_tests {
    ...
    // setting up the on-disk structures under a fake Kimix home.
    ...
    fn exact_match_returns_original_id() {
        ...
        // (We can't call resolve_local_session directly because it uses kimix_home(),
    ...
    fn no_match_returns_none() {
        ...
        fs::create_dir_all(root.join(crate::util::kimix_home::encode_cwd_dirname(cwd))).unwrap();
...
mod repo_wide_resolution_tests {
    ...
    fn skips_images_only_stub_and_resolves_real_sibling() {
        ...
        let encoded = crate::util::kimix_home::encode_cwd_dirname(exact_cwd);
