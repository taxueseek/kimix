...
use bytes::Bytes;
use kimix_tools::util::image_compress::{FilterType, ReEncodeParams, re_encode_under_limit};
use std::borrow::Cow;
...
/// `kimix_tools::reminders` (free-fn shape because this module has no
/// `SessionActor`; see `reminder_wrapper_tag`).
fn reminder_tag(is_cursor: bool) -> &'static str {
    let _ = is_cursor;
    kimix_tools::reminders::DEFAULT_REMINDER_TAG
}
...
pub(crate) fn persisted_image_reject_reason(bytes: &[u8]) -> Option<String> {
    ...
    use kimix_tools::util::image_validate as iv;
...
pub(crate) fn inline_attach_verdict(data_b64: &str) -> InlineAttachVerdict {
    ...
    let Ok((w, h, _)) = kimix_tools::util::image_validate::validate_image_bytes_with(&raw, false)
    else {
        return InlineAttachVerdict::Unreadable;
    };
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        kimix_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !kimix_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = kimix_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
...
mod tests {
    ...
    fn make_ico_content(width: u32, height: u32) -> ImageContent {
        let png = make_test_png(width, height);
        let buf = kimix_test_utils::image::ico_with_png_frame(&png, width as u8, height as u8);
        ImageContent::new(
    ...
    fn re_encode_fallback_notice_picks_tag_per_harness() {
        ...
        let kimix = render_re_encode_fallback_notice(&notes, false);
        assert!(kimix.contains("<system-reminder>"));
        assert!(kimix.contains("</system-reminder>"));
        assert!(!kimix.contains("<system_reminder>"));
        assert!(kimix.contains("<image_re_encode_fallback>"));
    }
    #[test]
    fn display_format() {
    ...
    fn persisted_image_reject_reason_verdicts() {
        ...
        let ico = kimix_test_utils::image::ico_with_png_frame(&make_test_png(16, 16), 16, 16);
    ...
    fn image_dropped_notice_picks_tag_per_harness() {
        ...
        let kimix = render_image_dropped_notice(&notes, false);
        assert!(kimix.contains("<system-reminder>"));
        assert!(kimix.contains("</system-reminder>"));
        assert!(!kimix.contains("<system_reminder>"));
        assert!(kimix.contains("<image_dropped_notice>"));
        assert!(kimix.contains("Image 5"));
