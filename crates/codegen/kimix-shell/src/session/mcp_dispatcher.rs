...
//!
//! Receives [`kimix_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`kimix_mcp::liveness`]),
//! - the [`kimix_mcp::servers::KimixClientHandler`] (server-pushed
...
//! `Kimix/mcp/server_status` and the payload schema defined by
...
/// [`kimix_mcp::servers::McpClientEventKind`]). Used as the
...
///   `ConfigRemoved` keys,
/// - emit one ACP `Kimix/mcp/server_status` push per surviving
///   buffer entry, via the provided gateway.
///
/// `gateway` is a [`kimix_acp_lib::AcpAgentGatewaySender`] (forwarded
...
/// 3. `flush_window` — emit ACP `Kimix/mcp/server_status` per
///    surviving entry.
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        use kimix_mcp::servers::{McpClient, McpState};
    ...
    fn discard_gateway() -> kimix_acp_lib::AcpAgentGatewaySender {
    ...
    async fn run_dispatcher_schedules_restart_on_transport_closed() {
        use kimix_mcp::servers::McpState;
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = kimix_acp_lib::AcpAgentGatewaySender::new(gw_tx);
