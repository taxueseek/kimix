...
pub struct PlanModeTracker {
    ...
    /// `~/.kimix/sessions/<cwd>/<session_id>/plan.md`
...
impl PlanModeTracker {
    ...
    /// directory (e.g., `~/.kimix/sessions/<encoded-cwd>/<session-id>/`).
...
pub fn plan_mode_reminder_full_template() -> &'static str {
    "\
Plan mode is active. This mode answers: **how to walk?**

Apply this engineering framework to decompose the plan:

1. **Step decomposition** — break the approach into concrete, executable steps.
2. **Dependency mapping** — identify which steps depend on others (topological order).
3. **Critical path** — find the longest dependency chain; this determines minimum time.
4. **Parallel opportunities** — find steps that can run concurrently (no dependencies between them).
5. **Checkpoints** — define verification points after key steps.

Output format for your plan:
```
## Execution Plan

| Step | Description | Depends On | Est. Time | Checkpoint |
|------|-------------|------------|-----------|------------|
| 1 | ... | — | 5 min | Verify X |
| 2 | ... | — | 3 min | — |
| 3 | ... | 1, 2 | 10 min | Verify Y |

### Critical Path
1 → 3 → 5 (total: 25 min)

### Parallel Opportunities
Steps 1 and 2 can run concurrently.
...
mod tests {
    ...
    fn plan_file_in_session_dir() {
        let t = PlanModeTracker::new(PathBuf::from("/home/user/.kimix/sessions/proj/abc-123"));
        assert_eq!(
            t.plan_file_path(),
            Path::new("/home/user/.kimix/sessions/proj/abc-123/plan.md")
        );
    }
    #[test]
    ...
    /// Build a test TemplateRenderer with standard Kimix tool mappings.
    ...
    fn is_plan_file_write_exact_match() {
        let plan = Path::new("/home/user/.kimix/sessions/proj/abc/plan.md");
        let target = Path::new("/home/user/.kimix/sessions/proj/abc/plan.md");
