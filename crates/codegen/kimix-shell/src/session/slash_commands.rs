...
use kimix_tools::implementations::skills::skill::format_skill_name;
use kimix_tools::implementations::skills::types::SkillInfo;
use std::sync::OnceLock;

/// Cache of user-defined commands loaded from `~/.kimix/commands/*.md`.
static USER_COMMANDS: OnceLock<Vec<UserCommand>> = OnceLock::new();

/// Scan `~/.kimix/commands/` for user-defined command files (`*.md`).
///
...
/// ---
/// Command body text. `$ARGUMENTS` is replaced with user input.
/// ```
///
/// Commands with duplicate names (after the first) are silently skipped.
/// Loaded lazily on first access via `OnceLock`.
pub(crate) fn load_user_commands() -> &'static [UserCommand] {
    USER_COMMANDS.get_or_init(|| {
        let dir = crate::util::kimix_home::kimix_home().join("commands");
        ...
        if let Ok(entries) = std::fs::read_dir(&dir) {
            for entry in entries.flatten() {
                ...
                if let Some(cmd) = parse_user_command_file(&path, &name) {
                    commands.push(cmd);
                }
            }
        }
...
fn parse_user_command_file(path: &std::path::Path, name: &str) -> Option<UserCommand> {
...
/// Build the expanded prompt for a user-defined command invocation.
fn build_user_command_prompt(cmd: &UserCommand, args: &str) -> String {
    let mut prompt = cmd.body.replace("$ARGUMENTS", args);
    // Also support $ARGS as shorthand
    prompt = prompt.replace("$ARGS", args);
    prompt
}

/// Record a slash command usage for MRU (Most Recently Used) tracking.
///
/// Maintains `~/.kimix/slash-mru.json` with `{command_name: unix_timestamp}`.
/// Commands used more recently have higher timestamps, which can be used
/// to boost their position in autocomplete suggestions.
pub(crate) fn record_slash_mru(command_name: &str) {
    let path = crate::util::kimix_home::kimix_home().join("slash-mru.json");
    let mut mru: std::collections::HashMap<String, u64> = std::fs::read_to_string(&path)
        .ok()
    ...
    let now = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    mru.insert(command_name.to_string(), now);
    // Keep only the 50 most recent entries
    if mru.len() > 50 {
        let mut entries: Vec<_> = mru.into_iter().collect();
        entries.sort_by_key(|(_k, v)| std::cmp::Reverse(*v));
        entries.truncate(50);
        mru = entries.into_iter().collect();
    }
    let _ = std::fs::write(&path, serde_json::to_string(&mru).unwrap_or_default());
}

/// Load MRU scores for boosting autocomplete suggestions.
/// Returns a map of command_name → recency score (0.0 to 1.0).
pub(crate) fn load_slash_mru_scores() -> std::collections::HashMap<String, f64> {
    let path = crate::util::kimix_home::kimix_home().join("slash-mru.json");
    let mru: std::collections::HashMap<String, u64> = std::fs::read_to_string(&path)
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
        .unwrap_or_default();
    if mru.is_empty() {
    ...
    let now = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let newest_ts = mru.values().copied().max().unwrap_or(now);
    let oldest_ts = mru.values().copied().min().unwrap_or(now);
    let range = (newest_ts.saturating_sub(oldest_ts)) as f64;
    if range < 1.0 {
        #[allow(clippy::iter_kv_map)]
        {
            return mru.into_iter().map(|(k, _)| (k, 0.5)).collect();
        }
    }
    mru.into_iter()
        .map(|(k, v)| {
            let score = (v.saturating_sub(oldest_ts)) as f64 / range;
            (k, score)
        })
        .collect()
}
...
/// A slash command — built-in, user-defined, or from a SKILL.md file.
...
/// A user-defined command loaded from `~/.kimix/commands/*.md`.
#[derive(Debug, Clone)]
pub(crate) struct UserCommand {
    pub name: String,
    pub description: String,
    pub argument_hint: Option<String>,
    /// The prompt body injected when the command is invoked.
    /// `$ARGUMENTS` is replaced with the user's arguments.
    pub body: String,
}
...
impl<'a> SlashCommand<'a> {
    ...
    pub fn description(&self) -> &str {
        match self {
            ...
            SlashCommand::User(u) => &u.description,
        }
    }
    ...
    pub fn argument_hint(&self) -> Option<&str> {
        match self {
            ...
            SlashCommand::User(u) => u.argument_hint.as_deref(),
        }
    }
}
...
pub(super) fn all_commands<'a>(
    ...
    // User-defined commands from ~/.kimix/commands/*.md
    let user_cmds = USER_COMMANDS.get().map(|v| v.as_slice()).unwrap_or(&[]);
    commands.extend(user_cmds.iter().map(SlashCommand::User));
    commands
}
        ...
        .flat_map(|cmd| {
            let entries: Vec<acp::AvailableCommand> = match cmd {
                ...
                SlashCommand::User(u) => {
                    ...
                    .cloned();
                    vec![
                        acp::AvailableCommand::new(u.name.clone(), cmd.description().to_string())
                            .input(cmd.argument_hint().map(|hint| {
                                acp::AvailableCommandInput::Unstructured(
                                    acp::UnstructuredCommandInput::new(hint.to_string()),
                                )
...
// ── Kimix/commands/list ext method ────────────────────────────────
...
///   matching today's Kimix flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use kimix_tools::implementations::skills::skill::{
...
pub(super) fn resolve(
    ...
    // Record MRU for autocomplete boost
    #[allow(clippy::needless_borrow)]
    record_slash_mru(&command_name);

    // User-defined commands from ~/.kimix/commands/*.md
    // They behave like prompt-only commands: their body is injected
    // as a user message with $ARGUMENTS replaced.
    if let Some(user_cmd) = USER_COMMANDS
        .get()
        .and_then(|cmds| cmds.iter().find(|c| c.name == command_name))
    {
        let expanded = build_user_command_prompt(user_cmd, args);
        let mut blocks = vec![acp::ContentBlock::Text(acp::TextContent::new(expanded))];
...
/// `Kimix-tools` so it stays identical to the pager's `LoopCommand` and the
...
fn build_loop_prompt_blocks(args: &str) -> Vec<acp::ContentBlock> {
    use kimix_tools::implementations::kimix::{loop_schedule_instruction, loop_usage_message};
