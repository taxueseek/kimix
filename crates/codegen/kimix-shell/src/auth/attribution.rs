...
//!
//! 1. [`kimix_log::unified_log::warn`] for the local
//!    `~/.kimix/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`kimix_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`kimix_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`kimix_sampler::SamplerConfig::attribution_callback`] at every
...
use kimix_sampler::{Auth401AttributionCallback, SamplingConsumer};
use kimix_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`kimix_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `kimix_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn kimix_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `kimix_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// Tool-side hook: each tool client (web_search) in `Kimix-tools`
...
pub(crate) enum ConsumerKind {
    ...
    /// `"IdleResumeModelRefresh"`.
    IdleResumeModelRefresh,
    /// `kimix_tools::ToolConsumer::WebSearch` -- web search via
...
/// All 401 emit sites in `Kimix-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.kimix/logs/unified.jsonl) + scrubbed
    ...
    kimix_log::unified_log::warn("auth 401 attribution", session_id, Some(payload.clone()));
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.kimix/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `kimix_tools::Auth401AttributionCallback`
    ...
    mod span_capture {
        ...
        impl<S: Subscriber + for<'a> LookupSpan<'a>> Layer<S> for SpanCollector {
            fn on_new_span(&self, attrs: &Attributes<'_>, _id: &tracing::Id, _ctx: Context<'_, S>) {
                ...
                self.spans.lock().expect("lock poisoned").push(captured);
            }
        }
    ...
    fn record_auth_401_emits_otel_span_with_attribution_fields() {
        ...
        let spans = captured.lock().expect("lock poisoned");
