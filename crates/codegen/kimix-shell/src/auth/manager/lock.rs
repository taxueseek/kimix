...
mod tests {
    ...
    // (`Kimix-crash-handler/tests/integration.rs`), the holder is this very
    // test binary re-executed via `current_exe()`, gated by the
    // `KIMIX_TEST_LOCK_HOLDER` env var on an `#[ignore]`d entry-point test —
    // no external `python3` dependency.

    /// Line printed to stdout once the subprocess holds the flock.
    #[cfg(unix)]
    const LOCK_HOLDER_READY: &str = "__KIMIX_LOCK_HOLDER_READY__";

    /// Subprocess entry point for the cross-process lock tests. Only does
    /// anything when re-executed with `KIMIX_TEST_LOCK_HOLDER` set; a normal
    ...
    fn subprocess_lock_holder() {
        let Ok(spec) = std::env::var("KIMIX_TEST_LOCK_HOLDER") else {
    ...
    fn spawn_lock_holder_subprocess(
        ...
        let mut child = std::process::Command::new(exe)
            .env("KIMIX_TEST_LOCK_HOLDER", spec)
            .args([
                "--ignored",
                "--exact",
                "--nocapture",
