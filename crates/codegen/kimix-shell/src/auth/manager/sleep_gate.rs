...
impl SleepGate {
    pub(super) fn raise(&self) {
        *self.raised_at.write() = Some(GateRaise::now());
        kimix_log::unified_log::warn("auth.sleep.gate_set", None, None);
    }
...
impl AuthManager {
    ...
    /// [`kimix_system_power::PowerState`] for the canonical explanation of what a
    ...
    pub(crate) fn is_dark_wake(&self) -> bool {
        ...
        matches!(
            kimix_system_power::current_power_state(),
            kimix_system_power::PowerState::DarkWake
        )
    }
    ...
    pub fn start_system_power_listener(self: &Arc<Self>) {
        ...
        let listener = kimix_system_power::SystemPowerListener::start(move |event| {
            if let Some(this) = weak.upgrade() {
                let imminent = matches!(event, kimix_system_power::PowerEvent::WillSleep);
