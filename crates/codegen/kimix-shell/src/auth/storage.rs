...
//
// PRD F1: the OAuth token set lives in the system keyring (service `Kimix`,
...
pub(crate) const KEYRING_SERVICE: &str = "kimix";
...
/// Keyring permanently disabled — no macOS keychain, no prompts.
...
/// `pub(crate)` wrapper so sibling auth modules (e.g. xAI OIDC) can persist
/// without going through AuthManager's Kimi-scoped path.
pub(crate) fn write_auth_json_public(
    auth_file: &Path,
    auth_store: &AuthStore,
) -> std::io::Result<()> {
    write_auth_json(auth_file, auth_store)
}
...
pub fn read_token_by_scope(kimix_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = kimix_home.join("auth.json");
    let store =
        read_auth_json(&path).map_err(|_| anyhow::anyhow!("Not logged in. Run `kimix login`."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your auth token is invalid. Run `kimix login` to re-authenticate.")
    })
}

/// Read the API key from the `Kimix::api_key` scope in auth.json.
pub fn read_api_key(kimix_home: &Path) -> Option<String> {
    let path = kimix_home.join("auth.json");
    let map = read_auth_json(&path).ok()?;
    map.get(API_KEY_SCOPE).map(|a| a.key.clone())
}

/// Store a plain API key in auth.json under the `Kimix::api_key` scope.
///
...
pub fn store_api_key(kimix_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = kimix_home.join("auth.json");
...
/// Remove the `Kimix::api_key` scope from auth.json.
pub fn clear_api_key(kimix_home: &Path) -> std::io::Result<()> {
    let path = kimix_home.join("auth.json");
