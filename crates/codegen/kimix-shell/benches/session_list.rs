...
//! Run: `cargo bench -p Kimix-shell --bench session_list`
...
use kimix_fast_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use kimix_shell::session::info::Info;
use kimix_shell::session::persistence::Summary;
use kimix_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use kimix_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = kimix_shell::util::kimix_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = kimix_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = kimix_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(kimix_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("KIMIX_SHARE_DIR", home.path());
    }
    assert_eq!(kimix_shell::util::kimix_home::kimix_home(), home.path());
