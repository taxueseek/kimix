...
//! Each test spawns a real `kimix agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!

//! Run locally:
//! ```bash
//! cargo test -p Kimix-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.Kimix/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".kimix").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let kimix_dir = home.path().join(".kimix");
        std::fs::create_dir_all(&kimix_dir).expect("create .kimix dir");
        std::fs::write(
...
"#,
        ...
        let client = KimixStdioClient::spawn_with_home(&server, workdir.path(), home).await;
