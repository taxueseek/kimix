...
//! `KIMIX_PERF_SESSION_SRC=/path/to/<session-dir>`.
//!

//! Run:
//!   cargo test -p Kimix-shell --test session_load_perf -- --nocapture
//!   cargo test -p Kimix-shell --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use kimix_shell::session::info::Info;
use kimix_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use kimix_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
impl GenOpts {
    fn from_env() -> Self {
        ...
        let scale = g("KIMIX_PERF_SCALE", 1).max(1);
        Self {
            turns: g("KIMIX_PERF_TURNS", 80) * scale,
            acu_per_turn: g("KIMIX_PERF_ACU_PER_TURN", 15),
            catalog_commands: g("KIMIX_PERF_CATALOG_COMMANDS", 64),
            catalog_desc_len: g("KIMIX_PERF_CATALOG_DESC_LEN", 320),
            agent_chunks_per_turn: g("KIMIX_PERF_AGENT_CHUNKS_PER_TURN", 8),
            agent_chunk_len: g("KIMIX_PERF_AGENT_CHUNK_LEN", 2000),
            rewind_points: g("KIMIX_PERF_REWIND_POINTS", 60) * scale,
            files_per_rewind: g("KIMIX_PERF_FILES_PER_REWIND", 40),
            file_content_len: g("KIMIX_PERF_FILE_CONTENT_LEN", 8000),
        }
    }
}
...
/// `Info` and the session directory path. Uses `KIMIX_PERF_SESSION_SRC` if set
...
async fn prepare_session(root: &Path, cwd: &Path, opts: &GenOpts) -> (Info, PathBuf) {
    ...
    if let Ok(src) = std::env::var("KIMIX_PERF_SESSION_SRC") {
...
/// `--ignored` (optionally `KIMIX_PERF_SESSION_SRC=...`) to get the numbers.
...
fn generate_or_restore_rewind(path: &Path, opts: &GenOpts) {
    if std::env::var("KIMIX_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
use kimix_shell::agent::mvp_agent::MvpAgent;
...
async fn full_session_load_e2e() {
    ...
    let server = kimix_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    let cwd = TempDir::new().unwrap();
    let opts = GenOpts::from_env();
    let instr_log = kimix_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (kimix_home()/instrumentation mode are OnceLock).
    unsafe {
        std::env::set_var("KIMIX_SHARE_DIR", kimix_home.path());
        std::env::set_var("KIMIX_INSTRUMENTATION", "log");
        std::env::set_var("KIMIX_INSTRUMENTATION_LOG", &instr_log);
        std::env::set_var("KIMIX_CODE_BASE_URL", server.url());
        std::env::set_var("KIMIX_API_BASE_URL", server.url());
        std::env::set_var("XAI_API_KEY", "test-key-for-ci");
        std::env::set_var("KIMIX_TELEMETRY_ENABLED", "false");
        std::env::set_var("KIMIX_FEEDBACK_ENABLED", "false");
        std::env::set_var("KIMIX_TRACE_UPLOAD", "false");
    }
    ...
    let _ = tracing_subscriber::registry()
        .with(kimix_shell::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(kimix_home.path(), cwd.path(), &opts).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            let _ = kimix_shell::instrumentation::finalize();
