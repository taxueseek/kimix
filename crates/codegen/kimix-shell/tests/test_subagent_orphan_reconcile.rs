...
//! This test spawns a real `kimix agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p Kimix-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // KimixStdioClient sets HOME=<temp>; the binary uses <HOME>/.Kimix as KIMIX_SHARE_DIR.
        let kimix_home = shared_home.path().join(".kimix");
        let session_dir = locate_session_dir(&kimix_home, session_id.0.as_ref());
