...
Launches kimix agent stdio in isolated environments ($HOME override)
...
Usage:
    ...
    python3 crates/codegen/kimix-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/kimix-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/kimix-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    KIMIX_BINARY=/path/to/kimix-tui python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a kimix agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated kimix environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.kimix/auth.json   (copied from real home)
      ~/.kimix/memory/     (pre-populated by tests)
      ~/.kimix/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="kimix-memtest-")
        ...
        # Create .kimix dirs in fake home
        self.kimix_home = os.path.join(self.fake_home, ".kimix")
        self.memory_dir = os.path.join(self.kimix_home, "memory")
        self.logs_dir = os.path.join(self.kimix_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.kimix/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.kimix_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.kimix/config.toml (where the agent reads config)."""
        with open(os.path.join(self.kimix_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("KIMIX_BINARY", "kimix")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}kimix binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
...
def _default_config() -> str:
    embed_model = os.environ.get("KIMIX_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"KIMIX_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """KIMIX_MEMORY=1 env var enables memory even without config."""
    section("13. KIMIX_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"KIMIX_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with KIMIX_MEMORY=1")
        else:
            fail("no index despite KIMIX_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.kimix/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("KIMIX_BINARY", "kimix")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["KIMIX_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["KIMIX_NO_LEADER"] = "1"
...
def build_binary():
    """Build kimix-tui with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "kimix-tui")

    # Find rg for KIMIX_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["KIMIX_SHELL_BUNDLE_RG_PATH"] = rg

    print(f"{B}Building kimix-tui (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "kimix-tui", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "KIMIX_BINARY" in os.environ:
        binary = os.environ["KIMIX_BINARY"]
        print(f"Binary: {binary} (from $KIMIX_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["KIMIX_BINARY"] = binary
