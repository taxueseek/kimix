//! Integration test for `_kimix/session/update` notifications.
//!
...
use kimix_shell::session::info::Info as SessionInfo;
use kimix_shell::session::persistence::default_model_id;
use kimix_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};
...
/// must survive `updates.jsonl` and reload as a replayable `_kimix/session/update`.
#[tokio::test]
