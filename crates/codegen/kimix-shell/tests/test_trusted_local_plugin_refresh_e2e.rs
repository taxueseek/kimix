...
//! 4. Smoke-validate session JSON under `$KIMIX_SHARE_DIR/sessions/` after exit.
//!

//! Requires a built `kimix` binary (`KIMIX_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!

//! ```bash
//! cargo test -p Kimix-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p Kimix-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
use std::collections::HashMap;
use std::path::{Path, PathBuf};

use kimix_agent::plugins::SharedPluginRegistryHandle;
use kimix_agent::plugins::discovery::DiscoveryConfig;
use kimix_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _kimix_guard = EnvVarGuard::set("KIMIX_SHARE_DIR", &kimix_home);
    ...
    let mut registry = InstallRegistry::empty(kimix_home.join("installed-plugins"));
    ...
    // kimix_home() caches the first KIMIX_SHARE_DIR per process; a separate test
    ...
    assert_eq!(plugin.scope, kimix_agent::plugins::PluginScope::CliOverride);
...
#[tokio::test]
#[ignore = "requires pre-built kimix binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/KIMIX_SHARE_DIR via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via kimix_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _kimix_guard = EnvVarGuard::set("KIMIX_SHARE_DIR", &kimix_home);
    ...
    cmd.env("HOME", &home);
    cmd.env("KIMIX_SHARE_DIR", &kimix_home);
    ...
    // Smoke: session storage under KIMIX_SHARE_DIR/sessions has JSON artifacts.
