...
async fn sync_fail_closed_policy(home: &std::path::Path, kp: &ring::signature::Ed25519KeyPair) {
    ...
    assert!(kimix_shell::managed_config::managed_policy_gate().is_ok());
}
...
/// A signature-rejected sync surfaces as failure in BOTH `kimix setup` and the
...
async fn rejected_signature_surfaces_as_setup_and_login_failure() {
    ...
    let outcome = kimix_shell::managed_config::run_setup().await;
    assert!(
        matches!(
            outcome,
            kimix_shell::managed_config::SetupOutcome::Failed(
                kimix_shell::managed_config::ManagedConfigError::SignatureRejected
            )
        ),
        "setup must surface the signature rejection, got {outcome:?}"
    );

    let login = kimix_shell::managed_config::post_login_sync(None).await;
    assert_eq!(
        login,
        kimix_shell::managed_config::ManagedConfigSync::Failed,
