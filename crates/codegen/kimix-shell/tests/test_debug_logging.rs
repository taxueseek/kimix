...
//! Runs the built Kimix binary against the mock inference server with a
//! caller-owned `$KIMIX_SHARE_DIR`, then inspects `~/.kimix/debug/`:
...
//!   `~/.kimix/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `KIMIX_LOG_SAMPLING`/`KIMIX_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `KIMIX_DEBUG_LOG=1`) writes a per-session
//!   `~/.kimix/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.kimix/debug/` files).
//! - `KIMIX_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p Kimix-shell --test test_debug_logging -- --ignored
//! ```
...
/// The per-session firehose directory under a pinned `$KIMIX_SHARE_DIR`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".kimix").join("debug")
}

/// List firehose `*.txt` files under `~/.kimix/debug` (excluding the `latest.txt`
...
/// Build a headless `kimix -p` command with a pinned `$KIMIX_SHARE_DIR` so the firehose
/// lands under `<home>/.Kimix/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    kimix_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home);
    // Pin the home location and drop inherited firehose toggles for determinism.
    cmd.env("KIMIX_SHARE_DIR", home.join(".kimix"));
    cmd.env_remove("KIMIX_DEBUG_LOG");
    cmd.env_remove("KIMIX_LOG_FILE");
    cmd.env_remove("KIMIX_LOG_SAMPLING");
    cmd.env_remove("KIMIX_HOOKS_LOG");
    cmd
}
...
/// assert it carries first-party (`xai_Kimix`) content. Panics with the captured
/// stderr tail if it never fills. Shared by the live-agent tests.
async fn read_session_firehose_when_ready(path: &Path, client: &KimixStdioClient) -> String {
...
/// headless `kimix -p` client is near-silent, so its lazily-opened firehose may
...
async fn debug_flag_enables_firehose_without_crashing() {
    ...
    assert_headless_success(&result, "kimix --debug headless", Some(&server));
...
async fn no_debug_flag_writes_no_debug_dir() {
    ...
    assert_headless_success(&result, "kimix headless (no --debug)", Some(&server));
    assert!(
...
/// A live `agent` session writes `~/.kimix/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let kimix_home = home.path().join(".kimix");
        let kimix_home_str = kimix_home.to_string_lossy().into_owned();

        let client = KimixStdioClient::spawn_with_home_and_env(
            &server,
            workdir.path(),
            home,
        ...
        let session_file = kimix_home.join("debug").join(format!("{sid}.txt"));
        ...
        // file, so `tail -f ~/.kimix/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
            let link = kimix_home.join("debug").join("latest.txt");
...
/// The `--debug` FLAG (not `KIMIX_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `KIMIX_LOG_SAMPLING`/`KIMIX_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // Drive `kimix --debug agent stdio`: the master switch (which runs before
        // the agent dispatch) must be what enables the firehose — NOT a direct
        // KIMIX_DEBUG_LOG env. The spawn helper clears inherited firehose toggles,
        // so the `--debug` flag is the only thing that can enable logging here.
        let client = KimixStdioClient::spawn_with_home_env_and_args(
            &server,
            workdir.path(),
            home,
        ...
        // clears KIMIX_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.kimix/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // KIMIX_INSTRUMENTATION=disabled, so that assertion would be vacuous.
        let sampling = kimix_home.join("logs").join("sampling.jsonl");
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.kimix/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn debug_file_flag_writes_single_file_and_bypasses_routing() {
    ...
    assert_headless_success(&result, "kimix --debug-file", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        explicit.exists(),
...
/// `KIMIX_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn kimix_log_file_explicit_path_is_written() {
    let server = MockInferenceServer::start()
        .await
        .expect("start mock server");
    let workdir = git_workdir();
    ...
    cmd.env("KIMIX_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "kimix KIMIX_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit KIMIX_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "KIMIX_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
