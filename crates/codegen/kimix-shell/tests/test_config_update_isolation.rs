...
/// All tests share this as KIMIX_SHARE_DIR (the `OnceLock` in Kimix-config
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = kimix_shell::config::load_effective_config().unwrap();
    let effective_cfg = kimix_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    kimix_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = kimix_shell::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("kimix-3"));
}
