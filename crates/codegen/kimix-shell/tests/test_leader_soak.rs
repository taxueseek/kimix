...
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p Kimix-shell --test test_leader_soak -- --ignored --nocapture
//! ```
...
/// Copied from `Kimix-codebase-graph/tests/memory_integration.rs`.
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = kimix_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            // the same wiring: Kimix-tui/src/app/leader_cluster/mod.rs
