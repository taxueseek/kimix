...
//! Each test spawns a real `kimix agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p Kimix-shell --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `model-a`: no agent_type (→ "Kimix")
/// - `model-b`: no agent_type (→ "Kimix")
...
/// `kimix` harness. The system prompt sent to the LLM should contain
/// the Kimix identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_kimix_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Kimix") || sys_prompt.contains("kimix"),
            "default model should use kimix harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        let original_has_kimix =
            original_sys_prompt.contains("Kimix") || original_sys_prompt.contains("kimix");
        let resumed_has_kimix =
            resumed_sys_prompt.contains("Kimix") || resumed_sys_prompt.contains("kimix");
        assert_eq!(
            original_has_kimix,
            resumed_has_kimix,
            "resumed session should use the same harness as the original\n\
             original identity markers: kimix={original_has_kimix}\n\
             resumed identity markers: kimix={resumed_has_kimix}\n\
...
/// default to `"Kimix"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_kimix() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Kimix") || sys_prompt.contains("kimix"),
            "model without agent_type should default to kimix harness\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
    })
    .await;
}
/// The `KIMIX_AGENT` escape hatch should override the model's agent_type.
/// Setting `KIMIX_AGENT=Kimix` with an alternate-agent model should use
/// Kimix harness.
#[tokio::test]
#[ignore]
async fn test_kimix_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
        ...
        cmd.args(["agent", "stdio"])
            ...
            .kill_on_drop(true);
        kimix_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
        cmd.env("KIMIX_AGENT", "kimix");
        let mut child = cmd.spawn().expect("spawn kimix");
        ...
        let incoming = kimix_acp_lib::LineBufferedRead::spawn_local(incoming);
        ...
        assert!(
            sys_prompt.contains("Kimix") || sys_prompt.contains("kimix"),
            "KIMIX_AGENT=kimix should override cursor model's agent_type\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
    })
    .await;
