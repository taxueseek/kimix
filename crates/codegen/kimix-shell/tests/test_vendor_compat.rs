...
//! `.Kimix`, `.cursor`, and `.claude` vendor dirs, spawns a real `kimix agent
//! stdio` process against the mock inference server (toggling the
//! `KIMIX_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!

//! - the Kimix-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p Kimix-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_KIMIX_SKILL: &str = "ZZ_KIMIX_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".kimix", "kimix-skill", MARKER_KIMIX_SKILL);
...
/// Terminal-minimal defaults (vendors off): Kimix skill present; cursor/claude
/// vendor skills absent unless explicitly enabled.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_defaults_native_only() {
    with_local_set(|| async {
        let body = run_scenario(&[]).await;
        assert!(
            body.contains(MARKER_KIMIX_SKILL),
            "kimix-skill must always be present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
            "cursor skill must be off by default (terminal-minimal)"
        );
        assert!(
            !body.contains(MARKER_CLAUDE_SKILL),
            "claude skill must be off by default (terminal-minimal)"
        );
        assert!(
...
/// `KIMIX_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; Kimix stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("KIMIX_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_KIMIX_SKILL),
            "kimix-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `KIMIX_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; Kimix stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("KIMIX_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_KIMIX_SKILL),
            "kimix-skill always present"
        );
...
/// `KIMIX_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("KIMIX_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `KIMIX_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("KIMIX_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `KIMIX_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("KIMIX_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `KIMIX_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("KIMIX_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("KIMIX_CURSOR_SKILLS_ENABLED", "false"),
            ("KIMIX_CURSOR_RULES_ENABLED", "false"),
            ("KIMIX_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_KIMIX_SKILL), "kimix always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("KIMIX_CLAUDE_SKILLS_ENABLED", "false"),
            ("KIMIX_CLAUDE_RULES_ENABLED", "false"),
            ("KIMIX_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only Kimix-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
