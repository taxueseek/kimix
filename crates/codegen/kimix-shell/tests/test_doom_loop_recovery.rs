...
use kimix_sampler::RetryPolicy;
use kimix_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
use kimix_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// `x-Kimix-turn-idx`) carries the opt-in header. Aux side-queries the binary
...
/// ```bash
/// cargo test -p Kimix-shell --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = kimix_test_support::git_workdir();
    let home = tempfile::TempDir::new().unwrap();

    let kimix_home = home.path().join(".kimix");
    std::fs::create_dir_all(&kimix_home).expect("create .kimix home");
    std::fs::write(
    ...
    let mut cmd = tokio::process::Command::new(kimix_test_support::kimix_binary());
    ...
    cmd.env_remove("KIMIX_LEADER_SOCKET");

    let result = kimix_test_support::run_headless_with_cmd(cmd).await;
    kimix_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
    ...
    let responses_posts: Vec<_> = requests
        ...
        .filter(|e| e.method == "POST" && e.path.contains("/responses"))
        .collect();
    // The session turn carries `x-Kimix-turn-idx`; aux side-queries (session
    // title, etc.) do not.
    let (turns, aux): (Vec<_>, Vec<_>) = responses_posts
        .into_iter()
        .partition(|e| e.header("x-kimix-turn-idx").is_some());
    assert!(
        !turns.is_empty(),
    ...
    for turn in turns {
        assert_eq!(
            turn.header("x-kimix-doom-loop-check"),
            Some("true"),
    ...
    for side_query in aux {
        assert_eq!(
            side_query.header("x-kimix-doom-loop-check"),
            None::<&str>,
