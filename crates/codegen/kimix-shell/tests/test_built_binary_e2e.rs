//! Built-binary end-to-end tests for the Kimix (Kimix-tui) binary.
//!

//! These tests verify that the built Kimix binary works end-to-end against a mock
...
//! The tests exercise:
//! - **Smoke** (`kimix --version`): binary loads without crashing
//! - **ACP stdio** (`kimix agent stdio`): full protocol lifecycle via ClientSideConnection
//!
...
//! ```bash
//! cargo test -p Kimix-shell --test test_built_binary_e2e -- --ignored
//! ```
//!

//! In CI, set `KIMIX_BINARY` to point at the release artifact:
//! ```bash
//! KIMIX_BINARY=./artifacts/Kimix-0.1.159-linux-x86_64 \
//!   cargo test -p Kimix-shell --test test_built_binary_e2e -- --ignored
//! ```
...
use kimix_test_support::env::test_env_cmd_tokio;
use kimix_test_support::*;
...
async fn kimix_server() -> MockInferenceServer {
    MockInferenceServer::start_with_models(vec![
        MockModelEntry::with_agent_type("kimix-4.5", "kimix")
...
async fn run_headless_with_env(
    ...
    let mut cmd = tokio::process::Command::new(kimix_binary());
    cmd.args(args)
        .current_dir(cwd)
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("KIMIX_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "kimix --version with KIMIX_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_session_in_git_repo() {
    ...
    assert_headless_success(&result, "kimix -p in git repo", Some(&server));
...
/// Verify Kimix works in a non-git directory (exercises the fallback codepath
...
async fn test_headless_session_in_non_git_dir() {
    ...
    assert_headless_success(&result, "kimix -p in non-git dir", Some(&server));
    assert_no_crashes(&result.stderr);
}
...
async fn test_headless_tools_allowlist_keeps_enabled_web_tools() {
    ...
    assert_headless_success(&result, "kimix -p --tools with web tools", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_tools_allowlist_does_not_fail_open_for_disabled_web_fetch() {
    ...
    assert_headless_success(
        &result,
        "kimix -p --tools with disabled web_fetch",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
    let names = inference_tool_names(&server);
...
async fn test_headless_terminal_only_allowlist_is_foreground_only() {
    ...
    assert_headless_success(&result, "kimix -p --tools run_terminal_cmd", Some(&server));
    assert_no_crashes(&result.stderr);
    let request = inference_request(&server);
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free Kimix usage limit"),
...
async fn test_headless_streaming_json_output() {
    ...
    assert_headless_success(
        &result,
        "kimix -p --output-format streaming-json",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_json_reports_server_cost() {
    use kimix_test_support::scripted::SseEvent;

    let server = single_model_server("kimix-4.5", "chat_completions").await;
    let chunk = |body: serde_json::Value| SseEvent::data(body.to_string());
    server.enqueue_response(
        "/v1/chat/completions",
        kimix_test_support::scripted::ScriptedResponse::sse(vec![
    ...
    assert_headless_success(&result, "kimix -p (scripted cost)", Some(&server));
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        kimix_test_support::scripted::ScriptedResponse::sse(
            kimix_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
async fn headless_json_schema_chat_completions_uses_response_format() {
    ...
    assert_headless_success(
        &result,
        "kimix -p --json-schema (chat_completions)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_responses_uses_text_format() {
    let server = single_model_server("kimix-4.5", "responses").await;
    server.set_response(r#"{"name":"Alice","age":30}"#);
    ...
    assert_headless_success(&result, "kimix -p --json-schema (responses)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_backend_uses_structured_output_tool() {
    ...
    assert_headless_success(&result, "kimix -p --json-schema (messages)", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_validates_text_when_tool_not_called() {
    ...
    assert_headless_success(
        &result,
        "kimix -p --json-schema (messages, text)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn headless_json_schema_messages_retries_on_schema_violation() {
    ...
    assert_headless_success(
        &result,
        "kimix -p --json-schema (messages, retry)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn invalid_json_schema_disables_structured_output_and_surfaces_error() {
    ...
    assert_headless_success(
        &result,
        "kimix -p --json-schema (invalid schema)",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
// ============================================================================
// ACP stdio tests (Kimix agent stdio)
//

// These test the agent as a server: spawn `kimix agent stdio`, speak the full
...
async fn test_stdio_full_session_lifecycle() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let client = KimixStdioClient::spawn(&server, workdir.path()).await;
...
/// Verify that Kimix/session/close frees the session.
...
async fn test_stdio_prompt_then_immediate_load_session() {
    with_local_set(|| async {
        let server = MockInferenceServer::start().await.expect("start mock server");
        let workdir = git_workdir();
        let mut writer = KimixStdioClient::spawn(&server, workdir.path()).await;
        ...
        let reader = KimixStdioClient::spawn_with_home(&server, workdir.path(), shared_home).await;
...
/// Isolated headless run with a custom `~/.kimix/`. Clean env (no leaked
/// host credentials). Write config files into `kimix_dir()` before `run()`.
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        std::fs::create_dir_all(home.path().join(".kimix")).unwrap();
        Self {
            home,
            workdir: git_workdir(),
            env: vec![
                ("KIMIX_CODE_BASE_URL".into(), server.url()),
                ("KIMIX_TELEMETRY_ENABLED".into(), "false".into()),
                ("KIMIX_FEEDBACK_ENABLED".into(), "false".into()),
                ("KIMIX_TRACE_UPLOAD".into(), "false".into()),
                ("KIMIX_INSTRUMENTATION".into(), "disabled".into()),
                ("KIMIX_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }

    fn kimix_dir(&self) -> std::path::PathBuf {
        self.home.path().join(".kimix")
    }
    ...
    async fn run(&self) -> HeadlessResult {
        ...
        cmd.args(["-p", "say hello", "--yolo"])
            ...
            // Windows resolves `~` via USERPROFILE, not HOME — pin the Kimix
            // home explicitly so the sandbox holds on all platforms (see
            // `test_env_cmd_tokio`).
            .env("KIMIX_SHARE_DIR", self.kimix_dir())
...
/// Enterprise BYOK: managed_config.toml overrides Kimix with a custom
...
async fn test_headless_managed_config_byok_sends_authorized_requests() {
    let server = MockInferenceServer::start_with_required_auth(
        vec![MockModelEntry::new("kimix-4.5")],
    ...
    std::fs::write(
        h.kimix_dir().join("managed_config.toml"),
        format!(
            r#"
[endpoints]
...
"#,
    ...
    .unwrap();
    h.env("KIMIX_TEST_BYOK_TOKEN", "test-byok-secret-token");
...
async fn headless_reasoning_efforts_payload_parses_and_legacy_effort_rides_wire() {
    ...
    assert_headless_success(&result, "kimix -p reasoning_efforts list", Some(&server));
    assert_no_crashes(&result.stderr);
...
async fn test_headless_timeout_exit_kills_pending_background_task() {
    ...
    assert_headless_success(
        &result,
        "kimix -p with pending background task",
        Some(&server),
    );
    assert_no_crashes(&result.stderr);
...
async fn test_headless_no_wait_exit_kills_background_task() {
    ...
    assert_headless_success(&result, "kimix -p --no-wait-for-background", Some(&server));
    assert_no_crashes(&result.stderr);
