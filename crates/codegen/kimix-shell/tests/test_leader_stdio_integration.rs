...
use kimix_shell::cpu_profile::ControlErrorCode;
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, kimix_shell::leader::ServerHandle) {
...
async fn test_session_new_valid_default_model_injected() {
    ...
    let meta = &json["params"]["_meta"];
    assert_eq!(meta["modelId"], "kimix-3-fast");
...
/// `Kimix/session_notification` (carrying the `ModelChanged` update) out
...
async fn test_set_model_broadcasts_to_session_subscribers() {
    ...
    invoker
        .send(format!(
            r#"{{"jsonrpc":"2.0","id":42,"method":"session/setModel","params":{{"sessionId":"{}","modelId":"kimix-4"}}}}"#,
            shared_sid
        ))
        .unwrap();
    let received = acp_rx.recv().await.unwrap();
    ...
    assert_eq!(json["params"]["modelId"], "kimix-4");
    ...
    let broadcast = format!(
        r#"{{"jsonrpc":"2.0","method":"kimix/session_notification","params":{{"sessionId":"{}","update":{{"sessionUpdate":"model_changed","model_id":"kimix-4","reasoning_effort":"high"}}}}}}"#,
        shared_sid
    );
    response_tx.send(broadcast.clone()).unwrap();
    let response = format!(
        r#"{{"jsonrpc":"2.0","result":{{"meta":{{"model":"kimix-4"}}}},"id":"{}"}}"#,
        setmodel_ns_id
    );
    response_tx.send(response).unwrap();
    ...
    assert_eq!(inv2["result"]["meta"]["model"], "kimix-4");
...
async fn test_extension_method_roundtrip() {
    ...
    let ext_call = r#"{"jsonrpc":"2.0","id":50,"method":"_kimix/search/fuzzy/open","params":{"sessionId":"sess-123","hidden":false}}"#;
    ...
    assert_eq!(json["method"], "_kimix/search/fuzzy/open");
...
async fn test_session_ownership_cleanup_on_disconnect() {
    use kimix_shell::leader::run_leader_server;
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(kimix_shell::leader::ShutdownReason::Manual);
        ...
        let _ = run_leader_server(
            ...
            client_count,
            agent_busy,
            kimix_shell::agent::activity::AgentActivity::default(),
            ready_rx,
            shutdown_tx,
            None,
            control_state,
    ...
    assert_eq!(eviction_json["method"], "kimix/internal/evict_sessions");
    ...
    let probe = r#"{"jsonrpc":"2.0","method":"kimix/probe","params":{"ping":true}}"#;
...
/// Verify that an `Kimix/code/status` extension request is forwarded to the
...
async fn test_code_status_ext_request_forwarded_to_agent() {
    ...
    // Send Kimix/code/status with a sessionId — the leader must forward it to the agent.
    let status_req = r#"{"jsonrpc":"2.0","id":42,"method":"extensions/ext","params":{"method":"kimix/code/status","params":{"sessionId":"sess-web-1","cwd":"/repo"}}}"#;
    ...
    assert_eq!(json["params"]["method"], "kimix/code/status");
...
async fn test_raw_registration_handshake_not_ready_then_ready() {
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            kimix_shell::agent::activity::AgentActivity::default(),
            ready_rx,
            watch::channel(kimix_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
/// Integration test: a connected client receives `Kimix/leader/version_mismatch`
...
async fn test_version_mismatch_notification_sent_to_client() {
    use kimix_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
    use std::sync::Arc;
    use std::sync::atomic::{AtomicBool, AtomicUsize};
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use kimix_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != kimix_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        kimix_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
async fn test_leader_code_nav_isolation_end_to_end() {
    ...
    // Web client sends Kimix/code/status (the primary non-starting code-nav call).
    let status_with_session = r#"{"jsonrpc":"2.0","id":10,"method":"extensions/ext","params":{"method":"kimix/code/status","params":{"sessionId":"web-session","cwd":"/repo"}}}"#;
    ...
    assert_eq!(status_json["params"]["method"], "kimix/code/status");
...
async fn setup_persistent_test_server(
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            tokio::sync::watch::channel(kimix_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
///
/// This binary does not sandbox KIMIX_SHARE_DIR, so on a dev machine these entries
/// land in the real `~/.kimix` log — accepted: the server already writes
...
fn orphan_log_count(request_id: &str) -> usize {
    let Some(bytes) = kimix_log::unified_log::snapshot_log() else {
        return 0;
    };
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = kimix_log::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        kimix_shell::leader::DisconnectReason::Connected,
...
async fn test_sever_mid_rpc_orphans_response_and_replay_recovers() {
    ...
    assert_eq!(evict_json["method"], "kimix/internal/evict_sessions");
