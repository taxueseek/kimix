...
use kimix_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use kimix_shell::session::storage::JsonlStorageAdapter;
use kimix_test_support::sse::responses_api_reasoning_and_text_events;
use kimix_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};
...
async fn test_chat_completions_streaming_tool_calls() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        ScriptedResponse::sse(chat_completion_tool_call_stream(tool_calls, "kimix-test")),
    );
    let client = create_test_client(&server.url(), ApiBackend::ChatCompletions);
...
/// Upgrade path, Kimix / Responses API: a legacy session whose
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    // 1. Seed a legacy Kimix chat_history.jsonl (inline reasoning
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy kimix reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_kimixbuild_legacy"},"model_id":"kimix"}"#,
            "\n",
        ),
    )
    .unwrap();
...
async fn messages_upgrade_emits_reconstructed_reasoning_as_thinking_block() {
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy anthropic thinking","encrypted":"SIGNATURE_abc","id":""},"model_id":"kimix-4.5"}"#,
            "\n",
        ),
    )
    .unwrap();
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use kimix_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use kimix_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use kimix_shell::sampling::rs::OutputMessageContent;
...
async fn test_request_includes_headers() {
    ...
    assert_eq!(request.header("x-kimix-conv-id"), None);
    assert_eq!(request.header("x-kimix-req-id"), None);
}
...
/// `SamplerConfig::doom_loop_recovery` puts `x-Kimix-doom-loop-check` on the
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use kimix_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
    ...
    assert_eq!(logged.header("x-kimix-doom-loop-check"), Some("true"));
}
...
async fn test_doom_loop_check_disabled_sends_no_header_and_drops_check_frames() {
    ...
    let logged = server.requests().pop().unwrap();
    assert!(logged.path.contains("/responses"));
    assert_eq!(logged.header("x-kimix-doom-loop-check"), None);
}
...
async fn test_responses_backend_hits_responses_endpoint_not_chat_completions() {
    ...
    // The session-level dispatch in `Kimix-sampler` selects the
