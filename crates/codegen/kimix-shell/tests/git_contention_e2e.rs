...
//!
//! With the fs-watch machinery on (`Kimix/hunkTracker` + `Kimix/gitHeadChanged`
...
//!
//! Knobs: KIMIX_PERF_GIT_FILES (default 300), KIMIX_PERF_GIT_PICKS (default 6).
...
use kimix_test_utils::env::env_usize;

/// Run a git command with deterministic author/committer; assert success.
fn git(dir: &Path, args: &[&str]) -> String {
    kimix_test_utils::git::run_git(dir, args)
}

// ── scan counting ─────────────────────────────────────────────────────────

use kimix_hunk_tracker::{REFRESH_SCAN_LOG_PREFIX, REFRESH_SKIP_LOG_PREFIX};
use kimix_test_utils::tracing_capture::MessagePrefixCounter;
...
fn install_global_scan_counter() -> ScanCounter {
    // KIMIX_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("KIMIX_E2E_LOG").ok();
    ScanCounter(
        kimix_test_utils::tracing_capture::install_prefix_counter_global(
...
fn build_repo(files: usize, picks: usize) -> (TempDir, String) {
    ...
    kimix_test_utils::git::write_fanout_tree(wd, files, 100);
    git(wd, &["add", "."]);
    git(wd, &["commit", "-m", "populate tree"]);

    let base = kimix_test_utils::git::make_feature_branch(wd, picks);
    (dir, base)
}
...
async fn run_storm(
    ...
    let files = env_usize("KIMIX_PERF_GIT_FILES", 300);
    let picks = env_usize("KIMIX_PERF_GIT_PICKS", 6);
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let server = mock_rt
        ...
        .expect("mock server");
    let kimix_home = TempDir::new().expect("kimix home");
    ...
    let on = agent_rt.block_on(run_storm(
        &server,
        Some(json!({
            "kimix/hunkTracker": { "mode": "agent_only" },
