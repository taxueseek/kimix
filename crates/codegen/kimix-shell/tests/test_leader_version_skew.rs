...
//! - `KIMIX_BINARY_LEADER` — the binary that elects the initial leader
//!   (typically the latest released stable, e.g. fetched from
//!   `https://storage.googleapis.com/Kimix-public-artifacts/cli/Kimix-<ver>-linux-x86_64`).
//! - `KIMIX_BINARY_CLIENT` — the second client (typically a freshly built main).
//!
...
//! ```bash
//! KIMIX_BINARY_LEADER=/path/to/Kimix-old KIMIX_BINARY_CLIENT=/path/to/Kimix-new \
//!   cargo test -p Kimix-shell --test test_leader_version_skew -- --ignored --nocapture
//! ```
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: KIMIX_BINARY_LEADER/KIMIX_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".kimix").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set KIMIX_BINARY_LEADER/KIMIX_BINARY_CLIENT and run with --ignored"]
...
async fn old_client_adopts_new_leader_and_still_functions() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            // Kimix/leader/version_mismatch notification's server-side warn).
...
/// `kimix update`'s relaunch signal against a REAL old leader: connect,
/// require `relaunch_v1`, send `RelaunchForUpdate`, and the leader exits so
/// the surviving client re-elects. Mirrors the private
/// `signal_leaders_to_relaunch` in `Kimix-bin/src/main.rs` (which is
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".kimix").join("leader.sock"),
                "kimix-pager-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".kimix").join("auth.json");
