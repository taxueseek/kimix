...
use kimix_acp_lib::{AcpAgentGatewaySender, AcpClientMessage};
use kimix_paths::AbsPathBuf;
use kimix_workspace::permission::types::{
...
/// Shared `KIMIX_SHARE_DIR` for the entire test binary. The `OnceLock` in
/// `Kimix-config` only allows one value per process, so all tests share
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("KIMIX_SHARE_DIR", &path) };
        path
    })
}
...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::KimixPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / KimixWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use kimix_workspace::permission::types::{PermissionConfig, PromptPolicy};
