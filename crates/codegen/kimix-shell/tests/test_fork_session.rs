...
use kimix_shell::sampling::ConversationItem;
...
async fn create_test_session(storage: &JsonlStorageAdapter, session_id: &str, cwd: &str) -> Info {
    ...
    let model_id = acp::ModelId::new("kimix-code-fast-1");
    ...
    storage
        .append_update(
            &info,
            &kimix_shell::session::storage::SessionUpdate::Acp(Box::new(notification)),
        )
        .await
        .unwrap();
...
async fn test_fork_session_creates_new_session_with_parent_tracking() {
    ...
    let options = kimix_shell::session::storage::CopySessionOptions {
    ...
    match &loaded.updates[0] {
        kimix_shell::session::storage::SessionUpdate::Acp(notification) => {
