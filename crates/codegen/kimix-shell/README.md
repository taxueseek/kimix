...
- **Using Kimix**
  ...
  - [SSH Passthrough](#ssh-passthrough-kimix-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`kimix inspect`)](#introspection)
...
- [Building with Kimix](#building-with-kimix) — headless API, ACP SDK integration
...
On first launch, Kimix opens your browser to authenticate with kimix.com:

```bash
kimix
```

Credentials are stored in `~/.kimix/auth.json` and persist across sessions. Tokens expire after 7 days; Kimix will prompt you to re-authenticate when needed.
...
export KIMIX_OIDC_ISSUER="https://acme.okta.com"
export KIMIX_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export KIMIX_CLI_CHAT_PROXY_BASE_URL="https://kimix-proxy.acme.com/v1"
```

**3. Run `kimix`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.kimix/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Kimix is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     Kimix     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. Kimix runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Kimix and saved to `~/.kimix/auth.json` as the access token
5. exit 0 → success; exit non-zero → Kimix falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | Kimix (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Kimix reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Kimix to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Kimix when to proactively refresh. Without it, Kimix assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Kimix captures this)
...
```bash
# Or via environment variables
export KIMIX_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export KIMIX_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export KIMIX_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Kimix cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with kimix.com". In headless mode (`kimix -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=kimix-cli")
...
When Kimix needs to refresh an expired token, it re-runs your binary with `KIMIX_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$KIMIX_AUTH_EXPIRED" = "1" ]; then
...
`KIMIX_AUTH_EXPIRED` is optional — if your binary ignores it, Kimix still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

Kimix supports automatic credential refresh for external auth providers and OIDC. When Kimix detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Kimix handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Kimix re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Kimix re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Kimix silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Kimix refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export KIMIX_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `kimix login` before starting — Kimix runs your binary automatically on first launch. You _can_ run `kimix login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Kimix tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
kimix --debug-file /tmp/kimix-auth.log -p "hello"
tail -f /tmp/kimix-auth.log
```
...
If you've authenticated with `kimix login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the kimix CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.kimix.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.kimix/auth.json)" \
  -H "X-XAI-Token-Auth: xai-kimix-cli" \
  -H "x-kimix-model-override: kimix" \
  -d '{
    "model": "kimix",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.kimix/auth.json` (set by `kimix login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-kimix-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-kimix-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `kimix` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `kimix`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `kimix login` to refresh.
...
kimix --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
kimix --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
kimix --always-approve --prompt "Format all files"
```
...
Run Kimix non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `kimix`)               |
...
kimix -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
kimix -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
kimix -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
kimix -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
kimix -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
kimix -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
kimix -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
kimix -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
kimix -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
kimix --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
kimix -p "What does this project do?"

# Use a specific model
kimix -p "Optimize this function" -m kimix

# Get JSON output for parsing
kimix -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
kimix -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
kimix -p "Remember: the secret number is 42" --output-format json
kimix -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
kimix -p "Continue where we left off" -c

# Run in a different directory
kimix -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
kimix -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
kimix -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
kimix -p "Generate a README" > README.md

# Parse JSON output with jq
kimix -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
kimix -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | kimix -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  kimix -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
kimix -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Kimix as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `kimix`)                           |
...
```bash
kimix agent headless --kimix-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`kimix ssh`)

Use `kimix ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
kimix ssh -t user@host
kimix ssh -L 8080:localhost:8080 user@host

# With remote command
kimix ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `kimix ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Kimix can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `kimix -p` and parses JSON output.
...
class KimixChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["kimix", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="kimix", stream=False):
...
class KimixChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("kimix", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await KimixACPChat(cwd=".").init()
...
class KimixACPChat {
  ...
  async init() {
    this.proc = spawn("kimix", ["agent", "stdio"]);
...
// Usage
const client = await new KimixACPChat(".").init();
...
Kimix implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Kimix reads configuration from `~/.kimix/config.toml`. If the file doesn't exist, Kimix uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "kimix"           # model used for new sessions
...
telemetry = false                      # anonymous usage telemetry (env: KIMIX_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: KIMIX_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: KIMIX_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: KIMIX_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: KIMIX_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: KIMIX_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: KIMIX_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: KIMIX_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `KIMIX_TELEMETRY_BUILD_EVENTS_URL`, `KIMIX_TELEMETRY_BUILD_EVENTS_API_KEY`, and `KIMIX_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Kimix can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Kimix language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Kimix can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Kimix can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Kimix looks for server definitions in:

- project config: `<repo>/.kimix/lsp.json`
- user config: `~/.kimix/lsp.json`
...
- LSP tools are enabled (`KIMIX_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
KIMIX_LSP_TOOLS=1 kimix
```
...
If LSP tools are enabled but no usable server config is found, Kimix emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Kimix does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `KIMIX_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-kimix"

[model.company-kimix]
model = "kimix"
base_url = "https://kimix-proxy.acme.com/"
name = "Kimix Latest (Proxy)"
...
With this config, `kimix` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Kimix reads these files and appends their contents to the system prompt.

Kimix scans for agent rules in this order:

1. `~/.kimix/` (global rules)
...
Within each directory, Kimix checks for these filenames:
...
Skills are reusable prompt packages that extend Kimix with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.
...
| --------------------------- | ----- | -------- |
| `./.kimix/skills/`           | Local | Highest  |
| `<repo_root>/.kimix/skills/` | Repo  | Medium   |
| `~/.kimix/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.kimix/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Kimix decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Kimix invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Kimix automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Kimix discovers agent definitions from `.kimix/agents/` (project), `~/.kimix/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `KIMIX_AGENT` env var
4. Default `kimix` agent
...
```bash
kimix --agent-profile ./my-agent.md
# or
export KIMIX_AGENT="my-custom-agent"
```
...
```bash
export KIMIX_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "kimix"              # route explore to a lighter model
```
...
# instructions_file = ".kimix/personas/concise.md"  # or load from file
```

Both are also discovered from `.kimix/roles/*.toml` and `.kimix/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Kimix with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.kimix/plugins/`            | Project |
| `~/.kimix/plugins/`          | User    |
...
Kimix discovers hooks from `.kimix/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `XAI_API_KEY`. If neither `api_key` nor `env_key` is set, Kimix falls back to the global `XAI_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Kimix falls back to built-in defaults for known models.
...
[model.kimix-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Kimix starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
Point Kimix at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `KIMIX_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `KIMIX_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export KIMIX_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
kimix
```

Kimix fetches the model list from `{KIMIX_MODELS_BASE_URL}/models` on startup and sends inference requests to `KIMIX_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `KIMIX_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.kimix/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Kimix uses API key auth (`Authorization: Bearer`) instead of session auth. `kimix login` is not required — only the API key.

---

## MCP Servers

Extend Kimix's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.kimix/config.toml`:
...
MCP servers can also be configured per-project in `.kimix/config.toml`. Kimix walks from the current directory up to the git repo root, loading `.kimix/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.kimix/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.kimix/config.toml` | This repository   | ↑        |
| `<cwd>/.kimix/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.kimix/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.kimix/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.kimix/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.kimix/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `KIMIX_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Kimix remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.kimix/memory/`:
- **Global** (`~/.kimix/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.kimix/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.kimix/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
At the end of each session, Kimix saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.kimix/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Kimix searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.kimix/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.kimix/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Kimix emits the `kimix-shell-memory_injection`
...
Kimix can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.kimix/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.kimix/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.kimix/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kimix/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.kimix/sandbox.toml` (global) or `.kimix/sandbox.toml` (per-project):
...
```bash
kimix --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire kimix process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Kimix logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.kimix/sandbox-events.jsonl`
...
Use `kimix inspect` to see everything Kimix discovers in the current directory:

```bash
kimix inspect          # human-readable output
kimix inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.kimix/skills/`, `~/.kimix/skills/`, plugins, and config paths
...
Kimix automatically discovers configuration from Claude Code directories alongside native `.kimix/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Kimix uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.kimix/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Kimix at runtime.

---

## Built-in Tools

Kimix includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `KIMIX_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (Kimix, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `x.ai/company`).

---

## Session Persistence

Kimix automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.kimix/sessions/`, organized by URL-encoded working directory:

```
~/.kimix/sessions/<encoded-cwd>/<session-id>/
...
kimix -p "Remember: X=42" -s my-session
kimix -p "What is X?" -s my-session

# Resume existing session (errors if not found)
kimix -p "Continue" -r my-session

# Continue most recent session in current directory
kimix -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
kimix -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.kimix/config.toml` | Configuration file                                  |
| `~/.kimix/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.kimix/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.kimix/memory/`     | Cross-session memory files and index                |
| `~/.kimix/skills/`     | User-scoped skill definitions                       |
| `~/.kimix/plugins/`    | User-scoped plugins                                 |
| `~/.kimix/agents/`     | User-scoped agent definitions                       |
| `.kimix/config.toml`   | Project-scoped config (MCP servers)                 |
| `.kimix/skills/`       | Project-scoped skill definitions                    |
| `.kimix/plugins/`      | Project-scoped plugins                              |
| `.kimix/agents/`       | Project-scoped agent definitions                    |
| `.kimix/hooks/`        | Project-scoped hooks                                |
| `.kimix/lsp.json`      | LSP server configuration                            |
...
| `KIMIX_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.kimix.com/v1`)                          |
| `KIMIX_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `KIMIX_MODELS_LIST_URL`          | Override the model list URL if it differs from `{KIMIX_MODELS_BASE_URL}/models`                                              |
| `KIMIX_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `KIMIX_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `KIMIX_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `KIMIX_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `KIMIX_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `KIMIX_SHARE_DIR`                     | Override config directory (default: `~/.kimix`)                                                           |
| `KIMIX_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `KIMIX_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `KIMIX_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `KIMIX_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `KIMIX_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `KIMIX_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `KIMIX_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `KIMIX_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `KIMIX_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `KIMIX_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.kimix/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `KIMIX_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `kimix` commands and flags.
...
kimix completions bash > ~/.local/share/bash-completion/completions/kimix
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Kimix-managed location):

```bash
mkdir -p ~/.kimix/completions/bash
kimix completions bash > ~/.kimix/completions/bash/kimix.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.kimix/completions/bash/kimix.bash" ]] && source "$HOME/.kimix/completions/bash/kimix.bash"
```
...
kimix completions zsh > ~/.zsh/completions/_kimix
```
...
```bash
mkdir -p ~/.kimix/completions/zsh
kimix completions zsh > ~/.kimix/completions/zsh/_kimix
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.kimix/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `kimix` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `kimix --debug` or `KIMIX_LOG_FILE` instead:

```bash
# Per-session debug log (~/.kimix/debug/<sessionId>.txt)
kimix --debug

# Log to a custom path
KIMIX_LOG_FILE=/tmp/kimix-debug.log kimix

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.kimix/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `KIMIX_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
KIMIX_LOG_FILE=/tmp/kimix-debug.log RUST_LOG="info,kimix_shell::auth=debug" kimix
```
...
cat ~/.kimix/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.kimix/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.kimix/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
