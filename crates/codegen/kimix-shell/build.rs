//! Build script for bundling ripgrep for the Kimix-shell crate.
//!

//! - If `KIMIX_SHELL_BUNDLE_RG_PATH` is set, always bundle it
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=KIMIX_SHELL_BUNDLE_RG_PATH");
    println!("cargo:rerun-if-env-changed=KIMIX_SHELL_RG_DOWNLOAD_BASE");
    ...
    let path_override = env::var("KIMIX_SHELL_BUNDLE_RG_PATH").ok();
    ...
    let gen_dir = if in_bazel {
        ...
        PathBuf::from(xai_root).join("target/tmp/kimix-shell-bundle-rg")
    } else {
        PathBuf::from(env::var("OUT_DIR")?)
    };
    ...
    // An explicit KIMIX_SHELL_BUNDLE_RG_PATH still bundles on Windows (the
    ...
    println!("cargo:rustc-env=KIMIX_SHELL_RG_VER={}", RG_VER);
    println!(
        "cargo:rustc-env=KIMIX_SHELL_RG_GEN_DIR={}",
        gen_dir.display()
    );
    ...
    if let Some(path) = path_override {
        ...
        println!("cargo:rustc-env=KIMIX_SHELL_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying KIMIX_SHELL_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set KIMIX_SHELL_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!("cargo:rustc-env=KIMIX_SHELL_RG_TARGET={}", asset_triple);
    ...
    // internal mirror (e.g. KIMIX_SHELL_RG_DOWNLOAD_BASE=http://<mirror>/github/
    // BurntSushi/ripgrep/releases/download). Defaults to the public GitHub
    // releases URL.
    let download_base = env::var("KIMIX_SHELL_RG_DOWNLOAD_BASE")
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet KIMIX_SHELL_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set KIMIX_SHELL_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set KIMIX_SHELL_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
