...
Memory lets Kimix recall facts, decisions, and patterns from earlier sessions. Kimix indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each Kimix session starts fresh: the model knows nothing about previous sessions. When you enable memory, Kimix can:
...
3. `KIMIX_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.kimix/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.kimix/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.kimix/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

Kimix suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, Kimix saves a structured metadata summary to that session's daily log. The summary contains:
...
Kimix builds the summary from conversation metadata without an LLM call, without added latency. Kimix skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask Kimix to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.kimix/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

Kimix records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, Kimix opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, Kimix shows `Memory saved to ~/.kimix/memory/MEMORY.md`.

### Forget

Ask Kimix to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.kimix/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what Kimix remembers:

```
> what do you remember?
```

Kimix searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.kimix/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, Kimix confirms in the scrollback:

```
Memory saved to ~/.kimix/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what Kimix has stored.
...
Dream also runs automatically. By default, Kimix checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, Kimix automatically searches memory for content relevant to the current project and injects it as context. This means Kimix starts with knowledge from previous sessions without a reminder.
...
Kimix searches memory automatically, but you can also trigger searches manually in the chat:
...
The `kimix memory` command manages memory from the shell. It has one subcommand, `clear`:
...
kimix memory clear --workspace

# Clear the global MEMORY.md
kimix memory clear --global

# Clear both workspace and global memory
kimix memory clear --all

# Skip the confirmation prompt (-y is the short form)
kimix memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.kimix/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.kimix/memory/` for external edits and reindex |
...
| `min_score` | unset | Score threshold for first-turn results. When unset, Kimix applies no threshold, which is equivalent to `0.0`. |
...
| `flush_model` | unset | Model for the flush turn. When unset, Kimix uses the session's primary model. |
...
1. Verify memory is enabled: check `kimix inspect` output.
2. Check the flag: `kimix --experimental-memory` or `KIMIX_MEMORY=1`.
3. Check for `--no-memory` or `KIMIX_MEMORY=0` overriding your config.
...
```bash
ls ~/.kimix/memory/
cat ~/.kimix/memory/MEMORY.md
$EDITOR ~/.kimix/memory/MEMORY.md
```
