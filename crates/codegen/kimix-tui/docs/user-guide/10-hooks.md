...
Hooks let you run a script or send an HTTP request at key moments in a Kimix session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Kimix calls when a specific lifecycle event occurs. Hooks can:
...
2. Create a hook file, e.g. `~/.kimix/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Kimix session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Kimix session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.kimix/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.kimix/hooks/*.json` | Requires trust | Per-repo automation |
...
The Claude and Cursor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.kimix/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run -- until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.kimix/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.kimix/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and cascades to subdirectories. Conversely, disabling folder-trust (`KIMIX_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Kimix accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Kimix skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Kimix maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
Kimix sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `KIMIX_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `KIMIX_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `KIMIX_SESSION_ID`     | The unique identifier of the current Kimix session. |
| `KIMIX_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `KIMIX_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Kimix additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `KIMIX_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `KIMIX_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`KIMIX_PLUGIN_ROOT`, `KIMIX_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
```json
{ "type": "http", "url": "https://hooks.example.com/kimix-event", "timeout": 15 }
```
...
Press `r` in the Hooks tab to reload all hooks from disk. Kimix re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.kimix/hooks/`) run with your user permissions -- treat them like shell scripts.
...
5. **Version control project hooks** -- commit `.kimix/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug KIMIX_LOG_FILE=/tmp/kimix.log kimix`, then check `/tmp/kimix.log`.
