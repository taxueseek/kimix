...
> **Status: alpha.** The schema below is versioned (`kimix_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version.

Kimix CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through Kimix.
...
- **Structurally separate** from Kimix-internal telemetry: its exporters carry
  only the headers you configure, never Kimix credentials.
- **Independent of Kimix data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams — those
  settings govern Kimix-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export KIMIX_EXTERNAL_OTEL=1                  # master switch
...
`KIMIX_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `KIMIX_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `KIMIX_TELEMETRY_ENABLED`, which controls Kimix-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `kimix_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `KIMIX_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`KIMIX_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `kimix setup` managed config /
...
## Metrics (meter scope `ai.kimix.code`)

| Metric | Unit | Attributes |
|---|---|---|
| `kimix_code.session.count` | `{session}` | base attrs only |
| `kimix_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `kimix_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `kimix_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `kimix_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `kimix_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `kimix_code.token.usage` with your own
...
|---|---|
| `kimix_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `kimix_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `kimix_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `kimix_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `kimix_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `kimix_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `kimix_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `kimix_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `kimix_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `kimix_code.permission_mode_changed` | `to_mode`, `trigger` |
| `kimix_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `kimix_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `kimix_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `kimix_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `kimix_code.auth` | `auth_method` |
| `kimix_code.internal_error` | `error_type` (class only — no message, no location) |
| `kimix_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(kimix_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(kimix_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(kimix_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(kimix_code_tool_decision_total[1h]))
```
