...
Agent mode runs Kimix as an ACP (Agent Client Protocol) server for integration with IDEs, editors, and custom tooling. Unlike single-prompt mode (`kimix -p`, which prints one response and exits), agent mode keeps a persistent process running and communicates through structured JSON-RPC messages.
...
These options belong to the `kimix agent` command and apply to every mode. Pass them before the mode name, for example `kimix agent --model kimix stdio`. The `stdio` subcommand itself takes no options.

| Flag                       | Description                                                       |
| -------------------------- | ---------------------------------------------------------------- |
| `-m, --model <MODEL>`      | Set the model ID (for example, `kimix`).                    |
...
```bash
kimix agent serve --bind 127.0.0.1:2419 --secret <token>
```

Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent generates a token and prints it at startup; you can also supply one through the `KIMIX_AGENT_SECRET` environment variable. The agent persists across reconnections, so a client can disconnect and later resume in-flight work.
...
Beyond the base ACP protocol, Kimix defines extension methods under the `x.ai/` prefix for Kimix-specific functionality. These cover:
...
The tables here show representative methods in each category. The `x.ai/*` set is Kimix-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
