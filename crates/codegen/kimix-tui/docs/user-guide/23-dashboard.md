...
- **`kimix dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.kimix/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Kimix · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Kimix's sibling views (
`tasks_pane`):
...
Set `KIMIX_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `kimix --bg`") would list
