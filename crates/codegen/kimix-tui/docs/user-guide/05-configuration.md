...
Kimix reads configuration from local config files, environment variables, and
...
2. **Environment variables** (e.g., `XAI_API_KEY`, `KIMIX_MEMORY`)
3. **config.toml** (`~/.kimix/config.toml`)
...
Location: `~/.kimix/config.toml`

If the file does not exist, Kimix uses built-in defaults. Specify only the values you want to override.
...
**Disable vim input mode**); Kimix writes your choice to `[ui] simple_mode` in
`config.toml`.
...
The setting can also be overridden with the `KIMIX_DEFAULT_SELECTED_PERMISSION`
...
(`/settings` → **Vim scrollback navigation**). Kimix writes the change to
`[ui] vim_mode` in `~/.kimix/config.toml` immediately and applies it to every
...
`kimix` launches. Configure it from `/settings` → **Default screen mode**
...
`KIMIX_SCROLL_SPEED`, `KIMIX_SCROLL_MODE`, `KIMIX_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `KIMIX_SCROLL_LINES`. Precedence: env var →
...
(`KIMIX_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`KIMIX_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
MCP servers can also be configured per-project in `.kimix/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.kimix/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.kimix/config.toml` (current dir) > `<repo-root>/.kimix/config.toml` > `~/.kimix/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `KIMIX_MEMORY=1`).
...
[subagents.models]
explore = "kimix"               # route to different models
```
...
`kimix inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. Kimix writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.kimix/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "kimix"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `kimix`. |
...
| Kimix Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

When `method = "auto"`, Kimix detects the terminal brand and selects the best
...
`$KIMIX_EVENT`, `$KIMIX_MESSAGE`, and `$KIMIX_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Kimix' -message '$KIMIX_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$KIMIX_MESSAGE' ntfy.sh/my-kimix-alerts"
...
fires, try `condition = "always"` as a fallback. Kimix supports focus tracking
...
Set these only to point telemetry at your own infrastructure or to turn parts of it off. The built-in endpoint and credentials are managed by Kimix; leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= KIMIX_EXTERNAL_OTEL)
...
| `KIMIX_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `KIMIX_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `KIMIX_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `KIMIX_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `KIMIX_OIDC_ISSUER` | OIDC issuer URL |
| `KIMIX_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `KIMIX_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `KIMIX_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `KIMIX_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `KIMIX_AGENT` | Custom agent definition path or name |
| `KIMIX_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `KIMIX_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `KIMIX_LOG_FILE` log and headless stderr output |
...
| `KIMIX_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `KIMIX_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `KIMIX_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `KIMIX_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.kimix/config.toml` | Main configuration file |
| `~/.kimix/pager.toml` | TUI appearance configuration |
...
| `~/.kimix/lsp.json` | LSP server configuration (user-scoped) |
| `~/.kimix/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.kimix/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.kimix/` within your repository:

| File | What it configures |
|------|--------------------|
| `.kimix/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.kimix/config.toml`) |
| `.kimix/skills/` | Project-specific skills |
| `.kimix/hooks/` | Project-specific lifecycle hooks |
| `.kimix/agents/` | Project-specific agent definitions |
| `.kimix/lsp.json` | LSP server configuration |
| `.kimix/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.kimix/lsp.json` | All projects |
| Project | `.kimix/lsp.json` | Current repository |
...
1. **Project** -- `.kimix/lsp.json`
2. **User** -- `~/.kimix/lsp.json`
