...
- **Shell builtins** -- handled by the agent backend (kimix-shell)
- **Pager builtins** -- handled by the TUI frontend (kimix-tui)
...
When the context window fills up, Kimix auto-compacts at 85% usage (configurable via `[session] auto_compact_threshold_percent` in config.toml).
...
`--fullscreen` CLI flags are likewise session-scoped. To make plain `kimix` open
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `KIMIX_MEMORY=1`. `/remember` is always available.
...
Run a prompt on a recurring interval. Specify the interval as `30m`, `1 hour`, or `every 2 days`. If you omit it, Kimix prompts you.
...
Set, manage, or check an autonomous goal. Kimix works toward the objective across turns and reports progress.
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.kimix/skills/commit/SKILL.md`, you can invoke it with:
