...
Headless mode runs Kimix non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
Kimix processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, Kimix emits an error object (process exit non-zero). Prompt-level
...
Kimix may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `kimix -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
kimix -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
kimix -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
kimix -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
kimix -p "What's the secret number?" --resume abc123
```
...
kimix -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
kimix --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  kimix -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
Kimix's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(kimix -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
kimix -p "Format all files" --yolo

# Run tests and fix failures
kimix -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| the model `env_key` variable        | API key for authentication (required when no browser login)   |
| `KIMIX_SHARE_DIR`                    | Override config directory (default: `~/.kimix`)                |
| `KIMIX_LOG_FILE`                | Path to a log file (used verbatim as the path; works in headless and TUI, honors `RUST_LOG`) |
| `RUST_LOG`                     | Log level filter (e.g. `debug`). Headless logs to stderr.     |

For CI environments without browser access, set the model `env_key` variable with an API key from [console.x.ai](https://console.x.ai):

```bash
export ENV_KEY="xai-..."
kimix -p "Run the test suite" --yolo
```
...
- **the model `env_key` variable** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`kimix login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`kimix login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `kimix -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug kimix -p "..." 2> debug.log`.

---

## Project Root Discovery

When Kimix starts, it discovers the project root by walking upward from `--cwd`
...
Kimix discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Kimix stores data in `~/.kimix` (override with `KIMIX_SHARE_DIR`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.kimix` read-only:

- Pre-populate `auth.json` or use the model `env_key` variable
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export ENV_KEY="xai-..."
export KIMIX_DISABLE_AUTOUPDATER=1
kimix -p "..." --no-auto-update
```
...
| `KIMIX_DISABLE_AUTOUPDATER=1`    | Process   |
...
- Resume: `kimix -p "continue" --resume "<id>"` or `kimix -p "continue" --continue`
