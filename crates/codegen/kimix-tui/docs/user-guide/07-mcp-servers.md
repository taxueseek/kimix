...
MCP (Model Context Protocol) servers extend Kimix with external tool integrations. They let Kimix interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Kimix over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Kimix's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.kimix/config.toml` under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

Kimix spawns a local process and communicates over stdin/stdout:
...
> `KIMIX_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `KIMIX_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Kimix-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.kimix/config.toml`) **or repo-level**
>   (`.kimix/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.kimix/config.toml` >
...
kimix mcp list
kimix mcp list --json          # Machine-readable output

# Add a stdio server. Everything after -- is the server command, so flags
# like -y reach the server instead of being parsed by kimix.
kimix mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
kimix mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
kimix mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
kimix mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
kimix mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
kimix mcp remove github

# Diagnose a server's configuration and connectivity
kimix mcp doctor               # Check every configured server
kimix mcp doctor github        # Check one server
kimix mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `kimix mcp add` writes to `~/.kimix/config.toml` (`--scope user`). Use `--scope project` to write to `.kimix/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `kimix mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`kimix mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.kimix/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Kimix handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Kimix's own OAuth client with the provider.

Kimix walks from the current directory up to the git repo root, loading `.kimix/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.kimix/config.toml` | All projects | Lowest |
| `<repo-root>/.kimix/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Kimix reads most other config sections only from `~/.kimix/config.toml`.
...
You can enable or disable MCP servers during a session without restarting Kimix.
...
Kimix loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Kimix config | `~/.kimix/config.toml`, `.kimix/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.kimix/config.toml` or the corresponding environment variable (`KIMIX_CURSOR_MCPS_ENABLED`, `KIMIX_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `kimix inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).

---

## MCP OAuth

For MCP servers that require OAuth authentication, Kimix handles the credential flow automatically. When an MCP server requests OAuth credentials, Kimix opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. Kimix stores the resulting tokens under `~/.kimix/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Kimix expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, Kimix captures the process's standard error to `~/.kimix/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.kimix/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `kimix inspect` to see all loaded MCP servers and their sources:

```bash
kimix inspect          # Human-readable
kimix inspect --json   # Machine-readable
```

### Debug Logging

```bash
RUST_LOG=debug KIMIX_LOG_FILE=/tmp/kimix.log kimix
tail -f /tmp/kimix.log
```
