...
Kimix is multi-model by design. You can attach any OpenAI-compatible or
Anthropic-compatible endpoint, self-hosted servers, and BYOK providers. This
guide covers selecting models, configuring endpoints, and wiring third-party
APIs.

Local config path: **`~/.kimix/config.toml`**.

---

## Default Models

The default model is whatever you set under `[models] default = "..."` (or the
CLI default if unset). There is no single “platform model” identity — pick the
providers you use day to day.

Authenticate with `kimix login` / `kimix login --xai`, or set API keys via the
`env_key` fields on each model entry.
...
Set a persistent default in `~/.kimix/config.toml`:
...
Kimix supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, Kimix uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. Kimix sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.kimix/config.toml` under `[model.<name>]` sections:
...
env_key = "ENV_KEY"                   # Env var holding the API key (optional; string or array)
...
Kimix resolves the API key in this order:
...
3. Your signed-in session token (from `kimix login`), for a model with no `api_key`/`env_key` of its own
4. Provider-specific environment variables named in each model’s `env_key`

### Context Window

The `context_window` value tells Kimix when to trigger auto-compaction. When you override a known model, Kimix inherits that model's context window. When you define a new model and omit `context_window`, Kimix defaults to 200,000 tokens, so set it explicitly to match your provider.
...
When you override a built-in model, Kimix starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which Kimix sends verbatim.
...
Point Kimix at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `KIMIX_MODELS_BASE_URL` | Yes | Base URL for inference. Kimix fetches the model list from `{base_url}/models`. |
| the model `env_key` variable | Yes | API key sent as `Authorization: Bearer`. Kimix also accepts `KIMIX_CODE_ENV_KEY`. |
| `KIMIX_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, Kimix inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, Kimix uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `kimix login` -- the API key is enough.
