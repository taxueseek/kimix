...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.kimix/agents/` or `~/.kimix/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.kimix/personas/`; applied during subagent resolution |
...
| **Examples** | `kimix`, `explore`, `plan` | `researcher`, `concise` |
...
Kimix discovers file-based personas from these locations, in priority order:

- `.kimix/personas/*.toml` (project)
- `~/.kimix/personas/*.toml` (user)
...
> **Note:** Kimix applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Kimix resolves the effective model and reasoning effort in this order, highest priority first:
...
Kimix manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "kimix"               # route explore to a specific model
```
...
# instructions_file = ".kimix/personas/concise.md"  # or load from a file
```

Kimix also discovers roles from `.kimix/roles/*.toml` and personas from `.kimix/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Kimix shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · kimix-3) — Thinking`
