...
Kimix runs as a full-screen TUI. To draw the interface, it relies on terminal escape sequences for color, clipboard, mouse, and full-screen control. Some terminals, multiplexers, and SSH sessions handle these sequences differently.
...
- Inside tmux, the variables Kimix needs to identify the terminal don't reach the pager.
...
**Fix**: Apply the two settings above, then restart Kimix.
...
Kimix writes to the clipboard through up to three routes, which match the **Clipboard routes** section of `/terminal-setup`:

- **native** — Kimix always writes to the native OS clipboard first.
- **tmux buffer** — inside tmux, Kimix also writes to the tmux paste buffer (`tmux load-buffer`).
- **OSC 52** — Kimix emits the OSC 52 escape sequence so the outer terminal updates its clipboard. Kimix always emits OSC 52 inside tmux. Outside tmux, it emits OSC 52 on Linux, over SSH, or in a container without a display.

**Linux Wayland**: on compositors that support the data-control protocol (GNOME 48+, KDE, Sway, Hyprland — the `data-control` line in `/terminal-setup` shows `yes`) copies work even if the terminal loses focus mid-copy. On older compositors (GNOME 46/47), keep the terminal focused until the copy toast confirms, and install the `wl-clipboard` package (provides `wl-copy`) for the most reliable route — Kimix shows a startup warning when this applies. If data-control misbehaves on your compositor, set `KIMIX_CLIPBOARD_NO_DATA_CONTROL=1` to stop Kimix from speaking that protocol entirely — copies then go through the CLI tools (`wl-copy`/`xclip`).

**Linux X11 selections**: X11 **PRIMARY** and **CLIPBOARD** are separate. Selecting text usually fills PRIMARY; an explicit Copy action fills CLIPBOARD. In Kimix:

- An unmodified middle click reads PRIMARY only when `DISPLAY` is non-empty. Pure X11 can fall back to the native arboard reader. XWayland must have `xclip` or `xsel` on `PATH`; Kimix deliberately disables the arboard fallback there so it cannot substitute Wayland PRIMARY.
...
**SSH and selected text**: a remote Kimix process usually cannot read the local terminal's PRIMARY or CLIPBOARD selection. Use terminal-native `Shift+Insert`, or hold `Shift` while middle-clicking when your terminal uses that gesture to bypass mouse reporting. The terminal then sends the local selection through the PTY instead of asking the remote process to access it.

**Known limitation — Apple Terminal + SSH**:
Apple Terminal ignores OSC 52, so copying from a Kimix session over SSH can't reach your local clipboard. Use the workaround below.

**Temporary workaround**: Use `kimix wrap ssh` instead of plain `ssh` (for example, `kimix wrap ssh user@host`). It runs the command in a local PTY that intercepts OSC 52 sequences, including tmux-wrapped ones, and writes their contents to your local clipboard. The same command wraps anything else whose clipboard can't reach you — for example `kimix wrap docker exec -it <container> bash` or `kimix wrap kubectl exec -it <pod> -- bash`.

> **Warning**: `kimix wrap` is **experimental** and may misbehave in some setups.
...
This setting is off by default for security reasons. Without it, OSC 52 writes from Kimix (or any TUI) will be ignored.
...
**Fix**:
- In Zellij or control mode, Kimix intentionally runs inline (no alt screen).
- Set `[terminal] alt_screen = "always"` in `~/.kimix/pager.toml` to force fullscreen.
...
### Problem: Zellij keybindings interfere with Kimix (Ctrl+g, Ctrl+o, etc.)

Zellij intercepts many Ctrl/Alt key combinations before they reach full-screen TUIs like Kimix.
...
After this, Zellij starts **locked**. Most keys pass through to Kimix. Press `Ctrl+g` to temporarily unlock Zellij when you need its pane/session management.

Zellij recommends this approach for TUI users.

### Problem: `Ctrl+Enter` doesn't interject in WezTerm

**Cause**: WezTerm ships with the Kitty keyboard protocol disabled. Kimix relies on it to tell `Ctrl+Enter` (interject) and `Shift+Enter` (send in multiline mode) apart from plain `Enter`. Most other terminals enable the protocol when Kimix requests it.

For the same reason, in Apple Terminal, Kimix binds `Ctrl+O` to interject.
...
Reload (`Cmd+Shift+R` or restart WezTerm) and restart `kimix`.

**Verify**: Run `/terminal-setup` inside Kimix. While a turn is active, you see the interject hint, and `Ctrl+Enter` interjects.
...
plain digits). Kimix therefore never negotiates the protocol for these
...
container): `TERM_PROGRAM` isn't forwarded, so Kimix sees an `Unknown`
terminal and skips the protocol for the same reason.

**Fix**: Use **`Alt+Enter`** to insert a newline. xterm.js delivers it
reliably as `ESC`+`CR` regardless of the keyboard protocol, and Kimix's
...
If Kimix's mouse-driven scrolling stops responding and your terminal falls back to its native scrollbar, mouse reporting is off.
