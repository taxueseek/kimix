...
Hooks let you run custom scripts or HTTP requests at key moments during a Kimix session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.kimix/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.kimix/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.kimix/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match Kimix's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
Kimix injects the following variables into every hook process:

- `KIMIX_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `KIMIX_HOOK_NAME` — the full configured name of this hook
- `KIMIX_SESSION_ID` — the current session identifier
- `KIMIX_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `KIMIX_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `KIMIX_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `KIMIX_PLUGIN_ROOT` and `KIMIX_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`KIMIX_PLUGIN_ROOT` and `KIMIX_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/kimix-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env Kimix itself sees).
...
`${KIMIX_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.kimix/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.kimix/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.kimix/logs`).

## More Examples

See the built-in examples in the `kimix-hooks` crate:

- [Safe Shell Guard](../../../kimix-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../kimix-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../kimix-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../kimix-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.kimix/hooks/` and customize.
