...
#[ignore = "PTY e2e; run with cargo test -p kimix-tui --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared KIMIX_SHARE_DIR so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let kimix_home = content.home().join(".kimix");
    std::fs::create_dir_all(&kimix_home).expect("create KIMIX_SHARE_DIR");
    std::fs::write(
    ...
    let state_path = kimix_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(kimix_home.join("config.toml")).expect("read config.toml");
    assert!(
