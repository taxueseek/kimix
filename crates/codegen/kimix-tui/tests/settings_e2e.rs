...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    kimix_tui::views::settings_modal::render_settings_modal(&mut buf, area, &mut s, false, None);
...
fn defaults_round_trip_through_registry() {
    use kimix_tui::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    kimix_tui::appearance::cache::set_keep_text_selection(
        kimix_tui::appearance::TextSelection::Flash,
    );
    kimix_tui::appearance::cache::set_show_thinking_blocks(true);
    kimix_tui::appearance::cache::set_prompt_suggestions(true);
    kimix_tui::appearance::cache::set_group_tool_verbs(true);
    kimix_tui::appearance::cache::set_scroll_mode(kimix_tui::appearance::ScrollMode::Auto);
    kimix_tui::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    kimix_tui::appearance::cache::set_scroll_lines(3);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("kimixnight"),
            "auto_dark_theme" => SettingValue::Enum("kimixnight"),
            "auto_light_theme" => SettingValue::Enum("kimixday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = kimix_tui::settings::default_value_for(meta);
...
// `crates/codegen/kimix-tui/src/app/dispatch.rs::tests` (next to
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use kimix_tui::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use kimix_tui::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    use kimix_tui::views::settings_modal::ResetConfirmOverlay;
    use ratatui::buffer::Buffer;
    ...
    kimix_tui::views::settings_modal::render_settings_modal(
        &mut buf,
        area,
        &mut s,
        false,
...
/// "Ask Kimix" tip footer at the bottom of the content area in
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask Kimix"),
            "[{fixture_label}] docs footer (`Ask Kimix`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change theme to kimixday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    kimix_tui::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use kimix_shell::agent::config::UiConfig;
    use kimix_tui::settings::{PagerLocalSnapshot, SettingsRegistry};
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use kimix_shell::agent::config::UiConfig;
    use kimix_tui::settings::{PagerLocalSnapshot, SettingsRegistry};
    use kimix_tui::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr14_default_model_picker_commits_resolved_model_id() {
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
            (
                "Kimix 4.5".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("kimix-4.5")),
            ),
            (
                "Kimix 3".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("kimix-3")),
            ),
        ],
        ..PagerLocalSnapshot::default()
    };
    ...
    // Walk down past row 0 ("(no override)") to row 1 ("Kimix 4.5").
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Down));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Changed),
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        kimix_tui::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        kimix_tui::settings::default_value_for(mt_meta),
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use kimix_tui::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use kimix_tui::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use kimix_tui::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use kimix_tui::appearance::TextSelection;
    ...
    kimix_tui::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    kimix_tui::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    kimix_tui::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    kimix_tui::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    kimix_tui::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    kimix_tui::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    kimix_tui::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    kimix_tui::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
