...
//!
//! These tests exercise the real `kimix-tui` binary through a PTY using
...
/// enterprise deploy report: with `KIMIX_GOAL=1`, `/goal` must
...
/// explicitly off (`KIMIX_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `KIMIX_FOLDER_TRUST=1` and a git repo that
...
/// also pins `KIMIX_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
