...
//!
//! Uses `kimix-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.kimix/auth.json` from `KIMIX_AUTH_JSON` (path) or the
//! developer's `~/.kimix/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p kimix-tui --test pty_auto_mode -- --ignored --nocapture`
use std::path::{Path, PathBuf};
use std::time::Duration;

use kimix_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.kimix/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("KIMIX_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    let _ = std::fs::create_dir_all(&kimix);
    if let Some(src) = auth_json_source() {
        let dest = kimix.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.kimix/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("KIMIX_SHARE_DIR".into(), kimix.display().to_string()),
    ...
    // of the developer's shell. `KIMIX_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (kimix_config::
    ...
    env.push((
        "KIMIX_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("KIMIX_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (kimix-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "KIMIX_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
