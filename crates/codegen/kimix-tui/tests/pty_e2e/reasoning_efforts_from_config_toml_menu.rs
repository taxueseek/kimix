...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("kimix-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.kimix/config.toml` with a per-model reasoning-effort menu.
    let kimix_home = content.home().join(".kimix");
    std::fs::create_dir_all(&kimix_home).expect("create .kimix");
    // Quote the dotted model id: bare `[model.kimix-4.5]` is TOML key-path syntax (model.kimix-4.5), not the id "kimix-4.5".
    std::fs::write(
