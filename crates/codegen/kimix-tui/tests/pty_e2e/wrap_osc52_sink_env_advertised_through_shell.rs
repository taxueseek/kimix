...
/// `KIMIX_OSC52_SINK=1` (the signal an inner `kimix` uses to trust OSC 52 over
...
fn wrap_osc52_sink_env_advertised_through_shell() {
    let (code, raw) = run_wrap(
        &["echo sink=$KIMIX_OSC52_SINK"],
        &[("SHELL", "/bin/sh"), ("KIMIX_OSC52_SINK", "0")],
    );

    assert!(
        raw.contains("sink=1"),
        "wrapped child must see KIMIX_OSC52_SINK=1 through the shell route\nraw:\n{raw}"
    );
    assert!(
        !raw.contains("sink=0"),
