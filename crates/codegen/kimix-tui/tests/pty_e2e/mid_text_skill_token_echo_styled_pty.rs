...
use kimix_pager_pty_harness::StyledLine;
...
/// Seed a user-invocable skill under `dir/.kimix/skills`. The workspace must
/// be a git repo: the live session advertises workspace-local skills only for
/// git workspaces (offline `kimix inspect` scans plain dirs too) — same shape
/// as the mid_text_skill_token_echo.yaml scenario's `git_init: true`.
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".kimix").join("skills").join("test-skill");
