...
use kimix_pager_pty_harness::PtyHarness;
pub(crate) use kimix_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`kimix-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `kimix-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `KIMIX_SCROLL_SPEED`) appended to the controller's env.
