...
/// turn and the turn-end `Kimix/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`kimix-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "KIMIX_PROMPT_SUGGESTIONS");
    env.push(("KIMIX_PROMPT_SUGGESTIONS".into(), "true".into()));
