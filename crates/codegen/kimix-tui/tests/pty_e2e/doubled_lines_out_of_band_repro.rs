...
//!
//! The stack is `tmux -> nvim :terminal -> Kimix`: nvim/tmux repaint Kimix's
//! pane out-of-band (no kimix PTY resize), and Kimix's diff renderer only
...
//! into the virtual terminal, bypassing Kimix) and then check whether kimix
//! heals it. `NVIM` is set in Kimix's env so it sees the embedded-editor
//! context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: kimix forces a full clear+repaint on `FocusGained` in
//! editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.
use super::common::*;

/// Unique sentinel that kimix would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    env.push((
        "NVIM".into(),
        "/tmp/kimix-pty-harness-fake-nvim.sock".into(),
    ));
    ...
    // virtual screen at col 1 — the static left margin Kimix's diff renderer
    ...
    // kimix must not self-heal out-of-band content via ordinary diff redraws:
    // it only rewrites cells whose own model changed, so this row is stranded.
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (kimix's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear+repaint that re-asserts Kimix's
