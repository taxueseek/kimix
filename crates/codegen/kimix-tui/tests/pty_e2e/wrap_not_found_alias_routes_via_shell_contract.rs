...
/// argv shape is the part `kimix wrap` owns. The quoting round-trip against a
...
fn wrap_not_found_alias_routes_via_shell_contract() {
    ...
    let (code, raw) = run_wrap(
        &["kimix-wrap-e2e-alias-xx", "with space"],
        &[("SHELL", &shell)],
    );
    ...
    assert!(
        raw.contains("ARG:kimix-wrap-e2e-alias-xx 'with space'"),
