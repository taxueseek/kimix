...
// ── A8: scroll-debug HUD e2e (`KIMIX_SCROLL_DEBUG`) ─────────────────────────
//
...
//
// - `KIMIX_SCROLL_MODE=trackpad` pins classification (the A5 env → live
...
/// **Env-on e2e.** `KIMIX_SCROLL_DEBUG=1` must paint the HUD (panel title +
...
async fn scroll_debug_hud_env_shows_hud_and_tracks_flood() {
    let (mut harness, _content, top_before) = spawn_bottom_pinned_marker_scrollback_with_env(
        MARKER_COUNT,
        &[
            ("KIMIX_SCROLL_DEBUG", "1"),
            ("KIMIX_SCROLL_MODE", "trackpad"),
        ],
    )
    .await;
    ...
    assert!(
        ...
        "HUD title missing with KIMIX_SCROLL_DEBUG=1\nscreen:\n{}",
...
async fn scroll_debug_hud_absent_without_env() {
    ...
    assert!(
        !harness.contains_text("scroll debug"),
        "HUD must stay dark without KIMIX_SCROLL_DEBUG\nscreen:\n{}",
        harness.screen_contents()
    );
...
async fn debug_scroll_command_toggles_hud_live() {
    ...
    assert!(
        !harness.contains_text("scroll debug"),
        "HUD must start dark without KIMIX_SCROLL_DEBUG\nscreen:\n{}",
        harness.screen_contents()
    );
