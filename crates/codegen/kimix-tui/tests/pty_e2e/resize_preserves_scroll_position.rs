...
//
// Bug: when the user has scrolled UP into Kimix's scrollback (NOT pinned to the
// bottom / not following) and the terminal is resized HORIZONTALLY, kimix loses
...
// in follow/tail mode kimix re-pins to the bottom (no jump).
//
...
//
// Runs FULLSCREEN (alt-screen): Kimix's mode-independent scroll-anchor logic is
// what's under test. `Viewport::Fullscreen` is autoresized on resize with no DSR
// cursor probe, so kimix itself re-wraps at the new width, and the alt-screen grid
// is not reflowed by the terminal — so the marker's position reflects Kimix's own
...
///
/// kimix renders markdown with SOFT line breaks (a single `\n` becomes a space),
...
async fn resize_preserves_scroll_position() {
    ...
    // terminal resize with NO DSR cursor probe, so kimix re-wraps the scrollback
    // at the new width and the stale `scroll_offset` surfaces the jump in Kimix's
    // OWN rendered output. The alt-screen grid is not reflowed by the terminal on
    // resize, so this isolates Kimix's scroll-anchor logic with no harness confound.
