...
async fn managed_policy_gate_refusal_reaches_real_terminal() {
    ...
    let mut harness = PtyHarness::new(
        ...
        DEFAULT_COLS,
        &["--no-auto-update"],
        // KIMIX_MANAGED_CONFIG=0 disables the background refetch so the gate decision is deterministic and offline.
        &[
            ("KIMIX_SHARE_DIR", home_str),
            ("KIMIX_MANAGED_CONFIG", "0"),
            ("NO_COLOR", "1"),
        ],
    )
    .expect("spawn pager");
