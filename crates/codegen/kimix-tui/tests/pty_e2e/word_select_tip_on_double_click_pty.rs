...
async fn word_select_tip_shows_and_ctrl_y_accepts() {
    ...
    let config_path = content.home().join(".kimix").join("config.toml");
...
async fn word_select_tip_skipped_when_contextual_hint_disabled() {
    let content = ContentController::start().await.expect("start content");
    // flash mode + tip explicitly disabled. KIMIX_CONTEXTUAL_HINTS is NOT set —
    ...
    // inherited KIMIX_CONTEXTUAL_HINTS from the runner's shell can't force
    // tips on and defeat the config opt-out under test.
    let mut env = content.env_for_pager();
    env.push(("KIMIX_CONTEXTUAL_HINTS".into(), String::new()));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    let mut harness =
