...
//
// `KIMIX_SCROLL_MODE=wheel` + `KIMIX_SCROLL_LINES=1` on a `TERM_PROGRAM=zed`
...
// - Without KIMIX_SCROLL_LINES=1, Zed's wheel profile prices the burst at
//   3 lines/event → 9 rows.
// - Without KIMIX_SCROLL_MODE=wheel, Auto finalizes a 3-event ZERO-interval
...
// no acceleration, and the harness's hermetic KIMIX_SHARE_DIR pins scroll_speed at
// its default (50 → 1.0x).
...
/// **Env-forced wheel pricing e2e.** The `KIMIX_SCROLL_MODE` /
/// `KIMIX_SCROLL_LINES` overrides must reach the live config: a 3-event burst
...
async fn forced_wheel_mode_env_scrolls_exact_rows() {
    let (mut harness, _content, top_before) = spawn_bottom_pinned_marker_scrollback_with_env(
        ...
        &[
            ("TERM_PROGRAM", "zed"),
            ("KIMIX_SCROLL_MODE", "wheel"),
            ("KIMIX_SCROLL_LINES", "1"),
        ],
    )
    .await;
