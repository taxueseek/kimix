...
pub(crate) use kimix_pager_pty_harness::{
...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("KIMIX_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `kimix_pager_pty_harness::flows` (re-exported above).
...
/// (`KIMIX_TEST_VERSION`) and an explicit `KIMIX_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/KIMIX_SHARE_DIR
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    // A self-built (unstamped) kimix auto-trusts and never prompts; simulate a
    ...
    // auto-trust.
    env.push(("KIMIX_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set KIMIX_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("KIMIX_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".kimix")
        .join(kimix_workspace::trust::TRUST_FILE_NAME);
    let store = kimix_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&kimix_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `kimix_pager_pty_harness::flows`
// (re-exported above).
...
/// Budget for session creation plus the `Kimix/mcp/list` round-trip.
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let kimix_home = content.home().join(".kimix");
    std::fs::create_dir_all(&kimix_home).expect("create fake KIMIX_SHARE_DIR");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(kimix_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in kimix-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.kimix/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{KIMIX_SHARE_DIR|HOME}/.kimix/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let kimix_home = content.home().join(".kimix");
    std::fs::create_dir_all(&kimix_home).expect("create .kimix");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let kimix_home = content.home().join(".kimix");
    std::fs::create_dir_all(&kimix_home).expect("create .kimix");
    // Minimal opt-in only — matches load_config's `{KIMIX_SHARE_DIR|HOME}/.kimix/config.toml`.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("KIMIX_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "KIMIX_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        kimix_tui::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `kimix wrap` run budget. Same contention math as the requirements-version
...
/// Run `kimix wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `KIMIX_SHARE_DIR`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap(wrap_args: &[&str], extra_env: &[(&str, &str)]) -> (Option<u32>, String) {
    ...
    let mut env: Vec<(&str, &str)> = vec![("KIMIX_SHARE_DIR", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness = PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env)
        .expect("spawn kimix wrap");
...
/// `kimix wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$KIMIX_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("KIMIX_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use kimix_pager_pty_harness::host_clipboard::{
...
pub(crate) use kimix_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
