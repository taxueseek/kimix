...
也可将图片文件拖入输入框。"
        ...
        "No agents match `a:{n}` — press Esc to clear the filter." => "没有匹配 `a:{n}` 的 Agent — 按 Esc 清除筛选。",
        "No agents in state `{}` — press Esc to clear the filter." => "没有处于 `{}` 状态的 Agent — 按 Esc 清除筛选。",
