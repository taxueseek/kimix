...
//! render mode.
//!
//! `kimix-tui` cannot depend on `kimix-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`kimix-bin`) wires it up once at
//! startup by calling `kimix_pager_minimal::install()`. When nothing has
