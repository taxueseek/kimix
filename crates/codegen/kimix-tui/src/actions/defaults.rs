...
pub fn default_actions(mouse_reporting_toggle_enabled: bool) -> Vec<ActionDef> {
    ...
    let mut actions = vec![
        ...
        ActionDef {
            ...
            requires_confirmation: false,
            long_help: Some(
                "Interrupts the agent's current turn and stops generation, keeping the session open.\nEsc cancels when the prompt is empty; with a non-empty draft it clears the prompt first and leaves the turn running.\nIt stops the turn, not the app; use the quit shortcut to exit.",
            ),
        },
        ActionDef {
            id: ActionId::CycleMode,
            label: "mode",
            description: "Cycle mode (Normal → Plan → Decision → Normal)",
            // Tab and all Shift+Tab encodings — see `input::key::shift_tab_keys()`.
            default_key: crate::input::key::shift_tab_keys()[0],
            alt_keys: {
                let mut keys = vec![key!(Tab)];
                keys.extend_from_slice(&crate::input::key::shift_tab_keys()[1..]);
                keys
            },
            category: Category::GettingStarted,
            context: When::PromptFocused,
            hint_priority: None,
            hint_key_display: Some("Tab"),
            requires_confirmation: false,
            long_help: Some(
                "Steps the session mode: Normal -> Plan -> Decision -> Normal.\nPlan keeps the agent planning first and writes no files; Decision applies first-principles analysis and MECE frameworks before executing.",
            ),
        },
    ...
    actions.extend([
        ...
        // rebind any of them through `~/.kimix/config.toml`.
        ActionDef {
