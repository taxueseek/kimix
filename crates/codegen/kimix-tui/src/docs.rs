...
/// Extract user-guide docs to `<kimix_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(kimix_home: &std::path::Path) {
    let docs_dir = kimix_home.join("docs").join("user-guide");
