//! Scroll flight recorder — `KIMIX_SCROLL_LOG` JSONL log of scroll-stream
...
//! (see [`super::mouse`]).
//!
//! Enablement: `KIMIX_SCROLL_LOG=1` (or set-but-empty) logs to
//! `~/.kimix/logs/scroll-log-<timestamp>.jsonl`; any other non-`0` value is
//! used as the target path. Unset (or `0`, matching `KIMIX_SCROLL_DEBUG`)
...
/// Appends [`ScrollLogRecord`]s to the `KIMIX_SCROLL_LOG` file. Owned as
...
impl ScrollLogRecorder {
    /// Build from `KIMIX_SCROLL_LOG` (see module docs for value semantics);
    /// `None` when unset or `0`.
    pub(crate) fn from_env_at(base: Instant) -> Option<Self> {
        let raw = std::env::var("KIMIX_SCROLL_LOG").ok()?;
        let value = raw.trim();
        if value == "0" {
            return None;
        }
...
/// `~/.kimix/logs/scroll-log-<utc-ts>.jsonl` — the input-debug dump's dir
