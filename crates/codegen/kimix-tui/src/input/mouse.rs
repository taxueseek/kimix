...
//! `scroll_mode` setting (or `KIMIX_SCROLL_MODE`) pins [`ScrollInputMode`], and `invert_scroll` /
...
// `kimix-pager-pty-harness/src/scroll_matrix/gestures.rs` (the harness
...
// (`KIMIX_SCROLL_CADENCE_MS` from the event loop); the harness still models
// the 16ms default.
...
pub struct ScrollDebugSnapshot {
    ...
    pub flush_cap: i32,
    /// Effective scroll flush cadence in ms (`KIMIX_SCROLL_CADENCE_MS` / default 16).
    pub cadence_ms: u64,
}
...
///   redraw cadence (default 16ms / ~60Hz; `KIMIX_SCROLL_CADENCE_MS`) to avoid floods.
...
pub struct MouseScrollState {
    ...
    /// `KIMIX_SCROLL_LOG` flight recorder ([`crate::input::scroll_log`]).
...
impl MouseScrollState {
    ...
    /// Inject flush cadence (`KIMIX_SCROLL_CADENCE_MS`); Default stays 16ms.
    ...
    /// Emit a flight-recorder line; a single branch when `KIMIX_SCROLL_LOG`
