...
//! pattern as kimix-shell's `compaction.rs`): entry from the queue pane,
...
pub enum PromptMode {
    ...
    EditingQueued {
        ...
        /// `Kimix/queue/changed` rebroadcast for the visual result) instead
...
impl AgentView {
    ...
    fn interject_edited_queued(
        ...
        match server_id {
            Some(server_id) => {
                // Server rows: the queue wire (`Kimix/queue/interject` newText)
