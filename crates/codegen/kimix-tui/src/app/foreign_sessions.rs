...
use kimix_workspace::foreign_sessions::{
...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, kimix_home: &Path) -> [PathBuf; 2] {
        ...
        [
            kimix_home
                ...
                .join(skill)
                .join("SKILL.md"),
            kimix_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(kimix_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, kimix_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, kimix_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        kimix_home: kimix_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use kimix_workspace::foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_supports_bundled_and_user_skill_locations() {
        let enabled = gated_sources_async_with(compat_all(), Path::new("/kimix"), |path| {
