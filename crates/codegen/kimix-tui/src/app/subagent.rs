...
thread_local! {
    static REPLAY_KIMIX_SHARE_DIR : std::cell::RefCell < Option < std::path::PathBuf >> = const
    { std::cell::RefCell::new(None) };
}
/// Override kimix home for disk-replay unit tests (thread-local; production never sets this).
#[cfg(test)]
pub(crate) fn set_replay_kimix_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_KIMIX_SHARE_DIR.with(|h| *h.borrow_mut() = home);
}
fn effective_kimix_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_KIMIX_SHARE_DIR.with(|h| h.borrow().clone()) {
        return home;
    }
...
pub(crate) fn enrich_from_meta(
    ...
    enrich_from_meta_with_home(info, &effective_kimix_home(), parent_cwd, parent_session_id);
}
...
pub(crate) fn replay_inherited_updates(
    ...
    let home = effective_kimix_home();
    let updates =
        match kimix_shell::session::storage::load_updates_for_replay_at(child_session_id, &home) {
            Ok(Some(u)) => u,
            Ok(None) => return,
            Err(e) => {
                tracing::debug!(
...
mod tests {
    ...
    fn ensure_subagent_child_replayed_releases_retained_memory_once() {
        ...
        set_replay_kimix_home_for_tests(Some(home.path().to_path_buf()));
        let mut parent = make_min_child_view();
        parent
            .subagent_views
        ...
        set_replay_kimix_home_for_tests(None);
    }
    #[test]
    ...
    fn subagent_meta_all_fields() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), Some("analyst"), Some("kimix-3")),
            " (researcher \u{00b7} analyst \u{00b7} kimix-3)"
        );
    }
    #[test]
    fn subagent_meta_partial_skips_nones() {
        assert_eq!(
            format_subagent_meta(Some("researcher"), None, Some("kimix-3")),
            " (researcher \u{00b7} kimix-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_collapses_duplicate_persona_role() {
        assert_eq!(
            format_subagent_meta(Some("reviewer"), Some("reviewer"), Some("kimix-3")),
            " (reviewer \u{00b7} kimix-3)"
        );
    }
    #[test]
    ...
    fn subagent_meta_drops_both_empty_persona_role() {
        assert_eq!(
            format_subagent_meta(Some(""), Some(" "), Some("kimix-3")),
            " (kimix-3)"
        );
    }
    #[test]
