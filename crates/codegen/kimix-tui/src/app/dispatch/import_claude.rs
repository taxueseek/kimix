...
pub(super) fn dispatch_import_claude(app: &mut AppView) -> Vec<Effect> {
    ...
    let plan = kimix_shell::claude_import::scan_importable_settings(&cwd);

    if plan.is_empty() {
        kimix_shell::claude_import_state::mark_dismissed(&cwd);
        // Always write the [claude_compat] imported = true marker so the user's
        // opt-in is recorded even on an empty plan.
        if let Err(e) = kimix_shell::claude_import::mark_claude_imported() {
...
pub(super) fn dispatch_import_claude_confirm(app: &mut AppView) -> Vec<Effect> {
    ...
    if selected_count > 0 {
        match kimix_shell::claude_import::apply_import(&filtered, &cwd) {
            Ok(result) => {
    ...
    // the slash command.
    kimix_shell::claude_import_state::mark_imported(&cwd);
    if let Err(e) = kimix_shell::claude_import::mark_claude_imported() {
        tracing::warn!(error = %e, "Failed to write Claude import marker");
    }
    app.has_claude_import = false;
