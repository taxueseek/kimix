...
fn worktree_session_created_sets_session_and_cwd() {
    ...
    let id = AgentId(0);
    let worktree_path = PathBuf::from("/tmp/kimix-worktrees/pager-123");
...
fn worktree_session_preserves_subdirectory_offset() {
    ...
    app.cwd = PathBuf::from("/repo/crates/codegen/kimix-tui");
    ...
    let id = AgentId(0);
    let worktree_root = PathBuf::from("/home/user/.kimix/worktrees/repo/pager-123");
    let session_cwd = worktree_root.join("crates/codegen/kimix-tui");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
...
fn worktree_session_created_drains_queued_prompts() {
    ...
    assert_eq!(app.agents[&id].session.queue_len(), 1);
    let worktree_path = PathBuf::from("/tmp/kimix-worktrees/pager-abc");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
            session_id: acp::SessionId::new("wt-drain-1"),
            worktree_path,
            session_cwd: PathBuf::from("/tmp/kimix-worktrees/pager-abc"),
            models: None,
        }),
        &mut app,
    );
...
/// trust. KIMIX_SHARE_DIR-isolated so the write hits a temp store, not the real one.
#[serial_test::serial(KIMIX_SHARE_DIR)]
#[test]
fn trust_folder_grants_and_resolves() {
    use kimix_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().expect("home tempdir");
    unsafe { std::env::set_var("KIMIX_SHARE_DIR", home.path()) };
...
/// only view that renders the multi-provider picker / device-code UI) and
/// stash the agent view for restoration. With `method_id: None` it lands
/// on the picker (Pending), not auto-Kimi authenticate.
#[test]
fn login_mid_session_switches_to_welcome_and_stashes_view() {
    ...
    assert!(
        matches!(app.auth_state, AuthState::Pending { .. }),
        "bare /login must show the multi-provider picker, got {:?}",
        app.auth_state
    );
    assert!(
        effects.is_empty(),
        "picker display is UI-only until a row is chosen, got {effects:?}"
    );
}

/// Selecting a concrete method mid-session still kicks off authenticate.
#[test]
fn login_mid_session_with_method_starts_auth() {
    let mut app = test_app_with_agent();
    let effects = dispatch(
        Action::Login {
            method_id: Some(acp::AuthMethodId::new("xai-session")),
        },
        &mut app,
    );
    assert_eq!(app.active_view, ActiveView::Welcome);
...
fn auth_complete_strips_reauth_prompt_after_mid_session_login() {
    ...
    // Start a concrete provider login (picker alone does not Authenticate).
    dispatch(
        Action::Login {
...
fn cycle_mode_pre_session_blocked_by_policy_pin() {
    ...
    app.yolo_policy_block = Some(POLICY_WARNING);
    {
        ...
        agent.deferred_session_mode = Some(kimix_tools::types::SessionMode::Plan);
    }
...
fn dispatch_cycle_mode_pre_session_cycles_locally_and_creates_session() {
    ...
    assert_eq!(
        ...
        Some(kimix_tools::types::SessionMode::Plan),
