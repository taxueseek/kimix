...
/// Turn end with prompt suggestions enabled fires the `Kimix/suggestPrompt`
...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `kimix-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `kimix-0.1`
...
/// `Kimix/queue/changed` rebroadcast — not client-side prediction — updates
...
/// The always-on pipeline end to end, with `KIMIX_SUGGESTIONS` semantics
