...
fn slash_model_valid_dispatches_set_default_model_with_switch_and_persist() {
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_id = acp::ModelId::new(std::sync::Arc::from("kimix-4.5"));
    app.agents
        .get_mut(&id)
        .unwrap()
        .session
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Kimix 4.5".to_string()),
        );
    let effects = dispatch(Action::SendPrompt("/model Kimix 4.5".into()), &mut app);
    assert_eq!(
        effects.len(),
        2,
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("kimix-test"));
    let info = acp::ModelInfo::new(id.clone(), "Kimix Test".to_string());
...
fn set_default_model_resolves_known_name() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("kimix-4.5"));
    let info = acp::ModelInfo::new(id.clone(), "Kimix 4.5".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
    ...
    assert!(
        matches!(& effects[0], Effect::PersistSetting { key : "default_model", value :
        crate ::settings::SettingValue::String(s), .. } if s == "kimix-4.5")
    );
...
fn set_default_model_idempotent_when_already_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("kimix-already"));
    let info = acp::ModelInfo::new(id.clone(), "Kimix Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
fn move_setting_away_from_default(app: &mut AppView, key: crate::settings::SettingKey) {
    match key {
        ...
        "language" => {
            let _ = dispatch(Action::SetLanguage("zh".to_string()), app);
        }
...
/// `clamp_to_terminal` might fold to KimixNight in non-truecolor
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        crate::theme::cache::set(crate::theme::ThemeKind::KimixNight);
        let effects = dispatch(Action::SetTheme("kimixday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("kimixday"));
                assert_eq!(*rollback_value, SettingValue::Enum("kimixnight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("kimixday"));
        assert!(
...
/// for the auto-dark sibling. Uses `kimixday` to avoid the
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("kimixday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
        ...
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("kimixday"));
    });
}
#[test]
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("kimixnight".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*rollback_value, SettingValue::Enum("kimixday"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="kimixnight"` (concrete) + system=Dark. User
/// commits `auto_dark_theme="kimixday"`. The setting is dormant
/// (parent theme is concrete, not auto), so the live display
/// must stay on KimixNight even though we're committing a
/// different theme. Uses `kimixday` to avoid `clamp_to_terminal`
...
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("kimixnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::KimixNight,
        );
        let _ = dispatch(Action::SetAutoDarkTheme("kimixday".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::KimixNight,
...
///
/// Uses `KimixDay` (non-truecolor-requiring) for the dark-mode
...
/// KimixNight. Using a non-truecolor theme avoids the clamp
...
fn set_auto_dark_theme_applies_when_theme_is_auto_and_system_is_dark() {
    with_theme_test_env(|| {
        ...
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::KimixDay,
...
/// Uses `kimixnight` for the auto_dark_theme
/// value to avoid `clamp_to_terminal` ambiguity (we want a
/// concrete kind that's clearly different from KimixDay, the
...
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("kimixnight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::KimixNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("kimixday".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("Kimix Day"),
            "toast must use display name `Kimix Day`, not canonical `kimixday`, got: {toast:?}",
        );
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Kimix Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Kimix Night"));
    });
}
...
///
/// Uses non-truecolor themes (`kimixday` ↔ `kimixnight`) to avoid
...
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("kimixnight"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("kimixday"),
                error: "disk full".into(),
            }),
            &mut app,
        );
        assert_eq!(app.current_ui.auto_light_theme.as_deref(), Some("kimixday"));
    });
}
