...
fn foreign_resume_hint(
    tool: kimix_workspace::foreign_sessions::ForeignSessionTool,
) -> kimix_workspace::foreign_sessions::RecentForeignSession {
    kimix_workspace::foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                kimix_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(kimix_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                kimix_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn plugins_action_success_sets_result_notice_and_autoreload_preserves_it() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(kimix_hooks_plugins_types::ActionOutcome {
                status: kimix_hooks_plugins_types::OutcomeStatus::Success,
...
fn uninstall_result_notice_is_footer_only_not_row_anchored() {
    ...
    let id = AgentId(0);
    {
        ...
        modal.last_plugins_action = Some(kimix_hooks_plugins_types::PluginsAction::Uninstall {
...
/// Regression (Bugbot): a failed `Kimix/subagent/cancel` RPC must NOT
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Kimix".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = kimix_shell::agent::config::ModelSwitchIncompatibleAgentError {
        code: "MODEL_SWITCH_INCOMPATIBLE_AGENT".into(),
        active_agent_type: "kimix".into(),
...
fn same_agent_type_switch_no_modal() {
    ...
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_a = acp::ModelId::new(std::sync::Arc::from("kimix-a"));
    let model_b = acp::ModelId::new(std::sync::Arc::from("kimix-b"));

    // Add both models to the catalog (no agentType → both use Kimix).
    let agent = app.agents.get_mut(&id).unwrap();
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "Kimix A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "Kimix B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = kimix_shell::extensions::session_search::SearchSessionHit {
