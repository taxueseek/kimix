...
/// Plant a Build session under the process `kimix_home()` (OnceLock-cached;
/// do not rely on setting `KIMIX_SHARE_DIR` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = kimix_shell::util::kimix_home::kimix_home();
    let encoded = kimix_shell::util::kimix_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
/// `Kimix/ask_user_question` ext-method request. Returns the args
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: kimix_tools::implementations::kimix::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    kimix_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str = kimix_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = kimix_acp_lib::AcpArgs {
