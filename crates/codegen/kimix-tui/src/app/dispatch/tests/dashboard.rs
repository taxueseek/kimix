...
/// `kimix dashboard` before login: the startup hook consumes the
/// `KIMIX_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `kimix dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    let mut app = test_app();
    seed_model(&mut app, "kimix-4.5", "Kimix 4.5");
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(kimix_shell::sampling::types::ReasoningEffort::High),
            display: "Kimix 4.5".to_string(),
        });
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(kimix_shell::sampling::types::ReasoningEffort::High)
        )),
...
/// `kimix dashboard`, and the startup hook.
#[serial_test::serial(KIMIX_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_slash_model_stages_pending_model() {
    ...
    seed_model(&mut app, "kimix-4.5", "Kimix 4.5");
    open_dashboard(&mut app);
    let effects = dispatch_dashboard_dispatch_slash(&mut app, "/model kimix-4.5".into());
    assert!(
        effects.is_empty(),
    ...
    assert_eq!(pending.id.0.as_ref(), "kimix-4.5");
    assert_eq!(pending.display, "Kimix 4.5");
...
fn dashboard_open_reseeds_pending_model_and_mode() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: acp::ModelId::new(std::sync::Arc::from("kimix-4.5")),
            effort: None,
            display: "Kimix 4.5".to_string(),
        });
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_kimix_home_for_tests(Some(home.path().to_path_buf()));
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("KIMIX_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
...
fn dashboard_peek_reply_with_image_sends_blocks() {
    ...
    let reply_text = {
        ...
        let img = crate::prompt_images::PastedImage {
            element_id: kimix_ratatui_textarea::ElementId::from_raw(0),
            display_number: 0,
            mime_type: "image/png".into(),
