...
fn plugins_list_response() -> kimix_hooks_plugins_types::PluginsListResponse {
    use crate::views::extensions_modal::test_plugin_info;
    kimix_hooks_plugins_types::PluginsListResponse {
        plugins: vec![
            test_plugin_info(
                "user-tool",
                Some(kimix_hooks_plugins_types::PluginOrigin::UserKimix),
            ),
            test_plugin_info(
                "claude-tool",
                Some(kimix_hooks_plugins_types::PluginOrigin::UserClaude),
            ),
        ],
    }
}
