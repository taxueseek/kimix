...
fn cancel_turn_leaves_shared_queue_for_agent_to_drain() {
    ...
    // mutate the queue locally — the `Kimix/queue/changed` rebroadcast is the
    // source of truth.
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    {
...
fn bg_task_killed_not_found_removes_task_from_inactive_agent() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            session_id: "sess-B".into(),
            task_id: "task-B-1".into(),
            outcome: Some(kimix_shell::extensions::task::KillOutcome::NotFound),
        }),
        &mut app,
    );
...
fn bg_task_killed_keeps_pending_kill_on_killed_outcome() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            session_id: "sess-B".into(),
            task_id: "task-B-1".into(),
            outcome: Some(kimix_shell::extensions::task::KillOutcome::Killed),
        }),
        &mut app,
    );
