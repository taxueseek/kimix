    ...
    match result {
        ...
        CommandResult::Action(Action::SetPlanMode(_)) => {
            ...
            if let Some(d) = app.dashboard.as_mut() {
                d.dispatch.set_text("");
                d.error_toast = None;
                d.pending_mode = match d.pending_mode {
                    DashboardDispatchMode::Normal => DashboardDispatchMode::Plan,
                    DashboardDispatchMode::Plan => DashboardDispatchMode::Decision,
                    DashboardDispatchMode::Decision => DashboardDispatchMode::Normal,
                    DashboardDispatchMode::AlwaysApprove => DashboardDispatchMode::Normal,
                };
            }
            vec![]
        }
...
fn stage_dashboard_model(
    ...
    effort: Option<kimix_shell::sampling::types::ReasoningEffort>,
) {
    let display = app
        .models
        .available
...
pub(super) fn apply_pending_dispatch_config(
    ...
    match pending_mode {
        ...
        DashboardDispatchMode::Decision => {
            agent.session.yolo_mode = false;
...
pub(super) fn dispatch_dashboard_permission_select(
        ...
        .filter(|_| option_id.0.as_ref() == "allow-always-mcp")
    {
        let selection = match scope.selected {
            crate::views::permission_view::McpScope::Tool => {
                kimix_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                }
            }
            crate::views::permission_view::McpScope::Server => match &scope.server_prefix {
                Some(prefix) => kimix_workspace::permission::McpScopeSelection::Server {
                    server: prefix.clone(),
                },
                None => kimix_workspace::permission::McpScopeSelection::Tool {
                    tool_name: scope.tool_name.clone(),
                },
            },
        };
        ...
        && perm.bash_selection_count > 0
    {
        ...
        serde_json::to_value(kimix_workspace::permission::BashCommandSelectedTerms {
            command_parts: parts,
        })
        .ok()
        .and_then(|v| v.as_object().cloned())
