...
pub(crate) fn dispatch_open_transcript_pager(app: &mut AppView) {
    ...
    let path = std::env::temp_dir().join(format!("kimix-transcript-{}.md", uuid::Uuid::new_v4()));
...
pub(super) fn dispatch_dump_input_log(app: &mut AppView) -> Vec<Effect> {
    ...
    let logs_dir = kimix_home.join("logs");
...
pub(super) fn handle_skills_toggle_done(
    ...
    // The toggle effect already called Kimix/skills/refresh-baseline
