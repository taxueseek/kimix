...
/// immediately, before the confirming `Kimix/queue/changed` broadcast.
...
/// that bridges the round-trip before the confirming `Kimix/queue/changed`
...
///
/// Delegates to the shared implementation in `kimix_tools::reminders`.
...
fn format_cron_prompt(prompt: &str, task_id: &str, human_schedule: &str) -> String {
    kimix_tools::reminders::format_scheduled_task_prompt(prompt, task_id, human_schedule)
}
...
pub(crate) fn apply_turn_start_shim(
    ...
    // seen ring, so a buffer-replayed `Kimix/follow_ups` for an older response
...
mod tests {
    ...
    /// `Kimix/follow_ups` for a PRIOR turn's response stays rejected (its
