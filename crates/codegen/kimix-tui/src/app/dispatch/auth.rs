...
/// Start an interactive login flow. Triggered by the welcome picker, the
/// `/login` slash command, or a 401 re-auth prompt.
///
/// - `method_id = Some(id)`: start that provider immediately (picker row).
/// - `method_id = None`: show the multi-provider picker (do **not** auto-start
///   Kimi Code — that was the Grok login footgun).
///
/// When invoked mid-session, stash the caller's view in `auth_return_view`
/// and switch to `Welcome` so the picker / device-code UI is visible.
pub(super) fn dispatch_login(
    app: &mut AppView,
    method_id: Option<acp::AuthMethodId>,
) -> Vec<Effect> {
    // Surface the auth UI when triggered from inside a session.
    if !matches!(app.active_view, ActiveView::Welcome) {
        app.auth_return_view = Some(app.active_view);
        show_welcome(app);
    }

    // `/login` with no selection → multi-provider picker.
    ...
    // Remember selection for retry / switch-account paths.
    app.login_method_id = Some(method_id.clone());
    if let Some(method) = app.auth_methods.iter().find(|m| m.id() == &method_id) {
...
pub(super) fn handle_auth_complete(
        ...
        && *current_seq == request_seq
    {
        if let Some(meta_val) = meta.as_ref()
            && let Ok(auth_meta) =
                serde_json::from_value::<kimix_shell::auth::AuthMeta>(meta_val.clone())
        {
