...
fn effective_cancel_subagents_preference(
    ...
    ui: &kimix_shell::agent::config::UiConfig,
) -> Option<bool> {
...
pub(super) fn do_cancel_turn(app: &mut AppView, cancel_subagents: bool) -> Vec<Effect> {
    ...
    // FRONT queued prompt as the next turn — its `Kimix/queue/changed`
...
/// Grace window between a driver-side `Kimix/session/prompt_complete`
...
// TODO: Add dispatch_cancel_command() once kimix-shell supports proper
