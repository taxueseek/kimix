...
/// Send a raw remember note for LLM-powered rewriting via `Kimix/memory/rewrite`.
...
/// agent is mid-turn. Fires the `Kimix/recap` ext method; the recap arrives
...
pub(super) fn dispatch_send_recap(app: &mut AppView, auto: bool) -> Vec<Effect> {
    ...
    // entirely when the feature is off so we never hit `Kimix/recap`.
            ...
            Ok(()) => {
                agent
                    ...
                    .push_block(crate::scrollback::block::RenderBlock::system(format!(
                        "Memory saved to {}",
                        crate::util::display_user_kimix_path("memory/MEMORY.md")
                    )));
            }
            Err(error) => {
                agent
