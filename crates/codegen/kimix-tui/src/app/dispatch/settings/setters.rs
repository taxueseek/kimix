...
/// `KIMIX_PROMPT_SUGGESTIONS` env var overrides the effective value.
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut kimix_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = kimix_shell::util::config::resolve_contextual_hints(
...
// ── language (commit path) ──────────────────────────────────────────

/// State + cache + persist for `language` commits.
///
/// `"auto"` means "follow system locale": runtime language re-resolves
/// via [`crate::i18n::Lang::detect`] and the config value is stored as
/// `"auto"` (skipped at read time).
pub(in crate::app::dispatch) fn set_language(app: &mut AppView, new: String) -> Vec<Effect> {
    let prev = crate::i18n::current();
    let trimmed = new.trim();
    let (canonical, resolved) = if trimmed.eq_ignore_ascii_case("auto") {
        ("auto", crate::i18n::Lang::detect())
    } else {
        match crate::i18n::Lang::parse(trimmed) {
            Some(l) => (l.code(), l),
            None => {
                tracing::error!(
                    target: "settings",
                    key = "language",
                    value = %new,
                    "Action::SetLanguage dispatched with unknown value — no-op",
                );
                return vec![];
            }
        }
    };
    crate::i18n::set_current(resolved);
    let toast = match resolved {
        crate::i18n::Lang::Zh => "✓ 界面语言：中文",
        crate::i18n::Lang::En => "✓ Language: English",
    };
    app.show_toast(toast);
    vec![Effect::PersistSetting {
        key: "language",
        value: crate::settings::SettingValue::Enum(canonical),
        rollback_value: crate::settings::SettingValue::Enum(prev.code()),
    }]
}
...
pub(in crate::app::dispatch) fn set_auto_dark_theme(app: &mut AppView, new: String) -> Vec<Effect> {
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        // No prior config: fall back to KimixNight (the default).
        .unwrap_or_else(|| crate::theme::ThemeKind::KimixNight.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
...
pub(in crate::app::dispatch) fn set_auto_light_theme(
    ...
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        .unwrap_or_else(|| crate::theme::ThemeKind::KimixDay.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
        Some(c) if c != crate::theme::ThemeKind::Auto.display_name() => c,
        _ => {
            tracing::error!(
...
pub(in crate::app::dispatch) fn set_default_model_inner(
    ...
    // the next `Kimix/models/update` roundtrip.
...
pub(in crate::app::dispatch) fn set_default_model(
    ...
    // so persisting the human-readable name (e.g. "Kimix")
    // would silently fail to resolve on the next startup.
    //
    // Chat (`--chat` / KIMIX_CHAT_MODE) catalogs use opaque `/rest/modes`
    // slugs that must not become the global Build `default_model`.
    let mut effects: Vec<Effect> = Vec::new();
    if !kimix_shell::agent::chat_modes::process_chat_mode_enabled() {
...
/// (`kimix_shell::models::default_model()`).
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = kimix_shell::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();
