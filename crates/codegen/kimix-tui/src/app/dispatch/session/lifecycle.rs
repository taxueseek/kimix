...
/// the workspace (writes `~/.kimix/trusted_folders.toml`), mark trust resolved,
...
pub(in crate::app::dispatch) fn dispatch_trust_folder(app: &mut AppView) -> Vec<Effect> {
    if let TrustState::Pending { workspace } = &app.trust_state {
        kimix_workspace::folder_trust::grant_folder_trust(workspace);
    }
    finish_trust(app)
}
...
/// (`--resume` / `--worktree` / initial-prompt / `kimix dashboard`). Extracted
