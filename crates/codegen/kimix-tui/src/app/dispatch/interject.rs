...
//! `Kimix/interject` effect, and prompt-history recording. Split out of
...
/// prompt, and fires the `Kimix/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `Kimix/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`Kimix/session/interjection`) is
