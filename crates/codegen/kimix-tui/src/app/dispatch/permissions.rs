...
///
/// Special case for [`kimix_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID`]:
...
/// "always-approve"` to `~/.kimix/config.toml`, and fire the
/// `Kimix/yolo_mode_changed` ACP notification. See the option-id constant
...
pub(super) fn dispatch_permission_select(
    ...
    let enable_always_approve =
        option_id.0.as_ref() == kimix_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
    ...
    let steers_next_cursor = !enable_always_approve
        && option_id.0.as_ref() != kimix_workspace::permission::ALLOW_EDITS_SESSION_OPTION_ID;
