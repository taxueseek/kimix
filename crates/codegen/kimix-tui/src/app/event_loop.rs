...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `Kimix/runningPromptId` from the reload response: the turn another
...
/// [`decide`](kimix_workspace::folder_trust::decide) precedence.
///
...
fn seed_trust_state(app: &mut AppView, remote: Option<&kimix_shell::util::config::RemoteSettings>) {
    use kimix_workspace::folder_trust::{
        TrustOutcome, decide, decide_inputs_with_interactive, feature_enabled,
    };
    use kimix_workspace::trust::workspace_key;
...
pub(crate) async fn run(
    ...
    remote_settings: Option<kimix_shell::util::config::RemoteSettings>,
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("KIMIX_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = kimix_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(kimix_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = kimix_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match kimix_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.show_resolved_model = remote_settings
        ...
        .unwrap_or(true);
    app.session_picker_grouped = std::env::var("KIMIX_SESSION_PICKER_GROUPED")
        .ok()
        ...
        .or_else(|| {
            kimix_shell::config::load_effective_config()
                .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            let kimix_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "kimi-code");
            if let Some(method) = kimix_com {
    ...
    let post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            dispatch::dispatch(Action::Login { method_id: None }, &mut app)
        }
    } else {
        vec![]
    };
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<kimix_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app
            .auth_methods
            .iter()
            .any(|m| m.id().0.as_ref() == kimix_shell::agent::auth_method::XAI_API_KEY_METHOD_ID);
    ...
    let requirements = kimix_shell::config::load_merged_requirements();
    let user_config = kimix_shell::config::load_from_disk().ok();
    let managed_config = kimix_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match kimix_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = kimix_shell::agent::config::resolve_compat_sessions_from_raw(
        effective_config.as_ref().ok_or(()),
        remote_settings.as_ref(),
    );
    app.foreign_session_compat = kimix_workspace::foreign_sessions::EnabledForeignSessionSources {
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        kimix_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        kimix_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        kimix_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use kimix_shell::util::config::resolve_tips;
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = kimix_shell::util::tips::pick_and_advance(&app.tips, &kimix_home);
        }
    }

    let hints = kimix_shell::util::config::resolve_hints(
    ...
    // CLI command instead of a modal. Disappears once `kimix import-kimi` has
    // run (marker file under ~/.kimix); the scan is read-only over ~/.kimi.
    if kimix_shell::kimi_import::has_pending_kimi_import() {
        app.startup_warnings.push(crate::startup::StartupWarning {
            severity: crate::startup::WarningSeverity::Info,
            message: i18n::tr("Found official kimi-cli settings in ~/.kimi.").to_string(),
            action: Some(
                i18n::tr("Run `kimix import-kimi` to import them (one-time).").to_string(),
            ),
        });
    }

    // Apply initial config (may come from existing ~/.kimix/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        kimix_shell::util::config::clamped_display_permission_mode(
            kimix_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        kimix_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = kimix_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = kimix_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // KIMIX_SCROLL_SPEED/_MODE/_LINES + KIMIX_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    // Initial prompt from the CLI positional (`kimix "fix the bug"`). When
    ...
    // `kimix dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `KIMIX_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("KIMIX_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.kimix/config.toml [ui]`
                // rather than `~/.kimix/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = kimix_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> kimix_shell::agent::config::UiConfig {
    use kimix_shell::agent::config::UiConfig;
    let Ok(root) = kimix_shell::config::load_effective_config() else {
