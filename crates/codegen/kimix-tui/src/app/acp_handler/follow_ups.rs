...
/// Deserialize shape of the `Kimix/follow_ups` params emitted by the shell
...
pub(super) struct FollowUpsParams {
    ...
    /// change. Parsed loosely as a JSON value to read the `"kimix/replayed"`
    /// key (a slash-bearing key prost cannot model as a field).
    #[serde(default, rename = "_meta")]
    meta: Option<serde_json::Value>,
}

/// A single `Kimix/follow_ups` suggestion. Only the human-facing `label` is
...
/// Handle `Kimix/follow_ups` — render follow-up suggestion chips for the
/// latest assistant response.
///
/// Newest-response-wins keying lives in [`AgentView::apply_follow_ups`]. The
/// reserved `_meta["kimix/replayed"] == true` marker suppresses rendering (it
...
pub(super) fn handle_follow_ups(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if params
        .meta
        .as_ref()
        .and_then(|m| m.get("kimix/replayed"))
