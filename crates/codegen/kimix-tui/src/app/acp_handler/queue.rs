...
/// Wire payload of `Kimix/session/prompt_complete`, emitted by
...
pub(super) fn handle_queue_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(changed) =
        ...
        tracing::warn!("Failed to parse kimix/queue/changed");
        return false;
    };
    ...
    tracing::debug!(
        ...
        "received kimix/queue/changed broadcast",
    );
...
pub(super) fn handle_prompt_complete(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(payload) = serde_json::from_str::<PromptCompletePayload>(notif.params.get()) else {
        tracing::warn!("Failed to parse kimix/session/prompt_complete");
        return false;
    };
