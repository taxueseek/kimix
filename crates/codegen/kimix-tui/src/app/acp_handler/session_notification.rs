use super::*;
use kimix_shell::sampling::error::rate_limited_user_message;
...
/// Handle `Kimix/session_notification` and replay-path `Kimix/session/update`.
///
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: kimix/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
        ...
        })
    {
        tracing::debug!(
            ...
            "kimix/session update DROPPED by dedup highwater (event_seq <= last_applied)"
        );
        return false;
    }
    ...
    let changed = match session_notif.update {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        kimix_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        kimix_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        kimix_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
        ...
        XaiSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(kimix_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        XaiSessionUpdate::PluginsChanged { plugins } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.plugins_data =
                    TabDataState::Loaded(kimix_hooks_plugins_types::PluginsListResponse {
                        plugins,
                    });
        ...
        | XaiSessionUpdate::RetryState(_) => {
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct = Some(kimix_token_estimation::usage_percentage_u8(
                        tokens_after,
                        cw,
                    ));
                }
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[kimix_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &kimix_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    ...
    let was_plan = agent.plan_mode_active;
    let now_plan = mode.is_plan();
    let now_decision = mode.is_decision();
    agent.plan_mode_active = now_plan;
    agent.plan_mode_pending = None;
    agent.decision_mode_active = now_decision;
    if was_plan != now_plan {
        tracing::info!(
            mode_id = % cmu.current_mode_id.0, plan_active = now_plan,
    ...
    if now_decision {
        tracing::info!(
            ...
            "Decision mode confirmed (from CurrentModeUpdate)"
        );
    }
    true
}
