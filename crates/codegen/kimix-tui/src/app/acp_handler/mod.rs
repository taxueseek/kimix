...
//! and xAI session extension notifications (`Kimix/session_notification` and
//! replay-path `Kimix/session/update`).
use std::collections::hash_map::Entry;
use std::path::PathBuf;
use std::sync::Arc;

use agent_client_protocol as acp;
use kimix_acp_lib::AcpClientMessage;

use super::actions::Effect;
use kimix_shell::extensions::notification::{
    SessionNotification, SessionUpdate as XaiSessionUpdate, is_reauthable_failure,
};
use kimix_shell::tools::todo::todo_item_from_plan_entry;
use kimix_workspace::permission::bash_command_splitting::BashCommandHighlights;
...
/// - `Kimix/session_notification` / `Kimix/session/update` → per-agent session updates
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "kimix/follow_ups" => handle_follow_ups(notif, app),
        "kimix/task_backgrounded" => handle_task_backgrounded(notif, app),
        "kimix/task_completed" => handle_task_completed(notif, app),
        "kimix/models/update" => handle_models_update(notif, app),
        "kimix/settings/update" => handle_settings_update(notif, app),
        "kimix/sessions/changed" => handle_sessions_changed(notif, app),
        "kimix/queue/changed" => handle_queue_changed(notif, app),
        ...
        "kimix/session/prompt_complete" => handle_prompt_complete(notif, app),
        "kimix/session/interjection" => handle_interjection(notif, app),
        "kimix/monitor_event" => handle_monitor_event(notif, app),
        "kimix/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "kimix/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "kimix/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "kimix/scheduled_task_inject_prompt" => handle_scheduled_task_inject_prompt(notif, app),
        "kimix/git_head_changed" => handle_git_head_changed(notif, app),
        "kimix/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "kimix/mcp/tools_changed" | "kimix/mcp_initialized" => handle_mcp_tools_changed(notif, app),
        "kimix/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "kimix/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}

/// Handle `Kimix/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse kimix/session/interjection");
        return false;
    };
    ...
    // emits the queue-emptying `Kimix/queue/changed` right after it).
...
fn handle_ext_method(ext: kimix_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "kimix/ask_user_question" => handle_ask_user_question(ext, app),
        "kimix/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        unknown => {
