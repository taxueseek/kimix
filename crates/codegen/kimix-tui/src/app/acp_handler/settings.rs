...
/// Handle `Kimix/models/update` — model list changed (etag-triggered refresh).
pub(super) fn handle_models_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    if let Ok(model_state) = serde_json::from_str::<acp::SessionModelState>(notif.params.get()) {
        ...
        tracing::info!(
            ...
            "models updated via kimix/models/update"
        );
        ...
        tracing::warn!("Failed to parse kimix/models/update");
        false
    }
}

/// Handle `Kimix/settings/update` — remote settings refreshed on `/new`.
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(update) = serde_json::from_str::<PagerSettingsUpdate>(notif.params.get()) else {
        tracing::warn!("Failed to parse kimix/settings/update");
        return false;
    };
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        kimix_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = kimix_shell::util::config::auto_permission_mode_enabled_from_disk();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("KIMIX_SESSION_PICKER_GROUPED")
            .ok()
    ...
    let (requirements, user_config, managed_config) = (
        kimix_shell::config::load_merged_requirements(),
        kimix_shell::config::load_from_disk().ok(),
        kimix_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = kimix_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = kimix_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = kimix_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    tracing::info!("settings updated via kimix/settings/update");
    true
}
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = kimix_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        kimix_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}

/// Tell live sessions to leave Auto on the mid-session kill-switch: fire the
/// `Kimix/yolo_mode_changed` notification the agent maps to
...
pub(super) fn notify_sessions_leave_auto(app: &AppView, session_ids: &[acp::SessionId]) {
    ...
    let args = kimix_acp_lib::AcpArgs {
...
/// Handle `Kimix/sessions/changed` — the leader broadcasts roster
...
pub(super) fn handle_sessions_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(changed) = serde_json::from_str::<crate::app::roster::RosterChanged>(notif.params.get())
        ...
        tracing::warn!("Failed to parse kimix/sessions/changed");
        return false;
    };
...
/// Deserialization type for the `Kimix/settings/update` notification payload.
///
/// This is intentionally a separate struct from `SettingsUpdateNotification` in
/// `kimix-shell/src/agent/mvp_agent.rs`. The shell side derives `Serialize`
...
/// `kimix-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
