...
pub(crate) fn is_server_initiated_prompt(prompt_id: &str) -> bool {
    kimix_shell::session::PromptOrigin::from_prompt_id(prompt_id).is_synthetic()
}
...
/// `Kimix/session/prompt_complete` turn-end signal. A viewer can thus safely
...
pub(crate) fn is_scheduler_fired_prompt(prompt_id: &str) -> bool {
    matches!(
        kimix_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        kimix_shell::session::PromptOrigin::SchedulerFired
    )
}
...
pub(crate) fn is_wake_prompt(prompt_id: &str) -> bool {
    matches!(
        kimix_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        kimix_shell::session::PromptOrigin::TaskCompleted { .. }
            | kimix_shell::session::PromptOrigin::SubagentCompleted { .. }
            | kimix_shell::session::PromptOrigin::NotificationDrain
    )
}
...
/// adoptable iff the turn emits a terminal `Kimix/session/prompt_complete`, the
