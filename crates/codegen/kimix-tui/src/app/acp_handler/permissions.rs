...
pub(super) fn handle_permission_request(
    perm: kimix_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    app: &mut AppView,
) -> bool {
...
fn enqueue_permission(
    ...
    // 1. Parse bash highlights from request meta (imported from kimix-shell).
    ...
    let bash_selection_count = bash_highlights
        .as_ref()
        .map(|h| kimix_workspace::permission::default_always_allow_scope(&h.highlighted_words))
        .unwrap_or(0);
    ...
    let mcp_scope = perm
        ...
        .and_then(|m| {
            serde_json::from_value::<kimix_workspace::permission::McpToolPermission>(
...
/// `kimix-tools` for typed access to `command` and `description`.
...
fn build_permission_display(
    ...
    let bash_input = req.tool_call.fields.raw_input.as_ref().and_then(|v| {
        serde_json::from_value::<kimix_tools::implementations::BashToolInput>(v.clone()).ok()
    });
    ...
    let title = if is_execute {
        ...
        if let Some(path) = file_path {
            ...
            format!(
                "Allow {}?",
                kimix_workspace::permission::mcp_pretty_name_if_qualified(t)
            )
        } else {
...
fn cancel_permission(perm: kimix_acp_lib::AcpArgs<acp::RequestPermissionRequest>) {
    perm.response_tx
        .send(Ok(acp::RequestPermissionResponse::new(
