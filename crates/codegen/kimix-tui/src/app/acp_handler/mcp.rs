...
/// `KIMIX_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        kimix_shell::util::config::resolve_mcp_push_server_status(
...
/// Handle `Kimix/mcp/tools_changed` and `Kimix/mcp_initialized`.
///
/// Routing rules (verified against the four shell emit sites in
/// `kimix-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`kimix_shell::extensions::mcp::McpToolsChanged`] struct), and
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    // Both `Kimix/mcp_initialized` and (newer shell)
    // `Kimix/mcp/tools_changed` carry `sessionId`. Route by it so a
    ...
    if method == "kimix/mcp_initialized"
...
/// Handle `Kimix/mcp/server_status`.
///
...
/// ([`kimix_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`kimix_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use kimix_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "kimix/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
...
/// Handle `Kimix/mcp/servers_updated`.
///
/// Emitted by the shell from `MvpAgent` on managed-config resolve and
/// on config reload (`crates/codegen/kimix-shell/src/agent/mvp_agent.rs`
