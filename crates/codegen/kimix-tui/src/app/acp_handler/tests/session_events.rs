#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn apply_retry_state_auth_failure_pushes_reauth_prompt() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) from https://cli-chat-proxy.kimix.com/v1/messages: \
                          no auth context"
                    .into(),
            },
            &mut session,
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `kimix logout` /
    /// `kimix login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `kimix logout` then `kimix login`"
                    .into(),
            },
            &mut session,
            &mut scrollback,
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(kimix_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
