#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// `Kimix/queue/changed` broadcast, and an empty broadcast clears it.
    #[test]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        // Shell and pager share the kimix-prompt-queue type, so serializing the payload as the
        // shell emits it pins the wire shape the handler consumes, not cross-crate compat.
        let shell_payload = kimix_shell::session::prompt_queue::QueueChanged {
        ...
        let notif = acp::ExtNotification::new("kimix/queue/changed", raw.into());
    ...
    /// broadcasts `Kimix/queue/changed` with `runningPromptId` = that synthetic
    ...
    fn adopted_running_prompt_lets_subsequent_chunk_through_gate() {
        ...
        let msg = AcpClientMessage::SessionNotification(kimix_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        handle(msg, &mut app);
    ...
    fn bash_kind_round_trips_and_adoption_sets_bash_turn() {
        // Shell and pager share the kimix-prompt-queue type; pin kind through a serde cycle.
        let shell = kimix_shell::session::prompt_queue::QueueChanged {
            session_id: "sess-1".to_string(),
            entries: vec![kimix_shell::session::prompt_queue::QueueEntryWire {
                id: "b1".to_string(),
                version: 0,
                owner: None,
                last_editor: None,
    ...
    fn viewer_does_not_enter_turn_running_for_server_initiated_turn() {
        ...
        // the actor and emits NO `Kimix/session/prompt_complete`. If a viewer
    ...
    fn viewer_enters_turn_running_for_scheduler_fired_cron_turn() {
        ...
        // `MvpAgent::prompt()` and DOES emit `Kimix/session/prompt_complete`. So a
    ...
    fn viewer_prompt_complete_finishes_turn() {
        // A viewer in TurnRunning receives Kimix/session/prompt_complete for its
