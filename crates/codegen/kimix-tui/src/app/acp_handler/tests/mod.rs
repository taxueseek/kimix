...
use kimix_shell::extensions::notification::RetryState;
use kimix_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> kimix_shell::extensions::notification::ImageCompressedEntry {
    kimix_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_exit_plan_ext(
    ...
    kimix_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<kimix_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn make_exit_plan_ext_with_tool_call_id(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    let request = acp::ExtRequest::new("kimix/exit_plan_mode", raw.into());
    let (tx, rx) = tokio::sync::oneshot::channel();
    (
        kimix_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        },
        rx,
...
/// Build a `Kimix/queue/changed` notification carrying `runningPromptId`.
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(kimix_acp_lib::AcpArgs {
...
pub(super) fn make_inject_notif(payload: &serde_json::Value) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "kimix/scheduled_task_inject_prompt",
...
pub(super) fn make_fired_notif(
    ...
    acp::ExtNotification::new("kimix/scheduled_task_fired", std::sync::Arc::from(raw))
}
...
pub(super) fn make_created_ext_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("kimix/scheduled_task_created", std::sync::Arc::from(raw))
}
pub(super) fn make_deleted_ext_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("kimix/scheduled_task_deleted", std::sync::Arc::from(raw))
}
...
/// Build an `Kimix/session/prompt_complete` ext-notification for `session_id`.
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("kimix/session/prompt_complete", std::sync::Arc::from(raw))
}
...
/// Build an `Kimix/session/prompt_complete` ext-notification with an explicit
...
/// Build an `Kimix/session/prompt_complete` ext-notification carrying a
...
/// Build a durable `TurnCompleted` update on the `Kimix/session/update` rail,
...
/// `Kimix/session/update` rail, optionally stamped `isReplay`.
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use kimix_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
/// Build an `Kimix/session/interjection` ext-notification (no id).
pub(super) fn interjection_ext(session_id: &str, text: &str) -> acp::ExtNotification {
    interjection_ext_with_id(session_id, text, None)
}
/// Build an `Kimix/session/interjection` ext-notification with an optional
...
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("kimix/session/interjection", std::sync::Arc::from(raw))
}
...
/// Build an `ExtNotification` envelope for `Kimix/session_notification`.
...
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(kimix_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
/// Shared temp `KIMIX_SHARE_DIR` for disk-replay tests. `kimix_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `KIMIX_SHARE_DIR`
...
/// Runs `f` with a thread-local kimix home override so disk replay tests do not
/// depend on process-wide `kimix_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_kimix_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_kimix_home_for_tests(None);
    out
}
...
pub(super) fn dispatch_goal_update(
    ...
    handle(
        AcpClientMessage::ExtNotification(kimix_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("kimix/session_notification", raw.into()),
            response_tx: tx,
        }),
        app,
    )
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(kimix_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
}
/// Build an `Kimix/session_notification` carrying
...
pub(super) fn interaction_resolved_ext(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("kimix/session_notification", std::sync::Arc::from(raw))
}
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = kimix_workspace::session::git::GitHeadChanged {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("kimix/git_head_changed", std::sync::Arc::from(raw))
}
pub(super) fn make_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("kimix/task_backgrounded", std::sync::Arc::from(raw))
}
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("kimix/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use kimix_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("kimix/task_completed", std::sync::Arc::from(raw))
}
pub(super) fn make_monitor_event_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("kimix/monitor_event", std::sync::Arc::from(raw))
}
...
pub(super) fn make_models_update_notif(
    ...
    acp::ExtNotification::new("kimix/models/update", std::sync::Arc::from(raw))
}
/// `Kimix/models/update` carrying a single reasoning-capable model whose
...
/// Helper: build an `Kimix/mcp/init_progress` notification.
pub(super) fn make_mcp_init_progress_notif(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("kimix/mcp/init_progress", std::sync::Arc::from(raw))
}
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: kimix_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use kimix_shell::extensions::mcp::{
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("kimix/mcp/server_status", std::sync::Arc::from(raw))
}
...
pub(super) fn make_servers_updated_notif() -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("kimix/mcp/servers_updated", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = kimix_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "kimix_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("kimix/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "kimix_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("kimix/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_mcp_initialized_notif(session_id: &str) -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("kimix/mcp_initialized", std::sync::Arc::from(raw))
}
