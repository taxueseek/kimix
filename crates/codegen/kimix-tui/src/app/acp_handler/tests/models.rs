#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: a machine-wide `Kimix/models/update` broadcast
    ...
    fn models_update_preserves_active_agent_model() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_3 = acp::ModelId::new(std::sync::Arc::from("kimix-3"));
        agent
            .session
            .models
            .available
            .insert(id_3.clone(), make_model_info("kimix-3"));
        agent.session.models.current = Some(id_3);

        let notif = make_models_update_notif("kimix-4", &["kimix-3", "kimix-4"]);
    ...
    fn models_update_uses_shell_default_when_agent_model_removed() {
        ...
        // kimix-3 removed from catalog.
        let notif = make_models_update_notif("kimix-4.3", &["kimix-4.3", "kimix-4.5"]);
        handle_models_update(&notif, &mut app);
    ...
    fn models_update_noop_when_agent_matches_shell_default() {
        ...
        let agent = app.agents.get_mut(&AgentId(0)).unwrap();
        let id_4 = acp::ModelId::new(std::sync::Arc::from("kimix-4"));
        agent
            .session
            .models
            .available
            .insert(id_4.clone(), make_model_info("kimix-4"));
        ...
        assert_eq!(
            app.models.current.as_ref().map(|id| id.0.as_ref()),
            Some("kimix-4"),
            "app.models.current must be kimix-4 when agent and shell agree"
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
            agent
                ...
                .map(|id| id.0.as_ref()),
            Some("kimix-4"),
            "agent model must remain kimix-4"
        );
    }
    ...
    fn models_update_non_active_agent_uses_shell_fallback_not_active_model() {
...

        {
            ...
            let id_5 = acp::ModelId::new(std::sync::Arc::from("kimix-4.5"));
            agent_b
                .session
                .models
                .available
                .insert(id_5.clone(), make_model_info("kimix-4.5"));
            agent_b.session.models.current = Some(id_5);
        }

        // kimix-5 removed from catalog.
        let notif = make_models_update_notif("kimix-4", &["kimix-3", "kimix-4"]);
        handle_models_update(&notif, &mut app);
        ...
        // B's kimix-5 was removed — must fall back to shell's kimix-4, not A's kimix-3.
    ...
    fn model_changed_dropped_when_model_unknown_to_catalog() {
        ...
        let notif = model_changed_ext("sess-1", "kimix-99-unknown", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(
            !changed,
    ...
    fn model_changed_applies_reasoning_effort_on_follower() {
        ...
        let notif = model_changed_ext("sess-1", "kimix-4", Some("high"));
    ...
    fn model_changed_dropped_for_unknown_session_id() {
        ...
        let notif = model_changed_ext("sess-OTHER", "kimix-4", None);
        let changed = handle_ext_notification(&notif, &mut app);
        assert!(!changed);
