#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: replay from `updates.jsonl` emits `Kimix/session/update` (not
    ...
    fn ext_session_notification_and_update_equivalent_for_subagent_spawned() {
        ...
        let (spawn_notif, finish_notif) =
            run_subagent_lifecycle_via_method("kimix/session_notification", child_sid);
        let (spawn_update, finish_update) =
            run_subagent_lifecycle_via_method("kimix/session/update", child_sid);
    ...
    fn ext_session_update_malformed_params_returns_false() {
        ...
        let request = acp::ExtNotification::new("kimix/session/update", raw.into());
        let msg = AcpClientMessage::ExtNotification(kimix_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        ...
        assert!(
            !affected,
            "malformed kimix/session/update params must not redraw"
        );
        assert!(
