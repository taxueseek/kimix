...
/// Handle `Kimix/task_backgrounded` — a bash command transitioned to background.
///
...
pub(super) fn handle_task_backgrounded(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse kimix/task_backgrounded");
        return false;
    };
...
/// Handle `Kimix/monitor_event` — background task or monitor emitted new output.
...
pub(super) fn handle_scheduled_task_inject_prompt(
    ...
    let payload: serde_json::Value = match serde_json::from_str(notif.params.get()) {
        Ok(v) => v,
        Err(e) => {
            tracing::warn!(error = %e, "Failed to parse kimix/scheduled_task_inject_prompt");
            return false;
        }
    };
    let Some(session_id) = payload["sessionId"].as_str() else {
        tracing::warn!("kimix/scheduled_task_inject_prompt: missing or non-string sessionId");
        return false;
    };
    let Some(prompt) = payload["prompt"].as_str().filter(|s| !s.is_empty()) else {
        tracing::warn!("kimix/scheduled_task_inject_prompt: missing or empty prompt");
        return false;
    };
    ...
    // `Kimix/scheduled_task_inject_prompt` notification is routed by the leader
...
pub(super) fn handle_git_head_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(params) =
        serde_json::from_str::<kimix_workspace::session::git::GitHeadChanged>(notif.params.get())
    else {
        return false;
    };
...
pub(super) fn handle_task_completed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse kimix/task_completed");
        return false;
    };
