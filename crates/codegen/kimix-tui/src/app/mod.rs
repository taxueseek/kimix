...
pub use kimix_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `KIMIX_MOUSE_REPORTING_TOGGLE`). Seeded once
...
///
/// Uses [`kimix_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `kimix-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`kimix-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
/// should print a message telling the user to relaunch `kimix`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
    ...
    let kimi_code_config = match kimix_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
    {
        ...
        Err(e) => {
            ...
            kimix_shell::auth::KimiCodeConfig::default()
        }
    };
    let refreshed_auth = kimix_shell::auth::try_ensure_fresh_auth(&kimi_code_config).await;
    // Fire-and-forget model-catalog warmup; nothing joins the handle now that
    // the xAI settings fetch it used to carry is gone.
    drop(kimix_shell::agent::models::start_early_prefetch_with_auth(
        refreshed_auth,
    ));
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(kimix_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = kimix_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("KIMIX_HUNK_TRACKER").ok();
    ...
    let launch_auto = kimix_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    // Join the in-process agent worker (and flush session actors) on every
    // exit path — including `?` bail after this point. Leader mode hands
    // `None` for the thread; the guard is then cancel-only.
    ...
    // Drop before kill_all so session actors finish SessionEnd / memory save
    // while their child processes still exist if hooks need them.
    drop(agent_guard);
    kimix_tty_utils::global_process_scope().kill_all();
    match result {
        Ok(run_result) => {
...
fn print_exit_resume_hint(session_id: &str, minimal: bool, w: &mut impl Write) {
    ...
    if minimal {
        let _ = writeln!(w, "  kimix --minimal --resume {session_id}");
    } else {
        let _ = writeln!(w, "  kimix --resume {session_id}");
    }
}
...
fn disable_mouse_paste_raw() {
    kimix_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(kimix_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        if mode.is_fullscreen() {
            kimix_shell::util::with_locked_stderr(|stderr| execute!(stderr, EnterAlternateScreen))?;
        }
        #[cfg(windows)]
        if want_minimal {
        ...
        kimix_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(kimix_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                kimix_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = kimix_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let backend =
                CrosstermBackend::new(crate::render::draw::TermWriter::new(frame_tx, writer_sync));
            let term = kimix_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - kimix"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "kimix");
        assert_eq!(terminal_title_string(""), "kimix");
        assert_eq!(terminal_title_string("My chat"), "My chat - kimix");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["kimix-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["kimix-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["kimix-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["kimix-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["kimix-pager", "--leader", "agent", "stdio"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["kimix-pager", "--no-leader", "agent", "stdio"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    ...
    fn cli_bare_agent_subcommand_defaults_to_stdio() {
        let args = try_parse_pager(&["kimix-pager", "agent"]).unwrap();
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["kimix-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["kimix-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["kimix-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["kimix-pager", "--chat"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["kimix-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["kimix-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["kimix-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["kimix-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["kimix-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["kimix-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["kimix-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["kimix-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["kimix-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["kimix-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args = try_parse_pager(&["kimix-pager", "-s", "a", "--resume", "b", "--fork-session"])
            .unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["kimix-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_kimix() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "kimix");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            first_5,
            vec![
                "kimix — standalone CLI agent",
                "",
                "Usage: kimix [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["kimix-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["kimix-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
