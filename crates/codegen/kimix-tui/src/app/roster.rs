...
//!
//! - Request/response `Kimix/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `Kimix/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `Kimix/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `Kimix/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `Kimix/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `kimix-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `Kimix/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `Kimix/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> kimix_shell::agent::roster::RosterEntry {
        use kimix_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `Kimix/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("kimix-4"));
        assert!(e.yolo);
    ...
    /// `Kimix/sessions/changed` round-trip: serialize the agent's `RosterChanged`
