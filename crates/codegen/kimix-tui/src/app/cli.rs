...
pub enum Command {
    ...
    Acp(Box<AgentArgs>),
    /// Run Kimix without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Kimix discovers for this directory
    Inspect {
    ...
    Logout,
    /// Sign in (Kimi Code by default; pass --xai for native xAI OIDC)
    Login {
        /// Native xAI device-code login; stores tokens in ~/.kimix/auth.json
        #[arg(long)]
        xai: bool,
    },
...
Examples:
  kimix wrap docker exec -it my-container bash
  kimix wrap kubectl exec -it my-pod -- bash

See ~/.kimix/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `~/.kimix/config.toml` or when the `KIMIX_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `kimix leader`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `kimix leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
pub struct AgentArgs {
    ...
    /// Agent runtime mode. Optional: bare `kimix agent` (and the `kimix acp`
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "kimix: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("kimix: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "KIMIX_AGENT_SECRET")]
...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = kimix_update::channel_label();
        format!(
            "{} — kimix CLI agent",
            kimix_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label),
        )
    })
}
...
#[command(
    ...
    about = "kimix — standalone CLI agent with multi-model support",
...
pub struct PagerArgs {
    ...
    /// Use a custom leader socket path instead of the default `~/.kimix/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `KIMIX_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            kimix_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `kimix -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "KIMIX_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only — does not write config. To default plain `kimix` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.kimix/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.kimix/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "KIMIX_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `kimix "fix the bug"` or `kimix --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`KIMIX_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                kimix_shell::session::persistence::resumed_session_sandbox_profile(Some(&id), cwd)
            }
            ResumeTarget::MostRecentForCwd => {
                kimix_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<kimix_sandbox::ProfileName>().ok()
                    != s.parse::<kimix_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `kimix "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["kimix", "--version"]).unwrap_err();
    ...
    fn version_short_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["kimix", "-v"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::DisplayVersion);
        assert!(
            err.exit_code() == 0,
            "-v must exit 0; got {}",
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["kimix"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["kimix", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["kimix", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["kimix", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["kimix", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["kimix", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["kimix", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["kimix", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["kimix", "--minimal", "--fullscreen"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);
    }
    /// kimi-cli parity (F6): bare `kimix acp` runs the stdio ACP server, and
    /// bare `kimix agent` defaults to stdio at dispatch (mode is optional).
    #[test]
    fn acp_alias_and_bare_agent_default_to_stdio() {
        let args = PagerArgs::try_parse_from(["kimix", "acp"]).unwrap();
        ...
        let args = PagerArgs::try_parse_from(["kimix", "agent"]).unwrap();
        ...
        let args = PagerArgs::try_parse_from(["kimix", "agent", "stdio"]).unwrap();
        let Some(Command::Agent(agent)) = args.command else {
            panic!("expected agent subcommand");
        };
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["kimix", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["kimix", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["kimix", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["kimix"]).expect("bare kimix parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["kimix", "leader", "list", "--json"])
            .expect("kimix leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["kimix", "leader", "info", "--pid", "42"])
            .expect("kimix leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill =
            PagerArgs::try_parse_from(["kimix", "leader", "kill"]).expect("kimix leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["kimix", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["kimix", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["kimix", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args =
            PagerArgs::try_parse_from(["kimix", "fix the bug"]).expect("positional prompt parses");
        assert_eq!(args.initial_prompt(), Some("fix the bug"));
        assert!(args.command.is_none());
        assert!(args.single.is_none());
    }
    #[test]
    fn bare_kimix_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["kimix"]).expect("bare kimix parses");
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["kimix", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["kimix"]).expect("bare kimix parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["kimix", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["kimix", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["kimix", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias = PagerArgs::try_parse_from(["kimix", "--effort", "high"])
    ...
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["kimix", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep = PagerArgs::try_parse_from(["kimix", "--reasoning-effort", "deep"])
            .expect("deep parses");
    ...
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["kimix", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["kimix", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn import_kimi_parses_with_and_without_dry_run() {
        let args = PagerArgs::try_parse_from(["kimix", "import-kimi", "--dry-run"])
        ...
        let args =
            PagerArgs::try_parse_from(["kimix", "import-kimi"]).expect("bare import-kimi parses");
        assert!(matches!(
            args.command,
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["kimix", "agent", "--effort", "max", "stdio"])
