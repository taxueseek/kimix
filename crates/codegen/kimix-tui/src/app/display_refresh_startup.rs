...
/// Inclusive bounds for motion cadence env knobs (`KIMIX_MIN_DRAW_MS`,
/// `KIMIX_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("KIMIX_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("KIMIX_SCROLL_CADENCE_MS", default_cadence_ms);
