...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Kimix session whose first prompt resumes a foreign tool session.
    ForeignResume {
...
/// Parse `newSessionId` from an `Kimix/session/fork` ACP response body.
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &kimix_shell::agent::config::Config,
) -> std::sync::Arc<kimix_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(kimix_shell::auth::AuthManager::new(
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["kimix"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["kimix", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["kimix", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["kimix", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["kimix", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["kimix", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["kimix", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["kimix"])).chat_mode);
    }
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = kimix_shell::util::kimix_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = kimix_shell::util::kimix_home::kimix_home()
            .join("sessions")
            .join(&encoded);
