...
//! inside [`kimix_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `kimix-tui __mermaid-render` (see
...
//!    via [`kimix_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`kimix_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `kimix-mermaid`). The parent also rejects obviously-oversized
...
pub fn render_via_subprocess(
    ...
    cmd.arg(MERMAID_RENDER_SUBCOMMAND)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .envs(kimix_tty_utils::pager_env());
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, kimix_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `KIMIX_SHARE_DIR` (whose `kimix_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.kimix`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_KIMIX_SHARE_DIR` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `KIMIX_SHARE_DIR` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    fn is_render_subcommand_matches_only_argv1() {
        ...
        assert!(!is_render_subcommand(&argv(&["kimix"])));
        assert!(!is_render_subcommand(&argv(&["kimix", "chat"])));
        assert!(!is_render_subcommand(&argv(&[])));
    ...
    fn per_theme_renders_land_in_distinct_files() {
        ...
        let light_key =
            MermaidCacheKey::derive(src, ThemeKind::KimixDay, 80, MermaidRenderQuality::Terminal);
        assert_ne!(
    ...
    /// hermetic, with no process-global `KIMIX_SHARE_DIR` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `kimix_home()` cache race.
    ...
    fn agent_with_session(name: &str) -> AgentView {
        ...
        let cwd = PathBuf::from("/kimix-mermaid-test").join(name);
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let click_key =
            MermaidCacheKey::derive(src, ThemeKind::KimixNight, 80, MermaidRenderQuality::Open);
        let mut rt = MermaidRuntime::new();
        rt.pending.push(PendingMermaidAction {
            key: click_key.clone(),
            action: MermaidClickAction::Open,
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::KimixDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
