...
impl AgentView {
    ...
    pub fn draw(
        ...
        link_spans_out: &mut Vec<kimix_ratatui_inline::LinkSpan>,
    ) -> (
        Option<(u16, u16)>,
        Option<crate::terminal::overlay::PostFlush>,
    ) {
        ...
        if self.should_show_plan_chip(&appearance) {
            ...
            status.push(
                "plan",
                Line::from(Span::styled(i18n::tr("Plan"), plan_style)),
            );
        }
        ...
        if show_worktree_label {
            ...
            let wt_label = i18n::tr("worktree ");
            path_offset += wt_label.width() as u16;
            parts.push(Span::styled(wt_label, label_style));
        }
        if let Some(profile) = kimix_sandbox::profile_name() {
        ...
        if effective_plan || casual_commenting {
            ...
            let plan_label: &str = if approval_is_commenting || casual_commenting {
                commenting_label = match commenting_range {
                    Some(r) if r.len() == 1 => format!("{} L{}", i18n::tr("commenting"), r.start),
                    Some(r) => format!("{} L{}-{}", i18n::tr("commenting"), r.start, r.end - 1),
                    None => i18n::tr("commenting").to_string(),
                };
                commenting_label.as_str()
            } else if self.plan_approval_view.is_some() {
                i18n::tr("plan approval")
            } else {
                i18n::tr("Plan")
            };
        ...
        if self.decision_mode_active {
            mode_flags_vec.push(PromptFlag {
                text: i18n::tr("Decision"),
                color: Some(theme.accent_plan),
                bold: false,
            });
        }
        ...
        if permission_view_h > 0 {
            ...
            if let Some(ref qv) = self.question_view {
                ...
                if footer_y < layout.prompt.y + layout.prompt.height && footer_w > 10 {
                    ...
                    left_spans.push(Span::styled(i18n::tr(" navigate"), hint_style));
                    if qv.questions.len() > 1 {
                        left_spans.push(Span::styled(" \u{b7} ", hint_style));
                        left_spans.push(Span::styled("\u{2190}/\u{2192}", hint_key));
                        left_spans.push(Span::styled(i18n::tr(" question"), hint_style));
                    }
                    left_spans.push(Span::styled(" \u{b7} ", hint_style));
                    left_spans.push(Span::styled("y", hint_key));
                    left_spans.push(Span::styled(i18n::tr(" copy"), hint_style));
        ...
        self.pane_areas = layout.pane_areas();
        {
            ...
            if route.emit_osc8 {
                ...
                *link_spans_out = self
                    ...
                    .map(|link| kimix_ratatui_inline::LinkSpan {
                        row: link.screen_row,
