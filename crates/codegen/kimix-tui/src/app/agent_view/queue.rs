...
impl AgentView {
    ...
    pub(in crate::app) fn force_interject_queue_row(&mut self, id: u64) -> InputOutcome {
        ...
        if is_server {
            // Server row: the agent promotes it to run next (`Kimix/queue/interject`); any kind may send now.
                ...
                && let Some(server_id) = row.server_id.clone()
            {
                ...
                // the intent; the confirming `Kimix/queue/changed` broadcast
    ...
    /// `Kimix/queue/changed` broadcast (pre-merge entries — the mirrored
    ...
    /// QUEUED — the caller fires `Kimix/queue/interject` with that
    ...
    pub(in crate::app) fn handle_queue_key(
        ...
        if let Some(event) = self.queue.handle_key(key, registry) {
            ...
            // Server-origin rows go to the agent as `Kimix/queue/*`
    ...
    /// Reorder payload for `Kimix/queue/reorder`. Omit only running; include
