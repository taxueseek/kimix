...
impl AgentView {
    ...
    pub(in crate::app) fn finish_text_drag(&mut self) -> bool {
        ...
        if let Some((text, kind)) = copied
            && !text.is_empty()
        {
            // Only keep the highlight when the clipboard write actually lands.
            // Previously we persisted the selection *before* copy and always
            // returned true, so a failed pbcopy left a "selected" look with an
            // empty clipboard — the classic "drag select auto-copy broke" symptom.
            ...
            tracing::warn!(
                len = text.len(),
                "finish_text_drag: clipboard write failed; selection not persisted"
            );
            return false;
        }
        if drag.is_some() {
            tracing::debug!(
                event = "finish_text_drag_empty",
                "scrollback text drag finished with no reconstructable text"
            );
        }
        false
    }
