...
impl AgentView {
    ...
    /// `session/load` response meta (`Kimix/runningPromptId`): enter
    ...
    pub fn apply_full_context_info(&mut self, next: kimix_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            Some(snap) => {
                ...
                snap.usage_pct = kimix_token_estimation::usage_percentage_u8(used, snap.total);
                snap.free_tokens = kimix_token_estimation::free_tokens(snap.total, used);
            }
            None => {
                self.context_state = Some(kimix_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
