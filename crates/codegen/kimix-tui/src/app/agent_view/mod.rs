...
impl McpInitProgress {
    ...
    ///   `Kimix/mcp_initialized` clears the progress.
...
/// (`Kimix/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    pub context_state: Option<kimix_shell::session::ContextInfo>,
    ...
    /// (optimistic echo). The shell broadcasts `Kimix/session/interjection` to
    ...
    /// Whether Decision Mode is active for this agent. When active, the agent
    /// Whether the session is in Decision mode. Set via Shift+Tab cycle
    /// (Normal → Plan → Decision → Normal). When active, the agent applies
    /// first-principles analysis and MECE frameworks before executing.
    pub(crate) decision_mode_active: bool,
    ...
    pub(crate) deferred_session_mode: Option<kimix_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `Kimix/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`Kimix/session/info` `agentName`).
    ...
    /// Armed when an `Kimix/session/prompt_complete` broadcast arrives for the
    ...
    /// `Kimix/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `Kimix/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `Kimix/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`Kimix/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `Kimix/follow_ups` that arrived for a turn that is NOT yet the
...
fn remember_mode_enabled() -> bool {
    let path = kimix_tools::util::kimix_home::kimix_home().join("config.toml");
...
fn resolve_action(action_id: Option<ActionId>) -> Option<InputOutcome> {
    let action = match action_id? {
        ...
        ActionId::TogglePlanMode => Action::TogglePlanMode,
        ActionId::CancelTurn
        | ActionId::Quit
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &kimix_tools::implementations::kimix::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(super) mod test_fixtures {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `Kimix/follow_ups`
    ...
    /// FINDING B (stamped path): a LATE FIRST-TIME (never-seen) `Kimix/follow_ups`
    ...
    /// upstream by the `_meta["kimix/replayed"]` gate); it is pinned here so the
    ...
    /// FIX (buffer-before-adoption): a stamped `Kimix/follow_ups` for a turn that
