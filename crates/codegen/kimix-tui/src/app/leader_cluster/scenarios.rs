...
#[test]
#[ignore = "leader-cluster: needs single-process isolation (process-global env + kimix_home OnceLock in the shared lib test binary); run: cargo test -p kimix-tui --lib -- app::leader_cluster --ignored --test-threads=1"]
...
/// Locate `<kimix_home>/sessions/<enc-cwd>/<sid>/updates.jsonl` without the
...
fn find_session_updates_file(sid: &str) -> Option<PathBuf> {
    let sessions = effective_kimix_home().join("sessions");
