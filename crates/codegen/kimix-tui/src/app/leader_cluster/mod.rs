...
//!
//! Env sandboxing follows this crate's `serial(KIMIX_SHARE_DIR)` idiom; note
//! `kimix_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_kimix_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `KIMIX_LEADER_SOCKET`, `KIMIX_SHARE_DIR`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `kimix_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p kimix-tui --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The kimix home the agent actually persisted under: `kimix_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_kimix_home() -> PathBuf {
...
struct PagerLeaderCluster {
    ...
    /// agent on the same KIMIX_SHARE_DIR (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(KIMIX_SHARE_DIR)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = kimix_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("KIMIX_SHARE_DIR", kimix_home.path()),
            crate::test_util::EnvVarGuard::set("KIMIX_CODE_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("KIMIX_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("KIMIX_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("KIMIX_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("KIMIX_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (kimix_version), not
        ...
        // kimix-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: kimix-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same KIMIX_SHARE_DIR when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `KIMIX_LEADER_SOCKET`, flock held
