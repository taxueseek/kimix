...
use kimix_shell::extensions::notification::GoalClassifierVerdict;
...
pub enum AgentCommand {
    ...
    /// `Kimix/session/fork` request is in flight.
    ForkSession,
}
impl AgentCommand {
...
pub struct AgentSession {
    ...
    /// (`Kimix/prompt_history` scoped via `filter_session_id`). Most-recent-first.
    /// Fetched on session create/load; prompts sent in this session are
    /// additionally front-inserted locally on send.
    pub prompt_history: Vec<String>,
    /// True until the session's startup/load `Kimix/prompt_history` fetch completes.
...
pub struct ChipElement {
    ...
    pub kind: kimix_ratatui_textarea::ElementKind,
