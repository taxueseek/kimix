...
/// [`kimix_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<kimix_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: kimix_shell::util::config::WorktreeHintMode) -> Self {
        use kimix_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        kimix_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) = kimix_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// methods ([`kimix_shell::agent::auth_method`]).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl PlatformLogin {
    ...
    pub fn from_method_id(id: &acp::AuthMethodId) -> Option<Self> {
        match id.0.as_ref() {
            kimix_shell::agent::auth_method::MOONSHOT_CN_METHOD_ID => Some(Self::MoonshotCn),
            kimix_shell::agent::auth_method::MOONSHOT_AI_METHOD_ID => Some(Self::MoonshotAi),
            _ => None,
        }
    }
    /// The registry platform whose `[platforms.<id>]` table stores the key.
    pub fn platform_id(self) -> kimix_shell::models::PlatformId {
        match self {
            Self::MoonshotCn => kimix_shell::models::PlatformId::MoonshotCn,
            Self::MoonshotAi => kimix_shell::models::PlatformId::MoonshotAi,
        }
    }
...
pub enum PendingMenuItem {
    /// Interactive session login (Kimi Code OAuth / xAI Session / Grok bridge).
    Login {
        label: String,
...
/// session logins (Kimi Code OAuth, xAI Session, optional Grok CLI bridge)
/// plus Moonshot API-key logins, then Quit. `xai.api_key` and `cached_token`
/// are non-interactive (eager-auth only) and never listed.
///
/// When the shell advertises no picker method at all, fall back to a single
/// Kimi Code row so the screen keeps its shape.
pub fn pending_menu_items(
    ...
    for method in auth_methods {
        ...
        // Kimi + xAI need a browser; Grok bridge is one-click import — all
        // appear as selectable Login rows so /login is not Kimi-only.
        let label = match kind {
            AuthMethodKind::KimiCode => format!("{} (OAuth)", method.name()),
            AuthMethodKind::XaiSession => format!("{} (device code)", method.name()),
            AuthMethodKind::GrokSession => method.name().to_string(),
            _ if kind.needs_interactive_login() => format!("{} (OAuth)", method.name()),
            _ => continue,
        };
...
/// [`kimix_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `KIMIX_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `KIMIX_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("KIMIX_ESC_DOUBLE_PRESS_MS").ok()))
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: kimix_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`kimix_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.kimix/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`KIMIX_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `KIMIX_FPS` env on release
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `kimix dashboard`, the
    ...
    /// `Kimix/sessions/list` polls and `Kimix/sessions/changed` broadcasts.
    ...
    /// poll outside leader mode, so we fetch the same `Kimix/session/list` the
    ...
    /// Reconciled from `Kimix/queue/changed` broadcasts so
    ...
    /// confirming `Kimix/queue/changed` broadcast has not yet arrived. Keyed by
    ...
    /// non-selectable headers. Gated by `KIMIX_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `KIMIX_CANCEL_REWIND` env /
    ...
    /// When false, the pager must not request recaps (zero `Kimix/recap` traffic).
    ...
    /// empty and inert outside `--minimal`; the `kimix-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<kimix_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        kimix_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `kimix_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["kimix/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `KIMIX_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: kimix_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<kimix_shell::util::config::ContextualHintsRemote>,
    ...
    /// `~/.kimix/config.toml`. `None` when the file/section is absent
...
impl AppView {
    ...
    pub fn apply_auth_meta(&mut self, meta: &kimix_shell::auth::AuthMeta) {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: kimix_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: kimix_shell::util::config::auto_permission_mode_enabled_from_disk(),
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<kimix_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    /// `Kimix/queue/changed` broadcast. The broadcast is
    ...
    /// `Kimix/queue/changed` broadcast replaces it (matched by `id`) rather than
    ...
    /// Rows the dev `KIMIX_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results: &'a Option<Vec<kimix_shell::extensions::session_search::SearchSessionHit>>,
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                kimix_shell::util::with_locked_stderr(|stderr| {
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: kimix_shell::util::config::ResolvedContextualHints,
    ) {
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = kimix_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![kimix_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: kimix_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = kimix_shell::auth::AuthMeta::default();
    ...
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&kimix_shell::auth::AuthMeta::default());
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(kimix_workspace::foreign_sessions::RecentForeignSession {
    ...
    #[test]
    fn esc_running_cancels_turn() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        app.agents.get_mut(&id).unwrap().session.state = AgentState::TurnRunning;
        let outcome = app.handle_input(&esc_key());
        assert!(matches!(outcome, InputOutcome::Action(Action::CancelTurn)));
    }
    #[test]
    fn esc_cancelling_escalates_to_quit_pending() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
    ...
    #[test]
    fn esc_idle_empty_prompt_sets_pending_quit() {
    ...
    #[test]
    fn esc_idle_empty_prompt_focused_sets_pending_quit() {
        crate::appearance::cache::set_simple_mode(true);
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        app.agents.get_mut(&id).unwrap().active_pane = crate::views::agent::ActivePane::Prompt;
    ...
    #[test]
    fn esc_double_press_idle_quits() {
        crate::appearance::cache::set_simple_mode(true);
        let mut app = test_app_with_agent();
        let _ = app.handle_input(&esc_key());
        assert!(app.pending_action.is_some());
        let outcome = app.handle_input(&esc_key());
        assert!(matches!(outcome, InputOutcome::Action(Action::Quit)));
        assert!(app.pending_action.is_none());
    ...
    #[test]
    fn esc_consumed_by_cancel_does_not_set_pending() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        app.agents.get_mut(&id).unwrap().session.state = AgentState::TurnRunning;
        let outcome = app.handle_input(&esc_key());
    ...
    #[test]
    fn esc_consumed_by_text_clear_does_not_set_pending() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        let agent = app.agents.get_mut(&id).unwrap();
        agent.active_pane = crate::views::agent::ActivePane::Prompt;
    ...
    #[test]
    fn esc_then_other_key_resets_pending() {
        crate::appearance::cache::set_simple_mode(true);
        let mut app = test_app_with_agent();
        let _ = app.handle_input(&esc_key());
        assert!(app.pending_action.is_some());
    ...
    #[test]
    fn esc_idle_prompt_with_text_clears_text() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        let agent = app.agents.get_mut(&id).unwrap();
        agent.active_pane = crate::views::agent::ActivePane::Prompt;
        ...
        assert!(
            matches!(outcome, InputOutcome::Changed),
            "Esc with text in idle prompt must Change (clear text), got: {outcome:?}",
        );
        assert!(
            app.agents[&id].prompt.textarea.text().is_empty(),
            "Esc must clear prompt text when agent is idle; got: {:?}",
            app.agents[&id].prompt.textarea.text(),
        );
    }
    #[test]
    fn esc_running_prompt_with_text_clears_text_and_preserves_turn() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        let agent = app.agents.get_mut(&id).unwrap();
        agent.session.state = AgentState::TurnRunning;
        ...
        assert!(
            matches!(outcome, InputOutcome::Changed),
            "Esc with text in a running prompt must clear the text, got: {outcome:?}",
        );
        assert!(
            app.agents[&id].prompt.textarea.text().is_empty(),
            "Esc must clear prompt text first; got: {:?}",
            app.agents[&id].prompt.textarea.text(),
        );
        assert!(
            app.agents[&id].session.state.is_turn_running(),
            "First Esc must NOT cancel the turn while a draft was present",
        );
        let outcome = app.handle_input(&esc_key());
        assert!(
            matches!(outcome, InputOutcome::Action(Action::CancelTurn)),
            "Second Esc on empty running prompt must CancelTurn, got: {outcome:?}",
        );
    }
    #[test]
    fn esc_from_prompt_pane_running_turn_cancels() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        let agent = app.agents.get_mut(&id).unwrap();
        agent.session.state = AgentState::TurnRunning;
        ...
        assert!(
            matches!(outcome, InputOutcome::Action(Action::CancelTurn)),
            "Esc while running must cancel turn, got {outcome:?}"
        );
        assert!(app.pending_action.is_none());
    }
    #[test]
    fn esc_from_prompt_pane_running_turn_with_draft_clears_text() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        let agent = app.agents.get_mut(&id).unwrap();
        agent.session.state = AgentState::TurnRunning;
        ...
        assert!(
            matches!(outcome, InputOutcome::Changed),
            "1st Esc with draft must clear text, got {outcome:?}"
        );
        assert!(
            app.agents[&id].prompt.textarea.text().is_empty(),
            "1st Esc must clear prompt text first; got: {:?}",
            app.agents[&id].prompt.textarea.text(),
        );
        assert!(
            app.agents[&id].session.state.is_turn_running(),
            "1st Esc must NOT cancel the turn while a draft was present"
        );
        let outcome = app.handle_input(&key_event(KeyCode::Esc, KeyModifiers::NONE));
        assert!(
            matches!(outcome, InputOutcome::Action(Action::CancelTurn)),
            "2nd Esc on empty running prompt must CancelTurn, got: {outcome:?}",
        );
    }
    #[test]
    fn esc_from_scrollback_pane_running_turn_cancels() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        let agent = app.agents.get_mut(&id).unwrap();
        agent.session.state = AgentState::TurnRunning;
        ...
        assert!(
            matches!(outcome, InputOutcome::Action(Action::CancelTurn)),
            "Esc from scrollback while running must CancelTurn, got {outcome:?}"
        );
        assert!(app.pending_action.is_none());
    }
    #[test]
    fn esc_while_cancelling_escalates_to_quit() {
        let mut app = test_app_with_agent();
        let id = super::super::agent::AgentId(0);
        let agent = app.agents.get_mut(&id).unwrap();
        ...
        assert!(
            matches!(outcome, InputOutcome::Changed),
            "Esc while cancelling must arm Quit, got {outcome:?}"
        );
        assert!(app.pending_action.is_some());
    ...
    fn welcome_pending_l_triggers_login() {
        ...
        // Empty auth_methods → fallback Kimi Code row.
        assert!(
            matches!(
                outcome,
                InputOutcome::Action(Action::Login { method_id: Some(ref id) })
                    if id.0.as_ref() == "kimi-code"
            ),
            "got {outcome:?}"
        );
    }
    ...
    /// Fresh-user methods: kimi-code + moonshot×2 + xAI Session.
    fn fresh_user_auth_methods() -> Vec<acp::AuthMethod> {
        kimix_shell::agent::auth_method::build_auth_methods(
            kimix_shell::agent::auth_method::AuthMethodsBuildInputs {
    ...
    fn pending_menu_items_lists_interactive_methods_plus_quit() {
        ...
        // Kimi + moonshot-cn + moonshot-ai + xAI Session + Quit
        assert_eq!(items.len(), 5, "4 login rows + Quit, got {items:?}");
        assert!(
            matches!(
                &items[0],
                PendingMenuItem::Login { label, method_id }
                    if label == "Kimi Code (OAuth)" && method_id.0.as_ref() == "kimi-code"
            ),
            "row 0 must be the Kimi OAuth login, got {:?}",
            items[0]
        );
        assert_eq!(
            items[1],
        ...
        assert!(
            matches!(
                &items[3],
                PendingMenuItem::Login { label, method_id }
                    if label.contains("xAI") && method_id.0.as_ref() == "xai-session"
            ),
            "row 3 must be xAI Session, got {:?}",
            items[3]
        );
        assert_eq!(items[4], PendingMenuItem::Quit);
        // The non-interactive methods must never appear as rows.
        let byok = kimix_shell::agent::auth_method::build_auth_methods(
            kimix_shell::agent::auth_method::AuthMethodsBuildInputs {
    ...
    fn login_picker_has_choice_only_with_multiple_login_rows() {
        ...
        let kimi_only = vec![kimix_shell::agent::auth_method::kimi_code_auth_method(None)];
    ...
    fn welcome_pending_arrows_select_moonshot_row() {
        ...
        // 'l' is muscle-memory for the first (Kimi OAuth) row regardless of the
        // arrow selection.
        let outcome = app.handle_input(&key_event(KeyCode::Char('l'), KeyModifiers::NONE));
        assert!(
            matches!(
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![kimix_workspace::file_system::FuzzyMatchResult {
    ...
    fn install_question_overlay(
        ...
        use kimix_tools::implementations::kimix::ask_user_question::{Question, QuestionOption};
