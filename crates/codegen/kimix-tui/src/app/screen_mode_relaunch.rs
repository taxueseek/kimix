...
pub(crate) const KIMIX_SCREEN_MODE_ENV: &str = "KIMIX_SCREEN_MODE";
...
/// `kimix "do the thing"` does not re-submit on resume. Keeps everything else
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    while let Some(arg) = iter.next() {
        ...
        // Bare positional prompt (e.g. `kimix "fix the bug"`). Must not re-fire
    ...
    // Keep a CLI mode flag for hand-pasted resume hints that omit KIMIX_SCREEN_MODE.
    if want_minimal {
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{KIMIX_SCREEN_MODE_ENV}={mode} kimix {flag} --resume {session_id}")
}
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(KIMIX_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));
...
/// Parse a [`KIMIX_SCREEN_MODE_ENV`] or config `[ui] screen_mode` value
...
/// Consume the one-shot screen-mode override env (see [`KIMIX_SCREEN_MODE_ENV`]).
///
...
/// (tool shells, workers, nested `kimix` invocations) would inherit a forced
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(KIMIX_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(KIMIX_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn adds_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["kimix", "--no-leader"]), "abc", true);
        assert_eq!(
            as_strs(&out),
    ...
    fn adds_fullscreen_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["kimix", "--no-leader"]), "abc", false);
        assert_eq!(
            as_strs(&out),
    ...
    fn strips_session_id_flag() {
        ...
        let out = build_screen_mode_relaunch_args(
            args(&["kimix", "-s", "11111111-1111-1111-1111-111111111111"]),
            "new",
            false,
        );
    ...
    fn strips_prior_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(
            args(&["kimix", "--minimal", "--resume", "old", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_fullscreen_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["kimix", "--fullscreen", "--resume", "old", "--no-leader"]),
            "new",
            true,
        );
        assert_eq!(
    ...
    fn strips_resume_equals_form() {
        let out = build_screen_mode_relaunch_args(
            args(&["kimix", "--resume=old-id", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn strips_positional_prompt() {
        let out = build_screen_mode_relaunch_args(
            args(&["kimix", "--no-leader", "fix the bug"]),
            "sid",
            true,
        );
        assert_eq!(
    ...
    fn double_dash_and_following_positionals_dropped() {
        // `kimix --no-leader -- "fix the bug"`: everything after `--` is the
    ...
    fn keeps_equals_form_and_short_model_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["kimix", "-m", "kimix-4", "--cwd=/tmp/proj", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn resume_without_value_then_flag_is_not_eaten() {
        // `kimix --resume --no-leader` (resume most-recent; next token is a flag).
        let out = build_screen_mode_relaunch_args(
            args(&["kimix", "--resume", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry KIMIX_SCREEN_MODE so following the
        ...
        assert_eq!(
            ...
            "KIMIX_SCREEN_MODE=fullscreen kimix --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "KIMIX_SCREEN_MODE=minimal kimix --minimal --resume abc-sid"
        );
    }
