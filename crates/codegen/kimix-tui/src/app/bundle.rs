...
//!
//! Pager-side cache of what `kimix-shell` reports from
//! `Kimix/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `Kimix/bundle/status` ACP responses.
...
/// Deserialized response from `Kimix/bundle/status`.
...
/// Deserialized response from `Kimix/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
