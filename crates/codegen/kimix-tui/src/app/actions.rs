...
use kimix_shell::sampling::types::ReasoningEffort;
...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: kimix_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// `prompt_id`. Routed to the agent as `Kimix/queue/remove`;
    /// the resulting `Kimix/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match
    /// `ordered_ids`. Routed as `Kimix/queue/reorder`.
    QueueReorderShared {
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-authoritative (shared) queued prompts.
    /// Routed as `Kimix/queue/clear`.
    QueueClearShared,
    /// Replace the text of a server-authoritative (shared) queued prompt.
    /// Routed to the agent as `Kimix/queue/edit`; the rebroadcast of
    /// `Kimix/queue/changed` is the source of truth. Last write wins via the
    ...
    /// merges its text into the in-flight turn. Routed as `Kimix/queue/interject`;
    /// the `Kimix/session/interjection` + `Kimix/queue/changed` rebroadcasts are
    ...
    OpenTranscriptPager,
    /// Minimal mode (`kimix --minimal`): re-print the most-recently committed
    ...
    ExecuteHooksAction(kimix_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(kimix_hooks_plugins_types::PluginsAction),
    /// Add or update an MCP server via Kimix/mcp/upsert.
    UpsertMcpServer {
        name: String,
        config: Box<kimix_shell::util::config::McpServerConfig>,
    },
    /// Delete an MCP server via Kimix/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via Kimix/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    DemoteToBackground,
    /// Request current bundle cache status via `Kimix/bundle/status`.
    RequestBundleStatus,
    ...
    CycleMode,
    /// Toggle Plan mode (Tab): Normal ↔ Plan.
    TogglePlanMode,
    ...
    /// Commit the active theme (canonical name, e.g. `"kimixnight"`, `"auto"`).
    SetTheme(String),
    /// Commit the UI language (`"auto"`, `"zh"`, `"en"`).
    SetLanguage(String),
    ...
    SwitchAccount,
    /// User pressed login on the welcome screen or ran `/login`.
    ///
    /// `method_id`:
    /// - `Some(id)` — start that auth method immediately (picker row selected)
    /// - `None` — show the multi-provider picker (Kimi / xAI / Moonshot)
    Login {
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `kimix dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.kimix/config.toml` and route ACP
/// `Kimix/yolo_mode_changed` notifications. The ACP notification is
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: kimix_workspace::foreign_sessions::EnabledForeignSessionSources,
        kimix_home: std::path::PathBuf,
    ...
    FetchSessionList {
        /// Text search pushed down to `Kimix/session/list` as `query` (chat
    ...
    /// `Kimix/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `Kimix/session/list` — the non-leader fallback for the
    ...
    /// Cancel a subagent via `Kimix/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.kimix/config.toml`.
    ...
    /// Persist a typed setting to `~/.kimix/config.toml`. On failure,
    ...
    /// `Kimix/queue/remove`. The agent re-broadcasts the authoritative queue.
    QueueRemove {
        session_id: acp::SessionId,
        id: String,
        expected_version: u64,
    },
    /// Reorder server-owned queued prompts: fire-and-forget `Kimix/queue/reorder`.
    QueueReorder {
        session_id: acp::SessionId,
        ordered_ids: Vec<String>,
    },
    ...
    /// `Kimix/queue/edit`. The session actor's serialized mailbox makes this
    /// last-writer-wins for concurrent edits; the rebroadcast of
    /// `Kimix/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    ...
    /// fire-and-forget `Kimix/queue/interject`. The session actor atomically
    ...
    /// Resolve the running agent name for a session (`Kimix/session/info`).
    ...
    /// Fetch MCP server list from the shell (Kimix/mcp/list).
    FetchMcpsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cache: bool,
    },
    /// Trigger MCP OAuth for a server (Kimix/mcp/auth_trigger).
    McpAuthTrigger {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    },
    /// Fetch hooks list from the shell (Kimix/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (Kimix/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Fetch skills list from the shell (Kimix/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Toggle a skill via Kimix/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    /// Upsert an MCP server via Kimix/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via Kimix/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via Kimix/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch and display session info via Kimix/session/info.
    ShowSessionInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
        show_resolved_model: bool,
    },
    /// Fetch and display detailed context usage via Kimix/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch current bundle cache status via `Kimix/bundle/status`.
    FetchBundleStatus,
    /// Fetch a bundled entry's raw content via `Kimix/bundle/entry/get`.
    ...
    /// Send raw note to Kimix/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via Kimix/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
        question: String,
    },
    /// Request a session recap via the Kimix/recap ext method. Fire-and-forget:
    /// the recap arrives later as a `SessionRecap` notification.
    SendRecap {
        session_id: acp::SessionId,
        auto: bool,
    },
    /// Send a mid-turn interjection via Kimix/interject ext method.
    SendInterject {
        agent_id: AgentId,
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `Kimix/session/interjection`
    ...
    /// Log out via `Kimix/auth/logout` (shell clears auth.json + in-memory state).
    Logout,
    ...
    /// registry (`~/.kimix/active_sessions.json`).
    ...
    /// Call `Kimix/session/fork` to create a peer session that resumes
    ...
    /// Fetch Kimi usage/quota rows from the agent's `Kimix/billing`
    ...
    /// Send an ACP `Kimix/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `Kimix/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`kimix-0.1` when the
...
/// Outcome of an `Kimix/subagent/cancel` request, telling dispatch whether the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<kimix_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionLoaded {
        ...
        /// `_meta["kimix/runningPromptId"]`), present only when the session was
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<kimix_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `Kimix/sessions/list`.
    RosterLoaded {
    ...
    CancelComplete,
    /// Response to `Kimix/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    AuthUrlReady {
        ...
        external: bool,
        /// Presentation mode from `Kimix/auth/get_url`; `None` on older agents.
        mode: Option<String>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<kimix_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<kimix_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<kimix_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<kimix_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<kimix_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    /// `Kimix/recap` request acknowledged (fire-and-forget). The recap itself
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<kimix_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `Kimix/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `Kimix/session/fork` failed. The placeholder agent stays in
    ...
    UsageFetched {
        agent_id: AgentId,
        result: Result<Vec<kimix_shell::extensions::billing::UsageRow>, String>,
    },
    ...
    /// Shell suggestions loaded from ACP `Kimix/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `Kimix/suggestPrompt`.
