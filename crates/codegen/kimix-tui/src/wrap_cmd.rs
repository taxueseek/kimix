//! `kimix wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `kimix ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`kimix wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("kimix wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = kimix_config::shell::is_command_available(program);
        (
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("kimix wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `kimix wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`kimix wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`kimix wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `kimix_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `kimix ssh`, `kimix wrap` does not gate on the terminal brand: the user
