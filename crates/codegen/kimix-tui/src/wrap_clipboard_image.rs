//! Host clipboard image paste mediated by `kimix wrap`.
//!
...
//! the inner `kimix`) can solicit the host pasteboard. That is intentional and
//! acceptable for `kimix wrap` because (1) the user opted into wrap on their
//! own host, (2) the answer stays inside their session, and (3) the remote
//! only requests when `osc52_sink_active()` (wrap already set
//! `KIMIX_OSC52_SINK` / `LC_KIMIX_OSC52_SINK`). Do not generalize this pattern
//! to untrusted multiplexers without an explicit allowlist.
use base64::Engine as _;
use kimix_pager_render::clipboard::{ImageData, osc52_sink_active};

/// OSC body after `ESC ]` for a host image request.
pub const REQUEST_BODY: &[u8] = b"999;KimixWrapClipboardImage?";
...
/// Successful host image frame: `KIMIX_WRAP_IMG\n<mime>\n<base64>`.
pub const MAGIC_IMG: &str = "KIMIX_WRAP_IMG";

/// Host has no image (`KIMIX_WRAP_NONE`; not a prefix of [`MAGIC_IMG`]).
pub const MAGIC_NONE: &str = "KIMIX_WRAP_NONE";
...
mod tests {
    ...
    fn garbage_is_not_wrap_paste() {
        ...
        assert_eq!(try_decode_wrap_host_image_paste("KIMIX_WRAP_IM"), None);
    }

    #[test]
    fn malformed_img_frame_consumed_not_text() {
        assert_eq!(
            try_decode_wrap_host_image_paste("KIMIX_WRAP_IMG"),
            Some(WrapImagePaste::NoImage)
        );
        assert_eq!(
            try_decode_wrap_host_image_paste("KIMIX_WRAP_IMG\nbad"),
    ...
    fn oversized_b64_rejected_before_decode() {
        ...
        let payload = format!("KIMIX_WRAP_IMG\nimage/png\n{huge}");
        assert_eq!(
