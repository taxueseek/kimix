...
pub fn summarize_warnings(warnings: &[TerminalWarning]) -> Option<crate::startup::StartupWarning> {
    ...
    let is_ssh = kimix_shell::util::clipboard::is_remote_session();
...
fn summarize_warnings_inner(
    ...
    Some(crate::startup::StartupWarning {
        ...
        message: crate::i18n::tr_owned("Clipboard may be unreachable."),
        action: Some(crate::i18n::tr_owned(
...
pub fn collect_startup_warnings(
    ...
    if ctx.brand == TerminalName::AppleTerminal && kimix_shell::util::clipboard::is_remote_session()
    {
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(kimix_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        message: format!(
            "Your project sandbox profile conflicts with user config.\nProfile: {profiles}\nProject config: .kimix/sandbox.toml\nUser config: ~/.kimix/sandbox.toml"
        ),
...
pub fn assemble_startup_warnings(
    ...
    if wayland_clipboard_warning.is_some() {
        summarized.insert(
            ...
            crate::startup::StartupWarning {
                severity: crate::startup::WarningSeverity::Warning,
                message: crate::i18n::tr_owned("Copies need this terminal to stay focused."),
                action: Some(crate::i18n::tr_owned("See /terminal-setup for details.")),
            },
        );
    }
    if wezterm_warning.is_some() {
        summarized.insert(
            ...
            crate::startup::StartupWarning {
                severity: crate::startup::WarningSeverity::Warning,
                message: crate::i18n::tr_owned(
                    "Shift+Enter newlines need a WezTerm config change.",
                ),
                action: Some(crate::i18n::tr_owned("See /terminal-setup for the fix.")),
            },
        );
    }
    summarized
...
pub fn diagnose_wayland_data_control_live() -> Option<TerminalWarning> {
    ...
    diagnose_wayland_data_control(
        is_wayland,
        is_wayland && kimix_shell::util::clipboard::wayland_data_control_supported(),
        is_wayland && kimix_shell::util::clipboard::native_tool_name() == "wl-copy",
    )
}
...
pub fn color_support_warning(
    ...
    if level == ColorLevel::None {
        ...
        warning.note = Some("Unset NO_COLOR and restart Kimix.".to_string());
        return Some(warning);
    }
    ...
    warning.note = Some("Persist in ~/.zshrc / ~/.bashrc and restart Kimix.".to_string());
    Some(warning)
}
...
mod tests {
    ...
    fn notification_none_protocol_no_warnings() {
        let ctx = TerminalContext {
            brand: TerminalName::KimixDesktop,
            ..Default::default()
        };
        let query = FakeTmuxQuery::healthy_modern();
        let w = collect_notification_warnings(
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::KimixDesktop));
    ...
    fn format_themes_env_line_lists_available_below_truecolor() {
        ...
        for level in [ColorLevel::Ansi256, ColorLevel::Basic, ColorLevel::None] {
            ...
            assert!(line.contains("kimixnight") && line.contains("kimixday"));
