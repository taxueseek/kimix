...
use kimix_tui::scrollback::text_selection::{
...
use kimix_tui::scrollback::types::slice_display_cols;
use kimix_tui::scrollback::{RenderBlock, ScratchBuffer, ScrollbackPane, ScrollbackState};
...
fn selection_highlight_duration_ms() -> u64 {
    ...
    if kimix_tui::appearance::cache::load_keep_text_selection().holds() {
        0
    } else {
        PLAYGROUND_FLASH_MS
    }
...
fn double_click_action_label() -> &'static str {
    if kimix_tui::appearance::cache::load_keep_text_selection().selects_word() {
        "word_select"
    } else {
        "toggle_fold"
    }
...
impl App {
    fn new() -> Self {
        ...
        scrollback.push_block(RenderBlock::agent_message(
            "I'm Kimix, an interactive CLI agent built to help with software engineering tasks like coding, debugging, refactoring, and exploring codebases.",
        ));
        scrollback.push_block(RenderBlock::agent_message(
