...
//!   `KIMIX_MEMTRACE_INTERVAL_SECS` (default 30s) from a detached thread.
...
//!   (`KIMIX_MEMTRACE_THRESHOLD_MB`, default 1 GiB, then doubling), a full
...
//! ## Files
//!
//! `$KIMIX_SHARE_DIR/memtrace/<start-ts>-<pid>.jsonl` (+ `.1` after 4 MiB
...
//!
//! The lib cannot depend on jemalloc; `kimix-bin` installs:
...
/// Whether a trace sink is installed ([`start`] ran and `KIMIX_MEMTRACE` is
...
/// Env: disable with `KIMIX_MEMTRACE=0|false|off`.
fn enabled_by_env() -> bool {
    !matches!(
        std::env::var("KIMIX_MEMTRACE").ok().as_deref(),
        Some("0") | Some("false") | Some("off")
    )
}

fn interval_from_env() -> Duration {
    let secs = std::env::var("KIMIX_MEMTRACE_INTERVAL_SECS")
        .ok()
...
fn first_threshold_from_env() -> u64 {
    std::env::var("KIMIX_MEMTRACE_THRESHOLD_MB")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1024)
        .saturating_mul(1 << 20)
...
/// `dir` (e.g. `$KIMIX_SHARE_DIR/memtrace/`) and spawn the detached sampler
...
/// don't trace. Inert when `KIMIX_MEMTRACE=0`.
///
...
/// (`kimix --version`, `kimix trace …`) leave no files behind.
