...
/// firehose (`KIMIX_DEBUG_LOG` / `KIMIX_LOG_FILE`).
pub use kimix_log::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `kimix-log` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use kimix_log::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "kimix_shell=info,kimix_tui=trace,kimix_tools=info,kimix_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let instrumentation_layer = kimix_log::instrumentation::layer();
    let sampling_log_layer = kimix_log::sampling_log::layer();
    let hooks_log_layer = kimix_log::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        .with(fmt_layer.with_filter(env_filter))
        .with(instrumentation_layer)
        .with(sampling_log_layer)
        .with(hooks_log_layer);
    kimix_log::debug_log::install_firehose(registry, "tui");
    TracingHandle { rx }
}
#[cfg(test)]
mod tests {
