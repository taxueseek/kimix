...
use clap::Subcommand;
use kimix_fast_worktree::WorktreeRecord;
...
async fn dispatch(command: WorktreeCommand, tx: &kimix_acp_lib::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &kimix_acp_lib::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &kimix_acp_lib::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
        WorktreeDbCommand::Stats => {
            let stats: DbStats = ext_call(tx, "kimix/git/worktree/db/stats", &()).await?;
        ...
        WorktreeDbCommand::Path => {
            ...
            let resp: PathResp = ext_call(tx, "kimix/git/worktree/db/path", &()).await?;
            println!("{}", resp.path);
            Ok(())
        }
        WorktreeDbCommand::Rebuild => {
            let report: RebuildReport = ext_call(tx, "kimix/git/worktree/db/rebuild", &()).await?;
...
mod tests {
    ...
    fn ext_request_builds_list_with_filters() {
        ...
        .unwrap();
        assert_eq!(req.method.as_ref(), "kimix/git/worktree/list");
    ...
    fn ext_request_builds_db_stats_empty_params() {
        let req = ext_request("kimix/git/worktree/db/stats", &()).unwrap();
        assert_eq!(req.method.as_ref(), "kimix/git/worktree/db/stats");
    }
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.kimix/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.kimix/worktrees.db");
    }
