//! `kimix mcp` — manage MCP server configurations from the command line.
...
Examples:
  ...
  # Add to the project config (./.kimix/config.toml) instead of ~/.kimix/config.toml
  kimix mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.kimix/config.toml`, available in all your projects
    User,
    /// `./.kimix/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// flags such as `-y` are passed to the server instead of kimix.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
    ...
    /// Config to write to: user (~/.kimix/config.toml) or project (./.kimix/config.toml)
...
async fn run_auth(name: &str) -> Result<()> {
    ...
    match kimix_shell::mcp_doctor::run_auth(&cwd, name).await {
        Ok(tool_count) => {
...
fn run_list(json: bool) -> Result<()> {
    ...
    let cwd = current_dir_or_exit();
    let servers = kimix_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    ...
    if json {
        ...
        println!("No MCP servers configured. Run `kimix mcp add --help` to get started.");
    } else {
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    kimix_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: kimix mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: kimix mcp add --transport {label} <name> <url>"
                );
            };
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => kimix_shell::util::config::user_config_path(),
        McpScope::Project => kimix_shell::util::config::project_config_path(&current_dir_or_exit()),
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_kimix_path("config.toml"),
...
fn select_remove_site(
    ...
    use kimix_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    kimix_shell::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        kimix_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    let (scope, path) = match select_remove_site(user_defined, find_project_site(), requested_scope)
    {
        ...
        Err(RemoveError::Ambiguous { project_path }) => {
            ...
            eprintln!("  user: {}", display_user_kimix_path("config.toml"));
            eprintln!("  project: {}", project_path.display());
            eprintln!("Specify which one to remove, e.g.: kimix mcp remove {name} --scope project");
            std::process::exit(1);
        }
    };
    ...
    // ancestor .kimix/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = kimix_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        kimix_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["kimix", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        let add = parse_add(&["kimix", "mcp", "add", "api", "https://mcp.example.com/mcp"]);
        ...
        let add = parse_add(&["kimix", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "kimix", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "kimix", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
        .expect_err("local is not a kimix scope");
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "kimix", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        let add = parse_add(&["kimix", "mcp", "add", "fs", "-e", "NOT_A_PAIR", "--", "npx"]);
        ...
        let add = parse_add(&["kimix", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["kimix", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["kimix", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["kimix", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["kimix", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = kimix_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.kimix/config.toml");
