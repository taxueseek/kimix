//! `kimix completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    // Ensure the script always uses the public "kimix" name.
    let mut cmd = PagerArgs::command().name("kimix");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "kimix", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "kimix", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `kimix worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:kimix-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:kimix-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("kimix");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "kimix", &mut buf);
    ...
    // so `kimix worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
