...
///   - [`update_from_notification`] — the `Kimix/git_head_changed` ACP
...
pub fn worktree_label_index() -> std::collections::HashMap<PathBuf, String> {
    ...
    let Ok(db) = kimix_fast_worktree::db::WorktreeDb::open_default() else {
        return map;
    };
    ...
    for rec in records {
        let Some(label) = rec
            .metadata
            .as_ref()
            .and_then(|m| m.get(kimix_shell::session::worktree::META_KEY_LABEL))
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty())
        else {
            continue;
...
fn lookup_worktree_label(cwd: &Path) -> Option<String> {
    let db = kimix_fast_worktree::db::WorktreeDb::open_default().ok()?;
    ...
    for ancestor in cwd.ancestors() {
        if let Ok(Some(record)) = db.get(&ancestor.to_string_lossy()) {
            let label = record
                .metadata
                .as_ref()?
                .get(kimix_shell::session::worktree::META_KEY_LABEL)?
                .as_str()
                .filter(|s| !s.is_empty())
                .map(String::from);
            return label;
...
///
/// `KIMIX_NERD_FONTS=1` forces Powerline; `KIMIX_NERD_FONTS=0` forces the fallback.
pub(crate) fn branch_icon() -> &'static str {
    static ICON: OnceLock<&str> = OnceLock::new();
    ICON.get_or_init(|| {
        decide_branch_icon(
            std::env::var("KIMIX_NERD_FONTS").ok().as_deref(),
            HostOs::current(),
...
///
/// An explicit `KIMIX_NERD_FONTS` override (`0`/`false` → off, anything else →
