...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: kimix_shell::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(&self, f: impl FnOnce(&[kimix_markdown::HyperlinkTarget]) -> R) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    use kimix_shell::session::ContextInfo;
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        assert!(text.contains("kimix-test"), "got: {text:?}");
    ...
    fn context_info_indexes_model_only() {
        ...
        assert_eq!(block.searchable_text().as_deref(), Some("kimix-4.5"));
    }
