    ...
    fn new() -> Self {
        ...
        let handle = thread::spawn(move || {
            ...
            while let Ok(msg) = rx.recv() {
                ...
                generation += 1;
                *out.lock().expect("lock poisoned") = SearchSnapshot {
                    matches,
                    generation,
                    query: query.clone(),
                };
...
impl ScrollbackSearchState {
    ...
    pub fn poll(&mut self) -> bool {
        ...
        let guard = self.daemon.shared.lock().expect("lock poisoned");
...
mod tests {
    ...
    fn poll_drops_results_for_a_superseded_query() {
        ...
        *search.daemon.shared.lock().expect("lock poisoned") = SearchSnapshot {
