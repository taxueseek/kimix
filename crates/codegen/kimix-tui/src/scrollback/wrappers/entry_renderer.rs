...
impl<'a> EntryRenderer<'a> {
    ...
    /// cache size off-screen entries on a bulk load (`kimix -r`) without word-wrapping
...
mod tests {
    ...
    fn gutter_keeps_code_block_background_on_no_background_block() {
        ...
        // carries no code background while this test's local `kimixnight()`
    ...
    fn affordance_row_adds_one_row_per_diagram() {
        ...
        // invariant a bulk load (`kimix -r`) relies on to avoid clipping.
