    ...
    fn recap_output(&self, ctx: &BlockContext, summary: &str) -> BlockOutput {
        ...
        match ctx.mode {
            DisplayMode::Collapsed => {
                let mut spans = vec![Span::styled(i18n::tr("Recap").to_string(), header_style)];
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "kimix-4.5".into(),
            new_model_id: "kimix".into(),
            reason: "Model \"kimix-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"kimix-4.5\" is no longer available. Switched to \"kimix\"."
        );
    }
