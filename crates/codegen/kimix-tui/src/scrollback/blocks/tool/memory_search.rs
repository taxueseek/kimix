...
fn shorten_path(path: &str) -> &str {
    let memory_root = kimix_config::kimix_home().join("memory");
...
**File:** /root/.kimix/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.kimix/memory/MEMORY.md");
...
**File:** /root/.kimix/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.kimix/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured kimix memory root keep one trailing segment group.
        let memory_root = kimix_config::kimix_home().join("memory");
