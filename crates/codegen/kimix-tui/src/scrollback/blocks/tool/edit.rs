...
mod tests {
    ...
    /// Pins KimixNight via the shared test-lock guard; hold it for the whole
    /// test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_kimixnight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::kimixnight().diff_uses_line_fg(),
            "KimixNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_kimixnight_syntect` (lock is not reentrant).
...
"
    ...
    fn syntax_highlight_splits_keyword_and_string_fg() {
        let _guard = pin_kimixnight_syntect();
