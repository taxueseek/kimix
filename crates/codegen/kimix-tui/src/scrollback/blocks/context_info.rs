...
use kimix_shell::session::{ContextInfo, count_detail};
    ...
    fn build_lines(&self, theme: &Theme, bar: BarLayout) -> Vec<Line<'static>> {
        ...
        let mut legend_rows = vec![
            LegendRow {
                glyph: system_glyph,
                color: system_color,
                label: crate::i18n::tr_owned("System prompt"),
            ...
            LegendRow {
                ...
                label: crate::i18n::tr_owned("Messages"),
        ...
        if overhead_tokens > 0 {
            legend_rows.push(LegendRow {
                ...
                label: crate::i18n::tr_owned("Reasoning/overhead"),
        ...
        legend_rows.push(LegendRow {
            glyph: free_glyph,
            color: empty_color,
            label: crate::i18n::tr_owned("Free"),
            tokens: free_tokens,
            detail: None,
        });
        let info_rows: Vec<LegendRow> = std::iter::once(LegendRow {
            glyph: tools_glyph,
            color: tools_color,
            label: crate::i18n::tr_owned("Tool definitions"),
            tokens: tool_tokens,
...
mod tests {
    use super::*;
    use kimix_shell::session::TokenUsageCategory;
    ...
    fn build_lines_contains_header_tokens_and_model() {
        let block = ContextInfoBlock::new(snapshot(), "kimix-4");
        ...
        assert_eq!(line_text(&lines, 3), "kimix-4");
    }
    ...
    fn build_lines_includes_compaction_tip_in_warning_band() {
        ...
        snap.usage_pct = 80;
        let block = ContextInfoBlock::new(snap, "kimix-4");
        let theme = test_theme();
        let lines = block.build_lines(&theme, BarLayout::WIDE);
