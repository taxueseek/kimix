...
//! the generic [`CodeBlockSpan`](kimix_markdown::CodeBlockSpan) API) and
...
use kimix_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](kimix_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](kimix_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a kimix_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `KimixDay` is the only light theme; every other concrete theme (and the
/// `KimixNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `kimix_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::KimixDay)
}
...
mod tests {
    ...
    use kimix_markdown::StreamingMarkdownRenderer;
    ...
    #[test]
    fn theme_is_dark_maps_kimixday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::KimixDay),
            "KimixDay is the light theme"
        );
        for dark in [
            ThemeKind::KimixNight,
