...
pub fn context_bar_line(
    ...
    context_bar_line_for_session(used_tokens, total_tokens, hovered, theme, false, None)
}

/// Like [`context_bar_line`], but omits the bar for gateway/chat-kind sessions.
///
/// When `cache_hit_rate` is `Some(rate)` where `rate > 0`, appends a cache
/// hit rate indicator in the format `缓存XX%` (Chinese) or `cache XX%` (English).
...
mod tests {
    ...
    #[test]
    fn test_context_bar_with_cache_hit_rate() {
        // Default state with cache hit rate shows `used / total 缓存XX%`.
        let theme = Theme::default();
        ...
        assert!(
            text.contains("缓存42%"),
            "expected line to contain '缓存42%', got: {text:?}"
        );
    }
    ...
    #[test]
    fn test_context_bar_with_zero_cache_hit_rate() {
        // Default state with zero cache hit rate shows only `used / total`.
        let theme = Theme::default();
        let line = context_bar_line_for_session(
            Some(8_500),
            Some(1_000_000),
