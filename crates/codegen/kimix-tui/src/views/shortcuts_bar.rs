...
impl Widget for ShortcutsBar<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        if let Some(pending) = &self.pending_confirmation {
            ...
            let label = crate::i18n::tr_owned("press again to ")
                + crate::i18n::tr_cow(pending.label).as_ref();
        ...
        for (i, hint) in effective.iter().enumerate() {
            ...
            let hint_label = crate::i18n::tr_cow(hint.label.as_ref());
            let action_span = Span::styled(hint_label.as_ref(), action_style);
            let action_width = hint_label.width() as u16;
