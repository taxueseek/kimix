...
//! `Kimix/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `Kimix/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `Kimix/suggest` response.
...
// (e.g. kimix-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `Kimix/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `Kimix/suggest` response.
...
/// Wire `limit` for `Kimix/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `KIMIX_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `KIMIX_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `Kimix/suggest` request.
    /// Resolved at construction from `KIMIX_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: kimix_config::env_bool("KIMIX_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: kimix_config::env_bool("KIMIX_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("KIMIX_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `Kimix/suggest` response arrives, with the text and
