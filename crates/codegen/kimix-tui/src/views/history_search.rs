...
fn publish_matches(
    ...
    if query.is_empty() {
        ...
        let mut all: Vec<HistoryMatchResult> = items
            ...
            .collect();
        all.reverse();
        *out.lock().expect("lock poisoned") = Snapshot {
            items: all.into(),
            generation,
        };
    } else {
...
impl HistorySearchState {
    ...
    fn activate_inner(&mut self, history: &[HistoryEntry], current_text: &str, browse: bool) {
        ...
        self.snapshot = self.daemon.shared.lock().expect("lock poisoned").clone();
    ...
    pub fn poll(&mut self) -> bool {
        ...
        let snap = self.daemon.shared.lock().expect("lock poisoned").clone();
