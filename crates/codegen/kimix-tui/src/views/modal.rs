...
pub enum ActiveModal {
    ...
    SessionPicker {
        ...
        content_results: Option<Vec<kimix_shell::extensions::session_search::SearchSessionHit>>,
...
pub fn default_palette_entries() -> Vec<PaletteEntry> {
    ...
    // 按界面语言翻译条目与分区标题（指令与快捷键名保持英文）。
    if matches!(crate::i18n::current(), crate::i18n::Lang::Zh) {
        for entry in &mut entries {
            entry.label = crate::i18n::tr_owned(&entry.label);
            if let PaletteCommand::SectionHeader(h) = &mut entry.command {
                *h = crate::i18n::tr_owned(h);
            }
        }
    }
    entries
...
impl ActiveModal {
    ...
    pub fn message(&self, drain_blocked: bool) -> &str {
        match self {
            ...
            ActiveModal::CommandPalette { .. } => crate::i18n::tr("Commands"),
            ActiveModal::SessionPicker { .. } => crate::i18n::tr("Resume session"),
            ActiveModal::ArgPicker {
                command,
                args_query,
                ..
                ...
                "model" | "m" => crate::i18n::tr("Pick model"),
                "theme" | "t" => crate::i18n::tr("Pick theme"),
                "lang" => crate::i18n::tr("Pick language"),
                _ => crate::i18n::tr("Pick option"),
            },
            ActiveModal::DocPicker { .. } => crate::i18n::tr("How-to Guides"),
            ActiveModal::DocViewer { title, .. } => title.as_str(),
            ActiveModal::ShortcutsHelp { .. } => crate::i18n::tr("Keyboard Shortcuts"),
            ActiveModal::MemoryBrowser { .. } => crate::i18n::tr("Memory"),
...
fn fit_docs_ask_kimix_tip(docs_path: &str, width: usize) -> String {
    use crate::render::line_utils::truncate_str;
    if width == 0 {
        return String::new();
    }
    let long =
        format!("Tip · Ask Kimix about the docs ({docs_path}) — e.g. \"how do I set up MCP?\"");
    if long.width() <= width {
        return long;
    }
    let short = format!("Tip · Ask Kimix about the docs · {docs_path}");
...
pub fn render_doc_picker_overlay(
    ...
    if let Some(tip_rect) = tip_area {
        let docs_path = crate::util::display_user_kimix_path(DOCS_USER_GUIDE_REL);
        let tip_line = fit_docs_ask_kimix_tip(&docs_path, tip_rect.width as usize);
...
mod doc_picker_tip_tests {
    use super::{
        ActiveModal, DOCS_USER_GUIDE_REL, fit_docs_ask_kimix_tip, howto_list_modal,
    ...
    fn fit_docs_tip_prefers_path_and_never_overflows() {
        let path = crate::util::display_user_kimix_path(DOCS_USER_GUIDE_REL);
        let long =
            format!("Tip · Ask Kimix about the docs ({path}) — e.g. \"how do I set up MCP?\"");
        let short = format!("Tip · Ask Kimix about the docs · {path}");
        let path_only = format!("Tip · {path}");
        assert_eq!(fit_docs_ask_kimix_tip(&path, long.width()), long);
        assert_eq!(fit_docs_ask_kimix_tip(&path, short.width()), short);
        assert_eq!(fit_docs_ask_kimix_tip(&path, path_only.width()), path_only);
        for w in [5usize, 12, 20, 30] {
            let line = fit_docs_ask_kimix_tip(&path, w);
    ...
    fn doc_picker_renders_tip_with_path() {
        ...
        assert!(
            all.contains("Tip") && all.contains("Ask Kimix"),
            "missing tip footer:\n{all}"
        );
        assert!(
