...
use kimix_shell::tools::{TodoItem, TodoStatus};
...
pub fn render_goal_detail(
    ...
    let mut status_spans = vec![
        Span::styled(i18n::tr("Status: "), Style::default().fg(theme.gray)),
        Span::styled(
            status_text,
            Style::default()
                .fg(status_color)
...
mod tests {
    ...
    fn render_per_model_breakdown_shows_each_model() {
        ...
        goal.live_tokens_by_model =
            vec![("kimix-4".into(), 12_300), ("kimix-3-mini".into(), 8_000)];
        let text = render_to_text(&goal);
        assert!(text.contains("kimix-4"), "first model must render:\n{text}");
        assert!(
            text.contains("kimix-3-mini"),
