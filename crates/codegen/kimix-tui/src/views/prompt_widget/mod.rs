...
//!  kimix-3 · yolo                             ← info line (optional)
//! ```
//!
...
use kimix_ratatui_textarea::{ElementId, ElementKind, TextArea, TextAreaState, TextElement};
...
pub struct PromptWidget {
    ...
    hovered_image_element_id: Option<kimix_ratatui_textarea::ElementId>,
    /// Image shown immediately after insertion without moving the edit cursor.
    post_insert_image_preview: Option<(kimix_ratatui_textarea::ElementId, usize)>,
        ...
        let applied = if snap.cursor_in_command {
            ...
            if let Some(ref range) = snap.command_range {
                    ...
                    && self.textarea.text().is_char_boundary(range.end)
                {
                    ...
                    // `/mod kimix-4` yields `/model kimix-4`, not `/model  kimix-4`
            ...
            // Replace the args range (e.g., "gr" → "Kimix 4 Fast").
...
impl PromptWidget {
    ...
    pub fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> PromptEvent {
        use kimix_ratatui_textarea::{MouseAction, TextElementEventKind};
    ...
    fn image_element_near_cursor(&self) -> Option<&kimix_ratatui_textarea::TextElement> {
    ...
    pub fn textarea(&self) -> &kimix_ratatui_textarea::TextArea {
        &self.textarea
    }
    ...
    pub fn draw(
        ...
        if self.textarea.text().is_empty() && ta_area.width > 0 && !style.focused {
            let placeholder = style
                ...
                .unwrap_or(crate::i18n::tr("Build anything"));
            buf.set_string(
                ta_area.x,
                ta_area.y,
                placeholder,
...
fn strip_all_elements(
    text: &str,
    cursor: usize,
    textarea: &kimix_ratatui_textarea::TextArea,
) -> (String, usize) {
...
fn map_clean_offset_to_raw(
    text: &str,
    clean_offset: usize,
    elements: &[kimix_ratatui_textarea::TextElement],
) -> usize {
    if elements.is_empty() {
