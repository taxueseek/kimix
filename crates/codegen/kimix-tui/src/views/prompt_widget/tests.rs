#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/kimix-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/kimix-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/kimix-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/kimix-test-image.png"),
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod kimix-4");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn accept_arg_completion_replaces_arg_range() {
        ...
        let model_id = agent_client_protocol::ModelId::new(Arc::from("kimix-4.5"));
        models.available.insert(
            model_id.clone(),
            agent_client_protocol::ModelInfo::new(model_id, "Kimix 4.5".to_string()),
        );
        ...
        // Accept arg completion → should replace "ki" with "Kimix 4.5".
        pw.accept_slash_completion(&models);
        let text = pw.textarea.text().to_string();
        assert!(
            text.contains("Kimix 4.5"),
    ...
    fn test_image() -> PastedImage {
        PastedImage {
            element_id: kimix_ratatui_textarea::ElementId::from_raw(0), // overwritten by insert_image
            display_number: 0,
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> kimix_workspace::file_system::FuzzyMatchResult {
        kimix_workspace::file_system::FuzzyMatchResult {
    ...
    /// after a turn ends with `Kimix/suggestPrompt` resolved.
