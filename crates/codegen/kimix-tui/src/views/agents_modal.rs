...
use kimix_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use kimix_shell::agent::config::AgentSelectionConfig;
...
/// User-level vs project-level config files (`~/.kimix` vs `{cwd}/.kimix`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `KIMIX_AGENT`,
    /// model `agentType`, then `kimix`).
...
/// Skips internal variants (KimixConcise, KimixPlan,
/// KimixPlanNoSubagents, KimixAskUser, Codex, Opencode,
/// CursorExtended, KimixOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = kimix_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.kimix/personas` and `{cwd}/.kimix/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = kimix_home.join("bundled").join("personas");
    ...
    let dirs = [
        (
            ...
            cwd.join(".kimix").join("personas"),
        ),
        (ConfigFileScope::User, kimix_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match kimix_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => kimix_config::kimix_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".kimix").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.kimix/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.kimix/{subdir}` or `{cwd}/.kimix/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let kimix_home = kimix_config::kimix_home();
    let in_user = dunce::canonicalize(kimix_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".kimix").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    kimix_shell::config::load_effective_config()
        .ok()
        .and_then(|root| kimix_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in kimix-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    kimix_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn set_default_agent(name: Option<&str>) -> Result<(), String> {
    let config_path = kimix_config::kimix_home().join("config.toml");
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        kimix_agent::config::PromptMode::Extend => "extend",
        kimix_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use kimix_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".kimix").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".kimix").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.kimix/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".kimix").join("personas").join("gone.toml");
