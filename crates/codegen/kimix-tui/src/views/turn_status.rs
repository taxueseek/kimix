...
use kimix_workspace::permission::mcp_pretty_name_if_qualified;
...
fn render_starting_session(
    ...
    let spans = vec![
        ...
        Span::styled(i18n::tr("Starting session…"), style),
...
mod tests {
    ...
    /// Serialises tests in this module that depend on the process-wide i18n
    /// language, so they don't run concurrently with language-switching tests.
    static LANG_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    /// Pin the i18n language to English so assertions on rendered text pass
    ...
    fn idle_zero_server_seed_renders_starting_session() {
        let _g = LANG_LOCK.lock().expect("lock poisoned");
        force_en();
