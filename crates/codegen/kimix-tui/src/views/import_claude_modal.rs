...
//! are written to `.kimix/config.toml`.
use crossterm::event::{KeyCode, KeyEvent, KeyModifiers};
use kimix_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
...
impl ItemKind {
    ...
    fn label(&self) -> &'static str {
        match self {
            Self::Permission => crate::i18n::tr("Permissions"),
            Self::EnvVar => crate::i18n::tr("Env vars"),
            Self::McpServer => "MCP servers",
            Self::Hook => crate::i18n::tr("Hooks"),
            Self::PathEntry => crate::i18n::tr("Paths"),
        }
    }
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.kimix/config.toml".to_string();
    ...
    if !plan.project_items.is_empty() {
        ...
        let project_config = find_project_root(cwd).join(".kimix").join("config.toml");
...
mod tests {
    use super::*;
    use kimix_shell::claude_import::PathKind;
    use kimix_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use kimix_shell::claude_import::ImportableItem;
