...
use kimix_tools::implementations::skills::types::SkillInfo;
...
fn fuzzy_matches(name: &str, query: &str) -> bool {
    ...
    let haystack = nucleo::Utf32String::from(name);
...
pub fn fuzzy_matches_hook(hook: &kimix_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<kimix_hooks_plugins_types::PluginOrigin>,
) -> kimix_hooks_plugins_types::PluginInfo {
    kimix_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: kimix_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: kimix_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: kimix_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    (u8, String, String),
    Vec<(usize, &'a kimix_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &kimix_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use kimix_hooks_plugins_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectKimix) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserKimix) => PluginGroup::new(2, "origin:user", "User"),
...
fn build_hook_groups<'a>(
    hooks: &'a [kimix_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[kimix_hooks_plugins_types::HookInfo],
    current: usize,
...
pub enum ButtonAction {
    ...
    HooksAction(kimix_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(kimix_hooks_plugins_types::PluginsAction),
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use kimix_hooks_plugins_types::{HooksAction, PluginsAction};
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use kimix_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<kimix_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<kimix_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<kimix_hooks_plugins_types::PluginsAction>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[kimix_hooks_plugins_types::PluginInfo]) {
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let kimix = kimix_config::kimix_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user kimix home (KIMIX_SHARE_DIR-aware)
    // or a project-scoped `{cwd}/.kimix/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User kimix home (KIMIX_SHARE_DIR-aware).
        if let Ok(rest) = source_path.strip_prefix(kimix.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.kimix/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".kimix" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    if let Some(name) = plugin_name("plugins").or_else(|| plugin_name("installed-plugins")) {
        return (format!("Plugin: {name}"), false);
    }
    // Global hooks under $KIMIX_SHARE_DIR/hooks
    let global_hooks = kimix.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.kimix/hooks") || source_dir.contains("/.kimix/hooks/") {
        return ("Project hooks".into(), false);
    }
    // Custom directory — removable
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&kimix) {
            let prefix = crate::util::display_kimix_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            kimix_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_kimix_home(path) {
                    crate::util::display_user_kimix_path("skills")
            ...
            kimix_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            kimix_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &kimix_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use kimix_hooks_plugins_types::McpStatus;
...
pub fn render_extensions_modal(
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &kimix_hooks_plugins_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
    ...
    if modal_msg_kind.is_some() {
        ...
        // Tab cycles tabs (handled in picker.rs). Click on
        // the hint cycles to the next tab only — sentinel id 98,
        // dispatched in `handle_extensions_modal_mouse`.
        shortcuts.push(Shortcut {
            label: "Tab tabs",
            clickable: true,
            id: 98,
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.kimix/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // kimix-home branch is KIMIX_SHARE_DIR-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.kimix/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.kimix/installed-plugins/vendor-abc123/skills");
    ...
    fn make_skill(
        ...
        kimix_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: kimix_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = kimix_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn make_plugin(name: &str) -> kimix_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: kimix_hooks_plugins_types::PluginOrigin,
    ) -> kimix_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(kimix_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    kimix_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    kimix_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                kimix_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> kimix_hooks_plugins_types::HookInfo {
        kimix_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: kimix_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: kimix_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn plugins_modal_state(
        plugins: Vec<kimix_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(kimix_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use kimix_hooks_plugins_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectKimix, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserKimix, 2, "origin:user", "User"),
            (
            ...
            (
                PluginOrigin::MarketplaceInstall {
                    source_name: Some("Kimix Official".into()),
                    git_url: Some("https://example.com/r.git".into()),
                },
                5,
                "kimix-mp:Kimix Official",
                "Kimix Official",
            ),
            (
                PluginOrigin::MarketplaceInstall {
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = kimix_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = kimix_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = kimix_hooks_plugins_types::PluginScope::Config;
        assert_eq!(plugin_group(&config).key, "origin:config");

        let mut mp = make_plugin("mp-tool");
        mp.marketplace_source = Some("Kimix Official".into());
        let group = plugin_group(&mp);
        assert_eq!(group.key, "kimix-mp:Kimix Official");
        assert_eq!(group.label, "Kimix Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            kimix_hooks_plugins_types::PluginOrigin::Unknown,
        );
        assert_eq!(plugin_group(&unknown).key, "origin:user");

        unknown.marketplace_source = Some("Kimix Official".into());
        assert_eq!(plugin_group(&unknown).key, "kimix-mp:Kimix Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserKimix),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use kimix_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserKimix),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use kimix_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserKimix);
    ...
    fn plugins_fallback_grouping_without_origin() {
        ...
        assert_eq!(buffer_count(&buf, "User (1 plugin)"), 1);
        assert_eq!(buffer_count(&buf, "Kimix Official (1 plugin)"), 1);
