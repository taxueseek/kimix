...
pub(crate) fn location_line_at(theme: &Theme, cwd: &Path) -> Line<'static> {
    ...
    if let Some(branch) = info.as_ref().and_then(|i| i.branch.as_deref()) {
        ...
        let git_text = if branch.is_empty() {
            format!("{icon} {}", crate::i18n::tr("detached"))
        } else {
            format!("{icon} {branch}")
        };
...
mod tests {
    ...
    /// Serialises tests in this module: they mutate the process-wide i18n
    /// language, so they must not run concurrently with each other.
    static LANG_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    /// Assertions check the English rendering; pin the language so they pass
    ...
    /// zh 模式：后缀翻译为中文（全角括号）。
    #[test]
    fn format_cwd_worktree_zh() {
        let _g = LANG_LOCK.lock().expect("lock poisoned");
        crate::i18n::set_current(crate::i18n::Lang::Zh);
        assert_eq!(
            format_cwd_parts("~/wt/session-1", Some("~/xai")),
            "~/wt/session-1（~/xai 的工作树）"
        );
        force_en();
    }
