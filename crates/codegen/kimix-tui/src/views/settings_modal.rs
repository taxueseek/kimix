...
use kimix_shell::agent::config::UiConfig;
                    ...
                    *supports_preview,
                    effective_enum_choices(key, choices, &self.pager_snapshot)
                        ...
                        .map(|c| OwnedEnumChoice {
                            ...
                            display: crate::i18n::tr(c.display).to_string(),
                            description: crate::i18n::tr(c.description).to_string(),
                        })
                        .collect(),
                ),
...
fn action_for_enum(key: SettingKey, choice: &'static str) -> Option<Action> {
    match key {
        ...
        "scroll_mode" => None,
        // Language applies immediately on commit (runtime i18n switch);
        // no per-keystroke preview path.
        "language" => None,
        _ => None,
    }
}
...
fn action_for_enum_commit(key: SettingKey, choice: &'static str) -> Option<Action> {
    match key {
        ...
        // Language switch is immediate: runtime `i18n::set_current` + toast +
        // persist `[ui].language` (handled by `dispatch::settings::set_language`).
...
fn render_docs_footer(buf: &mut Buffer, area: Rect, theme: &Theme) {
    const LONG: &str =
        "Tip · Ask Kimix: \"change theme to kimixday\" or \"what does compact mode do?\"";
    const SHORT: &str = "Tip · Ask Kimix to change a setting";
...
fn wrapped_description_height(meta: &SettingMeta, area_width: u16, cap: u16) -> u16 {
    ...
    let line = Line::from(Span::raw(crate::i18n::tr(meta.description)));
...
/// Kimix STT language list (25 codes + client-only `auto` = 26) with headroom.
...
fn render_sub_pane_header(
    ...
    // Translate title/description (Cow borrows when EN / untranslated).
    let title_tr = crate::i18n::tr_cow(title);
    let title: &str = &title_tr;
    let description_tr = crate::i18n::tr_cow(description);
    let description: &str = &description_tr;
...
fn render_picking_group(
    ...
    for (i, child_key) in children.iter().enumerate() {
        ...
        if value_x > label_x {
            ...
            let child_label = crate::i18n::tr(child_meta.label);
...
fn compute_settings_max_label_w(metas: &[SettingMeta], content_w: u16) -> u16 {
    ...
    metas
        .iter()
        .map(|m| crate::i18n::tr(m.label).width() as u16)
        .max()
        .unwrap_or(0)
        .min(cap)
}
    ...
    if let SettingKind::Enum { choices, .. } = kind {
        for c in *choices {
            if c.canonical == canonical {
                return crate::i18n::tr(c.display);
            }
        }
    }
...
fn row_layout(
    ...
    // Translate so CJK labels measure their rendered width.
    let label = crate::i18n::tr_cow(label);
...
fn render_setting_row(
    ...
    match layout {
        RowLayout::OneLine => {
            ...
            let label_text = format!("{triangle} {}", crate::i18n::tr(meta.label));
            ...
            let label_text: &str = match layout {
                RowLayout::TwoLineWithLabelTruncation => {
                    ...
                    if label_avail == 0 {
                        ...
                        label_text_owned =
                            truncate_str(crate::i18n::tr(meta.label), label_avail as usize);
                        &label_text_owned
                    }
                }
                _ => crate::i18n::tr(meta.label),
            };
...
fn render_expanded_description(buf: &mut Buffer, area: Rect, meta: &SettingMeta, theme: &Theme) {
    ...
    let desc_style = Style::default()
        ...
        .bg(theme.bg_base)
        .add_modifier(Modifier::ITALIC);
    let desc_src: &str = crate::i18n::tr(meta.description);
...
fn render_setting_row_no_value(
    ...
    let label = crate::i18n::tr(meta.label);
    let label_truncated: std::borrow::Cow<'_, str> = if label.width() <= label_max_w as usize {
        std::borrow::Cow::Borrowed(label)
    } else {
        std::borrow::Cow::Owned(truncate_str(label, label_max_w as usize))
    };
...
fn build_shortcuts(state: &SettingsModalState) -> Vec<Shortcut<'static>> {
    match state.mode {
        ...
        SettingsModalMode::FilterFocused => vec![
            ...
            Shortcut {
                label: i18n::tr("Backspace edit"),
                clickable: false,
                id: 0,
            },
            Shortcut {
                label: i18n::tr("Enter commit"),
                clickable: false,
                id: 0,
            },
            Shortcut {
                label: i18n::tr("Esc clear"),
                clickable: false,
                id: 0,
            },
        ],
...
mod tests {
    ...
    fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
        ...
        let snapshot = PagerLocalSnapshot {
            available_models: vec![(
                "Kimix Test".to_string(),
                acp::ModelId::new(Arc::from("kimix-test")),
            )],
            ..PagerLocalSnapshot::default()
        };
    ...
    fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
        let mut s = editor_render_fixture("Kimix Test", 4); // cursor between "Kimix" and " Test"
        let area = Rect {
            x: 0,
            y: 0,
            width: 60,
    ...
    fn picking_enum_esc_dispatches_preview_revert_for_each_key() {
        let cases: &[(&str, &str)] = &[
            ("theme", "kimixnight"),
            ("auto_dark_theme", "kimixnight"),
            ("auto_light_theme", "kimixday"),
        ];
    ...
    fn picking_enum_esc_returns_to_browse() {
        ...
        s.mode = SettingsModalMode::PickingEnum {
            key: "theme",
            choices_idx: 0,
            original_value: SettingValue::Enum("kimixnight"),
            supports_preview: true,
        };
        let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
        match outcome {
            ...
            other => panic!("expected Action::PreviewTheme(\"kimixnight\") on Esc, got {other:?}"),
        }
        assert!(matches!(s.mode, SettingsModalMode::Browse));
    }
    ...
    fn picker_visual_smoke_debug() {
        let entries = vec![SettingMeta {
            ...
            description: "Controls whether Kimix may retain and train on coding data.",
            keywords: &["test"],
            kind: SettingKind::Enum {
                default: "opt-out",
                choices: &[
                    EnumChoice {
                        canonical: "opt-in",
                        display: "Opt in",
                        description: "Allow Kimix to retain and use coding session data for training and product improvement.",
                    },
                    EnumChoice {
                        canonical: "opt-out",
                        display: "Opt out",
    ...
    fn picker_multi_line_choice_hit_rect_spans_all_lines() {
        ...
        let entries = vec![SettingMeta {
            ...
            owner: SettingOwner::Shared,
            label: "Coding data sharing",
            description: "Controls whether Kimix may retain coding data.",
            keywords: &["test"],
            kind: SettingKind::Enum {
                default: "opt-in",
                choices: &[
    ...
    fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
        ...
        // (catalog has 'Kimix 4 Fast'; "a" doesn't match) is
    ...
    fn editing_value_esc_returns_to_browse() {
        ...
        s.mode = SettingsModalMode::EditingValue {
            key: "default_model",
            buffer: "kimix-4".to_string(),
            cursor_byte: "kimix-4".len(),
            validation_error: None,
        };
        let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
        assert!(matches!(outcome, SettingsKeyOutcome::Changed));
        ...
        for (i, r) in s.rows.iter().enumerate() {
            ...
            let expected_substring = match r {
                ...
                RowEntry::Setting { meta_index, .. } => {
                    crate::i18n::tr(s.registry.all()[*meta_index].label).to_string()
                }
            };
            assert!(
    ...
    //    the "Tip · Ask Kimix…" docs footer and the keybindings hints.
    //
    ...
    fn docs_footer_tip_is_centered() {
        ...
        // SHORT = "Tip · Ask Kimix to change a setting" (34 cells);
        ...
        assert!(
            !row_short.contains("kimixday"),
            "width=40 must NOT render LONG path (no `kimixday`): {row_short:?}",
        );
        assert!(
    ...
    fn tip_line_has_blank_row_above() {
        ...
        for y in 0..area.height {
            let txt = buf_row_text(&buf, y, area.x, area.width);
            if txt.contains("Tip") && txt.contains("Ask Kimix") {
                tip_y = Some(y);
                break;
            }
        }
    ...
    fn settings_breadcrumb_rect_set_in_sub_pane_modes() {
        ...
        let title_w = crate::i18n::tr(MODAL_TITLE).width();
        let prefix_w = title_w + " \u{203A} ".width();
        assert!(
            (rect.width as usize) > title_w,
    ...
    fn click_settings_breadcrumb_collapses_picker_to_browse() {
        ...
        // `"kimixnight"`. Tightened from the previous `Action(_) |
        ...
        match outcome {
            ...
            other => panic!(
                "expected Action(PreviewTheme(\"kimixnight\")) — the keyboard \
    ...
    fn click_settings_breadcrumb_after_nav_reverts_to_original() {
        ...
        // Pick a different index. The default theme is `kimixnight`
    ...
    fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
        ...
        match outcome {
            SettingsKeyOutcome::ActionPair(
                ...
                // Default theme is `kimixnight`; entering the picker
                // captures `original_value = current value = kimixnight`,
    ...
    /// misleading: on dark themes (KimixNight, TokyoNight, RosePine
    /// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
    /// only on the KimixDay light theme is title darker. The
    ...
    fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
        ...
        let raw_theme = match crate::theme::Theme::current_kind() {
            crate::theme::ThemeKind::KimixNight => crate::theme::Theme::kimixnight(),
            crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
            crate::theme::ThemeKind::KimixDay => crate::theme::Theme::kimixday(),
            ...
            // sakura/forest 的构造函数在私有模块中，同 OscuraMidnight 走 current()。
            crate::theme::ThemeKind::Sakura | crate::theme::ThemeKind::Forest => {
                crate::theme::Theme::current()
            }
            // moon/grok/deepseek 主题的构造函数同在私有模块中，走 current()。
            crate::theme::ThemeKind::MoonDark
            | crate::theme::ThemeKind::MoonLight
            | crate::theme::ThemeKind::BloodMoon
            | crate::theme::ThemeKind::GrokDark
            | crate::theme::ThemeKind::DeepSeekBlue => crate::theme::Theme::current(),
            crate::theme::ThemeKind::Auto => crate::theme::Theme::kimixnight(),
        };
        assert_ne!(
