...
/// Env override for the whole feature: `KIMIX_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "KIMIX_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `KIMIX_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "KIMIX_PROMPT_SUGGESTIONS_MODEL";
...
pub struct PromptSuggestionController {
    ...
    /// `KIMIX_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| kimix_config::env_bool(PROMPT_SUGGESTIONS_ENV))
...
/// env override > `kimix-0.1` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `kimix-0.1` default): the shell
