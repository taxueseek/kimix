...
//! loop. Recipe: `KIMIX_FPS=1 KIMIX_SCROLL_DEBUG=1 kimix --resume <session>`,
//! then flip `scroll_mode` / `scroll_lines` / `invert_scroll` /
//! `scroll_speed` in `/settings` to compare variants live. For event-exact
//! capture beyond this per-frame sampling, add `KIMIX_SCROLL_LOG=1` — the
...
/// `KIMIX_SCROLL_DEBUG` (nonempty and not `"0"`) enables at startup, and the
...
impl ScrollDebugHud {
    pub fn new() -> Self {
        let env_on = std::env::var("KIMIX_SCROLL_DEBUG").is_ok_and(|v| !v.is_empty() && v != "0");
        Self { enabled: env_on }
    }
...
pub struct ScrollDebugPanel {
    ...
    /// Rows left free for FPS overlays stacked above (the dev `KIMIX_FPS`
