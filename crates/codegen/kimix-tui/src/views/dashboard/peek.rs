...
pub struct PeekPanelState {
    ...
    /// the bottom border (so the Tab mode cycle's three states are
    /// all visible). Set live by the render-time refresh.
    pub plan_mode: bool,
    /// Whether the peeked agent is in Decision mode.
    pub decision_mode: bool,
}
    ...
    match row {
        DashboardRowId::TopLevel(id) => match agents.get(id) {
            Some(agent) => {
                ...
                let decision = agent.decision_mode_active;
                PeekModeBadge {
...
fn paint_peek_config_badge(
    ...
    // permission mode (the gate in kimix-shell), so `plan` alone is the
...
mod tests {
    ...
    fn render_peek_shows_model_and_auto_approve_on_bottom_border() {
        ...
        panel.model_name = Some("Kimix 4 Fast".to_string());
        panel.auto_approve = true;
        let bottom = badge_row(&panel, 6);
        assert!(
            bottom.contains("Kimix 4 Fast"),
        ...
        qpanel.model_name = Some("Kimix 4 Fast".to_string());
        let qbottom = badge_row(&qpanel, 8);
        assert!(
            qbottom.contains("Kimix 4 Fast"),
        ...
        plain.model_name = Some("Kimix 4 Fast".to_string());
        ...
        planp.model_name = Some("Kimix 4 Fast".to_string());
