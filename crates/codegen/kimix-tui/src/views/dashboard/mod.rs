...
///
/// Order: env override (`KIMIX_AGENT_DASHBOARD=0` → off) wins, else the
...
pub fn dashboard_enabled() -> bool {
    if std::env::var_os("KIMIX_AGENT_DASHBOARD")
        .as_deref()
...
mod tests {
    ...
    /// `KIMIX_AGENT_DASHBOARD` env-mutating tests.
    ...
    fn switch_hint_minimal_is_resume_even_with_dashboard_disabled() {
        ...
        // `KIMIX_AGENT_DASHBOARD` key reads it concurrently.
    ...
    fn switch_hint_non_minimal_follows_dashboard_flag() {
        // SAFETY: see above — serialized on the KIMIX_AGENT_DASHBOARD key.
        unsafe { std::env::set_var("KIMIX_AGENT_DASHBOARD", "0") };
