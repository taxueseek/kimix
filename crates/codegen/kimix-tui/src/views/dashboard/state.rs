...
/// `~/.kimix/config.toml`. Lenient — corrupted fields fall back to
...
/// view's Tab cycle (Normal → Plan → Always-Approve → Normal).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
...
impl DashboardDispatchMode {
    ...
    pub fn cycle(self) -> Self {
        match self {
            ...
            Self::Plan => Self::Decision,
            Self::Decision => Self::Normal,
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<kimix_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
...
/// Load the full persisted dashboard from `~/.kimix/config.toml`.
///
...
/// `~/.kimix/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
pub fn write_persisted(p: &PersistedDashboard) -> std::io::Result<()> {
    let path = config_path()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no kimix home"))?;
...
mod tests {
    ...
    fn compact_cwd_strips_home() {
        let p = Path::new("/Users/alice/projects/kimix");
        assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/kimix");
    }
    ...
    /// per-key serial lock. The `KIMIX_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(KIMIX_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `KIMIX_AGENT_DASHBOARD` key reads it concurrently.
