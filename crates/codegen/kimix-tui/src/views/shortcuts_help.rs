...
pub fn build_entries(
    ...
    for (cat_idx, &(cat, label)) in CATEGORY_ORDER.iter().enumerate() {
        ...
        if vim_mode && cat == Category::ConversationNav {
            ...
            item.description = Some(i18n::tr("Search scrollback").into());
        ...
        if cat == Category::Input {
            ...
            item.description = Some(i18n::tr("Paste images (and text) from the clipboard").into());
            #[cfg(target_os = "windows")]
...
pub fn entry_display(entries: &[ShortcutsHelpEntry], idx: usize) -> (String, String) {
    match entries.get(idx) {
        ...
        Some(ShortcutsHelpEntry::SectionHeader { label, .. }) => {
            (i18n::tr(label).to_string(), String::new())
        }
...
fn hint_description(h: &HintItem) -> String {
    h.description
        .as_ref()
        .map(|d| i18n::tr_owned(d.as_ref()))
        .unwrap_or_else(|| {
            // Capitalize the short label as a fallback (already translated).
            let label = i18n::tr_cow(h.label.as_ref());
...
pub fn modal_footer_detail() -> Vec<crate::views::modal_window::Shortcut<'static>> {
    ...
    vec![
        ...
        Shortcut {
            label: i18n::tr("\u{2191}/\u{2193} scroll"),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: i18n::tr("Ctrl+./X close"),
            clickable: false,
            id: 0,
        },
    ]
...
pub fn render_detail_body<'a>(
    ...
    if dimmed_note {
        lines.push(Line::from(""));
        lines.push(Line::from(Span::styled(
            i18n::tr("(not active in current context)"),
...
pub fn modal_footer(filter_active: bool) -> Vec<crate::views::modal_window::Shortcut<'static>> {
    ...
    let mut shortcuts = vec![
        Shortcut {
            label: i18n::tr("\u{2191}/\u{2193} nav"),
            clickable: false,
            id: 0,
        },
        Shortcut {
        ...
        Shortcut {
            label: i18n::tr("e/Space/\u{2192} expand"),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: i18n::tr("\u{2190} collapse"),
            clickable: false,
            id: 0,
        },
        Shortcut {
            label: i18n::tr("Enter details"),
            clickable: false,
            id: 0,
        },
        Shortcut {
