...
use kimix_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: kimix_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `kimix-shell`, NOT duplicated locally.
...
pub fn render_permission_view(
    ...
    if show_scope_hint && y < area.y + area.height {
        ...
        let hint_line = Line::from(vec![
            Span::styled(i18n::tr("Use "), hint_style),
...
/// ```text
///  3 [ ] Tell Kimix what to do differently
...
mod tests {
    ...
    fn empty_view_state(mcp_scope: Option<McpScopeState>) -> PermissionViewState {
        ...
        let perm = kimix_acp_lib::AcpArgs {
            request,
            response_tx,
        };
        PermissionViewState {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from kimix\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from kimix\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p kimix-tui --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/kimix-tui
