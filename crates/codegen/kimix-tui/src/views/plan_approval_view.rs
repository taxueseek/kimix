...
pub use kimix_tools::implementations::kimix::exit_plan_mode::{
...
pub fn plan_approval_status_label(has_plan: bool) -> &'static str {
    if has_plan {
        crate::i18n::tr("Waiting on plan approval")
    } else {
        crate::i18n::tr("No plan written — approve or request changes")
    }
}
