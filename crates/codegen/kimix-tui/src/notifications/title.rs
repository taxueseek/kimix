...
impl TitleManager {
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("kimix");
        self.last_title.clear();
        self.last_title.push_str("kimix");
...
mod tests {
    ...
    #[test]
    fn kimix_only_produces_just_kimix() {
        let cfg = config_with_items(vec![TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        let state = idle_state();
        mgr.update(&state);
        assert_eq!(mgr.last_title, "kimix");
    }

    #[test]
    fn session_name_and_kimix_joined_with_separator() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - kimix");
    }
    ...
    fn spinner_only_shown_when_active() {
        let cfg = config_with_items(vec![TitleItem::Spinner, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - kimix"));
    ...
    fn activity_hidden_when_idle() {
        let cfg = config_with_items(vec![TitleItem::Activity, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        mgr.update(&idle_state());
        assert_eq!(mgr.last_title, "kimix");
    }
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - kimix");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - kimix");
    }
    ...
    fn action_required_visible_on_first_tick() {
        let cfg = config_with_items(vec![TitleItem::ActionRequired, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    #[test]
    fn empty_items_produces_kimix_fallback() {
    ...
    fn model_item_shown_when_present() {
        let cfg = config_with_items(vec![TitleItem::Model, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
            model: Some("kimix-3"),
            ..idle_state()
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "kimix-3 - kimix");
    }
    ...
    fn cwd_shows_last_component() {
        let cfg = config_with_items(vec![TitleItem::Cwd, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    fn turn_timer_shown_when_above_one_second() {
        let cfg = config_with_items(vec![TitleItem::TurnTimer, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - kimix");
    }
    ...
    #[test]
    fn reset_clears_state_and_emits_kimix() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Kimix]);
        let mut mgr = TitleManager::new(&cfg);
        let activity = TurnActivity::Thinking;
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "kimix");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("kimix"), "title missing 'kimix': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - kimix-3 - workspace - kimix"
        );
    }
