...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("KIMIX_EVENT", event_str)
        .env("KIMIX_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("KIMIX_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'KIMIX_EVENT=%s\\nKIMIX_MESSAGE=%s\\nKIMIX_SESSION_ID=%s\\n' \
             \"$KIMIX_EVENT\" \"$KIMIX_MESSAGE\" \"$KIMIX_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("KIMIX_EVENT=Turn complete"),
            "missing KIMIX_EVENT: {content}"
        );
        assert!(
            content.contains("KIMIX_MESSAGE=hello world"),
            "missing KIMIX_MESSAGE: {content}"
        );
        assert!(
            content.contains("KIMIX_SESSION_ID=sess-42"),
            "missing KIMIX_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("KIMIX_SESSION_ID"),
            "KIMIX_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("KIMIX_EVENT=Turn complete"));
        assert!(content.contains("KIMIX_MESSAGE=test body payload"));
        assert!(content.contains("KIMIX_SESSION_ID=test-session-123"));
    }
}
