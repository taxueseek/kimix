...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"kimixnight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"Kimix Night"`).
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `KIMIX_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        // language: read the persisted `[ui].language` value; unset / "auto" /
        // unrecognised → "auto" (follow system). Reflects the stored choice
        // (not the resolved runtime language) so the picker shows "Auto" when
        // the user has not overridden the system locale.
        "language" => {
            let raw = kimix_shell::config::load_effective_config()
                .ok()
                .and_then(|root| {
                    root.get("ui")
                        .and_then(|ui_table| ui_table.get("language"))
                        .and_then(|lang| lang.as_str())
                        .map(str::to_lowercase)
                });
            let canonical = match raw.as_deref() {
                Some(v) if crate::i18n::Lang::parse(v).is_some() => {
                    // `parse` succeeded above; unwrap is safe.
                    crate::i18n::Lang::parse(v).unwrap().code()
                }
                _ => "auto",
            };
            Some(SettingValue::Enum(canonical))
        }
        // fork_secondary_model: baseline value folds to empty string.
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = kimix_shell::models::default_model();
...
mod tests {
    ...
    fn defaults_match_ui_config_default() {
        ...
        for meta in reg.all() {
            ...
            match (meta.key, &meta.kind) {
                ...
                // language: no UiConfig mirror (read directly from
                // `[ui].language` via `load_effective_config`); "auto" =
                // follow system locale, resolved by `i18n::Lang::detect`.
                ("language", SettingKind::Enum { default, .. }) => {
                    assert_eq!(
                        *default, "auto",
                        "language registry default must be 'auto' \
                         (follow system locale; i18n::config_language skips it)"
                    );
                }
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in kimix-tools"
                    );
                }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: kimix_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
