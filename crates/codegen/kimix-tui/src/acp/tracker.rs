...
use kimix_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use kimix_tools::util::strip_redundant_session_cd;
use std::borrow::Cow;
...
pub fn format_waiting_for_subject(subject: &str) -> String {
    ...
    if clamped.is_empty() {
        crate::i18n::tr("Waiting on task output…").to_string()
    } else {
        format!("{clamped}…")
    }
}
impl WaitingReason {
    ...
    pub fn label(&self) -> String {
        match self {
            Self::Model => crate::i18n::tr("Waiting for response…").to_string(),
            Self::Subagent => crate::i18n::tr("Waiting on subagent…").to_string(),
            Self::TaskOutput {
                subject: Some(subject),
                ..
            } => format_waiting_for_subject(subject),
            Self::TaskOutput { .. } => crate::i18n::tr("Waiting on task output…").to_string(),
            Self::TasksComplete => crate::i18n::tr("Waiting on tasks…").to_string(),
            Self::Sleep => crate::i18n::tr("Sleeping…").to_string(),
        }
    }
...
pub struct AcpUpdateTracker {
    ...
    /// `Kimix/task_backgrounded` creates a `BgTask` block.
...
impl AcpUpdateTracker {
    ...
    fn handle_user_message(
        ...
        let mut block = if let Some(dt) = display_override {
            ...
            let skill_display =
                kimix_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && kimix_shell::session::PromptOrigin::from_prompt_id(pid).hide_user_echo_from_scrollback()
    {
        return true;
    }
...
///
/// These are deferred from scrollback — the `Kimix/task_backgrounded`
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use kimix_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(kimix_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use kimix_tools::types::output::MCPOutputDetails;
                match mcp.output() {
...
mod tests {
    use super::*;
    use std::sync::Arc;
    /// Default meta with no timestamps (simulates old kimix-shell or tests that
    ...
    fn test_search_tool_call_flow() {
        use kimix_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
        let mut tracker = AcpUpdateTracker::new();
    ...
    /// No stream_start_ms (old kimix-shell) should not break anything.
    #[test]
    ...
    #[test]
    fn is_task_tool_recognizes_kimix_variant() {
