...
//! kimix-shell hasn't been updated or meta is absent.
...
pub struct NotificationMeta {
    ...
    /// (`"{sessionId}-{counter}"`, see `kimix-shell util::event_id`). The
...
/// replayed notifications (`_meta.isReplay`, stamped by kimix-shell's
...
pub mod user_message_chunk_meta {
    ...
    /// ([`kimix_shell::session::PromptOrigin::hide_user_echo_from_scrollback`]).
...
mod tests {
    ...
    fn parse_missing_new_fields() {
        // Simulate old kimix-shell that doesn't send streamStartMs/turnStartMs
        let meta_json = json!({
            "totalTokens": 1000u64,
            "agentTimestampMs": 1700000000000i64,
        });
