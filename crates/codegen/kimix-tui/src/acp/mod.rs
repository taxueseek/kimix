...
use kimix_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a kimix-shell instance.
    pub is_kimix_shell: bool,
    ...
    /// disabled feature produces zero `Kimix/recap` traffic. Defaults to `false`
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<kimix_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_kimix_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(kimix_shell::auth::AuthManager::new(
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = kimix_shell::util::kimix_home::kimix_home().join("config.toml");
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    serde_json::json!({
        "kimix/incrementalBashOutput": true,
        "kimix/hunkTracker": { "mode": hunk_mode },
        "kimix/bashOutputNoColor": true,
        "kimix/gitHeadChanged": true,
    })
}
...
async fn initialize(
    ...
    // Check if this is a kimix-shell agent
    let is_kimix_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("kimixShell"))
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "kimixShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "kimixShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_kimix_com_no_provider_needs_login_pending() {
    ...
    #[test]
    fn startup_auth_kimix_com_with_external_provider_command() {
    ...
    #[test]
    fn startup_auth_non_kimix_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.kimix/config.toml` with a
    ...
    ///   `KIMIX_MOONSHOT_CN_API_KEY`-only e2e): eager auth selects
    ...
    fn moonshot_methods_are_never_selected_for_eager_auth() {
        use kimix_shell::agent::auth_method::{
    ...
    /// `startup_auth_kimix_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use kimix_shell::agent::auth_method::{KIMI_CODE_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(KIMI_CODE_METHOD_ID, "Kimix", None),
    ...
    fn client_capabilities_meta_defaults_absent_or_blank_mode_to_agent_only() {
        ...
        assert_eq!(absent["kimix/hunkTracker"]["mode"], "agent_only");
        let blank = client_capabilities_meta(&ConnectFlags {
            hunk_tracker_mode: Some("   ".into()),
            ..Default::default()
        });
        assert_eq!(blank["kimix/hunkTracker"]["mode"], "agent_only");
    }
    ...
    fn client_capabilities_meta_canonicalizes_off_and_mixed_case() {
        ...
        for raw in ["off", "OFF", "Disabled"] {
            ...
            assert_eq!(meta["kimix/hunkTracker"]["mode"], "off", "raw={raw}");
        }
    }
}
