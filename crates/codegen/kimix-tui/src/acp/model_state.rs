...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model (e.g. "kimix-4.5").
    ...
    /// correct today (all current Kimix models accept images, so nothing is
    ...
    /// not offer (e.g. `none` on kimix-4.5) are rejected so we fail in the TUI
    ...
    pub(crate) fn resolve_effort_token_for(
        ...
        // would 400 on the server (e.g. `none` on kimix-4.5).
