...
mod tests {
    ...
    fn with_test_env(f: impl FnOnce()) {
        ...
        theme_cache::set(ThemeKind::KimixNight);
        f();
        system_appearance::clear_mock();
        theme_cache::reset_for_test();
    }
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let items = cmd.suggest_args(&ctx, "").expect("should return items");
            let kimixnight = items
                .iter()
                .find(|i| i.insert_text == "kimixnight")
                .expect("kimixnight should be in list");
            assert!(
                kimixnight.description.contains("(active)"),
                "explicit theme should show (active), got: {}",
                kimixnight.description
            );
        });
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "kimixnight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "kimixnight");
                }
                other => panic!("expected Action::SetTheme(\"kimixnight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] = KimixNight; next is available[1].
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for KimixNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "kimixnight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"kimixnight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // Default auto config maps Light -> KimixDay.
            assert_eq!(Theme::current_kind(), ThemeKind::KimixDay);
        });
    }
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::KimixDay);
            let cmd = ThemeCommand;
