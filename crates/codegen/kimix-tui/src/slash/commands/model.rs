...
use kimix_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // often contain spaces ("Kimix 4.5"); if we split on the last token
        // first, a shorter catalog entry ("Kimix") would steal the prefix and
        ...
        // level (e.g. `none` on kimix-4.5) surfaces the effort error with the
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("kimix-4.5", "Kimix 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "Kimix 4.5").unwrap();
        assert_eq!(plain.insert_text, "Kimix 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "Kimix" (reasoning) and "Kimix 4.5". `/model Kimix 4.5`
        // must select the full name, not treat "4.5" as an effort on "Kimix".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("kimix", "Kimix");
        let (long_id, long_info) = model_with_reasoning("kimix-4.5", "Kimix 4.5");
        state.available.insert(short_id, short_info);
        state.available.insert(long_id.clone(), long_info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "Kimix 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(resolved_id)) => {
                assert_eq!(resolved_id, long_id);
            }
            other => panic!("expected SetDefaultModel(Kimix 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("kimix-4.5", "Kimix 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "Kimix 4.5 high");
    ...
    /// Case-insensitive matching against the catalog: `/model kimix 4.5`
    /// resolves to the same `ModelId` as `/model Kimix 4.5`.
    #[test]
