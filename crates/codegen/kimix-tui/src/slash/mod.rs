...
pub struct SuggestionRow {
    /// Display text (e.g., "/model" or "Kimix 4 Fast").
    pub display: String,
...
pub struct SlashInvocation<'a> {
    /// Command token (e.g., "model" for "/model kimix-4").
    pub token: &'a str,
...
mod tests {
    ...
    fn parses_invocation_with_args() {
        let inv = parse_invocation("/model kimix-code-fast-1").expect("parsed");
        assert_eq!(inv.token, "model");
        assert_eq!(inv.args, "kimix-code-fast-1");
    }
    ...
    fn required_arg_command_blocks_without_args() {
        ...
        assert!(!is_command_complete("/model ", &reg));
        assert!(is_command_complete("/model kimix-4", &reg));
    }
    ...
    fn cursor_inside_leading_command_with_args_filters_by_prefix() {
        ...
        // Cursor 3 in "/mod kimix-4" clamps the query to "mo".
        ctrl.refresh(&state, "/mod kimix-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
    ...
    fn cursor_inside_recognized_command_with_args_opens_menu() {
        ...
        ctrl.refresh(&state, "/model kimix-4", 3, &models);
        let snapshot = state.snapshot();
        assert!(snapshot.open);
