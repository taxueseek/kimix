...
//! system clipboard. This is the engine behind `kimix wrap` (see
...
///
/// This is the engine behind `kimix wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // `kimix` reached over SSH — that its OSC 52 clipboard writes are being
    // intercepted here and copied to the real local clipboard. The inner kimix
    // reads this (see `kimix_pager_render::clipboard::osc52_sink_active`) to
    ...
    // unknown brands look incapable and the inner kimix would otherwise report
    // "Copy failed" despite the copy actually working).
    //
    // `CommandBuilder::new` inherits the full parent environment; `env` overlays
    // these two without clearing it. The canonical `KIMIX_OSC52_SINK` is
    ...
    cmd.env("KIMIX_OSC52_SINK", "1");
    cmd.env("LC_KIMIX_OSC52_SINK", "1");
    ...
    // detached — `kimix wrap` is short-lived and exits with the child.
...
///
/// Delegates to [`kimix_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = kimix_shell::util::clipboard::set_text(text) {
...
fn host_clipboard_image_frame() -> Vec<u8> {
    let image = kimix_pager_render::clipboard::system_clipboard_get_image();
