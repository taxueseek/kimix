...
/// Plan-mode chord for the tip copy: always `tab`. Derived from the real
/// `CycleMode` binding (not a literal) — the Tab key accepts both plain Tab
/// and Shift+Tab encodings — so it can't drift.
fn plan_chord_label() -> String {
    use crate::input::key::KeyShortcut;
    use crossterm::event::{KeyCode, KeyModifiers};
    KeyShortcut::new(KeyCode::Tab, KeyModifiers::NONE)
        .display()
        .to_ascii_lowercase()
}
...
pub fn plan_nudge_tip() -> EphemeralTip {
    ...
    EphemeralTip::new(
        PLAN_NUDGE_KEY,
        Line::from(vec![
            Span::styled(i18n::tr("Planning? Check out plan mode via "), dim),
...
mod tests {
    ...
    #[test]
    fn plan_nudge_chord_is_tab() {
        // Always tab — derived from the real binding so it can't drift.
        assert_eq!(plan_chord_label(), "tab");
        // The advertised chord is genuinely a Tab event.
        assert_eq!(
