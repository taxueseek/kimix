...
fn main() {
    ...
    let updates_path = match std::env::var("KIMIX_SESSION_PATH") {
        Ok(p) => std::path::PathBuf::from(p),
        Err(_) => {
            eprintln!("Set KIMIX_SESSION_PATH to a session's updates.jsonl path");
            eprintln!(
                "Example: KIMIX_SESSION_PATH=$HOME/.kimix/sessions/<cwd-encoded>/019e0000-0000-7000-8000-000000000001/updates.jsonl"
            );
            return;
        }
    };
    ...
    if !updates_path.exists() {
        ...
        eprintln!("Set KIMIX_SESSION_PATH env var to point to a session's updates.jsonl");
        return;
    }
