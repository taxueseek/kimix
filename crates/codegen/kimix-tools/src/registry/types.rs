...
/// this crate: instead of `Kimix-tools` referencing an out-of-tree tool
/// pack, the pack calls
...
impl ToolConfig {
    ...
    /// `kimix_tool_runtime::Tool::id()`. Use this for built-in tools known
    ...
    where
        T: crate::types::tool_metadata::ToolMetadata + kimix_tool_runtime::Tool + Default,
    {
        Self::from(&T::default())
    }
...
impl<T: crate::types::tool_metadata::ToolMetadata + kimix_tool_runtime::Tool> From<&T>
    for ToolConfig
{
    fn from(tool: &T) -> Self {
        Self {
...
pub struct SessionContext {
    ...
    pub parent_scheduler_handle:
        Option<crate::implementations::kimix::scheduler::types::SchedulerHandle>,
    ...
    pub web_fetch_config: crate::implementations::kimix::web_fetch::WebFetchConfig,
    ...
    pub app_builder_deployer_config:
        crate::implementations::kimix::deploy_app::AppBuilderDeployerConfig,
...
pub async fn drain_value_stream(
    mut stream: kimix_tool_runtime::ToolStream<kimix_tool_runtime::TypedToolOutput>,
) -> Result<serde_json::Value, kimix_tool_runtime::ToolError> {
    use futures::StreamExt;
    while let Some(item) = stream.next().await {
        match item {
            kimix_tool_runtime::ToolStreamItem::Progress(_) => continue,
            kimix_tool_runtime::ToolStreamItem::Terminal(result) => {
...
fn stream_no_terminal_error() -> kimix_tool_runtime::ToolError {
    kimix_tool_runtime::ToolError::custom(
        "stream_no_terminal",
...
struct DispatchParts {
    ...
    lr_handle: kimix_tool_runtime::ArcTool,
...
struct ToolEntry {
    ...
    parse_input: Box<
        dyn Fn(serde_json::Value) -> Result<ToolInput, kimix_tool_runtime::ToolError> + Send + Sync,
    >,
    /// Registers this tool into a `LocalRegistry` using the concrete type.
    /// Captured at `register::<T>()` time when T is known.
    register_in_local: Box<dyn Fn(&kimix_tool_runtime::LocalRegistry) + Send + Sync>,
}
...
pub struct FinalizedToolset {
    ...
    local_registry: kimix_tool_runtime::LocalRegistry,
    ...
    workspace_viewer_ctx: Option<kimix_tool_runtime::WorkspaceViewerContext>,
}
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct ToolRegistryBuilder {
    ...
    shared_local_registry: Option<kimix_tool_runtime::LocalRegistry>,
}
...
impl ToolRegistryBuilder {
    ...
    /// Fully-qualified tool id (`"Kimix:read_file"`) → declared
    ...
    pub fn new() -> Self {
        ...
        b.register_with_params::<kimix::BashTool, kimix::bash::BashParams>();
        b.register_with_params::<kimix::ReadFileTool, kimix::read_file::ReadFileParams>();
        b.register_with_params::<
                kimix::SearchReplaceTool,
                kimix::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<kimix::ListDirTool, kimix::list_dir::ListDirParams>();
        b.register_with_params::<kimix::GrepTool, kimix::grep::GrepParams>();
        b.register::<kimix::KillTaskTool>();
        b.register::<kimix::KillTerminalCommandTool>();
        b.register::<kimix::TodoWriteTool>();
        b.register::<kimix::UpdateGoalTool>();
        b.register::<kimix::TaskOutputTool>();
        b.register::<kimix::GetTerminalCommandOutputTool>();
        b.register::<kimix::WaitTasksTool>();
        b.register::<kimix::TaskTool>();
        ...
        b.register_with_params::<
                kimix::AskUserQuestionTool,
                kimix::ask_user_question::AskUserQuestionParams,
            >();
        b.register::<kimix::MonitorTool>();
        ...
        b.register_with_params::<
            ...
            >();
        b.register_with_params::<
                kimix_concise::ReadFileConciseTool,
                kimix::read_file::ReadFileParams,
            >();
        b.register_with_params::<
                kimix_concise::SearchReplaceConciseTool,
                kimix::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<kimix_concise::BashConciseTool, kimix::bash::BashParams>();
        b.register_with_params::<
                kimix_hashline::HashlineReadTool,
                kimix_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                kimix_hashline::HashlineEditTool,
                kimix_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                kimix_hashline::HashlineGrepTool,
                kimix_hashline::config::HashlineSchemeParams,
            >();
    ...
    /// Compact registration: only L1 core + L2 essential tools.
    /// Skips duplicate variant tools (concise/hashline/codex/opencode)
    /// that provide alternative implementations of the same functionality.
    ...
    pub fn new_compact() -> Self {
        ...
        // L1 core coding tools (always needed)
        b.register_with_params::<kimix::BashTool, kimix::bash::BashParams>();
        b.register_with_params::<kimix::ReadFileTool, kimix::read_file::ReadFileParams>();
        b.register_with_params::<kimix::SearchReplaceTool, kimix::search_replace::SearchReplaceParams>();
        b.register_with_params::<kimix::ListDirTool, kimix::list_dir::ListDirParams>();
        b.register_with_params::<kimix::GrepTool, kimix::grep::GrepParams>();
        b.register::<kimix::KillTaskTool>();
        b.register::<kimix::TodoWriteTool>();
        ...
        b.register::<kimix::WaitTasksTool>();
        b.register::<kimix::TaskTool>();
        // L2 essential extended tools
        b.register::<kimix::WebSearchTool>();
        b.register_with_params::<kimix::WebFetchTool, kimix::web_fetch::WebFetchParams>();
        b.register::<kimix::EnterPlanModeTool>();
        b.register::<kimix::ExitPlanModeTool>();
        b.register_with_params::<kimix::AskUserQuestionTool, kimix::ask_user_question::AskUserQuestionParams>();
        b.register::<crate::implementations::memory::search_tool::MemorySearchImpl>();
        b.register::<crate::implementations::memory::get_tool::MemoryGetImpl>();
        b.register::<crate::implementations::search_tool::SearchTool>();
        b.register_with_params::<
            ...
            crate::implementations::use_tool::UseToolParams,
        >();
        // Write path used by AgentBuilder when write_file_enabled (OpenCode write).
        b.register::<opencode::OpenCodeWriteTool>();
        ...
        b.register_reminder(crate::reminders::TaskCompletionReminder);
        b.register_reminder(SkillDiscoveryReminder);
        // NOTE: deliberately excluded (duplicate variants / low-use):
        // - kimix_concise::* (3 tools) - concise variants of core tools
        // - kimix_hashline::* (3 tools) - hashline scheme variants
        // - codex::* (4 tools) - Codex-compatible variants
        // - opencode::* except write (6 tools)
        // - MonitorTool, Scheduler*Tool, UpdateGoalTool, KillTerminalCommandTool, GetTerminalCommandOutputTool
        for pack in tool_packs().lock().iter() {
            pack(&mut b);
        }
        b
    ...
    pub fn with_local_registry(mut self, registry: kimix_tool_runtime::LocalRegistry) -> Self {
    ...
    pub fn validate_config(&self, config: &ToolServerConfig) -> Vec<RequirementError> {
        ...
        }
        {
            let standard_file_ids: &[&str] =
                &["Kimix:read_file", "Kimix:search_replace", "Kimix:grep"];
            let hashline_file_ids: &[&str] = &[
                "KimixHashline:hashline_read",
                "KimixHashline:hashline_edit",
                "KimixHashline:hashline_grep",
            ];
    ...
    pub fn finalize_with_trunc_config(
        ...
        if let crate::implementations::kimix::web_fetch::WebFetchConfig::Enabled { params } =
            &ctx.web_fetch_config
        {
            match crate::implementations::kimix::web_fetch::WebFetchClient::new(
                params,
                ctx.api_key_provider.clone(),
            ) {
                Ok(client) => {
        ...
        let concise_ns = crate::types::tool::ToolNamespace::KimixConcise.to_string();
        ...
        resources.register_state::<crate::implementations::kimix::todo::TodoState>();
        ...
        resources
            ...
            >();
        resources
            .register_state::<crate::implementations::kimix::scheduler::types::SchedulerState>();
            ...
            ctx.parent_scheduler_handle
        {
            ...
            resources.insert(
                crate::implementations::kimix::scheduler::types::SchedulerHandle(scheduler_cmd_tx),
            );
        ...
        if let (Some(cmd_rx), Some(cancel_token)) = (scheduler_cmd_rx, &scheduler_cancel_token) {
            let actor = crate::implementations::kimix::scheduler::actor::SchedulerActor {
...
///
/// Implements the canonical `kimix_tool_runtime::ToolDispatch` trait so the
...
#[async_trait::async_trait]
impl kimix_tool_runtime::ToolDispatch for InnerDispatchForToolset {
    async fn call(
        &self,
        ...
        let result = self
            ...
            .and_then(|output| {
                let value = serde_json::to_value(&output).map_err(|e| {
                    kimix_tool_runtime::ToolError::custom("output_encoding", e.to_string())
                })?;
                Ok(kimix_tool_runtime::TypedToolOutput::from_value(
                    tool_id.clone(),
                    value,
                ))
            });
        kimix_tool_runtime::terminal_only(result)
    }
}
impl FinalizedToolset {
    ...
    pub fn empty_for_test() -> Self {
        Self {
            ...
            local_registry: kimix_tool_runtime::LocalRegistry::new(),
    ...
    /// `Kimix/*` tool `_meta` contract (tool normalization). Returns `None` for
    ...
    fn tool_not_found_error(tool_name: &str) -> kimix_tool_runtime::ToolError {
        let tid = kimix_tool_protocol::ToolId::new(tool_name)
            .unwrap_or_else(|_| kimix_tool_protocol::ToolId::new("unknown").expect("valid"));
        kimix_tool_runtime::ToolError::not_found(tid, format!("Tool not found: {tool_name}"))
    }
    pub async fn try_parse(
        &self,
        tool_name: &str,
        tool_params: &serde_json::Value,
    ) -> Result<ToolInput, kimix_tool_runtime::ToolError> {
    ...
    async fn call_raw(
        ...
        parent_ctx: kimix_tool_runtime::ToolCallContext,
    ) -> Result<crate::types::output::ToolOutput, kimix_tool_runtime::ToolError> {
        ...
        let mut ctx = kimix_tool_runtime::ToolCallContext::new(parent_ctx.call_id.clone());
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = parent_ctx.extensions.get::<kimix_tool_runtime::Cwd>() {
            ctx.extensions.insert((*cwd).clone());
        }
        let tool_id = kimix_tool_protocol::ToolId::new(&registry_id)
            .unwrap_or_else(|_| kimix_tool_protocol::ToolId::new("unknown").expect("valid"));
        let lr_handle = self.local_registry.find(&tool_id).ok_or_else(|| {
            kimix_tool_runtime::ToolError::not_found(
                tool_id,
    ...
    ) -> Result<ToolRunResult, kimix_tool_runtime::ToolError> {
        use futures::StreamExt;
        let mut stream = self.call_streaming(tool_name, tool_args, tool_call_id, cwd_override);
        while let Some(item) = stream.next().await {
            match item {
                kimix_tool_runtime::ToolStreamItem::Progress(_) => continue,
                kimix_tool_runtime::ToolStreamItem::Terminal(result) => return result,
            }
        }
        Err(stream_no_terminal_error())
    }
    ...
    ///
    /// [`ToolStream`]: kimix_tool_runtime::ToolStream
    /// [`ToolStreamItem::Progress`]: kimix_tool_runtime::ToolStreamItem::Progress
    pub fn call_streaming(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> kimix_tool_runtime::ToolStream<ToolRunResult> {
        use futures::StreamExt;
        ...
        Box::pin(async_stream::stream! {
            ...
            kimix_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; } }; let
            ...
            match item { kimix_tool_runtime::ToolStreamItem::Progress(p) => { yield
            kimix_tool_runtime::ToolStreamItem::Progress(p); }
            kimix_tool_runtime::ToolStreamItem::Terminal(Err(e)) => { yield
            kimix_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; }
            kimix_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => { let run_result
            = this.finalize_output(typed.value, & output_converter,
            effective_tool_name). await; yield
            kimix_tool_runtime::ToolStreamItem::Terminal(run_result); return; } } }
            yield
            kimix_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> Result<DispatchParts, kimix_tool_runtime::ToolError> {
        let (registry_id, output_converter, reverse_params) = {
            let tools = self.tools.read();
            let entry = tools
                .iter()
        ...
        let rt_call_id = kimix_tool_protocol::ToolCallId::new(tool_call_id)
            .unwrap_or_else(|_| kimix_tool_protocol::ToolCallId::new_v7());
        let mut ctx = kimix_tool_runtime::ToolCallContext::new(rt_call_id);
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = cwd_override {
            ctx.extensions.insert(kimix_tool_runtime::Cwd(cwd));
        }
        if let Some(ref version) = contract_version {
            ctx.extensions
                .insert(kimix_tool_runtime::BehaviorVersion(version.clone()));
        }
    ...
    ///
    /// The tool must implement `kimix_tool_runtime::Tool + ToolMetadata`.
    ...
    ) -> Result<(), kimix_tool_runtime::ToolError>
    where
        T: kimix_tool_runtime::Tool + ToolMetadata + std::fmt::Debug + Send + Sync + 'static,
        T::Output: serde::Serialize,
    {
        ...
        let kind = tool.kind();
        let registry_id = kimix_tool_runtime::Tool::id(&tool).as_str().to_owned();
        if tools.iter().any(|t| t.registry_id == registry_id) {
            return Err(kimix_tool_runtime::ToolError::invalid_arguments(format!(
                "Tool with registry_id already registered: {registry_id} (client_name={name})"
            )));
        }
    ...
    pub fn unregister_tools_by_prefix(&self, prefix: &str) -> usize {
        ...
        let to_remove: Vec<_> = tools
            ...
            .filter_map(|t| kimix_tool_protocol::ToolId::new(&t.registry_id).ok())
            .collect();
    ...
    pub fn unregister_tool_by_name(&self, name: &str) -> bool {
        ...
        let tool_id = tools
            ...
            .and_then(|t| kimix_tool_protocol::ToolId::new(&t.registry_id).ok());
        let before = tools.len();
    ...
    match entry.id.as_str() {
                    ...
                    .unwrap_or(true) =>
        {
            ...
            if !has_tool_kind(proposed, ToolKind::BackgroundTaskAction) {
                missing.push("get_task_output");
            }
            if !has_tool_kind(proposed, ToolKind::KillTaskAction) {
                missing.push("kill_task");
            }
        ...
        "get_task_output" if is_primary_ns => {
            let has_kimix_bash = ["Kimix", "Kimix"].iter().any(|ns| {
                has_tool_with_bool_param(
                    proposed,
                    ns,
                    "run_terminal_cmd",
                    "enabled_background",
            ...
            let has_kimix_concise_bash = ["KimixConcise", "KimixConcise"].iter().any(|ns| {
                has_tool_with_bool_param(
                    proposed,
                    ns,
                    "run_terminal_cmd",
            ...
            let has_task = ["Kimix", "Kimix"]
                .iter()
                .any(|ns| has_tool(proposed, ns, "task"));
            let mut notes = vec![];
            if ["Kimix", "Kimix"]
                .iter()
                .any(|ns| has_tool(proposed, ns, "run_terminal_cmd"))
                && !has_kimix_bash
            {
                notes.push("run_terminal_cmd is present but enabled_background=false");
            }
            if ["KimixConcise", "KimixConcise"]
                .iter()
                .any(|ns| has_tool(proposed, ns, "run_terminal_cmd"))
                && !has_kimix_concise_bash
            {
                notes.push("concise run_terminal_cmd is present but enabled_background=false");
            }
            let mut message = "Kimix:get_task_output requires a background-capable bash tool (Kimix:run_terminal_cmd with enabled_background=true), OpenCode:bash, or Kimix:task"
                .to_string();
            let has_provider = has_kimix_bash
                || has_kimix_concise_bash
                || has_opencode_bash
                || has_task;
            ...
            RequirementError::new(fq_tool_id, message)
                .with_field_path("tools")
                .with_expected(
                    "include a background-capable bash tool, OpenCode:bash, or task",
                )
                .with_category("requirements")
        }
        "search_replace"
            if is_primary_ns
                && !params
                    ...
                    .and_then(|value| value.as_bool())
                    .unwrap_or(false)
                && !has_tool_kind(proposed, ToolKind::Read) =>
        {
            RequirementError::new(
                    fq_tool_id,
                ...
                .with_expected(
                    "set skip_read_before_edit=true or include a Read tool such as Kimix:read_file",
                )
                .with_bad_value(serde_json::Value::Bool(false))
                .with_category("requirements")
        }
        "enter_plan_mode" if is_primary_ns => {
            RequirementError::new(
                    fq_tool_id,
                    "enter_plan_mode requires exit_plan_mode so plan mode can always be exited",
                )
                .with_field_path("tools")
                .with_expected("include exit_plan_mode")
                .with_category("requirements")
        }
        "exit_plan_mode" if is_primary_ns => {
            RequirementError::new(
                    fq_tool_id,
                    "exit_plan_mode requires enter_plan_mode so plan mode can be entered before exiting",
                )
                .with_field_path("tools")
                .with_expected("include enter_plan_mode")
                .with_category("requirements")
        }
        _ => {
...
mod tests {
    ...
    fn test_session_context(tmp: &TempDir) -> SessionContext {
        SessionContext {
            ...
            web_fetch_config: crate::implementations::kimix::web_fetch::WebFetchConfig::default(),
            lsp: None,
            app_builder_deployer_config:
                crate::implementations::kimix::deploy_app::AppBuilderDeployerConfig::default(),
    ...
    /// appeared before `Kimix:search_replace` (`ToolKind::Edit`, input:
    ...
    async fn kind_params_merged_across_multiple_tools_of_same_kind() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "Kimix:search_replace".to_string(),
                    params: Some(
                        serde_json::json!({
    ...
    /// Smoke test: finalize the full Kimix toolset and verify every
    ...
    async fn full_toolset_descriptions_render_cleanly() {
        use crate::implementations::kimix::{
        ...
        let config = ToolServerConfig {
            ...
            .into_iter()
            .map(|id| ToolConfig::from_id(format!("Kimix:{id}")))
            .collect(),
            behavior_preset: None,
        };
        let tmp = TempDir::new().unwrap();
    ...
    /// name: `run_terminal_cmd` (Kimix).
    #[tokio::test]
    async fn tool_name_for_kind_resolves_execute() {
        use crate::types::tool::ToolKind;
        let tmp = TempDir::new().unwrap();
        let kimix = ToolRegistryBuilder::new()
            .finalize(
                ToolServerConfig {
                    tools: vec![
                        ToolConfig::from_id("Kimix:run_terminal_cmd".to_string()),
                        ToolConfig::from_id("Kimix:get_task_output".to_string()),
                        ToolConfig::from_id("Kimix:kill_task".to_string()),
                    ],
                    behavior_preset: None,
                },
                test_session_context(&tmp),
            )
            .expect("kimix toolset should finalize");
        assert_eq!(
            kimix.tool_name_for_kind(ToolKind::Execute).as_deref(),
            Some("run_terminal_cmd")
        );
    }
    /// `merge_tool_meta` (the harness emission path) must stamp `Kimix/tool` for a
    ...
    fn tool_config_kind_sinks_unknown_to_other() {
        ...
        let known = parse(serde_json::json!({ "id" : "Kimix:read_file", "kind" : "read" }));
        assert_eq!(known.kind, Some(ToolKind::Read));
        let typo = parse(serde_json::json!({ "id" : "Kimix:read_file", "kind" : "raed" }));
        assert_eq!(typo.kind, Some(ToolKind::Other));
        let absent = parse(serde_json::json!({ "id" : "Kimix:read_file" }));
    ...
    /// Verify KimixConcise tools can be finalized and produce concise output.
    #[tokio::test]
    async fn test_concise_namespace_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "KimixConcise:read_file".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "KimixConcise:search_replace".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "KimixConcise:run_terminal_cmd".to_string(),
                    params: Some(
                ...
                ToolConfig::for_tool::<kimix::GrepTool>(),
                ToolConfig::for_tool::<kimix::KillTaskTool>(),
                ToolConfig::for_tool::<kimix::TaskOutputTool>(),
                ToolConfig {
                    id: "Kimix:list_dir".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    fn has_tool_id_knows_pinned_tool_config_ids() {
        let builder = ToolRegistryBuilder::new();
        for id in [
            "Kimix:run_terminal_cmd",
            "Kimix:read_file",
            "Kimix:search_replace",
            "Kimix:list_dir",
            "Kimix:grep",
            "Kimix:get_terminal_command_output",
            "Kimix:kill_terminal_command",
        ] {
            assert!(
        ...
        assert!(
            !builder.has_tool_id("Kimix:does_not_exist"),
    ...
    fn known_tool_kinds_maps_pinned_tool_config_ids() {
        ...
        for (id, expected) in [
            ("Kimix:run_terminal_cmd", ToolKind::Execute),
            ("Kimix:read_file", ToolKind::Read),
            ("Kimix:search_replace", ToolKind::Edit),
            ("Kimix:grep", ToolKind::Search),
            ("Kimix:list_dir", ToolKind::List),
        ] {
            assert_eq!(
                kinds.get(id),
                Some(&expected),
        ...
        assert!(
            !kinds.contains_key("Kimix:does_not_exist"),
    ...
    /// (e.g. `"read_file"`). If both `Kimix:read_file` and
    ...
    fn validate_config_reports_param_field_path_and_value() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "Kimix:run_terminal_cmd".to_string(),
                params: Some(
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "Kimix:run_terminal_cmd");
        assert_eq!(
    ...
    fn validate_config_reports_semantic_hashline_param_error() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "KimixHashline:hashline_read".to_string(),
                params: Some(
    ...
    impl kimix_tool_runtime::Tool for FakeMcpTool {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kimix_tool_protocol::ToolId {
            kimix_tool_protocol::ToolId::new("fake_mcp").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kimix_tool_runtime::ListToolsContext,
        ) -> kimix_tool_types::ToolDescription {
            kimix_tool_types::ToolDescription::new("fake_mcp", &self.description)
        }
        async fn run(
            &self,
            _ctx: kimix_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<String, kimix_tool_runtime::ToolError> {
            Ok("ok".into())
        }
    }
    #[tokio::test]
    ...
    impl kimix_tool_runtime::Tool for NonStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kimix_tool_protocol::ToolId {
            kimix_tool_protocol::ToolId::new("non_streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kimix_tool_runtime::ListToolsContext,
        ) -> kimix_tool_types::ToolDescription {
            kimix_tool_types::ToolDescription::new("non_streaming_stub", "non-streaming stub")
        }
        async fn run(
            &self,
            _ctx: kimix_tool_runtime::ToolCallContext,
    ...
    impl kimix_tool_runtime::Tool for StreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kimix_tool_protocol::ToolId {
            kimix_tool_protocol::ToolId::new("streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kimix_tool_runtime::ListToolsContext,
        ) -> kimix_tool_types::ToolDescription {
            kimix_tool_types::ToolDescription::new("streaming_stub", "streaming stub")
        }
        async fn run(
            &self,
            _ctx: kimix_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            _ctx: kimix_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> kimix_tool_runtime::ToolStream<String> {
            Box::pin(futures::stream::iter(vec![
                kimix_tool_runtime::ToolStreamItem::Progress(
                    kimix_tool_runtime::ToolProgress::Text {
                        text: "progress-1".into(),
                    },
                ),
                kimix_tool_runtime::ToolStreamItem::Terminal(Ok("terminal-value".to_string())),
            ]))
        }
    }
    ...
    async fn call_non_streaming_stub_back_compat() {
        ...
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig::for_tool::<kimix::ReadFileTool>()],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
        let toolset = Arc::new(builder.finalize(config, ctx).unwrap());
        ...
        while let Some(item) = stream.next().await {
            match item {
                kimix_tool_runtime::ToolStreamItem::Progress(p) => {
                    ...
                    assert!(matches!(p, kimix_tool_runtime::ToolProgress::Text { .. }));
                    progress_count += 1;
                }
                kimix_tool_runtime::ToolStreamItem::Terminal(result) => {
    ...
    impl kimix_tool_runtime::Tool for NoTerminalStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> kimix_tool_protocol::ToolId {
            kimix_tool_protocol::ToolId::new("no_terminal_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::kimix_tool_runtime::ListToolsContext,
        ) -> kimix_tool_types::ToolDescription {
            kimix_tool_types::ToolDescription::new("no_terminal_stub", "no-terminal stub")
        }
        async fn run(
            &self,
            _ctx: kimix_tool_runtime::ToolCallContext,
    ...
    async fn tool_definitions_builtins_only_hides_mcp_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<kimix::ReadFileTool>(),
                ToolConfig::for_tool::<kimix::GrepTool>(),
            ],
            behavior_preset: None,
        };
    ...
    fn task_tool_rejected_without_get_task_output_and_kill_task() {
        ...
        assert!(
            errors.iter().any(|e| e.tool == "Kimix:task"
                && e.message.contains("Kimix:get_task_output")
                && e.message.contains("Kimix:kill_task")),
    ...
    fn task_tool_rejected_with_only_get_task_output() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "Kimix:get_task_output".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.tool == "Kimix:task" && e.message.contains("Kimix:kill_task")),
    ...
    fn task_tool_rejected_with_only_kill_task() {
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.tool == "Kimix:task" && e.message.contains("Kimix:get_task_output")),
    ...
    /// Kimix agent config (all tools present) and that the new examples
    ...
    async fn background_param_templates_reference_real_schema_keys() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                tool("Kimix:run_terminal_cmd"),
                tool("Kimix:task"),
                tool("Kimix:get_task_output"),
                tool("Kimix:wait_tasks"),
                tool("Kimix:kill_task"),
            ],
            behavior_preset: None,
        };
        let tmp = TempDir::new().unwrap();
    ...
    fn bash_with_enabled_background_reports_missing_task_tools() {
        ...
        assert!(error.message.contains("Kimix:get_task_output"));
        assert!(error.message.contains("Kimix:kill_task"));
        assert!(
            error
                .expected
                .as_deref()
    ...
    fn task_requirement_error_lists_missing_background_tools() {
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "Kimix:task");
        assert!(error.message.contains("Kimix:get_task_output"));
        assert!(error.message.contains("Kimix:kill_task"));
    ...
    fn search_replace_requirement_error_mentions_read_tool() {
        ...
        let error = errors
            .iter()
            .find(|error| error.tool == "Kimix:search_replace")
    ...
    fn ask_user_question_validates_standalone() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "Kimix:ask_user_question".to_string(),
                params: None,
                name_override: None,
                params_name_overrides: None,
                description_override: None,
    ...
    async fn task_description_renders_with_default_agent_config() {
        ...
        let toolset = builder
            .finalize(config, ctx)
            .expect("finalize should succeed with default kimix tools");
        let defs = toolset.tool_definitions();
        let task_def = defs
            .iter()
    ...
    fn hashline_tools_registered_in_builder() {
        let builder = ToolRegistryBuilder::new();
        assert!(
            builder.tools.contains_key("KimixHashline:hashline_read"),
            "hashline_read should be registered"
        );
        assert!(
            builder.tools.contains_key("KimixHashline:hashline_edit"),
            "hashline_edit should be registered"
        );
        assert!(
            builder.tools.contains_key("KimixHashline:hashline_grep"),
    ...
    async fn hashline_config_finalizes_successfully() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("KimixHashline:hashline_read"),
                hashline_tool_config("KimixHashline:hashline_edit"),
                hashline_tool_config("KimixHashline:hashline_grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn standard_and_hashline_each_finalize_independently() {
        ...
        let standard_config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("Kimix:read_file"),
                hashline_tool_config("Kimix:search_replace"),
                hashline_tool_config("Kimix:grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn empty_struct_schema_has_properties_and_required() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<kimix::EnterPlanModeTool>(),
                ToolConfig::for_tool::<kimix::ExitPlanModeTool>(),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn hashline_toolset_e2e_with_utilities() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "KimixHashline:hashline_read".to_owned(),
                    params: Some(
                ...
                ToolConfig::for_tool::<kimix_hashline::HashlineEditTool>(),
                ToolConfig::for_tool::<kimix_hashline::HashlineGrepTool>(),
                ToolConfig::for_tool::<kimix::ListDirTool>(),
            ],
            behavior_preset: None,
        };
        let errors = builder.validate_config(&config);
    ...
    fn bash_config_with_background() -> ToolConfig {
        ToolConfig {
            id: "Kimix:run_terminal_cmd".to_owned(),
            params: Some(
                serde_json::json!({ "enabled_background" : true })
                    .as_object()
                    .unwrap()
    ...
    async fn kimix_bridge(tmp: &TempDir) -> crate::bridge::ToolBridge {
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<kimix::ListDirTool>(),
                ToolConfig::for_tool::<kimix::ReadFileTool>(),
                ToolConfig::for_tool::<kimix::SearchReplaceTool>(),
                bash_config_with_background(),
                ToolConfig::for_tool::<kimix::TaskOutputTool>(),
                ToolConfig::for_tool::<kimix::KillTaskTool>(),
                ToolConfig::for_tool::<kimix::GrepTool>(),
                ToolConfig::for_tool::<kimix::TodoWriteTool>(),
            ],
            behavior_preset: None,
        };
    ...
    async fn hub_dispatch_list_dir() {
        ...
        let bridge = kimix_bridge(&tmp).await;
        let result = bridge
            .call(
                "list_dir",
    ...
    async fn hub_dispatch_parity_list_dir() {
        ...
        let hub_bridge = kimix_bridge(&tmp).await;
        ...
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig::for_tool::<kimix::ListDirTool>()],
            behavior_preset: None,
        };
    ...
    async fn prepare_dispatch_stamps_workspace_viewer_ctx_when_present() {
        let (toolset, _tmp) =
            toolset_with_viewer_ctx(Some(kimix_tool_runtime::WorkspaceViewerContext {
        ...
        let wvc = parts
            .ctx
            .extensions
            .get::<kimix_tool_runtime::WorkspaceViewerContext>()
    ...
    async fn bash_streaming_progress_emitted_via_finalized_toolset_when_gate_on() {
        ...
        while let Some(item) = stream.next().await {
            match item {
                kimix_tool_runtime::ToolStreamItem::Progress(_) => progress_count += 1,
                kimix_tool_runtime::ToolStreamItem::Terminal(r) => {
