...
/// Resolve `wire_name` in `toolset` and merge the canonical `Kimix/tool` object
...
/// the `Kimix/tool` `_meta` object. Equivalent tools across toolsets emit the
...
pub fn canonical_input(input: &ToolInput) -> Option<serde_json::Value> {
    ...
    Some(match input {
        ...
        ToolInput::Outline(o) => obj([(field::PATH, req(&o.file_path))]),
...
mod tests {
    ...
    fn canonical_omits_absent_options_not_null() {
        let kimix = parse(serde_json::json!({ "variant" : "ReadFile", "target_file" : "/a" }));
        let g = canonical_input(&kimix).unwrap();
