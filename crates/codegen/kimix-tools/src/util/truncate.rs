...
/// width-based truncation in the TUI, see `kimix_tui`'s own helpers.
...
/// Thin wrapper around [`kimix_token_estimation::estimate_tokens`] preserving
/// the historical `usize` return type used by tool-side callers
/// (`read_file`, `attach_file`, `inspect`, `compaction` file gates).
pub fn estimate_tokens(s: &str) -> usize {
    kimix_token_estimation::estimate_tokens(s) as usize
}

/// Estimate the number of chars per token using the bytes/4 heuristic.
/// Thin wrapper around [`kimix_token_estimation::estimate_chars`].
pub fn estimate_chars(s: u64) -> u64 {
    kimix_token_estimation::estimate_chars(s)
}
