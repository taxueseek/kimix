...
//!    [`ENV_KIMIX_MAX_MCP_OUTPUT_BYTES`] / [`ENV_MAX_MCP_OUTPUT_BYTES`]
//! 4. Built-in default
use std::path::PathBuf;
use std::sync::atomic::{AtomicUsize, Ordering};

use kimix_tool_runtime::ToolCallContext;
...
/// Kimix-native env override for the MCP inline output cap (bytes).
pub const ENV_KIMIX_MAX_MCP_OUTPUT_BYTES: &str = "KIMIX_MAX_MCP_OUTPUT_BYTES";
...
/// Env tier: `KIMIX_MAX_MCP_OUTPUT_BYTES` then `MAX_MCP_OUTPUT_BYTES`.
///
/// Kimix-native wins when both are set. Positive integers only. Used by the
...
pub fn mcp_max_output_bytes_from_env() -> Option<usize> {
    parse_positive_bytes_env(ENV_KIMIX_MAX_MCP_OUTPUT_BYTES)
...
mod tests {
    ...
    fn host_set_overrides_env_fallback() {
        with_mcp_limit_lock(|| {
            ...
            let prev_kimix = std::env::var(ENV_KIMIX_MAX_MCP_OUTPUT_BYTES).ok();
            unsafe {
                std::env::remove_var(ENV_MAX_MCP_OUTPUT_BYTES);
                std::env::remove_var(ENV_KIMIX_MAX_MCP_OUTPUT_BYTES);
            }
            assert_eq!(
            ...
            unsafe {
                ...
                match prev_kimix {
                    Some(v) => std::env::set_var(ENV_KIMIX_MAX_MCP_OUTPUT_BYTES, v),
                    None => std::env::remove_var(ENV_KIMIX_MAX_MCP_OUTPUT_BYTES),
                }
            }
    ...
    fn env_parser_rejects_zero_and_junk() {
        with_mcp_limit_lock(|| {
            ...
            // KIMIX_* wins over MAX_* when both set.
            unsafe { std::env::set_var(ENV_KIMIX_MAX_MCP_OUTPUT_BYTES, "99999") };
            assert_eq!(mcp_max_output_bytes_from_env(), Some(99_999));

            unsafe { std::env::remove_var(ENV_KIMIX_MAX_MCP_OUTPUT_BYTES) };
            assert_eq!(mcp_max_output_bytes_from_env(), Some(12_345));
