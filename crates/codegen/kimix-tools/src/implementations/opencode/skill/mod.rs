...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.kimix/skills/ or .kimix/skills/)
${%- endif %}
</available_skills>"#;
...
impl kimix_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "skill");
