...
impl kimix_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(kimix_tool_runtime::ToolError::execution(
                    kimix_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = kimix_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        ...
        let tool = BashTool;
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "bash");
    ...
    async fn truncation_cfg_override() {
        ...
        impl TerminalBackend for CapturingMockTerminal {
            async fn run(
                ...
                *self.captured_request.lock().expect("lock poisoned") = Some(request);
        ...
        let captured = capturing.captured_request.lock().expect("lock poisoned");
        let req = captured
            .as_ref()
    ...
    //
    // Same shape as Kimix/bash: the opencode tool inherits the same
