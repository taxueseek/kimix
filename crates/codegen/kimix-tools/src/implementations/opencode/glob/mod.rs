...
impl kimix_tool_runtime::ToolOutput for GlobOutput {}
...
impl kimix_tool_runtime::Tool for GlobTool {
    type Args = GlobInput;
    type Output = GlobOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("glob").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: GlobInput,
    ) -> Result<GlobOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = GlobTool;
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "glob");
    ...
    async fn gitignore_respected() {
        ...
        kimix_test_utils::git::ensure_hermetic_git_on_path();
