...
impl kimix_tool_runtime::Tool for WriteTool {
    type Args = WriteInput;
    type Output = WriteOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("write").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: WriteInput,
    ) -> Result<WriteOutput, kimix_tool_runtime::ToolError> {
            ...
            && !parent.as_os_str().is_empty()
        {
            tokio::fs::create_dir_all(parent).await.map_err(|e| {
                ...
                kimix_tool_runtime::ToolError::execution(
                    kimix_tool_protocol::ToolId::new("write").expect("valid"),
                    ce.to_string(),
                )
            })?;
        }
...
mod tests {
    ...
    async fn write_new_file_creates_with_correct_content() {
        ...
        let result =
            kimix_tool_runtime::Tool::run(&tool, test_ctx(shared_resources.clone()), input)
                .await
                .unwrap();
    ...
    fn tool_metadata() {
        ...
        let tool = WriteTool;
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "write");
    ...
    async fn missing_filesystem_resource() {
        ...
        let result =
            kimix_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input).await;
