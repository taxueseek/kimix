...
impl kimix_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
...
async fn ensure_parent_dirs(path: &std::path::Path) -> Result<(), kimix_tool_runtime::ToolError> {
    if let Some(parent) = path.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            kimix_tool_runtime::ToolError::execution(
                kimix_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `Kimix::search_replace::helpers` — shared across
// both the Kimix and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "edit");
