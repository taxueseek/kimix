...
impl kimix_tool_runtime::Tool for ReadTool {
    type Args = ReadInput;
    type Output = ReadFileOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("read").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        if let Ok(meta) = crate::implementations::kimix::read_file::bytes_to_metadata(&file_bytes)
            && meta.is_image()
        {
