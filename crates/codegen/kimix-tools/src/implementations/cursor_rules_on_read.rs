...
mod tests {
    ...
    use crate::implementations::kimix::read_file::{
        ReadFileInput, ReadFileParams, ReadFileTool as KimixReadFileTool,
    };
    ...
    fn resources_with_kimix_rules_on_read(root: &Path) -> SharedResources {
        let mut resources = Resources::new();
        resources.insert(Cwd(root.to_path_buf()));
        resources.insert(FileSystem(Arc::new(LocalFs)));
        resources.insert(NotificationHandle(ToolNotificationHandle::noop()));
    ...
    #[tokio::test]
    async fn kimix_read_file_output_includes_matching_rule_once() {
        let temp = TempDir::new().unwrap();
        let root = temp.path();
        ...
        let shared = resources_with_kimix_rules_on_read(root);
        ...
        let first = kimix_tool_runtime::Tool::run(&KimixReadFileTool, test_ctx(shared), input)
            .await
            .unwrap();
