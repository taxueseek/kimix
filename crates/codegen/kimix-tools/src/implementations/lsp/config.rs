//! LSP server configuration from `.Kimix/lsp.json`.
...
/// This is the canonical merge function — both session startup and `kimix inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::kimix_home::kimix_home().join("lsp.json");
    let project_path = cwd.join(".kimix").join("lsp.json");
...
/// Load LSP server configs from `~/.kimix/lsp.json` and `<cwd>/.Kimix/lsp.json`.
