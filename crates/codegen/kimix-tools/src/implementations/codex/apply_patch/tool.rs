        ...
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            kimix_tool_runtime::ToolError::execution(
                kimix_tool_protocol::ToolId::new("apply_patch").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
impl kimix_tool_runtime::Tool for ApplyPatchTool {
    type Args = ApplyPatchInput;
    type Output = ApplyPatchOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("apply_patch").expect("valid tool id")
    }
    ...
    ) -> Result<ApplyPatchOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn add_file_creates_with_correct_content() {
        ...
        let result =
            kimix_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), make_input(&patch))
                .await
                .unwrap();
