...
///
/// Shares `ToolKind::Search` with the Kimix `GrepTool`. These tools are
/// namespace-exclusive — consumers enable either `Kimix` or `Codex` search,
...
impl kimix_tool_runtime::Tool for CodexGrepFilesTool {
    type Args = CodexGrepFilesInput;
    type Output = CodexGrepFilesOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("grep_files").expect("valid tool id")
    }
    ...
    ) -> Result<CodexGrepFilesOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn test_ctx(cwd: &Path) -> kimix_tool_runtime::ToolCallContext {
    ...
    async fn tool_reports_empty_pattern_error() {
        ...
        let result = kimix_tool_runtime::Tool::run(&tool, test_ctx(tmp.path()), input)
            .await
            .unwrap();
        match result {
