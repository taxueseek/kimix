...
//! absolute paths. See the plan document for the full diff vs the Kimix
//! `ListDirTool`.
...
impl kimix_tool_runtime::Tool for CodexListDirTool {
    type Args = CodexListDirInput;
    type Output = ListDirOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("list_dir").expect("valid tool id")
    }
    ...
    ) -> Result<ListDirOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn tool_lists_directory() {
        ...
        let ctx = kimix_tool_runtime::ToolCallContext::default();
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = kimix_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {
