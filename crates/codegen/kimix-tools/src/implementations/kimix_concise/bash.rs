...
use crate::implementations::kimix::bash::{BashTool, BashToolInput, BashToolOutput, KillReason};
...
impl kimix_tool_runtime::Tool for BashConciseTool {
    ...
    async fn run(
        ...
        let result = kimix_tool_runtime::Tool::run(&BashTool, ctx, input).await?;
...
mod tests {
    ...
    fn no_double_header_after_default_prebake() {
        ...
        bash.output_for_prompt = crate::implementations::kimix::bash::format_default_prompt(&bash);
