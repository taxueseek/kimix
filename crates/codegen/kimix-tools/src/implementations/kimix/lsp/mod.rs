...
//! `LspTool` (Tool trait impl) under the `Kimix` namespace.
use std::sync::Arc;
...
impl kimix_tool_runtime::ToolOutput for LspToolOutput {}
...
impl kimix_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, kimix_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    kimix_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.kimix/lsp.json or <cwd>/.kimix/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
