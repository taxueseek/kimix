...
pub use kimix_tools_api::slash_commands::UPDATE_GOAL_TOOL_NAME;
...
impl kimix_tool_runtime::ToolOutput for UpdateGoalOutput {}
...
impl kimix_tool_runtime::Tool for UpdateGoalTool {
    type Args = UpdateGoalInput;
    type Output = UpdateGoalOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new(UPDATE_GOAL_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<UpdateGoalOutput, kimix_tool_runtime::ToolError> {
...
pub fn render_ack_into_output(
    ...
    match ack {
        ...
        UpdateGoalAck::Rejected { reason, detail } => Err(kimix_tool_runtime::ToolError::custom(
            reason.error_code(),
            detail,
        )),
    }
