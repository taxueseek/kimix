...
crate::register_resource!("kimix", "Todo", TodoState);
...
impl kimix_tool_runtime::Tool for TodoWriteTool {
    type Args = TodoWriteInput;
    type Output = TodoWriteOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
    ...
    ) -> Result<TodoWriteOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn name_and_description() {
        ...
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "todo_write");
    ...
    async fn replace_mode_creates_items() {
        ...
        let output = expect_success(
            kimix_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
                .await
                .unwrap(),
        );
    ...
    async fn replace_clears_previous_state() {
        ...
        kimix_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input1)
            .await
            .unwrap();
        ...
        let output = expect_success(
            kimix_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input2)
                .await
                .unwrap(),
        );
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("kimix.Todo").is_some(),
            "TodoState should serialize under 'kimix.Todo'"
        );
