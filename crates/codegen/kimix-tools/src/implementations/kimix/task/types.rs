...
//! (in `Kimix-tools`) and the subagent coordinator (in `Kimix-shell`).
//!
...
use educe::Educe;
use kimix_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"Kimix-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`kimix_tool_types::is_not_sentinel`] for existing call sites.
pub use kimix_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `Kimix-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "Kimix:run_terminal_cmd" | "KimixConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `Kimix-shell` relies on
...
register_resource!("kimix", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-Kimix
...
pub type MonitorEventBuffer = kimix_tool_types::EventQueue<MonitorEventNotification>;

register_resource!("kimix", "MonitorEventBuffer", MonitorEventBuffer);
...
/// Kimix-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in Kimix-chat-state (via `RunningSubagentSummary`)
...
register_resource!("kimix", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("kimix", "TaskModelValidator", TaskModelValidator);
...
register_resource!("kimix", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by Kimix-shell immediately before a prompt turn begins executing so
/// subagents launched during that turn can be cancelled together if the user
/// aborts the turn.
#[derive(Debug, Clone)]
pub struct CurrentPromptIdResource(pub String);

register_resource!("kimix", "CurrentPromptIdResource", CurrentPromptIdResource);

/// True while a `/goal` loop is active. Set by Kimix-shell at turn start.
...
register_resource!("kimix", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("Kimix:run_terminal_cmd", ToolKind::Execute),
                tc("Kimix:read_file", ToolKind::Read),
                tc("Kimix:list_dir", ToolKind::List),
                tc("Kimix:grep", ToolKind::Search),
                tc("Kimix:kill_task", ToolKind::KillTaskAction),
                tc("Kimix:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("Kimix:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
