...
use kimix_tool_types::{SubagentCompletedOutput, SubagentIsolationMode, TaskToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for TaskTool {
    ...
    fn description_template(&self) -> &str {
        // The Task tool description for Kimix is *never* taken from here.
        // It is always supplied via `ToolConfig::with_description(...)` using
        // the dynamically built string from `build_task_description()` in
        // Kimix-agent/src/builder.rs (HEADER + per-subagent blocks + FOOTER).
        //
        ...
        "<see build_task_description() in kimix-agent>"
    }
...
impl kimix_tool_runtime::Tool for TaskTool {
    type Args = TaskToolInput;
    type Output = ToolOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("task").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        // 0. Validate required fields (prompt and description are optional in
        //    the schema for robustness, but must be non-empty at execution time).
        if input.prompt.trim().is_empty() {
            return Err(kimix_tool_runtime::ToolError::invalid_arguments(
                "missing required field: prompt (the full task for the subagent to execute)",
            ));
        }
        if input.description.trim().is_empty() {
            return Err(kimix_tool_runtime::ToolError::invalid_arguments(
                "missing required field: description (a short 3-5 word summary of the task)",
            ));
        }
            ...
            && resume_from.is_none()
        {
            ...
            if !p.is_dir() {
                ...
                return Err(kimix_tool_runtime::ToolError::invalid_arguments(detail));
            }
        }
        ...
        if let Some(ref requested) = model {
            ...
            if let Some(error) = validator.error_for(requested) {
                return Err(kimix_tool_runtime::ToolError::invalid_arguments(error));
            }
        }
        ...
        if input.run_in_background {
            ...
            return Ok(ToolOutput::Text(
                kimix_tool_types::format_subagent_started_background(
                    &id,
                    &input.subagent_type,
                    &input.description,
                    &task_output_name,
        ...
        if result.success {
            ...
            Err(kimix_tool_runtime::ToolError::invalid_arguments(
                result
                    .error
...
mod tests {
    use super::*;
    use crate::implementations::kimix::task::backend::{ChannelBackend, SubagentBackendResource};
    use crate::types::resources::Resources;
    use crate::types::tool_metadata::test_ctx;
    use kimix_tool_types::SubagentCapabilityMode;
    use std::sync::Arc;
    use tokio::sync::mpsc;
    ...
    async fn invalid_model_returns_error_before_background_spawn() {
        ...
        let result =
            kimix_tool_runtime::Tool::run(&TaskTool, test_ctx(resources.into_shared()), input)
                .await;
    ...
    async fn model_whitespace_is_trimmed() {
        ...
        let mut input = task_input("general-purpose", false);
        input.model = Some("  test-model  ".into());
        let _ = kimix_tool_runtime::Tool::run(&TaskTool, test_ctx(shared), input)
            .await
            .unwrap();
        handle.await.unwrap();
    }
