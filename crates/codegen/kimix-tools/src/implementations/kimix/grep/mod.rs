...
// Re-export the shared KimixIntegerSchema from types module
pub use crate::types::KimixIntegerSchema;
use ripgrep::rg_path;
...
crate::register_resource!("kimix", "Grep", GrepParams);
...
static GREP_CAPABILITIES: LazyLock<kimix_tool_protocol::ToolCapabilities> =
...
impl kimix_tool_runtime::Tool for GrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("grep").expect("valid tool id")
    }
    ...
    async fn execute(
    ...
    ) -> kimix_tool_runtime::ToolStream<GrepSearchOutput> {
        ...
        let Some(spec) = admitted_spec else {
            return kimix_tool_runtime::terminal_only(self.run(ctx, input).await);
        };
    ...
    async fn run(
        ...
        ctx: kimix_tool_runtime::ToolCallContext,
        input: GrepSearchInput,
    ) -> Result<GrepSearchOutput, kimix_tool_runtime::ToolError> {
...
fn grep_progress_stream(
    ctx: kimix_tool_runtime::ToolCallContext,
    input: GrepSearchInput,
    spec: &'static kimix_tool_protocol::StreamingSpec,
    span: tracing::Span,
) -> kimix_tool_runtime::ToolStream<GrepSearchOutput> {
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Early(out)) => {
                ...
                yield kimix_tool_runtime::ToolStreamItem::Terminal(Ok(out));
                return;
            }
            Err(e) => {
                yield kimix_tool_runtime::ToolStreamItem::Terminal(Err(e));
                return;
            }
        };
        ...
        if timed_out {
            ...
            if stdout_buf.is_empty() {
                yield kimix_tool_runtime::ToolStreamItem::Terminal(Ok(grep_timeout_output(secs)));
            } else {
                ...
                output.exit_code = -1;
                yield kimix_tool_runtime::ToolStreamItem::Terminal(Ok(output));
            }
            return;
        }
...
async fn prepare_grep(
    ctx: &kimix_tool_runtime::ToolCallContext,
    input: &GrepSearchInput,
) -> Result<GrepStep, kimix_tool_runtime::ToolError> {
...
struct BodyStreamer<'a> {
    spec: &'a kimix_tool_protocol::StreamingSpec,
...
impl<'a> BodyStreamer<'a> {
    fn new(spec: &'a kimix_tool_protocol::StreamingSpec, config: &'a GrepFormatConfig) -> Self {
        Self {
            spec,
            config,
            body: String::new(),
    ...
    fn feed(&mut self, bytes: &[u8]) -> Vec<kimix_tool_runtime::ToolProgress> {
    ...
    fn finish(&mut self) -> Option<kimix_tool_runtime::ToolProgress> {
    ...
    fn push_line(&mut self, line: &[u8]) -> Option<kimix_tool_runtime::ToolProgress> {
        ...
        kimix_tool_runtime::stream_chunk(
            self.spec,
            self.body.as_bytes(),
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        let tool = GrepTool;
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "grep");
    ...
    async fn tool_grep_no_matches() {
        ...
        let tool = GrepTool;
        let output = kimix_tool_runtime::Tool::run(
            &tool,
    ...
    async fn deny_read_globs_exclude_denied_files() {
        ...
        let run = |resources: Resources, input| async {
            let out =
                kimix_tool_runtime::Tool::run(&GrepTool, test_ctx(resources.into_shared()), input)
                    .await
                    .unwrap();
    ...
    fn read_grep_delta(p: &kimix_tool_runtime::ToolProgress) -> String {
        match p {
    ...
    async fn grep_streaming_body_matches_card_body() {
        ...
        let mut terminal: Option<Result<GrepSearchOutput, kimix_tool_runtime::ToolError>> = None;
    ...
    async fn grep_streaming_suppressed_when_gate_off() {
        ...
        let tool = GrepTool;
        let mut stream =
            kimix_tool_runtime::Tool::execute(&tool, ctx, make_grep_input("findme")).await;

        let mut progress = 0usize;
        let mut terminal: Option<Result<GrepSearchOutput, kimix_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                kimix_tool_runtime::ToolStreamItem::Progress(_) => progress += 1,
                kimix_tool_runtime::ToolStreamItem::Terminal(r) => {
                    assert!(terminal.is_none(), "more than one Terminal yielded");
                    terminal = Some(r);
                }
    ...
    async fn grep_streaming_body_matches_card_body_when_truncated() {
        ...
        let tool = GrepTool;
        let mut stream =
            kimix_tool_runtime::Tool::execute(&tool, test_ctx(resources.into_shared()), input)
                .await;
