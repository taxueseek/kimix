...
impl crate::types::tool_metadata::ToolMetadata for EnterPlanModeTool {
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        use crate::implementations::kimix::exit_plan_mode::ExitPlanModeTool;
        Expr::Value(ToolRequirement::Tool {
            namespace: crate::types::tool_metadata::ToolMetadata::tool_namespace(&ExitPlanModeTool)
                .to_string(),
            id: kimix_tool_runtime::Tool::id(&ExitPlanModeTool).to_string(),
            if_params: None,
        })
    }
}

impl kimix_tool_runtime::Tool for EnterPlanModeTool {
    type Args = EnterPlanModeInput;
    type Output = EnterPlanModeOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("enter_plan_mode").expect("valid tool id")
    }
    ...
    ) -> Result<EnterPlanModeOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_is_read_only() {
        let tool = EnterPlanModeTool;
        assert!(kimix_tool_runtime::Tool::capabilities(&tool).is_read_only);
    }
    ...
    async fn no_absolute_path_does_not_write() {
        ...
        assert_eq!(plan_file_path, ".kimix/plan.md");
    ...
    async fn uses_plan_file_path_resource_when_set() {
        let mut resources = Resources::new();
        let session_plan = PathBuf::from("/home/user/.kimix/sessions/abc123/plan.md");
        ...
        assert!(!plan_file_path.contains(".kimix/plan.md"));
    }
    ...
    async fn falls_back_to_cwd_when_no_plan_file_path_resource() {
        ...
        assert_eq!(plan_file_path, "/workspace/my-project/.kimix/plan.md");
    }
    ...
    async fn plan_file_path_prefers_resource_over_cwd() {
        ...
        resources.insert(PlanFilePath(PathBuf::from(
            "/home/user/.kimix/sessions/xyz/plan.md",
        )));
        let shared = resources.into_shared();
        ...
        assert_eq!(plan_file_path, "/home/user/.kimix/sessions/xyz/plan.md");
        assert!(!plan_file_path.contains(".kimix/plan.md"));
    }
}
