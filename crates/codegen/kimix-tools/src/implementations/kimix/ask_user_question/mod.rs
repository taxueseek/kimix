...
/// params are injected, `KIMIX_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `KIMIX_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "KIMIX_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("kimix", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. Kimix callers leave it `None`.
...
/// session-owned coordinator (in Kimix-shell), which performs an ACP
...
impl AskUserQuestionTool {
    ...
    ) -> Result<AskUserQuestionOutput, kimix_tool_runtime::ToolError> {
...
impl kimix_tool_runtime::Tool for AskUserQuestionTool {
    type Args = AskUserQuestionInput;
    type Output = AskUserQuestionOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("ask_user_question").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        if sender.0.send(request).is_err() {
            return Err(kimix_tool_runtime::ToolError::execution(
                kimix_tool_protocol::ToolId::new("ask_user_question").expect("valid"),
        ...
        match result {
            ...
            Err(UserQuestionError::TransportError(msg)) => {
                Err(kimix_tool_runtime::ToolError::execution(
                    kimix_tool_protocol::ToolId::new("ask_user_question").expect("valid"),
...
mod tests {
    ...
    fn tool_is_read_only() {
        assert!(kimix_tool_runtime::Tool::capabilities(&AskUserQuestionTool).is_read_only);
    }
    ...
    async fn fallback_ask_single_question() {
        ...
        let result =
            kimix_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "test-call"), input)
                .await
                .unwrap();
    ...
    async fn fallback_sends_notification() {
        ...
        kimix_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "call-q"), input)
            .await
            .unwrap();
    ...
    async fn blocking_round_trip_accepted() {
        ...
        let handle = tokio::spawn({
            ...
            async move {
                kimix_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-1"), input)
                    .await
            }
        });
    ...
    async fn blocking_round_trip_cancelled() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                kimix_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-4"), input)
                    .await
            }
        });
    ...
    async fn answer_before_timeout_still_succeeds() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                kimix_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-ok"), input)
                    .await
            }
        });
    ...
    async fn channel_drop_returns_error() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                kimix_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-5"), input)
                    .await
            }
        });
    ...
    async fn transport_error_not_cancel() {
        ...
        let handle = tokio::spawn({
            let shared = shared.clone();
            async move {
                kimix_tool_runtime::Tool::run(&tool, test_ctx_with_call_id(shared, "tc-6"), input)
                    .await
            }
        });
