...
//!
//! - **`Kimix-tools`** — tool blocks on a oneshot, formats the result.
//! - **`Kimix-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`Kimix-tui`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `Kimix-tools`.
...
register_resource!("kimix", "UserQuestionSender", UserQuestionSender);
