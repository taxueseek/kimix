...
impl kimix_tool_runtime::Tool for WebFetchTool {
    type Args = WebFetchInput;
    type Output = WebFetchOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("web_fetch").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: WebFetchInput,
    ) -> Result<WebFetchOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(kimix_tool_runtime::Tool::id(&tool).as_str(), "web_fetch");
        assert_eq!(
