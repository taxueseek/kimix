...
/// `TaskOutput` cap. Env override: `KIMIX_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("KIMIX_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let kimix_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashTool).to_string(),
        id: kimix_tool_runtime::Tool::id(&BashTool).as_str().to_string(),
    ...
    let kimix_concise_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashConciseTool).to_string(),
        id: kimix_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
...
pub(crate) fn task_output_requires_expr() -> Expr<ToolRequirement> {
    ...
    let task_tool = Expr::Value(ToolRequirement::Tool {
        ...
        id: kimix_tool_runtime::Tool::id(&TaskTool).as_str().to_string(),
        if_params: None,
    });
...
impl TaskOutputTool {
    async fn run_single_task(
    ...
    ) -> Result<TaskOutputOutput, kimix_tool_runtime::ToolError> {
        let contract_version = ctx
            .extensions
            .get::<kimix_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
        ...
        let waits = kimix_tool_types::task_output_waits(timeout_ms);
...
pub(crate) use kimix_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&kimix_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default Kimix toolset.
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            kimix_tool_types::build_task_output_description(
                &kimix_tool_types::TaskOutputToolNaming {
...
/// [`kimix_tool_types::build_task_output_description`] builder so the CLI and
...
fn task_output_description(
    ...
    kimix_tool_types::build_task_output_description(&kimix_tool_types::TaskOutputToolNaming {
...
impl kimix_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default Kimix
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::kimix::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::kimix::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::kimix::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::kimix::task::types::SubagentEvent,
    ) -> crate::implementations::kimix::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::kimix::task::types::SubagentEvent::Query(q) => q,
