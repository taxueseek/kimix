...
use kimix_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            kimix_tool_types::build_wait_tasks_description(&kimix_tool_types::WaitTasksToolNaming {
...
/// [`kimix_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl kimix_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout =
            crate::implementations::kimix::task_output::capped_wait_timeout(input.timeout_ms);
