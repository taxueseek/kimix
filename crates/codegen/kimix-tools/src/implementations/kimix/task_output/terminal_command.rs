...
use kimix_tool_types::{TaskOutputOutput, TaskOutputToolInput};
...
impl kimix_tool_runtime::Tool for GetTerminalCommandOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("get_terminal_command_output").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        kimix_tool_runtime::Tool::run(&TaskOutputTool, ctx, input).await
    }
}
...
mod tests {
    use super::*;
    use crate::implementations::kimix::task_output::test_helpers::{
