...
//! ## Plan File
//!
//! The plan file path defaults to `.Kimix/plan.md` relative to the session
...
Use this after you have finished writing your plan to the plan file in plan mode."#
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        use crate::implementations::kimix::enter_plan_mode::EnterPlanModeTool;
        Expr::Value(ToolRequirement::Tool {
            namespace: crate::types::tool_metadata::ToolMetadata::tool_namespace(
                &EnterPlanModeTool,
            )
            .to_string(),
            id: kimix_tool_runtime::Tool::id(&EnterPlanModeTool).to_string(),
            if_params: None,
        })
    }
}

impl kimix_tool_runtime::Tool for ExitPlanModeTool {
    type Args = ExitPlanModeInput;
    type Output = ExitPlanModeOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("exit_plan_mode").expect("valid tool id")
    }
    ...
    ) -> Result<ExitPlanModeOutput, kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_is_read_only() {
        assert!(kimix_tool_runtime::Tool::capabilities(&ExitPlanModeTool).is_read_only);
    }
    ...
    async fn exit_with_plan_content() {
        ...
        let plan_dir = tmp.path().join(".kimix");
        ...
        match result {
            ExitPlanModeOutput::PlanReady {
                ...
                assert!(plan_file_path.ends_with(".kimix/plan.md"));
            }
    ...
    async fn sends_plan_mode_exited_notification_with_content() {
        ...
        match notification {
            ToolNotification::PlanModeExited(exited) => {
                ...
                assert!(exited.plan_file_path.ends_with(".kimix/plan.md"));
            }
    ...
    async fn prompt_format_includes_plan_content() {
        ...
        assert!(prompt.contains(".kimix/plan.md"));
