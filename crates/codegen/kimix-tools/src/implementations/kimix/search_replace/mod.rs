...
register_resource!("kimix", "SearchReplace", SearchReplaceParams);
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(kimix_tool_runtime::ToolError::execution(
                        kimix_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl kimix_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = kimix_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = kimix_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
