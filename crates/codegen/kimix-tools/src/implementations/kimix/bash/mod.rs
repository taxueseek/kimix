...
pub struct BashParams {
    ...
    /// Kimix opts up to 10h via Kimix-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "kimix.Bash";
...
impl kimix_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<kimix_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => kimix_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production Kimix sets it to 10h in Kimix-shell's
...
/// [`kimix_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `KIMIX_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("KIMIX_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in Kimix bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the Kimix
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`KIMIX_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `KIMIX_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = kimix_config::shell::chain_separator();
...
impl kimix_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: kimix_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> kimix_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield kimix_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield kimix_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield kimix_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, kimix_tool_runtime::ToolError> {
        ...
        let ampersand = kimix_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(kimix_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing Kimix callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (Kimix backend) and can surface an educational
...
mod tests {
    ...
    impl TerminalBackend for MockTerminal {
        ...
        async fn run_background(
            ...
            *self.captured_bg_request.lock().expect("lock poisoned") = Some(request);
    ...
    fn read_chunk_progress(p: &kimix_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let mut ctx = kimix_tool_runtime::ToolCallContext::default();
        ctx.extensions.insert(resources.into_shared());
        let tool = BashTool;
        let mut stream = kimix_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, kimix_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                kimix_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = kimix_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
        ...
        let req = captured.lock().expect("lock poisoned");
        let req = req
            .as_ref()
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = kimix_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(kimix_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // Kimix-shell opts up to 10h via max_timeout_secs. Background unaffected.
    ...
    mod versioned_background_check {
        ...
        fn legacy_rt_ctx(
            ...
            let mut ctx =
                kimix_tool_runtime::ToolCallContext::new(kimix_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(kimix_tool_runtime::BehaviorVersion(
