...
mod versions;
use crate::types::schema::KimixIntegerSchema;
...
crate::register_resource!("kimix", "ReadFile", ReadFileParams);
...
/// `PlainText` / `Append`.
static READ_FILE_CAPABILITIES: LazyLock<kimix_tool_protocol::ToolCapabilities> =
...
/// an empty window (harness-aligned; not a Kimix-line clamp).
    ...
    if !is_skill_file && token_count > MAX_NUM_TOKENS {
        let (grep_name, execute_name);
        {
            ...
            grep_name = renderer
                ...
                .map_err(|e| kimix_tool_runtime::ToolError::invalid_arguments(e.to_string()))?;
...
impl kimix_tool_runtime::Tool for ReadFileTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;
    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("read_file").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn execute(
        ...
        input: ReadFileInput,
    ) -> kimix_tool_runtime::ToolStream<ReadFileOutput> {
        ...
        let Some(spec) = admitted_spec else {
            ...
            return Box::pin(async_stream::stream! {
                yield kimix_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input)
                . await);
            });
        };
        Box::pin(async_stream::stream! {
            ...
            kimix_tool_runtime::stream_chunk(spec, & content[..window_end], window_end
            as u64, & mut last_total, false,) { yield
            kimix_tool_runtime::ToolStreamItem::Progress(p); } window_start =
            window_end; } } yield
            kimix_tool_runtime::ToolStreamItem::Terminal(Ok(output)); } Err(e) => yield
            kimix_tool_runtime::ToolStreamItem::Terminal(Err(e)), }
        })
    }
...
impl ReadFileTool {
    ...
    async fn read_with_streamability(
        ctx: &kimix_tool_runtime::ToolCallContext,
        input: ReadFileInput,
    ) -> Result<(ReadFileOutput, bool), kimix_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn read_file_basic() {
        ...
        let result = kimix_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
            .await
            .unwrap();
        match result {
    ...
    async fn legacy_read_file_not_found_returns_exact_historical_message() {
        ...
        let result = kimix_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        let expected = format!(
            "Failed to read file: {}",
    ...
    async fn current_read_file_not_found_returns_structured_not_found() {
        ...
        let result = kimix_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap();
        match result {
            ReadFileOutput::FileNotFound(msg) => {
...
";
    ...
    async fn skill_file_ignores_offset_and_limit() {
        let tmp = TempDir::new().unwrap();
        let skill_dir = tmp.path().join(".kimix/skills/commit");
        ...
        let resources = test_resources(tmp.path());
        let input = ReadFileInput {
            path: ".kimix/skills/commit/SKILL.md".to_string(),
            offset: Some(3),
            limit: Some(1),
            pages: None,
            format: None,
    ...
    async fn run_read_file_on(filename: &str, content: &[u8]) -> ReadFileOutput {
        ...
        kimix_tool_runtime::Tool::run(&ReadFileTool, test_ctx(resources.into_shared()), input)
            .await
            .unwrap()
    }
    #[tokio::test]
    ...
    async fn execute_collect(
        ...
        let mut stream = kimix_tool_runtime::Tool::execute(&ReadFileTool, ctx, input).await;
    ...
    /// phantom field only (start past any `split_inclusive` line), so Kimix
