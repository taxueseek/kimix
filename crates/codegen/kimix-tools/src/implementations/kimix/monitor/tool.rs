...
impl kimix_tool_runtime::Tool for MonitorTool {
    type Args = MonitorInput;
    type Output = MonitorOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("monitor").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: MonitorInput,
    ) -> Result<MonitorOutput, kimix_tool_runtime::ToolError> {
