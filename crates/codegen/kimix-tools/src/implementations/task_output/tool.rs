...
//! Helper functions remain for use by `Kimix/task_output/`.
...
mod tests {
    ...
    fn test_snapshot_to_result_wraps_long_lines() {
        ...
        let output = ToolOutput::TaskOutput(kimix_tool_types::TaskOutputOutput::Result(result));
    ...
    fn test_snapshot_to_result_prefers_display_command() {
        ...
        snapshot.display_command = Some("cargo test -p kimix-shell".to_string());

        let result = snapshot_to_result(snapshot, "read_file", DEFAULT_TOOL_OUTPUT_BYTES);
        assert_eq!(
            result.command, "cargo test -p kimix-shell",
