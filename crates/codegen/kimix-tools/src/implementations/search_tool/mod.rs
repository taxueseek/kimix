...
impl kimix_tool_runtime::Tool for SearchTool {
    type Args = SearchToolInput;
    type Output = ToolOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("search_tool").expect("valid tool id")
    }
...
mod tests {
    ...
    fn hash_value_pinned_output() {
        ...
        let h = hash_value(&"kimix-mcp-fingerprint-stability-test");
        assert_eq!(h, hash_value(&"kimix-mcp-fingerprint-stability-test"));
