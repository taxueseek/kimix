...
///
/// `skills` is the standard layout (`.Kimix/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Kimix sessions.
...
/// name (e.g. `~/.kimix/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Kimix.
...
/// `cwd` (exclusive). At each directory, checks for `.Kimix/skills/`,
...
/// visits known config roots (`.Kimix`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.Kimix` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".kimix", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.kimix is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.kimix/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.Kimix/ → kept (user content).
        let kimix_shell = tmp.path().join(".kimix").join("skills").join("shell");
        std::fs::create_dir_all(&kimix_shell).unwrap();
        std::fs::write(
            kimix_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (kimix_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.kimix/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .Kimix skill in an intermediate dir.
        ...
        .unwrap();
        let kimix_skill = sub.join(".kimix").join("skills").join("kimix-dyn");
        std::fs::create_dir_all(&kimix_skill).unwrap();
        std::fs::write(kimix_skill.join("SKILL.md"), "---\nname: kimix-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"kimix-dyn"),
            "kimix-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only Kimix-dyn discovered.
        ...
        assert!(
            names_off.contains(&"kimix-dyn"),
            "kimix-dyn missing: {names_off:?}"
        );
        assert!(
