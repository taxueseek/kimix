...
// New implementation is in `Kimix/skill/`.
...
///
/// Supported variables (Kimix-native names + compat aliases):
///
...
/// | `${KIMIX_PLUGIN_ROOT}` | `${CLAUDE_PLUGIN_ROOT}` | Plugin root dir (plugin-backed skills) |
/// | `${KIMIX_PLUGIN_DATA}` | `${CLAUDE_PLUGIN_DATA}` | Plugin data dir (plugin-backed skills) |
///
...
/// `load_skill_content` in `Kimix/skill/mod.rs` is a duplicate
/// of this.
...
It has multiple lines."#;
    ...
    fn test_build_skill_message_exact_format() {
        let skill = SkillInfo {
            ...
            author: None,
            argument_hint: None,
            path: "/home/user/.kimix/skills/commit/SKILL.md".to_string(),
            scope: SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
        ...
        let expected = "\
<skill name=\"commit\" description=\"Create a git commit\" path=\"/home/user/.kimix/skills/commit/SKILL.md\">
# Git Commit Skill
...
<skill name=\"empty\" description=\"An empty skill\" path=\"/skills/empty/SKILL.md\">
    ...
    fn test_build_skill_message_multiline_content() {
        let skill = SkillInfo {
            ...
            path: "/repo/.kimix/skills/review/SKILL.md".to_string(),
            ..SkillInfo::default()
        };
        ...
        let expected = "\
<skill name=\"review\" description=\"Review code\" path=\"/repo/.kimix/skills/review/SKILL.md\">
# Code Review
...
- No warnings
    ...
    fn test_substitutions_skill_dir() {
        ...
        apply_substitutions(
            ...
            None,
            &SubstitutionContext {
                skill_dir: Some("/home/user/.kimix/skills/deploy"),
                ..Default::default()
            },
        );
        assert_eq!(
            content,
            "Config at /home/user/.kimix/skills/deploy/config.json"
        );
    }
    ...
    #[test]
    fn test_kimix_plugin_aliases_substitution() {
        let mut content = "Root ${KIMIX_PLUGIN_ROOT}, data ${KIMIX_PLUGIN_DATA}".to_string();
        apply_substitutions(
            &mut content,
            None,
            &SubstitutionContext {
...
Review code.
    ...
    fn test_build_skill_information_single() {
        ...
        let refs = vec![SkillRef {
            name: "commit",
            path: "/home/user/.kimix/skills/commit/SKILL.md",
        }];
        ...
        assert!(
            result.contains(
                "<skill name=\"commit\" path=\"/home/user/.kimix/skills/commit/SKILL.md\"/>"
            )
        );
    ...
    fn test_build_skill_information_multi() {
        ...
        let refs = vec![
            SkillRef {
                name: "review",
                path: "/project/.kimix/skills/review/SKILL.md",
            },
            SkillRef {
                name: "lint",
                path: "/project/.kimix/skills/lint/SKILL.md",
            },
        ];
        let result = build_skill_information(&blocks, &refs);
        assert!(result.starts_with("<skill_information>\n"));
