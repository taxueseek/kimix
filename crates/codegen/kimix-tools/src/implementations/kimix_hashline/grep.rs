...
use crate::implementations::kimix::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl kimix_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = kimix_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(kimix_tool_runtime::Cwd(cwd));
        let mut result = kimix_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                kimix_tool_runtime::ToolError::execution(
                    kimix_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
