...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"Kimix:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"Kimix:run_terminal_cmd"`).
...
/// between namespaces (e.g. `Kimix:run_terminal_cmd` vs.
/// `KimixConcise:run_terminal_cmd`).
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("Kimix:run_terminal_cmd", "legacy-0.4.10"),
            ("Kimix:read_file", "legacy-0.4.10"),
            ("Kimix:search_replace", "legacy-0.4.10"),
            ("Kimix:get_task_output", "legacy-0.4.10"),
            ("Kimix:kill_task", "legacy-0.4.10"),
            ("Kimix:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("Kimix:run_terminal_cmd", "current"),
            ("Kimix:read_file", "current"),
            ("Kimix:search_replace", "current"),
            ("Kimix:list_dir", "current"),
            ("Kimix:grep", "current"),
            ("Kimix:kill_task", "current"),
            ("Kimix:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "Kimix:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "Kimix:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "Kimix:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "Kimix:web_search", None).unwrap();
        assert_eq!(v, None);
    }
    ...
    fn unknown_preset_errors() {
        let err = resolve_version("nonexistent", "Kimix:run_terminal_cmd", None).unwrap_err();
        assert!(err.contains("unknown behavior_preset"));
    }

    #[test]
    fn unknown_override_version_errors() {
        let err = resolve_version("current", "Kimix:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // KimixConcise tools should NOT be version-managed.
        assert!(!is_version_managed("KimixConcise:run_terminal_cmd"));
        let v = resolve_version("current", "KimixConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("Kimix:run_terminal_cmd"));
        assert!(is_version_managed("Kimix:read_file"));
        assert!(is_version_managed("Kimix:search_replace"));
        assert!(is_version_managed("Kimix:list_dir"));
        assert!(is_version_managed("Kimix:grep"));
        assert!(is_version_managed("Kimix:kill_task"));
        // Not managed:
        assert!(!is_version_managed("Kimix:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res = resolve_version_with_warnings("current", "Kimix:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "Kimix:run_terminal_cmd", None).unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let err = validate_and_resolve("Kimix:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res = resolve_version_with_warnings("current", "Kimix:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err = resolve_version_with_warnings("current", "Kimix:grep", Some("legacy-0.4.10"))
            .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
