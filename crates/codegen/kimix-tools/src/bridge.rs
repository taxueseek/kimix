//! ToolBridge: adapter that wraps `Kimix-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// Get a compact builder (L1+L2 tools only, ~28 tools vs ~45).
    /// Reduces initial context by ~40%.
    pub fn get_builder_compact() -> ToolRegistryBuilder {
        ToolRegistryBuilder::new_compact()
    }
    ...
    /// `Some("get_task_output")` for the Kimix agent and `None` for
    ...
    pub async fn seed_skill_discovery(
        ...
        tracker.set_compat(compat);
        // Production default: do not inject the full skill table at session
        // start. Index lives on disk (SKILL-ROUTER.md); mid-session discovery
        // announcements remain budget-capped.
        tracker.set_router_only(true);
        tracker.seed(
            cwd,
            git_root,
            startup_skills,
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, kimix_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
            Ok(terminal.kill_task(task_id).await)
        } else {
            Err(kimix_tool_runtime::ToolError::invalid_arguments(format!(
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, kimix_tool_runtime::ToolError> {
        use crate::implementations::kimix::scheduler::types::{SchedulerCommand, SchedulerHandle};
        let sender = {
            let res = self.registry.resources.lock().await;
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    kimix_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                kimix_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
...
mod tests {
    ...
    impl kimix_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> kimix_tool_protocol::ToolId {
            kimix_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::kimix_tool_runtime::ListToolsContext,
        ) -> kimix_tool_types::ToolDescription {
            kimix_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + Kimix's snake_case in one registry
