...
const BASH_STATE_START_MARKER: &str = "__KIMIX_BASH_STATE_START__";
const BASH_STATE_END_MARKER: &str = "__KIMIX_BASH_STATE_END__";
const ZSH_STATE_START_MARKER: &str = "__KIMIX_ZSH_STATE_START__";
const ZSH_STATE_END_MARKER: &str = "__KIMIX_ZSH_STATE_END__";

/// Marker emitted by the init path to separate login-shell noise (MOTD, etc.)
/// from the actual state dump on stdout.
const INIT_STATE_MARKER: &str = "__KIMIX_INIT_STATE_MARKER__";
...
pub fn shell_env_overrides() -> HashMap<String, String> {
    HashMap::from([
        ...
        // Re-applied last via [`crate::util::apply_kimix_agent_marker`] so request
        // env cannot clear it.
        (
            crate::util::KIMIX_AGENT_ENV.to_string(),
            crate::util::KIMIX_AGENT_ENV_VALUE.to_string(),
        ),
    ])
}
...
dump_bash_state() {
  ...
  _emit_encoded() {
    ...
    local var_name="$2"
    if [[ -n "$content" ]]; then
      builtin printf 'kimix_snap_%s=$(command base64 -d <<'"'"'KIMIX_SNAP_EOF_%s'"'"'\n' "$var_name" "$var_name"
      command base64 <<<"$content" | command tr -d '\n'
      builtin printf '\nKIMIX_SNAP_EOF_%s\n' "$var_name"
      builtin printf ')\n'
      builtin printf 'eval "$kimix_snap_%s"\n' "$var_name"
    fi
  }

  _emit "__KIMIX_BASH_STATE_START__"

  _emit "$PWD"

  local env_vars
  env_vars=$(builtin export -p 2>/dev/null | command grep -viE '_proxy=|KIMIX_SANDBOX|KIMIX_AGENT=|SUDO_ASKPASS|KIMIX_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
  ...
  _emit "__KIMIX_BASH_STATE_END__"
}
"##;
...
function dump_zsh_state() {
  ...
  _emit "__KIMIX_ZSH_STATE_START__"

  _emit "$PWD"

  local env_vars
  env_vars=$(builtin typeset -xp 2>/dev/null | command grep -viE '_proxy=|KIMIX_SANDBOX|KIMIX_AGENT=|SUDO_ASKPASS|KIMIX_ASKPASS|ELECTRON_RUN_AS_NODE|SSH_AUTH_SOCK|DBUS_SESSION_BUS_ADDRESS|XDG_RUNTIME_DIR|WAYLAND_DISPLAY|GPG_TTY' || true)
  _emit_encoded "$env_vars" "ENV_VARS_B64"
...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match kimix_config::shell::detect_unix_shell_kind() {
            kimix_config::shell::UnixShellKind::Bash => Self::Bash,
            kimix_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }

    /// Resolved absolute path to the shell binary. Falls back from `$SHELL` →
    /// `which` → common dirs → `/bin/<name>`. Result is cached process-wide
    /// in `kimix_config::shell::unix_shell_path`. See that function for
    /// the full cascade. Returns `&'static str`.
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            Self::Bash => kimix_config::shell::UnixShellKind::Bash,
            Self::Zsh => kimix_config::shell::UnixShellKind::Zsh,
        };
        kimix_config::shell::unix_shell_path(kind)
    }
    ...
    pub fn prepare_command(
        ...
        let wrapper = match self.shell {
            ShellKind::Bash => format!(
                ...
                // Re-export KIMIX_AGENT=1 after snapshot eval so agent-definition
...
mod tests {
    ...
    fn shell_env_overrides_marks_agent_terminal() {
        ...
        assert_eq!(
            env.get(crate::util::KIMIX_AGENT_ENV).map(String::as_str),
            Some(crate::util::KIMIX_AGENT_ENV_VALUE)
        );
    ...
    fn parse_dump_valid_bash() {
        let raw = "__KIMIX_BASH_STATE_START__\n\
                    /home/user/project\n\
                    export FOO=bar\n\
                    # end of bash state dump\n\
                    __KIMIX_BASH_STATE_END__\n";
    ...
    fn parse_dump_valid_zsh() {
        let raw = "__KIMIX_ZSH_STATE_START__\n\
                    /tmp\n\
                    typeset -x FOO=bar\n\
                    # end of zsh state dump\n\
                    __KIMIX_ZSH_STATE_END__\n";
    ...
    fn parse_after_marker_found() {
        let output = "Welcome to Ubuntu\nMOTD line\n__KIMIX_INIT_STATE_MARKER__\nactual data\n";
        let result = parse_after_marker(output, "__KIMIX_INIT_STATE_MARKER__");
    ...
    fn shell_state_update_from_dump() {
        ...
        let dump = "__KIMIX_BASH_STATE_START__\n\
                     /new/dir\n\
                     export X=1\n\
                     # end of bash state dump\n\
                     __KIMIX_BASH_STATE_END__\n";
    ...
    async fn test_init_bash() {
        ...
        assert!(
            state.snapshot.contains("kimix_snap_") || state.snapshot.is_empty(),
    ...
    async fn test_prepare_command_and_roundtrip() {
        ...
        // Run "export KIMIX_TEST_VAR=hello" and capture the new state
        let prep = state
            .prepare_command(
                "export KIMIX_TEST_VAR=hello",
                None,
    ...
    async fn test_gpg_tty_excluded_from_snapshot_bash() {
        ...
        let (code, _) = run_command(&mut state, "export GPG_TTY=/kimix-sentinel-tty").await;
        assert_eq!(code, 0);

        let (code, stdout) = run_command(&mut state, "echo \"[$GPG_TTY]\"").await;
        assert_eq!(code, 0);
        assert!(
            !stdout.contains("/kimix-sentinel-tty"),
