...
impl LocalTerminalActor {
    ...
    async fn spawn_persistent_command(
        ...
        // Apply SHELL_ENV_OVERRIDES (TERM=dumb, NO_COLOR, KIMIX_AGENT=1, etc.)
        ...
        crate::util::apply_kimix_agent_marker(&mut cmd);
        ...
        #[cfg(target_os = "linux")]
        if kimix_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| kimix_sandbox::child_net::install_child_network_filter());
            }
        }
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::kimix::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
fn spawn_shell_command(
    ...
    let mut cmd = {
        ...
        // The parent (Kimix) process retains network for the LLM API.
    ...
    let mut build_cmd = |with_breakaway: bool| {
        ...
        let inv = kimix_config::shell::shell_command_argv(command);
        ...
        //     have JOB_OBJECT_LIMIT_BREAKAWAY_OK set — common when Kimix-agent
...
mod tests {
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export KIMIX_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $KIMIX_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/kimix-sentinel-tty".to_string());
