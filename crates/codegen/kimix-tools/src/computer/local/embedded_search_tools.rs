...
//! config helper `Kimix-shell::util::config::resolve_search_tools_enabled`
//! (requirements > env `KIMIX_TOOLS_FIND_BFS` / `KIMIX_TOOLS_GREP_UGREP` (+
//! `KIMIX_FIND_BFS` / `KIMIX_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
//! (release builds, self-extracted to `~/.kimix/vendor/<name>-<ver>-<target>`) →
//! `~/.kimix/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
//! function (which tags itself with a `__kimix_shadow_{name}` marker) or a
...
// Binaries embedded by build.rs when `KIMIX_TOOLS_BUNDLE_{BFS,UGREP}_PATH` is set
// (release pipeline). Self-extracted to `~/.kimix/vendor` on first use, mirroring
// the ripgrep bundling in `Kimix::grep::ripgrep`.
#[cfg(bundle_bfs)]
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("KIMIX_TOOLS_BFS_VER"),
    "-",
    env!("KIMIX_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("KIMIX_TOOLS_UGREP_VER"),
    "-",
    env!("KIMIX_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
/// OS binary again. Gated on the `__kimix_shadow_{name}` marker that
...
fn restore_command(name: &str) -> String {
    format!(
        "if [ -n \"${{__kimix_shadow_{name}-}}\" ]; then \
           unalias {name} 2>/dev/null || true; \
           unset -f {name} 2>/dev/null || true; \
           unset __kimix_shadow_{name} 2>/dev/null || true; \
         fi"
    )
}
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "KIMIX_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "KIMIX_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}

/// Write embedded `bytes` to `~/.kimix/vendor/<versioned_name>` (chmod 755) on
...
fn extract_bundled(versioned_name: &str, bytes: &[u8]) -> std::io::Result<PathBuf> {
    ...
    let dir = crate::util::kimix_home().join("vendor");
...
fn resolve_tool(bin_name: &str, env_override: &str, bundled: Option<PathBuf>) -> Option<PathBuf> {
    resolve_tool_from(
        ...
        bundled,
        crate::util::kimix_home().join("vendor").join(bin_name),
        bin_name,
    )
}

/// Resolution order: explicit env path → bundled (self-extracted) →
/// `~/.kimix/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
/// `__kimix_shadow_{name}=1` marks this as a harness shadow so `restore_command`
...
fn shell_function(
    ...
    // `local __kimix_bin` is re-resolved every call. The host hint is trusted
    ...
    // binary. `|| __kimix_bin=''` keeps the lookup `set -e`-safe (a failed
    ...
    format!(
        "unalias {name} 2>/dev/null || true; \
         {name}() {{ \
           local __kimix_bin={qpref}; \
           [ -x \"$__kimix_bin\" ] || __kimix_bin=$(command -v {bin_name} 2>/dev/null) || __kimix_bin=''; \
           if [ -z \"$__kimix_bin\" ]; then command {name} \"$@\"; return; fi; \
           if [[ -z ${{ZSH_VERSION-}} ]] && (( BASH_SUBSHELL > 0 )); then \
             exec -a {name} \"$__kimix_bin\" {prepend}\"$@\"; \
           else \
             (exec -a {name} \"$__kimix_bin\" {prepend}\"$@\"); \
           fi; \
         }}; \
         __kimix_shadow_{name}=1"
    )
}
...
mod tests {
    ...
    fn shell_function_shape() {
        ...
        // Preferred path is the fast-path hint; the shadow execs `$__kimix_bin`.
        assert!(fn_body.contains("local __kimix_bin=/tmp/bfs"));
        assert!(fn_body.contains("exec -a find \"$__kimix_bin\" \"$@\""));
        // Hint is trusted only when executable (`[ -x ]`, not `[ -f ]`), so a
        // non-exec hint falls through instead of hard-failing exec.
        assert!(fn_body.contains("[ -x \"$__kimix_bin\" ]"));
        assert!(!fn_body.contains("[ -f \"$__kimix_bin\" ]"));
        ...
        assert!(fn_body.contains("__kimix_shadow_find=1"));
    ...
    fn shell_function_unresolved_uses_empty_hint() {
        ...
        assert!(fn_body.contains("local __kimix_bin=''"));
        assert!(fn_body.contains("command -v bfs"));
        assert!(fn_body.contains("command find \"$@\""));
    }
    ...
    fn grep_prepends_ugrep_defaults() {
        ...
        assert!(fn_body.contains("local __kimix_bin=/tmp/ugrep"));
        assert!(fn_body.contains("\"$__kimix_bin\" -G --ignore-files --hidden -I"));
    ...
    fn bash_safe_quote_escapes_metacharacters() {
        ...
        assert!(
            body.contains("local __kimix_bin='/tmp/evil$(id)'"),
            "{body}"
        );
    ...
    fn restore_command_is_marker_gated() {
        ...
        assert!(r.contains("if [ -n \"${__kimix_shadow_find-}\" ]"));
        assert!(r.contains("unalias find"));
        assert!(r.contains("unset -f find"));
        assert!(r.contains("unset __kimix_shadow_find"));
    }
    ...
    fn build_injection_off_emits_marker_gated_restore_not_function() {
        ...
        assert!(inject.contains("if [ -n \"${__kimix_shadow_find-}\" ]"));
        assert!(inject.contains("if [ -n \"${__kimix_shadow_grep-}\" ]"));
    ...
    fn build_injection_on_shadows_resolved_tools() {
        ...
        assert!(inject.contains("__kimix_shadow_find=1"));
    }
    ...
    /// `KIMIX_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.kimix/vendor` and the extracted `bfs` runs.
    #[cfg(all(bundle_bfs, bundle_ugrep))]
    #[test]
    fn bundled_binaries_extract_and_run() {
        let vendor = crate::util::kimix_home().join("vendor");
    ...
    /// mode-0644 `KIMIX_TOOLS_*_PATH` / vendor copy) must fall through to the OS
