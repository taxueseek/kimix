...
impl AsyncFileSystem for LocalFs {
        ...
        match fs::read(path).await {
            ...
            Err(e) => {
                if is_permission_error(&e) {
                    kimix_sandbox::log_violation(&path.display().to_string(), "read");
                }
                Err(e.into())
            }
        }
    ...
    async fn write_file(&self, path: &Path, data: &[u8]) -> Result<(), ComputerError> {
        ...
        {
            if is_permission_error(&e) {
                kimix_sandbox::log_violation(&dir.display().to_string(), "mkdir");
            }
            return Err(e.into());
        }
        if let Err(e) = write_file_with_transient_lock_retries(path, data).await {
            if is_permission_error(&e) {
                kimix_sandbox::log_violation(&path.display().to_string(), "write");
            }
            return Err(e.into());
        }
        Ok(())
    ...
    async fn delete_file(&self, path: &Path) -> Result<(), ComputerError> {
        if let Err(e) = fs::remove_file(path).await {
            if is_permission_error(&e) {
                kimix_sandbox::log_violation(&path.display().to_string(), "delete");
            }
            return Err(e.into());
        }
        Ok(())
