...
use crate::implementations::kimix::task::types::{
...
impl AutoWakeDeliveredIds {
    ...
    ///
    /// Used by the between-turn completion drain in `Kimix-shell` to
...
crate::register_resource!("kimix", "AutoWakeDeliveredIds", AutoWakeDeliveredIds);
...
crate::register_resource!("kimix", "ReportedTaskCompletions", ReportedTaskCompletions);
...
/// events from the structured `Kimix/monitor_event` notification, never by
/// parsing this text.
///
...
pub fn format_monitor_events(
    events: &[crate::implementations::kimix::task::types::MonitorEventNotification],
    task_output_name: Option<&str>,
) -> Option<String> {
    use std::fmt::Write as _;
    ...
    match events {
        ...
        [event] => {
            ...
            let label =
                crate::implementations::kimix::monitor::event::sanitize_monitor_description(label);
            Some(format!(
        ...
        _ => {
            type Event = crate::implementations::kimix::task::types::MonitorEventNotification;
            ...
            for (task_id, group) in &groups {
                let description = group
                    ...
                    .unwrap_or("event");
                let description =
                    crate::implementations::kimix::monitor::event::sanitize_monitor_description(
                        description,
                    );
                let _ = write!(
                    buf,
...
/// scope only when it has no recorded owner (legacy / non-Kimix backends)
...
/// - `Kimix-shell`'s `SessionActor` to sweep matching synthetic
    ...
    async fn collect_reminders(
        ...
        if let Some(terminal) = terminal {
            ...
            let goal_loop_active = res
                .get::<crate::implementations::kimix::task::types::GoalLoopActive>()
                .is_some_and(|g| g.0);
            let surface_reminders =
                !goal_loop_active
                    && res
                        .get::<crate::types::resources::Params<
                            crate::implementations::kimix::bash::BashParams,
                        >>()
...
mod tests {
    ...
    use kimix_tool_types::{MultiTaskOutputResult, TaskOutputResult};
    use std::sync::Arc;
    ...
    fn shared_with_reminders_disabled(tasks: Vec<TaskSnapshot>) -> SharedResources {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        let params = crate::implementations::kimix::bash::BashParams {
    ...
    async fn completions_suppressed_when_goal_loop_active() {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        res.insert(crate::implementations::kimix::task::types::GoalLoopActive(
            true,
        ));
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        res.insert(SubagentEventSender(tx));
        ...
        shared
            .lock()
            .await
            .insert(crate::implementations::kimix::task::types::GoalLoopActive(
                false,
            ));
        let second = reminder.collect_reminders(shared, &output).await;
        assert!(
    ...
    fn format_monitor_events_single_vs_batched() {
        use crate::implementations::kimix::task::types::MonitorEventNotification;
        ...
        let bare = crate::implementations::kimix::task::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::kimix::monitor::event::wrap_monitor_event;
