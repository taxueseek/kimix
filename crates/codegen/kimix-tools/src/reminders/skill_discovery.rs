...
//! after every tool call to check for SKILL.md files in `.Kimix/skills/`,
...
/// Directories that contain skill definitions (`.Kimix/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".kimix", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.Kimix/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.Kimix/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.kimix/) are outside the git root so the walk breaks early.
