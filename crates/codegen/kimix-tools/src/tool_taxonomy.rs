...
/// object (mirroring `Kimix/mcp_tool`). Consumers deserialize it into
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "kimix/tool";
...
impl ToolKind {
    ...
    /// `Kimix/tool`. Exhaustive, so a new `ToolKind` must add a label to compile.
...
///   should read `namespace` loosely (as a string) and, on any `Kimix/tool`
...
impl CanonicalToolMeta {
    ...
    /// (`bash_mode`, `backend`, `Kimix/mcp_tool`, …).
...
mod tests {
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::Kimix => ("kimix", "Kimix"),
                ToolNamespace::KimixConcise => ("kimix_concise", "KimixConcise"),
                ToolNamespace::KimixHashline => ("kimix_hashline", "KimixHashline"),
