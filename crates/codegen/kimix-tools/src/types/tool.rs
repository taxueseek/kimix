...
//!
//! The tool runtime contract (`Tool` trait) lives in `kimix_tool_runtime`.
...
///
/// Serializes to snake_case (`kimix`, `mcp`, …) for the
...
/// (e.g. `"Kimix:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(rename = "kimix", alias = "kimix", alias = "Kimix")]
    Kimix,
    #[serde(rename = "kimix_concise", alias = "KimixConcise")]
    KimixConcise,
    #[serde(rename = "kimix_hashline", alias = "KimixHashline")]
    KimixHashline,
    #[serde(alias = "Codex")]
    Codex,
