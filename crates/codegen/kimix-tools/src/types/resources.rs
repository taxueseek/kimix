...
/// `"namespace.Name"` (e.g., `"Kimix.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"Kimix.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("Kimix", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "Kimix.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"Kimix.ReadFile"`).
    id: String,
...
impl Resources {
    ...
    pub fn require<T: Send + Sync + 'static>(&self) -> Result<&T, kimix_tool_runtime::ToolError> {
...
/// tool falls back to `Cwd/.Kimix/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".kimix/plan.md";
...
/// [`Cwd`]`/.Kimix/plan.md`, else the bare relative `.Kimix/plan.md`.
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), kimix_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn kimix_tool_runtime::ToolDispatch>` so it can
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn kimix_tool_runtime::ToolDispatch>);
...
register_resource!("kimix", "WebCitation", WebCitationCounter);
...
/// depending on `Kimix-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("kimix", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("kimix", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("kimix.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "kimix.ReadFile".to_string(),
            serde_json::json!({ "files_read" : ["loaded.rs"] }),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "kimix.Edit".to_string(),
            serde_json::json!(
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "kimix.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "kimix.Edit").is_none());
    }
    #[test]
