...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.kimix/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.kimix/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.Kimix/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.kimix/ or ~/.claude/.
    User { path: PathBuf },
