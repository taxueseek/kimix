...
//!
//! `ToolSearchIndex` is defined in `Kimix-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `Kimix-shell`
...
/// in `Kimix-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `Kimix-shell` after MCP initialization.
#[derive(Clone)]
