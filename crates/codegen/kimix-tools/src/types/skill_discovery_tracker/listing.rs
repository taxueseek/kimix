...
/// Soft fraction of the context window considered for the skill listing.
/// Always clamped by [`LISTING_HARD_CAP_CHARS`] so large windows (1M+) cannot
/// re-inflate the startup listing.
pub(super) const SKILL_BUDGET_CONTEXT_PERCENT: f64 = 0.02;
/// Hard ceiling on skill-listing characters injected into the conversation.
/// Independent of model context size — large windows must not grow this budget.
/// ~8k chars ≈ 2k tokens; full skill bodies stay on disk (SKILL-ROUTER.md).
pub const LISTING_HARD_CAP_CHARS: usize = 8_000;
/// Default character budget when context window is unknown (= hard cap).
pub(super) const DEFAULT_CHAR_BUDGET: usize = LISTING_HARD_CAP_CHARS;

/// Compute the listing char budget from an optional context window.
///
/// `min(context_tokens * 4 * percent, LISTING_HARD_CAP_CHARS)`. When the
/// window is unknown, returns the hard cap so callers always have a bound.
pub fn compute_listing_budget_chars(
    ...
    let scaled = context_window_tokens
        .map(|tokens| (tokens as f64 * 4.0 * percent) as usize)
        .unwrap_or(LISTING_HARD_CAP_CHARS);
    scaled.min(LISTING_HARD_CAP_CHARS)
}
...
fn listing_header(_tool_name: &str) -> String {
    // Keep short — paths resolve via SKILL-ROUTER.md / skill name (no Absolute path lines).
...
impl<'a> SkillEntry<'a> {
    ...
    fn format(&self, func_desc: &str, desc_budget: usize, wtu_budget: usize) -> String {
        // No Absolute path in markdown listings — paths are resolved from
        // SKILL-ROUTER.md / skill name at invocation time (saves ~100 chars/skill).
    ...
    /// Byte length of the fixed overhead (name, Use when label) excluding content.
    fn overhead(&self) -> usize {
        // "- " + name + ": " + "\n"
        let base = "- ".len() + self.name.len() + ": ".len() + "\n".len();
        if self.when_to_use.is_some() {
            base + "  Use when: ".len() + "\n".len()
        } else {
            base
    ...
    // ── Budgeted XML rendering (Kimix build harness) ─────────────
...
///
/// `"/path/.Kimix/skills/my-skill/SKILL.md"` -> `"/path/.Kimix/skills/"`
...
/// Used by the budgeted (Kimix build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Kimix build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the Kimix
...
/// triggers) instead of a hand-rolled divergent format.
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // Kimix drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: Kimix build harness (budgeted XML) ───────────
    ...
    fn single_skill_within_budget() {
        ...
        assert!(
            ...
            "markdown listing must not embed absolute paths: {text}"
        );
    ...
    #[test]
    fn hard_cap_clamps_large_context_window() {
        // 1M context × 2% × 4 would be ~84k chars; hard cap must win.
        let budget = compute_listing_budget_chars(Some(1_048_576), None);
        assert_eq!(budget, LISTING_HARD_CAP_CHARS);
        let budget_small = compute_listing_budget_chars(Some(300), None);
        assert!(budget_small < LISTING_HARD_CAP_CHARS);
        assert_eq!(
            compute_listing_budget_chars(None, None),
            LISTING_HARD_CAP_CHARS
        );
    }

    #[test]
    fn three_hundred_skills_respect_hard_cap() {
        let skills: Vec<SkillInfo> = (0..300)
            .map(|i| {
                let mut s = skill(&format!("skill-{i:03}"), &"D".repeat(200));
                s.when_to_use = Some("when needed".into());
                s
            })
            .collect();
        let text = announce(&skills, LISTING_HARD_CAP_CHARS).unwrap();
        assert!(
            text.len() <= LISTING_HARD_CAP_CHARS + 100,
            "listing exceeded hard cap: {} chars",
            text.len()
        );
        assert!(!text.contains("Absolute path:"));
    }
