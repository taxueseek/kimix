...
pub struct SkillManager {
    ...
    /// When true, baseline skill dumps (session start / plugin reload /
    /// `/clear`) do **not** inject a full skill listing into the conversation.
    /// Skills remain available via slash commands, skill tools, and
    /// on-disk `SKILL-ROUTER.md`. Mid-session *discovery* announcements still
    /// emit (budget-capped). Default `false` for unit-test compatibility;
    /// production sessions enable this via [`Self::set_router_only`].
    router_only: bool,
...
/// without depending on Kimix-agent.
...
impl SkillManager {
    ...
    /// Suppress baseline skill-listing injection (router-only mode).
    ///
    /// When enabled, session-start / plugin-reload / `/clear` baseline changes
    /// update `AvailableSkills` and mark names announced, but do not inject a
    /// bulk `<system-reminder>` skill table. The model uses SKILL-ROUTER.md.
    pub fn set_router_only(&mut self, enabled: bool) {
        self.router_only = enabled;
    }

    /// Whether baseline skill listings are suppressed.
    pub fn router_only(&self) -> bool {
        self.router_only
    }
    ...
    /// the first `apply_pending_skill_update()` updates `AvailableSkills`
    /// (and, unless [`Self::router_only`], injects a skill listing
    /// `<system-reminder>`).
    ///
    ...
    /// contains any prior skill listing from the previous session.
    ///
    ...
    ///
    /// Listing budget is always hard-capped at [`LISTING_HARD_CAP_CHARS`]
    ...
    pub fn seed(
        ...
        self.listing_budget_chars = Some(compute_listing_budget_chars(
        ...
        // Router-only still needs BaselineChange pending so AvailableSkills
        // is written; take_pending suppresses the reminder text.
    ...
    pub fn take_pending(&mut self) -> Option<(Vec<SkillInfo>, SkillUpdateEffects)> {
        ...
        // Router-only: keep AvailableSkills / slash updates, skip bulk baseline dump.
        // Still mark every listable startup skill announced so mid-session
        // discovery does not re-emit the whole table one-by-one.
        let system_reminder = if self.router_only && matches!(pending, PendingKind::BaselineChange)
        {
            for s in skills {
                if s.enabled && !s.disable_model_invocation {
                    announced.insert(s.dedup_key());
                }
            }
            None
        } else {
            render_listing(skills, &mut announced, &self.render_params())
        };
...
mod tests {
    ...
    // Markdown listings no longer embed Absolute path lines; path rewriting
    // still applies to XML announcements (fullPath attribute).
    ...
    fn display_cwd_rewrites_announcement_paths() {
        ...
        mgr.seed(
            ...
            vec![make_skill(
                "deploy",
                "/overlay/worktree/.kimix/skills/deploy/SKILL.md",
            )],
    ...
    fn no_display_cwd_shows_real_paths() {
        ...
        mgr.seed(
            ...
            vec![make_skill(
                "deploy",
                "/real/path/.kimix/skills/deploy/SKILL.md",
            )],
            None,
            None,
            None,
    ...
    #[test]
    fn markdown_listing_omits_absolute_paths() {
        let mut mgr = SkillManager::new();
        mgr.seed(
            Some("/real/path".into()),
            None,
    ...
    fn budget_cap_truncates_long_descriptions() {
        ...
        let skills: Vec<SkillInfo> = (0..50)
            ...
            .collect();
        // Large context must still be hard-capped.
        let context_window: u64 = 128_000;
        let expected_budget = compute_listing_budget_chars(Some(context_window), None);
        assert_eq!(expected_budget, LISTING_HARD_CAP_CHARS);
    ...
    fn budget_cap_names_only_when_extreme() {
        ...
        let skills: Vec<SkillInfo> = (0..200)
            ...
            .collect();
        // Tiny context window → scaled budget well under hard cap.
    ...
    fn budget_cap_no_truncation_within_budget() {
        ...
        // Hard-capped budget still fits two short skills easily.
    ...
    #[test]
    fn one_meg_context_budget_never_exceeds_hard_cap() {
        let mut mgr = SkillManager::new();
        let skills: Vec<SkillInfo> = (0..300)
            .map(|i| {
                let mut s = make_skill(&format!("s{i}"), &format!("/s/{i}/SKILL.md"));
                s.description = "D".repeat(250);
                s
            })
            .collect();
        mgr.seed(None, None, skills, None, Some(1_048_576), None);
        let r = mgr.take_pending_reconciliation().unwrap();
        let text = r.effects.system_reminder.unwrap();
        assert!(
            text.len() <= LISTING_HARD_CAP_CHARS + 100,
            "1M-window listing must respect hard cap, got {}",
            text.len()
        );
    }

    #[test]
    fn router_only_suppresses_baseline_reminder_keeps_runtime() {
        let mut mgr = SkillManager::new();
        mgr.set_router_only(true);
        let skills = vec![
            make_skill("commit", "/s/commit/SKILL.md"),
            make_skill("review", "/s/review/SKILL.md"),
        ];
        ...
        let r = mgr.take_pending_reconciliation().unwrap();
        assert!(
            r.effects.system_reminder.is_none(),
            "router-only must not inject baseline skill listing"
        );
        assert!(r.effects.send_available_commands);
        assert_eq!(r.effects.kind, SkillUpdateKind::BaselineChange);
        assert_eq!(r.runtime_skills.len(), 2);
        // Startup skills marked announced so they are not re-dumped as discovery.
        assert!(mgr.announced_names().contains("commit"));
        assert!(mgr.announced_names().contains("review"));
    }

    #[test]
    fn router_only_still_announces_mid_session_discovery() {
        let mut mgr = SkillManager::new();
        mgr.set_router_only(true);
        mgr.seed(
            None,
    ...
    fn default_format_produces_markdown() {
        ...
        let text = r.effects.system_reminder.unwrap();
        assert!(
            text.contains("commit:") || text.contains("- commit"),
            "default format should produce markdown listing: {text}"
        );
        assert!(
            !text.contains("Absolute path:"),
            "markdown must not embed absolute paths: {text}"
        );
        assert!(
