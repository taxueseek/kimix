...
///
/// Use with `#[schemars(with = "KimixIntegerSchema")]` on `Option<usize>` fields.
pub struct KimixIntegerSchema;
impl schemars::JsonSchema for KimixIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "kimix_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `Kimix-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use kimix_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
