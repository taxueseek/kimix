use crate::implementations::kimix::bash::BashError;
use crate::implementations::kimix::todo::TodoError;
use crate::implementations::kimix::web_fetch::WebFetchError;
...
/// Generate `impl From<$err> for kimix_tool_runtime::ToolError` that wraps the
/// error as an execution failure tagged with the given tool ID.
macro_rules! impl_runtime_error_from {
    ($($err:ty => $tool_id:literal),+ $(,)?) => {
        $(
            impl From<$err> for kimix_tool_runtime::ToolError {
                fn from(err: $err) -> Self {
                    kimix_tool_runtime::ToolError::execution(
                        kimix_tool_protocol::ToolId::new($tool_id).expect("valid static tool id"),
                        err.to_string(),
                    )
                }
            }
