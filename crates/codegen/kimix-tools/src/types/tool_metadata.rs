//! `ToolMetadata` — Kimix-tools-specific metadata for tools.
//!

//! Each tool implements two traits:
//! 1. `kimix_tool_runtime::Tool` — typed Args/Output, `run()` with actual logic
...
//! Tools access session state through `kimix_tool_runtime::ToolCallContext`
...
/// Kimix-tools-specific metadata trait.
///
/// Each tool struct implements this alongside `kimix_tool_runtime::Tool`.
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (Kimix, Cursor, OpenCode, ...).
...
) -> Result<SharedResources, kimix_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
pub async fn resolve_cwd(
...
) -> Result<PathBuf, kimix_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<kimix_tool_runtime::Cwd>() {
    ...
    res.get::<crate::types::resources::Cwd>()
        .map(|c| c.0.clone())
        .ok_or_else(|| {
            kimix_tool_runtime::ToolError::custom("missing_cwd", "Cwd not available in Resources")
        })
}
...
pub fn test_ctx(resources: SharedResources) -> kimix_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(kimix_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = kimix_tool_protocol::ToolCallId::new(call_id)
        .unwrap_or_else(|_| kimix_tool_protocol::ToolCallId::new_v7());
    let mut ctx = kimix_tool_runtime::ToolCallContext::new(id);
    ctx.extensions.insert(resources);
    ctx.extensions
        .insert(kimix_tool_runtime::WorkspaceViewerContext {
            stream_tool_progress: true,
...
pub fn behavior_version(ctx: &kimix_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
