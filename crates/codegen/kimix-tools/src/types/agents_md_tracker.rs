...
#[cfg(test)]
pub(crate) const RULES_DIRS: &[&str] = &[".kimix/rules", ".claude/rules", ".cursor/rules"];
    ...
    pub async fn check_path(&mut self, target_path: &Path) -> Vec<PathBuf> {
        ...
        while let Some(dir) = current {
            ...
            if !self.checked_dirs.contains(&dir_buf) {
                ...
                // Check for rules files in .Kimix/rules/, .claude/rules/, and
...
mod tests {
    ...
    #[test]
    fn compat_all_on_matches_legacy_constants() {
        use crate::types::compat::CompatConfig;
        let c = CompatConfig::all_on();
