//! Canonical external-settings tool name ↔ Kimix tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`Kimix-hooks`) needs the
//! Kimix tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`Kimix-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `kimix` names are test-checked against the live registry.
use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Kimix, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Kimix [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Kimix tool names this Claude tool maps to (empty when there is no direct
    /// Kimix tool — the entry then only contributes a `kind`).
    kimix: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, kimix: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        kimix,
...
const fn match_only(claude: &'static str, kimix: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        kimix,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (kimix-build reads SKILL.md)
...
/// The Kimix [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Kimix tool names a Claude matcher term fires on.
pub fn kimix_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.kimix)
...
/// The Claude names that map to `kimix_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(kimix_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.kimix.contains(&kimix_name))
        .map(|t| t.claude)
}

/// Every distinct Kimix name the table references, for the `Kimix-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn kimix_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.kimix.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Kimix name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.kimix.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
