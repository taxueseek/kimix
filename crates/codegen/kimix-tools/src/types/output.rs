use kimix_tool_types::SubagentCompletedOutput;
...
use crate::implementations::kimix::todo::{TodoItem, TodoState};
...
impl ToolRunResult {
    ...
    ) -> kimix_tool_runtime::TypedToolOutput {
...
pub(crate) fn typed_tool_output_preserving_cco(
    ...
    source: &impl kimix_tool_runtime::ToolOutput,
) -> kimix_tool_runtime::TypedToolOutput {
    let cco = source.chat_completion_output();
    let value = serde_json::to_value(payload).unwrap_or(serde_json::Value::Null);
    kimix_tool_runtime::TypedToolOutput::from_value(tool_id, value).with_chat_completion_output(cco)
}
...
/// protocol boundary in `Kimix-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the Kimix `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the Kimix backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when Kimix persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
use kimix_tool_types::KillTaskOutput;
...
impl kimix_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<kimix_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => kimix_tool_runtime::ToolOutput::chat_completion_output(bash),
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::kimix::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::kimix::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::kimix::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::kimix::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::kimix::update_goal::UpdateGoalOutput),
...
impl kimix_tool_runtime::ToolOutput for ToolOutput {
    fn chat_completion_output(&self) -> Option<kimix_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => kimix_tool_runtime::ToolOutput::chat_completion_output(bash),
            _ => None,
...
impl kimix_tool_runtime::ToolOutput for BashOutput {
    fn chat_completion_output(&self) -> Option<kimix_tool_runtime::ToolChatCompletionResponse> {
        ...
        Some(kimix_tool_runtime::ToolChatCompletionResponse {
            result: Some(kimix_tool_runtime::ToolChatCompletion {
                sender: "assistant".into(),
                message_tag: Some("raw_function_result".into()),
                code_execution_result: Some(kimix_tool_runtime::ToolCodeExecutionResult {
                    stdout,
                    stderr: String::new(),
...
impl kimix_tool_runtime::ToolOutput for GrepSearchOutput {}
impl kimix_tool_runtime::ToolOutput for ReadFileOutput {}
impl kimix_tool_runtime::ToolOutput for ListDirOutput {}
impl kimix_tool_runtime::ToolOutput for SearchReplaceOutput {}
impl kimix_tool_runtime::ToolOutput for TodoWriteOutput {}
impl kimix_tool_runtime::ToolOutput for WebSearchOutput {}
impl kimix_tool_runtime::ToolOutput for WebFetchOutput {}
impl kimix_tool_runtime::ToolOutput for SkillOutput {}
impl kimix_tool_runtime::ToolOutput for ApplyPatchOutput {}
impl kimix_tool_runtime::ToolOutput for CodexGrepFilesOutput {}
impl kimix_tool_runtime::ToolOutput for SearchToolOutput {}
impl kimix_tool_runtime::ToolOutput for EnterPlanModeOutput {}
impl kimix_tool_runtime::ToolOutput for ExitPlanModeOutput {}
impl kimix_tool_runtime::ToolOutput for AskUserQuestionOutput {}
impl kimix_tool_runtime::ToolOutput for MCPOutput {}
#[cfg(test)]
mod tests {
    use super::*;
    use crate::implementations::kimix::todo::{TodoPriority, TodoStatus};
    use kimix_tool_types::KillTaskResult;
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/kimix-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/kimix-worktree/wt-agent</worktree_path>"),
    ...
    fn assert_cer(
        resp: &kimix_tool_runtime::ToolChatCompletionResponse,
        stdout: &str,
        exit_code: i32,
        timed_out: bool,
    ) {
    ...
    fn bash_output_chat_completion_carries_exit_and_stdout() {
        let resp = kimix_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(
            0, b"hello\n", false,
        ))
        .unwrap();
    ...
    fn bash_output_chat_completion_empty_stdout_still_emits() {
        let resp =
            kimix_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(0, b"", false))
                .unwrap();
    ...
    fn bash_output_chat_completion_truncation_marker_and_extra() {
        ...
        let resp = kimix_tool_runtime::ToolOutput::chat_completion_output(&bash).unwrap();
        let result = resp.result.as_ref().unwrap();
    ...
    fn bash_tool_output_foreground_delegates_background_skips() {
        let resp = kimix_tool_runtime::ToolOutput::chat_completion_output(&BashToolOutput::Bash(
            sample_bash(0, b"ok", false),
        ))
        .unwrap();
        assert_cer(&resp, "ok", 0, false);
        assert!(
            kimix_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn aggregate_tool_output_bash_delegates_background_skips() {
        let resp = kimix_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Bash(
        ...
        assert!(
            kimix_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Text(
    ...
    fn bash_tool_id() -> kimix_tool_protocol::ToolId {
        kimix_tool_protocol::ToolId::new("bash").unwrap()
    }
    #[test]
    fn into_typed_tool_output_preserves_bash_foreground_cco() {
        ...
        let dropped =
            kimix_tool_runtime::TypedToolOutput::from_value(typed.tool_id, expected_value);
    ...
    fn into_typed_tool_output_non_bash_cco_is_none() {
        ...
        let typed = run.into_typed_tool_output(kimix_tool_protocol::ToolId::new("text").unwrap());
