...
//!
//! `MemoryBackend` is defined in `Kimix-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `Kimix-tools`.
/// Mirrors `kimix_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "kimix_memory";
