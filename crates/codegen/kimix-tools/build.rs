//! Build script for bundling ripgrep for the Kimix-tools crate.
//!

//! - If `KIMIX_TOOLS_BUNDLE_RG_PATH` is set, always bundle it
...
/// `KIMIX_TOOLS_BUNDLE_<NAME>_PATH` points at one (supplied by the release
/// pipeline). Emits
...
/// bundled (the runtime resolver falls back to `~/.kimix/vendor` / `$PATH`);
...
fn bundle_search_tool(
    ...
    let override_env = format!("KIMIX_TOOLS_BUNDLE_{name_uc}_PATH");
    ...
    println!("cargo:rustc-env=KIMIX_TOOLS_{name_uc}_VER={ver}");
    println!("cargo:rustc-env=KIMIX_TOOLS_{name_uc}_TARGET=override");
    Ok(())
}
...
fn bundle_rg() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=KIMIX_TOOLS_BUNDLE_RG_PATH");
    ...
    let path_override = env::var("KIMIX_TOOLS_BUNDLE_RG_PATH").ok();
    ...
    // KIMIX_TOOLS_BUNDLE_RG_PATH still bundles regardless of target.
    ...
    println!("cargo:rustc-env=KIMIX_TOOLS_RG_VER={}", RG_VER);
    ...
    if let Some(path) = path_override {
        ...
        println!("cargo:rustc-env=KIMIX_TOOLS_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying KIMIX_TOOLS_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set KIMIX_TOOLS_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!("cargo:rustc-env=KIMIX_TOOLS_RG_TARGET={}", asset_triple);
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet KIMIX_TOOLS_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set KIMIX_TOOLS_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set KIMIX_TOOLS_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
