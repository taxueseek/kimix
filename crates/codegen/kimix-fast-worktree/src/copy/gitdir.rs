...
mod tests {
    ...
    fn test_copy_git_dir_preserves_worktree_source_marker() {
        ...
        // `Kimix-worktree-source` marker so it still points at the ultimate
        ...
        std::fs::create_dir_all(&source_git).unwrap();
        std::fs::write(source_git.join("HEAD"), "ref: refs/heads/main\n").unwrap();
        std::fs::write(source_git.join("kimix-worktree-source"), "/main/repo").unwrap();

        copy_git_dir(&source_git, &dest_git).unwrap();

        assert_eq!(
            std::fs::read_to_string(dest_git.join("kimix-worktree-source")).unwrap(),
            "/main/repo"
        );
    }
