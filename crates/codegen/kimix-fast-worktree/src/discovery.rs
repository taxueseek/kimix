...
pub fn discover_worktrees(kimix_home: &Path) -> DiscoveryReport {
    let mut report = DiscoveryReport::default();
    scan_two_level_dir(
        &kimix_home.join("worktrees"),
...
pub fn rebuild_worktree_db(
    ...
    let discovery = discover_worktrees(kimix_home);
...
mod tests {
    ...
    fn discover_session_worktrees() {
        ...
        let wt = kimix_home.join("worktrees/myrepo/worktree-abc123");
        make_fake_linked_worktree(&wt, "/repo/.git/worktrees/abc123");

        let report = discover_worktrees(kimix_home);
    ...
    fn discover_pool_worktrees() {
        ...
        let wt = kimix_home.join("worktree_pool/inst-1/pool-a");
    ...
    fn skips_dot_prefixed_and_markers() {
        ...
        let base = kimix_home.join("worktrees/myrepo");
    ...
    fn rebuild_registers_and_skips_duplicates() {
        ...
        let wt = kimix_home.join("worktrees/repo/worktree-sess1");
        make_fake_standalone_worktree(&wt);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();

        let r1 = rebuild_worktree_db(&db, kimix_home).unwrap();
        assert_eq!(r1.discovered, 1);
        assert_eq!(r1.registered, 1);
        assert_eq!(r1.already_tracked, 0);

        let r2 = rebuild_worktree_db(&db, kimix_home).unwrap();
    ...
    fn rebuild_keeps_same_basename_worktrees_in_different_repos() {
        ...
        let tmp = tempfile::TempDir::new().unwrap();
        // Canonicalize the fake home: production `resolve_kimix_home()` always
        ...
        let kimix_home = canon_home.as_path();

        let wt_a = kimix_home.join("worktrees/repo-a/wt-abc");
        let wt_b = kimix_home.join("worktrees/repo-b/wt-abc");
        make_fake_standalone_worktree(&wt_a);
        make_fake_standalone_worktree(&wt_b);

        let db = crate::db::WorktreeDb::open_in_memory().unwrap();
        let report = rebuild_worktree_db(&db, kimix_home).unwrap();
        ...
        let report2 = rebuild_worktree_db(&db, kimix_home).unwrap();
