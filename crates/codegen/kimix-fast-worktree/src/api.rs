...
/// handle `~/.kimix/worktrees/<repo>/<session>/`) and calls `remove_worktree()`
...
fn try_btrfs_remove(
    ...
    if worktree_path.is_symlink() {
        ...
        if let Ok(Some(_)) = btrfs::is_btrfs_subvolume(&resolved) {
            ...
            // `btrfs` refuses non-subvolumes, the snapshot dir is Kimix-owned, and
    ...
    if btrfs_info.bind_mount_source.is_some() {
        ...
        kimix_tty_utils::detach_std_command(&mut umount_cmd);
...
pub mod gc {
    ...
    /// symlinked `$KIMIX_SHARE_DIR` or custom worktree path would otherwise never match.
...
mod tests {
    ...
    fn test_cleanup_worktrees_in_with_nested_dirs() {
        ...
        // Create ~/.kimix/worktrees/<repo>/<session>/ structure.
        let worktrees_dir = tmp.path().join("worktrees");
    ...
    fn test_cleanup_worktrees_in_removes_nested_dangling_symlink() {
        // Dangling symlink one level deeper (~/.kimix/worktrees/<repo>/<session>):
    ...
    fn remove_with_delegate_deregisters_plain_worktree_without_calling_delegate() {
        kimix_test_utils::require_git!();
        use kimix_test_utils::git::{git_commit_all, init_git_repo};
        // Isolate KIMIX_SHARE_DIR so the post-removal unregister writes to a private DB.
        #[cfg(feature = "metadata")]
        let _fx = crate::db::KimixHomeFixture::new();
    ...
    fn delete_fallback_invokes_delegate_when_direct_delete_fails() {
        ...
        let report = delete_snapshot_with_delegate_fallback(
            ...
            Path::new("/home/u/.kimix/worktrees/repo/wt"),
            Some(&delegate),
    ...
    fn test_cleanup_orphaned_btrfs_skips_when_mount_target_parent_missing() {
        ...
        let mount_target = unrestored_home.join(".kimix/worktrees/x/wt-live");
        assert!(
    ...
    fn test_cleanup_orphaned_btrfs_skips_active() {
        ...
        let mount_target = std::path::PathBuf::from("/home/user/.kimix/worktrees/active-wt");
    ...
    mod metadata_integration {
        ...
        fn register_worktree_writes_correct_fields() {
            // Isolate KIMIX_SHARE_DIR so register_worktree's open_default write lands
            ...
            // (KIMIX_SHARE_DIR is process-global) can't INSERT-OR-REPLACE our row.
        ...
        fn gc_with_delegate_removes_expired_and_unregisters() {
            ...
            // KIMIX_SHARE_DIR == the gc DB dir so remove_worktree's open_default
