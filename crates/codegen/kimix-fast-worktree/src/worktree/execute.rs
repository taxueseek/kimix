...
/// Record the source repo root in `<worktree>/.git/Kimix-worktree-source`.
///
...
fn record_main_repo_marker(source: &Path, worktree: &Path) {
    ...
    let marker = git_dir.join("kimix-worktree-source");
    if marker.exists() {
        return;
    }
...
mod tests {
    ...
    fn test_marker_written_for_standalone_git_dir() {
        ...
        kimix_test_utils::git::init_git_repo(&source);
        ...
        let marker = dest.join(".git/kimix-worktree-source");
    ...
    fn test_marker_skipped_for_linked_worktree() {
        ...
        assert!(!dest.join(".git/kimix-worktree-source").exists());
    }
