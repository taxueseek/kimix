...
use kimix_sqlite_journal::JournalMode;
...
impl WorktreeDb {
    /// Open (or create) the DB at `kimix_home/worktrees.db`.
    pub fn open(kimix_home: &Path) -> Result<Self> {
        Self::open_at(&kimix_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.kimix/worktrees.db`.
    ///
    /// Discovers Kimix home via `$KIMIX_SHARE_DIR`, falling back to the canonicalized
    /// `$HOME/.Kimix` (matching `kimix_config::kimix_home`).
...
pub fn resolve_kimix_home() -> Result<PathBuf> {
    // Prefer KIMIX_SHARE_DIR; accept legacy KIMIX_SHARE_DIR for old scripts.
    if let Ok(v) = std::env::var("KIMIX_SHARE_DIR").or_else(|_| std::env::var("KIMIX_SHARE_DIR")) {
        return Ok(PathBuf::from(v));
    }
    let home =
        PathBuf::from(std::env::var("HOME").context("neither $KIMIX_SHARE_DIR nor $HOME is set")?);
    // Canonicalize so worktree paths share the same physical ~/.kimix tree as
    // trust/hooks even when $HOME is a symlink. Must stay in sync with
    // kimix_config::default_kimix_home() (always `.kimix`, never `.kimix`).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".kimix"))
}

/// Serializes tests that mutate the process-global `KIMIX_SHARE_DIR` env var so they
...
#[cfg(test)]
static KIMIX_SHARE_DIR_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`KIMIX_SHARE_DIR_ENV_LOCK`] (serializing concurrent setters), points
/// `KIMIX_SHARE_DIR` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `KIMIX_SHARE_DIR` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct KimixHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated Kimix home; pass to `WorktreeDb::open` to read the same DB
...
#[cfg(test)]
impl KimixHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = KIMIX_SHARE_DIR_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("kimix-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via KIMIX_SHARE_DIR, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until KIMIX_SHARE_DIR points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("KIMIX_SHARE_DIR");
        unsafe { std::env::set_var("KIMIX_SHARE_DIR", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for KimixHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("KIMIX_SHARE_DIR", p),
                None => std::env::remove_var("KIMIX_SHARE_DIR"),
            }
        }
    }
}
