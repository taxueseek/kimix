...
pub(crate) fn git_command() -> Command {
    ...
    cmd.envs(kimix_tty_utils::pager_env());
...
fn scratch_index_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "kimix-snapshot-index-{}-{nanos}-{seq}",
        std::process::id()
    ))
}
...
///
/// `ref_name` must be a fully-qualified ref (e.g. `refs/Kimix/subagents/<id>`);
...
fn snapshot_worktree_to_ref_inner(
    ...
    const NAME: &str = "Kimix Snapshot";
    const EMAIL: &str = "kimix-snapshot@example.com";
...
mod tests {
    ...
    fn test_snapshot_worktree_captures_tracked_and_untracked() {
        ...
        let ref_name = "refs/kimix/snapshots/test";
    ...
    fn test_snapshot_excludes_ignored_files() {
        ...
        let ref_name = "refs/kimix/snapshots/ignored";
    ...
    fn test_snapshot_captures_tracked_then_ignored_file() {
        ...
        let ref_name = "refs/kimix/snapshots/tracked-ignored";
    ...
    fn test_snapshot_clean_tree_equals_head_tree() {
        ...
        let ref_name = "refs/kimix/snapshots/clean";
    ...
    fn test_snapshot_overwrites_ref_on_second_call() {
        ...
        let temp = TempDir::new().unwrap();
        let (_repo, wt) = repo_with_worktree(&temp);
        let ref_name = "refs/kimix/snapshots/overwrite";
    ...
    fn test_snapshot_survives_worktree_removal() {
        ...
        std::fs::write(wt.join("tracked.txt"), "edited").unwrap();
        std::fs::write(wt.join("untracked.txt"), "brand new").unwrap();
        let ref_name = "refs/kimix/snapshots/survives";
    ...
    fn test_snapshot_does_not_mutate_real_index() {
        ...
        snapshot_worktree_to_ref(&wt, "refs/kimix/snapshots/noindex", "no mutate").unwrap();
    ...
    fn test_snapshot_preserves_crlf_under_autocrlf_config() {
        ...
        let ref_name = "refs/kimix/snapshots/crlf";
    ...
    fn test_snapshot_captures_non_ascii_spaced_path() {
        ...
        let ref_name = "refs/kimix/snapshots/unicode";
    ...
    fn test_rehydrate_round_trip_restores_working_state() {
        ...
        let snap =
            snapshot_worktree_to_ref(&wt, "refs/kimix/snapshots/roundtrip", "round trip").unwrap();
    ...
    fn test_rehydrate_base_missing_falls_back_to_snapshot() {
        ...
        let snap = snapshot_worktree_to_ref(&wt, "refs/kimix/snapshots/orphan-src", "src").unwrap();
    ...
    fn test_rehydrate_self_heals_over_leftover_dest_dir() {
        ...
        std::fs::write(wt.join("tracked.txt"), "edited").unwrap();
        std::fs::write(wt.join("untracked.txt"), "brand new").unwrap();
        let snap = snapshot_worktree_to_ref(&wt, "refs/kimix/snapshots/idem", "idem").unwrap();
        crate::remove_worktree(&wt).unwrap();
    ...
    fn test_transfer_snapshot_to_repo_makes_standalone_ref_durable() {
        ...
        let ref_name = "refs/kimix/subagents/standalone";
    ...
    fn test_rehydrate_registers_worktree_in_db() {
        ...
        // Isolate the worktree DB (lock + KIMIX_SHARE_DIR → private tmp + restore).
        let fx = crate::db::KimixHomeFixture::new();

        let (repo_path, wt) = repo_with_worktree(&temp);
        std::fs::write(wt.join("tracked.txt"), "edited").unwrap();
        let snap = snapshot_worktree_to_ref(&wt, "refs/kimix/snapshots/db", "db test").unwrap();
        crate::remove_worktree(&wt).unwrap();
        ...
        // global KIMIX_SHARE_DIR) DB and INSERT-OR-REPLACE our row.
        ...
        // other subagent rows since KIMIX_SHARE_DIR is process-global.
