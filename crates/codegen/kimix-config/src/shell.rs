...
//! C++/C#/.NET builds.
//!
//! Set `KIMIX_SHELL` to override auto-detection: `pwsh`, `powershell`,
...
///
/// If `KIMIX_SHELL` is set, it takes precedence over auto-detection.
...
pub fn detect_windows_shell() -> &'static WindowsShell {
    ...
    CACHED.get_or_init(|| {
        // Explicit override via KIMIX_SHELL.
        if let Ok(val) = std::env::var("KIMIX_SHELL") {
            match val.trim().to_ascii_lowercase().as_str() {
                "pwsh" => {
                    tracing::info!("Windows shell (KIMIX_SHELL override): pwsh");
                    return WindowsShell::Pwsh;
                }
                "powershell" => {
                    tracing::info!("Windows shell (KIMIX_SHELL override): powershell.exe");
                ...
                "bash" | "gitbash" | "git-bash" => {
                    if let Some(path) = find_git_bash() {
                        tracing::info!(
                            shell = path,
                            "Windows shell (KIMIX_SHELL override): Git Bash"
                        );
                        return WindowsShell::GitBash(path);
                    }
                    tracing::warn!(
                        "KIMIX_SHELL={val} but Git Bash not found; falling through to auto-detect"
                    );
                }
                "cmd" | "cmd.exe" => {
                    tracing::info!("Windows shell (KIMIX_SHELL override): cmd.exe");
                    return WindowsShell::Cmd;
                }
                other => {
                    tracing::warn!(
                        "KIMIX_SHELL={other} is not recognized \
...
//
//   1. `$KIMIX_SHELL` override, if it names the requested kind and is runnable.
...
fn resolve_unix_shell_path(kind: UnixShellKind) -> String {
    ...
    // 1) Explicit override via $KIMIX_SHELL.
    if let Ok(s) = std::env::var("KIMIX_SHELL") {
...
///
/// The probe is spawned via `kimix_tty_utils::detach_std_command` so that
...
fn is_executable(path: &std::path::Path) -> bool {
    ...
    // Nix fallback. Detach from the controlling TTY via kimix_tty_utils so
