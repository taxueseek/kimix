...
//! preference domain `ai.x.Kimix` (`requirements_toml_base64`). Only admin-*forced*
...
#[cfg(target_os = "macos")]
const MANAGED_PREFERENCES_DOMAIN: &str = "ai.x.kimix";
#[cfg(target_os = "macos")]
...
pub const MDM_REQUIREMENTS_SOURCE: &str = "ai.x.kimix:requirements_toml_base64";
...
fn read_forced_requirements() -> Option<String> {
    ...
    // per-user domain, which a local user can set (`defaults write ai.x.Kimix`)
...
mod tests {
    ...
    fn forced_payload_is_not_env_expanded() {
        ...
        let prior = std::env::var("KIMIX_MDM_NO_EXPAND_TEST").ok();
        unsafe { std::env::set_var("KIMIX_MDM_NO_EXPAND_TEST", "attacker") };
        let decoded = decode_managed_toml(&b64("base_url = \"${KIMIX_MDM_NO_EXPAND_TEST}/v1\"\n"));
        unsafe {
            match prior {
                Some(p) => std::env::set_var("KIMIX_MDM_NO_EXPAND_TEST", p),
                None => std::env::remove_var("KIMIX_MDM_NO_EXPAND_TEST"),
            }
        }
        assert_eq!(
            decoded
                ...
                .and_then(|v| v.as_str()),
            Some("${KIMIX_MDM_NO_EXPAND_TEST}/v1"),
