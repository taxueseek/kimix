...
fn managed_config_stale_at_is_true_without_synced_marker() {
    let dir = std::env::temp_dir().join(format!("kimix-stale-nomark-{}", std::process::id()));
...
fn managed_config_stale_at_is_false_after_fresh_sync() {
    let dir = std::env::temp_dir().join(format!("kimix-stale-fresh-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn managed_deployment_id_at_requires_matching_fingerprint() {
    let dir = std::env::temp_dir().join(format!("kimix-dep-id-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn managed_config_stale_at_is_true_for_old_sync() {
    let dir = std::env::temp_dir().join(format!("kimix-stale-old-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn managed_config_stale_when_served_artifact_deleted() {
    let dir = std::env::temp_dir().join(format!("kimix-stale-artgone-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    mark_managed_config_synced_at(
        &dir,
        SyncMarker {
...
fn managed_config_not_stale_when_nothing_served() {
    let dir = std::env::temp_dir().join(format!("kimix-stale-noart-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    mark_managed_config_synced_at(
        &dir,
        SyncMarker {
...
fn managed_config_stale_on_identity_mismatch() {
    let dir = std::env::temp_dir().join(format!("kimix-stale-ident-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    mark_managed_config_synced_at(
        &dir,
        SyncMarker {
...
fn managed_config_legacy_marker_is_conservative() {
    let dir = std::env::temp_dir().join(format!("kimix-stale-legacy-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn hard_stale_only_on_missing_or_identity() {
    let dir = std::env::temp_dir().join(format!("kimix-hardstale-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    mark_managed_config_synced_at(
        &dir,
        SyncMarker {
...
fn hard_stale_without_marker() {
    let dir = std::env::temp_dir().join(format!("kimix-hardstale-nomark-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let _ = std::fs::remove_file(dir.join(MANAGED_CONFIG_CACHE_FILE));
    assert!(is_managed_config_hard_stale_for_at(&dir, &team("team-a")));
    let _ = std::fs::remove_dir_all(&dir);
...
fn corrupt_marker_reads_as_no_marker_and_allows() {
    let dir = std::env::temp_dir().join(format!("kimix-corrupt-marker-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn deployment_key_switch_is_stale_and_tampered_offline() {
    let dir = std::env::temp_dir().join(format!("kimix-dk-switch-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn pre_upgrade_marker_without_fingerprint_does_not_fire_on_key() {
    let dir = std::env::temp_dir().join(format!("kimix-dk-preupgrade-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap()
...
fn team_path_keys_on_principal_not_key_fingerprint() {
    let dir = std::env::temp_dir().join(format!("kimix-team-nofp-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    mark_managed_config_synced_at(
        &dir,
        SyncMarker {
...
fn identity_changed_only_on_confirmed_switch() {
    let dir = std::env::temp_dir().join(format!("kimix-ident-changed-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn blank_principal_is_never_a_confirmed_switch() {
    let dir = std::env::temp_dir().join(format!("kimix-ident-blank-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn compromised_only_when_opted_in_and_deleted_or_substituted() {
    let dir = std::env::temp_dir().join(format!("kimix-compromised-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn compromised_on_managed_config_deletion_when_fail_closed() {
    let dir = std::env::temp_dir().join(format!("kimix-compromised-mc-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn compromised_on_deployment_key_switch_when_fail_closed() {
    let dir = std::env::temp_dir().join(format!("kimix-compromised-dk-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn gate_excludes_pure_identity_mismatch_but_keeps_artifact_and_key_tamper() {
    let dir = std::env::temp_dir().join(format!("kimix-gate-fix1-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
fn mark_keeps_fail_closed_armed_without_on_disk_file() {
    let dir = std::env::temp_dir().join(format!("kimix-mark-disarm-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
