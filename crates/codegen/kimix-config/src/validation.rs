...
/// [`fail_closed_flag_from_str`]. Env tightening (file vs `KIMIX_MANAGED_CONFIG_FAIL_CLOSED`)
...
/// with `KIMIX_MANAGED_CONFIG_URL`; only applies to `requirements.toml`.
pub(crate) const FAIL_CLOSED_ENV: &str = "KIMIX_MANAGED_CONFIG_FAIL_CLOSED";
...
impl RequirementsSource {
    ...
    /// id (`ai.x.Kimix:…`). For diagnostics and matching only; the MDM layer has no
...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`KIMIX_SHARE_DIR`-influenced, could carry `..`).
    pub is_system: bool,
}
...
pub fn requirements_layers() -> Vec<RequirementsLayer> {
    let mut out = Vec::new();
    if let Some(user_path) = user_kimix_home().map(|g| g.join("requirements.toml"))
...
pub(crate) fn load_requirements() -> Option<toml::Value> {
    load_user_requirements(user_kimix_home().as_deref())
}

/// User requirements layer from `<home>/requirements.toml`, or `None` with no
/// resolvable user home (rather than reading a cwd-relative `.Kimix`).
...
fn validate_requirements_value(
    ...
    let Ok(version) = kimix_version::installed_semver() else {
        return Ok(());
    };
...
pub fn validate_requirements() -> Result<(), RequirementsError> {
    validate_user_requirements(user_kimix_home().as_deref())?;
    if let Some(dir) = system_config_dir() {
...
/// no-op (no cwd-relative `.Kimix/requirements.toml` is read or enforced).
...
mod tests {
    ...
    fn load_requirements_layer_soft_fails_on_invalid_version_overrides() {
        ...
        let dir = std::env::temp_dir().join(format!("kimix-vo-soft-{}", std::process::id()));
...
"#
    ...
    fn validate_requirements_layer_errs_on_fail_closed_violation() {
        ...
        let dir = std::env::temp_dir().join(format!("kimix-vo-validate-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        writeln!(
...
"#
    ...
    fn validate_requirements_layer_ok_without_fail_closed() {
        ...
        let dir = std::env::temp_dir().join(format!("kimix-vo-soft2-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
        writeln!(
...
"#
    ...
    fn fail_closed_key_is_stripped_from_returned_layer() {
        ...
        let dir = std::env::temp_dir().join(format!("kimix-vo-strip-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("requirements.toml");
        let mut f = std::fs::File::create(&path).unwrap();
    ...
    fn load_user_requirements_reads_layer_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("kimix-req-load-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn validate_user_requirements_errs_on_fail_closed_violation() {
        ...
        let dir = std::env::temp_dir().join(format!("kimix-req-validate-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let mut f = std::fs::File::create(dir.join("requirements.toml")).unwrap();
        writeln!(
            f,
