//! Filesystem locations for Kimix config files and binaries.
use std::path::PathBuf;
use std::sync::OnceLock;

static KIMIX_SHARE_DIR: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user Kimix directory (`~/.kimix`, canonicalized) used when
/// `KIMIX_SHARE_DIR` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`kimix_home()`] is the default without duplicating the computation.
///
...
/// `~/.kimix/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `kimix_fast_worktree::db::resolve_kimix_home` (deliberately standalone crate).
pub fn default_kimix_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    let canonical = dunce::canonicalize(&home).unwrap_or(home);

    // Kimix and Kimix are separate products. Always use ~/.kimix.
    // Do NOT fall back to ~/.kimix when that directory happens to exist —
    // that silently mixed two projects' configs, sessions, and auth.
    // Override with $KIMIX_SHARE_DIR when an alternate root is required.
    //
    // One-shot note: if ~/.kimix exists and ~/.kimix has no config.toml yet,
    // callers may copy config manually; we never auto-read kimix data.
    let kimix = canonical.join(".kimix");
    if !kimix.join("config.toml").exists() {
        let legacy = canonical.join(".kimix").join("config.toml");
        if legacy.is_file() {
            tracing::warn!(
                kimix = %legacy.display(),
                kimix = %kimix.join("config.toml").display(),
                "kimix uses ~/.kimix only; found legacy ~/.kimix/config.toml — copy models/auth into ~/.kimix if needed"
            );
        }
    }
    kimix
...
/// Per-user config directory: `$KIMIX_SHARE_DIR` or `~/.kimix`. Created if needed.
pub fn kimix_home() -> PathBuf {
    KIMIX_SHARE_DIR
        .get_or_init(|| {
            let kimix_home = if let Ok(v) = std::env::var("KIMIX_SHARE_DIR") {
                PathBuf::from(v)
            } else {
                default_kimix_home()
            };
...
/// The user-global Kimix home, but only when one genuinely resolves: `Some` when
/// `$KIMIX_SHARE_DIR` is set or a home directory is found, `None` otherwise. Unlike
/// [`kimix_home()`], this never falls back to a cwd-relative `.kimix`, so callers
/// that *scan* user-global Kimix resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.kimix` tree for the user-global one when no home resolves.
pub fn user_kimix_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable =
        std::env::var_os("KIMIX_SHARE_DIR").is_some() || std::env::home_dir().is_some();
    resolvable.then(kimix_home)
}

/// Canonical kimix application path: `$KIMIX_SHARE_DIR/bin/kimix` (Unix) or `kimix.exe` (Windows).
pub fn kimix_application() -> PathBuf {
    let name = if cfg!(windows) { "kimix.exe" } else { "kimix" };
    kimix_home().join("bin").join(name)
}

/// System-wide config directory: `/etc/kimix/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/kimix"))
    } else {
        None
    }
}
...
/// `kimix_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    kimix_home().join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = kimix_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_kimix_home_has_no_verbatim_prefix() {
        ...
        let home = default_kimix_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(
            home.ends_with(".kimix"),
            "default home must be ~/.kimix, got {}",
            home.display()
        );
    }
