//! Config file loading for Kimix.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/Kimix/managed_config.toml`
//! 2. `$KIMIX_SHARE_DIR/managed_config.toml`
//! 3. `$KIMIX_SHARE_DIR/config.toml`
//! 4. `$KIMIX_SHARE_DIR/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/Kimix/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.Kimix`, admin-forced) — macOS only
//!
...
pub use paths::{
    ...
    default_kimix_home, encode_cwd_dirname, ensure_sessions_cwd_dir, kimix_application, kimix_home,
    sessions_cwd_dir, system_config_dir, user_kimix_home,
};
pub use validation::{
