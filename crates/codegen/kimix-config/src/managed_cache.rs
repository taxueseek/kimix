...
//! OS-protected layers (root-owned `/etc/Kimix`, MDM).
use std::path::Path;

use crate::paths::user_kimix_home;
...
pub fn is_managed_config_stale_for(identity: &ServingIdentity) -> bool {
    managed_config_stale_at(user_kimix_home().as_deref(), identity)
}
...
pub fn mark_managed_config_synced(marker: SyncMarker<'_>) {
    if let Some(home) = user_kimix_home() {
        mark_managed_config_synced_at(&home, marker);
    }
}

/// Server-side KimixDeployment UUID from the last deploy-key managed-config
...
pub fn managed_deployment_id(key_fingerprint: &str) -> Option<String> {
    managed_deployment_id_at(user_kimix_home()?.as_path(), key_fingerprint)
}
...
pub fn managed_config_identity_changed(
    ...
    user_kimix_home().is_some_and(|home| {
...
pub fn managed_policy_compromised_for(identity: &ServingIdentity) -> bool {
    user_kimix_home().is_some_and(|home| managed_policy_compromised_for_at(&home, identity))
}
...
/// Override with `KIMIX_DEPLOYMENT_CONFIG_CACHE_TTL_SECS` for testing.
fn managed_config_stale_threshold() -> std::time::Duration {
    if let Ok(s) = std::env::var("KIMIX_DEPLOYMENT_CONFIG_CACHE_TTL_SECS")
