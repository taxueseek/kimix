...
use crate::paths::{system_config_dir, user_kimix_home};
...
/// Kimix config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_kimix_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_kimix_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.Kimix/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.Kimix` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/Kimix`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_kimix_home().as_deref())
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `KIMIX_CAMPAIGNS_OVERRIDE` env layer.
...
/// `KIMIX_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("KIMIX_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`kimix_shell::util::config::load_effective_config`) when remote campaigns
/// or `KIMIX_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// Path to `$KIMIX_SHARE_DIR/campaigns_state.json` under `home`.
pub fn campaigns_state_path(home: &std::path::Path) -> std::path::PathBuf {
    home.join(CAMPAIGNS_STATE_FILE)
}

/// Fail-open dismissed ids from `$KIMIX_SHARE_DIR/campaigns_state.json`.
pub fn load_dismissed_ids_from_home() -> std::collections::HashSet<String> {
    let Some(home) = crate::user_kimix_home() else {
...
/// can't be parsed (broken `KIMIX_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match kimix_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("kimix-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("kimix-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    /// `KIMIX_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `KIMIX_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("KIMIX_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads KIMIX_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("KIMIX_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("KIMIX_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("KIMIX_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("KIMIX_CAMPAIGNS") },
        }
    }
}
