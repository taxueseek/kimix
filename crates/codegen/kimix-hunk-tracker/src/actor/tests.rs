...
impl TestHarness {
    ...
    fn populate(&self, files: usize, files_per_dir: usize) {
        kimix_test_utils::git::write_fanout_tree(&self.working_dir, files, files_per_dir);
    ...
    fn feature_branch(&self, picks: usize) -> String {
        kimix_test_utils::git::make_feature_branch(&self.working_dir, picks)
    }
}
...
/// never wall-clock.
///
/// Knobs: KIMIX_PERF_GIT_FILES (default 5000), KIMIX_PERF_GIT_PICKS (default 10).
#[tokio::test]
#[ignore = "perf repro; run with --ignored --nocapture"]
async fn refresh_storm_scan_count_during_rebase() {
    let files = env_usize("KIMIX_PERF_GIT_FILES", 5000);
    let picks = env_usize("KIMIX_PERF_GIT_PICKS", 10);
    ...
    let t = Instant::now();
    kimix_test_utils::git::run_git_with_env(
        &wd,
        &["rebase", "-i", &base],
