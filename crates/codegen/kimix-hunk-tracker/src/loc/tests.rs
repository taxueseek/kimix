...
impl HunkRecordWriter for SharedWriter {
    ...
    async fn flush(&mut self) -> std::io::Result<()> {
        self.0.lock().expect("lock poisoned").flush_count += 1;
        Ok(())
    }
}
...
async fn sink_processes_added_and_content_changed() {
    ...
    let w = shared.lock().expect("lock poisoned");
    assert_eq!(
        w.records.len(),
        3,
...
async fn sink_continues_after_writer_failure() {
    ...
    impl HunkRecordWriter for FailingWriter {
        ...
        async fn flush(&mut self) -> std::io::Result<()> {
            *self.0.lock().expect("lock poisoned") = true;
            Ok(())
        }
    }
    ...
    assert!(
        *flush_called.lock().expect("lock poisoned"),
