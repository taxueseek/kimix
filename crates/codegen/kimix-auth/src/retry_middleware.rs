...
mod tests {
    ...
    impl MockProvider {
        ...
        fn refresh_count(&self) -> u32 {
            *self.refresh_count.lock().expect("lock poisoned")
        }
    }
    ...
    impl AuthCredentialProvider for MockProvider {
        fn snapshot(&self) -> CredentialSnapshot {
            CredentialSnapshot {
                token: self.token.lock().expect("lock poisoned").clone(),
                ..Default::default()
            }
        }

        async fn refresh_after_unauthorized(&self) -> bool {
            *self.refresh_count.lock().expect("lock poisoned") += 1;
            self.refresh_result
        }
    }
    ...
    impl AuthCredentialProvider for SimulatedAuthManager {
        ...
        async fn refresh_after_unauthorized(&self) -> bool {
            *self.refresh_count.lock().expect("lock poisoned") += 1;
            *self.token.lock().expect("lock poisoned") = Some(self.fresh_token.clone());
            true
        }
    }
    ...
    async fn test_e2e_stale_token_refreshed_and_retried() {
        ...
        assert_eq!(*p.refresh_count.lock().expect("lock poisoned"), 1);
