...
//!
//! These tests touch global process state — `KIMIX_SHARE_DIR` (a `OnceLock` in
//! `Kimix-config`), `KIMIX_TEST_VERSION` — so they
//! must run serially. Once `KIMIX_SHARE_DIR` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the KIMIX_SHARE_DIR isolation used in other
//! integration tests.
mod common;
...
use kimix_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_kimix_home() {
    let _ = test_home();
    reset();
...
/// Write a `KimixVersion`-shaped JSON file with an arbitrary timestamp.
...
/// `KimixVersion` JSON layout.
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under KIMIX_SHARE_DIR,
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every Kimix launch.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_kimix_version env override
//
// The function honors `KIMIX_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_kimix_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("KIMIX_TEST_VERSION", "9.9.9");
    }
    let v = kimix_update::version::get_installed_kimix_version();
    assert_eq!(v, "9.9.9");
    unsafe {
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        kimix_update::version::get_installed_kimix_version()
    };

    unsafe {
        std::env::set_var("KIMIX_TEST_VERSION", "0.0.0-test");
    }
    let overridden = kimix_update::version::get_installed_kimix_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("KIMIX_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = kimix_update::version::get_installed_kimix_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("KIMIX_TEST_VERSION", "not-a-version");
    }
    let v = kimix_update::version::get_installed_kimix_version();
