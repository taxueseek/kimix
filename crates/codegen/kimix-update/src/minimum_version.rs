...
//!
//! When `cli.minimum_version` is set in any config layer, Kimix refuses to
//! start below that floor. With auto-update on, we install
//! `max(latest, minimum)`; otherwise the user is asked to run `kimix update`.
//!
//! Set `KIMIX_TEST_VERSION` to manually exercise either path without producing
//! a real out-of-date build.
use crate::auto_update::{get_installer, run_install_script};
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_kimix_version, write_version_cache,
};
...
pub(crate) enum MinimumVersionError {
    ...
    #[error(
        "The minimum version \"{value}\" in your Kimix configuration \
    ...
    #[error(
        "This version of Kimix ({current}) is no longer supported. \
         Run `kimix update` to install version {minimum} or later."
    )]
    ...
    /// `kimix update --version X` requested a version below the floor.
    #[error(
        "Cannot install Kimix {target}: the configured minimum is {minimum}. \
         Run `kimix update` to install the latest allowed version."
    )]
...
/// Used by `kimix update --version X`.
...
/// Used by `kimix update` to keep the install target at or above the pin.
...
///
/// `KIMIX_TEST_VERSION` lets devs override the running version to skip
...
pub async fn enforce_minimum_version_or_exit(update_config: &UpdateConfig) {
    ...
    match enforce_minimum_version(Some(&min), update_config).await {
        ...
        Ok(EnforcementOutcome::Upgraded) => {
            // TODO: restart_kimix uses exec() which carries the same
...
mod tests {
    ...
    fn version_env_var_flows_through_to_decision() {
        let saved = std::env::var("KIMIX_TEST_VERSION").ok();

        // SAFETY: #[serial] excludes other env-touching tests.
        unsafe { std::env::set_var("KIMIX_TEST_VERSION", "0.1.50") };
        let decision =
            evaluate_minimum_version(&get_installed_kimix_version(), Some("0.1.100")).unwrap();
        assert!(matches!(
            decision,
            MinimumVersionDecision::BelowMinimum { .. }
        ));
        ...
        match saved {
            Some(v) => unsafe { std::env::set_var("KIMIX_TEST_VERSION", v) },
            None => unsafe { std::env::remove_var("KIMIX_TEST_VERSION") },
        }
    }
}
