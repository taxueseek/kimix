...
/// SDK-side id echoed back in `Kimix/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["Kimix/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["Kimix/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `Kimix/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    /// Persisted to `~/.kimix/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: kimix_file_utils::events::EventWriter,
    ...
    /// `Kimix/mcp/server_status` notifications.
    ///
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: kimix_file_utils::events::EventWriter::noop(),
    ...
    /// `KimixClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: kimix_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &kimix_file_utils::events::EventWriter {
        &self.event_writer
    }
...
pub enum McpInitStrategy {
    ...
    /// Terminal default: session create does not wait on MCP handshake.
    #[default]
    Progressive,
}
...
impl McpError {
    ...
    pub fn error_category(&self) -> kimix_file_utils::events::McpErrorCategory {
        use kimix_file_utils::events::McpErrorCategory;
        match self {
...
///   the frontend via `Kimix/mcp/tools_changed` notifications and callable via
///   `Kimix/mcp/call`.
...
/// `kimix_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl kimix_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> kimix_tool_protocol::ToolId {
        ...
        kimix_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| kimix_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        kimix_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(kimix_file_utils::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(kimix_file_utils::events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            kimix_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(kimix_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(kimix_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name,
            duration_ms,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &kimix_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, kimix_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| kimix_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(kimix_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(kimix_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(kimix_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(kimix_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(kimix_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(kimix_file_utils::events::Event::McpTransportDecodeError {
...
enum PendingTransport {
    ...
    /// (`Kimix/mcp/sdk_call`). Rebuildable from its `server_id` + invoker, so handshake
...
/// Uses [`KimixClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, KimixClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`KimixClientHandler`] when the server pushes a notification we
...
///
/// Consumers fan these out to ACP `Kimix/mcp/server_status` after 50 ms
...
pub struct McpClient {
    ...
    /// [`KimixClientHandler`]** constructed by
...
/// and the [`KimixClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to Kimix-mcp
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `Kimix/mcp/sdk_call`; the
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: kimix_log::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, KimixClientHandler>, McpError> {
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `Kimix/mcp/sdk_call`: the larger of the
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("kimix-shell-{server_name}"),
    ...
    /// Build the [`KimixClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> KimixClientHandler {
        KimixClientHandler {
    ...
    /// references (the [`KimixClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer = kimix_log::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = kimix_log::instrumentation::timer("mcp_list_tools");
        loop {
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("kimix-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = kimix_log::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = kimix_log::instrumentation::timer("mcp_stdio_spawn");
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    kimix_log::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            kimix_file_utils::events::Event::McpOAuthDiscoveryTimeout {
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = kimix_log::instrumentation::timer("mcp_start_servers");
...
/// `Kimix/mcp/server_status` events.
///
...
impl ClientHandler for KimixClientHandler {
    ...
    // See the [`KimixClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            kimix_file_utils::events::EventWriter::noop(),
        );
    ...
    #[test]
    fn ensure_figma_user_agent_sets_kimix_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use kimix_tool_runtime::LocalRegistry;
        use kimix_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = kimix_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = kimix_file_utils::events::EventWriter::open(tmp.path());
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = KimixClientHandler {
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            kimix_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
