...
//!
//! In-process SDK MCP servers (the official `Kimix-agent-sdk`'s `@tool` /
...
//! reverse `Kimix/mcp/sdk_call` request and feeding the response back. This module
...
/// channel (`Kimix/mcp/sdk_call`) and returns its JSON-RPC response. The `Err` string is
...
/// `Kimix/mcp/sdk_call` and writes the server→client response back. The pump exits when
...
async fn read_requests(
    ...
    loop {
        ...
        // `Kimix/mcp/sdk_call`s without a JSON-RPC id, so id-less messages (e.g. rmcp's
    ...
    async fn pump_forwards_the_configured_timeout_to_the_invoker() {
        ...
        impl AcpReverseInvoker for RecordingInvoker {
            async fn invoke(
                ...
                *self.0.lock().expect("lock poisoned") = Some(timeout);
                let id = message.get("id").cloned().unwrap_or(Value::Null);
                Ok(serde_json::json!({ "jsonrpc": "2.0", "id": id, "result": {} }))
            }
        }
        ...
        assert_eq!(read_line(&mut reader).await["id"], 1);
        assert_eq!(*seen.lock().expect("lock poisoned"), Some(configured));
    }
