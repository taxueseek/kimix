...
//!
//! Credentials are stored in `$KIMIX_SHARE_DIR/mcp_credentials.json`, keyed by a
...
/// File name for the credential store inside `$KIMIX_SHARE_DIR`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$KIMIX_SHARE_DIR/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$KIMIX_SHARE_DIR/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user kimix home (set $KIMIX_SHARE_DIR or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$KIMIX_SHARE_DIR/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(kimix_config::user_kimix_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$KIMIX_SHARE_DIR/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("kimix-mcp-credentials-test");
