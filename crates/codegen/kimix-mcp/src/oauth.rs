...
const MCP_OAUTH_CLIENT_NAME: &str = "Kimix";
...
// mode disabled, multiple `kimix` invocations).
//
// Layer 1 (cross-process): filesystem lock at $KIMIX_SHARE_DIR/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    kimix_config::kimix_home().join(format!("mcp_auth_{safe}.lock"))
}
