...
//! show.
use kimix_tui::app::PagerTerminal;
use kimix_tui::app::app_view::{ActiveView, AppView};
use kimix_tui::minimal_api;
use kimix_tui::render::Renderable;
use kimix_tui::scrollback::state::ScrollbackState;
use kimix_tui::scrollback::wrappers::EntryRenderer;
use kimix_tui::theme::Theme;
use kimix_tui::views::prompt_widget::PromptStyle;
use kimix_tui::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &kimix_tui::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub(super) fn prompt_style(appearance: &kimix_tui::appearance::AppearanceConfig) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    kimix_tui::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                kimix_tui::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(kimix_tui::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a kimix_tui::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &kimix_tui::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
) -> EntryRenderer<'a> {
...
fn minimal_watchers(agent: &kimix_tui::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            .session
            .bg_tasks
            .values()
            .filter(|t| t.is_monitor && t.status == kimix_tui::app::agent::BgTaskStatus::Running)
            .count(),
...
fn minimal_advance_phase_timer(
    agent: &mut kimix_tui::app::agent_view::AgentView,
) -> Option<kimix_tui::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &kimix_tui::app::agent_view::AgentView,
    activity: &Option<kimix_tui::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = kimix_tui::app::minimal_auto_set_for_mouse_leak();
    let switch_back = kimix_tui::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use kimix_tui::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = kimix_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<kimix_tui::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> kimix_tui::app::agent_view::AgentView {
        minimal_api::test_agent_view(Some("s1"), std::path::PathBuf::from("/tmp"))
    }
    #[test]
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use kimix_tui::app::agent::AgentState;
        use kimix_tui::scrollback::RenderBlock;
        use kimix_tui::scrollback::entry::ScrollbackEntry;
        use kimix_tui::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &kimix_tui::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use kimix_tui::acp::tracker::TurnActivity;
        use kimix_tui::app::agent::AgentState;
        let theme = Theme::current();
        ...
        kimix_tui::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        kimix_tui::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            kimix_tui::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(kimix_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &kimix_tui::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use kimix_tui::app::app_view::PendingAction;
        use kimix_tui::input::key::KeyShortcut;
