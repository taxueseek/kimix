...
use kimix_tui::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = kimix_tui::appearance::cache::load_show_thinking_blocks();
        ...
        kimix_tui::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: kimix_tui::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(kimix_tui::scrollback::block::RenderBlock::system(
                    "No conversation transcript to view yet",
                ));
        }
        return;
    }
    let path = std::env::temp_dir().join(format!("kimix-transcript-{}.ansi", uuid::Uuid::new_v4()));
    match std::fs::write(&path, out) {
        ...
        Err(e) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                agent
                    .scrollback
                    .push_block(kimix_tui::scrollback::block::RenderBlock::system(format!(
...
mod tests {
    use super::*;
    use kimix_tui::scrollback::block::RenderBlock;
