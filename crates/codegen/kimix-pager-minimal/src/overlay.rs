...
//! state is a no-op.
//!
//! [`Terminal::set_viewport_height`]: kimix_ratatui_inline::Terminal::set_viewport_height
use ratatui::buffer::Buffer;
use ratatui::layout::Rect;
use ratatui::style::Style;

use kimix_tui::app::PagerTerminal;
use kimix_tui::app::agent_view::AgentView;
use kimix_tui::app::app_view::{ActiveView, AppView};
use kimix_tui::appearance::LayoutConfig;
use kimix_tui::minimal_api;
use kimix_tui::render::SafeBuf as _;
use kimix_tui::theme::Theme;
use kimix_tui::views::prompt_widget::{PromptStyle, PromptWidget};
...
fn active(prompt: &PromptWidget, items_width: u16) -> Option<(Kind, u16)> {
    use kimix_tui::views::completion_dropdown::MAX_VISIBLE_ROWS;
    use kimix_tui::views::file_search::dropdown::MAX_DROPDOWN_ROWS as FILE_MAX;
    use kimix_tui::views::slash_dropdown::desired_item_rows;
...
pub fn render(
    ...
    match kind {
        Kind::FileSearch => {
            kimix_tui::views::file_search::dropdown::render_dropdown(
                buf,
                items_rect,
                &prompt.file_search,
                theme,
        ...
        Kind::Slash => {
            ...
            kimix_tui::views::slash_dropdown::render_dropdown(
                buf, items_rect, &snap, hovered, theme,
            );
        }
        Kind::Completion => {
            kimix_tui::views::completion_dropdown::render_dropdown(
                buf,
                items_rect,
...
pub fn modal_height(modal: Modal, agent: &mut AgentView, screen_h: u16, content_w: usize) -> u16 {
    match modal {
        Modal::Permission => agent
            ...
            .front()
            .map(|p| {
                kimix_tui::views::permission_view::permission_view_height(p, screen_h, content_w)
            })
            .unwrap_or(0),
        Modal::Question => {
            let input_mode = minimal_api::question_view(agent).is_some_and(|qv| {
                qv.focus == kimix_tui::views::question_view::QuestionFocus::InputMode
            });
            ...
            minimal_api::question_view_mut(agent)
                .map(|qv| {
                    kimix_tui::views::question_view::question_view_height(qv, screen_h, content_w)
                        .saturating_add(editor_extra)
                })
                .unwrap_or(0)
        }
        Modal::Rewind => minimal_api::rewind_state(agent)
            .map(|rw| kimix_tui::views::rewind::rewind_overlay_height(&rw.phase, screen_h))
            .unwrap_or(0),
        Modal::Cancel => {
            if minimal_api::cancel_turn_view(agent).is_some() {
                kimix_tui::views::modal::cancel_turn_panel_height(screen_h)
            } else {
                0
            }
        }
    ...
    match modal {
        ...
        Modal::Rewind => {
            if let Some(rw) = minimal_api::rewind_state(agent) {
                kimix_tui::views::rewind::render_rewind_overlay(buf, area, &rw.phase, true);
            }
            None
        }
        Modal::Cancel => {
            ...
            let drawn = if let Some(ctv) = minimal_api::cancel_turn_view(agent) {
                kimix_tui::views::modal::render_cancel_turn_panel(
                    buf,
                    area,
                    ctv,
                    true,
...
fn render_permission(
    ...
    let result = kimix_tui::views::permission_view::render_permission_view(
        buf,
        area,
        perm,
        &followup,
...
fn render_question(
    ...
    use kimix_tui::views::question_view::{QUESTION_VIEW_HPAD, QuestionFocus};
    ...
    if let Some(qv) = minimal_api::question_view_mut(agent) {
        let vis = qv.questions.get(qv.active_tab).map(|question| {
            kimix_tui::views::question_view::visible_options_height(
                question,
                q_area.height,
                content_w,
                qv.focused_preview(),
    ...
    if let Some(qv) = minimal_api::question_view(agent) {
        kimix_tui::views::question_view::render_question_view(
            buf,
            q_area,
            qv,
    ...
    if input_mode {
        ...
        let freeform_sel = minimal_api::question_view(agent)
            ...
            .copied()
            .unwrap_or(false);
        let embed = kimix_tui::views::modal_window::embedded_row_style(theme, true);
        ...
        let marker = if is_multi {
            ...
            format!("({})", kimix_tui::glyphs::filled_dot())
        } else {
            "(\u{25cb})".to_string()
        };
        ...
        buf.set_span_safe(
            area.x + 9,
            row_y,
            &ratatui::text::Span::styled(kimix_tui::glyphs::prompt_arrow(), accent),
            2,
        );
        ...
        let editor = Rect {
            x: area.x + 11,
            y: row_y,
            width: kimix_tui::views::question_view::inline_text_width(area.width),
            height: input_h,
        };
        return agent
            .prompt
...
fn question_editor_h(agent: &AgentView, area_w: u16, cap: u16, theme: &Theme) -> u16 {
    let text_w = kimix_tui::views::question_view::inline_text_width(area_w);
    agent
        .prompt
...
mod tests {
    use super::*;
    use kimix_tui::views::suggestion_controller::CompletionItemParsed;
    ...
    fn question_input_agent(text: &str) -> AgentView {
        use kimix_tui::views::prompt_widget::StashedPrompt;
        use kimix_tui::views::question_view::{Question, QuestionOption, QuestionViewState};
