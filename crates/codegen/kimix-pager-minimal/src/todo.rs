...
use kimix_shell::tools::TodoStatus;
...
pub(super) fn todo_panel_height(agent: &kimix_tui::app::agent_view::AgentView, force: bool) -> u16 {
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (kimix_tui::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    kimix_tui::glyphs::ballot_x(),
...
mod tests {
    use super::*;
    use kimix_shell::tools::{TodoItem, TodoPriority};
