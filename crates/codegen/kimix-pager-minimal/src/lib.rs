//! Minimal (scrollback-native) render mode — `kimix --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `kimix_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `Kimix-tui` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`kimix_tui::minimal_hook`]) of function pointers, and the
//! composition-root binary (`Kimix-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `Kimix-tui`.
///
...
/// subsequent calls are ignored (see [`kimix_tui::minimal_hook`]).
pub fn install() {
    kimix_tui::minimal_hook::install(kimix_tui::minimal_hook::MinimalHooks { draw });
}
