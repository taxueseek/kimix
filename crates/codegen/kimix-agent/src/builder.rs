...
use kimix_tools::bridge::ToolBridge;
use kimix_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use kimix_tools::notification::ToolNotificationHandle;
use kimix_tools::registry::types::SessionContext;
use kimix_tools::types::tool::ToolKind;
use std::collections::HashMap;
use std::path::PathBuf;
use std::sync::Arc;
/// The Kimix [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-Kimix
/// tool registry in `Kimix-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    kimix_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.kimix/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<kimix_tools::implementations::kimix::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn kimix_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: kimix_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: kimix_tools::implementations::kimix::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn kimix_tools::implementations::lsp::LspBackend>>,
    ...
    /// dirs are scanned for skills / rules / AGENTS.md. Defaults to all-off
    /// (terminal-minimal); enable via `[compat]` or env.
    ...
    api_key_provider: Option<kimix_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<kimix_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in Kimix-shell).
    ...
    preloaded_skills: Option<Vec<kimix_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
///
/// IDs are derived from the tool types (currently `Kimix:…`) so a namespace
/// rename cannot reintroduce a double-inject that fails session create with
/// `duplicate_client_name`.
fn ensure_plan_mode_tools(tool_config: &mut kimix_tools::registry::types::ToolServerConfig) {
    use kimix_tools::implementations::kimix;
    use kimix_tools::registry::types::ToolConfig;
    let candidates = [
        ToolConfig::from(&kimix::EnterPlanModeTool),
        ToolConfig::from(&kimix::ExitPlanModeTool),
        ToolConfig::from(&kimix::AskUserQuestionTool),
    ];
...
/// Low-use tools stripped from the terminal-default (compact) tool config.
fn is_low_use_tool_id(id: &str) -> bool {
...
/// Tools that require the full registry (variant namespaces). When any of
/// these appear in `tool_config`, compact registration is skipped so
/// curated presets (codex / concise / hashline) keep working.
fn needs_full_tool_registry(id: &str) -> bool {
    if id.starts_with("Codex:")
        || id.starts_with("KimixConcise:")
        || id.starts_with("KimixHashline:")
        || id.starts_with("KimixConcise:")
        || id.starts_with("KimixHashline:")
    {
        return true;
    }
    // OpenCode write is registered in compact; other OpenCode tools are not.
    id.starts_with("OpenCode:") && !id.ends_with(":write")
}

/// Drop low-use tools so the compact registry validate_config path stays green.
fn strip_low_use_tools(tool_config: &mut kimix_tools::registry::types::ToolServerConfig) {
    tool_config.tools.retain(|tc| !is_low_use_tool_id(&tc.id));
}
...
fn merge_tool_params(
    tool_config: &mut kimix_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(kimix_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<kimix_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    /// Enable compact tool registration (L1+L2 only, skip duplicate variants).
    /// Reduces initial context by ~40% (45 → ~28 tools).
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn kimix_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: kimix_tools::implementations::kimix::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: kimix_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `KIMIX_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: kimix_tools::implementations::kimix::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn kimix_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: kimix_tools::implementations::kimix::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: kimix_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the Kimix Task description.
    ...
    ///
    /// When disabled, `Kimix:ask_user_question` is stripped from the
    ...
    pub fn with_compat_config(mut self, compat: kimix_tools::types::compat::CompatConfig) -> Self {
        self.compat = compat;
        self
    }
    /// Set the skills config (custom paths, ignore globs) from config.toml.
    /// Without this, only auto-discovered skills (cwd/.Kimix/skills, ~/.kimix/skills)
    /// are included — custom paths added via `Kimix/skills/add` would be ignored.
    ...
    fn resolve_definition(&self) -> AgentDefinition {
        ...
        let mut def = AgentDefinition::default_kimix();
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        // Skill router: write a compact keyword index to disk and keep it out
        // of the prompt. Production sessions use router-only mode (no bulk
        // skill listing system-reminder). The agent loads matching SKILL.md
        // via read_file when keywords match.
        if !skill_info.is_empty() {
            let router = crate::prompt::skills::build_skill_router_prompt(
                &skill_info,
                &preloaded_skill_paths,
            );
            if !router.is_empty() {
                let router_path =
                    kimix_tools::util::kimix_home::kimix_home().join("SKILL-ROUTER.md");
                let _ = std::fs::write(&router_path, &router);
            }
            // Lightweight reference only — do not inject the full skill table.
            definition.prompt_body = Some(
                definition.prompt_body.unwrap_or_default()
                    + "
## Skills (on-demand)
Do not expect a full skill catalog in context. Use `read_file` on the skill router \
(`~/.kimix/SKILL-ROUTER.md` or `~/.kimix/SKILL-ROUTER.md`) to find matching skills by keyword, \
then `read_file` the listed SKILL.md path only when needed.
",
            );
        }
        ...
        if definition.inject_default_tools {
            if self.memory_backend.is_some() {
                use kimix_tools::implementations::memory;
                tool_config
                    .tools
            ...
            if self.web_search_config.is_enabled() {
                use kimix_tools::implementations::kimix;
                tool_config.tools.push((&kimix::WebSearchTool).into());
            }
            // Lightweight search discipline for any path that wants web search
            // (client HTTP and/or hosted). Tool description carries the full
            // rules; this keeps a short reminder in the system prompt.
            if self.web_search_config.wants_search() {
                definition.prompt_body = Some(
                    definition.prompt_body.unwrap_or_default()
                        + "\n## Web search discipline\n\
When using web_search: prefer high Evidence credibility; do not treat SERP/jump \
URLs or social posts as sole fact sources; for high-stakes claims search then \
web_fetch top sources before concluding; never merge conflicting numbers without \
labeling 口径.\n",
                );
            }
            if self.web_fetch_config.is_enabled() {
                use kimix_tools::implementations::kimix;
                tool_config.tools.push((&kimix::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&kimix_tools::implementations::kimix::LspTool).into());
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&kimix_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        // Compact when requested AND the selected toolset does not need
        // codex/concise/hashline/opencode variants. Curated presets fall back
        // to the full registry automatically.
        let use_compact = self.compact_mode
            && !tool_config
                .tools
                .iter()
                .any(|tc| needs_full_tool_registry(&tc.id));
        if use_compact {
            strip_low_use_tools(&mut tool_config);
        }
        let tool_bridge_builder = if use_compact {
            ToolBridge::get_builder_compact()
        } else {
            ToolBridge::get_builder()
        };
        if self.memory_backend.is_none() {
            let kimix_ns = kimix_tools::types::tool::ToolNamespace::Kimix.to_string();
            let mem_search_id = format!(
                "{kimix_ns}:{}",
                kimix_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{kimix_ns}:{}",
                kimix_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if task_stripped {
            use kimix_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::Kimix, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::KimixConcise, "run_terminal_cmd", true)
        ...
        {
            let web_fetch_id = format!(
                "{}:web_fetch",
                kimix_tools::types::tool::ToolNamespace::Kimix
            );
            merge_tool_params(&mut tool_config, &[web_fetch_id.as_str()], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            ...
            let bash_concise_id = format!(
                "{}:run_terminal_cmd",
                kimix_tools::types::tool::ToolNamespace::KimixConcise
            );
            merge_tool_params(
                &mut tool_config,
                &[bash_id.as_str(), bash_concise_id.as_str()],
                bash_params,
            );
        }
        if let Some(ref ask_params) = self.ask_user_question_params_json {
            ...
            merge_tool_params(&mut tool_config, &[ask_id.as_str()], ask_params);
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full kimix toolset"
                );
            }
        }
        tool_config
        ...
        // Hosted WebSearch needs only user desire (`wants_search`) so
        // API-key sessions without OAuth still get backend search when
        // the model supports it. Client function tool registration still
        // uses `is_enabled()` earlier in this build path.
        let web_search_desired = self.web_search_config.wants_search();
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                kimix_tools::types::resources::TruncationCfg(
                    kimix_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_desired && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(kimix_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(kimix_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI naming for the shared [`kimix_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: kimix_tool_types::TaskToolNaming<'static> =
    kimix_tool_types::TaskToolNaming {
...
/// CLI [`kimix_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: kimix_tool_types::SubagentToolNaming<'static> =
    kimix_tool_types::SubagentToolNaming {
...
/// shared [`kimix_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => kimix_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => kimix_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => kimix_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`kimix_tool_types::SubagentDescriptor`] and defers to
/// [`kimix_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<kimix_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            kimix_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = kimix_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        kimix_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: kimix_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(kimix_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(kimix_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: kimix_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("kimix", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use kimix_tools::types::template_renderer::TemplateRenderer;
        use kimix_tools::types::tool::ToolKind;
    ...
    async fn build_pager_agent(
        ...
        use kimix_tools::computer::local::LocalTerminalBackend;
        use kimix_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "kimix / subagents+ask_user",
                profile: AgentDefinition::default_kimix,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "kimix / subagents / no-ask-user",
                profile: AgentDefinition::default_kimix,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "kimix / no-subagents / ask_user",
                profile: AgentDefinition::default_kimix,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "kimix / no-subagents / no-ask-user",
                profile: AgentDefinition::default_kimix,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "kimix-ask-user / subagents",
                profile: AgentDefinition::kimix_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "kimix-ask-user / no-subagents",
                profile: AgentDefinition::kimix_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "kimix-plan",
                profile: AgentDefinition::kimix_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "kimix-plan / no-ask-user",
                profile: AgentDefinition::kimix_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "kimix-plan-no-subagents",
                profile: AgentDefinition::kimix_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "kimix-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::kimix_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use kimix_tools::computer::local::LocalTerminalBackend;
        use kimix_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_kimix();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use kimix_tools::computer::local::LocalTerminalBackend;
        use kimix_tools::implementations::kimix::ask_user_question::AskUserQuestionParams;
        use kimix_tools::notification::ToolNotificationHandle;
        use kimix_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_kimix();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id.ends_with(":ask_user_question")),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use kimix_tools::computer::local::LocalTerminalBackend;
        use kimix_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_kimix();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("Kimix:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use kimix_tools::computer::local::LocalTerminalBackend;
        use kimix_tools::implementations::kimix::bash::BashParams;
        use kimix_tools::notification::ToolNotificationHandle;
        use kimix_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_kimix();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Kimix
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_kimix_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their Kimix
    ...
    /// treat `mcp__*` as always-on, so Kimix keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        ...
        use kimix_tools::implementations::web_search::WebSearchConfig;
        use kimix_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_kimix();
    ...
    /// Kimix toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to Kimix's `search_tool` (MCP
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, kimix_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, kimix_sampling_types::HostedTool::XSearch)),
    ...
    /// Kill-switch (`Disabled`): no hosted WebSearch. XSearch still attaches.
    #[tokio::test]
    ...
    /// HostedOnly (no OAuth client HTTP): hosted WebSearch still attaches when
    /// backend search is on — availability is not gated solely on OAuth.
    #[tokio::test]
    async fn hosted_web_search_when_hosted_only_config() {
        use kimix_tools::computer::local::LocalTerminalBackend;
        use kimix_tools::implementations::web_search::WebSearchConfig;
        use kimix_tools::notification::ToolNotificationHandle;
        let def = crate::config::AgentDefinition::default_kimix();
        let agent = AgentBuilder::new(
            std::env::temp_dir(),
            Arc::new(LocalTerminalBackend::new()),
            ToolNotificationHandle::noop(),
        ...
        .from_definition(def)
        .with_web_search_config(WebSearchConfig::HostedOnly)
        .with_backend_search(true)
        .build()
        .await
        .expect("agent should build with HostedOnly web search");
        let hosted = agent.hosted_tools();
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, kimix_sampling_types::HostedTool::WebSearch { .. })),
            "HostedOnly + backend_search must attach WebSearch, got: {hosted:?}"
        );
        let has_fn = agent
            .tool_definitions()
            .await
        ...
        assert!(
            !has_fn,
            "HostedOnly must not register the local function web_search tool"
        );
    }
