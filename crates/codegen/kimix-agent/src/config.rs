//! Agent definition types — parsed from `.Kimix/agents/*.md` files.
use crate::error::AgentBuildError;
use crate::prompt::context::TemplateOverride;
use crate::prompt::user_message::UserMessageTemplate;
use kimix_tools::implementations::codex;
use kimix_tools::implementations::kimix;
use kimix_tools::implementations::kimix_concise;
use kimix_tools::implementations::memory;
use kimix_tools::implementations::opencode;
use kimix_tools::implementations::search_tool;
use kimix_tools::implementations::use_tool;
use kimix_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard Kimix
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&kimix::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&kimix::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&kimix::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&kimix::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&kimix::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_kimix_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_kimix_toolset() -> ToolServerConfig {
    let mut tools = default_kimix_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&kimix::EnterPlanModeTool).into());
    tools.push((&kimix::ExitPlanModeTool).into());
    tools.push((&kimix::AskUserQuestionTool).into());
    tools.push((&kimix::WebSearchTool).into());
    tools.push((&kimix::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&kimix::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `Kimix-computer` (workspace/sandbox) preset.
fn kimix_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&kimix::ReadFileTool).into(),
        (&kimix::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&kimix::ListDirTool).into(),
        (&kimix::GrepTool).into(),
        (&kimix::KillTerminalCommandTool).into(),
        (&kimix::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("kimix", workspace_kimix_toolset()),
        ("kimix-concise", kimix_concise_toolset()),
        ("kimix-plan", kimix_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("kimix-computer", kimix_computer_toolset()),
    ]
}
...
/// Terminal-default toolset: L1 coding + task lifecycle + MCP bridge tools.
/// Scheduler / monitor / update_goal are omitted (enable via full profile or
/// `compact_mode = false` + explicit tool list).
fn default_kimix_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&kimix::ReadFileTool).into(),
...
fn kimix_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&kimix_concise::BashConciseTool).into(),
            (&kimix_concise::ReadFileConciseTool).into(),
            (&kimix_concise::SearchReplaceConciseTool).into(),
            (&kimix::ListDirTool).into(),
            (&kimix::GrepTool).into(),
            kill_task_tool_config(),
            (&kimix::TodoWriteTool).into(),
...
pub fn kimix_hashline_toolset(
    hashline_tools: Vec<kimix_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<kimix_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
/// Kimix + plan mode toolset.
///
/// Extends the default `kimix` toolset with plan mode tools:
...
fn kimix_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&kimix::ReadFileTool).into(),
            ...
            (&search_tool::SearchTool).into(),
            (&use_tool::UseTool).into(),
            (&kimix::EnterPlanModeTool).into(),
            (&kimix::ExitPlanModeTool).into(),
            (&kimix::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
/// Kimix + plan mode toolset WITHOUT subagent tools.
///
/// Same as `kimix_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn kimix_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&kimix::ReadFileTool).into(),
...
/// Default Kimix toolset + `ask_user_question`.
///
/// Same as `default_kimix_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn kimix_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&kimix::ReadFileTool).into(),
...
/// - `model: Kimix-3-fast` → `Override("Kimix-3-fast")`
...
/// Map a legacy pre-rebrand agent-type name onto its current `Kimix*`
/// equivalent: `<prefix>` → `kimix`, `<prefix>-plan` → `Kimix-plan`, and so on.
...
pub fn canonical_agent_type(name: &str) -> std::borrow::Cow<'_, str> {
    match name.strip_prefix(LEGACY_AGENT_TYPE_PREFIX) {
        Some("") => std::borrow::Cow::Borrowed("kimix"),
        Some(rest) if rest.starts_with('-') => std::borrow::Cow::Owned(format!("kimix{rest}")),
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::Kimix => AgentDefinition::default_kimix(),
            Self::KimixConcise => AgentDefinition::kimix_concise(),
            Self::KimixPlan => AgentDefinition::kimix_plan(),
            Self::KimixPlanNoSubagents => AgentDefinition::kimix_plan_no_subagents(),
            Self::KimixAskUser => AgentDefinition::kimix_ask_user(),
            ...
            Self::KimixOrchestrator => AgentDefinition::kimix_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .Kimix/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_kimix_toolset")]
    ...
    pub capability_mode: Option<kimix_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .Kimix/agents/ (project-level, highest priority)
    Project,
    /// ~/.kimix/agents/ (user-level)
    User,
    /// ~/.kimix/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_Kimix(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.kimix/agent-memory/<name>/`
    User,
    /// `<project>/.Kimix/agent-memory/<name>/`
    Project,
    /// `<project>/.Kimix/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: kimix_config::kimix_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".kimix/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".kimix/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `kimix_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let kimix = kimix_config::user_kimix_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), kimix.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".kimix/agents/")
            || path_str.contains(".kimix/agents/")
            || path_str.contains(".kimix\\agents\\")
            || path_str.contains(".kimix\\agents\\")
        {
            return AgentScope::Project;
        }
        if path_str.contains(".kimix/bundled/agents/")
            || path_str.contains(".kimix/bundled/agents/")
            || path_str.contains(".kimix\\bundled\\agents\\")
            || path_str.contains(".kimix\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `Kimix*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<kimix_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["Kimix:read_file", "KimixHashline:hashline_read"],
            ["Kimix:search_replace", "KimixHashline:hashline_edit"],
            ["Kimix:grep", "KimixHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_kimix_toolset(),
    ...
    pub fn default_kimix() -> Self {
        Self::base(
            BuiltinAgentName::Kimix,
            "Kimix agent for software engineering tasks.",
        )
    }
    /// Kimix Concise agent definition — concise output format for SFT/RL.
    pub fn kimix_concise() -> Self {
        Self {
            tool_config: kimix_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::KimixConcise,
                "Kimix agent with concise output format.",
            )
        }
    }
    /// Kimix agent with plan mode tools.
    pub fn kimix_plan() -> Self {
        Self {
            tool_config: kimix_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::KimixPlan,
                "Kimix agent with plan mode support.",
            )
        }
    }
    /// Kimix + plan mode WITHOUT subagent tools.
    pub fn kimix_plan_no_subagents() -> Self {
        Self {
            tool_config: kimix_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::KimixPlanNoSubagents,
                "Kimix agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Kimix agent with the `ask_user_question` tool.
    pub fn kimix_ask_user() -> Self {
        Self {
            tool_config: kimix_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::KimixAskUser,
                "Kimix agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: kimix_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: kimix_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: kimix_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// Kimix Orchestrator — GBL model with full Kimix tools
    ...
    pub fn kimix_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::KimixOrchestrator,
                "Kimix orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_kimix_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("kimix").unwrap();
    ...
    fn kimix_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&kimix::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&kimix::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn kimix_computer_preset_is_curated_kimix_subset() {
        let gc = toolset_for_preset("kimix-computer").unwrap();
        let gb = toolset_for_preset("kimix").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = kimix_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "kimix-computer tool `{}` must also ship in the kimix preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "kimix-computer should be a curated subset of kimix"
        );
    }
    #[test]
    fn kimix_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("kimix-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&kimix::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        ...
        for t in &gc.tools {
            if t.id == ToolConfig::from(&kimix::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&kimix::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The Kimix-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the Kimix
    ...
    #[test]
    fn kimix_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("kimix-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "kimix-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn kimix_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("kimix-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&kimix::LspTool).id,
            ToolConfig::from(&kimix::EnterPlanModeTool).id,
            ToolConfig::from(&kimix::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "kimix-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_kimix_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_kimix_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::KimixOrchestrator => true,
            BuiltinAgentName::Kimix
            | BuiltinAgentName::KimixConcise
            | BuiltinAgentName::KimixPlan
            | BuiltinAgentName::KimixPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "kimix-orchestrator", "kimix-orchestrator"] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("kimix-3-fast".to_string()).to_string(),
            "kimix-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("kimix-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.kimix/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.kimix/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_kimix_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have kimix tools"
        );
    }
    #[test]
    ...
    fn test_scope_from_path_detects_bundled_agents() {
        ...
        for home_name in [".kimix", ".kimix"] {
            ...
            assert_eq!(
                def.scope,
                AgentScope::Bundled,
                "path under {home_name}/bundled/agents should be Bundled"
            );
        }
    ...
    fn test_from_json_has_default_toolset_with_task_tool() {
        let json = serde_json::json!(
            { "name" : "kimix", "description" : "Multi-surface coding agent.",
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"kimix-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("kimix-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("kimix-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"kimix-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        ...
        assert_eq!(
            def.model,
            ModelOverride::Override("kimix-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "kimix-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("kimix-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("kimix", BuiltinAgentName::Kimix),
            ("kimix", BuiltinAgentName::Kimix), // legacy alias
            ("kimix-concise", BuiltinAgentName::KimixConcise),
            ("kimix-ask-user", BuiltinAgentName::KimixAskUser),
            ("kimix-orchestrator", BuiltinAgentName::KimixOrchestrator),
            ("kimix-orchestrator", BuiltinAgentName::KimixOrchestrator),
    ...
    /// must map onto the current `Kimix*` names; everything else passes
    /// through unchanged.
    #[test]
    fn test_canonical_agent_type_maps_legacy_names() {
        ...
        for (suffix, expected) in [
            ("", "kimix"),
            ("-concise", "kimix-concise"),
            ("-plan", "kimix-plan"),
            ("-plan-no-subagents", "kimix-plan-no-subagents"),
            ("-ask-user", "kimix-ask-user"),
            ("-orchestrator", "kimix-orchestrator"),
        ] {
            let name = legacy(suffix);
            assert_eq!(canonical_agent_type(&name), expected, "for {name}");
            // Every mapped name resolves to a real builtin (kimix* aliases accepted).
            assert!(
        ...
        assert_eq!(canonical_agent_type("kimix-plan"), "kimix-plan");
        assert_eq!(canonical_agent_type("kimix-plan"), "kimix-plan");
    ...
    fn test_by_name_resolves_legacy_agent_type() {
        ...
        // definition().name uses the canonical as_ref (first serialize string).
        assert!(
            def.name == "kimix-plan" || def.name == "kimix-plan",
            "unexpected plan agent name: {}",
            def.name
        );
        assert!(
            is_strict_harness_agent_type(&format!("{LEGACY_AGENT_TYPE_PREFIX}-concise"))
                == is_strict_harness_agent_type("kimix-concise")
        );
    }
    #[test]
