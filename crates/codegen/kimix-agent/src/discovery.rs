...
//!
//! Searches `.Kimix/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.kimix/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.Kimix/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".kimix/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.Kimix/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.Kimix/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), kimix.as_deref())
}
...
fn all_subagents_with_home(
    ...
    home: Option<&Path>,
    kimix_home: Option<&Path>,
) -> Vec<SubagentEntry> {
    let discovered = discover_with_home(cwd, home, kimix_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.kimix/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.Kimix/agents/` walking from `cwd` up to repo root
/// 2. `~/.kimix/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.kimix/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user Kimix agents, `.claude`
/// compat agents, then bundled. `.Kimix` dirs resolve from `kimix_home`
/// (KIMIX_SHARE_DIR-aware) plus the legacy literal `~/.kimix` when KIMIX_SHARE_DIR points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    kimix_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.kimix, included only when it differs from kimix_home
    // (i.e. KIMIX_SHARE_DIR points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_kimix = home
        .map(|h| h.join(".kimix"))
        .filter(|legacy| kimix_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = kimix_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_kimix {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let kimix = kimix_config::user_kimix_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), kimix.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, kimix_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let kimix = kimix_config::user_kimix_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), kimix.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, kimix_home);
...
///
/// Project-level `.Kimix/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let kimix = kimix_config::user_kimix_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), kimix.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, kimix_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, kimix_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Kimix
...
/// Load project agent definitions from every `.Kimix/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectKimix,
            PluginScope::User => PluginOrigin::UserKimix,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_kimix_when_kimix_home_differs() {
        let home = Path::new("/home/u");
        let kimix = Path::new("/custom/kimixhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(kimix))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&kimix.join("agents")));
        assert!(paths.contains(&home.join(".kimix").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&kimix.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".kimix").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_kimix_home_is_dot_kimix() {
        let home = Path::new("/home/u");
        let kimix = home.join(".kimix");
        let count = user_agent_dirs(Some(home), Some(&kimix))
            .into_iter()
            .filter(|(p, _)| *p == kimix.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.kimix/agents when kimix_home == ~/.kimix"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_kimix() {
        let def = by_name("kimix");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "kimix");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".kimix").join("agents");
        let agents_dir_2 = inner_dir.join(".kimix").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".kimix").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".kimix")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".kimix").join("agents");
        let bundled_dir = home.join(".kimix").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".kimix")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".kimix").join("agents");
        let bundled_dir = home.join(".kimix").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "Kimix" that shadows the built-in
        write_agent_file(&agents_dir, "kimix.md", "kimix", "Custom kimix");

        let def = by_name_in_cwd("kimix", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "kimix");
        assert_eq!(def.description, "Custom kimix");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .Kimix/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("kimix-orchestrator")
            .expect("from_str must resolve kimix-orchestrator");
        assert_eq!(variant, BuiltinAgentName::KimixOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "kimix-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("kimix-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find kimix-orchestrator");
        assert_eq!(def.name, "kimix-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.kimix/agents/) agent named "explore" should NOT shadow
