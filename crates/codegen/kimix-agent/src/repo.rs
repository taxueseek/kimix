...
//! primitive consumed cross-crate by `Kimix-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.Kimix/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.Kimix`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `Kimix-workspace`, which depends on THIS crate) to keep the dep edge
