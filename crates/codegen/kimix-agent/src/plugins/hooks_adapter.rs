...
//! `Kimix-hooks` runtime.  It pre-filters unsupported events from plugin
...
//! `Kimix-hooks` crate's parser and runtime.
use std::collections::HashMap;
use std::path::{Path, PathBuf};

use kimix_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        if let Some(cmd) = &spec.command {
            ...
            // `Kimix-hooks::runner::command` for env vars whose
            // values are already known at load time.
            let substituted = substitute_env_vars(&cmd_str, plugin_root, plugin_data);
            let expanded = kimix_config::expand_env_vars_in_string(&substituted);
            if expanded != cmd_str {
...
mod tests {
    ...
    fn parse_plugin_hooks_from_file() {
        ...
        assert_eq!(
            specs[0].extra_env.get("KIMIX_PLUGIN_ROOT").unwrap(),
            "/path/to/plugin"
        );
        assert_eq!(
        ...
        assert_eq!(
            specs[0].extra_env.get("KIMIX_PLUGIN_DATA").unwrap(),
            "/path/to/data"
        );
    ...
    /// `${CLAUDE_PLUGIN_ROOT}` (or its `KIMIX_PLUGIN_ROOT` alias) must be
    ...
    fn parse_plugin_hooks_substitutes_plugin_root_in_command() {
            ...
            "hooks": {
                "PreToolUse": [
                    {"hooks": [
                        ...
                        {"type": "command", "command": "${KIMIX_PLUGIN_ROOT}/hooks/alias.sh"},
        ...
        assert!(
            raws.contains(&"${KIMIX_PLUGIN_ROOT}/hooks/alias.sh"),
            "command_raw must preserve the source string verbatim, got {raws:?}"
        );
        assert!(
    ...
    /// `Kimix-hooks::runner::command`, which can fail for hooks
    ...
    /// (CLAUDE_PLUGIN_ROOT, KIMIX_PLUGIN_ROOT, CLAUDE_PLUGIN_DATA,
    /// KIMIX_PLUGIN_DATA). This preserves the plugin contract while still
    ...
    fn parse_plugin_hooks_user_env_merged_with_plugin_precedence() {
                    ...
                    {"hooks": [
                        {
                            ...
                            "env": {
                                ...
                                "KIMIX_PLUGIN_ROOT": "/user/wins?",
                                "CLAUDE_PLUGIN_DATA": "/user/wins?",
                                "KIMIX_PLUGIN_DATA": "/user/wins?"
                            }
                        }
                    ]}
                ]
        ...
        // overridden. CLAUDE_PLUGIN_ROOT and KIMIX_PLUGIN_ROOT both map
        // to plugin_root; CLAUDE_PLUGIN_DATA and KIMIX_PLUGIN_DATA both
        // map to plugin_data.
        for (key, expected) in [
            ("CLAUDE_PLUGIN_ROOT", "/actual/plugin/root"),
            ("KIMIX_PLUGIN_ROOT", "/actual/plugin/root"),
            ("CLAUDE_PLUGIN_DATA", "/actual/plugin/data"),
            ("KIMIX_PLUGIN_DATA", "/actual/plugin/data"),
        ] {
            assert_eq!(
