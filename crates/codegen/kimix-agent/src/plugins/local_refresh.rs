...
fn promote_tmp_to_dest(tmp: &Path, dest: &Path) -> std::io::Result<()> {
    #[cfg(test)]
    {
        if std::env::var_os("KIMIX_TEST_FAIL_REFRESH_PROMOTE").is_some() {
...
mod tests {
    ...
    fn refresh_local_install_picks_up_new_agent() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".kimix").join("installed-plugins"));
        ...
        let trust = TrustStore::load_from(home.join(".kimix").join("trusted-plugins"));
    ...
    fn refresh_promote_failure_rolls_back_to_prior_snapshot() {
        ...
        let trust = TrustStore::load_from(home.join(".kimix").join("trusted-plugins"));
        let summary = {
            let _fail = EnvVarGuard::set("KIMIX_TEST_FAIL_REFRESH_PROMOTE", "1");
    ...
    fn refresh_preserves_install_subdir_scope() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".kimix/installed-plugins"));
    ...
    fn refresh_keeps_stale_when_legacy_subdir_scope_lost() {
        ...
        let install_dir = home.join(".kimix").join("installed-plugins");
