...
impl LoadedPlugin {
    /// Data directory for this plugin: `~/.kimix/plugin-data/<plugin_id>/`.
...
impl SharedPluginRegistryHandle {
    ...
    pub fn snapshot(&self) -> Option<std::sync::Arc<PluginRegistry>> {
        self.inner.read().expect("rwlock read poisoned").clone()
    }
    ...
    pub fn reload(
        ...
        let registry =
            PluginRegistry::from_discovered(discovered, &config.disabled, &config.enabled);
        *self.inner.write().expect("rwlock write poisoned") = if registry.is_empty() {
            None
        } else {
...
mod tests {
    ...
    fn data_dir_uses_plugin_id() {
        ...
        // Should be under ~/.kimix/plugin-data/<plugin_id>/
