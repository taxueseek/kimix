...
///
/// Replaces `${KIMIX_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${KIMIX_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`kimix_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    kimix_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".kimix-plugin")).unwrap();

        // Write manifest in .Kimix-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".kimix-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${KIMIX_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${KIMIX_PLUGIN_DATA}/cache";
