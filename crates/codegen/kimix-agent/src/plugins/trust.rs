...
//!
//! Plugins from project directories (`.Kimix/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.kimix/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.kimix/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.kimix/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_kimix_home() so a project's `.Kimix/trusted-plugins` is never
        // read as the user trust store when neither KIMIX_SHARE_DIR nor a home dir resolves.
        let Some(kimix) = kimix_config::user_kimix_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = kimix.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.kimix/trusted-plugins`.
    ...
    /// rewrites `~/.kimix/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.kimix/trusted-plugins`.
