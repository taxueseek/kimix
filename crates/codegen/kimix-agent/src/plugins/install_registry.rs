...
/// Default install directory name under `~/.kimix/`.
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("KIMIX_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
    ...
    /// 2. Default: `~/.kimix/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        kimix_config::kimix_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
