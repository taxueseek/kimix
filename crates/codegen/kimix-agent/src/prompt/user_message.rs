...
//! - `Default` -- the legacy Kimix prefix (built by the shell layer).
...
use kimix_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Kimix prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .Kimix/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `kimix_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.kimix/, ~/.claude/).
    ...
    /// (`~/.kimix/projects/<encoded-cwd>/mcps`). Surfaced in
