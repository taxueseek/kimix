...
//!
//! Searches from cwd to repo root, plus `~/.kimix/`. Also discovers
//! `*.md` files in `.Kimix/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.Kimix/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.kimix/, git repo root, and session cwd.
...
/// rules / project-instruction files; pass `CompatConfig::all_on()` to
...
async fn read_agents_config_with_options(
    ...
    let global_dir = kimix_tools::util::kimix_home::kimix_home();
    ...
    // Always include kimix_home (~/.kimix/) first, then ~/.claude/ and ~/.cursor/
...
/// Maximum content length (chars) per AGENTS.md file before truncation.
/// ~4000 chars ≈ 1000 tokens. Larger files are truncated with a marker
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .Kimix/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.kimix/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            kimix_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
...
mod tests {
    ...
    #[test]
    fn format_agents_md_section_truncates_long_content() {
        // Content exceeding MAX_AGENTS_MD_CONTENT_CHARS is truncated with a marker.
        ...
        let section = format_agents_md_section(&configs).unwrap();
        // Truncated content should NOT contain the full 5000 chars.
        assert!(
        ...
        assert!(
            section.contains("[Content truncated"),
            "truncation marker should be present"
        );
        // Should contain the file path so agent can read the full file.
        assert!(
            section.contains("/repo/AGENTS.md"),
            "file path should be preserved"
        );
    }

    #[test]
    fn format_agents_md_section_preserves_short_content() {
        // Content under the limit is preserved verbatim.
        let short_content = "Short instructions here.";
        let configs = vec![AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
        ...
        let section = format_agents_md_section(&configs).unwrap();
        assert!(
            section.contains(short_content),
            "short content must be preserved verbatim"
        );
        assert!(
