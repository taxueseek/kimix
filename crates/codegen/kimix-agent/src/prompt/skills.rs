...
//! live in `kimix_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use kimix_tools::implementations::skills::types::skill_name_from_path;
pub use kimix_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `kimix_tools` directly.
pub use kimix_tools::types::compat::CompatConfig;

use kimix_tools::implementations::skills::discovery::{
...
///
/// Priority order: Local (cwd/.kimix/skills, cwd/.agents/skills, cwd/.claude/skills) → Intermediate dirs →
/// Repo (repo_root/.kimix/skills, repo_root/.agents/skills, repo_root/.claude/skills) → User (~/.kimix/skills, ~/.agents/skills, ~/.claude/skills)
/// → additional paths from `config.paths`
/// → Server (injected `config.server_skill_dirs`)
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.kimix/bundled`; lowest precedence).
///
...
/// `CompatConfig::all_on()` to scan every vendor surface.
...
/// (e.g. `Repo` for `.kimix/plugins/`). Native skills always win bare-name
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &kimix_tools::util::kimix_home::kimix_home(),
        compat,
    )
    .await;
...
pub fn collect_skill_config_dirs(
    ...
    let kimix_home = global_dir.to_path_buf();
    ...
    // config; `.kimix` and `.agents` are always present. When all cells are on
    // this list equals the historical `[".kimix", ".agents", ".claude", ".cursor"]`.
    ...
    // Priority 3: Global user dirs. `.kimix` comes from `kimix_home` (which may
...
fn scope_for_config_dir(dir: &Path, cwd: Option<&Path>, git_root: Option<&Path>) -> SkillScope {
    // Home-level dirs (e.g. ~/.kimix/, ~/.agents/, ~/.claude/) are User scope.
    #[allow(deprecated)]
    if let Some(home) = std::env::home_dir()
...
/// only visits known config roots (`.kimix`, `.agents`, `.claude`, `.cursor`),
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            kimix_tools::types::config_source::ConfigSource::ConfigToml {
...
fn stamp_plugin_fields(skills: &mut [SkillInfo], plugin: &crate::plugins::LoadedPlugin) {
    ...
    for skill in skills.iter_mut() {
        ...
        skill.config_source = Some(kimix_tools::types::config_source::ConfigSource::Plugin {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill
        ...
        .as_ref()
        .filter(|b| !b.is_empty())
        .map(|body| kimix_tools::implementations::skills::skill::build_skill_message(skill, body))
}
...
/// Build a compact skill router table for injection into the system prompt.
///
/// Instead of full skill bodies (~40K tokens), this produces a keyword→skill
/// routing table (~2K tokens). The agent uses `read_file` to load the full
/// SKILL.md on demand when a keyword matches the user's intent.
///
/// Inspired by grok-build's SKILL-ROUTER.md.
///
/// `exclude_paths` — paths of already-preloaded skills to omit from the router.
pub(crate) fn build_skill_router_prompt(
    skills: &[SkillInfo],
    exclude_paths: &std::collections::HashSet<String>,
) -> String {
    const HEADER_LINES: usize = 5;

    // Filter: enabled only, exclude preloaded, sort by name for cache stability
    let mut active: Vec<&SkillInfo> = skills
        .iter()
        .filter(|s| s.enabled && !s.disable_model_invocation)
        .filter(|s| !exclude_paths.contains(&s.path))
        .collect();
    if active.is_empty() {
        return String::new();
    }
    active.sort_by_key(|s| &s.name);

    let mut lines = vec![
        "\n## Skill Router\n".to_string(),
        "When the user's request matches a keyword below, use `read_file` to load the full"
            .to_string(),
        "SKILL.md at the listed path. Do NOT pre-load skills — load only when keywords match.\n"
            .to_string(),
        "| Keywords | Skill | Path |".to_string(),
        "|----------|-------|------|".to_string(),
    ];
    let mut has_skills = false;
    for skill in &active {
        let keywords = extract_skill_keywords(skill);
        if keywords.is_empty() {
            continue;
        }
        lines.push(format!(
            "| {} | {} | {} |",
            escape_md_table(&keywords),
            escape_md_table(&skill.name),
            escape_md_table(&skill.path)
        ));
        has_skills = true;
    }
    if !has_skills || lines.len() <= HEADER_LINES {
        return String::new();
    }
    lines.join("\n")
}

/// Escape Markdown table special characters to prevent injection.
fn escape_md_table(s: &str) -> String {
    s.replace('|', "\\|").replace('\n', " ")
}

/// Extract searchable keywords from a skill's name, triggers, and description.
///
/// Priority: `when_to_use` / triggers > `display_name` > `name` > `short_description` > `description`.
fn extract_skill_keywords(skill: &SkillInfo) -> String {
    // If explicit trigger phrases exist, use them directly
    if let Some(ref wtu) = skill.when_to_use
        && !wtu.is_empty()
    {
        return wtu.to_lowercase();
    }

    let mut parts: Vec<String> = Vec::new();
    let effective_name = skill.display_name.as_deref().unwrap_or(&skill.name);
    for part in effective_name.split(&['-', '_', ' '][..]) {
    ...
    // Add short_description words if available — more signal than full description
    let desc = skill
        .short_description
        .as_deref()
        .unwrap_or(&skill.description);
    if !desc.is_empty() {
        for word in desc.split_whitespace().take(5) {
            let clean: String = word
                .to_lowercase()
                .chars()
                .filter(|c| c.is_alphanumeric())
                .collect();
            if clean.len() > 2 && !parts.contains(&clean) {
                parts.push(clean);
            }
        }
    }
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || kimix_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match kimix_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    async fn server_skills_discovered_and_shadowed_by_local() {
        ...
        write_skill_md(&cwd.path().join(".kimix").join("skills").join("dup"), "dup");
    ...
    fn find_skill_paths_flat_layout() {
        ...
        let kimix_dir = tmp.path().join(".kimix");

        write_skill_md(&kimix_dir.join("skills").join("alpha"), "alpha");
        write_skill_md(&kimix_dir.join("skills").join("beta"), "beta");

        let paths = find_skill_paths(&kimix_dir);
    ...
    fn find_skill_paths_nested_layout() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let kimix_dir = tmp.path().join(".kimix");
        let skills = kimix_dir.join("skills");
    ...
    fn find_skill_paths_no_skills_dir() {
        // .kimix exists but no skills/ subdirectory
        let tmp = tempfile::tempdir().unwrap();
        let kimix_dir = tmp.path().join(".kimix");
        fs::create_dir_all(&kimix_dir).unwrap();
    ...
    fn parse_model_and_effort() {
        let content = "---\nname: my-skill\ndescription: test\nmodel: kimix-3\neffort: high\n---\n";
        let parsed = parse_skill_frontmatter(content, None).unwrap();
        assert_eq!(parsed.model.as_deref(), Some("kimix-3"));
    ...
    async fn list_skills_includes_workspace_user_skills() {
        ...
        write_skill_md(
            &user_dir.join(".kimix").join("skills").join("my-tool"),
            "my-tool",
        );
    ...
    async fn list_skills_workspace_user_dedup_when_cwd_inside_user_dir() {
        ...
        let user_dir = repo_root.join("x").join("testuser");
        write_skill_md(
            &user_dir.join(".kimix").join("skills").join("dedup-skill"),
            "dedup-skill",
        );
    ...
    async fn list_skills_no_workspace_user_dir_no_extra_skills() {
        ...
        write_skill_md(
            &user_dir.join(".kimix").join("skills").join("ghost-skill"),
            "ghost-skill",
        );
    ...
    async fn list_skills_workspace_user_dir_with_recursive_skills() {
        ...
        let skills_base = user_dir.join(".kimix").join("skills");
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(kimix_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    fn stamp_plugin_fields_sets_root_and_data() {
        ...
        let plugin = LoadedPlugin {
            ...
            origin: crate::plugins::PluginOrigin::UserKimix,
            trusted: true,
            enabled: true,
    ...
    async fn list_skills_deduplicates_auto_and_config_overlap() {
        ...
        let auto_dir = repo_root.join(".kimix").join("skills").join("dup-skill");
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        assert!(
            matches!(
                ...
                Some(kimix_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    async fn list_skills_ignore_allows_lower_priority_same_name_fallback() {
        ...
        write_skill_md(&cwd.join(".kimix").join("skills").join("same"), "same");
        let repo_skill_dir = repo_root.join(".kimix").join("skills").join("same");
    ...
    // these functions now live in kimix-tools::implementations::skills::discovery
    // and kimix-tools::types::skill_discovery_tracker, tested there.
    ...
    async fn disabled_config_marks_skill_enabled_false() {
        ...
        write_skill_md(
            &repo_root.join(".kimix").join("skills").join("commit"),
            "commit",
        );
        write_skill_md(
            &repo_root.join(".kimix").join("skills").join("review"),
            "review",
        );
    ...
    async fn disabled_config_empty_leaves_all_enabled() {
        ...
        write_skill_md(
            &repo_root.join(".kimix").join("skills").join("deploy"),
            "deploy",
        );
    ...
    fn min_plugin(name: &str) -> crate::plugins::LoadedPlugin {
        ...
        crate::plugins::LoadedPlugin {
            ...
            origin: crate::plugins::PluginOrigin::ProjectKimix,
            trusted: true,
            enabled: true,
            version: Some("1.0.0".to_string()),
            description: None,
    ...
    fn collect_skill_config_dirs_gates_vendor_dirs() {
        ...
        for name in [".kimix", ".agents", ".claude", ".cursor"] {
            fs::create_dir_all(cwd.join(name)).unwrap();
        }

        let ends_with = |dirs: &[PathBuf], suffix: &str| dirs.iter().any(|d| d.ends_with(suffix));

        // all_on → both vendor dirs present (historical multi-vendor behavior).
        let all =
            collect_skill_config_dirs(Some(cwd), None, tmp.path(), &[], CompatConfig::all_on());
        ...
        let mut compat = CompatConfig::all_on();
        ...
        assert!(
            ends_with(&dirs, ".kimix") || ends_with(&dirs, ".kimix"),
            "native kimix/kimix dir must remain: {dirs:?}"
        );
    }
    ...
    fn dedupe_same_scope_cross_harness_loser_resurfaces() {
        // A `.claude` skill claiming a `.kimix`-owned name (both User scope)
        // was silently hidden before; it now re-keys to its dir basename.
        let out = dedupe_skills(vec![
            named_skill(
                "review",
                "/u/.kimix/skills/review/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "review",
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.kimix/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.kimix/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.kimix/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.kimix/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.kimix/skills/japandi/SKILL.md",
        ...
        assert_eq!(out.len(), 1);
        assert!(out[0].path.contains(".kimix"));
    }
    ...
    async fn copied_skill_dir_with_stale_frontmatter_name_surfaces_both() {
        ...
        let skills_dir = repo_root.join(".kimix").join("skills");
