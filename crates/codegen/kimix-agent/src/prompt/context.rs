...
//! `TemplateRenderer` in `Kimix-tools`. This struct does NOT own a
...
/// Default identity on trim-tool-descriptions (`You are Kimix, an AI coding agent`).
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "Kimix";
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "Kimix Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "Kimix Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.kimix/worktrees/my-project/ab-123-a-overlay";
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a Kimix agent"),
