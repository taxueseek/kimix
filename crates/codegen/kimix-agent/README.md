...
This crate extracts a first-class `Agent` type from `kimix-shell`.
...
`kimix-shell`, another in-process host, or a headless batch runner.
...
in `.kimix/agents/` (project-level) or `~/.kimix/agents/` (user-level).

```rust
use kimix_agent::{AgentDefinition, AgentBuilder};
use kimix_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".kimix/agents/code-reviewer.md")?;
...
```rust
use kimix_agent::discovery;

// Find all .md files in .kimix/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  kimix-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`kimix-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `kimix-agent` depends on it for tool setup.
- **`kimix-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `kimix-agent` (AGENTS.md
...
|---|---|---|
| `kimix` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p kimix-agent

# Test
cargo test -p kimix-agent

# Clippy
cargo clippy -p kimix-agent --fix --allow-dirty
