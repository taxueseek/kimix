...
//!   nothing. `KIMIX_FSNOTIFY_PER_DIR=1|0` overrides the platform default.
...
/// Whether Sapling (`.sl`) support is enabled (default on; `KIMIX_FSNOTIFY_SAPLING=0`
...
pub(crate) fn sapling_enabled() -> bool {
    !matches!(
        std::env::var("KIMIX_FSNOTIFY_SAPLING").ok().as_deref(),
        Some("0") | Some("false")
    )
}
...
/// Resolve the strategy: `KIMIX_FSNOTIFY_PER_DIR=1|true` forces per-dir,
...
pub(crate) fn watch_strategy() -> WatchStrategy {
    match std::env::var("KIMIX_FSNOTIFY_PER_DIR").ok().as_deref() {
...
/// Per-dir mode's total watch budget (`KIMIX_FSNOTIFY_MAX_WATCHES` overrides).
///
...
pub(crate) fn max_watch_budget() -> usize {
    std::env::var("KIMIX_FSNOTIFY_MAX_WATCHES")
        .ok()
                        ...
                        if dirs.len() > budget {
                            tracing::warn!(
                                "fs_notify: {} non-ignored dirs exceed watch budget {budget}; \
                                 shedding the deepest (raise with KIMIX_FSNOTIFY_MAX_WATCHES)",
                                dirs.len()
                            );
...
mod tests {
    ...
    mod helper_tests {
        ...
        fn external_ancestor_sl_arms_in_recursive_root_mode() {
            ...
            // (e.g. `kimix` run in `crates/codegen`): the production guard must
        ...
        fn sapling_enabled_respects_kill_switch() {
            ...
            impl Drop for Restore {
                fn drop(&mut self) {
                    ...
                    unsafe { std::env::remove_var("KIMIX_FSNOTIFY_SAPLING") };
                }
            }
            ...
            for off in ["0", "false"] {
                unsafe { std::env::set_var("KIMIX_FSNOTIFY_SAPLING", off) };
                assert!(!sapling_enabled(), "{off:?} must disable Sapling");
            }
            unsafe { std::env::set_var("KIMIX_FSNOTIFY_SAPLING", "1") };
