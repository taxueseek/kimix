...
use kimix_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource};
...
async fn shared_dedupes_by_directory() {
    ...
    let Ok(a) = kimix_fsnotify::shared(path.clone(), FsConfig::default()) else {
        eprintln!("skipping: OS watcher unavailable (resource limit?)");
        return;
    };
    let before = kimix_fsnotify::stats();
    let b = kimix_fsnotify::shared(path.clone(), FsConfig::default()).unwrap();
    ...
    let after = kimix_fsnotify::stats();
    assert_eq!(
    ...
    let c = kimix_fsnotify::shared(other.path().to_path_buf(), FsConfig::default()).unwrap();
    assert!(!Arc::ptr_eq(&a, &c), "different dirs must not share");
    assert_eq!(
        kimix_fsnotify::stats().created_total - after.created_total,
        1,
    ...
    drop(a);
    drop(b);
    let d = kimix_fsnotify::shared(path, FsConfig::default()).unwrap();
    assert_eq!(
        Arc::strong_count(&d),
        1,
...
///
/// ```bash
/// cargo test -p Kimix-fsnotify --test integration \
...
async fn shared_watcher_scaling_demo() {
    ...
    for _ in 0..SESSIONS {
        let Ok(src) = kimix_fsnotify::shared(path.clone(), FsConfig::default()) else {
            eprintln!("skipping: OS watcher unavailable (resource limit?)");
            return;
        };
        handles.push(src);
