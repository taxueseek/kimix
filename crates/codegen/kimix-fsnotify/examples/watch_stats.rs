...
//! startup latency, under either strategy (`KIMIX_FSNOTIFY_PER_DIR=0|1`).
//!

//! ```bash
//! # Generate a synthetic tree, then measure both strategies against it:
//! cargo run --release -p Kimix-fsnotify --example watch_stats -- gen js /tmp/js-repo
//! KIMIX_FSNOTIFY_PER_DIR=0 cargo run --release -p Kimix-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! KIMIX_FSNOTIFY_PER_DIR=1 cargo run --release -p Kimix-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "Kimix holds 55k inotify watches" report).
...
use kimix_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("KIMIX_FSNOTIFY_PER_DIR").unwrap_or_default();
