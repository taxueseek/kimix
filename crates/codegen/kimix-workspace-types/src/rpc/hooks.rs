...
//!
//! `kimix_hooks` pulls in `git2`/`reqwest`/`Kimix-tools`, too heavy for
...
/// Wire mirror of `kimix_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `kimix_hooks::config::HookSpec`.
///
...
/// `Kimix-hooks`, so a server-side test (`Kimix-workspace`'s
...
/// Wire mirror of `kimix_hooks::event::HookEventName`.
///
