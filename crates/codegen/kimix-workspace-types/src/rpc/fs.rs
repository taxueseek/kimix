...
//! the shell's `Kimix/fs/*` ACP methods.
...
// workspace server (`Kimix-workspace`) and the remote backend
...
// `workspace.fs_*` ops above already serve the shell's `Kimix/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `Kimix/fs/list`
/// node `type` strings.
...
/// `Kimix/fs/list` plus `offset` pagination).
