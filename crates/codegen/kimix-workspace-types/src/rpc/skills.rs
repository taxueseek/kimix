...
//! `Kimix-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.Kimix/skills
    Local,
    /// repo_root/.Kimix/skills
    Repo,
    /// ~/.kimix/skills
    User,
    /// ~/.kimix/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.kimix/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the Kimix-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.kimix/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.kimix/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.kimix/plugins/infra-plugin",
            "plugin_data": "/root/.kimix/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "kimix-4",
            "effort": "high",
            "user_invocable": true,
