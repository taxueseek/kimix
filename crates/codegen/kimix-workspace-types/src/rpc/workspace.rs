...
/// over `kimix_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `kimix_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// One TODO list item (slim DTO over `kimix_tools`'s `TodoState`). `status`
...
/// in `Kimix-workspace/src/hub_server.rs`.
