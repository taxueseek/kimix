//! Shared API definitions for Kimix tools: protobuf types, config validation,
...
pub mod pb {
    include!(concat!(env!("OUT_DIR"), "/kimix.tools.v1.rs"));
}
...
///
/// Tool ids are colon-separated `Namespace:tool` (e.g. `Kimix:grep`); the
...
mod default_client_name_tests {
    ...
    fn pins_first_colon_derivation() {
        assert_eq!(default_client_name("Kimix:grep"), "grep");
