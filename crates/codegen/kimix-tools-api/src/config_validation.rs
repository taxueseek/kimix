...
mod tests {
    ...
    fn unset_params_is_ok_none() {
        assert_eq!(parse_params_json(0, "Kimix:grep", None), Ok(None));
    }

    #[test]
    fn valid_object_is_returned() {
        let parsed = parse_params_json(0, "Kimix:grep", Some(r#"{"max_results":50}"#)).unwrap();
        assert_eq!(
            parsed,
            Some(
    ...
    fn empty_string_is_a_parse_error() {
        let err = parse_params_json(3, "Kimix:grep", Some("")).unwrap_err();
    ...
    fn name_override_unset_or_valid_is_ok() {
        assert_eq!(validate_name_override(0, "Kimix:grep", None), Ok(()));
        for name in ["search", "Kimix:grep", "a-b_C9"] {
            assert_eq!(
                validate_name_override(0, "Kimix:grep", Some(name)),
                Ok(()),
                "name={name:?}"
            );
        }
    ...
    fn name_override_outside_charset_is_rejected() {
        for name in ["has space", "", "a:b:c", "dot.name"] {
            let err = validate_name_override(2, "Kimix:grep", Some(name)).unwrap_err();
    ...
    fn all_ids_present_returns_none() {
        let entries = [entry("Kimix:grep"), entry("Kimix:read_file")];
        let allowed = allowed(&["Kimix:grep", "Kimix:read_file", "Kimix:bash"]);
        assert_eq!(first_unknown_tool_id(&entries, &allowed), None);
    }

    #[test]
    fn empty_entries_returns_none() {
        assert_eq!(first_unknown_tool_id(&[], &allowed(&["Kimix:grep"])), None);
    }
    ...
    fn first_unknown_id_is_returned_with_index() {
        let entries = [
            entry("Kimix:grep"),
            entry("Kimix:nonexistent"),
            entry("Kimix:also_missing"),
        ];
        let allowed = allowed(&["Kimix:grep"]);
        assert_eq!(
            first_unknown_tool_id(&entries, &allowed),
            Some((1, "Kimix:nonexistent"))
        );
    }
}
