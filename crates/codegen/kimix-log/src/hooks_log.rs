...
//! file logger that writes to `~/.kimix/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `kimix_hooks` or `kimix_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! KIMIX_HOOKS_LOG=1 Kimix              # enable, write to ~/.kimix/logs/hooks.log
//! KIMIX_HOOKS_LOG=/tmp/h.log Kimix     # write to custom path
//! KIMIX_HOOKS_LOG=0 Kimix              # explicitly disable
//! tail -f ~/.kimix/logs/hooks.log     # watch in another terminal
//! ```
use std::fmt;
...
const ENV_HOOKS_LOG: &str = "KIMIX_HOOKS_LOG";
...
///
/// Writes to `~/.kimix/logs/hooks.log` (or custom path via `KIMIX_HOOKS_LOG`).
/// Filters to hooks (`kimix_hooks`) and plugins (`kimix_agent::plugins`) targets.
/// Set `KIMIX_HOOKS_LOG=0` to disable, `KIMIX_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter =
        tracing_subscriber::filter::EnvFilter::new("kimix_hooks=debug,kimix_agent::plugins=debug");
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || kimix_home().join("logs").join("hooks.log");
