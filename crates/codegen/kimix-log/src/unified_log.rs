...
//! over ACP (`Kimix/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "kimix/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "kimix-pager")]
    #[serde(rename = "kimix-pager")]
    KimixPager,
    #[strum(serialize = "kimix-desktop")]
    #[serde(rename = "kimix-desktop")]
    KimixDesktop,
}
...
/// Wire format for the `Kimix/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn log_path() -> PathBuf {
    kimix_home().join(LOG_DIR).join(LOG_FILE)
}
...
///
/// Called by the `Kimix/log` notification handler. Entries from
