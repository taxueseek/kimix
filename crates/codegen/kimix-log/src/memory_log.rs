...
//!
//! Provides a dedicated tracing target (`kimix_memory`) with an optional
//! file logger that writes to `~/.kimix/logs/memory.log`.
//!
//! ## When to use
//!
...
//! KIMIX_MEMORY_LOG=0 Kimix                # disable even when enabled
//! tail -f ~/.kimix/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "kimix_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "KIMIX_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.kimix/logs/memory.log`. Filters to `kimix_memory=trace`.
    /// Set `KIMIX_MEMORY_LOG=0` to disable, `KIMIX_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || kimix_home().join("logs").join("memory.log");
