...
//! - PerSession (`KIMIX_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.kimix/debug/<session_id>.txt` (one file per session), with a
...
//! - SingleFile (explicit path via `KIMIX_LOG_FILE` or `KIMIX_DEBUG_LOG=<path>`):
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::KimixLogFile => "KIMIX_LOG_FILE",
            Self::KimixDebugLog => "KIMIX_DEBUG_LOG",
        }
    }
}
...
///
/// Lives here (not in `Kimix-tui`) so the firehose directives below and
...
/// safe. See `Kimix-tui/src/tracing.rs` for the consumer side.
...
// Broad firehose filter for the routing/KIMIX_DEBUG_LOG sources: capture our
// crates at debug regardless of a narrowing RUST_LOG, with deps at info so they
// don't flood. Curated first-party allowlist: new Kimix crates default to `info`
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,kimix_tui=debug,kimix_shell=debug,kimix_tools=debug,kimix_log=debug,kimix_agent=debug,kimix_mcp=debug,kimix_acp_lib=debug,sampling_log=off";
...
// KIMIX_DEBUG_LOG single-file source (mirrors `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the KIMIX_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves KIMIX_LOG_FILE back-compat).
...
/// filter by source (KIMIX_LOG_FILE respects RUST_LOG). Open failures warn AFTER
    ...
    match resolve_debug_target() {
        ...
        Some(DebugTarget::SingleFile { path, src }) => {
            let filter = match src {
                DebugSource::KimixLogFile => default_file_filter(),
                DebugSource::KimixDebugLog => firehose_filter(),
            };
...
pub(crate) enum DebugTarget {
    /// `KIMIX_DEBUG_LOG=1` → route per session into `<dir>` (`~/.kimix/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path → one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit KIMIX_LOG_FILE wins
/// (single file, RUST_LOG filter); else KIMIX_DEBUG_LOG — a truthy bool routes
/// per session into `~/.kimix/debug`, an explicit path writes a single file.
///
/// Read via `var_os` (not `var`) so a non-UTF-8 path isn't silently dropped.
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let kimix_log_file = std::env::var_os("KIMIX_LOG_FILE");
    let kimix_debug_log = std::env::var_os("KIMIX_DEBUG_LOG");
    resolve_debug_target_inner(
        kimix_log_file.as_deref(),
        kimix_debug_log.as_deref(),
        &kimix_home().join("debug"),
    )
}
...
fn resolve_debug_target_inner(
    kimix_log_file: Option<&OsStr>,
    kimix_debug_log: Option<&OsStr>,
    debug_dir: &Path,
) -> Option<DebugTarget> {
    if let Some(raw) = kimix_log_file
        && !is_blank(raw)
    {
        return Some(DebugTarget::SingleFile {
            path: os_path(raw),
            src: DebugSource::KimixLogFile,
        });
    }
    let raw = kimix_debug_log?;
...
/// `~/.kimix/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
...
pub(crate) fn sweep_old_logs() {
    prune_old_logs(&kimix_home().join("debug"), LOG_RETENTION);
}
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for KIMIX_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for KIMIX_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace KIMIX_LOG_FILE is treated as unset (mirrors KIMIX_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 KIMIX_DEBUG_LOG value is a path, not a bool keyword, and its
        ...
        match target {
            DebugTarget::SingleFile { path, src } => {
                assert_eq!(src, DebugSource::KimixDebugLog);
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "kimix_shell", "inside session");
            });
            tracing::info!(target: "kimix_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `kimix_log::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "kimix_log::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "kimix_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "kimix_shell", "one first");
                tracing::info!(target: "kimix_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "kimix_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn prune_old_logs_missing_dir_is_noop() {
        ...
        prune_old_logs(
            Path::new("/no/such/kimix/debug/dir"),
