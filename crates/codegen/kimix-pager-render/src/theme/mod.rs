...
//! The default theme is KimixNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! KimixNight is defined in `Color::Rgb` (truecolor). At startup,
...
pub enum ThemeKind {
    ...
    /// 暗夜森林 — 墨绿底 + 青玉绿 accent。
    Forest = 7,
    /// 月之暗面 — 深空黑底 + 冷蓝 accent，月球暗面朝观众。
    MoonDark = 8,
    /// 月之亮面 — 月白底 + 暖银 accent，月球亮面朝观众。
    MoonLight = 9,
    /// 红月 — 深红黑底 + 血红 accent，血月光晕动画。
    BloodMoon = 10,
    /// Grok 暗黑 — 参考 xAI Grok 暗黑风格。
    GrokDark = 11,
    /// DeepSeek 蓝鲸 — 参考 DeepSeek 蓝色鲸鱼风格。
    DeepSeekBlue = 12,
...
impl ThemeKind {
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::KimixNight => "kimixnight",
            Self::TokyoNight => "tokyonight",
            Self::KimixDay => "kimixday",
            Self::RosePineMoon => "rosepine-moon",
            Self::OscuraMidnight => "oscura-midnight",
            Self::Sakura => "sakura",
            Self::Forest => "forest",
            Self::MoonDark => "moon-dark",
            Self::MoonLight => "moon-light",
            Self::BloodMoon => "blood-moon",
            Self::GrokDark => "grok-dark",
            Self::DeepSeekBlue => "deepseek-blue",
            Self::Auto => "auto",
        }
    }
    ...
    pub fn requires_truecolor(self) -> bool {
        match self {
            ...
            Self::MoonLight => true,
            Self::BloodMoon => true,
            // Grok / DeepSeek 风格需要真彩色
            Self::GrokDark => true,
            Self::DeepSeekBlue => true,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "kimixnight" | "kimix-night" | "dark" => Some(Self::KimixNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "kimixday" | "kimix-day" | "light" | "day" => Some(Self::KimixDay),
            "rosepine" | "rose-pine" | "rosepine-moon" | "rose-pine-moon" => {
                Some(Self::RosePineMoon)
            }
            "oscura" | "oscura-midnight" => Some(Self::OscuraMidnight),
            "sakura" | "sakura-night" | "樱花" => Some(Self::Sakura),
            "forest" | "forest-night" | "森林" => Some(Self::Forest),
            "moon-dark" | "moondark" | "月之暗面" => Some(Self::MoonDark),
            "moon-light" | "moonlight" | "月之亮面" => Some(Self::MoonLight),
            "blood-moon" | "bloodmoon" | "红月" => Some(Self::BloodMoon),
            "grok-dark" | "grokdark" | "grok" => Some(Self::GrokDark),
            "deepseek-blue" | "deepseekblue" | "deepseek" | "蓝鲸" => Some(Self::DeepSeekBlue),
            _ => None,
        }
    }
...
/// `"kimixnight"` → `"Kimix Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "kimixnight" => "Kimix Night",
        "kimixday" => "Kimix Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.kimix/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::KimixNight => Self::kimixnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::KimixDay => Self::kimixday(),
            ThemeKind::RosePineMoon => Self::rosepine_moon(),
            ThemeKind::OscuraMidnight => Self::oscura_midnight(),
            ThemeKind::Sakura => Self::sakura(),
            ThemeKind::Forest => Self::forest(),
            ThemeKind::MoonDark => Self::moon_dark(),
            ThemeKind::MoonLight => Self::moon_light(),
            ThemeKind::BloodMoon => Self::blood_moon(),
            ThemeKind::GrokDark => Self::grok_dark(),
            ThemeKind::DeepSeekBlue => Self::deepseek_blue(),
            // Auto is resolved to a concrete theme before being stored;
            // if reached, fall back to KimixNight.
            ThemeKind::Auto => Self::kimixnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `KIMIX_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on kimixnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., KimixNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // KimixNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    kimix_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::KimixNight.is_auto());
        assert!(!ThemeKind::KimixDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::kimixday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::kimixnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::kimixday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. KimixNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::KimixNight => Theme::kimixnight(),
                ThemeKind::KimixDay => Theme::kimixday(),
                ...
                ThemeKind::Sakura => Theme::sakura(),
                ThemeKind::Forest => Theme::forest(),
                ThemeKind::MoonDark => Theme::moon_dark(),
                ThemeKind::MoonLight => Theme::moon_light(),
                ThemeKind::BloodMoon => Theme::blood_moon(),
                ThemeKind::GrokDark => Theme::grok_dark(),
                ThemeKind::DeepSeekBlue => Theme::deepseek_blue(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_kimixnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark KimixNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::kimixnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("kimixnight"),
            Some(ThemeKind::KimixNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::KimixNight));
        assert_eq!(ThemeKind::from_name("kimixday"), Some(ThemeKind::KimixDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::KimixDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("kimixnight", ThemeKind::KimixNight),
            ("kimix-night", ThemeKind::KimixNight),
            ("dark", ThemeKind::KimixNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("kimixday", ThemeKind::KimixDay),
            ("kimix-day", ThemeKind::KimixDay),
            ("light", ThemeKind::KimixDay),
            ("day", ThemeKind::KimixDay),
