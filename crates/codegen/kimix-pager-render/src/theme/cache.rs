...
//! lookup must be cheaper than re-loading from `~/.kimix/config.toml`.
...
//! `kimix_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::KimixNight as u8);
...
/// back to `KimixNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::KimixNight as u8 => ThemeKind::KimixNight,
        x if x == ThemeKind::KimixDay as u8 => ThemeKind::KimixDay,
        ...
        x if x == ThemeKind::Sakura as u8 => ThemeKind::Sakura,
        x if x == ThemeKind::Forest as u8 => ThemeKind::Forest,
        x if x == ThemeKind::MoonDark as u8 => ThemeKind::MoonDark,
        x if x == ThemeKind::MoonLight as u8 => ThemeKind::MoonLight,
        x if x == ThemeKind::BloodMoon as u8 => ThemeKind::BloodMoon,
        x if x == ThemeKind::GrokDark as u8 => ThemeKind::GrokDark,
        x if x == ThemeKind::DeepSeekBlue as u8 => ThemeKind::DeepSeekBlue,
        x if x == ThemeKind::Auto as u8 => ThemeKind::Auto,
        _ => ThemeKind::KimixNight,
    }
}
...
/// When `None`, `to_theme_kind()` defaults to `KimixNight` / `KimixDay`.
...
///
/// On the first call, reads from `~/.kimix/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    kimix_markdown::set_color_level_cap(if locked {
        kimix_markdown::ColorLevel::Basic
    } else {
        kimix_markdown::ColorLevel::TrueColor
    });
}
...
///
/// Precedence:
/// 1. Environment variable (`KIMIX_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `KimixNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::KimixNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `KimixNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `kimix_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = kimix_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = kimix_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::KimixNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::KimixDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::KimixNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::KimixDay);
            assert_eq!(applied, ThemeKind::KimixNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::KimixNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::kimixday().text_primary,
                "must not serve the cached (KimixDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // kimixday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::KimixDay), false),
                ThemeKind::KimixDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::KimixNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_kimixnight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::KimixNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_kimixday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::KimixDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_kimixnight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_kimixnight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::KimixDay), true);
            assert_eq!(result, ThemeKind::KimixDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::KimixDay);
            assert_eq!(current_kind(), ThemeKind::KimixDay);
        });
    }
