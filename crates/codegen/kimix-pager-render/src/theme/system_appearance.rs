...
/// in `config.toml`. When `None`, defaults to `KimixNight` / `KimixDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::KimixDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::KimixNight),
    }
}
