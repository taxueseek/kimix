...
pub use kimix_config::kimix_home;

/// Path to `$KIMIX_SHARE_DIR/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    kimix_home().join("pager.toml")
}

/// User-facing label for the user Kimix directory (``~/.kimix`` or ``$KIMIX_SHARE_DIR``).
///
/// Derived from resolved [`kimix_home()`] vs `kimix_config::default_kimix_home()`,
/// not from whether `KIMIX_SHARE_DIR` is set in the environment.
pub fn display_kimix_home_prefix() -> String {
    if kimix_home() == kimix_config::default_kimix_home() {
        "~/.kimix".to_string()
    } else {
        "$KIMIX_SHARE_DIR".to_string()
    }
}

/// User-facing path under [`kimix_home()`], e.g. ``~/.kimix/config.toml``.
pub fn display_user_kimix_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_kimix_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`kimix_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let kimix = kimix_home();
    if let Ok(rest) = path_buf.strip_prefix(&kimix) {
        let prefix = display_kimix_home_prefix();
...
/// True when `path` is under user [`kimix_home()`] (not project `{cwd}/.Kimix`).
pub fn is_under_user_kimix_home(path: &Path) -> bool {
    path.starts_with(kimix_home())
}
...
mod tests {
    ...
    #[test]
    fn display_kimix_home_prefix_default_install() {
        if std::env::var("KIMIX_SHARE_DIR").is_ok() {
            return;
        }
        assert_eq!(display_kimix_home_prefix(), "~/.kimix");
    }

    #[test]
    fn display_user_kimix_path_joins_relative() {
        let path = display_user_kimix_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".kimix") || path.contains("$KIMIX_SHARE_DIR"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_kimix() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.kimix/memory/MEMORY.md");
