...
//! frame diff (see `kimix_ratatui_inline::Terminal::flush_with_links`).
...
/// One path segment without spaces (`main.rs`, `.Kimix`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — Kimix
/// session media lives under `~/.kimix/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.Kimix/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.kimix/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    #[test]
    fn scan_detects_kimix_session_media_path() {
        // Dot-directory (`.Kimix`), percent-encoded session segment, and a
        // trailing sentence period — the shape of media-tool output prose.
        let line = make_line("Saved to /Users/alice/.kimix/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.kimix/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.kimix/sessions/%2FUsers%2Fali");
        ...
        let expected_url = "file:///Users/alice/.kimix/sessions/%252FUsers%252Fali\
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.kimix/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.kimix/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*link.url,
                "file:///Users/me/.kimix/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/kimix-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/kimix-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/Kimix-tui/src/render` should
        // NOT produce a link for the `/Kimix-tui/src/render` substring.
        let line = make_line("find crates/codegen/kimix-tui/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
