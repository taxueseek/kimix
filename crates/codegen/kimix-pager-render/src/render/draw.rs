...
//! [`kimix_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use kimix_ratatui_inline::LinkSpan;
use ratatui::Frame;
...
pub type PagerTerminal = kimix_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
...
pub fn spawn_writer_thread() -> (mpsc::Sender<Vec<u8>>, WriterSync, WriterThread) {
    ...
    let test_delay = std::env::var("KIMIX_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = kimix_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(data) = rx.recv() {
                ...
                {
                    let _guard = kimix_shared::stderr::stderr_lock();
...
mod tests {
    ...
    fn writer_thread_preserves_multibyte_utf8() {
        ...
        let handle = std::thread::spawn(move || {
            ...
            *capture2.lock().expect("lock poisoned") = buf;
        });
        tx.send(expected_bytes.clone()).unwrap();
        drop(tx);
        handle.join().unwrap();
        let captured = capture.lock().expect("lock poisoned");
        assert_eq!(
