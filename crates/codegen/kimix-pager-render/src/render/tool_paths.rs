...
fn resolve_tool_path_with_home(
    ...
    let display_path = target
        .as_deref()
        .map(kimix_paths::normalize_lexically)
        .unwrap_or_else(|| PathBuf::from(path));
    let relative_to_cwd = target.as_ref().and_then(|_| {
        let cwd = kimix_paths::normalize_lexically(cwd?);
...
mod tests {
    ...
    fn shorten_path_fish_style() {
        let result = shorten_path("crates/codegen/kimix-tui/src/views/foo.rs", 25);
    ...
    fn shorten_path_front_truncate() {
        let result = shorten_path(
            "crates/codegen/kimix-tui/src/views/very_long_filename.rs",
            20,
        );
    ...
    fn expanded_surface_uses_worktree_cwd() {
        let cwd = Path::new("/Users/me/.kimix/worktrees/foo");
        let path = "/Users/me/.kimix/worktrees/foo/crates/x/a.rs";
        assert_eq!(
    ...
    fn home_relative_target_preserves_filesystem_spelling_for_io() {
        ...
        assert_eq!(
            ...
            kimix_paths::normalize_lexically(&home.join("notes.md"))
        );
    }
