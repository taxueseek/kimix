...
//! `Kimix-ratatui-textarea`, and adds [`SystemClipboard`] backed by `arboard`.
//!
//! Multi-fire writes (native / tmux / OSC 52); user-facing success is [`trust`].
mod trust;

pub use kimix_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(kimix_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(kimix_shared::clipboard::is_containerized_without_display)
}

/// Cached result of the "an upstream OSC 52 sink is capturing our output" check.
///
/// `kimix wrap` runs a command inside a local PTY, scans its output for OSC 52
/// clipboard sequences, and writes their payload to the *real* (local) system
/// clipboard (see `Kimix-tui`'s `pty_wrap` module). It advertises this to
/// the wrapped program via an environment variable so the
/// inner `kimix` knows its OSC 52 writes are reliably intercepted and copied,
...
///
/// Two names are accepted: the canonical `KIMIX_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_KIMIX_OSC52_SINK`, which the default
/// OpenSSH client/server configs forward (`SendEnv LANG LC_*` /
/// `AcceptEnv LANG LC_*`) so the signal survives the hop into a remote `kimix`.
pub fn osc52_sink_active() -> bool {
    static SINK: OnceLock<bool> = OnceLock::new();
    *SINK.get_or_init(|| {
        std::env::var_os("KIMIX_OSC52_SINK").is_some()
            || std::env::var_os("LC_KIMIX_OSC52_SINK").is_some()
    })
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if kimix_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
pub fn resolve_clipboard_route(ctx: &TerminalContext) -> ClipboardRoute {
    ...
    ClipboardRoute {
        ...
        // upstream `kimix wrap` sink is capturing our output and will forward
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = kimix_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = kimix_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`kimix_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        kimix_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = kimix_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match kimix_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    kimix_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !kimix_shared::clipboard::x11_display_env_present() {
        return None;
    }
    kimix_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && kimix_shared::clipboard::x11_display_env_present()
    }
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    match kimix_shared::clipboard::get_attachments() {
...
pub use kimix_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    kimix_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    kimix_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    kimix_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(kimix_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    match kimix_shared::clipboard::get_image() {
        Ok(img) => Ok(img),
        Err(e) => {
