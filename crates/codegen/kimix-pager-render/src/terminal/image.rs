...
/// minimal mode (`kimix --minimal`) sets this once at startup: it never runs the
...
pub fn kitty_format_from_bytes(image_data: &[u8]) -> Option<KittyImageFormat> {
    match kimix_shared::clipboard::mime_from_bytes(image_data) {
...
fn convert_via_sips(image_data: &[u8]) -> Option<Vec<u8>> {
    ...
    let ts = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let src = tmp_dir.join(format!("kimix-sips-{id}-{ts}.dat"));
    let dst = tmp_dir.join(format!("kimix-sips-{id}-{ts}.png"));
    ...
    kimix_tty_utils::detach_std_command(&mut sips_cmd);
