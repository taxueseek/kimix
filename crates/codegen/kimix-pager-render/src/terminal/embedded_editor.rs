//! Detects whether Kimix is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` Kimix is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `Kimix-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
