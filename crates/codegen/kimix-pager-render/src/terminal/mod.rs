...
pub enum TerminalName {
    ...
    JetBrains,
    /// Kimix Desktop (Electron app).
    #[strum(to_string = "Kimix Desktop")]
    KimixDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` Kimix is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches Kimix, which
    ...
    pub fn diagnostics_snapshot(&self) -> TerminalDiagnostics {
        ...
        TerminalDiagnostics {
            ...
            clipboard_native_tool: kimix_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> kimix_shared::session::FeedbackTerminalInfo {
        use kimix_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(kimix_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `kimix ssh` can
    // forward env vars.
...
/// `Kimix-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "kimixdesktop" => Some(TerminalName::KimixDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.kimix/pager.toml` and
