...
pub struct AnimationConfig {
    ...
    /// Also enabled by the `KIMIX_FPS=1` env var. Default: false.
    pub show_fps: bool,
}
...
pub struct RawAnimationConfig {
    ...
    /// Requires a debug build. Also enabled by KIMIX_FPS=1 env var. Default: false.
    pub show_fps: bool,
}
...
fn lookup_named_color(name: &str) -> Result<Color, String> {
    // Named colors use the KimixNight RGB palette. They are quantized via
...
impl RawAppearanceConfig {
    pub fn to_toml_with_comments() -> String {
        ...
        let pager_path = crate::util::display_user_kimix_path("pager.toml");
        let header = format!(
            "\
# Kimix Pager Appearance Configuration ({pager_path})
...
pub fn persist_respect_manual_folds(enabled: bool) -> std::io::Result<()> {
    ...
    if kimix_config::user_kimix_home().is_none() {
        return Err(Error::new(
            ErrorKind::NotFound,
            "no user kimix home resolved; refusing to write a cwd-relative pager.toml \
