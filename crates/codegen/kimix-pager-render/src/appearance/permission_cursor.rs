...
use kimix_workspace::permission::is_enable_always_approve_option;
...
///
/// 1. `KIMIX_DEFAULT_SELECTED_PERMISSION` env var (headless / agent testing —
...
pub fn load_default_selected_permission() -> DefaultSelectedPermission {
    CONFIG_LOADED.with(|loaded| {
        if !loaded.get() {
            let resolved = std::env::var("KIMIX_DEFAULT_SELECTED_PERMISSION")
                .ok()
...
mod tests {
    use super::*;
    use kimix_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
