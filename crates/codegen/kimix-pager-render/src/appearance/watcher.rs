...
//!
//! In dev mode, watches ~/.kimix/pager.toml for changes and hot-reloads.
...
///
/// In dev mode: reads from ~/.kimix/pager.toml, watches for changes.
...
impl ConfigWatcher {
    ...
    ///
    /// - In dev mode: reads/creates ~/.kimix/pager.toml, watches for changes
    ...
    fn start_static() -> io::Result<Self> {
        let config = kimix_config::user_kimix_home()
