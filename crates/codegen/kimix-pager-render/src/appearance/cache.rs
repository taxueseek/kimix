...
//!
//! Disk writes live in `kimix_shell::util::config::set_<field>()`.
...
use kimix_shared::ui_config::UiConfig;
...
/// Default ON when unset. The `KIMIX_PROMPT_SUGGESTIONS` env var overrides
...
/// `KIMIX_SCROLL_MODE` overrides `[ui].scroll_mode` (the `KIMIX_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("KIMIX_SCROLL_MODE")
                .ok()
...
/// `KIMIX_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("KIMIX_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `KIMIX_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("KIMIX_SCROLL_LINES")
                .ok()
...
fn load_bool_from_effective_config(key: &str, default: bool) -> bool {
    let root = match kimix_config::load_effective_config_disk_only() {
        Ok(r) => r,
