...
///
/// `KIMIX_FORCE_LEGACY_CONSOLE=1` (or `true`) forces this on regardless of
...
/// Read the `KIMIX_FORCE_LEGACY_CONSOLE` escape hatch from the environment.
fn forced_legacy_console_override() -> Option<bool> {
    parse_forced_legacy_console(std::env::var("KIMIX_FORCE_LEGACY_CONSOLE").ok().as_deref())
}
