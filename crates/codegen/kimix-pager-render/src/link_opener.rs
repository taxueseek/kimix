...
pub fn open_url(url: &str) {
    ...
    if let Ok(path) = std::env::var("KIMIX_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "KIMIX_TEST_OPEN_URL_FILE write failed");
        }
        return;
    }
    ...
    kimix_tools::util::detach_std_command(&mut command);
...
///
/// The returned command is TTY-guarded via [`kimix_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    kimix_tty_utils::detach_std_command(&mut command);
    command
}
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/kimix session/image 1.jpg");
