...
use kimix_ratatui_textarea::ElementId;
...
pub fn decode_image_dimensions(bytes: &[u8]) -> Option<(u32, u32)> {
    ...
    kimix_tools::util::image_validate::validate_image_bytes_unrestricted(bytes, false)
        .ok()
        .map(|(w, h, _)| (w, h))
}
...
impl ImageViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        let mime_type = kimix_shared::clipboard::mime_from_bytes(&bytes).to_owned();
...
impl VideoViewerState {
    ...
    pub fn open_from_path(path: &std::path::Path) -> Option<Self> {
        ...
        kimix_tty_utils::detach_std_command(&mut ffmpeg_cmd);
...
use kimix_shared::clipboard::mime_to_extension;
...
///
/// Delegates to [`kimix_shared::clipboard::mime_to_extension`].
...
fn read_image_at_path(path: &std::path::Path) -> Option<PastedImage> {
    ...
    let mime_type = kimix_shared::clipboard::mime_from_bytes(&data);
...
pub fn session_images_dir(
    ...
    let sid = session_id?;
    let info = kimix_shared::session::info::Info {
        id: sid.clone(),
        cwd: cwd.to_string_lossy().into_owned(),
    };
    Some(kimix_shared::session::session_dir(&info).join("images"))
}
...
pub fn session_mermaid_dir(
    ...
    Some(kimix_shared::session::session_dir(&info).join("mermaid"))
}
...
///   [`kimix_shell::session::placeholder_images::load_placeholder_image`]
...
pub fn build_content_blocks_with_workspace(
    ...
    let allowed: Option<Vec<std::path::PathBuf>> =
        workspace_cwd.map(kimix_shared::placeholder_images::default_allowed_prefixes);
...
pub fn build_content_blocks_with_prefixes(
    ...
    build_content_blocks_with_prefixes_and_caps(
        ...
        images,
        allowed_prefixes,
        kimix_shared::placeholder_images::MAX_PLACEHOLDER_AGGREGATE_BYTES,
    )
}
...
/// [`kimix_shell::session::placeholder_images::recover_orphan_placeholders_with_prefixes_and_caps`].
...
pub fn build_content_blocks_with_prefixes_and_caps(
    ...
    // attachment.
    let rewritten_text =
        kimix_shared::placeholder_images::strip_paths_from_image_placeholders(rewritten_text);
    ...
    for img in &images {
        ...
        blocks.push(ContentBlock::Image(
            ImageContent::new(data, mime_type)
                ...
                .meta(Some(kimix_shared::placeholder_images::display_number_meta(
                    img.display_number,
                ))),
        ));
    }
...
fn resolve_orphan_placeholders(
    ...
    let placeholders = kimix_shared::placeholder_images::extract_placeholders(&text);
    ...
    for ph in &placeholders {
        ...
        match kimix_shared::placeholder_images::load_placeholder_image(&ph.path, allowed) {
            Ok(loaded) => {
...
mod tests {
    ...
    fn file_url_single_slash_rejected_at_anchor_gate() {
        ...
        // URL parsing runs.
        let pasted = "file:/tmp/should_not_exist_abc_kimix_pager.png";
    ...
    fn dropped_path_nonexistent_bare_path_not_intercepted() {
        ...
        let entries = dropped_paths("/tmp/definitely_does_not_exist_xyz_kimix_pager.txt");
        assert!(
            entries.is_empty(),
    ...
    fn build_blocks_sets_display_number_meta() {
        ...
        assert_eq!(
            kimix_shared::placeholder_images::display_number_from_meta(ic.meta.as_ref()),
            Some(3),
    ...
    fn build_blocks_uri_prefers_durable_session_path() {
        ...
        img.session_image_path = Some(PathBuf::from("/Users/test/.kimix/session/image.png"));
        let blocks = build_blocks_no_workspace("text".into(), vec![img]);
        assert_eq!(blocks.len(), 2);
        if let agent_client_protocol::ContentBlock::Image(ic) = &blocks[1] {
            assert_eq!(
                ic.uri.as_deref(),
                Some("file:///Users/test/.kimix/session/image.png"),
