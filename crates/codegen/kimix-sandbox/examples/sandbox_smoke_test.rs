...
//! cargo run -p Kimix-sandbox --example sandbox_smoke_test
//!

//! # Test strict profile
//! cargo run -p Kimix-sandbox --example sandbox_smoke_test -- strict
//!

//! # Test read-only profile
//! cargo run -p Kimix-sandbox --example sandbox_smoke_test -- read-only
//! ```
use kimix_sandbox::{ProfileName, SandboxManager};
use std::path::Path;
...
fn main() {
    ...
    let tmp_test = Path::new("/tmp/.kimix-sandbox-test");
