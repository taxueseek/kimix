...
//! `off`. Custom profiles via `~/.kimix/sandbox.toml` or `.Kimix/sandbox.toml`.
...
/// Load sandbox config from `~/.kimix/sandbox.toml` and `.Kimix/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.kimix/sandbox.toml
    let global_path = kimix_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.Kimix/sandbox.toml (additive only)
    let project_path = workspace.join(".kimix").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&kimix_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".kimix").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub fn to_capability_set_with_config(
        ...
        // directories like ~/.kimix/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.kimix/sandbox.toml or .kimix/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
