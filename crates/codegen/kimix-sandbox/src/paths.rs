...
// ── Kimix state directory ────────────────────────────────────────────────────

/// Kimix state directory — always writable (`$KIMIX_SHARE_DIR` or `~/.kimix`).
pub(crate) fn kimix_home() -> PathBuf {
...
pub(crate) fn essential_writable_paths(workspace: &Path) -> Vec<PathBuf> {
    let mut paths = vec![workspace.to_path_buf(), kimix_home()];
    paths.extend(temp_writable_paths());
    paths
}

/// Writable directory paths for the read-only profile (minimal: just ~/.kimix + temp).
/// Device files are handled separately via `allow_file` in `to_capability_set_with_config`.
#[cfg(all(feature = "enforce", unix))]
pub(crate) fn essential_writable_paths_minimal() -> Vec<PathBuf> {
    let mut paths = vec![kimix_home()];
    paths.extend(temp_writable_paths());
    paths
}
