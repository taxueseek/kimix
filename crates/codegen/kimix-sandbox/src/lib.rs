//! OS-level sandboxing for Kimix via macOS Seatbelt sandbox profiling.
//!
...
//!
//! ```rust,no_run
//! use kimix_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__KIMIX_INSIDE_BWRAP";
...
/// Zero-permission placeholder (file or dir) under `kimix_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent Kimix
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::kimix_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    /// Create a temp workspace whose `.Kimix/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("kimix-{tag}-{}-{nanos}", std::process::id()));
        let kimix = ws.join(".kimix");
        std::fs::create_dir_all(&kimix).unwrap();
        std::fs::write(kimix.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("kimix-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::kimix_home()
