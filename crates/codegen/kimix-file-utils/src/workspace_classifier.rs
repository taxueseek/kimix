...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".kimix", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
fn has_excluded_component(path: &Path) -> bool {
    for component in path.components() {
        if let std::path::Component::Normal(name) = component {
            ...
            if name_lower.starts_with(".kimix-") {
                return true;
            }
        }
    }
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn kimix_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".kimix")));
                assert!(!is_project_dir(&home.join(".kimix/bin")));
            }
        }

        #[test]
        fn kimix_prefixed_dirs_are_unsafe() {
            if let Some(home) = dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".kimix-proxy-work")));
            }
        }
