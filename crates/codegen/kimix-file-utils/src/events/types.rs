...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `Kimix-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `Kimix-shell::session::events`.
    ...
    /// discriminator constants in `Kimix-shell::session::events`.
    ...
    /// `Kimix-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `Kimix-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
pub enum InterjectionSource {
    /// Direct `Kimix/interject` while a turn was running (Ctrl+Enter).
    Direct,
...
pub enum RedirectKind {
    /// Mid-turn interjection — Ctrl+O / `Kimix/interject`, or "Send now" on a
...
/// Telemetry mirror of `Kimix-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `Kimix-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `Kimix-shell`'s `GoalPauseReason`. The two types
