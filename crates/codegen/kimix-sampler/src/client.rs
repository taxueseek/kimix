...
use kimix_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
/// Product identifier baked into User-Agent strings (PRD F3: `Kimix/{version}`).
const AGENT_PRODUCT: &str = "kimix";
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    ...
    let max_completion_tokens = headers
        .get("x-kimix-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // wire — the old xAI proxy's `x-Kimix-*` marker headers are gone
    ...
    /// gate that adds the opt-in `x-Kimix-doom-loop-check` request header, so
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_kimix_conv_id = request.x_kimix_conv_id.clone();
        let x_kimix_req_id = request.x_kimix_req_id.clone();
        let x_kimix_session_id = request.x_kimix_session_id.clone();
        let x_kimix_turn_idx = request.x_kimix_turn_idx.clone();
        let x_kimix_agent_id = request.x_kimix_agent_id.clone();
        ...
        let extra_tools = kimix_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_kimix_conv_id = x_kimix_conv_id;
        wrapper.x_kimix_req_id = x_kimix_req_id;
        wrapper.x_kimix_session_id = x_kimix_session_id;
        wrapper.x_kimix_turn_idx = x_kimix_turn_idx;
        wrapper.x_kimix_agent_id = x_kimix_agent_id;
...
mod tests {
    ...
    use kimix_sampling_types::types::ChatRequestMessage;
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the Kimix agent product.
        assert!(ua.starts_with("my-client kimix/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = kimix_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
    ...
    fn record_401_attribution_invokes_callback_with_extracted_bearer() {
        ...
        let calls = cb.invocations.lock().expect("lock poisoned");
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "kimix", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
