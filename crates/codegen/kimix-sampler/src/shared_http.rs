...
/// Kill switch: `KIMIX_SAMPLER_SHARED_CLIENT=0` (or `false`, any case)
...
fn sharing_disabled() -> bool {
    ...
    *DISABLED.get_or_init(|| {
        let disabled = match std::env::var("KIMIX_SAMPLER_SHARED_CLIENT") {
            Ok(v) => v == "0" || v.eq_ignore_ascii_case("false"),
            Err(_) => false,
        };
        if disabled {
            tracing::info!("sampler HTTP client sharing disabled via KIMIX_SAMPLER_SHARED_CLIENT");
        }
        disabled
    })
}
...
fn build_http_client() -> Result<reqwest::Client, reqwest::Error> {
    ...
    let pool_idle_timeout_secs: u64 = std::env::var("KIMIX_POOL_IDLE_TIMEOUT_SECS")
        ...
        .and_then(|v| v.parse().ok())
        .unwrap_or(90);
    let connect_timeout_secs: u64 = std::env::var("KIMIX_CONNECT_TIMEOUT_SECS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(10);
