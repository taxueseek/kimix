//! Kimix-sampler - Actor-based sampling layer for the Kimi inference APIs.
//!

//! This crate extracts the HTTP streaming + retry logic out of
//! `Kimix-shell`'s session actor into a standalone, reusable
//! component built on the same actor pattern as `Kimix-hunk-tracker`.
//!
