...
//! `kimix_sampling_types::SamplingConfig` so that the sampler crate
...
/// composing chat-state's `kimix_sampling_types::SamplingConfig`
...
pub struct SamplerConfig {
    ...
    /// Client identity for the User-Agent header (`Kimix/{version}` plus an
    /// optional origin product). The old xAI proxy's identity headers
    /// (`x-Kimix-client-identifier` / `-client-version` / `-deployment-id` /
    ...
    /// client itself sends the opt-in `x-Kimix-doom-loop-check` header on
