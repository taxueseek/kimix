...
pub fn stream_responses<'a>(
    ...
    async_stream::stream! {
        ...
        let mut chunk_timestamps: Vec<Instant> = Vec::with_capacity(256);
        ...
        loop {
            ...
            match event {
                ...
                ResponseStreamEvent::ResponseFailed(failed_event) => {
                    ...
                    // Map status code from error when available; fall back to 500.
        ...
        let mut items = kimix_sampling_types::response_to_conversation_items(response);
        kimix_sampling_types::inject_streaming_reasoning_fallback(&mut items, reasoning_acc);
...
mod tests {
    ...
    async fn doom_loop_collector_signals_land_on_completed_response() {
        use kimix_sampling_types::doom_loop::{
