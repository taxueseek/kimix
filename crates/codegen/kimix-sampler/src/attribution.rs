...
//! `Kimix-sampler` is intentionally decoupled from `Kimix-shell`
...
/// tools live in `Kimix-tools`
/// (`crates/codegen/Kimix-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `kimix_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `kimix_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `Kimix-shell/src/auth/manager.rs` to match, otherwise the
