...
use kimix_sampler::SamplingClient;
use kimix_test_support::spawn_counting_server;
use support::{send_one, test_config};

/// Pin the env these assertions depend on before any client is built, so
/// ambient shell exports (`KIMIX_SAMPLER_SHARED_CLIENT=0`,
/// `KIMIX_POOL_MAX_IDLE=0`) cannot flip the expected pooling behavior.
fn pin_env() {
    ...
    PIN.call_once(|| {
        ...
        unsafe {
            std::env::remove_var("KIMIX_SAMPLER_SHARED_CLIENT");
            std::env::set_var("KIMIX_POOL_MAX_IDLE", "2");
            std::env::set_var("KIMIX_POOL_IDLE_TIMEOUT_SECS", "90");
        }
    });
}
