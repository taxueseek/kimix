use kimix_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// UI 语言："auto"（跟随系统）/ "zh" / "en"。未设置等同 auto。
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `KIMIX_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A Kimix STT
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            language: None,
            fork_secondary_model: kimix_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
