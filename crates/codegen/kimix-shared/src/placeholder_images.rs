...
//! image bytes.
//!
//! Both the TUI ([`kimix_tui::prompt_images`]) and the server-side
...
//! [`kimix_tui::prompt_images::display_text`]. The shape of this
...
///   separator — see [`kimix_tui::prompt_images::display_text`].
...
/// [`kimix_tui::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    kimix_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// `kimix_tui::prompt_images::display_text`. A
