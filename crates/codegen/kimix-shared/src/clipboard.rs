...
//! `KIMIX_CLIPBOARD_NO_NATIVE_READ=1` disables the in-process read entirely
...
/// the `KIMIX_CLIPBOARD_NO_DATA_CONTROL` kill-switch env var is set. The kill
...
mod platform {
    ...
    /// path, and `None` when `KIMIX_CLIPBOARD_NO_NATIVE_READ` is set (kill
    ...
    pub(super) fn native_image_read() -> Option<super::ImageData> {
        if std::env::var_os("KIMIX_CLIPBOARD_NO_NATIVE_READ").is_some() {
            return None;
        }
        let _guard = PASTEBOARD_LOCK.lock();
        objc2::rc::autoreleasepool(|_| {
    ...
    fn attachments_probe_temp_paths() -> (std::path::PathBuf, std::path::PathBuf, std::path::PathBuf)
    {
        ...
        (
            temp_dir.join("kimix-clipboard-probe.png"),
            temp_dir.join("kimix-clipboard-probe.tiff"),
            temp_dir.join("kimix-clipboard-probe.jpg"),
        )
    }
...
mod platform {
    ...
    /// Memoized read of the `KIMIX_CLIPBOARD_NO_DATA_CONTROL` kill switch —
    ...
    fn data_control_kill_switch_set() -> bool {
        ...
        *SET.get_or_init(|| std::env::var_os("KIMIX_CLIPBOARD_NO_DATA_CONTROL").is_some())
    }

    /// True when the arboard leg must be skipped entirely: on Wayland the
    /// `KIMIX_CLIPBOARD_NO_DATA_CONTROL` kill switch has to stop arboard from
        ...
        LEASE
            .get_or_init(|| {
                if arboard_wayland_bypassed() {
                    tracing::debug!("arboard leg disabled (KIMIX_CLIPBOARD_NO_DATA_CONTROL)");
                    return None;
                }
    ...
    fn arboard_read_with_deadline<T: Send + 'static>(
        ...
        if arboard_wayland_bypassed() {
            anyhow::bail!("arboard leg disabled (KIMIX_CLIPBOARD_NO_DATA_CONTROL)");
        }
    ...
    pub(super) fn wayland_data_control_supported() -> bool {
        ...
        // switch (precedent: KIMIX_CLIPBOARD_NO_NATIVE_READ) and non-Wayland
...
mod tests {
    ...
    #[ignore]
    fn round_trip() {
        let sentinel = format!("kimix-clipboard-test-{}", std::process::id());
...
mod macos_probe_smoke {
    ...
    /// `cargo test -p Kimix-shared -- --ignored probe_smoke`.
    #[test]
