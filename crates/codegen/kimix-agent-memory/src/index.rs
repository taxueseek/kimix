...
impl MemoryIndex {
    ...
    /// a stale one).  Used by `kimix memory doctor` to detect stuck states.
    ...
    ///
    /// Used by `kimix memory doctor` to detect orphaned chunks (chunks whose
...
mod tests {
    ...
    fn test_open_or_create_uses_wal_on_local_fs() {
        ...
        if std::env::var("KIMIX_SQLITE_JOURNAL_MODE").is_ok() {
            return;
        }
        let tmp = TempDir::new().unwrap();
    ...
    /// 3. Run the same orphan-removal logic as `kimix memory reindex`:
    ...
    ///
    /// This proves that `kimix memory reindex`'s Phase 1 actually fixes the
    /// state that `kimix memory doctor` warns about.
    #[test]
    fn test_reindex_maintenance_removes_orphaned_chunks() {
        ...
        // Simulate `kimix memory reindex` Phase 1: compare indexed vs current.
    ...
    ///
    /// Verifies that `kimix memory reindex` Phase 0 correctly bails when a
    ...
    /// `kimix memory reindex` Phase 3 resets the stale reindex claim.
    ///
    /// Verifies that `release_claim()` clears `meta.reindex_claim` so that
    /// `kimix memory doctor` no longer reports a stale lock after reindex runs.
    #[test]
    fn test_reindex_maintenance_resets_stale_claim() {
        ...
        // Simulate `kimix memory reindex` Phase 3: release the claim.
        idx.release_claim();
