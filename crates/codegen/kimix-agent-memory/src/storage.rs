...
//! `~/.kimix/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
use std::path::{Path, PathBuf};

use kimix_tools::util::kimix_home::kimix_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `kimix_agent::config::MemoryScope` (agent memory dir).
...
/// `~/.kimix/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.kimix/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.kimix/memory/`
    global_dir: PathBuf,
    /// `~/.kimix/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.kimix/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| kimix_home().join("memory"));
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see kimix_sqlite_journal::JournalMode::open_readonly.
        kimix_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.kimix/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    pub fn ensure_initialized(&self) -> std::io::Result<()> {
        ...
        if !global_file.exists() {
            std::fs::write(
                ...
                "# Global Memory\n\
                 \n\
                 > This file is automatically managed by Kimix's memory system.\n\
