...
pub(super) fn build_middleware_client(
    ...
    kimix_http::with_auth_retry(kimix_http::shared_client(), credentials)
}

fn build_static_middleware_client(
    api_key: Option<String>,
) -> reqwest_middleware::ClientWithMiddleware {
    let provider: std::sync::Arc<dyn kimix_auth::AuthCredentialProvider> = std::sync::Arc::new(
        kimix_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl kimix_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = kimix_http::shared_client()
                    .post(format!("{}/embeddings", self.api_base))
                    .json(&body_json)
                    .header("x-kimix-client-version", kimix_version::VERSION);
