...
pub enum ContentBlock {
    ...
    Thinking {
        thinking: String,
        /// Anthropic emits a signature for thinking blocks; some compatible
        /// proxies (e.g. LongCat) omit it on stream `content_block_start`.
        #[serde(default)]
        signature: String,
    },
    /// Catch-all for future block types (e.g. `server_tool_use`).
    /// Deserialized when the JSON `type` field doesn't match any known variant.
    #[serde(other)]
    Unknown,
}
...
pub struct MessagesUsage {
    /// Some Anthropic-compatible proxies (e.g. LongCat) emit `usage: {}` on
    /// `message_start` before token counts are known. Default to 0 so the
    /// stream is not aborted; later `message_delta` may still fill real counts.
    #[serde(default)]
...
#[cfg(test)]
mod longcat_compat_tests {
    use super::*;

    #[test]
    fn message_start_accepts_empty_usage_object() {
        // LongCat SSE: usage is {} on message_start
        let event: MessageStreamEvent = serde_json::from_str(
            r#"{"type":"message_start","message":{"id":"x","type":"message","role":"assistant","model":"LongCat-2.0","content":[],"stop_reason":null,"stop_sequence":null,"usage":{}}}"#,
        )
        .expect("empty usage should deserialize");
        match event {
    ...
    #[test]
    fn thinking_block_accepts_missing_signature() {
        let event: MessageStreamEvent = serde_json::from_str(
