...
pub enum HostedTool {
    ...
    /// Social/X search executed server-side by the backend's agentic sampler.
    /// Wire type is the Responses extension `{"type":"x_search"}` (not in
    /// stock OpenAI `rs::Tool`), so the sampler injects it as raw JSON after
    /// serialization. Vendor-neutral: any backend that accepts that type works.
    XSearch,
}
...
pub struct ConversationRequest {
    ...
    pub x_kimix_conv_id: Option<String>,
    pub x_kimix_req_id: Option<String>,
    pub x_kimix_session_id: Option<String>,
    pub x_kimix_turn_idx: Option<String>,
    pub x_kimix_agent_id: Option<String>,
    pub x_kimix_deployment_id: Option<String>,
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-Kimix-doom-loop-check` header). Empty when the
...
// Part of the Kimix Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/Kimix-compaction` operate over Kimix's
...
impl kimix_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> kimix_compaction::CompactionRole {
        use kimix_compaction::CompactionRole;
        // Kimix has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // Kimix has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing Kimix history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<kimix_compaction::CompactionFileRef> {
        // Kimix `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `KimixTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<kimix_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl kimix_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/Kimix-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`Kimix-shell`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/Kimix-sampler/src/stream/chat_completions.rs])
...
impl From<ConversationRequest> for ChatCompletionRequest {
    fn from(req: ConversationRequest) -> Self {
        ...
        ChatCompletionRequest {
            ...
            x_kimix_conv_id: req.x_kimix_conv_id,
            x_kimix_req_id: req.x_kimix_req_id,
            x_kimix_session_id: req.x_kimix_session_id,
            x_kimix_turn_idx: req.x_kimix_turn_idx,
...
impl From<&ConversationRequest> for rs::CreateResponse {
    fn from(req: &ConversationRequest) -> Self {
        ...
        rs::CreateResponse {
            ...
            prompt: None,
            prompt_cache_key: req.x_kimix_session_id.clone(),
...
fn build_responses_tools(req: &ConversationRequest) -> Vec<rs::Tool> {
    ...
    for hosted in &req.hosted_tools {
        match hosted {
            ...
            // XSearch uses a Responses extension type not in async_openai's
            // rs::Tool enum; injected as raw JSON after serialization.
            HostedTool::XSearch => {}
        }
    }
...
/// Return raw JSON tool definitions for Responses-extension hosted tools that
/// cannot be represented by `async_openai`'s `rs::Tool` enum.
///
/// The sampler client injects these into the serialized request body's
/// `tools` array before sending to the API. No vendor hostname checks.
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_kimix_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_kimix_req_id = Some(req_id.into());
        self
    }
...
pub fn build_messages_request(req: &ConversationRequest) -> crate::messages::MessagesRequest {
    ...
    // output without tools. The Kimix-shell agent does NOT use this path — a
...
/// ([crates/codegen/Kimix-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use kimix_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // Kimix has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_conversation_item_roundtrip() {
        ...
        let assistant = ConversationItem::assistant_with_model("Hi there!", "kimix-3");
    ...
    fn test_conversation_request_to_chat_completion() {
        ...
        assert_eq!(chat_req.model, Some("kimix-3".to_string()));
    ...
    fn test_conversation_request_to_responses_api() {
        ...
        assert_eq!(responses_req.model, Some("kimix-3".to_string()));
    ...
    fn test_responses_api_response_to_conversation_item() {
        ...
        assert_eq!(a.model_id, Some("kimix-3".to_string()));
        assert_eq!(
    ...
    fn test_assistant_with_content_and_tool_calls() {
        ...
        let assistant = AssistantItem {
            ...
            model_id: Some("kimix-3".to_string()),
            model_fingerprint: None,
            reasoning_effort: None,
        };
        ...
        assert_eq!(chat_msg.tool_calls.len(), 1);
        assert_eq!(chat_msg.model_id, Some("kimix-3".to_string()));
    }
    ...
    #[test]
    fn test_responses_request_uses_session_id_as_prompt_cache_key() {
        let req = ConversationRequest {
            x_kimix_session_id: Some("session-123".to_string()),
            ..Default::default()
        };

        let responses_req: crate::rs::CreateResponse = (&req).into();
        assert_eq!(
            responses_req.prompt_cache_key.as_deref(),
            Some("session-123")
        );
    }
    ...
    fn test_btw_stripped_reasoning_produces_no_thinking_blocks() {
        ...
        // `strip_reasoning_blocks` in Kimix-chat-state. Here the assistant
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.kimix/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.kimix/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.kimix/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("kimix-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_kimix_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_kimix_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        .with_model_id("kimix-3");

        assert_eq!(item.model_id, Some("kimix-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("kimix-3");
        ...
        let user = ConversationItem::user("Hi").with_model_id("kimix-3");
    ...
    fn responses_api_conversion_preserves_model_fingerprint() {
        ...
        assert_matches!(item, ConversationItem::Assistant(ref a) => {
            ...
            assert_eq!(a.model_id.as_deref(), Some("kimix-4.5"));
    ...
    //      (earlier Kimix / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_kimix_shape() {
