...
// Re-export ToolDefinition and FunctionTool from Kimix-tools.
...
pub use kimix_tools::types::definition::{FunctionTool, ToolDefinition};
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "kimix" }).as_object().unwrap()))
                .is_none()
        );
    }
