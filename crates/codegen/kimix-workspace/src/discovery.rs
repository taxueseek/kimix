...
//! `Kimix-agent` and `Kimix-tools` rather than duplicating them.
...
//! Re-exports [`AgentsMdTracker`] from `Kimix-tools` for external
...
// workspace crate without a direct Kimix-tools dependency.
pub use kimix_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use kimix_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use kimix_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use kimix_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`kimix_agent::prompt::skills::list_skills`] with
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = kimix_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
    ...
    files
        .into_iter()
        .map(|mut file| {
            // Strip rules-file YAML frontmatter so it does not leak as raw YAML (matches Kimix render).
            if file.file_path.contains("/.kimix/rules/")
                || file.file_path.contains("/.claude/rules/")
            {
                file.content =
                    kimix_tools::implementations::skills::skill::extract_skill_body(&file.content);
            }
            file
        })
...
///
/// Delegates to [`kimix_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = kimix_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
/// Load the project config from `<root_cwd>/.Kimix/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".kimix").join("config.toml");
    match kimix_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.kimix/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".kimix").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".kimix").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes Kimix's AgentConfigFile and the remote
        ...
        // break discovery.
        let src = kimix_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: kimix_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // Discovery also scans the real `~/.kimix`, so fixtures use test-unique names.
    #[tokio::test]
    async fn discover_agents_md_strips_rules_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let rules_dir = tmp.path().join(".kimix").join("rules");
        ...
        let rule = files
            ...
            .find(|f| {
                f["file_path"]
                    .as_str()
                    .is_some_and(|p| p.ends_with("/.kimix/rules/xyzzy-discover-agents-md-test.md"))
            })
    ...
    // from `~/.kimix/plugins/`, so tests check for specific plugins.
    ...
    fn discover_plugins_json_has_expected_fields() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".kimix").join("plugins").join("field-test");
        fs::create_dir_all(plugins_dir.join("skills")).unwrap();
        fs::write(
            plugins_dir.join("plugin.json"),
    ...
    // developer machine with Kimix installed it may return non-Null even
