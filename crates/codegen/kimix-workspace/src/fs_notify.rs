...
//! FsNotify adapter functions bridging [`kimix_fsnotify`] events to
...
//! sent to the [`IndexManagerHandle`](kimix_codebase_graph::IndexManagerHandle).
//! The [`refresh_codebase_graph_after_head_change`] helper handles
//! git HEAD changes by diffing `ORIG_HEAD..HEAD`.
use std::path::{Path, PathBuf};

use kimix_fsnotify::{FsEvent, FsEventKind};
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> kimix_codebase_graph::FileEvent {
    use kimix_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`kimix_fsnotify::FsEventKind`] to the wire-type
/// [`kimix_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> kimix_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => kimix_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => kimix_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => kimix_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => kimix_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => kimix_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](kimix_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<kimix_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
    cancel: tokio_util::sync::CancellationToken,
    codebase_index: Option<std::sync::Arc<kimix_codebase_graph::IndexManagerHandle>>,
) {
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    kimix_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<kimix_codebase_graph::FileEvent> {
    let mut parts = line.splitn(3, '\t');
    let status = parts.next()?.trim();
    let path = parts.next()?;

    match status.chars().next()? {
        'A' => Some(kimix_codebase_graph::FileEvent::created(
            repo_root.join(path),
        )),
        'D' => Some(kimix_codebase_graph::FileEvent::removed(
            repo_root.join(path),
        )),
        'R' | 'C' => {
            let new_path = parts.next()?;
            Some(kimix_codebase_graph::FileEvent::renamed(
                repo_root.join(path),
                repo_root.join(new_path),
            ))
        }
        _ => Some(kimix_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &kimix_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<kimix_workspace_types::WorkspaceEvent>,
) {
    ...
    kimix_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(kimix_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            kimix_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: kimix_workspace_types::FsEventKind,
) -> kimix_codebase_graph::FileEvent {
    use kimix_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        kimix_workspace_types::FsEventKind::Created => FileEventKind::Created,
        kimix_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        kimix_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        kimix_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.kimix/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use kimix_workspace_types::FsEventKind as WsKind;
        assert_eq!(
    ...
    fn codebase_graph_event_mapping() {
        use kimix_codebase_graph::FileEventKind;
