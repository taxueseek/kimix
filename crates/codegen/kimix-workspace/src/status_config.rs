...
//!
//! All values are read once at startup from `KIMIX_WORKSPACE_*` environment
...
// documented fallbacks for each `KIMIX_WORKSPACE_*` env var.
...
pub struct StatusConfig {
    ...
    /// Legacy single-phase drain timeout (`KIMIX_WORKSPACE_DRAIN_TIMEOUT_SECS`),
    /// retained for compatibility; the SIGTERM and server-evict paths now use the
    /// two-phase drain bounded by `KIMIX_WORKSPACE_TERMINATION_GRACE_MS`.
    ...
    /// Opt-in foreground-only idle (`KIMIX_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS`);
    ...
    /// (`KIMIX_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS`).
    pub preview_activity_window: Duration,
    /// Cadence at which the preview-activity scraper polls the proxy
    /// (`KIMIX_WORKSPACE_PREVIEW_ACTIVITY_SCRAPE_INTERVAL_MS`); kept strictly
    ...
    /// injects `KIMIX_SESSION_RESTORED=true`; a first boot never does.
    pub session_restored: bool,
    /// True when restore injects `KIMIX_REVIVE_SCRIPT_CONFIGURED=true` (launchable
...
impl StatusConfig {
    /// Populate from `KIMIX_WORKSPACE_*`. Unset or unparseable vars fall
    /// back to the default with a `warn!`. Never fails.
    pub fn from_env() -> Self {
        let defaults = Self::default();
        let (agent_rpc, agent_connect) = Self::agent_timeouts_from_env();
        let mut cfg = Self {
            heartbeat: secs_or("KIMIX_WORKSPACE_HEARTBEAT_SECS", defaults.heartbeat),
            keepalive: secs_or("KIMIX_WORKSPACE_KEEPALIVE_SECS", defaults.keepalive),
            ws_ping: secs_or("KIMIX_WORKSPACE_WS_PING_SECS", defaults.ws_ping),
            ws_reconnect_backoff: backoff_schedule_from_env(
                "KIMIX_WORKSPACE_WS_RECONNECT_BACKOFF_MS",
            ),
            hub_warn_threshold: parse_or(
                "KIMIX_WORKSPACE_HUB_WARN_THRESHOLD",
                defaults.hub_warn_threshold,
            ),
            hub_backoff_base: ms_or(
                "KIMIX_WORKSPACE_HUB_BACKOFF_BASE_MS",
                defaults.hub_backoff_base,
            ),
            session_idle_prune: secs_or(
                "KIMIX_WORKSPACE_SESSION_IDLE_PRUNE_SECS",
                defaults.session_idle_prune,
            ),
            drain_timeout: secs_or("KIMIX_WORKSPACE_DRAIN_TIMEOUT_SECS", defaults.drain_timeout),
            agent_rpc_timeout: agent_rpc,
            agent_connect_timeout: agent_connect,
            idle_ignores_background: parse_or(
                "KIMIX_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS",
            ...
            session_restored: std::env::var("KIMIX_SESSION_RESTORED").as_deref() == Ok("true"),
            revive_script_configured: std::env::var("KIMIX_REVIVE_SCRIPT_CONFIGURED").as_deref()
                == Ok("true"),
        };
        cfg.validate();
        cfg
    ...
    pub fn agent_timeouts_from_env() -> (Duration, Duration) {
        let defaults = Self::default();
        const RPC_VAR: &str = "KIMIX_WORKSPACE_AGENT_RPC_TIMEOUT_SECS";
        const CONNECT_VAR: &str = "KIMIX_WORKSPACE_AGENT_CONNECT_TIMEOUT_SECS";
        (
            nonzero_secs_or(
                RPC_VAR,
    ...
    fn validate(&mut self) {
        if self.keepalive <= self.heartbeat {
            tracing::warn!(
                ...
                "KIMIX_WORKSPACE keepalive <= heartbeat; transport may time out between heartbeats"
            );
        }
        ...
        if window != self.preview_activity_window || scrape != self.preview_activity_scrape_interval
        {
            tracing::warn!(
                ...
                "KIMIX_WORKSPACE preview scrape interval/window out of range; clamped to 1ms <= scrape < window"
            );
...
fn parse_or<T: FromStr>(var: &str, default: T) -> T {
    match std::env::var(var) {
        ...
        Ok(raw) => match raw.parse::<T>() {
            Ok(value) => value,
            Err(_) => {
                tracing::warn!(var, value = %raw, "Unparseable KIMIX_WORKSPACE value; using default");
                default
            }
        },
    }
...
fn nonzero_secs_or(var: &str, secs: u64, default: Duration) -> Duration {
    if secs == 0 {
        tracing::warn!(
            var,
            default = ?default,
            "KIMIX_WORKSPACE agent timeout of 0s is invalid; using default"
        );
        return default;
    }
        ...
        match part.parse::<u64>() {
            ...
            Err(_) => {
                tracing::warn!(
                    var,
                    value = %raw,
                    "Unparseable KIMIX_WORKSPACE backoff schedule; using SDK default"
                );
                return None;
            }
        }
...
mod tests {
    ...
    fn parse_or_unset_returns_default() {
        ...
        let var = "KIMIX_WORKSPACE_TEST_PARSE_OR_UNSET";
    ...
    fn parse_or_valid_parses() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_PARSE_OR_VALID";
    ...
    fn parse_or_invalid_falls_back_without_panic() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_PARSE_OR_INVALID";
    ...
    fn secs_or_parses_into_duration() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_SECS_OR_VALID";
    ...
    fn secs_or_unset_returns_default() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_SECS_OR_UNSET";
        unsafe { std::env::remove_var(var) };
        assert_eq!(
            secs_or(var, Duration::from_secs(30)),
    ...
    fn secs_or_invalid_falls_back_without_panic() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_SECS_OR_INVALID";
    ...
    fn ms_or_parses_into_duration() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_MS_OR_VALID";
    ...
    fn ms_or_invalid_falls_back_without_panic() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_MS_OR_INVALID";
    ...
    /// With none of the `KIMIX_WORKSPACE_*` vars set, `from_env` reproduces
    ...
    fn from_env_clean_matches_default() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        for var in [
            "KIMIX_WORKSPACE_HEARTBEAT_SECS",
            "KIMIX_WORKSPACE_KEEPALIVE_SECS",
            "KIMIX_WORKSPACE_WS_PING_SECS",
            "KIMIX_WORKSPACE_WS_RECONNECT_BACKOFF_MS",
            "KIMIX_WORKSPACE_HUB_WARN_THRESHOLD",
            "KIMIX_WORKSPACE_HUB_BACKOFF_BASE_MS",
            "KIMIX_WORKSPACE_SESSION_IDLE_PRUNE_SECS",
            "KIMIX_WORKSPACE_DRAIN_TIMEOUT_SECS",
            "KIMIX_WORKSPACE_AGENT_RPC_TIMEOUT_SECS",
            "KIMIX_WORKSPACE_AGENT_CONNECT_TIMEOUT_SECS",
            "KIMIX_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS",
            "KIMIX_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS",
            "KIMIX_WORKSPACE_PREVIEW_ACTIVITY_SCRAPE_INTERVAL_MS",
            "KIMIX_SESSION_RESTORED",
            "KIMIX_REVIVE_SCRIPT_CONFIGURED",
        ] {
            unsafe { std::env::remove_var(var) };
        }
    ...
    fn from_env_reads_session_restored_true_only() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("KIMIX_SESSION_RESTORED", "true") };
        let restored = StatusConfig::from_env().session_restored;
        unsafe { std::env::set_var("KIMIX_SESSION_RESTORED", "1") };
        let non_canonical = StatusConfig::from_env().session_restored;
        unsafe { std::env::remove_var("KIMIX_SESSION_RESTORED") };
        assert!(restored);
    ...
    fn from_env_reads_revive_script_configured_true_only() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("KIMIX_REVIVE_SCRIPT_CONFIGURED", "true") };
        let configured = StatusConfig::from_env().revive_script_configured;
        unsafe { std::env::set_var("KIMIX_REVIVE_SCRIPT_CONFIGURED", "1") };
        let non_canonical = StatusConfig::from_env().revive_script_configured;
        unsafe { std::env::remove_var("KIMIX_REVIVE_SCRIPT_CONFIGURED") };
        assert!(configured);
        assert!(!non_canonical);
    }
    ...
    fn from_env_reads_idle_ignore_background_true() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("KIMIX_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS", "true") };
        let cfg = StatusConfig::from_env();
        unsafe { std::env::remove_var("KIMIX_WORKSPACE_IDLE_IGNORE_BACKGROUND_TASKS") };
        assert!(cfg.idle_ignores_background);
    }
    ...
    fn from_env_reads_preview_activity_window() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("KIMIX_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS", "120000") };
        let cfg = StatusConfig::from_env();
        unsafe { std::env::remove_var("KIMIX_WORKSPACE_PREVIEW_ACTIVITY_WINDOW_MS") };
    ...
    fn backoff_schedule_unset_returns_none() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_BACKOFF_UNSET";
        unsafe { std::env::remove_var(var) };
    ...
    fn backoff_schedule_valid_list_parses_in_order() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_BACKOFF_VALID";
    ...
    fn backoff_schedule_malformed_returns_none() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let var = "KIMIX_WORKSPACE_TEST_BACKOFF_MALFORMED";
