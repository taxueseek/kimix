...
/// `~/.kimix/sessions/{cwd}/{session_id}/pasted/` so the model can `read_file`
...
fn session_scratch_root() -> PathBuf {
    std::env::temp_dir().join(format!("kimix-test-sessions-{}", std::process::id()))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_kimix_render_embedded_resource_uses_file_contents_tag() {
        let _info = test_info("kimix-text");
        let resource = EmbeddedResource::new(EmbeddedResourceResource::TextResourceContents(
            agent_client_protocol::TextResourceContents::new(
                "const x = 1;\nconst y = 2;\n",
    ...
    #[tokio::test]
    async fn test_kimix_render_embedded_resource_full_file_uses_is_full_file() {
        let _info = test_info("kimix-full-file");
        let resource = EmbeddedResource::new(EmbeddedResourceResource::TextResourceContents(
            agent_client_protocol::TextResourceContents::new(
                "fn main() {}\n",
