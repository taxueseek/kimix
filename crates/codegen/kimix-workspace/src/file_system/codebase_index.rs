...
use kimix_codebase_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use kimix_tools::util::kimix_home::kimix_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.kimix/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `Kimix/code/status` reporting `reason: notStarted`.
    ...
    /// In `Kimix/code/status`, `resolve_index_handle()` calls
