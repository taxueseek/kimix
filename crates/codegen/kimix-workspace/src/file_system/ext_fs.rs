...
//! for the shell's `Kimix/fs/*` ACP extension methods.
//!

//! These mirror the pure functions that previously lived only in the
//! shell (`Kimix-shell/src/session/file_system.rs`) so that, in proxy
//! mode, a `Kimix/fs/*` request executes on the *remote* workspace server
...
use kimix_workspace_types::rpc::fs::FsReadEncoding;
