//! AcpFsAdapter: implements `Kimix-tools::AsyncFileSystem` using ACP gateway calls.
//!

//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates Kimix-tools' `AsyncFileSystem` trait into ACP protocol calls:
...
use kimix_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps Kimix-shell's ACP gateway to satisfy Kimix-tools' AsyncFileSystem.
///
