//! Permission resolution engine: merges native `.Kimix/config.toml`,
...
/// (fail-safe) with a warn + skip record for `kimix inspect`.
...
/// user `~/.kimix` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    kimix_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.Kimix/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.Kimix/config.toml` when there is no git repo).
///
/// Returned paths are ordered from repo root (lowest priority) to `cwd`
/// (highest priority), matching `Kimix-shell::config::find_project_configs`.
fn find_project_kimix_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".kimix").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".kimix").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
/// Load `[permission]` rules from native Kimix TOML config files:
///
///   * `~/.kimix/config.toml` (lowest priority)
///   * Each `.Kimix/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.kimix/config.toml` first (lowest priority within this layer).
    // Gated on user_kimix_home() so a project's .Kimix/config.toml is never read as
    // global permissions when neither KIMIX_SHARE_DIR nor a home dir resolves.
    if let Some(global_path) = kimix_config::user_kimix_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match kimix_config::load_config_file(&global_path) {
    ...
    for path in find_project_kimix_configs(cwd) {
        match kimix_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[kimix_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Kimix and Claude sources.
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `kimix inspect`.
...
impl ResolveInputs<'static> {
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(&kimix_config::managed_config_layers()),
        }
    }
}
...
/// bypass is pinned off via Kimix `requirements.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `kimix inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in find_claude_settings_paths(cwd) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `kimix inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `kimix inspect` via the outer resolver.
...
/// [`kimix_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = kimix_config::claude_managed_settings_path() else {
...
/// not consulted: Kimix must not inherit a host-wide always-approve lockdown from
/// that file. Kimix still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in Kimix use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = kimix_config::requirements_layers();
...
/// without `~/.kimix`); `path` only names the layer in a non-bool warning.
...
pub const MANAGED_MCP_PREFIX: &str = "kimix_com_";

/// Max `char` length of a managed runtime name (`kimix_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `kimix_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `KIMIX_SHARE_DIR`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // KIMIX_SHARE_DIR-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _marker_guard = EnvVarGuard::unset("_KIMIX_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate KIMIX_SHARE_DIR (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate KIMIX_SHARE_DIR so the claude-import marker reads clean (an
    ...
    fn parse_toml_non_array_compact_value_warns() {
        ...
        let out = String::from_utf8(buf.lock().expect("lock poisoned").clone()).unwrap();
    ...
    fn parse_mcp_entries_capturing_logs(
        ...
        let logs = String::from_utf8(buf.lock().expect("lock poisoned").clone()).unwrap();
        (entries, logs)
    }
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "kimix_com_foo"));
        assert!(mcp_name_matches("kimix_com_foo", "foo"));
        assert!(mcp_name_matches("kimix_com_foo", "kimix_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "kimix_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "kimix_com_slack"));
        assert!(mcp_name_matches("My Server", "kimix_com_my_server"));
        assert!(mcp_name_matches("kimix_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("kimix_com_", "kimix_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        let managed = http_named("kimix_com_foo", "https://foo.example.com/mcp");
        assert!(al.is_server_denied(&managed));
        assert!(!al.is_server_allowed(&managed));

        // Name match is transport-agnostic.
        let stdio = stdio_named("kimix_com_foo", "npx");
        ...
        for unrelated in ["foobar", "kimix_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("kimix_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("kimix_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("kimix_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "kimix_com_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("kimix_com_foo", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
        assert!(!al.is_server_denied(&http_named("kimix_com_foobar", "https://x.example.com/mcp")));
    }
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/Kimix/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/Kimix/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.kimix/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/Kimix/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.Kimix/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/Kimix/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/Kimix.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for kimix inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers = kimix_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            kimix_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
