...
///
/// ```bash
/// XAI_API_KEY='xai-some-key' cargo run --bin Kimix-tui
/// ```
///
...
///   highlighted_words: ["cargo", "run", "--bin", "Kimix-tui"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "XAI_API_KEY='xai-some-key' cargo run --bin kimix-tui";
        assert_eq!(
        ...
        let redirection_command = "cargo build --bin kimix-tui 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package kimix-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
