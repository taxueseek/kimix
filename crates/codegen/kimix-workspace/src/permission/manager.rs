...
use kimix_tools::implementations::kimix::web_fetch::{DomainMatcher, domain::normalize_domain};
...
fn permission_mode_artifact_str(mode: kimix_config_types::PermissionMode) -> &'static str {
    use kimix_config_types::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // Extract session ID for per-session permission isolation (BUG-002 fix)
        let session_id_str = Some(session_id.0.to_string());
        let mut state = load_state_from_disk(&cwd, client_id_ref, session_id_str.as_deref()).await;
        ...
        // persist it to ~/.kimix/sessions/<cwd>/permission.toml. This caused
        // the allow to survive full restarts (new Kimix process, new agent
            ...
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    let permission_mode = if yolo_mode {
                        kimix_config_types::PermissionMode::AlwaysApprove
                    } else if auto_mode {
                        kimix_config_types::PermissionMode::Auto
                    } else {
                        kimix_config_types::PermissionMode::Ask
                    };
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && kimix_sandbox::should_auto_allow_bash()
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        ...
                        _ => {
                            ...
                            let (decision, outcome_str) = match &prompt_outcome {
                                ...
                                PromptOutcome::AllowAlways => {
                                    // Fallback clients (Generic / KimixWeb /
...
mod tests {
    ...
    /// always-approve option is only offered for `KimixTUI | KimixPager | Desktop`).
    ...
    fn manager_with_recording_client_remember(
        ...
        let (gateway, receiver) = kimix_acp_lib::acp_gateway::<acp::AgentSide, _>(client);
    ...
    impl crate::permission::auto_mode::PermissionClassifier for ContextCapturingClassifier {
        fn classify<'a>(
            ...
            >,
        > {
            self.seen.lock().expect("lock poisoned").push(context);
            let v = self.verdict;
    ...
    async fn cancelled_and_error_prompts_record_no_decisions() {
        ...
        local
            .run_until(async {
                ...
                assert_eq!(d, Decision::Allow);
                assert!(
                    seen.lock().expect("lock poisoned")[0].turns.is_empty(),
                ...
                assert_eq!(d, Decision::Allow);
                assert!(
                    seen2.lock().expect("lock poisoned")[0].turns.is_empty(),
    ...
    async fn auto_classifier_block_prompt_still_offers_always_approve() {
        ...
        local
            .run_until(async {
                ...
                let client = RecordingClient::default();
                let prompts = client.prompts.clone();
                // KimixPager wires the always-approve option through to its YOLO
                // toggle; it is the option set the auto path prompts under.
                let (mgr, _e) =
                    manager_with_recording_client(&cwd, None, client, ClientType::KimixPager);
                mgr.set_auto_mode(true);
