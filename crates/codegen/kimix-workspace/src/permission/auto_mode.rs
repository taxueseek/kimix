...
//!
//! Port of common agent auto-permission classifier semantics adapted to Kimix's
...
impl HeuristicPermissionClassifier {
    pub fn classify_sync(
        ...
        // Interpreter commands with inline code flags (-c, -e, -m, <<) always
        // require explicit approval: they execute arbitrary code that cannot be
        // statically analyzed by the heuristic. This closes the bypass where
...
fn classify_bash(cmd: &str) -> ClassifierVerdict {
    ...
    // head and fail the check), else Block. NOTE: python/node/make are NOT in
    // ROUTINE_PREFIXES — they require explicit LLM classification or user approval.
    // Remaining project code-runners (cargo/pytest/cmake/bazel) execute
    // project-controlled code; this heuristic is a fail-closed FALLBACK and the
    // real safety boundary is the LLM side-query + managed policy.
...
/// mapped to Kimix access kinds + known names).
...
pub fn mcp_access_detail(name: &str, input: &serde_json::Value) -> String {
    ...
    kimix_tools::util::truncate_line(&format!("{name} {compact}"), MCP_ACCESS_DETAIL_MAX_LEN)
        .into_owned()
}
...
pub fn permission_decision_args(access: &AccessKind, access_detail: Option<&str>) -> String {
    ...
    kimix_tools::util::truncate_str_with_marker(&raw, CLASSIFIER_TURN_MAX_LEN).into_owned()
}
