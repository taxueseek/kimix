...
pub enum ClientType {
    ...
    Generic,
    /// Kimix TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "kimix-tui", alias = "kimix_tui")]
    KimixTUI,
    /// Kimix Web client - identified by clientIdentifier "Kimix-web"
    #[serde(rename = "kimix_web")]
    KimixWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "Kimix-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Kimix Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to KimixTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"Kimix-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"kimix_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "kimix-pager", alias = "kimix_pager")]
    KimixPager,
    /// Kimix Desktop (Electron) client - identified by clientIdentifier "Kimix-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "kimix_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header. The first-party clients
    /// (the bundled TUI/pager and the headless runner) all report `kimix`, so
    /// their User-Agent collapses to `Kimix/{version}` (PRD F3); the remaining
    /// labels identify external ACP clients.
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "kimix",
            Self::KimixTUI => "kimix-tui",
            Self::KimixWeb => "kimix-web",
            Self::Nebula => "nebula",
            Self::Extension => "kimix-code-extension",
            Self::KimixPager => "kimix",
            Self::Desktop => "kimix-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"Kimix-web"`, `"Kimix-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("kimix-web") => Self::KimixWeb,
            Some("nebula") => Self::Nebula,
            Some("kimix-code-extension") => Self::Extension,
            Some("kimix-desktop") => Self::Desktop,
            Some("kimix-pager") => Self::KimixPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::KimixTUI | Self::KimixPager => "tui",
            Self::KimixWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&kimix_tools::types::ToolInput> for AccessKind {
    fn from(input: &kimix_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.kimix/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use kimix_tools::implementations::kimix_hashline::edit::types::HashlineEditInput;
        use kimix_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use kimix_tools::implementations::kimix::bash::BashToolInput;
        use kimix_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use kimix_tools::implementations::use_tool::UseToolInput;
        use kimix_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use kimix_tools::implementations::kimix::monitor::types::MonitorInput;
        use kimix_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use kimix_tools::implementations::kimix::web_fetch::WebFetchInput;
        use kimix_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use kimix_tools::implementations::kimix::web_search::WebSearchInput;
        use kimix_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_kimix_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("kimix-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("kimix_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
