...
pub struct ClaudeSettings {
    ...
    /// Kimix-only root legacy). Recognized values: `acceptEdits`,
...
///     under `permissions.*`. Root-level keys are **Kimix legacy only** (not in
...
pub fn load_claude_settings(path: &Path) -> Option<ClaudeSettings> {
    ...
    // Canonical vendor settings store these under `permissions`; root is Kimix legacy only.
...
///
/// Root `defaultMode` is Kimix-only back-compat for older tests / hand-written
...
pub(crate) fn extract_default_mode(value: &serde_json::Value, path: &Path) -> Option<String> {
    ...
    // Nested key absent — optional Kimix legacy root.
        ...
        Some(dm) => match dm.as_str() {
            ...
            None => {
                warn!(
                    path = %path.display(),
                    actual_type = %dm.type_of(),
                    "root defaultMode (kimix legacy): expected string, ignoring"
                );
                None
            }
        },
...
fn extract_additional_directories(value: &serde_json::Value, path: &Path) -> Option<Vec<String>> {
    ...
    // when it is present but the wrong type, do *not* resurrect the Kimix-legacy
...
// permissions), consider extracting to a shared helper (e.g., in Kimix-hooks
...
// `Kimix-shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in Kimix-shell (inaccessible from here at runtime). They also
    // set this env var so the workspace-resident gate honours the override
    // without a cross-crate dependency.
    if std::env::var("_KIMIX_CLAUDE_MARKER_OVERRIDE").as_deref() == Ok("1") {
        return true;
    }
    let Some(config_path) = kimix_config::user_kimix_home().map(|g| g.join("config.toml")) else {
        return false;
    };
