...
use kimix_file_utils::events::{Event, EventWriter, PermissionDecision};
use kimix_tools::implementations::kimix::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell Kimix what to do differently";
...
///        `~/.kimix/config.toml` via the `Effect::PersistPermissionMode` effect
///     4. Sends the existing `Kimix/yolo_mode_changed` ACP notification so
...
/// (global yolo) option that is prepended for KimixTUI / KimixPager / Desktop.
///
...
/// (`KimixTUI`, `KimixPager`, `Desktop`) that wire the option id through
...
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::KimixTUI | ClientType::KimixPager | ClientType::Desktop
    )
}
...
/// Canonical definition lives in `kimix_workspace_types` (so both the
/// permission-validation layer and the MCP transport in `Kimix-mcp` can
...
/// reached `kimix_workspace::permission::MCP_TOOL_NAME_DELIMITER`.
...
/// `"list_issues"` → `"List Issues"`, `"kimix_com_notion"` →
/// `"Kimix Com Notion"`, and `"getMyTaskList"` → `"GetMyTaskList"`.
...
pub struct McpToolPermission {
    ...
    /// (e.g. `"kimix_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server segment (everything before the single `__` separator,
    /// e.g. `"kimix_com_notion"`). `None` if the tool name has no `__`,
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for KimixTUI - interactive selection with expandable/contractable terms
    ...
    fn build_options_inner(
        ...
        match access {
            ...
            AccessKind::Bash(bash_command) => {
                // For KimixTUI clients, use the fancy interactive options with term selection
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::KimixPager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::KimixPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::KimixWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::KimixPager, true);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::KimixPager);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::KimixTUI);
        let access = AccessKind::MCPTool {
            name: "linear__list".to_owned(),
            input: serde_json::Value::Null,
        };
    ...
    fn mcp_fallback_client_returns_plain_allow_always() {
        // non-TUI clients (Generic / KimixWeb / Extension / …) see `fallback_options`. The
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        assert_eq!(mcp_titleize_segment("kimix_com_notion"), "Kimix Com Notion");
    ...
    /// Same pin as above for `KimixTUI` and `Desktop` — both client types
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::KimixTUI, ClientType::Desktop] {
            let p = prompter(ct);
