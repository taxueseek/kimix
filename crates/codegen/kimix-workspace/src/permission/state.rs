...
use kimix_tools::util::kimix_home::kimix_home;
...
pub struct PermissionState {
    ...
    /// Exact MCP tool names (e.g. `"kimix_com_notion__notion-fetch"`)
    ...
    /// e.g. `"kimix_com_notion"`) for which the user has granted
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    kimix_config::sessions_cwd_dir(cwd.as_str())
}

/// Build the state directory for a specific session.
/// Sessions are isolated: each session gets its own permission state file
/// to prevent cross-session permission leakage (BUG-002 fix).
fn state_dir_for_session(cwd: &AbsPathBuf, session_id: &str) -> std::path::PathBuf {
    let sanitized = sanitize_client_id(session_id);
    state_dir_for_cwd(cwd).join("per_session").join(sanitized)
}
...
/// Load permission state from disk. When `session_id` is provided, uses a
/// session-specific path to prevent cross-session permission leakage.
/// Falls back to the legacy per-cwd path for backward compatibility.
pub(crate) async fn load_state_from_disk(
    cwd: &AbsPathBuf,
    client_identifier: Option<&str>,
    session_id: Option<&str>,
) -> PermissionState {
    // Prefer session-specific path when available
    if let Some(sid) = session_id {
        let session_dir = state_dir_for_session(cwd, sid);
        let state = load_state_from_dir(&session_dir, client_identifier).await;
        // If session-specific state exists, use it
        if !state.allowed_bash_commands.is_empty()
            || !state.allowed_web_fetch_domains.is_empty()
            || !state.allowed_mcp_tools.is_empty()
            || !state.allowed_mcp_servers.is_empty()
            || state.edit_policy != EditPolicy::Ask
        {
            return state;
        }
        // Otherwise fall back to legacy per-cwd path for backward compatibility
        let legacy = load_state_from_dir(&state_dir_for_cwd(cwd), client_identifier).await;
        return legacy;
    }
...
pub(crate) async fn persist_state(
    ...
    // Always write to session-specific path when session_id is available
    let dir = if let Some(sid) = session_id {
        state_dir_for_session(cwd, sid)
    } else {
...
pub async fn cleanup_stale_permission_state(max_age: std::time::Duration) {
    let sessions_dir = kimix_home().join("sessions");
...
mod tests {
    ...
    fn roundtrip_with_allowed_mcp_tools() {
        ...
        state
            .allowed_mcp_tools
            .insert("kimix_com_notion__notion-fetch".to_string());
        state
            .allowed_mcp_tools
        ...
        assert!(
            restored
                .allowed_mcp_tools
                .contains("kimix_com_notion__notion-fetch")
        );
    ...
    fn roundtrip_with_allowed_mcp_servers() {
        ...
        state
            .allowed_mcp_servers
            .insert("kimix_com_slack".to_string());
        ...
        assert!(restored.allowed_mcp_servers.contains("kimix_com_slack"));
...
"#;
    ...
    async fn persist_and_load_roundtrip() {
        ...
        // We can't easily override kimix_home(), so instead test
