...
//! [`crate::trust::TrustStore`] (`~/.kimix/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `Kimix-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `Kimix-shell`
//! concern, documented there.)
use std::io::IsTerminal;
use std::path::Path;

use kimix_config_types::{BoolFlag, RemoteSettings};
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.kimix`).
...
/// this binary — true on a local/dev build (no `KIMIX_VERSION` release stamp).
///
/// THE single security short-circuit: every explicit trust auto-grant site calls
/// this (greppable via `folder_trust_inert`). When true a self-built Kimix never
...
/// (`KIMIX_VERSION` unset at compile time) — i.e. a local/dev build.
///
/// Kept local (not in `Kimix-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned KIMIX_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(kimix_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("KIMIX_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `KIMIX_VERSION` release stamp) the feature is OFF
/// regardless of env/config/remote — a self-built Kimix auto-trusts (never
...
/// env `KIMIX_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit KIMIX_FOLDER_TRUST/config
    // opt-in is ignored here so a self-built Kimix never prompts.
    if is_local_build {
        return false;
    }
    ...
    let user = kimix_config::load_from_disk().ok();
    let managed = kimix_config::load_managed_config().ok();
    BoolFlag::env("KIMIX_FOLDER_TRUST")
...
/// same `~/.kimix/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `.claude` / `.Kimix/agents` / subdir-launch gaps. NOT itself the trust gate.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = kimix_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.Kimix/config.toml` declaring repo-controlled code-exec: a
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = kimix_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.Kimix/lsp.json`.
    if cwd.join(".kimix").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.Kimix/hooks/evil.json`) would
    ...
    if hook_root.join(".kimix").join("hooks").is_dir()
    ...
    // that must be gated — else a plugin clone (e.g. `.Kimix/plugins/evil/`, even
    // one in a subdir launched via `cd sub && Kimix`) would resolve trusted and
    ...
    if !kimix_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.Kimix/agents` / `.claude/agents`): a project agent
    ...
    if !kimix_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .kimix/lsp.json / hooks) \
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_kimix_config_mcp_servers() {
        let tmp = repo_tmp();
    ...
    #[test]
    fn repo_configs_present_detects_kimix_lsp_json() {
        let tmp = repo_tmp();
        let kimix = tmp.path().join(".kimix");
        std::fs::create_dir_all(&kimix).unwrap();
        std::fs::write(kimix.join("lsp.json"), "{}").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.Kimix/agents`-only clone must be gated: a project agent definition
        // can carry an inline `hooks:` block (code-exec) and can shadow a built-in
        // subagent by name.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".kimix").join("agents")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".kimix").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".kimix").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_configs_present_false_for_empty_mcp_servers_table() {
        ...
        let kimix = tmp.path().join(".kimix");
        std::fs::create_dir_all(&kimix).unwrap();
        std::fs::write(kimix.join("config.toml"), "[mcp_servers]\n").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_kimix_config_plugins_paths() {
    ...
    fn repo_config_kinds_matches_gate_and_reports_all_kinds() {
        ...
        // `.Kimix/agents` — even when launched from a SUBDIR (the cwd→git-root walk
        // that `first_only` shares). Guards against silent drift between the two.
        let tmp = repo_tmp();
        let kimix = tmp.path().join(".kimix");
        std::fs::create_dir_all(kimix.join("agents")).unwrap();
        std::fs::write(kimix.join("config.toml"), "[plugins]\npaths = [\"./x\"]\n").unwrap();
    ...
    // KIMIX_SHARE_DIR-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`Kimix-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(kimix_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvVarGuard::unset("KIMIX_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty KIMIX_SHARE_DIR (no
        // config.toml/managed_config.toml) + KIMIX_FOLDER_TRUST unset. nextest's
        // process-per-test makes kimix_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvVarGuard::unset("KIMIX_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // KIMIX_FOLDER_TRUST=1 does NOT enable the feature (so a self-built Kimix
        // never prompts). KIMIX_SHARE_DIR isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvVarGuard::set("KIMIX_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty KIMIX_SHARE_DIR (no config.toml/managed config) +
        // KIMIX_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _flag = EnvVarGuard::unset("KIMIX_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned KIMIX_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(kimix_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no KIMIX_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(kimix_version::TEST_VERSION_ENV);
        if option_env!("KIMIX_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); KIMIX_SHARE_DIR-isolated
        // and ENV_LOCK-serialized so toggling KIMIX_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _unset = EnvVarGuard::unset(kimix_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // KIMIX_SHARE_DIR-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // KIMIX_SHARE_DIR-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("KIMIX_SHARE_DIR", home.path());
        let _sim = simulate_release_build();
