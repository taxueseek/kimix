...
//! Moved from `Kimix-shell/src/session/worktree.rs` into the workspace
...
use kimix_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
// Canonical in Kimix-workspace-types; re-exported for existing paths.
pub use kimix_workspace_types::rpc::worktree::{
...
pub(crate) fn to_creation_mode(t: WorktreeType) -> kimix_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => kimix_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => kimix_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => kimix_fast_worktree::CreationMode::GitCheckout,
    }
}
...
/// Resolve the Kimix home for worktree paths via the **same** resolver used for
/// `worktrees.db` (`kimix_fast_worktree::resolve_kimix_home`), so checkout dirs and
/// the metadata DB always live under the same `.Kimix` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `kimix_config::kimix_home()`,
/// so worktree paths also agree with trust/hooks and other Kimix-home paths.
fn kimix_home() -> std::path::PathBuf {
    kimix_fast_worktree::resolve_kimix_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
/// Returns `~/.kimix/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    kimix_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.kimix/worktrees/<repo_name>`)
/// for a given source path, correctly handling Kimix-managed worktrees.
///
/// When `source_path` is already under `~/.kimix/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the Kimix worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = kimix_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.kimix/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the Kimix-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.kimix/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Map WorktreeCopyMode to kimix_fast_worktree::WorkingTreeMode
    ...
    // Use Kimix-fast-worktree for high-performance worktree creation
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(kimix_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        kimix_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        kimix_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        kimix_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        kimix_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        kimix_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.kimix/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        kimix_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
...
pub async fn create_worktree_from_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    // Use Kimix-fast-worktree -- it handles copying from any worktree path
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(kimix_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
///
/// ACP equivalent of `kimix -w -r <session_id>` (optionally with `--ref`).
...
/// Response from `Kimix/git/worktree/resume_session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response from `Kimix/session/rehydrate`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
use kimix_fast_worktree::{
    DbStats, GcOptions, GcReport, ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord,
    gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_kimix_home,
};
...
pub fn worktree_db_rebuild() -> Result<kimix_fast_worktree::RebuildReport> {
    let home = resolve_kimix_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = kimix_fast_worktree::ListFilter {
...
/// Scan `~/.kimix/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[kimix_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/kimix/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/kimix/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/kimix/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/kimix/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `KIMIX_SHARE_DIR` at an isolated tempdir (`resolve_kimix_home` re-reads
    ...
    /// and restores `KIMIX_SHARE_DIR` on drop (before releasing the lock), so the
    ...
    fn worktree_db_fixture(
        ...
        let env = LockedTestEnv::lock().set("KIMIX_SHARE_DIR", &home);
        ...
        let record = WorktreeRecord {
            ...
            status: kimix_fast_worktree::WorktreeStatus::Alive,
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (KimixHomeFixture is not visible across crates).
