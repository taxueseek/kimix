...
/// Directory (under `<cwd>/.Kimix`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.Kimix/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
