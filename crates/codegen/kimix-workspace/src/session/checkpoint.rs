...
use kimix_hunk_tracker::{HunkId, HunkTrackerSnapshot, HunkTurnDelta};
...
/// Resolve `workspace_rewind_hunks` from `KIMIX_WORKSPACE_REWIND_HUNKS` (default off).
pub(crate) fn rewind_hunks_enabled() -> bool {
    kimix_config::env_bool("KIMIX_WORKSPACE_REWIND_HUNKS").unwrap_or(false)
}
/// Resolve `workspace_rewind_durable` from `KIMIX_WORKSPACE_REWIND_DURABLE`
/// (default off). Off ⇒ the legacy in-memory-only path with no disk I/O.
pub(crate) fn rewind_durable_enabled() -> bool {
    kimix_config::env_bool("KIMIX_WORKSPACE_REWIND_DURABLE").unwrap_or(false)
}
impl WorkspaceSession {
    ...
    pub(crate) async fn restore_hunk_checkpoints(&self, target_prompt_index: usize) {
        ...
        let mut file_states: HashMap<
            std::path::PathBuf,
            kimix_hunk_tracker::FileHunkStateSnapshot,
        > = HashMap::new();
...
mod tests {
    ...
    async fn turn_end_with_durable_flag_off_writes_nothing_to_disk() {
        ...
        handle
            .on_turn_boundary("main", TurnBoundary::rewind_finalize(0))
            .await;
        let store_root = cwd.join(".kimix").join("rewind-checkpoints");
        assert!(
            !store_root.exists(),
