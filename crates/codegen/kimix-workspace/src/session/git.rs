...
pub async fn git_cli(cwd: &Path, args: &[&str]) -> Result<String> {
    ...
    for &(key, val) in kimix_tty_utils::GIT_AUTH_SUPPRESSION_ENVS.iter() {
        cmd.env(key, val);
    }
...
///
/// ```
/// use kimix_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
/// Payload for the `Kimix/git_head_changed` ACP extension notification.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
///
/// 1. `supplied_cwd` is a Kimix-managed worktree (`~/.kimix/worktrees/...`).
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = kimix_tools::util::kimix_home::kimix_home().join("worktrees");
...
const REWIND_GIT_ENV: &str = "KIMIX_WORKSPACE_REWIND_GIT";
...
pub fn git_rewind_enabled() -> bool {
    kimix_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
...
mod tests {
    ...
    fn test_effective_worktree_cwd_empty_offset() {
        let result =
            effective_worktree_cwd("/home/user/.kimix/worktrees/repo/ab-123-a", Path::new(""));
        assert_eq!(result, "/home/user/.kimix/worktrees/repo/ab-123-a");
    }
    #[test]
    ...
    fn test_effective_worktree_cwd_nested_offset() {
        let result = effective_worktree_cwd(
            "/home/user/.kimix/worktrees/repo/ab-123-b",
            Path::new("packages/frontend/src"),
        );
        assert_eq!(
            result,
            "/home/user/.kimix/worktrees/repo/ab-123-b/packages/frontend/src"
        );
    }
    #[test]
    ...
    fn test_effective_cwd_roundtrip_with_compute_offset() {
        ...
        let worktree_root = "/home/user/.kimix/worktrees/myrepo/ab-test-a";
...
mod restore_code_tests {
    ...
    async fn stash_before_destructive_op_dirty_tracked_returns_ref() {
        ...
        assert!(
            list.contains("kimix: pre-test sess-2"),
    ...
    fn restore_code_checkout_allowed_worktree_cwd_is_allowed() {
        let worktrees = Path::new("/home/u/.kimix/worktrees");
        assert!(restore_code_checkout_allowed_in(
            Path::new("/home/u/.kimix/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            Some("/home/u/repo"),
            worktrees,
        ));
    }
    ...
    fn restore_code_checkout_allowed_source_repo_with_worktree_session_is_refused() {
        ...
        assert!(!restore_code_checkout_allowed_in(
            Path::new("/home/u/repo"),
            Some("/home/u/.kimix/worktrees/home-u-repo/2026-05-22-9f2e51ce"),
            worktrees,
        ));
    }
    #[test]
