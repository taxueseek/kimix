...
use kimix_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<kimix_tool_runtime::LocalRegistry>,
    ...
    notification_handle: Option<kimix_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<kimix_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<kimix_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn kimix_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            kimix_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = kimix_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(kimix_tools::computer::local::LocalFs)
            as Arc<dyn kimix_tools::computer::types::AsyncFileSystem>;
        let notification_handle = kimix_tools::notification::ToolNotificationHandle::noop();
        kimix_tools::registry::types::SessionContext {
            backend,
            fs,
            cwd,
            ...
            app_builder_deployer_config:
                kimix_tools::implementations::kimix::deploy_app::AppBuilderDeployerConfig::default(),
...
/// `KIMIX_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> kimix_tools::implementations::kimix::web_fetch::WebFetchConfig {
    use kimix_tools::implementations::kimix::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("KIMIX_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("KIMIX_WEB_FETCH_PROXY") {
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("Kimix:search_replace", Some(ToolKind::Edit)),
                tc("Kimix:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![test_support::tc("Kimix:read_file", Some(ToolKind::Read))],
            behavior_preset: None,
        };
        let mut mcp_dup = test_support::tc("Kimix:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("Kimix:search_replace".to_owned(), ToolKind::Edit),
            ("Kimix:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("Kimix:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("Kimix:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("Kimix:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("Kimix:read_file", None),
                test_support::tc("Kimix:grep", None),
                test_support::tc("Kimix:list_dir", None),
                test_support::tc("Kimix:search_replace", None),
                test_support::tc("Kimix:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("Kimix:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"Kimix:read_file"),
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use kimix_tools::types::resources::{State, WebCitationCounter};
