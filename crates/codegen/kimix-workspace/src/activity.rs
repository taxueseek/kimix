...
use dashmap::DashMap;
use kimix_file_utils::events::{Event, EventWriter, ToolOutcome};
use kimix_tool_protocol::{ToolServerLifecycleStatus, ToolServerStatusPayload};
...
pub struct ActivityTracker {
    ...
    /// `KIMIX_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    pub fn snapshot_session(&self, session_id: &str) -> ToolServerStatusPayload {
        ...
        ToolServerStatusPayload {
            status,
            session_id: kimix_tool_protocol::SessionId::new(session_id).ok(),
            connection_id: None,
