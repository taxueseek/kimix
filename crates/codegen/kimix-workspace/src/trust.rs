...
//! Persists per-folder trust decisions to `~/.kimix/trusted_folders.toml`.
...
//! The store is rooted at [`kimix_config::user_kimix_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.Kimix`) when
//! neither `$KIMIX_SHARE_DIR` nor a home directory is set (e.g. a minimal container /
...
//! a `./.Kimix/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.kimix/`.
...
impl TrustStore {
    /// Load the trust store from `<user_kimix_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_kimix_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`kimix_config::user_kimix_home`], never
    /// [`kimix_config::kimix_home`], so it never falls back to a cwd-relative
    /// `./.Kimix` — that fallback would let an untrusted cloned repo's `.Kimix`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(kimix_config::user_kimix_home())
    }

    /// Map a resolved user-Kimix-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_kimix_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_kimix_home?.join(TRUST_FILE_NAME))
    }
    ...
    fn record_decision(&mut self, workspace_key: &Path, trusted: bool) -> io::Result<()> {
        ...
        let Some(path) = self.path.as_deref() else {
            tracing::warn!(
                path = %canonical.display(),
                trusted,
                "folder trust: no user kimix home resolved; trust decision not recorded"
            );
            return Ok(());
        };
...
///
/// A Kimix-managed worktree first collapses onto its recorded source repo's git
/// ROOT (via the `~/.kimix/worktrees.db` registry), so every `kimix -w` worktree
...
/// `kimix -w` was launched from (the recorded source repo may be a repo subdir).
...
/// `kimix -w` worktree of a repo shares one trust key. The collapse fires ONLY
...
fn git_derived_workspace_key(cwd: &Path) -> PathBuf {
    // A Kimix-managed worktree (any creation mode, incl. standalone clones git
    ...
    if let Some(source_repo) = crate::worktree::source_repo_for_cwd(&cwd.to_string_lossy()) {
        ...
        // ONE key regardless of the subdir Kimix -w was launched from (parity with
...
/// sharing `~/.kimix/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.kimix/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = kimix_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global Kimix-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match kimix_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.kimix");
        assert_eq!(
        ...
        // cwd-relative `./.Kimix` that kimix_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.Kimix/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_kimix_home() {
        // Thin source-pin: the production accessor reads user_kimix_home()
        // (Option, no cwd fallback), not kimix_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            kimix_config::user_kimix_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    fn workspace_key_collapses_linked_worktrees_onto_main_checkout() {
        // Every linked `kimix -w` worktree of a repo must share ONE trust key:
    ...
    // ── workspace_key registry collapse (Kimix-managed worktrees) ─────────
    ...
    /// Point `KIMIX_SHARE_DIR` at an isolated tempdir and register one Kimix-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `KIMIX_SHARE_DIR` on
    /// drop (before releasing the lock), so the caller may bind it any way.
    fn register_kimix_worktree(
        temp: &tempfile::TempDir,
        name: &str,
        source_repo: &Path,
        creation_mode: &str,
    ) -> (LockedTestEnv, PathBuf) {
        use kimix_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
    ...
    #[test]
    fn workspace_key_collapses_standalone_kimix_worktree_onto_source_repo() {
        ...
        let (_env, wt) = register_kimix_worktree(&temp, "wt", &source_repo, "standalone");

        let expected = canonicalize_or_owned(&source_repo);
        assert_eq!(
            workspace_key(&wt),
            expected,
            "a standalone kimix worktree must collapse onto its recorded source repo"
        );
    ...
    fn workspace_key_collapses_worktree_onto_source_repo_git_root() {
        ...
        let (_env, wt) = register_kimix_worktree(&temp, "wt", &subdir, "standalone");
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<kimix_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        // worktree cwd, and `outside` is its OWN git repo (under Kimix HOME but not
        ...
        let (_env, _wt) = register_kimix_worktree(&temp, "wt", &source_repo, "standalone");

        // Under Kimix HOME but NOT under `<home>/worktrees`, and its own git repo.
        let outside = root.join("kimix-home").join("not-worktrees").join("proj");
        ...
        assert_eq!(
            ...
            "a cwd outside <kimix_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,
