...
use kimix_tools::types::output::ToolRunResult;
pub use kimix_workspace_types::rpc::WorkspaceRpc;
pub use kimix_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use kimix_workspace_types::rpc::code_nav::{
...
pub use kimix_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use kimix_workspace_types::rpc::hunks::{
...
pub use kimix_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use kimix_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use kimix_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use kimix_workspace_types::rpc::workspace::WorkspaceInfoReq;
...
fn hunk_line_info_to_wire(info: &kimix_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: kimix_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use kimix_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &kimix_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: kimix_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use kimix_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: kimix_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: kimix_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &kimix_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: kimix_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: kimix_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> kimix_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => kimix_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => kimix_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<kimix_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = kimix_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                kimix_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &kimix_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<kimix_hooks::discovery::HookRegistry> {
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<kimix_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<kimix_codebase_graph::QueryResult, kimix_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<kimix_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
/// [`FinalizedToolset`](kimix_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: kimix_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<kimix_hooks::discovery::HookRegistry> {
...
mod tests {
    ...
    /// request types are defined in `Kimix-workspace-types` (and re-exported
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset =
            std::sync::Arc::new(kimix_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    fn hunk_to_wire_serializes_identically() {
        use kimix_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use kimix_hunk_tracker::FileContentEntry;
        use kimix_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use kimix_hunk_tracker::SessionSummary;
        use kimix_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
        use std::sync::Arc;
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = kimix_hooks::config::HookSpec {
            ...
            event: kimix_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.kimix/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = kimix_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use kimix_hooks::event::HookEventName as E;
    ...
    /// can't depend on `Kimix-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use kimix_hooks::config::HookSpec;
