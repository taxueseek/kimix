...
//! `.Kimix/config.toml` files by walking from `cwd` up to the git root.
//!
...
use kimix_agent::repo::RepoDirChain;
...
/// True when `config_path` is `$KIMIX_SHARE_DIR/config.toml` (user tier, not project).
fn is_user_kimix_config_file(config_path: &Path) -> bool {
    let Some(user_home) = kimix_config::user_kimix_home() else {
        return false;
    };
...
/// Find all `.Kimix/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.Kimix/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.kimix/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.kimix/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        .iter()
        .rev()
        .map(|dir| dir.join(".kimix").join("config.toml"))
        .filter(|config_path| config_path.is_file() && !is_user_kimix_config_file(config_path))
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn find_project_configs_excludes_user_kimix_config_file() {
        let Some(user_home) = kimix_config::user_kimix_home() else {
            return;
        };
        let user_config = user_home.join("config.toml");
        if user_config.is_file() {
            ...
            assert!(
                !from_home.iter().any(|p| is_user_kimix_config_file(p)),
                "user config leaked into project configs: {from_home:?}"
            );
            assert!(is_user_kimix_config_file(&user_config));
        }

        let tmp = tempfile::tempdir().unwrap();
        let project = tmp.path().join("repo");
        std::fs::create_dir_all(project.join(".kimix")).unwrap();
        std::fs::write(project.join(".kimix/config.toml"), "# project\n").unwrap();
        let found = find_project_configs(&project);
        assert_eq!(found.len(), 1);
        assert!(!is_user_kimix_config_file(&found[0]));
    }
}
