//! Headless markdown analysis sharing Kimix's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `Kimix-markdown` so analysis matches what Kimix
...
/// The exact `pulldown-cmark` option set Kimix uses to render markdown.
///
...
/// Offset event stream from Kimix's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Kimix's options; count elements and flag structural issues.
