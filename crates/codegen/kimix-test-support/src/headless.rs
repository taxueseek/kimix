//! Headless mode (`kimix -p`) test runner.
//!
//! Runs the Kimix binary as a subprocess with the mock server, captures output.
use std::path::Path;
...
/// Run `kimix` with the given args against the mock server, with a 60s timeout.
...
pub async fn run_headless_with_cmd(mut cmd: tokio::process::Command) -> HeadlessResult {
    ...
    let mut child = cmd
        .spawn()
        .unwrap_or_else(|e| panic!("failed to spawn kimix binary at {}: {e}", binary.display()));
    ...
    .await
    {
        Ok(result) => (
            result.unwrap_or_else(|e| {
                panic!("failed to wait for kimix binary {}: {e}", binary.display())
            }),
            false,
        ),
        Err(_) => {
            ...
            let status = child.wait().await.unwrap_or_else(|e| {
                panic!(
                    "failed to kill timed out kimix binary {}: {e}",
                    binary.display()
                )
            });
            (status, true)
