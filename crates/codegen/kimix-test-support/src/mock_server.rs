...
impl RequestLog {
    ...
    fn record(
        ...
        self.entries.lock().expect("lock poisoned").push(LogEntry {
...
impl MockInferenceServer {
    ...
    pub fn set_models(&self, models: Vec<MockModelEntry>) {
        let mut guard = self.models.write().expect("rwlock write poisoned");
    ...
    pub fn set_agent_turns(&self, turns: impl IntoIterator<Item = String>) {
        *self.agent_turns.lock().expect("lock poisoned") = turns.into_iter().collect();
    }
    ...
    pub fn set_settings(&self, settings: impl serde::Serialize) {
        ...
        let mut guard = self.settings.write().expect("rwlock write poisoned");
        *guard = Some(value);
    }
    ...
    pub fn set_user_subscription_tier(&self, tier: Option<&str>) {
        *self.user_tier.write().expect("rwlock write poisoned") = tier.map(str::to_owned);
    }
    ...
    pub fn set_chunk_delay(&self, delay: Option<Duration>) {
        *self.chunk_delay.write().expect("rwlock write poisoned") = delay;
    }
    ...
    pub fn requests(&self) -> Vec<LogEntry> {
        self.log.entries.lock().expect("lock poisoned").clone()
    }
    ...
    pub fn request_log_summary(&self) -> String {
        let entries = self.log.entries.lock().expect("lock poisoned");
        if entries.is_empty() {
    ...
    pub fn storage_uploads(&self) -> Vec<StorageUpload> {
        self.storage.uploads.lock().expect("lock poisoned").clone()
    }
    ...
    fn pop_agent_turn(
        ...
        agent_turns.lock().expect("lock poisoned").pop_front()
    }
                ...
                post(move |headers: HeaderMap, Json(body): Json<Value>| {
                    ...
                    async move {
                        ...
                        let (events, gate) = match Self::pop_agent_turn(&agent_turns, &body) {
                            ...
                            None => {
                                let events = match &*mode.read().expect("rwlock read poisoned") {
            ...
            .route(
                ...
                post(move |headers: HeaderMap, Json(body): Json<Value>| {
                    ...
                    async move {
                        ...
                        let stop = stop_reason.read().expect("rwlock read poisoned").clone();
                ...
                get({
                    ...
                    move || {
                        ...
                        async move {
                            ...
                            let models_json = models.read().expect("rwlock read poisoned").clone();
                            Json(json!({
                                "object": "list",
                                "data": models_json,
                            }))
                ...
                get({
                    ...
                    move || {
                        ...
                        async move {
                            ...
                            let maybe = settings.read().expect("rwlock read poisoned").clone();
                            match maybe {
                ...
                get(
                    move |axum::extract::RawQuery(query): axum::extract::RawQuery| {
                        ...
                        async move {
                            ...
                            let tier = user_tier.read().expect("rwlock read poisoned").clone();
                            let mut body = json!({
                                "userId": "mock-user",
