//! Leader-mode (`kimix agent --leader stdio`) test harness.
//!
...
use kimix_acp_lib::LineBufferedRead;
use tokio_util::compat::{TokioAsyncReadCompatExt, TokioAsyncWriteCompatExt};

use crate::env::kimix_binary;
...
/// (version-skew) test. Falls back to [`kimix_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "KIMIX_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`kimix_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "KIMIX_BINARY_CLIENT";
...
/// (`KIMIX_BINARY_LEADER`, else the shared [`kimix_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`KIMIX_BINARY_CLIENT`,
/// else the shared [`kimix_binary`] resolution).
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "kimix/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `kimix agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&kimix_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export KIMIX_* vars
            // (e.g. KIMIX_LEADER_SOCKET pointing at a REAL leader on this
            ...
            // ~/.kimix/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "kimix_shell=debug");
    ...
    pub fn stderr_text(&self) -> String {
        String::from_utf8_lossy(&self.stderr.lock().expect("lock poisoned")).into_owned()
    }

    pub fn captured_text(&self) -> String {
        self.capture.chunks.lock().expect("lock poisoned").join("")
    }
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".kimix").join("leader.lock")
}
...
///
/// The `Kimix/leader_reconnected` ext notification is dropped by the typed
/// `ClientSideConnection` (bare `Kimix/*` methods are rejected by the ACP
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".kimix").join("leader.log")).unwrap_or_default()
}
