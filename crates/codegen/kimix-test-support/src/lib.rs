...
//! Shared test utilities for Kimix crates: mock inference server, SSE
...
//! - [`KimixStdioClient`] — ACP client that drives `kimix agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `kimix agent --leader stdio` (unix)
//! - [`run_headless`] — Run `kimix -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`kimix_binary`] — Resolve the Kimix binary path (KIMIX_BINARY env or cargo_bin)
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{KimixStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, kimix_binary};
pub use headless::{
