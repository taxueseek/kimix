//! ACP stdio clients for testing Kimix sessions end-to-end: the typed
//! [`KimixStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
use crate::env::{kimix_binary, test_env_cmd_tokio};
use crate::headless::stderr_tail;
use crate::mock_server::MockInferenceServer;
use crate::process::spawn_piped_with_stderr_capture;

/// Spawn `kimix agent stdio` with the canonical hermetic test env: the sandbox
/// from [`test_env_cmd_tokio`] plus the debug-logging kill-list, so the
/// hermeticity setup exists exactly once for the typed ([`KimixStdioClient`])
...
/// kill-list so a test can still set e.g. `KIMIX_DEBUG_LOG=1` explicitly.
...
/// Drives `kimix agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`KimixStdioClient`] (`ClientSideConnection`,
