...
//! TTY/console detach via `kimix_tty_utils::detach_std_command`); this module only
...
/// direct child.
///
/// `kimix_tty_utils::detach_std_command` runs `setsid` (EPERM fallback
/// `setpgid(0,0)`), so the child is its own group leader and its pgid equals its
/// pid. We send the signal directly because `kimix_tty_utils::ProcessGroup` only
