...
//!
//! ```
//! use kimix_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `KimixDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `KimixNight`, `TokyoNight`).
    Dark,
}
