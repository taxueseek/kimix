...
mod imp {
    ...
    pub fn install(crash_dir: &Path, kimix_version: &str) -> bool {
        ...
        unsafe {
            ...
            version.fill(0);
            let copy_len = kimix_version.len().min(format::VERSION_STRING_LEN);
            version[..copy_len].copy_from_slice(&kimix_version.as_bytes()[..copy_len]);
        }
...
mod tests {
    ...
    fn full_install_replaces_minimal_handler() {
        ...
        let dir = std::env::temp_dir().join("kimix-crash-handler-test-replace");
