//! Integration tests for Kimix-crash-handler.
//!
...
fn subprocess_entry() {
    ...
    let config = kimix_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    kimix_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob =
        kimix_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report = kimix_crash_handler::check_previous_crash(tmp.path())
