//! Headless single-turn mode (`kimix -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_kimix_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use kimix_acp_lib::{AcpAgentTx, AcpClientMessageBox, acp_send};
use kimix_shell::agent::auth_method::AuthMethodKind;
use kimix_shell::agent::config::Config as AgentConfig;
use kimix_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
use kimix_shell::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
use kimix_shell::sampling::types::{
    REASONING_EFFORT_META_KEY, parse_canonical_effort_token, reasoning_effort_meta_value,
};
use kimix_shell::util::config as cli_config;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<kimix_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use kimix_workspace::permission::types::RuleAction;
...
pub fn resolve_agent_arg(agent: &str) -> ResolvedAgent {
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<kimix_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = kimix_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut kimix_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    kimix_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `kimix login` to authenticate with Kimi Code.".to_string()
    } else {
        "Not signed in. Run `kimix login` to authenticate with Kimi Code \
         (the device flow prints a URL you can open on any machine).\n\
         Alternatively, set a Moonshot open-platform API key via \
         KIMIX_MOONSHOT_API_KEY."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::auth::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            use std::io::IsTerminal;
            let interactive = std::io::stdin().is_terminal()
                && !kimix_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !kimix_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &kimix_tools::types::compat::CompatConfig::default());
            ...
            acp::LoadSessionRequest::new(acp::SessionId::new(sid.to_string()), cwd.to_path_buf())
                ...
                .meta({
                    ...
                    if let Some(true) = restore_code {
                        m.insert("kimix/restore_code".into(), serde_json::Value::Bool(true));
                    }
                    Some(m)
                }),
            acp_tx,
...
async fn open_session_with_id(
    ...
    crate::session_startup::ensure_session_id_available(session_id, &cwd_str)?;
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &kimix_tools::types::compat::CompatConfig::default());
    let new_resp: acp::NewSessionResponse = acp_send(
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'kimix models' to see available models.",
                name,
                e
            )
        } else {
...
fn headless_materialize_ctx(has_worktree: bool) -> crate::session_startup::MaterializeCtx {
    crate::session_startup::MaterializeCtx {
        has_worktree,
...
pub async fn run_single_turn(
    ...
    agent_config.mode = kimix_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = kimix_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        kimix_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_kimix_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let default_auth_method_id = crate::auth::parse_default_auth_method_id(init_resp.meta.as_ref());
    ...
    use crate::session_startup::{self, MaterializedStartup, SessionStartupFlags};
    ...
    let track_active = std::env::var("KIMIX_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ =
            kimix_shell::active_sessions::register(kimix_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(kimix_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(kimix_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Pending background work: bash/monitor via Kimix/task_backgrounded +
    // task_completed; background subagents via SubagentSpawned + SubagentFinished
    // on Kimix/session_notification (prefixed `subagent:{id}` in pending_bg).
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.kimix can't hang exit.
        let _ = kimix_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = kimix_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_headless_acp_message(
    ...
    match msg {
        ...
        AcpClientMessageBox::WaitForTerminalExit(args) => {
            args.response_tx
                .send(Err(crate::auth::wait_for_exit_not_supported(
                    "headless mode",
                )))
                .ok();
        }
...
fn handle_ext_notification(
    ...
    if method == "kimix/task_completed" {
        #[derive(serde::Deserialize)]
    ...
    if method == "kimix/monitor_event" {
        return ExtEvent::MonitorEvent;
    }

    match method {
        "kimix/session_notification" | "kimix/session/update" => {}
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "kimix/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "kimix/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use kimix_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            kimix_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> kimix_acp_lib::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_task_backgrounded_parses_task_id() {
        ...
        // the inner update object (matching the real `Kimix/task_backgrounded`
        // wire shape: `{ "update": { "sessionUpdate": ..., "task_id": ... } }`).
        let notif = make_ext_notif(
            "kimix/task_backgrounded",
            serde_json::json!({
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = kimix_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("kimix/other", raw.into()),
            response_tx: tx,
        }
        .boxed();
        assert!(matches!(
