...
/// Built-in fallback effort menu when the server sends no `reasoningEfforts`.
fn legacy_effort_options() -> Vec<ReasoningEffortOption> {
    const EFFORT_LEVELS: &[ReasoningEffort] = &[
        ReasoningEffort::Xhigh,
        ReasoningEffort::High,
        ReasoningEffort::Medium,
        ReasoningEffort::Low,
    ];
    fn effort_description(level: ReasoningEffort) -> &'static str {
        match level {
...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model.
    ...
    /// The reasoning-effort menu for the current model.
    ...
    /// Menu for a specific catalog model id.
    pub fn reasoning_effort_options_for(&self, id: &acp::ModelId) -> Vec<ReasoningEffortOption> {
    ...
    /// Display token for the current reasoning effort.
    ...
    /// Map a typed/selected effort token to its canonical value for the current model.
    ...
    pub fn resolve_effort_token_for(
        &self,
        id: &acp::ModelId,
        token: &str,
    ...
    /// then resolve the token (menu id or canonical level).
    ...
    /// Resolve a user-supplied name to a `ModelId`.
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        let id = acp::ModelId::new(Arc::from("kimix"));
        let mut state = ModelState::default();
        state
            .available
            .insert(id.clone(), model_with_effort("kimix", "Kimix", "high"));
        ...
        refreshed.insert(id.clone(), model_with_effort("kimix", "Kimix", "high"));
