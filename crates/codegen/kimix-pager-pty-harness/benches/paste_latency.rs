//! `paste-latency` — clipboard paste latency benchmark for `Kimix-tui`.
//!
...
//! ```bash
//! cargo bench -p Kimix-pager-pty-harness --bench paste_latency -- --iterations 10
//!

//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p Kimix-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/Kimix-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for kimix-tui (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
