//! `pty-bench` — PTY benchmark CLI for `Kimix-tui`.
//!
...
//! ```bash
//! cargo bench -p Kimix-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/Kimix-${VERSION}-linux-x86_64 \
//!   cargo bench -p Kimix-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for kimix-tui",
    long_about = None,
)]
struct Cli {
