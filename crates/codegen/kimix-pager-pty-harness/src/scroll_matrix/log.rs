//! `KIMIX_SCROLL_LOG` JSONL parsing, per-stream grouping, and finalize
//! synchronization.
//!
//! Wire schema source of truth: the pager's `ScrollLogRecord` in
//! `Kimix-tui/src/input/scroll_log.rs`. [`ScrollLogLine`] mirrors it
...
/// Parse a `KIMIX_SCROLL_LOG` JSONL file into records.
///
...
/// session (`KIMIX_SCROLL_LOG` set at spawn): `stream_start` → `flush`* →
...
mod tests {
    ...
    // in `Kimix-tui/src/input/mouse/tests.rs`.
