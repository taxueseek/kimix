...
pub struct ExpectedProfile {
    ...
    pub invert: bool,
    /// Speed multiplier (NOT the 1-100 setting): `KIMIX_SCROLL_SPEED=100`
...
pub struct MatrixCell {
    ...
    /// appends `KIMIX_SCROLL_LOG`; the harness's env strips guarantee the
...
const SPEED100: (&str, &str) = ("KIMIX_SCROLL_SPEED", "100");
const MODE_WHEEL: (&str, &str) = ("KIMIX_SCROLL_MODE", "wheel");
const MODE_TRACKPAD: (&str, &str) = ("KIMIX_SCROLL_MODE", "trackpad");
const LINES1: (&str, &str) = ("KIMIX_SCROLL_LINES", "1");
const INVERT: (&str, &str) = ("KIMIX_INVERT_SCROLL", "1");
...
mod tests {
    ...
    /// pager would (`from_terminal_context` + the `KIMIX_SCROLL_*` env
    ...
    fn derive_expected(env: &'static [(&'static str, &'static str)]) -> ExpectedProfile {
        ...
        if let Some(lines) = get("KIMIX_SCROLL_LINES") {
        ...
        let speed = match get("KIMIX_SCROLL_SPEED") {
            None | Some("50") => 1.0,
            Some("100") => 6.0,
            Some(other) => panic!("unmapped KIMIX_SCROLL_SPEED {other:?}"),
        };
        ExpectedProfile {
            mode: get("KIMIX_SCROLL_MODE").unwrap_or("auto"),
            ept,
            wheel_lpt,
            trackpad_lpt,
            invert: get("KIMIX_INVERT_SCROLL") == Some("1"),
            speed,
        }
    }
