...
//! elected leader) plus a fixed leader socket beneath its `KIMIX_SHARE_DIR`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `KIMIX_SHARE_DIR`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared KIMIX_SHARE_DIR => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let kimix_home = content.home().join(".kimix");
        std::fs::create_dir_all(&kimix_home).context("create kimix home")?;
        let socket = kimix_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `KIMIX_SHARE_DIR/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".kimix").join("sessions")
    }
...
mod tests {
    ...
    fn envelope(update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_kimix/session/update","params":{{"sessionId":"s","update":{update_json}}}}}"#
        )
    }
    ...
    fn parse_update_payloads_unwraps_and_tolerates_torn_trailing_line() {
        let body = format!(
            "{}\n{}\n{{\"timestamp\":2,\"method\":\"_kimix/sess",
            envelope(
