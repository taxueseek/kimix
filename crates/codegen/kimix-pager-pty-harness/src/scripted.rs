//! Scripted TUI scenario runner for Kimix-tui.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$KIMIX_SHARE_DIR`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&kimix_home).with_context(|| {
                format!("create scenario KIMIX_SHARE_DIR {}", kimix_home.display())
            })?;
            fs::write(kimix_home.join("config.toml"), config_toml)
