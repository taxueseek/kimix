...
fn local_pager_binary_path() -> Result<PathBuf> {
    Ok(target_dir()?
        .join("debug")
        .join(format!("kimix{}", std::env::consts::EXE_SUFFIX)))
}
...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    cmd.current_dir(workspace_root()?)
        .args(["build", "-p", "kimix-bin", "--bin", "kimix"])
        .stdin(Stdio::null())
        .envs(kimix_tty_utils::pager_env());
    kimix_tty_utils::detach_std_command(&mut cmd);
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build the kimix binary"))?;

    if !output.status.success() {
        bail!(
            "failed to build the kimix binary (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
    ...
    if !binary.exists() {
        bail!(
            "kimix build completed but binary missing at {}",
            binary.display()
        );
    }
    Ok(())
...
/// 2. `CARGO_BIN_EXE_Kimix` (set by `cargo test`)
/// 3. Build locally via `cargo build -p Kimix-bin` (the composition-
///    root package that owns the `kimix` binary)
pub fn pager_binary() -> Result<PathBuf> {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_kimix") {
        let p = PathBuf::from(path);
        if p.exists() {
            return Ok(p);
        }
