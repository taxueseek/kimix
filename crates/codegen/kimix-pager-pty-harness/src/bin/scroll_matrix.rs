//! `scroll-matrix` — scroll validation matrix sweep for Kimix-tui.
//!
...
use kimix_pager_pty_harness::pager_binary;
use kimix_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against kimix-tui",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_Kimix,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves KIMIX_SCROLL_LOG against ITS cwd (the
