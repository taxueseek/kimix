...
//! [`MockInferenceServer`] from `Kimix-test-support` and provides the
...
pub use kimix_test_support::mock_server::LogEntry;
pub use kimix_test_support::mock_server::MockModelEntry as MockModel;
pub use kimix_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use kimix_test_support::sse;
pub use kimix_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.kimix
    ...
    ///
    /// Mirrors `kimix_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit KIMIX_SHARE_DIR prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // KIMIX_SHARE_DIR is set in the test runner's env).
            ("KIMIX_SHARE_DIR".into(), kimix_home),
            ("KIMIX_CODE_BASE_URL".into(), self.url()),
            ("KIMIX_API_BASE_URL".into(), self.url()),
            ...
            ("KIMIX_MOONSHOT_CN_BASE_URL".into(), self.url()),
            ("KIMIX_MOONSHOT_AI_BASE_URL".into(), self.url()),
            ...
            ("KIMIX_PROMPT_SUGGESTIONS".into(), "false".into()),
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("KIMIX_SHARE_DIR").as_deref(),
            content.home().join(".kimix").to_str()
        );
        assert_eq!(get("KIMIX_CODE_BASE_URL"), Some(content.url()));
        assert_eq!(get("KIMIX_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("KIMIX_MOONSHOT_CN_BASE_URL"), Some(content.url()));
        assert_eq!(get("KIMIX_MOONSHOT_AI_BASE_URL"), Some(content.url()));
        assert_eq!(get("XAI_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("KIMIX_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("KIMIX_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("KIMIX_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("KIMIX_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("KIMIX_MAX_RETRIES").as_deref(), Some("0"));
