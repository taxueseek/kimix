//! Unified PTY harness for Kimix-tui.
//!
...
//!    `Kimix-tui/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `KIMIX_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use kimix_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through Kimix's stdout. Used to reproduce the doubled-line class of bugs
    /// where Kimix's diff renderer never re-asserts a region it didn't write
