...
//! ```bash
//! cargo test -p Kimix-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use kimix_pager_pty_harness::{ContentController, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.Kimix/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".kimix").join("sessions");
