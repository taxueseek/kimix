...
//! ```rust
//! use serde::Serialize;
//! use kimix_tool_runtime::render::ToolOutput;
//! use kimix_tool_runtime::ContentBlock;
//!
//! #[derive(Serialize)]
//! struct BashOutput {
//!     stdout: String,
...
/// Lets `kimix_tool_types::TaskOutputOutput` be used directly as a `Tool::Output`
/// (handy for stub/test tools and pass-through proxies).
impl ToolOutput for kimix_tool_types::TaskOutputOutput {}

/// Lets `kimix_tool_types::SubagentCompletedOutput` be used directly as a
/// `Tool::Output` (the `task` tool's structured completion output).
impl ToolOutput for kimix_tool_types::SubagentCompletedOutput {}

/// Lets `kimix_tool_types::KillTaskOutput` be used directly as a `Tool::Output`
/// (the `kill_task` tool's typed result / not-found output).
impl ToolOutput for kimix_tool_types::KillTaskOutput {}
...
pub fn extract_content_blocks(value: &Value) -> Vec<ContentBlock> {
    ...
    if let Some(obj) = value.as_object() {
        // Kimix `ToolRunResult`: the model sees `prompt_text` (reminders
