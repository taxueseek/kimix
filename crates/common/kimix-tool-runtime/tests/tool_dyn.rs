...
//! `KimixToolDyn` / `KimixToolFamily` tests but against the runtime's
//! typed `Tool` trait.
use std::sync::Arc;
...
use kimix_tool_protocol::{ToolCapabilities, ToolId};
use kimix_tool_runtime::{
...
impl Tool for BlockingEcho {
    ...
    fn description(&self, _ctx: &::kimix_tool_runtime::ListToolsContext) -> ToolDescription {
...
async fn tool_dyn_delegates_id_description_capabilities() {
    ...
    assert_eq!(
        tool.description(&kimix_tool_runtime::ListToolsContext::default())
            .name,
        "blocking_echo"
    );
