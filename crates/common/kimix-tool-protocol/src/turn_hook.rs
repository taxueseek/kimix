...
/// `kimix_file_utils::events::SessionRelationship::Primary`).
pub const DEFAULT_SESSION_RELATIONSHIP: &str = "primary";

/// Default `schema_version` wire value. Bare literal (not the
/// `Kimix-file-utils` constant) to avoid a dependency cycle.
...
pub struct BeforeTurnPayload {
    ...
    pub turn_number: u64,
    /// Model being used for this turn (e.g. "Kimix-3").
    pub model_id: String,
    ...
    /// (`"primary"` | `"subagent"`). A `String`, not the `Kimix-file-utils`
...
pub struct AfterTurnPayload {
    ...
    /// the workspace decodes it into the `Kimix-file-utils`
