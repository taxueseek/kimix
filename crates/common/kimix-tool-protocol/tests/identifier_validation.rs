...
fn tool_id_accepts_bare_and_namespaced_names() {
    ...
    let namespaced = ToolId::new("Kimix:read_file").unwrap();
    assert_eq!(namespaced.as_str(), "Kimix:read_file");
...
fn tool_id_try_from_string_works() {
    let id: ToolId = "Kimix:read_file".to_owned().try_into().unwrap();
    assert_eq!(id.as_str(), "Kimix:read_file");
}
...
fn server_id_synthesis_starts_with_auto_prefix() {
    ...
    assert_eq!(synth_ns.as_str(), "auto:tool:Kimix:read_file");
}

#[test]
fn server_id_synthesis_is_deterministic() {
    let conn = ConnectionId::new("conn-abc").unwrap();
    let tool = ToolId::new("Kimix:read_file").unwrap();
