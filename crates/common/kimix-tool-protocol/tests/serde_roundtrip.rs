...
fn tool() -> ToolId {
    ToolId::new("Kimix:read_file").unwrap()
}
...
fn id_types_round_trip_as_bare_strings() {
    ...
    assert_eq!(roundtrip(&tool()), json!("Kimix:read_file"));
...
fn tool_registration_round_trips_with_and_without_server_id() {
    let base = ToolRegistration {
        ...
        user_id: user(),
        server_id: None,
        description: sample_description("read_file", Some("Kimix")),
...
fn registration_outcome_variants_round_trip() {
    ...
    for (outcome, expected_tag) in cases {
        ...
        assert_eq!(json["tool_id"], json!("Kimix:read_file"));
    }
}
...
fn tool_registration_round_trips() {
    let reg = ToolRegistration {
        ...
        user_id: user(),
        server_id: None,
        description: ToolDescription::new("read_file", "d").with_namespace("Kimix"),
        input_schema: None,
        capabilities: None,
        notification_schemas: None,
        transport_kind: TransportKind::Remote,
...
fn session_lifecycle_payloads_round_trip() {
    ...
    let bind_result = SessionBindServerResult {
        ...
        unserved_tool_ids: vec!["Kimix:monitor".to_owned()],
    ...
    assert_eq!(v["unserved_tool_ids"], json!(["Kimix:monitor"]));
    assert_eq!(
        v["resolve_error"],
...
fn tools_changed_round_trips_with_per_array_skip_when_empty() {
    ...
    assert_eq!(v["added"][0], json!("Kimix:read_file"));
    assert_eq!(v["updated"][0], json!("Kimix:read_file"));
...
fn tool_error_wire_terminal_error_round_trips_with_string_code() {
    let err = ToolErrorWire::TerminalError {
        tool_id: ToolId::new("Kimix:bash").unwrap(),
        message: "exit 137".to_owned(),
    };
    let v = roundtrip(&err);
    assert_eq!(v["code"], json!("terminal_error"));
    assert_eq!(v["tool_id"], json!("Kimix:bash"));
