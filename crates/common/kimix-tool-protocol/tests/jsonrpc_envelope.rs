...
fn full_call_envelope_serialises_to_expected_shape() {
    use kimix_tool_protocol::{ToolCallId, ToolCallParams, ToolId};
    let req = JsonRpcRequest {
        ...
        params: ToolCallParams {
            ...
            tool_id: ToolId::new("Kimix:read_file").unwrap(),
    ...
    assert_eq!(v["session_id"], json!("sess_abc"));
    assert_eq!(v["method"], json!("tool.call"));
    assert_eq!(v["params"]["tool_id"], json!("Kimix:read_file"));
...
fn envelope_session_id_and_inner_params_session_id_are_distinct_layers() {
    use kimix_tool_protocol::{ToolDefinitionMode, ToolsListParams};
    let req = JsonRpcRequest {
        jsonrpc: JsonRpcVersion,
