...
use kimix_tool_protocol::{IdError, ToolDescriptionWithSchema, ToolId};
...
fn namespaced_name_derives_to_namespaced_tool_id() {
    let derived = entry("read_file", Some("Kimix")).derive_tool_id().unwrap();
    assert_eq!(derived, ToolId::new("Kimix:read_file").unwrap());
}
...
fn duplicate_derivations_in_a_batch_are_detectable() {
    let batch = [
        entry("read_file", Some("Kimix")),
        entry("write_file", Some("Kimix")),
        entry("read_file", Some("Kimix")),
    ];
    ...
    assert_eq!(id, &ToolId::new("Kimix:read_file").unwrap());
    assert_eq!(indices, &vec![0, 2]);
}

#[test]
fn derivation_does_not_collide_across_namespaces() {
    let a = entry("read_file", Some("Kimix")).derive_tool_id().unwrap();
