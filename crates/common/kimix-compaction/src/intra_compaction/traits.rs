...
pub enum CompactionTarget {
    ...
    /// with a single summary — Kimix's full-replace strategy. No tail is
...
/// stream processor. Implemented by Kimix chat's
/// `StreamProcessor` (`Item = Arc<KimixTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<KimixTurn>`) already satisfies it.
