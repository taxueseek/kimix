...
// The `Shared` summarizer reuses Kimix's full-replace summarization core
...
/// `FullReplace` strategy (default): Kimix's full-replace — summarize the
...
/// `<kimix_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching Kimix. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through Kimix's shared core.
    ...
    //     Shared with Kimix chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<kimix_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<kimix_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<kimix_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): Kimix's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<kimix_user_queries>` preamble so
...
/// — Kimix's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and Kimix stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // Kimix appends the summarization prompt as the final user message;
    ...
    .await
    {
        // Kimix returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Kimix chat's
...
/// the literal error messages produced by the Kimix chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Kimix chat sampler when the
    ...
    impl MockStreamProc {
        ...
        fn was_applied(&self) -> bool {
            self.applied.lock().expect("lock poisoned").is_some()
        }
    }
    ...
    impl CompactionStreamProc for MockStreamProc {
        ...
        async fn get_accumulated_turns_for_compaction(&self) -> Vec<MockItem> {
            self.turns.lock().expect("lock poisoned").clone()
        }
        async fn replace_with_compaction(
            ...
            let mut guard = self.turns.lock().expect("lock poisoned");
            match target {
                CompactionTarget::Steps => {
                    if n > guard.len() {
            ...
            *self.applied.lock().expect("lock poisoned") = Some((n, turn));
            Ok(())
        }
    }
    ...
    impl IntraCompactionObserver for RecordingObserver {
        fn on_error(&self, status: &'static str) {
            self.errors.lock().expect("lock poisoned").push(status);
        }
        fn on_success(&self, target: CompactionTarget, _b: u32, _a: u32, _t: u32, _e: Duration) {
            self.successes.lock().expect("lock poisoned").push(target);
        }
    }
    ...
    async fn compact_replaces_turns_on_success_and_notifies_observer() {
        ...
        assert_eq!(
            obs.successes.lock().expect("lock poisoned").as_slice(),
            &[CompactionTarget::Steps]
        );
        assert!(obs.errors.lock().expect("lock poisoned").is_empty());
    }
    ...
    async fn compact_skips_on_insufficient_reduction() {
        ...
        assert_eq!(
            obs.errors.lock().expect("lock poisoned").as_slice(),
            &["insufficient_reduction"]
        );
    }
    ...
    async fn full_replace_compacts_whole_conversation_and_notifies_observer() {
        ...
        assert_eq!(sp.turns.lock().expect("lock poisoned").len(), 1);
    ...
    async fn full_replace_appends_active_reminder_to_summary() {
        ...
        let committed = sp.turns.lock().expect("lock poisoned");
    ...
    /// — guards the forwarding that Kimix chat (`Arc<KimixTurn>`) relies on.
    #[test]
