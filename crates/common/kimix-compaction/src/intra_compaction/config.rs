...
///
/// - `FullReplace` (default): Kimix's full-replace strategy — summarize
...
/// time (token threshold only, matching Kimix) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as Kimix full-replace auto-compact). Worthless early passes are
    ...
    /// `<kimix_user_queries>` preamble prepended to the history compaction
