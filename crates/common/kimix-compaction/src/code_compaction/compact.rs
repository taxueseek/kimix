//! Kimix's full-replace compaction pass.
//!
//! Kimix does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `Kimix-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). Kimix keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (Kimix's
...
/// Run Kimix's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam Kimix's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // Kimix appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce Kimix's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_kimix_history_shape() {
        let llm_turns = vec![
    ...
    async fn observer_receives_attempt_and_success_callbacks() {
        ...
        impl FullReplaceObserver for RecordingObserver {
            ...
            fn on_error(&self, _attempts: u32) {
                *self.errors.lock().expect("lock poisoned") += 1;
            }
        }
        ...
        assert_eq!(
            *observer.attempts.lock().expect("lock poisoned"),
            vec!["degenerate", "success"]
        );
        assert_eq!(*observer.successes.lock().expect("lock poisoned"), 1);
        assert_eq!(*observer.errors.lock().expect("lock poisoned"), 0);
    }
}
