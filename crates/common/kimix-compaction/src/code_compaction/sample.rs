...
//!
//! The canonical `sample → classify → retry` loop, used by **both** Kimix's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Kimix chat's intra `Shared` summarizer
...
//! the winning summary (Kimix cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the Kimix host uses to step down its input size).
