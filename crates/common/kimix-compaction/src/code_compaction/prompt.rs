//! Kimix's session-level summarization prompt.
//!
//! Split out of the crate-root `prompt` module so Kimix's full-replace
//! prompt lives alongside the rest of its [`code_compaction`](crate::code_compaction)
//! subsystem. The Kimix chat's step-level intra prompt
//! ([`format_compaction_prompt`](crate::prompt::format_compaction_prompt))
//! stays at the crate root.

/// Build Kimix's session-level summarization prompt (no chat history).
///
...
/// `Kimix-shell::session::helpers::session_compact::build_compaction_prompt`
...
/// (mirrors `Kimix-shell`'s `SELF_SUMMARIZATION_PROMPT`). Framed
...
/// so the structured (Kimix) and short self-summary prompts stay
/// shared instead of duplicated per harness.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SummaryPromptKind {
    /// Kimix's detailed, numbered-section summary prompt.
    #[default]
    Structured,
...
mod tests {
    ...
    fn kind_structured_matches_build_summary_prompt() {
        ...
        // so routing through the selector never changes Kimix's prompt.
        assert_eq!(
