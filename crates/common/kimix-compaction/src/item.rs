...
//! harness implements [`CompactionItem`] for its `KimixTurn`;
//! Kimix implements it for `kimix_sampling_types::ConversationItem`.
//!
...
//! `<kimix_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `KimixRole` (Kimix chat) and the
/// `ConversationItem` variants (Kimix).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Kimix chat) — maps to System on harnesses without a
...
/// extraction (`<kimix_file id=".." name=".." />` lines in the
/// `<kimix_user_queries>` preamble).
...
///
/// Implementors:
/// - Kimix chat: `KimixTurn`
/// - Kimix: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Kimix chat's
    /// `KimixTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Kimix chat: a
    ...
    /// strips their `<kimix_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<kimix_file>` lines in the
    /// `<kimix_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Kimix chat: a `Developer` turn with category
    ...
    /// requests/results (Kimix chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up Kimix's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Kimix chat's
/// `KimixTurn`; adding these constructors to it as required methods would break
/// that impl. They are also Kimix-specific (Kimix chat's tail-keep path
...
///
/// The Kimix implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Kimix chat stores turns as `Arc<KimixTurn>`).
