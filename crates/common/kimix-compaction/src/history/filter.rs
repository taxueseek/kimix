...
//!
//! Behavior is byte-for-byte identical for Kimix chat (`T = Arc<KimixTurn>`).
use tracing::info;
...
///   summaries so their `<kimix_user_queries>` sections can be split out.
...
/// Extracts **all** `<kimix_user_queries>...</kimix_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<kimix_user_queries>";
    let end_tag = "</kimix_user_queries>";
...
/// Extract a `<kimix_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<kimix_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<kimix_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<kimix_user_queries>\n");
    ...
    result.push_str("</kimix_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<kimix_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<kimix_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<kimix_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<kimix_user_queries>` block.
...
/// `<kimix_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<kimix_user_queries>` from earlier rounds.
...
///   would see the prior `<kimix_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<kimix_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<kimix_user_queries>"));
        assert!(out.ends_with("</kimix_user_queries>"));
        assert!(out.contains("<kimix_query>hello world</kimix_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<kimix_user_queries>\n<kimix_query>first</kimix_query>\n</kimix_user_queries>";
        let inner2 =
            "<kimix_user_queries>\n<kimix_query>second</kimix_query>\n</kimix_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<kimix_user_queries>\n<kimix_query>Q1</kimix_query>\n</kimix_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<kimix_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<kimix_user_queries>"));
