...
//! stay in each product host (for example `Kimix-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `Kimix-sampling-types`. It is decoupled from both Kimix chat and
//! Kimix hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — Kimix's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Kimix chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Kimix chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Kimix chat and
//! Kimix; hosts still own snapshotting and host-only sections).
...
// Kimix's full-replace subsystem now lives under `code_compaction`;
