...
//! extraction, chunk summarisation, and the final `<kimix_user_queries>`
//! + `<chunk_summary>` assembly — is shared. The harness supplies the
//! candidate items, the *current* user-queries preamble (Kimix chat
...
pub struct ChunkedCompactionOutput {
    /// `<kimix_user_queries>` preamble + `<chunk_summary index="i">` blocks.
...
/// 2. [`separate_prior_user_queries`] — split prior `<kimix_user_queries>`
...
/// round's user messages (Kimix chat: verbatim from the raw request, with
/// attachment refs). `conversation_id` / `response_id` are threaded
/// through for log correlation only.
///
/// Observer events (the Kimix chat observer maps them to the
...
pub async fn sample_compaction_chunked<T: CompactionItemBuilder + Send + Sync>(
    ...
    // Step 2 — split prior `<kimix_user_queries>` out of every prior
