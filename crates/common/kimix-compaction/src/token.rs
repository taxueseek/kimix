...
//!
//! - Kimix chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `KimixTurn::get_num_tokens`.
//! - Kimix estimates with `bytes / 4`.
//!
...
//! (Kimix chat: `ItemTokenCounter<Arc<KimixTurn>>`), so exactly one mechanism
//! is in play.
