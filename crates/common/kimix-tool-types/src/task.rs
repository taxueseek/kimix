...
pub struct TaskToolInput {
    ...
    // serde is lenient (missing field deserializes to "" and is rejected at
    // execution time), but the model-facing schema keeps the field required.
    #[schemars(required)]
    #[serde(default)]
    pub prompt: String,
...
/// A built-in subagent type shared by the CLI (`Kimix-agent`) and other
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "kimix-3"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("kimix-3"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("kimix-3".into())).as_deref(),
            Some("kimix-3")
        );
        assert_eq!(
            sanitize_optional_arg(Some("  kimix-3  ".into())).as_deref(),
    ...
    // match what the Kimix-shell MiniJinja templates render for the default
    // Kimix toolset (monitor + task + bash + read present, POSIX). The
