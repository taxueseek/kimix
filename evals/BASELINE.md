...
| 2026-08-03（v0.1.17） | 16 | 100% | 0 | 51.2s | 扩库至 16 题，含 repo 模式 |

**缺口进展**：用例从 5 题扩到 16 题（大文件局部编辑、跨文件引用、长输出
判断、多步计划、测试循环、只读问答、repo 真实仓库）。16 题全 PASS、
edit_fail=0，但平均耗时从 5.5s 升到 51.2s（真实模型调用），仍是缺口项。
...
| 指标 | 当前基线 | 目标 | 测量方式 |
|------|:--------:|:----:|----------|
| evals 通过率 | 100%（16/16） | ≥ 90% | `python3 evals/runner.py --report evals/last.json` |
| edit_failures 次数 | 0 | ≤ 2 | runner 自动统计 |
| 用例库规模 | 16 | 20–50 | 持续扩充 cases/ |
...
- evals 用例 16 题（2026-08-03 扩库后），距 20–50 题目标仍差一半，
  优先补：git 冲突解决、依赖升级、配置漂移类真实仓库场景。
- 验证器回流（runner 首轮失败后把断言反馈喂回模型再修一轮）已内置，
  但尚未验证其在「故意失败」用例上的回退行为，也无独立 Verifier trait。
