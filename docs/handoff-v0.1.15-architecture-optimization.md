...
- **规模**：约 117.5 万行仓库代码（含 grok-build hard fork 及后续改进/移植，**非**全量从零自研）+ 2.6 万行 third_party，64 个 crate
...
**第一性原理**：每一行代码都是维护成本。约 117.5 万行（含上游改造与移植）中，有多少是「本仓库必须继续维护、无法删或替」的？
